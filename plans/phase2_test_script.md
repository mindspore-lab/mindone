...
- Inputs collected
  - input_folder: `mindone/transformers/models/upernet`

- Immediate checks
  - Verify `mindone/transformers/models/upernet` exists (warn user if it is empty)
  - Verify `tests/transformers_tests/models/upernet/` exists (create it if it is empty)

- Scope
  - Read all Python files under `mindone/transformers/models/upernet`
  - Read all arguments of `mindone/transformers/models/upernet/configuration_upernet.py`
...
```
  - Reduce the argument values (e.g., num_hidden_layers, vocab_size, batch_size, seq_len) in `configuration_upernet.py` to initiate a reduced-size model for testing
  - Write the test script to `tests/transformers_tests/models/upernet/test_modeling_upernet.py`
