...
- Inputs collected
  - input_folder: `mindone/transformers/models/{falcon_h1}`

- Immediate checks
  - Verify `mindone/transformers/models/{falcon_h1}` exists (warn user if it is empty)
  - Verify `tests/transformers_tests/models/{falcon_h1}/` exists (create it if it is empty)

- Scope
  - Read all Python files under `mindone/transformers/models/{falcon_h1}`
  - Read all arguments of `mindone/transformers/models/{falcon_h1}/configuration_{falcon_h1}.py`
...
```
  - Reduce the argument values (e.g., num_hidden_layers, vocab_size, batch_size, seq_len) in `configuration_{falcon_h1}.py` to initiate a reduced-size model for testing
  - Write the test script to `tests/transformers_tests/models/{falcon_h1}/test_modeling_{falcon_h1}.py`
