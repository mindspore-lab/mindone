...
from mindone.metrics.functional.image.inception_score import inception_score as inception_score_func
...
def test_is_func_input():
    ...
    assert mindspore.ops.isclose(is_tensor_input, is_list_input, rtol=1e-5)

    fid_with_batch, _ = inception_score_func(images_tensor, batch_size=3)
    assert mindspore.ops.isclose(is_tensor_input, fid_with_batch, rtol=1e-5)
...
def test_compare_is(normalize):
    ...
    assert np.allclose(torch_mean.numpy(), mindone_mean.asnumpy(), rtol=1e-5)
    assert np.allclose(torch_mean.numpy(), mindone_func_mean.asnumpy(), rtol=1e-5)
