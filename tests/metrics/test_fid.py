...
from mindone.metrics.functional.fid import fid as fid_func
...
def test_fid_func_input():
    ...
    assert mindspore.ops.isclose(fid_tensor_input, fid_list_input, rtol=1e-3)

    fid_with_batch = fid_func(real_images_tensor, fake_images_tensor, batch_size=3)
    assert mindspore.ops.isclose(fid_tensor_input, fid_with_batch, rtol=1e-3)
...
def test_fid_same_input(feature):
    ...
    val = metric.compute()
    assert mindspore.ops.isclose(val, mindspore.ops.zeros_like(val), atol=1e-3)
...
def test_compare_fid(feature_num, normalize):
    ...
    print(np.allclose(torch_fid_score.numpy(), mindone_fid_score.asnumpy(), rtol=1e-3))
    print(np.allclose(torch_fid_score.numpy(), mindone_func_fid_score.asnumpy(), rtol=1e-3))
