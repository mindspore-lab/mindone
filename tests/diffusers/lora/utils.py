...
from mindone.diffusers.utils.testing_utils import floats_tensor, require_peft_version_greater
...
def check_if_lora_correctly_set(model: nn.Cell) -> bool:
    """
...
class PeftLoraLoaderMixinTests:
    """
    Base test class for LoRA-related tests.

    This class contains multiple tests related to LoRA and compares them with the original test cases in
    diffusers. The following changes have been made compared to the original diffusers tests:

    1. To maintain consistency, we have opted to build unit tests based on pytest rather than unittest.
    2. CPU offload-related test cases have been removed because we do not support explicit offloading.
    3. We only support checkpoint files in the safetensors format, so we default to using
       `safe_serialization=True` when saving and loading weight files.
    4. Parameters related to `use_dora` have been commented out because our built-in peft version does
       not support this feature.
    5. In NaN-related tests, the `output_type` parameter has been adjusted from "np" to "latent".
       The former's post-processing `clamp` operator would unexpectedly clamp NaN values to a number in
       specified range, causing the test cases to fail.
    """
    ...
    text_encoder_cls, text_encoder_id, text_encoder_revision = None, None, None
    text_encoder_2_cls, text_encoder_2_id, text_encoder_2_revision = None, None, None
    text_encoder_3_cls, text_encoder_3_id, text_encoder_3_revision = None, None, None
    ...
    def get_dummy_components(self, scheduler_cls=None, use_dora=False):
        ...
        text_encoder = self.text_encoder_cls.from_pretrained(self.text_encoder_id, revision=self.text_encoder_revision)
        ...
        if self.text_encoder_2_cls is not None:
            text_encoder_2 = self.text_encoder_2_cls.from_pretrained(
                self.text_encoder_2_id, revision=self.text_encoder_2_revision
            )
        ...
        if self.text_encoder_3_cls is not None:
            text_encoder_3 = self.text_encoder_3_cls.from_pretrained(
                self.text_encoder_3_id, revision=self.text_encoder_3_revision
            )
    ...
    def get_dummy_inputs(self, with_generator=True):
        ...
        ms.set_seed(0)
        input_ids = ops.randint(1, sequence_length, size=(batch_size, sequence_length))
    ...
    def get_dummy_tokens(self):
        ...
        ms.set_seed(0)
        inputs = ops.randint(2, 56, size=(1, max_seq_length))
    ...
    def test_simple_inference_with_text_lora(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            assert not np.allclose(output_lora, output_no_lora, atol=1e-3, rtol=1e-3), "Lora should change the output"
        ...
        for scheduler_cls in scheduler_classes:
            ...
            if self.unet_kwargs is not None:
                output_lora_scale = pipe(
                    **inputs, generator=np.random.default_rng(0), cross_attention_kwargs={"scale": 0.5}
                )[0]
            else:
                output_lora_scale = pipe(
                    **inputs, generator=np.random.default_rng(0), joint_attention_kwargs={"scale": 0.5}
                )[0]
            assert not np.allclose(
                output_lora, output_lora_scale, atol=1e-3, rtol=1e-3
            ), "Lora + scale should change the output"

            if self.unet_kwargs is not None:
                output_lora_0_scale = pipe(
                    **inputs, generator=np.random.default_rng(0), cross_attention_kwargs={"scale": 0.0}
                )[0]
            else:
                output_lora_0_scale = pipe(
                    **inputs, generator=np.random.default_rng(0), joint_attention_kwargs={"scale": 0.0}
                )[0]
            assert np.allclose(
                output_no_lora, output_lora_0_scale, atol=1e-3, rtol=1e-3
    ...
    def test_simple_inference_with_text_lora_fused(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            assert not np.allclose(
                ouput_fused, output_no_lora, atol=1e-3, rtol=1e-3
    ...
    def test_simple_inference_with_text_lora_save_load(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            assert np.allclose(
                images_lora, images_lora_from_pretrained, atol=1e-3, rtol=1e-3
    ...
    def test_simple_inference_with_partial_text_lora(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            assert not np.allclose(
                output_partial_lora, output_lora, atol=1e-3, rtol=1e-3
    ...
    def test_simple_inference_save_pretrained(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            assert check_if_lora_correctly_set(
                pipe_from_pretrained.text_encoder
            ...
            assert np.allclose(
                images_lora, images_lora_save_pretrained, atol=1e-3, rtol=1e-3
            ), "Loading from saved checkpoints should give same results."
    ...
    def test_simple_inference_with_text_denoiser_lora_save_load(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            assert check_if_lora_correctly_set(denoiser_to_checked), "Lora not correctly set in Unet"
            ...
            assert check_if_lora_correctly_set(pipe.text_encoder), "Lora not correctly set in text encoder"
            denoiser_to_checked = pipe.unet if self.unet_kwargs is not None else pipe.transformer
            assert check_if_lora_correctly_set(denoiser_to_checked), "Lora not correctly set in denoiser"
    ...
    def test_simple_inference_with_text_denoiser_lora_unloaded(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            assert not check_if_lora_correctly_set(pipe.text_encoder), "Lora not correctly unloaded in text encoder"
            denoiser_to_checked = pipe.unet if self.unet_kwargs is not None else pipe.transformer
            assert not check_if_lora_correctly_set(denoiser_to_checked), "Lora not correctly unloaded in denoiser"
    ...
    def test_simple_inference_with_text_denoiser_lora_unfused(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            denoiser_to_checked = pipe.unet if self.unet_kwargs is not None else pipe.transformer
            assert check_if_lora_correctly_set(denoiser_to_checked), "Unfuse should still keep LoRA layers"
            ...
            assert np.allclose(
                output_fused_lora, output_unfused_lora, atol=1e-3, rtol=1e-3
            ), "Fused lora should change the output"
    ...
    def test_simple_inference_with_text_denoiser_multi_adapter(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            # Fuse and unfuse should lead to the same results
            assert not np.allclose(
                output_adapter_1, output_adapter_2, atol=1e-3, rtol=1e-3
            ), "Adapter 1 and 2 should give different results"

            assert not np.allclose(
                output_adapter_1, output_adapter_mixed, atol=1e-3, rtol=1e-3
            ), "Adapter 1 and mixed adapters should give different results"

            assert not np.allclose(
                output_adapter_2, output_adapter_mixed, atol=1e-3, rtol=1e-3
            ...
            assert np.allclose(
                output_no_lora, output_disabled, atol=1e-3, rtol=1e-3
    ...
    def test_simple_inference_with_text_denoiser_block_scale(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            assert not np.allclose(
                output_weights_1, output_weights_2, atol=1e-3, rtol=1e-3
            ), "LoRA weights 1 and 2 should give different results"
            assert not np.allclose(
                output_no_lora, output_weights_1, atol=1e-3, rtol=1e-3
            ), "No adapter and LoRA weights 1 should give different results"
            assert not np.allclose(
                output_no_lora, output_weights_2, atol=1e-3, rtol=1e-3
    ...
    def test_simple_inference_with_text_denoiser_multi_adapter_delete_adapter(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            assert np.allclose(
                output_deleted_adapter_1, output_adapter_2, atol=1e-3, rtol=1e-3
            ), "Adapter 1 and 2 should give different results"
            ...
            assert np.allclose(
                output_no_lora, output_deleted_adapters, atol=1e-3, rtol=1e-3
            ), "output with no lora and output with lora disabled should give same results"
    ...
    def test_simple_inference_with_text_denoiser_multi_adapter_weighted(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            assert not np.allclose(
                output_adapter_mixed_weighted, output_adapter_mixed, atol=1e-3, rtol=1e-3
    ...
    def test_lora_fuse_nan(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            out = pipe("test", num_inference_steps=2, output_type="latent")[0]
    ...
    def test_get_list_adapters(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            assert adapter_names == dicts_to_be_checked
    ...
    def test_simple_inference_with_text_lora_denoiser_fused_multi(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            pipe.set_adapters(["adapter-1", "adapter-2"])
            ouputs_all_lora = pipe(**inputs, generator=np.random.default_rng(0))[0]

            pipe.set_adapters(["adapter-1"])
            ouputs_lora_1 = pipe(**inputs, generator=np.random.default_rng(0))[0]
            ...
            assert np.allclose(
                ouputs_lora_1, outputs_lora_1_fused, atol=1e-3, rtol=1e-3
            ...
            assert np.allclose(
                output_all_lora_fused, ouputs_all_lora, atol=1e-3, rtol=1e-3
            ), "Fused lora should not change the output"
    ...
    def test_simple_inference_with_dora(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            assert not np.allclose(
                output_dora_lora, output_no_dora_lora, atol=1e-3, rtol=1e-3
    ...
    def test_simple_inference_with_text_denoiser_lora_unfused_torch_compile(self):
        ...
        for scheduler_cls in scheduler_classes:
            ...
            pipe.unet.construct = ms.jit(pipe.unet.construct)
            pipe.text_encoder.construct = ms.jit(pipe.text_encoder.construct)

            if self.has_two_text_encoders or self.has_three_text_encoders:
                pipe.text_encoder_2.construct = ms.jit(pipe.text_encoder_2.construct)

            # Just makes sure it works..
            _ = pipe(**inputs, generator=np.random.default_rng(0))[0]

    @pytest.mark.skip("Setting pad mode in runtime is not available.")
    def test_modify_padding_mode(self):
        if self.pipeline_class.__name__ in ["StableDiffusion3Pipeline", "FluxPipeline"]:
            return

        def set_pad_mode(network: nn.Cell, mode="circular"):
