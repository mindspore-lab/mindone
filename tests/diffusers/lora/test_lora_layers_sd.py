...
class LoraIntegrationTests:
    ...
    def test_not_empty_state_dict(self):
        ...
        pipe = AutoPipelineForText2Image.from_pretrained("runwayml/stable-diffusion-v1-5", mindspore_dtype=ms.float16)
    ...
    def test_sdv1_5_lcm_lora(self):
        pipe = DiffusionPipeline.from_pretrained("runwayml/stable-diffusion-v1-5", mindspore_dtype=ms.float16)
        pipe.scheduler = LCMScheduler.from_config(pipe.scheduler.config)
    ...
    def test_sdv1_5_lcm_lora_img2img(self):
        pipe = AutoPipelineForImage2Image.from_pretrained("runwayml/stable-diffusion-v1-5", mindspore_dtype=ms.float16)
        pipe.scheduler = LCMScheduler.from_config(pipe.scheduler.config)
