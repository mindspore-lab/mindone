...
from mindone.diffusers import FlowMatchEulerDiscreteScheduler, FluxPipeline, FluxTransformer2DModel
...
class FluxLoRATests(PeftLoraLoaderMixinTests):
    ...
    def test_with_alpha_in_state_dict(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            state_dict_with_alpha = load_file(os.path.join(tmpdirname, "pytorch_lora_weights.safetensors"))
            alpha_dict = {}
            for k, v in state_dict_with_alpha.items():
                ...
                if "transformer" in k and "to_k" in k and "lora_A" in k:
                    alpha_dict[f"{k}.alpha"] = float(ops.randint(10, 100, size=()))
        ...
        assert not np.allclose(images_lora_with_alpha, images_lora, atol=1e-3, rtol=1e-3)
