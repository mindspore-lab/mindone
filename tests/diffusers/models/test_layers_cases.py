...
# ConsistencyDecoderVAE: random factor, outputs can't be same
# VQModel: volatile in fp16(fyi: 2%-20% diff when torch.fp16 vs torch.fp32)
# T2I_ADAPTER_CASES: downsample(nn.AvgPool2d(kernel_size=2, stride=2, ceil_mode=True)) precision problem in pynative
ALL_CASES = (
    NORMALIZATION_CASES
    + EMBEDDINGS_CASES
    + UPSAMPLE2D_CASES
