...
@ddt
class StableDiffusionXLImg2ImgPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    pipeline_config = [
        [
            "scheduler",
    ...
    @data(*test_cases)
    @unpack
    def test_stable_diffusion_xl_img2img_euler(self, mode, dtype):
        ms.set_context(mode=mode)

        pt_components, ms_components = self.get_dummy_components()
        pt_pipe_cls = get_module("diffusers.pipelines.stable_diffusion_xl.StableDiffusionXLImg2ImgPipeline")
        ms_pipe_cls = get_module("mindone.diffusers.pipelines.stable_diffusion_xl.StableDiffusionXLImg2ImgPipeline")
...
@slow
@ddt
class StableDiffusionXLImg2ImgPipelineIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    @data(*test_cases)
    @unpack
    def test_stable_diffusion_xl_img2img_default_case(self, mode, dtype):
        ms.set_context(mode=mode)
        ms_dtype = getattr(ms, dtype)

        pipe_cls = get_module("mindone.diffusers.pipelines.stable_diffusion_xl.StableDiffusionXLImg2ImgPipeline")
        pipe = pipe_cls.from_pretrained("stabilityai/stable-diffusion-xl-refiner-1.0", mindspore_dtype=ms_dtype)
