...
class KandinskyV22ControlnetPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    ...
    @data(*test_cases)
    @unpack
    def test_kandinsky_controlnet(self, mode, dtype):
        ms.set_context(mode=mode)

        pt_components, ms_components = self.get_dummy_components()
        pt_pipe_cls = get_module("diffusers.pipelines.kandinsky2_2.KandinskyV22ControlnetPipeline")
        ms_pipe_cls = get_module("mindone.diffusers.pipelines.kandinsky2_2.KandinskyV22ControlnetPipeline")
...
@slow
@ddt
class KandinskyV22ControlnetPipelineIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    @data(*test_cases)
    @unpack
    def test_kandinsky_controlnet(self, mode, dtype):
        ms.set_context(mode=mode)
        ...
        pipeline_cls = get_module("mindone.diffusers.pipelines.kandinsky2_2.KandinskyV22ControlnetPipeline")
