...
class StoppingCriteriaTestCase(unittest.TestCase):
    ...
    def test_confidence_criteria(self):
        ...
        scores = (mint.randn((1, vocab_size)),)
    ...
    def test_stop_string_criteria(self):
        ...
        true_input_ids = tokenizer(true_strings, return_tensors="np", padding="longest", add_special_tokens=False)
        false_input_ids = tokenizer(false_strings, return_tensors="np", padding="longest", add_special_tokens=False)
        for key in true_input_ids.keys():
            true_input_ids[key] = ms.tensor(true_input_ids[key])
        for key in false_input_ids.keys():
            false_input_ids[key] = ms.tensor(false_input_ids[key])
    ...
    def test_stop_string_criteria_vocab_size_mismatch(self):
        ...
        input_ids = ms.tensor([[len(tokenizer) + 1024, 1, 2]])
        scores = None
    ...
    def test_criterias_per_row(self):
        ...
        tokenizer = AutoTokenizer.from_pretrained("openai-community/gpt2")
        tokenizer.pad_token_id = tokenizer.eos_token_id
        inputs = tokenizer(text, return_tensors="np", add_special_tokens=False)
