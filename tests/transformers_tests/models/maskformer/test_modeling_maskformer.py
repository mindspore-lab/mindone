...
class MaskFormerModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        mask_labels = (np.random.rand(self.batch_size, self.num_labels, self.min_size, self.max_size) > 0.5).as_type(
            np.float32
        )
        class_labels = (np.random.rand(self.batch_size, self.num_labels) > 0.5).as_type(np.int64)
