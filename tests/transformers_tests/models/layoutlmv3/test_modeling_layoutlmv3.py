...
"""Testing suite for the Mindspore LayoutLMv3 model."""
import inspect
...
class LayoutLMv3ModelTester:
    ...
    def prepare_config_and_inputs(self):
        input_ids = ids_numpy([self.batch_size, self.text_seq_length], self.vocab_size)

        bbox = ids_numpy([self.batch_size, self.text_seq_length, 4], self.range_bbox)
        ...
        if self.use_token_type_ids:
            token_type_ids = ids_numpy([self.batch_size, self.text_seq_length], self.type_vocab_size)

        sequence_labels = None
        token_labels = None
        if self.use_labels:
            sequence_labels = ids_numpy([self.batch_size], self.type_sequence_label_size)
            token_labels = ids_numpy([self.batch_size, self.text_seq_length], self.num_labels)
...
model_tester = LayoutLMv3ModelTester()
(
    config,
    inputs_dict,
...
LAYOUTLMV3_CASES = [
    [
        ...
        "mindone.transformers.LayoutLMv3Model",
        (config,),
        {},
        (),
        {
            "input_ids": inputs_dict["input_ids"],
            "bbox": inputs_dict["bbox"],
