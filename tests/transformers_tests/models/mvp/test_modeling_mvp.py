...
def prepare_mvp_inputs_dict(
    ...
    if decoder_attention_mask is None:
        decoder_attention_mask = np.tril(np.ones_like(input_ids))
...
class MvpModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        sequence_labels = ids_numpy([self.batch_size], 2)
        token_labels = ids_numpy([self.batch_size, self.seq_length], 2)
        config = self.get_config()
        inputs_dict = prepare_mvp_inputs_dict(config, input_ids, decoder_input_ids)
        return config, inputs_dict, sequence_labels, token_labels
...
model_tester = MvpModelTester()
(config, inputs_dict, sequence_labels, token_labels) = model_tester.prepare_config_and_inputs()
...
BERT_CASES = [
    [
        ...
        "mindone.transformers.MvpForCausalLM",
        (config,),
        {},
    ...
    [
        ...
        "mindone.transformers.MvpForQuestionAnswering",
        (config,),
        {},
        (inputs_dict["input_ids"],),
        {
    ...
    [
        ...
        "mindone.transformers.MvpForSequenceClassification",
        (config,),
        {},
        (inputs_dict["input_ids"],),
        {
