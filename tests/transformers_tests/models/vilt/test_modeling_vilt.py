"""Adapted from https://github.com/huggingface/transformers/tree/main/tests//models/vilt/test_modeling_vilt.py."""
...
class ViltModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        if self.add_multiple_images:
            pixel_values = floats_numpy([self.batch_size, 2, self.num_channels, self.image_size, self.image_size])
        else:
...
VILT_CASES = [
    [
        ...
        "mindone.transformers.ViltModel",
        (config,),
        {},
        (),
        {
    ...
    [
        ...
        "mindone.transformers.ViltForTokenClassification",
        (config,),
        {},
        (),
        {
...
def test_named_modules(
    ...
    # monkey patch mint.multinomial
    def fake_multinomial(input, num_samples, replacement=False):
        result = torch.multinomial(torch.tensor(input.numpy()), num_samples=num_samples.item(), replacement=replacement)
        return ms.tensor(result.numpy())

    original_multinomial = mint.multinomial
    mint.multinomial = fake_multinomial
    try:
        (
            pt_model,
            ms_model,
        ...
        # restore after test
        mint.multinomial = original_multinomial
