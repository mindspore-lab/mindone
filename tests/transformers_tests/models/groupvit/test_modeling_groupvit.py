...
import pytest
import torch
from transformers import GroupViTTextConfig, GroupViTVisionConfig
...
class GroupViTVisionModelTester:
    ...
    def prepare_config_and_inputs(self):
        rng = random.Random(0)
        pixel_values = floats_numpy([self.batch_size, self.num_channels, self.image_size, self.image_size], rng=rng)
...
model_tester = GroupViTVisionModelTester()
config, pixel_values = model_tester.prepare_config_and_inputs()
GROUPVITVISION_CASES = [
    [
        "GroupViTVisionModel",
        "transformers.GroupViTVisionModel",
        "mindone.transformers.GroupViTVisionModel",
        (config,),
        {},
        (pixel_values,),
        {},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        for case in GROUPVITVISION_CASES
...
class GroupViTTextModelTester:
    ...
    def prepare_config_and_inputs(self):
        rng = random.Random(0)
        input_ids = ids_numpy([self.batch_size, self.seq_length], self.vocab_size, rng=rng)
...
model_tester = GroupViTTextModelTester()
config, input_ids, input_mask = model_tester.prepare_config_and_inputs()
GROUPVITTEXT_CASES = [
    [
        "GroupViTTextModel",
        "transformers.GroupViTTextModel",
        "mindone.transformers.GroupViTTextModel",
        (config,),
        {},
        (input_ids,),
        {
...
@pytest.mark.parametrize(
    ...
    [
        ...
        for case in GROUPVITTEXT_CASES
        for dtype in DTYPE_AND_THRESHOLDS.keys()
        for mode in MODES
    ],
)
