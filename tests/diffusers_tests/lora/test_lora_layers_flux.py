...
from mindone.diffusers import FlowMatchEulerDiscreteScheduler, FluxControlPipeline, FluxPipeline, FluxTransformer2DModel
from mindone.diffusers.utils import load_image, logging
from mindone.diffusers.utils.testing_utils import CaptureLogger, floats_tensor, numpy_cosine_similarity_distance, slow
...
class FluxLoRATests(unittest.TestCase, PeftLoraLoaderMixinTests):
    ...
    text_encoder_2_cls, text_encoder_2_id, text_encoder_2_revision = (
        T5EncoderModel,
    ...
    def test_with_alpha_in_state_dict(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            state_dict_with_alpha = ms.load_checkpoint(
                os.path.join(tmpdirname, "pytorch_lora_weights.safetensors"), format="safetensors"
            )
            alpha_dict = {}
            for k, v in state_dict_with_alpha.items():
                ...
                if "transformer" in k and "to_k" in k and "lora_A" in k:
                    alpha_dict[f"{k}.alpha"] = float(mint.randint(10, 100, ()))
        ...
        images_lora_with_alpha = pipe(**inputs, generator=np.random.default_rng(0))[0]
    ...
    def test_lora_expansion_works_for_absent_keys(self):
        ...
        self.assertTrue(check_if_lora_correctly_set(pipe.transformer), "Lora not correctly set in transformer")
        images_lora_with_absent_keys = pipe(**inputs, generator=np.random.default_rng(0))[0]
    ...
    def test_lora_expansion_works_for_extra_keys(self):
        ...
        pipe.set_adapters(["one", "two"])
        self.assertTrue(check_if_lora_correctly_set(pipe.transformer), "Lora not correctly set in transformer")
        images_lora_with_extra_keys = pipe(**inputs, generator=np.random.default_rng(0))[0]
...
class FluxControlLoRATests(unittest.TestCase, PeftLoraLoaderMixinTests):
    ...
    def test_with_norm_in_state_dict(self):
        ...
        logger = logging.get_logger("mindone.diffusers.loaders.lora_pipeline")
        ...
        for norm_layer in ["norm_q", "norm_k", "norm_added_q", "norm_added_k"]:
            norm_state_dict = {}
            for name, module in pipe.transformer.cells_and_names():
                if norm_layer not in name or not hasattr(module, "weight") or module.weight is None:
                    continue
                norm_state_dict[f"transformer.{name}.weight"] = ms.Parameter(
                    mint.randn(module.weight.shape, dtype=module.weight.dtype), name=f"transformer.{name}.weight"
                )

                with CaptureLogger(logger) as cap_logger:
                    pipe.load_lora_weights(norm_state_dict)
                lora_load_output = pipe(**inputs, generator=np.random.default_rng(0))[0]
                ...
                pipe.unload_lora_weights()
                lora_unload_output = pipe(**inputs, generator=np.random.default_rng(0))[0]
    ...
    def test_lora_parameter_expanded_shapes(self):
        ...
        dummy_lora_A = mint.nn.Linear(2 * in_features, rank, bias=False)
        dummy_lora_B = mint.nn.Linear(rank, out_features, bias=False)
        lora_state_dict = {
        ...
        lora_out = pipe(**inputs, generator=np.random.default_rng(0))[0]
        ...
        pipe = self.pipeline_class(**components)
        pipe.set_progress_bar_config(disable=None)
        dummy_lora_A = mint.nn.Linear(1, rank, bias=False)
        dummy_lora_B = mint.nn.Linear(rank, out_features, bias=False)
        lora_state_dict = {
            "transformer.x_embedder.lora_A.weight": dummy_lora_A.weight,
            "transformer.x_embedder.lora_B.weight": dummy_lora_B.weight,
    ...
    def test_normal_lora_with_expanded_lora_raises_error(self):
        ...
        shape_expander_lora_A = mint.nn.Linear(2 * in_features, rank, bias=False)
        shape_expander_lora_B = mint.nn.Linear(rank, out_features, bias=False)
        lora_state_dict = {
        ...
        _, _, inputs = self.get_dummy_inputs(with_generator=False)
        lora_output = pipe(**inputs, generator=np.random.default_rng(0))[0]

        normal_lora_A = mint.nn.Linear(in_features, rank, bias=False)
        normal_lora_B = mint.nn.Linear(rank, out_features, bias=False)
        lora_state_dict = {
        ...
        lora_output_2 = pipe(**inputs, generator=np.random.default_rng(0))[0]
    ...
    def test_fuse_expanded_lora_with_regular_lora(self):
        ...
        lora_output_3 = pipe(**inputs, generator=np.random.default_rng(0))[0]
        ...
        lora_output_4 = pipe(**inputs, generator=np.random.default_rng(0))[0]
    ...
    def test_lora_unload_with_parameter_expanded_shapes(self):
        ...
        lora_out = control_pipe(**inputs, generator=np.random.default_rng(0))[0]
        ...
        unloaded_lora_out = loaded_pipe(**inputs, generator=np.random.default_rng(0))[0]
    ...
    def test_lora_unload_with_parameter_expanded_shapes_and_no_reset(self):
        ...
        no_lora_out = control_pipe(**inputs, generator=np.random.default_rng(0))[0]
...
@slow
class FluxLoRAIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    num_inference_steps = 10
    seed = 0

    def setUp(self):
        super().setUp()
        self.pipeline = FluxPipeline.from_pretrained("black-forest-labs/FLUX.1-dev", mindspore_dtype=ms.bfloat16)
...
@slow
class FluxControlLoRAIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    num_inference_steps = 10
    seed = 0
    ...
    def setUp(self):
        ...
        self.pipeline = FluxControlPipeline.from_pretrained("black-forest-labs/FLUX.1-dev", mindspore_dtype=ms.bfloat16)

    def test_lora_canny(self):
        lora_ckpt_id = "black-forest-labs/FLUX.1-Canny-dev-lora"
    ...
    def test_lora_depth(self):
        lora_ckpt_id = "black-forest-labs/FLUX.1-Depth-dev-lora"
        self.pipeline.load_lora_weights(lora_ckpt_id)
        self.pipeline.fuse_lora()
        self.pipeline.unload_lora_weights()
    ...
    def test_lora_with_turbo_canny(self):
        lora_ckpt_id = "black-forest-labs/FLUX.1-Canny-dev-lora"
        self.pipeline.load_lora_weights(lora_ckpt_id)
    ...
    def test_lora_with_turbo_depth(self):
        lora_ckpt_id = "black-forest-labs/FLUX.1-Depth-dev-lora"
        self.pipeline.load_lora_weights(lora_ckpt_id)
        self.pipeline.load_lora_weights("ByteDance/Hyper-SD", weight_name="Hyper-FLUX.1-dev-8steps-lora.safetensors")
        self.pipeline.fuse_lora()
