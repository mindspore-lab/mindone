...
ms.set_device("Ascend", 4)


@ddt
class QwenImageControlNetPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    pipeline_config = [
        [
            "transformer",
        ...
        [
            "controlnet",
            "diffusers.models.controlnets.controlnet_qwenimage.QwenImageControlNetModel",
            "mindone.diffusers.models.controlnets.controlnet_qwenimage.QwenImageControlNetModel",
            dict(
                patch_size=2,
                in_channels=16,
                out_channels=4,
    ...
    def test_inference(self, mode, dtype):
        ...
        pt_pipe_cls = get_module("diffusers.pipelines.qwenimage.QwenImageControlNetPipeline")
        ms_pipe_cls = get_module("mindone.diffusers.pipelines.qwenimage.QwenImageControlNetPipeline")
...
@slow
@ddt
class QwenImageControlNetPipelineIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    def get_inputs(self):
        ...
        inputs = {
            "prompt": (
                "Aesthetics art, traditional asian pagoda, elaborate golden accents, sky blue and white color palette, "
                "swirling cloud pattern, digital illustration, east asian architecture, ornamental rooftop, intricate "
                "detailing on building, cultural representation."
            ),
    ...
    def test_inference(self, mode, dtype):
        ...
        if dtype == "bfloat16":
            pytest.skip("Skipping this case since not supported in a single NPU")
        ...
        pipe = QwenImageControlNetPipeline.from_pretrained(
            "Qwen/Qwen-Image", controlnet=controlnet, mindspore_dtype=ms_dtype
        )
        ...
        expected_image = load_numpy_from_local_file(
            ...
            f"qwenimage_controlnet_{dtype}.npy",
            subfolder="qwenimage",
        )
