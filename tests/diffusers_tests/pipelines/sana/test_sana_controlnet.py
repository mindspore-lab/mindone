...
class SanaControlNetPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    pipeline_config = [
        [
            "controlnet",
            "diffusers.models.controlnets.controlnet_sana.SanaControlNetModel",
            "mindone.diffusers.models.controlnets.controlnet_sana.SanaControlNetModel",
            dict(
                patch_size=1,
                in_channels=4,
                out_channels=4,
    ...
    def test_inference(self, mode, dtype):
        ...
        pt_pipe_cls = get_module("diffusers.pipelines.sana.pipeline_sana_controlnet.SanaControlNetPipeline")
        ms_pipe_cls = get_module("mindone.diffusers.pipelines.sana.pipeline_sana_controlnet.SanaControlNetPipeline")
...
@slow
@ddt
class SanaControlNetPipelineIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    @data(*test_cases)
    @unpack
    def test_inference(self, mode, dtype):
        ms.set_context(mode=mode)
