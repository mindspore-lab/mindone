...
class SanaPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    ...
    def test_inference(self, mode, dtype):
        ...
        pt_pipe_cls = get_module("diffusers.pipelines.sana.pipeline_sana.SanaPipeline")
        ms_pipe_cls = get_module("mindone.diffusers.pipelines.sana.pipeline_sana.SanaPipeline")
...
@slow
@ddt
class SanaPipelineIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    prompt = 'a cyberpunk cat with a neon sign that says "Sana"'

    @data(*test_cases)
    @unpack
    def test_sana(self, mode, dtype):
        ms.set_context(mode=mode)
        ms_dtype = getattr(ms, dtype)

        pipe = SanaPipeline.from_pretrained(
            "Efficient-Large-Model/Sana_1600M_1024px_BF16_diffusers", mindspore_dtype=ms_dtype
        )
