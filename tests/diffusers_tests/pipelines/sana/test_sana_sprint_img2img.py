...
class SanaSprintImg2ImgPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    ...
    def test_inference(self, mode, dtype):
        ...
        pt_pipe_cls = get_module("diffusers.pipelines.sana.pipeline_sana_sprint_img2img.SanaSprintImg2ImgPipeline")
        ms_pipe_cls = get_module(
            "mindone.diffusers.pipelines.sana.pipeline_sana_sprint_img2img.SanaSprintImg2ImgPipeline"
        )
...
@slow
@ddt
class SanaSprintImg2ImgPipelineIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    @data(*test_cases)
    @unpack
    def test_inference(self, mode, dtype):
        if dtype == "float16":
            pytest.skip("this pipeline has precision issue in float16")
