...
from mindone.diffusers.utils.testing_utils import load_numpy_from_local_file, load_video_from_local_file, slow
...
@slow
@ddt
class SkyReelsV2DiffusionForcingVideoToVideoIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    @data(*test_cases)
    @unpack
    def test_inference(self, mode, dtype):
        if dtype == "float32":
        ...
        video = load_video_from_local_file(
        ...
        expected_image = load_numpy_from_local_file(
            "mindone-testing-arrays",
            f"skyreels_v2_df_v2v_{dtype}.npy",
            subfolder="skyreels_v2",
        )
