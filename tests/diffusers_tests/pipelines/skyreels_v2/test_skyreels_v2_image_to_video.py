...
from mindone.diffusers import AutoencoderKLWan, SkyReelsV2ImageToVideoPipeline, UniPCMultistepScheduler
...
class SkyReelsV2ImageToVideoPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    ...
    def test_inference(self, mode, dtype):
        ...
        pt_pipe_cls = get_module(
            "diffusers.pipelines.skyreels_v2.pipeline_skyreels_v2_i2v.SkyReelsV2ImageToVideoPipeline"
        )
        ms_pipe_cls = get_module(
            "mindone.diffusers.pipelines.skyreels_v2.pipeline_skyreels_v2_i2v.SkyReelsV2ImageToVideoPipeline"
        )
...
@slow
@ddt
class SkyReelsV2ImageToVideoPipelineIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    @data(*test_cases)
    @unpack
    def test_inference(self, mode, dtype):
        if dtype == "float32":
