...
class SkyReelsV2DiffusionForcingImageToVideoPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    ...
    def test_inference(self, mode, dtype):
        if dtype == "float16" or dtype == "bfloat16":
            pytest.skip("fp16 and bf16 will raise error in torch")

        ms.set_context(mode=mode)

        pt_components, ms_components = self.get_dummy_components()
        pt_pipe_cls = get_module(
            "diffusers.pipelines.skyreels_v2.pipeline_skyreels_v2_diffusion_forcing_i2v.SkyReelsV2DiffusionForcingImageToVideoPipeline"
        )
        ms_pipe_cls = get_module(
            "mindone.diffusers.pipelines.skyreels_v2.pipeline_skyreels_v2_diffusion_forcing_i2v.SkyReelsV2DiffusionForcingImageToVideoPipeline"
        )
...
@slow
@ddt
class SkyReelsV2DiffusionForcingImageToVideoPipelineIntegrationTests(PipelineTesterMixin, unittest.TestCase):
    @data(*test_cases)
    @unpack
    def test_inference(self, mode, dtype):
        if dtype == "float32":
        ...
        expected_image = load_numpy_from_local_file(
            "mindone-testing-arrays",
            f"skyreels_v2_df_i2v_{dtype}.npy",
            subfolder="skyreels_v2",
        )
