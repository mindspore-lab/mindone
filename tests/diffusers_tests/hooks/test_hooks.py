...
from mindone.diffusers.hooks import HookRegistry, ModelHook
from mindone.diffusers.utils.logging import get_logger
from mindone.diffusers.utils.testing_utils import CaptureLogger

logger = get_logger(__name__)  # pylint: disable=invalid-name


class DummyBlock(ms.nn.Cell):
    def __init__(self, in_features: int, hidden_features: int, out_features: int) -> None:
        super().__init__()

        self.proj_in = mint.nn.Linear(in_features, hidden_features)
        self.activation = mint.nn.ReLU()
        self.proj_out = mint.nn.Linear(hidden_features, out_features)
...
class DummyModel(ms.nn.Cell):
    def __init__(self, in_features: int, hidden_features: int, out_features: int, num_layers: int) -> None:
        super().__init__()

        self.linear_1 = mint.nn.Linear(in_features, hidden_features)
        self.activation = mint.nn.ReLU()
        self.blocks = ms.nn.CellList(
            [DummyBlock(hidden_features, hidden_features, hidden_features) for _ in range(num_layers)]
        )
        self.linear_2 = mint.nn.Linear(hidden_features, out_features)
...
class AddHook(ModelHook):
    ...
    def pre_construct(self, module: ms.nn.Cell, *args, **kwargs):
        logger.debug("AddHook pre_construct")
        args = ((x + self.value) if ops.is_tensor(x) else x for x in args)
        return args, kwargs

    def post_construct(self, module, output):
        logger.debug("AddHook post_construct")
        return output
...
class MultiplyHook(ModelHook):
    ...
    def pre_construct(self, module, *args, **kwargs):
        logger.debug("MultiplyHook pre_construct")
        args = ((x * self.value) if ops.is_tensor(x) else x for x in args)
        return args, kwargs

    def post_construct(self, module, output):
        logger.debug("MultiplyHook post_construct")
        return output
...
class StatefulAddHook(ModelHook):
    ...
    def pre_construct(self, module, *args, **kwargs):
        logger.debug("StatefulAddHook pre_construct")
        add_value = self.value + self.increment
        self.increment += 1
        args = ((x + add_value) if ops.is_tensor(x) else x for x in args)
        return args, kwargs
...
class SkipLayerHook(ModelHook):
    ...
    def pre_construct(self, module, *args, **kwargs):
        logger.debug("SkipLayerHook pre_construct")
        return args, kwargs

    def new_construct(self, module, *args, **kwargs):
        logger.debug("SkipLayerHook new_construct")
        if self.skip_layer:
            return args[0]
        return self.fn_ref.original_construct(*args, **kwargs)

    def post_construct(self, module, output):
        logger.debug("SkipLayerHook post_construct")
        return output
...
class HookTests(unittest.TestCase):
    ...
    def test_stateful_hook(self):
        ...
        input = mint.randn(1, 4, generator=self.get_generator())
        num_repeats = 3
        ...
        self.assertTrue(mint.allclose(output1, output2))
    ...
    def test_skip_layer_internal_block(self):
        ...
        with self.assertRaises(ValueError) as cm:
            self.model(input).mean().item()
        self.assertIn(
            "the elements of the input 'mat1' should be same as the elements of the input 'mat2'", str(cm.exception)
        )
    ...
    def test_invocation_order_stateful_first(self):
        ...
        expected_invocation_order_log = (
            (
                "MultiplyHook pre_construct\n"
                "AddHook pre_construct\n"
                "StatefulAddHook pre_construct\n"
                "AddHook post_construct\n"
                "MultiplyHook post_construct\n"
            )
            .replace(" ", "")
            .replace("\n", "")
        )
