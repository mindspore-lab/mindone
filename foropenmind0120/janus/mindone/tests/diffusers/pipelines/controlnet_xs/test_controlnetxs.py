...
@ddt
class ControlNetXSPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    pipeline_config = [
        [
            "controlnet",
    ...
    def test_controlnet_lcm(self, mode, dtype):
        ...
        pt_components, ms_components = self.get_dummy_components(time_cond_proj_dim=8)
        pt_pipe_cls = get_module("diffusers.pipelines.controlnet_xs.StableDiffusionControlNetXSPipeline")
        ms_pipe_cls = get_module("mindone.diffusers.pipelines.controlnet_xs.StableDiffusionControlNetXSPipeline")
...
@slow
@ddt
class ControlNetXSPipelineSlowTests(PipelineTesterMixin, unittest.TestCase):
    @data(*test_cases)
    @unpack
    def test_canny(self, mode, dtype):
        ...
        pipe = pipe_cls.from_pretrained(
            "stabilityai/stable-diffusion-2-1-base", controlnet=controlnet, mindspore_dtype=ms_dtype
        )
