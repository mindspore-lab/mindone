...
def compute_img_diff(img1, img2, matches1, matches1_from_2, vis=False):
    ...
    # Gray Diff
    img1_gray_small = (
        cv2.resize(img1_gray, (int(width * scale), int(height * scale)), interpolation=cv2.INTER_LINEAR) / 255.0
    )
    img2_gray_small = (
        cv2.resize(img2_gray, (int(width * scale), int(height * scale)), interpolation=cv2.INTER_LINEAR) / 255.0
    )
