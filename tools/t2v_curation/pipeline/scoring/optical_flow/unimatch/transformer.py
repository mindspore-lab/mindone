...
class TransformerLayer(nn.Cell):
    def __init__(
        ...
        if not self.no_ffn:
            ...
            self.mlp = nn.SequentialCell(
                nn.Dense(in_channels, in_channels * ffn_dim_expansion, has_bias=False),
                nn.GELU(),
                nn.Dense(in_channels * ffn_dim_expansion, d_model, has_bias=False),
            )
    ...
    def construct(
        ...
        if not self.no_ffn:
            message = self.mlp(ops.cat([source, message], axis=-1))
...
class FeatureTransformer(nn.Cell):
    ...
    def construct(
        ...
        **kwargs,
        ...
        feature0 = feature0.flatten(start_dim=-2).permute(0, 2, 1)  # [B, H*W, C]
        feature1 = feature1.flatten(start_dim=-2).permute(0, 2, 1)  # [B, H*W, C]
        ...
        concat0 = ops.cat((feature0, feature1), axis=0)  # [2B, H*W, C]
        concat1 = ops.cat((feature1, feature0), axis=0)  # [2B, H*W, C]
        ...
        for i, layer in enumerate(self.layers):
            ...
            # update feature1
            concat1 = ops.cat(concat0.chunk(chunks=2, axis=0)[::-1], axis=0)

        feature0, feature1 = concat0.chunk(chunks=2, axis=0)  # [B, H*W, C]

        # reshape back
        feature0 = feature0.reshape(b, h, w, c).permute(0, 3, 1, 2).contiguous()  # [B, C, H, W]
        feature1 = feature1.reshape(b, h, w, c).permute(0, 3, 1, 2).contiguous()  # [B, C, H, W]
