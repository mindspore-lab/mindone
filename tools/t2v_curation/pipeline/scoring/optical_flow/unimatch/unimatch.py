...
class UniMatch(nn.Cell):
    def __init__(
        ...
        if not self.reg_refine or task == "depth":
            ...
            self.upsampler = nn.SequentialCell(
                nn.Conv2d(
                ...
                nn.Conv2d(
                    in_channels=256,
                    out_channels=upsample_factor ** 2 * 9,
                    kernel_size=1,
                    stride=1,
                    padding=0,
                    pad_mode='pad',  # 对应PyTorch的padding_mode='zeros'
                    has_bias=True  # PyTorch默认bias=True
                ),
            )
    ...
    def extract_feature(self, img0, img1):
        concat = ops.cat((img0, img1), axis=0)  # [2B, C, H, W]
        ...
        for i in range(len(features)):
            feature = features[i]
            chunks = ops.chunk(feature, 2, 0)  # tuple
    ...
    def upsample_flow(self, flow, feature, bilinear=False, upsample_factor=8, is_depth=False):
        if bilinear:
            ...
            up_flow = (
                ops.interpolate(flow, scale_factor=float(upsample_factor),
                                mode="bilinear", align_corners=True, recompute_scale_factor=True) * multiplier
            )
        else:
            concat = ops.cat((flow, feature), axis=1)
        ...
        **kwargs,
        ...
        for scale_idx in range(self.num_scales):
            ...
            if task == "depth":
                ...
                depth_candidates = ops.linspace(min_depth, max_depth, num_depth_candidates).astype(feature0.dtype)
                depth_candidates = depth_candidates.reshape(1, num_depth_candidates, 1, 1).tile((
                    b, 1, h, w
                ))  # [B, D, H, W]
            ...
            if (pred_bidir_flow or pred_bidir_depth) and scale_idx == 0:
                feature0 = ops.cat((feature0, feature1), axis=0)  # [2*B, C, H, W] for propagation
            ...
            if scale_idx == self.num_scales - 1:
                if not self.reg_refine:
                    ...
                    if task == "stereo":
                        flow_pad = ops.cat((-flow, ops.zeros_like(flow)), axis=1)  # [B, 2, H, W]
                        flow_up_pad = self.upsample_flow(flow_pad, feature0)
                        flow_up = -flow_up_pad[:, :1]  # [B, 1, H, W]
                    elif task == "depth":
                        depth_pad = ops.cat((flow, ops.zeros_like(flow)), axis=1)  # [B, 2, H, W]
                    ...
                    for refine_iter_idx in range(num_reg_refine):
                        ...
                        if task == "stereo":
                            ...
                            if pred_bidir_depth and refine_iter_idx == 0:
                                intrinsics_curr = intrinsics_curr.tile((2, 1, 1))
                                pose = ops.cat((pose, ops.inverse(pose)), axis=0)

                                feature0_ori, feature1_ori = ops.cat((feature0_ori, feature1_ori), axis=0), ops.cat(
                                    (feature1_ori, feature0_ori), axis=0
                                )
                        ...
                        net, inp = ops.chunk(proj, chunks=2, axis=1)
