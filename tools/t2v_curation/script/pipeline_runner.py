...
def main():
    ...
    if pipeline_steps['run']:
        ...
        if pipeline_steps['deduplication']['run']:
            ...
            run_command(f'python -m pipeline.datasets.deduplication {input_meta_csv} '
                        f'--hash {hash} --threshold {threshold}')
        ...
        if captioning['run']:
            ...
            if captioning['clean_caption']['run']:
                ...
                run_command(f'python -m pipeline.datasets.datautil {input_meta_csv} {clean_options} --output {output_meta_csv}')
                input_meta_csv = output_meta_csv
