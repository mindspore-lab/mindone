...
parser.add_argument(
    ...
    "-m",
    type=str,
    choices=[STABLE_DIFFUSION_V1, STABLE_DIFFUSION_V2, CONTROLNET_V1, DIFFUSERS_V2],
...
def PYTORCH_MINDSPORE_CONTROLNET_V1():
    with open("tools/model_conversion/ms_exceptcond_v1.txt") as file_ms:
        lines_ms = file_ms.readlines()
    with open("tools/model_conversion/pt_exceptcond_v1.txt") as file_pt:
        lines_pt = file_pt.readlines()

    # no need for ["state_dict"] due to using model.state_dict() in export script
    source_data = torch.load(args.source, map_location="cpu")
    target_data = []
    i = j = 0
...
SUPPORTED_CONVERSIONS = {
    ...
    (PYTORCH, MINDSPORE, CONTROLNET_V1, PYTORCH_MINDSPORE_CONTROLNET_V1),
}
