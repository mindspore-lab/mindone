...
If you want to use your own dataset, please make sure the structure aligh with the datasets:
```text
dir
    ├── img_txt.csv
    ├── control
...
`control` and `original` is the filefolder with all imgs. The difference is the images under `original` folder are original pokemon images from official release, and the images under `control` are the canny image generated from original images. (eg. `control/img0.png` is the canny image of `original/img0.png` )
