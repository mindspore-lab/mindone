...
Try to customize your own controlnet? Downloading the SD base weight, parparing your dataset and start training!
...
### Prepare dataset
You can use the dataset we generated for canny2image task based on pokemon_dataset: [pokemon_blip_canny](https://openi.pcl.ac.cn/congw729/mindone/datasets). We have 833 samples for training and 167 samples for test.
...
`control` and `original` is the filefoder with all imgs. The difference is the images under `orginal` folder are original pokemon images from official release, and the images under `control` are the canny image generated from original images. (eg. `control/img0.png` is the canny image of `original/img0.png` )
...
We will use the `scripts/run_train_controlnet_v1.sh` script for finetuning. Before running, please modify the argument `data_path`, `pretrained_model_path`, and `pretrained_model_file` to your own path, for example
...
After training, the lora checkpoint will be saved in `{output_path}/{task_name}/ckpt/sd-{epochs}.ckpt` by default.
