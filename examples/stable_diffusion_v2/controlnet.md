...
Before running the scripts below, make sure you are under `stable_diffusion_v2` folder.
...
1. Canny edge maps:
   ...
   ```
   The results would be saved at `stable_diffusion_v2/output/canny2image` by default.
...
2. Segmentation edge maps (DeeplabV3Plus):
   ...
   The results would be saved at `stable_diffusion_v2/output/seg2image` by default.
...
- `low_threshold` and `high_threshold`: The thresholds used by Canny edge maps control, the default values are 100 and 200 for each.
...
## Fine-tuneing using ControlNet

Try to customize your own ControlNet? Downloading the SD base weight, preparing your dataset and start training!

Now only support training based on SD1.5.

### Download pretrain weights
Please download the pretrained [sd_v1.5-d0ab7146.ckpt](https://download.mindspore.cn/toolkits/mindone/stable_diffusion/sd_v1.5-d0ab7146.ckpt), and put it under `stable_diffusion_v2/models` folder.

### Prepare dataset
You can use the dataset we generated for canny2image task based on pokemon_dataset: [pokemon_blip_canny](https://openi.pcl.ac.cn/congw729/mindone/datasets), and put it under `stable_diffusion_v2/dataset` folder. We have 833 samples for training and 167 samples for test.
We also provide the fill50k dataset:[fill50k_edit](https://openi.pcl.ac.cn/congw729/mindone/datasets). Kindly noted that this version is different with the official release version. We have modified the file folders' name and the prompts text file to make it suitable to our data process pipeline.
...
`control` and `original` is the file folder with all images. The difference is the images under `original` folder are original image or called target image, and the images under `control` are the canny image generated from original images. (eg. `control/img0.png` is the canny image of `original/img0.png` )
...
We will use the `scripts/run_train_controlnet_v1.sh` script for finetuning. Before running, please make sure the arguments `data_path`, `pretrained_model_path`, and `pretrained_model_file` are set to your own path, for example
...
Then, check the training settings in `train_config_file`, some params default setting are as belows:
```text
"train_batch_size": 4,
"optim": "adamw",
...
```shell
sh scripts/train_controlnet.sh
```

After training, the checkpoint will be saved in `{output_path}/{task_name}/ckpt/sd-{epochs}.ckpt` by default.
