...
def main(opts):
    ...
    opt['seed'] = 42
    opt['dpm_solver'] = True
    opt['prompt'] = 'A cute wolf in winter forest'
    opt['output_path'] = os.path.join(opts.output_path, 'inference')
    opt['n_iter'] = 4
    opt['n_samples'] = 8
    opt['ddim_steps'] = 50
    opt['H'] = 512
    ...
    opt['scale'] = None
    opt['ddim_eta'] = 0.0
    # inference(LatentDiffusionWithLoss, opt)
    ...
    if args.save_lora:
        LatentDiffusionWithLoss.init_parameters_data()
        param_dict = OrderedDict()
        for param in LatentDiffusionWithLoss.trainable_params():
            param_dict[param.name] = param
        param_list = []
        for param_name, param in param_dict.items():
            each = {"name": param_name}
            param_data = Tensor(param.data.asnumpy())
            if param_name in LatentDiffusionWithLoss.parameter_layout_dict:
                param_data = _get_merged_param_data(LatentDiffusionWithLoss, param_name, param_data,
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--betas', default=list([0.9, 0.999]), type=list, help='betas for adamw')
