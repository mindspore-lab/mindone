...
class Body(object):
    def __init__(self, model_path):
        ...
        param_not_load, ckpt_not_load = ms.load_param_into_net(self.model, model_dict)
            ...
            if nA != 0 and nB != 0:
                ...
                for i in range(nA):
                    for j in range(nB):
                        ...
                        vec_x = np.array(
                            [
                                score_mid[int(round(startend[j][1])), int(round(startend[j][0])), 0]
                                for j in range(len(startend))
                            ]
                        )
                        vec_y = np.array(
                            [
                                score_mid[int(round(startend[j][1])), int(round(startend[j][0])), 1]
                                for j in range(len(startend))
                            ]
                        )
...
if __name__ == "__main__":
    body_estimation = Body("../model/ms_body_pose_model.ckpt")
    test_image = "../images/pose2.png"
