...
Put them in an arbitrary directory on your machine. For example, `text2video_zero/videos`. We already put some in it.
...
1. Canny edge maps:
   ...
   Open the config file `./config/text2video-zero.yaml`, set `video_path` with the path of video that you want to impaint. set corresponding text prompt in `prompt`. set pretrained model path in `pretrained_ckpt`, e.g. models/control_canny_sd_v1.5_static-6350d204.ckpt`.
