...
class FlowUniPCMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        sigmas = 1.0 - alphas
        sigmas = ms.from_numpy(sigmas).to(dtype=ms.float32)
    ...
    def set_timesteps(
        ...
        self.sigmas = ms.from_numpy(sigmas)
        self.timesteps = ms.from_numpy(timesteps).to(dtype=ms.int32)
    ...
    def _threshold_sample(self, sample: Tensor) -> Tensor:
        ...
        # TODO: ops.quantile -> mint.quantile
    ...
    def multistep_uni_p_bh_update(
        ...
        **kwargs,
        ...
        if len(D1s) > 0:
            ...
            if order == 2:
                rhos_p = Tensor([0.5], dtype=x.dtype)
            else:
        ...
        if self.predict_x0:
            ...
            if D1s is not None:
                pred_res = mint.einsum("k,bkc...->bc...", rhos_p, D1s)
            else:
                pred_res = 0
    ...
    def multistep_uni_c_bh_update(
        ...
        **kwargs,
        ...
        if order == 1:
            rhos_c = Tensor([0.5], dtype=x.dtype)
        else:
        ...
        if self.predict_x0:
            x_t_ = sigma_t / sigma_s0 * x - alpha_t * h_phi_1 * m0
            if D1s is not None:
                corr_res = mint.einsum("k,bkc...->bc...", rhos_c[:-1], D1s)
            else:
                corr_res = 0
            D1_t = model_t - m0
    ...
    def scale_model_input(self, sample: Tensor, *args, **kwargs) -> Tensor:
        """
