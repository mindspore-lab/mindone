...
def cal_2d_sincos_pos_embed(
    ...
    pos_embed = _cal_2d_sincos_pos_embed_from_grid(embed_dim, grid)
    return pos_embed


def _cal_2d_sincos_pos_embed_from_grid(embed_dim: int, grid: Tensor) -> Tensor:
    assert embed_dim % 2 == 0

    # use half of dimensions to encode grid_h
    emb_w = _cal_1d_sincos_pos_embed_from_grid(embed_dim // 2, grid[0])  # (H*W, D/2)
    emb_h = _cal_1d_sincos_pos_embed_from_grid(embed_dim // 2, grid[1])  # (H*W, D/2)

    emb = ops.concat([emb_w, emb_h], axis=1)  # (H*W, D)
    return emb


def _cal_1d_sincos_pos_embed_from_grid(embed_dim: int, pos: Tensor) -> Tensor:
    assert embed_dim % 2 == 0
