...
class PixArtMS(PixArt):
    ...
    def _cal_pos_embed(self, x: Tensor) -> Tensor:
        ...
        pos_emb = cal_2d_sincos_pos_embed(self.hidden_size, nh, nw=nw, scale=self.lewei_scale, base_size=self.base_size)
        return pos_emb
    ...
    def construct(self, x: Tensor, t: Tensor, y: Tensor, mask_y: Optional[Tensor] = None) -> Tensor:
        ...
        ar = Tensor([[h / w]], dtype=x.dtype)  # NOTE: double check if ar is a float number
