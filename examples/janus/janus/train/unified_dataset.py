...
class UnifiedDataset:
    ...
    def __getitem__(self, idx: int) -> Tuple[int, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        if task_type == 0:
            task_type, input_ids, labels, attention_mask, image_seq_mask, image = self.vqa_dataset.__getitem__(idx)
        elif task_type == 1:
            task_type, input_ids, labels, attention_mask = self.text_dataset.__getitem__(idx)
            image_seq_mask = np.zeros((self.max_token_length), dtype=np.bool_)
            image = np.zeros((1, 3, self.image_size, self.image_size), dtype=np.float32)  # t 3 h w, t is reserved for future video generation
        elif task_type == 2:
            task_type, input_ids, labels, attention_mask, image_seq_mask, image = self.t2i_dataset.__getitem__(idx)
        else:
