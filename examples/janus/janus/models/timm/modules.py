...
class SwiGLU(nn.Cell):
    ...
    def __init__(
        ...
        self.fc1_g = mint.nn.Linear(in_features, hidden_features, bias=bias[0])
        self.fc1_x = mint.nn.Linear(in_features, hidden_features, bias=bias[0])
        self.act = act_layer()
        self.drop1 = nn.Dropout(p=drop_probs[0])
        self.norm = norm_layer([hidden_features]) if norm_layer is not None else nn.Identity()
        self.fc2 = mint.nn.Linear(hidden_features, out_features, bias=bias[1])
        self.drop2 = nn.Dropout(p=drop_probs[1])
...
class GatedMlp(nn.Cell):
    ...
    def __init__(
        ...
        self.fc1 = mint.nn.Linear(in_features, hidden_features, bias=bias[0])
        self.act = act_layer()
        self.drop1 = nn.Dropout(p=drop_probs[0])
...
class GlobalResponseNormMlp(nn.Cell):
    """ MLP w/ Global Response Norm (see grn.py), mint.nn.Linear or 1x1 Conv2d
