...
class DepthBaseDecodeHead(nn.Cell):
    """Base class for BaseDecodeHead.

    Args:
        in_channels (List): Input channels.
        channels (int): Channels after modules, before conv_depth.
        conv_layer (nn.Cell): Conv layers. Default: None.
        act_layer (nn.Cell): Activation layers. Default: nn.ReLU.
    ...
    def construct(self, inputs, img_metas):
    ...
    def forward_train(self, img, inputs, img_metas, depth_gt):
        ...
        """
        depth_pred = self.construct(inputs, img_metas)
    ...
    def forward_test(self, inputs, img_metas):
        ...
        """
        return self.construct(inputs, img_metas)
    ...
    def depth_pred(self, feat):
        ...
        if self.classify:
            ...
            if self.bins_strategy == "UD":
                bins = ops.linspace(self.min_depth, self.max_depth, self.n_bins)
            elif self.bins_strategy == "SID":
                bins = ops.logspace(self.min_depth, self.max_depth, self.n_bins)

            # following Adabins, default linear
            if self.norm_strategy == "linear":
                logit = ops.relu(logit)
                eps = 0.1
                logit = logit + eps
                logit = logit / logit.sum(axis=1, keepdims=True)
            elif self.norm_strategy == "softmax":
                logit = mint.nn.functional.softmax(logit, dim=1)
            elif self.norm_strategy == "sigmoid":
                logit = mint.sigmoid(logit)
                logit = logit / logit.sum(axis=1, keepdims=True)

            output = ops.einsum("ikmn,k->imn", [logit, bins]).unsqueeze(1)
    ...
    def losses(self, depth_pred, depth_gt):
        ...
        if not isinstance(self.loss_decode, nn.CellList):
    ...
    def log_images(self, img_path, depth_pred, depth_gt, img_meta):
        ...
        show_img = copy.deepcopy(img_path.permute((1, 2, 0)))
        show_img = show_img.asnumpy().astype(np.float32)
        ...
        depth_pred_color = copy.deepcopy(depth_pred)
        depth_gt_color = copy.deepcopy(depth_gt)
...
class BNHead(DepthBaseDecodeHead):
    ...
    def _forward_feature(self, inputs, img_metas=None, **kwargs):
        ...
        for i, x in enumerate(inputs):
            if len(x) == 2:
                ...
                inputs[i] = mint.cat((x, cls_token), 1)
            else:
                x = x[0]
                if len(x.shape) == 2:
                    x = x[:, :, None, None]
    ...
    def construct(self, inputs, img_metas=None, **kwargs):
        """Forward function."""
...
class ConvModule(nn.Cell):
    ...
    Args:
        ...
        bias (bool | str): If specified as `auto`, it will be decided by the
            ...
            False. Default: "auto".
        conv_layer (nn.Cell): Convolution layer. Default: None,
            which means using conv2d.
        norm_layer (nn.Cell): Normalization layer. Default: None.
        act_layer (nn.Cell): Activation layer. Default: nn.ReLU.
    ...
    def __init__(
        ...
        if self.with_norm:
            ...
            self.norm = partial(norm_layer, num_features=norm_channels)
            self.norm_name = "norm"
    ...
    def init_weights(self):
        ...
        #    and conv layers, they will be initialized by
        #    this method with default ``kaiming_init``.
        # Note: For conv layers, they will be overwritten by our
        ...
        if self.with_norm:
            if hasattr(self.norm, "weight") and self.norm.weight is not None:
                weight = initializer("ones", self.norm.weight.shape)
                self.norm.weight.set_data(weight)
            if hasattr(self.norm, "bias") and self.norm.bias is not None:
                bias_weight = initializer("zeros", self.norm.bias.shape)
                self.norm.bias.set_data(bias_weight)

    def construct(self, x, activate=True, norm=True):
...
class ReassembleBlocks(nn.Cell):
    ...
    def construct(self, inputs):
        ...
        for i, x in enumerate(inputs):
            ...
            if self.readout_type == "project":
                ...
                x = self.readout_projects[i](mint.cat((x, readout), -1))
                x = x.permute((0, 2, 1)).reshape(feature_shape)
            elif self.readout_type == "add":
                x = x.flatten(start_dim=2) + cls_token.unsqueeze(-1)
...
class PreActResidualConvUnit(nn.Cell):
    """ResidualConvUnit, pre-activate residual unit.
    Args:
        in_channels (int): number of channels in the input feature map.
        act_layer (nn.Cell): activation layer.
        norm_layer (nn.Cell): norm layer.
...
class FeatureFusionBlock(nn.Cell):
    ...
    Args:
        ...
        act_layer (nn.Cell): activation layer for ResidualConvUnit.
...
class DPTHead(DepthBaseDecodeHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.fusion_blocks = nn.CellList()
