...
class DepthEncoderDecoder(nn.Cell):
    ...
    def encode_decode(self, img, img_metas, rescale=True, size=None):
        ...
        out = mint.clamp(out, min=self.decode_head.min_depth, max=self.decode_head.max_depth)
        if rescale:
            if size is None:
    ...
    def slide_inference(self, img, img_meta, rescale, stride, crop_size):
        ...
        preds = img.new_zeros((batch_size, 1, h_img, w_img)).to(img.dtype)
        count_mat = img.new_zeros((batch_size, 1, h_img, w_img)).to(img.dtype)
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                preds += ops.pad(depth_pred, (int(x1), int(preds.shape[3] - x2), int(y1), int(preds.shape[2] - y2)))
    ...
    def construct(self, img, img_metas, return_loss=True, **kwargs):
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        Args:
            ...
            optimizer (:obj:`optim.Optimizer` | dict): The optimizer of
