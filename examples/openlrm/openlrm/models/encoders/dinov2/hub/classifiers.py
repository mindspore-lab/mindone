...
def _make_dinov2_linear_classification_head(
    ...
    **kwargs,
    ...
    linear_head = nn.Dense((1 + layers) * embed_dim, 1_000)
...
class _LinearClassifierWrapper(nn.Cell):
    def __init__(self, *, backbone: nn.Cell, linear_head: nn.Cell, layers: int = 4):
        super().__init__()
    ...
    def construct(self, x):
        if self.layers == 1:
            ...
            # fmt: off
            linear_input = mint.cat([
                cls_token,
                patch_tokens.mean(axis=1),
            ], dim=1)
            # fmt: on
        elif self.layers == 4:
