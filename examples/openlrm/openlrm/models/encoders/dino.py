...
# This code is adapted from https://github.com/TencentARC/InstantMesh
...
class ViTEmbeddings(nn.Cell):
    ...
    def __init__(self, config: ViTConfig, use_mask_token: bool = False) -> None:
        ...
        self.cls_token = ms.Parameter(ops.randn(1, 1, config.hidden_size))
        self.mask_token = ms.Parameter(ops.zeros(1, 1, config.hidden_size)) if use_mask_token else None
        self.patch_embeddings = ViTPatchEmbeddings(config)
        num_patches = self.patch_embeddings.num_patches
        self.position_embeddings = ms.Parameter(ops.randn(1, num_patches + 1, config.hidden_size))
        self.dropout = nn.Dropout(p=config.hidden_dropout_prob)
        self.config = config

    def interpolate_pos_encoding(self, embeddings: ops.Tensor, height: int, width: int) -> ops.Tensor:
        """
    ...
    def construct(
        self,
        pixel_values: ops.Tensor,
        bool_masked_pos: Optional[ops.Tensor] = None,
...
class ViTPatchEmbeddings(nn.Cell):
    ...
    def construct(self, pixel_values: ops.Tensor, interpolate_pos_encoding: bool = False) -> ops.Tensor:
        batch_size, num_channels, height, width = pixel_values.shape
...
class ViTSelfAttention(nn.Cell):
    ...
    def transpose_for_scores(self, x: ops.Tensor) -> ops.Tensor:
        new_x_shape = x.shape[:-1] + (self.num_attention_heads, self.attention_head_size)
        x = x.view(new_x_shape)
        return x.permute(0, 2, 1, 3)

    def construct(
        self, hidden_states, head_mask: Optional[ops.Tensor] = None, output_attentions: bool = False
    ) -> Union[Tuple[ops.Tensor, ops.Tensor], Tuple[ops.Tensor]]:
...
class ViTSelfOutput(nn.Cell):
    ...
    def construct(self, hidden_states: ops.Tensor, input_tensor: ops.Tensor) -> ops.Tensor:
...
class ViTAttention(nn.Cell):
    ...
    def construct(
        ...
        head_mask: Optional[ops.Tensor] = None,
...
class ViTIntermediate(nn.Cell):
    ...
    def construct(self, hidden_states: ops.Tensor) -> ops.Tensor:
        hidden_states = self.dense(hidden_states)
...
class ViTModel(ViTPreTrainedModel):
    ...
    def construct(
        self,
        pixel_values: Optional[ops.Tensor] = None,
        adaln_input: Optional[ops.Tensor] = None,
        bool_masked_pos: Optional[ops.Tensor] = None,
        head_mask: Optional[ops.Tensor] = None,
