    ...
    if use_relative_movement:
        ...
        if use_relative_jacobian:
            jacobian_diff = ops.MatMul()(
                kp_driving['jacobian'], ops.inverse(kp_driving_initial['jacobian']))
            kp_new['jacobian'] = ops.MatMul()(
...
def get_rotation_matrix(yaw, pitch, roll):
    ...
    pitch_mat = ops.cat([ops.ones_like(pitch), ops.zeros_like(pitch), ops.zeros_like(pitch),
                         ops.zeros_like(pitch), ops.cos(
                             pitch), -ops.sin(pitch),
                         ops.zeros_like(pitch), ops.sin(pitch), ops.cos(pitch)], axis=1)
    pitch_mat = pitch_mat.view(pitch_mat.shape[0], 3, 3)

    yaw_mat = ops.cat([ops.cos(yaw), ops.zeros_like(yaw), ops.sin(yaw),
                       ops.zeros_like(yaw), ops.ones_like(
                           yaw), ops.zeros_like(yaw),
                       -ops.sin(yaw), ops.zeros_like(yaw), ops.cos(yaw)], axis=1)
    yaw_mat = yaw_mat.view(yaw_mat.shape[0], 3, 3)

    roll_mat = ops.cat([ops.cos(roll), -ops.sin(roll), ops.zeros_like(roll),
                        ops.sin(roll), ops.cos(roll), ops.zeros_like(roll),
