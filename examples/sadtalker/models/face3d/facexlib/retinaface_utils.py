...
def encode(matched, priors, variances):
    ...
    return ops.cat([g_cxcy, g_wh], axis=1)  # [num_priors,4]
...
def batched_decode(b_loc, priors, variances):
    ...
    boxes = (
        ...
        priors[:, :, 2:] * ops.exp(b_loc[:, :, 2:] * variances[1]),
    )
    boxes = ops.cat(boxes, axis=2)
...
def nms(boxes, scores, overlap=0.5, top_k=200):
    ...
    keep = ms.Tensor(scores.shape[0]).fill_(0).long()
    if boxes.numel() == 0:
        return keep
    x1 = boxes[:, 0]
    y1 = boxes[:, 1]
    ...
    while idx.numel() > 0:
        ...
        ops.index_select(x1, 0, idx, out=xx1)
        ops.index_select(y1, 0, idx, out=yy1)
        ops.index_select(x2, 0, idx, out=xx2)
        ops.index_select(y2, 0, idx, out=yy2)
        # store element-wise max with next highest score
        xx1 = ops.clamp(xx1, min=x1[i])
        yy1 = ops.clamp(yy1, min=y1[i])
        xx2 = ops.clamp(xx2, max=x2[i])
        yy2 = ops.clamp(yy2, max=y2[i])
        w.resize_as_(xx2)
        h.resize_as_(yy2)
        w = xx2 - xx1
        h = yy2 - yy1
        # check sizes of xx1 and xx2.. after each iteration
        w = ops.clamp(w, min=0.0)
        h = ops.clamp(h, min=0.0)
        inter = w * h
        # IoU = i / (area(a) + area(b) - i)
        rem_areas = ops.index_select(area, 0, idx)  # load remaining areas)
