...
class NormStyleCode(nn.Cell):
    ...
    def construct(self, x):
        ...
        """
        return x * ops.rsqrt(ops.mean(x**2, axis=1, keep_dims=True) + 1e-8)


class ModulatedConv2d(nn.Cell):
...
class StyleConv(nn.Cell):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, num_style_feat, demodulate=True, sample_mode=None):
        ...
        self.normal = msd.Normal(mean=0, sd=1)

    def construct(self, x, style, noise=None):
        # modulate
        out = self.modulated_conv(x, style) * 2**0.5  # for conversion
        # noise injection
        if noise is None:
            b, _, h, w = out.shape
            noise = self.normal.sample((b, 1, h, w)).astype(out.dtype)
...
class ConstantInput(nn.Cell):
    ...
    def __init__(self, num_channel, size):
        ...
        self.weight = ms.Parameter(ops.randn((1, num_channel, size, size)))

    def construct(self, batch):
        out = self.weight.repeat(batch, axis=0)
        return out


# @ARCH_REGISTRY.register() # TODO!!
class StyleGAN2GeneratorClean(nn.Cell):
    ...
    def make_noise(self):
        ...
        noises = [ops.randn((1, 1, 4, 4))]

        for i in range(3, self.log_size + 1):
            for _ in range(2):
                noises.append(ops.randn((1, 1, 2**i, 2**i)))
    ...
    def mean_latent(self, num_latent):
        latent_in = ops.randn((num_latent, self.num_style_feat))
