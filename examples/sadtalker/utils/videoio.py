...
def save_video_with_watermark(video, audio, save_path, watermark=False):
    ...
    cmd = r'./ffmpeg-6.0-amd64-static/ffmpeg -y -hide_banner -loglevel error -i "%s" -i "%s" -vcodec copy "%s"' % (
    ...
    if watermark is False:
        ...
        cmd = r'./ffmpeg-6.0-amd64-static/ffmpeg -y -hide_banner -loglevel error -i "%s" -i "%s" -filter_complex "[1]scale=100:-1[wm];[0][wm]overlay=(main_w-overlay_w)-10:10" "%s"' % (
