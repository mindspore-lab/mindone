...
class Preprocesser:
    ...
    def align_face(self, img, lm, output_size=1024):
        ...
        x /= np.hypot(*x)  # The hypot func calculates the hypotenuse length of a right triangle
        # and uses the hypotenuse length to normalize the two straight sides of the triangle.
        x *= max(
            np.hypot(*eye_to_eye) * 2.0, np.hypot(*eye_to_mouth) * 1.8
        )  # For the difference between the eyes and mouth, choose the larger one as the reference scale.
        y = np.flipud(x) * [-1, 1]
        c = eye_avg + eye_to_mouth * 0.1
        # Define a quadrilateral and translate up, down, left and right
        # with the facial reference position as the center to obtain four vertices.
