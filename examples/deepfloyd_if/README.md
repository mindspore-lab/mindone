# DeepFloyd-IF based on MindSpore

This is an **unofficial** implementation of DeepFloyd-IF based on mindspore.
...
- StageIII-StableDiffusion-X4. (Depends on `diffusers`)
- StageIII-IF. (The author has not released the pretrained weights)

### Limits

You may need to reduce the number of inputs to avoid OOM problems。
...
3. (optional) If you meet importing error of `skimage`, the following commands may be useful:
    ```shell
    conda install -c conda-forge libstdcxx-ng
    ```

4. Add the root path of project(`${PROJECT_ROOT}`) to `PYTHONPATH`. Make sure you are in `${PROJECT_ROOT}`, run
    ```shell
...
The root path of downloading must be `~/.cache/IF_`, where `~` means you home path.

- T5. from [huggingface](https://huggingface.co/DeepFloyd/t5-v1_1-xxl/tree/main)
- IF-I-M. from [huggingface](https://huggingface.co/DeepFloyd/IF-I-M-v1.0)
- IF-I-M. from [huggingface](https://huggingface.co/DeepFloyd/IF-II-M-v1.0)

Larger model like `IF-I-XL` and `IF-II-L` is also supported, but need more memory.

### Convert

Edit script `demo/pt2np.py`, modify the variable `pytorch_model_paths` in it to the weight path you need to convert,
like `pytorch_model_paths=[a.bin, b.bin, c.bin]`.

In another virtual environment with `torch`, run the following script：

```shell
python demo/pt2np.py
```
...
```shell
python demo/super_resolution.py
```

### IV. Zero-shot Inpainting

```shell
python demo/inpainting.py
```
