...
class BeamScorer(ABC):
    ...
    def process(
        self,
        input_ids: ms.Tensor,  # torch.LongTensor
        next_scores: ms.Tensor,
        next_tokens: ms.Tensor,  # torch.LongTensor
        next_indices: ms.Tensor,  # torch.LongTensor
        **kwargs,
    ) -> Tuple[ms.Tensor]:
...
class BeamSearchScorer(BeamScorer):
    ...
    def finalize(
        ...
        final_beam_tokens: ms.Tensor,  # torch.LongTensor
        final_beam_indices: ms.Tensor,  # torch.LongTensor
        max_length: int,
        pad_token_id: Optional[int] = None,
        eos_token_id: Optional[Union[int, List[int]]] = None,
        beam_indices: Optional[ms.Tensor] = None,  # torch.LongTensor
    ) -> Tuple[ms.Tensor]:  # torch.LongTensor
        batch_size = len(self._beam_hyps)
        ...
        decoded: ms.Tensor = input_ids.new(batch_size * self.num_beam_hyps_to_keep, sent_max_len)  # torch.LongTensor

        if len(best_indices) > 0 and best_indices[0] is not None:
            indices: ms.Tensor = input_ids.new(batch_size * self.num_beam_hyps_to_keep, sent_max_len)  # torch.LongTensor
        else:
            indices = None
...
class ConstrainedBeamSearchScorer(BeamScorer):
    ...
    def step_sentence_constraint(
        ...
        sent_beam_tokens: ms.Tensor,  # torch.LongTensor
        sent_beam_indices: ms.Tensor,  # torch.LongTensor
        ...
        for seq_idx, pre_seq in enumerate(this_batch_input_ids):
            ...
            if not advance_state.completed:
                advance_tokens = ms.Tensor(advance_state.advance(), dtype=ms.int64)
                for advance_token in advance_tokens:
                    ...
                    advance_seq = ops.cat((pre_seq, advance_token.unsqueeze(0)), -1).cpu().tolist()
...
class BeamHypotheses:
    ...
    def add(self, hyp: ms.Tensor, sum_logprobs: float, beam_indices: Optional[ms.Tensor] = None):  # LongTensor
        """
