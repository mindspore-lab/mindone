...
# todo: input_ids is torch.LongTensor
...
class NoBadWordsLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, bad_words_ids: List[List[int]], eos_token_id: Union[int, List[int]]):
        ...
        self.static_bad_words_mask: Optional[ms.Tensor] = None  # torch.LongTensor
    ...
    def _calc_static_bad_word_mask(self, scores: ms.Tensor) -> ms.Tensor:  # torch.BoolTensor
    ...
    def _set_scores_to_inf_for_banned_tokens(
        self, scores: ms.Tensor, banned_tokens: List[List[int]]
    ) -> ms.Tensor:
        """
        ...
        if not banned_mask_list and self.static_bad_words_mask is None:
            ...
            if banned_mask_list:
                indices = ops.ones(len(banned_mask_list))
                banned_mask = ms.Tensor(banned_mask_list, device=indices.device)
...
class HammingDiversityLogitsProcessor(LogitsProcessor):
    ...
    def __call__(
        ...
        input_ids: ms.Tensor,
        scores: ms.Tensor,
        current_tokens: ms.Tensor,  # torch.LongTensor
        beam_group_idx: int,
    ) -> ms.Tensor:
        ...
        for batch_idx in range(batch_size):
            ...
            token_frequency = ops.bincount(previous_group_tokens, minlength=vocab_size).to(scores.device)
...
class WhisperTimeStampLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids, scores):
        ...
        logprobs = ops.log_softmax(scores.float(), dim=-1)
        for k in range(input_ids.shape[0]):
