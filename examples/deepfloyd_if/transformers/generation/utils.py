...
class GreedySearchDecoderOnlyOutput(ModelOutput):
    ...
    sequences: ms.Tensor = None  # torch.LongTensor
...
class ContrastiveSearchDecoderOnlyOutput(ModelOutput):
    ...
    Args:
        sequences (`ms.Tensor` of shape `(batch_size, sequence_length)`):
            The generated sequences. The second dimension (sequence_length) is either equal to `max_length` or shorter
            if all batches finished early due to the `eos_token_id`.
        scores (`tuple(ms.Tensor)` *optional*, returned when `output_scores=True` is passed or when
        ...
        attentions (`tuple(tuple(ms.Tensor))`, *optional*, returned when `output_attentions=True` is passed or `config.output_attentions=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `ms.Tensor` of shape `(batch_size, num_heads, generated_length, sequence_length)`.
        hidden_states (`tuple(tuple(ms.Tensor))`, *optional*, returned when `output_hidden_states=True` is
...
class SampleDecoderOnlyOutput(ModelOutput):
    ...
    Args:
        ...
        attentions (`tuple(tuple(ms.Tensor))`, *optional*, returned when `output_attentions=True` is passed or `config.output_attentions=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `ms.Tensor` of shape `(num_return_sequences*batch_size, num_heads, generated_length,
            sequence_length)`.
        hidden_states (`tuple(tuple(ms.Tensor))`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `ms.Tensor` of shape `(num_return_sequences*batch_size, generated_length, hidden_size)`.
    """
...
class SampleEncoderDecoderOutput(ModelOutput):
    ...
    Args:
        ...
        scores (`tuple(ms.Tensor)` *optional*, returned when `output_scores=True` is passed or when `config.output_scores=True`):
            ...
            each generated token), with each tensor of shape `(batch_size*num_return_sequences, config.vocab_size)`.
        encoder_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or `config.output_attentions=True`):
            Tuple of `ms.Tensor` (one for each layer of the decoder) of shape
        ...
        decoder_attentions (`tuple(tuple(ms.Tensor))`, *optional*, returned when `output_attentions=True` is passed or `config.output_attentions=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `ms.Tensor` of shape `(batch_size*num_return_sequences, num_heads, generated_length,
            sequence_length)`.
        cross_attentions (`tuple(tuple(ms.Tensor))`, *optional*, returned when `output_attentions=True` is passed or `config.output_attentions=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `ms.Tensor` of shape `(batch_size, num_heads, generated_length, sequence_length)`.
        decoder_hidden_states (`tuple(tuple(ms.Tensor))`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `ms.Tensor` of shape `(batch_size*num_return_sequences, generated_length, hidden_size)`.
    """
...
class BeamSearchDecoderOnlyOutput(ModelOutput):
    ...
    scores: Optional[Tuple[ms.Tensor]] = None
    beam_indices: Optional[ms.Tensor] = None  # torch.LongTensor
    attentions: Optional[Tuple[Tuple[ms.Tensor]]] = None
    hidden_states: Optional[Tuple[Tuple[ms.Tensor]]] = None
...
class BeamSampleDecoderOnlyOutput(ModelOutput):
    ...
    Args:
        sequences (`ms.Tensor` of shape `(batch_size*num_return_sequences, sequence_length)`):
            The generated sequences. The second dimension (sequence_length) is either equal to `max_length` or shorter
            if all batches finished early due to the `eos_token_id`.
        sequences_scores (`ms.Tensor` of shape `(batch_size * num_return_sequence)`, *optional*, returned when `output_scores=True` is passed or when `config.output_scores=True`):
            Final beam scores of the generated `sequences`.
        scores (`tuple(ms.Tensor)` *optional*, returned when `output_scores=True` is passed or when `config.output_scores=True`):
            Beam transition scores for each vocabulary token at each generation step. Beam transition scores consisting
            of log probabilities of tokens conditioned on log softmax of previously generated tokens in this beam.
        ...
        hidden_states (`tuple(tuple(ms.Tensor))`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple (one element for each generated token) of tuples (one element for each layer of the decoder) of
            `ms.Tensor` of shape `(batch_size*num_beams, generated_length, hidden_size)`.
    """
...
class BeamSampleEncoderDecoderOutput(ModelOutput):
    ...
    Args:
        sequences (`ms.Tensor` of shape `(batch_size*num_beams, sequence_length)`):
            The generated sequences. The second dimension (sequence_length) is either equal to `max_length` or shorter
            if all batches finished early due to the `eos_token_id`.
        sequences_scores (`ms.Tensor` of shape `(batch_size * num_return_sequence)`, *optional*, returned when `output_scores=True` is passed or when `config.output_scores=True`):
            Final beam scores of the generated `sequences`.
...
class GenerationMixin:
    ...
    def adjust_logits_during_generation(self, logits: ms.Tensor, **kwargs) -> ms.Tensor:
        """
    ...
    def _prepare_attention_mask_for_generation(
        ...
        if is_input_ids and is_pad_token_in_inputs and is_pad_token_not_equal_to_eos_token_id:
            ...
            return ops.ones(inputs.shape[:2], dtype=ms.int64)

    def _prepare_encoder_decoder_kwargs_for_generation(
        self, inputs_tensor: ms.Tensor, model_kwargs, model_input_name: Optional[str] = None
    ) -> Dict[str, Any]:
        # 1. get encoder
    ...
    def _prepare_decoder_input_ids_for_generation(
    ...
    ) -> Tuple[ms.Tensor, Dict[str, ms.Tensor]]:  # LongTensor
        ...
        decoder_input_ids_start = ops.ones((batch_size, 1), dtype=ms.int64) * decoder_start_token_id
    ...
    def _expand_inputs_for_generation(
        ...
        input_ids: Optional[ms.Tensor] = None,  # LongTensor
        **model_kwargs,
    ) -> Tuple[ms.Tensor, Dict[str, Any]]:  # LongTensor
        """Expands tensors from [batch_size, ...] to [batch_size * expand_size, ...]"""

        def _expand_dict_for_generation(dict_to_expand):
            for key in dict_to_expand:
                if dict_to_expand[key] is not None and isinstance(dict_to_expand[key], ms.Tensor):
    ...
    def _get_logits_processor(
        ...
        encoder_input_ids: ms.Tensor,  # LongTensor
    ...
    def compute_transition_scores(
        ...
        Parameters:
            ...
            scores (`tuple(ms.Tensor)`):
                ...
                `ms.Tensor` with up to `max_new_tokens` elements (one element for each generated token), with
    ...
    # @torch.no_grad()
    # todo: is it safe to remove torch.no_grad?
    def generate(
        self,
        inputs: Optional[ms.Tensor] = None,
        ...
        **kwargs,
    ) -> Union[GenerateOutput, ms.Tensor]:  # LongTensor
        r"""
        ...
        if synced_gpus is None:
            if is_deepspeed_zero3_enabled() and ms.communication.get_group_size() > 1:
                synced_gpus = True
            else:
                synced_gpus = False
    ...
    def contrastive_search(
        self,
        input_ids: ms.Tensor,  # LongTensor
        ...
        streamer: Optional["BaseStreamer"] = None,
        **model_kwargs,
    ) -> Union[ContrastiveSearchOutput, ms.Tensor]:  # LongTensor
        r"""
        ...
        if isinstance(eos_token_id, int):
            eos_token_id = [eos_token_id]
        eos_token_id_tensor = ms.Tensor(eos_token_id).to(input_ids.device) if eos_token_id is not None else None
        ...
        unfinished_sequences = ops.ones(input_ids.shape[0], dtype=ms.int64)
        ...
        while True:
            if synced_gpus:
                ...
                this_peer_finished_flag = ms.Tensor(0.0 if this_peer_finished else 1.0).to(input_ids.device)
                # send 0.0 if we finished, 1.0 otherwise
                ops.AllReduce()(this_peer_finished_flag)
    ...
    def greedy_search(
        ...
        streamer: Optional["BaseStreamer"] = None,
        **model_kwargs,
    ) -> Union[GreedySearchOutput, ms.Tensor]:  # LongTensor
        r"""
    ...
    def sample(
        ...
        streamer: Optional["BaseStreamer"] = None,
        **model_kwargs,
    ) -> Union[SampleOutput, ms.Tensor]:  # LongTensor
        r"""
    ...
    def beam_search(
        ...
        synced_gpus: bool = False,
        **model_kwargs,
    ) -> Union[BeamSearchOutput, ms.Tensor]:  # LongTensor
        r"""
    ...
    def beam_sample(
        ...
        synced_gpus: bool = False,
        **model_kwargs,
    ) -> Union[BeamSampleOutput, ms.Tensor]:  # LongTensor
        r"""
        ...
        while True:
            ...
            for _ in range(int(assistant_model.max_assistant_tokens)):
                ...
                if "assistant_past_key_values" in model_kwargs:
                    ...
                    assist_attn = ops.ones_like(candidate_input_ids)
            ...
            if "past_key_values" in model_kwargs:
                model_attn = ops.ones_like(candidate_input_ids)
...
def _ranking_fast(
    ...
    cosine_matrix = ops.matmul(norm_context_hidden, norm_next_hidden.transpose(1, 2)).squeeze(-1)  # [B*K, S]
