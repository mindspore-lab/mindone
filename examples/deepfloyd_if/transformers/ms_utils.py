...
def softmax_backward_data(parent, grad_output, output, dim, self):
    ...
    raise NotImplementedError("MindSpore does not support 'softmax_backward_data'!")


def prune_linear_layer(layer: nn.Dense, index: ms.Tensor, dim: int = 0) -> nn.Dense:  # torch.LongTensor
    """
...
class Conv1D(nn.Cell):
    ...
    def __init__(self, nf, nx):
        super().__init__()
        self.nf = nf
        self.weight = ms.Parameter(ops.randn(nx, nf))
        self.bias = ms.Parameter(ops.zeros(nf))
...
def prune_conv1d_layer(layer: Conv1D, index: ms.Tensor, dim: int = 1) -> Conv1D:  # torch.LongTensor
    """
...
def prune_layer(
    layer: Union[nn.Dense, Conv1D], index: ms.Tensor, dim: Optional[int] = None  # torch.LongTensor
) -> Union[nn.Dense, Conv1D]:
    """
...
def find_pruneable_heads_and_indices(
...
) -> Tuple[Set[int], ms.Tensor]:  # torch.LongTensor
    """
    ...
    index: ms.Tensor = ops.arange(len(mask))[mask].long()  # torch.LongTensor
    return heads, index
...
def id_tensor_storage(tensor: ms.Tensor) -> Tuple[None, int, int]:
    """
    ...
    """
    raise NotImplementedError("MindSpore does not support 'id_tensor_storage'!")
