...
from .ms_utils import (  # noqa: F401
    Conv1D,
...
def get_parameter_device(parameter: Union[nn.Cell, GenerationMixin, "ModuleUtilsMixin"]):
    try:
        return next(parameter.parameters()).device
    except StopIteration:
        # For nn.DataParallel compatibility in PyTorch 1.5

        def find_tensor_attributes(module: nn.Cell) -> List[Tuple[str, Tensor]]:
...
def get_first_parameter_dtype(parameter: Union[nn.Cell, GenerationMixin, "ModuleUtilsMixin"]):
    """
    ...
    for t in parameter.parameters():
        ...
        if t.is_floating_point():
            ...
            if XLA_USE_BF16 in ENV_VARS_TRUE_VALUES and is_torch_tpu_available():
                raise NotImplementedError("MindSpore does not support bfloat16 for now!")
            if XLA_DOWNCAST_BF16 in ENV_VARS_TRUE_VALUES and is_torch_tpu_available():
                if t.dtype == ms.float32:
                    return ms.float16
                if t.dtype == ms.float64:
                    return ms.float32
            return t.dtype
...
def load_sharded_checkpoint(model, folder, strict=True, prefer_safe=True):
    ...
    loader = safe_load_file if load_safe else partial(ms.load, map_location="cpu")
...
def _move_model_to_meta(model, loaded_state_dict_keys, start_prefix):
    ...
    for k in loaded_state_dict_keys:
        ...
        if submodule is not None:
            ...
            if isinstance(new_val, ms.Parameter):
                # isinstance returns False for Params on meta device, so switch after the check
                new_val = ms.Parameter(new_val.to("meta"))
            else:
...
class ModuleUtilsMixin:
    ...
    def get_extended_attention_mask(
        self, attention_mask: Tensor, input_shape: Tuple[int], device = None, dtype: ms.float32 = None
    ) -> Tensor:
        """
    ...
    def floating_point_ops(
        self, input_dict: Dict[str, Union[ms.Tensor, Any]], exclude_embeddings: bool = True
    ) -> int:
        """
...
class PreTrainedModel(nn.Cell, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
    ...
    def _tie_or_clone_weights(self, output_embeddings, input_embeddings):
        ...
        if self.config.torchscript:
            output_embeddings.weight = nn.Parameter(input_embeddings.weight.copy())
        else:
    ...
    def _get_resized_lm_head(
        ...
        new_lm_head = nn.Dense(*new_lm_head_shape, bias=has_new_lm_head_bias)
