...
try:
    from mindspore.ops._op_impl._custom_op.flash_attention.flash_attention_impl import (
        get_flash_attention,
    )
...
class FeedForward(nn.Cell):
    def __init__(
        self, dim, dim_out=None, mult=4, glu=False, dropout=1.0, dtype=ms.float32
    ):
        super().__init__()
        ...
        project_in = (
            nn.SequentialCell(
                nn.Dense(dim, inner_dim).to_float(dtype), nn.GELU().to_float(dtype)
            )
            if not glu
...
class CrossAttention(nn.Cell):
    def __init__(
        ...
        self.enable_flash_attention = (
            ...
            and FLASH_IS_AVAILABLE
            and (ms.context.get_context("device_target") == "Ascend")
        )
        if enable_flash_attention and not self.enable_flash_attention:
            print("WARNING: flash attention not available.")

        self.attention = Attention(dim_head, self.heads)
        self.flash_attention = (
            FlashAttention(self.heads, dim_head)
    ...
    def construct(self, x, context=None, mask=None):
        ...
        kv_input = context if self.cross_attend and context is not None else x
...
class FlashAttention(nn.Cell):
    ...
    def construct(
        self, q, k, v, attention_mask=None, dropout_mask=None, alibi_mask=None
    ):
        ...
        o, l, m = self.flash_attention(
            q, k, v, dim_mask, attention_mask, dropout_mask, alibi_mask
        )
...
class SpatialTransformer(nn.Cell):
    ...
    def __init__(
        ...
        if not use_linear:
            ...
            self.proj_out = zero_module(
                nn.Dense(in_channels, inner_dim).to_float(dtype)
            )
