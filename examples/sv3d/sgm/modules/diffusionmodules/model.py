...
class MemoryEfficientAttnBlock(nn.Cell):
    ...
    def attention(self, h_: Tensor, mask=None) -> Tensor:
        ...
        head_dim = q.shape[-1]
        if c_q % 16 == 0 and c_k % 16 == 0 and head_dim <= 256:
            if mask is None:
                mask = ops.zeros((b_q, c_q, c_q), ms.uint8)
            if not USE_NEW_FA:
                h_ = self.flash_attention(q.to(ms.float16), k.to(ms.float16), v.to(ms.float16), mask.to(ms.uint8))
