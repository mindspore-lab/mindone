...
def set_default(args):
    ...
    if args.enable_modelarts:
        from sgm.util.modelarts import sync_data
...
def create_model_sv3d(
    ...
    if checkpoints is None:
        ckpt = config.model.params.ckpt_path
    else:
        ckpt = checkpoints
    # create model
...
def load_model_from_config(
    ...
    if ckpts:
        logger.info(f"Loading model from {ckpts} with the cfg file")
...
def get_discretization(discretization, sigma_min=0.002, sigma_max=80, rho=7.0):
    if discretization == "LegacyDDPMDiscretization":
        ...
        discretization_config = {
            "target": "sgm.modules.diffusionmodules.discretizer.DiffusersDDPMDiscretization",
        }
    else:
        raise NotImplementedError
...
def init_sampling(
    ...
    if thresholding:
        dyn_thresh_config = {
            "target": "sgm.modules.diffusionmodules.sampling_utils.DynamicThresholding",
            "params": {
