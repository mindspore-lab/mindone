...
class AttnBlock(nn.Cell):
    def __init__(self, in_channels, dtype=ms.float32):
        ...
        self.q = mint.nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, bias=True).to_float(
            dtype
        )
        self.k = mint.nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, bias=True).to_float(
            dtype
        )
        self.v = mint.nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, bias=True).to_float(
            dtype
        )
    ...
    def __init__(
        ...
        **ignore_kwargs,
        ...
        self.conv_in = mint.nn.Conv2d(
            in_channels, self.ch, kernel_size=3, stride=1, padding=1, bias=True
        ).to_float(self.dtype)
...
class Decoder(nn.Cell):
    def __init__(
        ...
        **ignorekwargs,
        ...
        # z to block_in
        self.conv_in = mint.nn.Conv2d(
            z_channels, block_in, kernel_size=3, stride=1, padding=1, bias=True
        ).to_float(self.dtype)
        ...
        self.norm_out = Normalize(block_in)
        self.conv_out = mint.nn.Conv2d(
            block_in, out_ch, kernel_size=3, stride=1, padding=1, bias=True
        ).to_float(self.dtype)
