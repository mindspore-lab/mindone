...
from mindone.transformers import Wav2Vec2ForCTC, Wav2Vec2Processor


def get_sample_indices(
    original_fps: float, total_frames: int, target_fps: float, num_sample: int, fixed_start: Optional[int] = None
) -> np.ndarray:
...
def linear_interpolation(
    features: ms.Tensor, input_fps: float, output_fps: float, output_len: Optional[int] = None
) -> ms.Tensor:
    """
...
class AudioEncoder:
    ...
    def extract_audio_feat(
        self, audio_path: str, return_all_layers: bool = False, dtype: Any = ms.float32
    ) -> ms.Tensor:
        ...
        if return_all_layers:
            feat = mint.cat(res.hidden_states)
        else:
    ...
    def get_audio_embed_bucket(
        self, audio_embed: ms.Tensor, stride: int = 2, batch_frames: int = 12, m: int = 2
    ) -> Tuple[ms.Tensor, int]:
        ...
        for bi in batch_idx:
            if bi < audio_frame_num:
                ...
                frame_audio_embed = (
                    mint.zeros([audio_dim * (2 * m + 1)])
                    if not return_all_layers
                    else mint.zeros([num_layers, audio_dim * (2 * m + 1)])
                )
            batch_audio_eb.append(frame_audio_embed)
        batch_audio_eb = mint.cat([c.unsqueeze(0) for c in batch_audio_eb], dim=0)

        return batch_audio_eb, min_batch_num

    def get_audio_embed_bucket_fps(
        self, audio_embed: ms.Tensor, fps: int = 16, batch_frames: int = 81, m: int = 0
    ) -> Tuple[ms.Tensor, int]:
        num_layers, audio_frame_num, audio_dim = audio_embed.shape
