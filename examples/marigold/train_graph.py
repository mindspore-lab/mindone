...
def parse_args():
    ...
    parser.add_argument("--use_fp16", default=True, type=str2bool, help="train with mindspore.float16")
...
def main(args):
    ...
    replace_unet_conv_in(latent_diffusion_with_loss, args.use_fp16)
    ...
    train_dataset: BaseDepthDataset = get_dataset(
        ...
        tokenizer=tokenizer,
        dtype=mindspore.float16 if args.use_fp16 else mindspore.float32
    )
    ...
    # train
    model.train(
        epochs, dataset, callbacks=callback, dataset_sink_mode=args.dataset_sink_mode, initial_epoch=start_epoch
    )
