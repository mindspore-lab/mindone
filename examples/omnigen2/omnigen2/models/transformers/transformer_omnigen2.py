...
from mindspore import Parameter, Tensor, mint, nn, tensor
...
from mindone.models.utils import constant_, normal_, xavier_uniform_, zeros_
...
from ..attention_processor import OmniGen2AttnProcessor, OmniGen2AttnProcessorFlash2Varlen
...
class OmniGen2TransformerBlock(nn.Cell):
    ...
    def initialize_weights(self) -> None:
        ...
        """
        xavier_uniform_(self.attn.to_q.weight)
        xavier_uniform_(self.attn.to_k.weight)
        xavier_uniform_(self.attn.to_v.weight)
        xavier_uniform_(self.attn.to_out[0].weight)

        xavier_uniform_(self.feed_forward.linear_1.weight)
        xavier_uniform_(self.feed_forward.linear_2.weight)
        xavier_uniform_(self.feed_forward.linear_3.weight)

        if self.modulation:
            zeros_(self.norm1.linear.weight)
            zeros_(self.norm1.linear.bias)

    def construct(
        self, hidden_states: Tensor, attention_mask: Tensor, image_rotary_emb: Tensor, temb: Optional[Tensor] = None
    ) -> Tensor:
        """
...
class OmniGen2Transformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        self.ref_image_refiner = nn.CellList(
            [
                OmniGen2TransformerBlock(
                    hidden_size,
                    num_attention_heads,
    ...
    def initialize_weights(self) -> None:
        ...
        """
        xavier_uniform_(self.x_embedder.weight)
        constant_(self.x_embedder.bias, 0.0)

        xavier_uniform_(self.ref_image_patch_embedder.weight)
        constant_(self.ref_image_patch_embedder.bias, 0.0)

        zeros_(self.norm_out.linear_1.weight)
        zeros_(self.norm_out.linear_1.bias)
        zeros_(self.norm_out.linear_2.weight)
        zeros_(self.norm_out.linear_2.bias)

        normal_(self.image_index_embedding, std=0.02)
    ...
    def img_patch_embed_and_refine(
        ...
        if ref_image_hidden_states.shape[1] < 1:  # if no reference images
            return hidden_states
        else:
            ...
            batch_ref_img_mask = ref_image_hidden_states.new_zeros((num_ref_images, max_ref_img_len), dtype=ms.bool_)
            ...
            for i, (ref_img_len, img_len) in enumerate(zip(l_effective_ref_img_len, l_effective_img_len)):
                ...
                combined_img_hidden_states[i, sum(ref_img_len) : sum(ref_img_len) + img_len] = hidden_states[
                    i, :img_len
                ]
    ...
    def flat_and_pad_to_seq(self, hidden_states, ref_image_hidden_states):
        ...
        img_sizes = [(img.shape[1], img.shape[2]) for img in hidden_states]
        l_effective_img_len = [(H // p) * (W // p) for (H, W) in img_sizes]

        if ref_image_hidden_states is not None:
            ref_img_sizes = [
                [(img.shape[1], img.shape[2]) for img in imgs] if imgs is not None else None
        ...
        padded_ref_img_hidden_states = mint.zeros(
            (batch_size, max_ref_img_len, flat_hidden_states[0].shape[-1]), dtype=flat_hidden_states[0].dtype
        )
        padded_ref_img_mask = mint.zeros((batch_size, max_ref_img_len), dtype=ms.bool_)
        for i in range(batch_size):
            if ref_img_sizes[i] is not None:
                padded_ref_img_hidden_states[i, : sum(l_effective_ref_img_len[i])] = flat_ref_img_hidden_states[i]
                padded_ref_img_mask[i, : sum(l_effective_ref_img_len[i])] = True

        padded_hidden_states = mint.zeros(
            (batch_size, max_img_len, flat_hidden_states[0].shape[-1]), dtype=flat_hidden_states[0].dtype
        )
        padded_img_mask = mint.zeros((batch_size, max_img_len), dtype=ms.bool_)
        for i in range(batch_size):
    ...
    def construct(
        ...
        ref_image_hidden_states: Optional[list[list[Tensor]]] = None,
        ...
        batch_size = len(hidden_states)
        is_hidden_states_tensor = isinstance(hidden_states, Tensor)
        ...
        (
            ...
            freqs_cis, text_attention_mask, l_effective_ref_img_len, l_effective_img_len, ref_img_sizes, img_sizes
        )
