# Adapted from https://github.com/VectorSpaceLab/OmniGen2/blob/main/omnigen2/pipelines/image_processor.py
...
from mindone.diffusers.image_processor import PipelineImageInput, VaeImageProcessor, is_valid_image_imagelist
...
class OmniGen2ImageProcessor(VaeImageProcessor):
    ...
    def get_new_height_width(
        self,
        image: Union[Image, np.ndarray, Tensor],
    ...
    def preprocess(
        ...
        """
        supported_formats = (Image, np.ndarray, Tensor)

        # Expand the missing dimension for 3-dimensional mindspore tensor or numpy array that represents grayscale image
        if self.config.do_convert_grayscale and isinstance(image, (Tensor, np.ndarray)) and image.ndim == 3:
        ...
        if isinstance(image, list) and isinstance(image[0], Tensor) and image[0].ndim == 4:
            warnings.warn(
                "Passing `image` as a list of 4d Tensor is deprecated."
                "Please concatenate the list along the batch dimension and pass it as a single 4d Tensor",
                FutureWarning,
            )
            image = mint.cat(image, axis=0)
        ...
        elif isinstance(image[0], Tensor):
            image = mint.cat(image, axis=0) if image[0].ndim == 4 else mint.stack(image, axis=0)
