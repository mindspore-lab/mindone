...
class sde:
    ...
    def __init__(self, drift, diffusion, *, t0, t1, num_steps, sampler_type):
        ...
        self.t = mint.linspace(t0, t1, num_steps)
    ...
    def __Euler_Maruyama_step(self, x, mean_x, t, model, **model_kwargs):
        w_cur = mint.randn(x.shape).to(x.dtype)
        t = mint.ones(x.shape[0]).to(x.dtype) * t
        dw = w_cur * mint.sqrt(self.dt)
        drift = self.drift(x, t, model, **model_kwargs)
        diffusion = self.diffusion(x, t)
        mean_x = x + drift * self.dt
        x = mean_x + mint.sqrt(2 * diffusion) * dw
        return x, mean_x

    def __Heun_step(self, x, _, t, model, **model_kwargs):
        w_cur = mint.randn(x.shape).to(x.dtype)
        dw = w_cur * mint.sqrt(self.dt)
        t_cur = mint.ones(x.shape[0]).to(x.dtype) * t
        diffusion = self.diffusion(x, t_cur)
        xhat = x + mint.sqrt(2 * diffusion) * dw
    ...
    def __forward_fn(self):
        ...
        sampler_dict = {"Euler": self.__Euler_Maruyama_step, "Heun": self.__Heun_step}
...
class ode:
    ...
    def __init__(
        self, drift, *, t0, t1, sampler_type, num_steps, atol, rtol, do_shift=False, time_shifting_factor=None
    ):
    ...
    def sample(self, x, model, **model_kwargs):
        ...
        def _fn(t, x):
            t = mint.ones(x[0].shape[0]) * t if isinstance(x, tuple) else mint.ones(x.shape[0]) * t
