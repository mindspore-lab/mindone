# Adapted from https://github.com/VectorSpaceLab/OmniGen2/blob/main/omnigen2/schedulers/scheduling_dpmsolver_multistep.py
...
from mindone.diffusers.schedulers.scheduling_utils import KarrasDiffusionSchedulers, SchedulerMixin, SchedulerOutput
from mindone.diffusers.utils import deprecate, is_scipy_available
...
class DPMSolverMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if trained_betas is not None:
            ...
            self.betas = mint.linspace(beta_start, beta_end, num_train_timesteps, dtype=ms.float32)
        elif beta_schedule == "scaled_linear":
            # this schedule is very specific to the latent diffusion model.
            self.betas = mint.linspace(beta_start**0.5, beta_end**0.5, num_train_timesteps, dtype=ms.float32) ** 2
        ...
        self.timesteps = ms.from_numpy(timesteps)
    ...
    def set_timesteps(
        self, num_inference_steps: int = None, timesteps: Optional[list[int]] = None, num_tokens: Optional[int] = None
    ):
        ...
        timesteps = ms.from_numpy(timesteps).to(dtype=ms.float32)
        sigmas = mint.cat([1 - timesteps, mint.zeros((1,))])
    ...
    def convert_model_output(self, model_output: ms.Tensor, *args, sample: ms.Tensor = None, **kwargs) -> ms.Tensor:
        """
    ...
    def dpm_solver_first_order_update(
        self, model_output: ms.Tensor, *args, sample: ms.Tensor = None, noise: Optional[ms.Tensor] = None, **kwargs
    ) -> ms.Tensor:
        """
    ...
    def step(
        ...
        if self.config.algorithm_type in ["sde-dpmsolver", "sde-dpmsolver++"] and variance_noise is None:
            ...
            noise = variance_noise.to(ms.float32)
        else:
            noise = None
    ...
    def add_noise(self, original_samples: ms.Tensor, noise: ms.Tensor, timesteps: ms.Tensor) -> ms.Tensor:
        # Make sure sigmas and timesteps have the same dtype as original_samples
        sigmas = self.sigmas.to(original_samples.dtype)
        schedule_timesteps = self.timesteps
