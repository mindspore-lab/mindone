# Adapted from https://github.com/VectorSpaceLab/OmniGen2/blob/main/omnigen2/schedulers/scheduling_flow_match_euler_discrete.py
...
class FlowMatchEulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(self, num_train_timesteps: int = 1000, dynamic_time_shift: bool = True):
        timesteps = mint.linspace(0, 1, num_train_timesteps + 1, dtype=ms.float32)[:-1]
    ...
    def set_timesteps(
        self, num_inference_steps: int = None, timesteps: Optional[list[float]] = None, num_tokens: Optional[int] = None
    ):
        """
        ...
        _timesteps = mint.cat([timesteps, mint.ones((1,))])
    ...
    def step(
        ...
        if isinstance(timestep, int) or (timestep.dtype not in (ms.float32, ms.float16, ms.bfloat16)):
            raise ValueError(
                (
