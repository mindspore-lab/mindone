...
class VQVAE(nn.Cell):
    def __init__(
        ...
        self.post_quant_conv = mint.nn.Conv2d(self.Cvae, self.Cvae, quant_conv_ks, stride=1,
    ...
    def img_to_idxBl(self, inp_img_no_grad: ms.Tensor,
        ...
        ms.Tensor]:  # return List[Bl]
        f = self.quant_conv(self.encoder(inp_img_no_grad))
        return self.quantize.f_to_idxBl_or_fhat(f, to_fhat=False, v_patch_nums=v_patch_nums)

    def idxBl_to_img(self, ms_idx_Bl: List[ms.Tensor], same_shape: bool, last_one=False) -> Union[
