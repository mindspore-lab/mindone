...
class VAR(nn.Cell):
    def __init__(
        ...
        # 4. backbone blocks
        self.shared_ada_lin = nn.SequentialCell(mint.nn.SiLU(inplace=False),
                                            SharedAdaLin(self.D, 6 * self.C)) if shared_aln else mint.nn.Identity()

        norm_layer = partial(mint.nn.LayerNorm, eps=norm_eps)
        self.drop_path_rate = drop_path_rate
        dpr = [x.item() for x in
               mint.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule (linearly increasing)
        ...
        attn_bias_for_masking = mint.where(d >= dT, 0., -ms.numpy.inf).reshape(1, 1, self.L, self.L)
    ...
    def init_weights(self, init_adaln=0.5, init_adaln_gamma=1e-5, init_head=0.02, init_std=0.02, conv_std_or_gain=0.02):
        ...
        for block_idx, sab in enumerate(self.blocks):
            ...
            if hasattr(sab, 'ada_lin'):
                ops.assign(sab.ada_lin[-1].weight.data[2 * self.C:], sab.ada_lin[-1].weight.data[2 * self.C:].mul(init_adaln))
                ops.assign(sab.ada_lin[-1].weight.data[:2 * self.C], sab.ada_lin[-1].weight.data[:2 * self.C].mul(init_adaln_gamma))
