...
class VectorQuantizer2(nn.Cell):
    ...
    def __init__(
        ...
        if share_quant_resi == 0:  # non-shared: \phi_{1 to K} for K scales
            self.quant_resi = PhiNonShared(
                [(Phi(Cvae, quant_resi) if abs(quant_resi) > 1e-6 else mint.nn.Identity()) for _ in
            ...
            self.quant_resi = PhiPartiallyShared(nn.CellList(
                [(Phi(Cvae, quant_resi) if abs(quant_resi) > 1e-6 else mint.nn.Identity()) for _ in
    ...
    def construct(self, f_BChw: ms.Tensor, ret_usages=False) -> Tuple[ms.Tensor, List[float], ms.Tensor]:
        ...
        for si, pn in enumerate(self.v_patch_nums):  # from small to large
            ...
            if self.using_znorm:
                ...
                d_no_grad = mint.addmm(d_no_grad, rest_NC, self.embedding.weight.data.T, alpha=-2, beta=1)  # (B*h*w, vocab_size)
    ...
    def f_to_idxBl_or_fhat(self, f_BChw: ms.Tensor, to_fhat: bool,
    ...
    def get_next_autoregressive_input(self, si: int, SN: int, f_hat: ms.Tensor, h_BChw: ms.Tensor) -> Tuple[
        Optional[ms.Tensor], ms.Tensor]:  # only used in VAR inference
