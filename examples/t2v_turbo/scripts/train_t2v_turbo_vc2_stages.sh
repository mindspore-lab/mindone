...
# stage 1: train with distillation loss and image-text loss
OUTPUT_DIR=outputs/t2v-train-s1/
...
# stage 2: train with distillation loss and video-text reward
OUTPUT_DIR=outputs/t2v-train-s2/
lora_path=outputs/t2v-train-s1/ckpt/t2v-turbo-e20.ckpt
python train_t2v_turbo_vc2.py \
  --pretrained_model_path $t2v_model_path \
  --pretrained_enc_path $t2v_encoder_path \
