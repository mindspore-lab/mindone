...
class AttentionPoolingBlock(AttentiveBlock):
    def forward(self, x, return_attn=False):
        ...
        x, attn = super().forward(
            x_q, x_kv, pos_q, pos_k, bool_masked_pos=None, rel_pos_bias=None, return_attn=return_attn
        )
        x = x.squeeze(1)
        return x, attn
...
def internvl_clip_6b(img_size, clip_norm_type="l2", return_attn=True, clip_return_layer=1, clip_return_interval=1):
    model = InternVL_CLIP(
        img_size=img_size,
        layerscale_no_force_fp32=False,
