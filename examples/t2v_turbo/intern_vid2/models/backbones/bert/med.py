...
from .xbert import BertAttention, BertIntermediate, BertOnlyMLMHead, BertOutput, BertPooler
...
class BertLayer(nn.Module):
    def __init__(self, config, layer_num):
        ...
        if hasattr(self.config, "cross_freq") and self.config.cross_freq > 0:
            ...
            add_cross_attention = self.fusion_layer <= layer_num and self.config.add_cross_attention
...
class XBertEncoder(BertModel, BaseEncoder):
    ...
    def from_config(cls, med_config_path, pretrained=None, fusion_layer=None):
        ...
        if pretrained is not None:
            return cls.from_pretrained(pretrained, config=med_config, add_pooling_layer=False, local_files_only=True)
        else:
