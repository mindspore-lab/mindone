...
class MetricLogger(object):
    ...
    def get_global_avg_dict(self, prefix=""):
        ...
        d = {f"{prefix}{k}": m.global_avg if m.count > 0 else 0.0 for k, m in self.meters.items()}
        return d
...
def match_key_and_shape(state_dict1, state_dict2):
    ...
    for k in list(keys1):
        if state_dict1[k].shape != state_dict2[k].shape:
            print(f"k={k}, state_dict1[k].shape={state_dict1[k].shape}, state_dict2[k].shape={state_dict2[k].shape}")
            mismatch += 1
