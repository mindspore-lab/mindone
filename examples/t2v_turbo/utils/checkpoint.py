...
class CheckpointManager:
    ...
    def save_top_k(self, network, perf, ckpt_name, verbose=True, append_dict=None):
        ...
        if len(self.ckpt_queue) > self.k:
            ...
            if to_del[1] != ckpt_name:
                save_lora_weight(
                    network, os.path.join(self.ckpt_save_dir, ckpt_name), self.lora_manager.unet_replace_modules
                )
    ...
    def save_latest_k(self, network, ckpt_name, append_dict):
        ...
        save_lora_weight(network, os.path.join(self.ckpt_save_dir, ckpt_name), self.lora_manager.unet_replace_modules)
