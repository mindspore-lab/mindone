...
import audioldm.latent_diffusion as ldm
import flan_t5_large
...
def inject_trainable_lora_to_textencoder(
    net: nn.Cell, target_modules=["T5Attention"], rank=4, dropout_p=0.0, scale=1.0, use_fp16=False, verbose=0
):
    """
    ...
    target_modules = [getattr(flan_t5_large.t5, m) for m in target_modules]
...
def inject_trainable_lora(
    ...
    2. In order to find the target layers, currently the attention moduel must have the attribute of to_q, to_k,
        ...
        of a dense layer and a dropout layer.
    """
    target_modules = [getattr(ldm.attention, m) for m in target_modules]
