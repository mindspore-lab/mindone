...
class STFT(nn.Cell):
    ...
    def __init__(self, filter_length, hop_length, win_length, window="hann"):
        ...
        inverse_basis = ms.Tensor(
            np.linalg.pinv(scale * fourier_basis).T[:, None, :], dtype=dtype
        )
    ...
    def inverse(self, magnitude, phase):
        recombine_magnitude_phase = ops.cat(
            [magnitude * ops.cos(phase), magnitude * ops.sin(phase)], dim=1
        )
        ...
        if self.window is not None:
            ...
            approx_nonzero_indices = ms.Tensor.from_numpy(
