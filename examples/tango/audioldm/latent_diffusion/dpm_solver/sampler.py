        ...
        **kwargs,
        ...
        if conditioning is not None:
            if isinstance(conditioning, dict):
                ...
                if cbs != batch_size:
                    _logger.warning(f"Warning: Got {cbs} conditionings but batch-size is {batch_size}")
            else:
        ...
        **kwargs,
        ...
        if conditioning is not None:
            if isinstance(conditioning, dict):
                cbs = conditioning[list(conditioning.keys())[0]].shape[0]
                if cbs != x.shape[0]:
                    _logger.warning(f"Warning: Got {cbs} conditionings but batch-size is {x.shape[0]}")
            else:
