...
from .mininlp.configs import HF_TOKENIZER_CONFIG_URL_BASE
from .mininlp.pretrained_tokenizer import PreTrainedTokenizer
from .t5_config import T5_SUPPORT_LIST

PRETRAINED_VOCAB_MAP = {model: HF_TOKENIZER_CONFIG_URL_BASE.format(model) for model in T5_SUPPORT_LIST}
...
class T5Tokenizer(PreTrainedTokenizer):
    ...
    def __call__(self, text_input):
        ...
        if isinstance(text_input, str):
            ...
            raise TypeError(f"Input should be a text line in 1-D NumPy format, got {type(text_input)}.")
