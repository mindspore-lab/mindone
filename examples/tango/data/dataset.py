...
def build_dataset(args, group_size, rank, tokenizer, is_train=True):
    ...
    def read_wav(filename, sr=16000, target_length=160 * 1024):
        filename = str(filename).replace("b'", "").replace("'", "")
        _wav, _sr = read(filename)
        signed_int16_max = 2**15
        if _wav.dtype == np.int16:
            _wav = _wav.astype(np.float32) / signed_int16_max
    ...
    def tokenise(text):
        ...
        return input_ids.asnumpy(), attention_mask.asnumpy()
    ...
    ds = ds.project(["audio", "input_ids", "attention_mask"])
