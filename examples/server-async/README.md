...
> However, up until now, due to MindSpore's lack of support for multi-threading operations, the maximum number of workers in the thread pool used for model inference has been strictly limited to 1.

## ⚠️ IMPORTANT

* The example demonstrates how to run pipelines like `StableDiffusion3-3.5` concurrently while keeping a single copy of the heavy model parameters on NPU.
...
## What `mindone.diffusers-async` adds / Why we needed it
...
`mindone.diffusers-async` / this example addresses that by:
...
1. **Single model instance** is loaded into memory (Ascend NPU) when the server starts.
...
3. **Result**: inference completes, images are moved to CPU & saved (if requested), internal buffers freed (GC + `mindspore.runtime.empty_cache()`).
...
# install dependencies for server-async
cd examples/server-async
