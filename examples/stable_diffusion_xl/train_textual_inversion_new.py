...
from gm.helpers import SD_XL_BASE_RATIOS, VERSION2SPECS, create_model, get_learning_rate, get_optimizer, set_default
from gm.models.trainer_factory_new import LatentDiffusionWithLoss
...
def get_parser_train():
    ...
    parser.add_argument("--loss_scaler", default="static", type=str, help="use static or dynamic loss scaler")
    parser.add_argument("--init_loss_scale", default=1024.0, type=float, help="loss scale")
    parser.add_argument(
        "--loss_scale_factor", default=2, type=float, help="loss scale factor, only applies for dynamic loss scale"
    )
    parser.add_argument(
        "--scale_window", default=100, type=float, help="scale window, only applies for dynamic loss scale"
    )
    parser.add_argument(
        "--template_file",
        type=str,
    ...
    parser.add_argument(
        "--clip_grad", default=True, type=ast.literal_eval, help="Specify whether to apply gradient clipping."
    )
    parser.add_argument(
        "--max_grad_norm",
        default=1.0,
...
def get_loss_scaler(ms_loss_scaler="static", scale_value=1024, scale_factor=2, scale_window=1000):
    if ms_loss_scaler == "dynamic":
        ...
        loss_scaler = DynamicLossScaleUpdateCell(scale_value, scale_factor=scale_factor, scale_window=scale_window)
    elif ms_loss_scaler == "static":
        from mindspore.nn import FixedLossScaleUpdateCell

        loss_scaler = FixedLossScaleUpdateCell(scale_value)
    elif ms_loss_scaler in ("none", "None"):
        from mindspore.nn import FixedLossScaleUpdateCell

        loss_scaler = FixedLossScaleUpdateCell(1.0)
    else:
...
def train(args):
    ...
    # 4. Training funcs, train_step_fn, lr scheduler, optimizer etc.
    ...
    scaler = get_loss_scaler(
        ms_loss_scaler=args.loss_scaler,
        scale_value=args.init_loss_scale,
        scale_factor=args.loss_scale_factor,
        scale_window=args.scale_window,
    )

    ldm_with_loss = LatentDiffusionWithLoss(model)
