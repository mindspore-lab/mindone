from io import BytesIO
from urllib.request import urlopen
import librosa
import mindspore as ms
from mindspore import Tensor
from transformers import AutoProcessor, Qwen2AudioConfig
from mindone.transformers import Qwen2AudioForConditionalGeneration
import numpy as np
from mindone.transformers.mindspore_adapter.utils import dtype_to_str
if __name__ == '__main__':
    model_name = "Qwen/Qwen2-Audio-7B"
    dtype_name = ms.bfloat16
    config = Qwen2AudioConfig.from_pretrained(
        model_name,
        attn_implementation="eager",
        tie_word_embeddings=False,
    )
    if getattr(config, "head_dim", None) is None:
        config.head_dim = config.text_config.hidden_size // config.text_config.num_attention_heads
    if getattr(config, "num_key_value_heads", None) is None:
        config.num_key_value_heads = config.text_config.num_key_value_heads
    if getattr(config, "num_hidden_layers", None) is None:
        config.num_hidden_layers = config.text_config.num_hidden_layers
    if getattr(config, "mindspore_dtype", None) is None:
        config.text_config.mindspore_dtype = dtype_to_str(dtype_name)

    model = Qwen2AudioForConditionalGeneration.from_pretrained(
        model_name,
        config=config,
        mindspore_dtype=dtype_name,
    )

    processor = AutoProcessor.from_pretrained(model_name)

    prompt = "<|audio_bos|><|AUDIO|><|audio_eos|>Generate the caption in English:"
    # url = "https://qianwen-res.oss-cn-beijing.aliyuncs.com/Qwen2-Audio/audio/glass-breaking-151256.mp3"
    # audio, _ = librosa.load(BytesIO(urlopen(url).read()), sr=self.processor.feature_extractor.sampling_rate)
    audio_path = "/path/to/audio"
    audio, _ = librosa.load(audio_path, sr=processor.feature_extractor.sampling_rate)

    inputs = processor(text=prompt, audios=audio, return_tensors="np")

    for key,value in inputs.items():
        if isinstance(value, np.ndarray):
            inputs[key] = ms.Tensor(value)
        if isinstance(value, list):
            inputs[key] = ms.Tensor(value)
        if inputs[key].dtype == ms.int64:
            inputs[key] = inputs[key].to(ms.int32)
        if key == 'input_features':
            inputs[key] = ms.Tensor(value).to(dtype_name)

    # Generate
    generate_ids = model.generate(**inputs, max_new_tokens=30, use_cache=False)
    print(generate_ids)
    answer = processor.batch_decode(generate_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
    print("response:",answer)
    # "Generate the caption in English: Glass is breaking."
