...
for step in range(5):
    # encode the new user input, add the eos_token and return a tensor in MindSpore
    new_user_input_ids = ms.tensor(tokenizer.encode(input(">> User:") + tokenizer.eos_token, return_tensors='np'))

    # append the new user input tokens to the chat history
    bot_input_ids = mint.cat([chat_history_ids, new_user_input_ids.to(ms.int32)], dim=-1) if step > 0 else new_user_input_ids
