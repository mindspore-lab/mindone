# Adapted from https://github.com/QwenLM/Qwen2.5-Omni/blob/main/qwen-omni-utils/src/qwen_omni_utils/v2_5/vision_process.py
...
def _read_video_decord(
    ...
    video = vr.get_batch(idx).asnumpy()  # THWC
    sample_fps = nframes / max(total_frames, 1e-6) * video_fps
    return video, sample_fps
...
def fetch_video(
    ...
    if isinstance(ele["video"], str):
        ...
        video = [Image.fromarray(image) for image in images]
...
) -> tuple[list[Image.Image] | None, list[list[Image.Image]] | None, Optional[dict]]:
