...
def expand_t_like_x(t, x):
    ...
    """
    dims = [1] * len(x.shape)
    t = ops.reshape(t, (t.shape[0], *dims))
    return t
...
class ICPlan:
    ...
    def compute_diffusion(self, x, t, form="constant", norm=1.0):
        ...
        choices = {
            ...
            "linear": norm * (1 - t),
            "decreasing": 0.25 * (norm * ops.cos(np.pi * t) + 1) ** 2,
            "inccreasing-decreasing": norm * ops.sin(np.pi * t) ** 2,
        }
...
class VPCPlan(ICPlan):
    ...
    def compute_sigma_t(self, t):
        ...
        sigma_t = ops.sqrt(1 - ops.exp(p_sigma_t))
        d_sigma_t = ops.exp(p_sigma_t) * (2 * self.d_log_mean_coeff(t)) / (-2 * sigma_t)
...
class GVPCPlan(ICPlan):
    ...
    def compute_alpha_t(self, t):
        """Compute coefficient of x1"""
        alpha_t = ops.sin(t * np.pi / 2)
        d_alpha_t = np.pi / 2 * ops.cos(t * np.pi / 2)
        return alpha_t, d_alpha_t

    def compute_sigma_t(self, t):
        """Compute coefficient of x0"""
        sigma_t = ops.cos(t * np.pi / 2)
        d_sigma_t = -np.pi / 2 * ops.sin(t * np.pi / 2)
        return sigma_t, d_sigma_t

    def compute_d_alpha_alpha_ratio_t(self, t):
        """Special purposed function for computing numerical stabled d_alpha_t / alpha_t"""
        return np.pi / (2 * ops.tan(t * np.pi / 2))
