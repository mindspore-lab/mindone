...
class MMDoubleStreamBlock(nn.Cell):
    ...
    def construct(
        ...
        token_replace_vec: ms.Tensor = None,
        ...
        if self.condition_type == "token_replace":
            img_mod1, token_replace_img_mod1 = self.img_mod(
                vec, condition_type=self.condition_type, token_replace_vec=token_replace_vec
            )
            (
                img_mod1_shift,
                img_mod1_scale,
            ...
            (
                ...
                tr_img_mod2_scale,
                tr_img_mod2_gate,
            ) = token_replace_img_mod1.chunk(6, axis=-1)
        else:
            (
                img_mod1_shift,
                img_mod1_scale,
...
class MMSingleStreamBlock(nn.Cell):
    ...
    def construct(
        ...
        """
        if self.condition_type == "token_replace":
            mod, tr_mod = self.modulation(vec, condition_type=self.condition_type, token_replace_vec=token_replace_vec)
            (mod_shift, mod_scale, mod_gate) = mod.chunk(3, axis=-1)
            (tr_mod_shift, tr_mod_scale, tr_mod_gate) = tr_mod.chunk(3, axis=-1)
        else:
...
class HYVideoDiffusionTransformer(ModelMixin, ConfigMixin):
    ...
    def construct(
        ...
        if self.i2v_condition_type == "token_replace":
            token_replace_t = mint.zeros_like(t)
        ...
        # text modulation
        vec_2 = self.vector_in(text_states_2.to(self.param_dtype))
        vec = vec + vec_2
        if self.i2v_condition_type == "token_replace":
