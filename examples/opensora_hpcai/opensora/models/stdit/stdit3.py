...
class STDiT3(nn.Cell):
    def __init__(
        ...
        if self.enable_sequence_parallelism:
            ...
            logger.info(f"Initialize model with sequence parallel group `{sp_group}`.")
            self.sp_size = get_group_size(sp_group)
            self.split = SplitFowardGatherBackward(dim=2, grad_scale="down", group=sp_group)
            self.gather = GatherFowardSplitBackward(dim=2, grad_scale="up", group=sp_group)
        ...
        **kwargs,
        ...
        if self.enable_sequence_parallelism:
            ...
            if h_pad_size > 0:
                ...
                H += h_pad_size
                x = mint.pad(x, (0, 0, 0, hx_pad_size))
