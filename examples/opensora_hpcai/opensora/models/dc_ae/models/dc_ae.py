...
class DCAE(nn.Cell):
    ...
    def load_model(self):
        if self.cfg.pretrained_source == "dc-ae":
            load_checkpoint_to_net(self, self.cfg.pretrained_path)
        else:
    ...
    def encode_single(self, x: ms.tensor, is_video_encoder: bool = False) -> ms.tensor:
        assert x.shape[0] == 1
        is_video = x.dim() == 5
    ...
    def spatial_tiled_encode(self, x: ms.tensor) -> ms.tensor:
        ...
        return mint.cat(result_rows, dim=-2)
    ...
    def temporal_tiled_encode(self, x: ms.tensor) -> ms.tensor:
        ...
        return mint.cat(result_row, dim=2)
    ...
    def decode_single(self, z: ms.tensor, is_video_decoder: bool = False) -> ms.tensor:
        assert z.shape[0] == 1
        is_video = z.dim() == 5
    ...
    def construct(self, x: ms.tensor) -> tuple[Any, ms.tensor, dict[Any, Any]]:
        x_type = x.dtype
