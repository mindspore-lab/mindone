...
class AlltoAll(nn.Cell):
    def __init__(self, split_dim: int = 2, concat_dim: int = 1, group: str = GlobalComm.WORLD_COMM_GROUP) -> None:
        ...
        self.alltoall = ops.AlltoAll(split_count=world_size, split_dim=split_dim, concat_dim=concat_dim, group=group)
        self.alltoall_grad = ops.AlltoAll(
            split_count=world_size, split_dim=concat_dim, concat_dim=split_dim, group=group
        )
