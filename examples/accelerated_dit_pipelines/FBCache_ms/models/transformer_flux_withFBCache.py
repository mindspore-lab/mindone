...
def FBCache_transformer_construct(
    ...
    cache_context = getattr(self, "fb_cache_context", None)
    if is_negative:
        neg_cache_context = getattr(self, "fb_cache_context_neg", None)
        if neg_cache_context is not None:
            cache_context = neg_cache_context
    if cache_context is None:
        raise AttributeError(
            "FBCache transformer requires `fb_cache_context` (and optionally `fb_cache_context_neg` for true CFG)."
        )
    if cache_context.enable_taylor:
        cache_context.step()
    ...
    can_use_cache = are_two_tensors_similar(
        cache_context.first_residual,
        first_hidden_states_residual,
        threshold=residual_diff_threshold,
    )
    if can_use_cache:
        hidden_states = hidden_states + cache_context.get_residual()
    else:
        cache_context.update_first_residual(first_hidden_states_residual)
        original_hidden_states = hidden_states
