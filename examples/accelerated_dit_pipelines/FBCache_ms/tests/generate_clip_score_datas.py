...
def generate_cache_imgs(args, parent_path, data_root_path):
    ...
    hw_threshold_map = {
        1024: [0.08, 0.12],  # 对1024分辨率使用这两个阈值
        2048: [0.06, 0.08]   # 对2048分辨率使用这两个阈值
    }
    ...
    for hw, thresholds in hw_threshold_map.items():
        for threshold in thresholds:
            for taylorseer_derivative in taylorseer_derivative_options:
                ...
                avg_time = all_spend_time / i
                print(f"generate {hw} img with threshold={threshold}, taylorseer_derivative={taylorseer_derivative} spend time: {avg_time:.3f}s")
