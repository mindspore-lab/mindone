...
from mindone.diffusers.loaders import (
    FluxIPAdapterMixin,
    FluxLoraLoaderMixin,
    FromSingleFileMixin,
...
from .models.transformer_flux_withFBCache import FBCache_transformer_construct
from .utils import CacheContext
...
class FluxPipeline(
    ...
    def _register_func(self):
        # Register the FBCache-enhanced transformer forward method to the transformer model.
    ...
    def __call__(
        ...
        # Initialize cache context for FBCache with batch size, sequence length, and hidden dimension.
        bs, seq, _ = latents.shape
        # NOTE: FluxTransformer2DModel has CacheMixin.cache_context() method, so avoid using attribute name `cache_context`.
        self.transformer.fb_cache_context = CacheContext(
            batch_size=bs,
            seq_len=seq,
            inner_dim=self.transformer.inner_dim,
            taylorseer_derivative=taylorseer_derivative,
        )
        # For true CFG, keep a separate cache context for the negative branch to avoid cache pollution.
        self.transformer.fb_cache_context_neg = (
            CacheContext(
                batch_size=bs,
                seq_len=seq,
                inner_dim=self.transformer.inner_dim,
