...
def load(root_path: str, force_fp32: bool = False) -> Dict[str, Tensor]:
    ...
    for x in tqdm(filelist, desc="Loading the safetensors"):
        params_chunk = load_safetensors(x, force_fp32=force_fp32)
...
def main():
    parser = argparse.ArgumentParser(description="Convert LLama checkpoints into Mindspore Format")
    parser.add_argument("src", help="Directory storing the safetensors")
    parser.add_argument("-o", "--output", default="models/llama.ckpt", help="Name of the output Mindspore checkpoint")
    parser.add_argument("--force_fp32", action="store_true", help="Force to save the ckpt in fp32 format.")

    args = parser.parse_args()

    params = load(args.src, force_fp32=args.force_fp32)
