...
class LlamaRotaryEmbedding(nn.Cell):
    def __init__(self, dim: int, max_position_embeddings: int = 2048, base: float = 10000.0) -> None:
        ...
        self.inv_freq = 1.0 / (base ** (ops.arange(0, self.dim, 2, dtype=ms.float32) / self.dim))

    def construct(self, x: Tensor, position_ids: Tensor) -> Tensor:
        ...
        cos = ops.cos(emb)
        sin = ops.sin(emb)
        output = ops.stack([cos.to(x.dtype), sin.to(x.dtype)])
        return output
...
class LlamaAttention(nn.Cell):
    ...
    def construct(
        ...
        if attention_mask is not None:
            attn_weights = ops.masked_fill(attn_weights, ~attention_mask, -ms.numpy.inf)
