...
# Adapted from https://github.com/facebookresearch/sam2/blob/main/sam2/modeling/sam2_base.py
...
class SAM2Base(nn.Cell):
    def __init__(
        ...
        use_mask_input_as_output_without_sam=False,  # on frames with mask input, whether to directly output the input
        # mask without using a SAM prompt encoder + mask decoder
        # The maximum number of conditioning frames to participate in the memory attention (-1 means no limit;
        # if there are more conditioning frames than this limit,  we only cross-attend to the temporally
        # closest `max_cond_frames_in_attn` conditioning frames in the encoder when tracking each frame). This gives the
        # model a temporal locality when handling a large number of annotated frames and also avoids GPU OOM.
        ...
        # whether to also use multimask output for tracking (not just for the first click on initial conditioning frames; only
        #  relevant when `multimask_output_in_sam=True`)
    ...
    def construct(self, *args, **kwargs):
        ...
        # For now, it raises an error to indicate it's not meant to be called directly.
    ...
    def _construct_sam_heads(
        ...
        if multimask_output:
            ...
            best_iou_inds = mint.argmax(ious, dim=-1)
            batch_inds = mint.arange(B)
        ...
        if not is_init_cond_frame:
            ...
            if self.use_obj_ptrs_in_encoder:
                ...
                if len(pos_and_ptrs) > 0:
                    ...
                    obj_ptrs = mint.stack(ptrs_list, dim=0)
                    # a temporal positional embedding based on how far each object pointer is from
                    # the current frame (sine embedding normalized by the max pointer num).
                    if self.add_tpos_enc_to_obj_ptrs:
                        t_diff_max = max_obj_ptrs_in_encoder - 1
                        tpos_dim = C if self.proj_tpos_enc_in_obj_ptrs else self.mem_dim
                        obj_pos = ms.Tensor(pos_list, ms.int32)
                    ...
                    if self.mem_dim < C:
                        ...
                        obj_pos = mint.repeat_interleave(obj_pos, C // self.mem_dim, dim=0)
    ...
    def _apply_non_overlapping_constraints(self, pred_masks):
        ...
        max_obj_inds = mint.argmax(pred_masks, dim=0, keepdim=True)
        # "batch_obj_inds": object index of each object slice (along dim 0) in `pred_masks`
        batch_obj_inds = mint.arange(batch_size)[:, None, None, None]
        keep = max_obj_inds == batch_obj_inds
        # suppress overlapping regions' scores below -10.0 so that the foreground regions
        # don't overlap (here sigmoid(-10.0)=4.5398e-05)
        pred_masks = mint.where(keep, pred_masks, mint.clamp(pred_masks, max=-10.0))
        return pred_masks
