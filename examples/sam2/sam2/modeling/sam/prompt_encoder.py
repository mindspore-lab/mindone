...
# Adapted from https://github.com/facebookresearch/sam2/blob/main/sam2/modeling/sam/prompt_encoder.py
...
from mindspore.mint import nn as nn
...
class PromptEncoder(nn.Cell):
    def __init__(
        ...
        Arguments:
          ...
          activation (nn.Cell): The activation to use when encoding
            input masks.
        """
        super().__init__()
    ...
    def _embed_points(
        ...
        point_embedding = mint.where(
            ...
            mint.zeros_like(point_embedding) + self.not_a_point_embed.weight,
            point_embedding,
        )
        point_embedding = mint.where(
    ...
    def construct(
        ...
        if points is not None:
            ...
            sparse_embeddings = mint.cat([sparse_embeddings, point_embeddings], dim=1)
        if boxes is not None:
            box_embeddings = self._embed_boxes(boxes)
            sparse_embeddings = mint.cat([sparse_embeddings, box_embeddings], dim=1)
