...
class SAM2ImagePredictor:
    ...
    def set_image(
        ...
        backbone_out = self.model.construct_image(input_image)
    ...
    def set_image_batch(
        ...
        backbone_out = self.model.construct_image(img_batch)
        _, vision_feats, _, _ = self.model._prepare_backbone_features(backbone_out)
        # Add no_mem_embed, which is added to the lowest rest feat. map during training on videos
        if self.model.directly_add_no_mem_embed:
            vision_feats[-1] = vision_feats[-1] + self.model.no_mem_embed
    ...
    def predict_batch(
        ...
        for img_idx in range(num_images):
            ...
            masks_np = masks.squeeze(0).float().asnumpy()
            iou_predictions_np = iou_predictions.squeeze(0).float().asnumpy()
            low_res_masks_np = low_res_masks.squeeze(0).float().asnumpy()
    ...
    def predict(
        ...
        Arguments:
          ...
          normalize_coords (bool): If true, the point coordinates will be normalized to the range [0,1]\
    ...
    def _prep_prompts(self, point_coords, point_labels, box, mask_logits, normalize_coords, img_idx=-1):
        ...
        if point_coords is not None:
            ...
            point_coords = ms.Tensor(point_coords, dtype=mstype.float32)
            unnorm_coords = self._transforms.transform_coords(
                point_coords, normalize=normalize_coords, orig_hw=self._orig_hw[img_idx]
            )
            labels = ms.Tensor(point_labels, dtype=mstype.int32)
            if len(unnorm_coords.shape) == 2:
                unnorm_coords, labels = unnorm_coords[None, ...], labels[None, ...]
        if box is not None:
            box = ms.Tensor(box, dtype=mstype.float32)
            unnorm_box = self._transforms.transform_boxes(
                box, normalize=normalize_coords, orig_hw=self._orig_hw[img_idx]
            )  # Bx2x2
        if mask_logits is not None:
            mask_input = ms.Tensor(mask_logits, dtype=mstype.float32)
    ...
    def _predict(
        ...
        if boxes is not None:
            ...
            box_labels = ms.tensor([[2, 3]], dtype=ms.int32)
            box_labels = box_labels.repeat(boxes.shape[0], 1)
            # we merge "boxes" and "points" into a single "concat_points" input (where
            # boxes are added at the beginning) to sam_prompt_encoder
            if concat_points is not None:
                concat_coords = mint.cat([box_coords, concat_points[0]], dim=1)
                concat_labels = mint.cat([box_labels, concat_points[1]], dim=1)
        ...
        low_res_masks = mint.clamp(low_res_masks, -32.0, 32.0)
