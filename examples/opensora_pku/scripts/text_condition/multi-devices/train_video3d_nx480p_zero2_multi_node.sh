# Stage 3: 29x480p, two-nodes, 16 NPUs
NUM_FRAME=29

MS_WORKER_NUM=16                      # the total number of works in all nodes
...
msrun --bind_core=True --node_rank=$NODE_RANK --worker_num=$MS_WORKER_NUM --local_worker_num=$LOCAL_WORKER_NUM --master_addr=$MASTER_NODE_ADDRESS --master_port=9000 --log_dir="rank-${NODE_RANK}-t2v-video3d-${NUM_FRAME}x480p_zero2/parallel_logs" \
  opensora/train/train_t2v_diffusers.py \
    ...
    --output_dir="rank-${NODE_RANK}-t2v-video3d-${NUM_FRAME}x480p_zero2" \
