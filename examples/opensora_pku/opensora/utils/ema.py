...
class EMA(EMA_):
    def __init__(
        ...
        self.power = power
        self.cur_decay_value = None  # set in self.ema_update()

    def get_decay(self, updates: int) -> float:
        """
        Compute the decay factor for the exponential moving average.
        """
        step = max(0, updates - self.update_after_step - 1)
        ...
        cur_decay_value = min(cur_decay_value, self.ema_decay)
    ...
    def ema_update(self):
        ...
        success = self.hyper_map(F.partial(_ema_op, self.cur_decay_value), self.ema_weight, self.net_weight)
