...
def vanilla_d_loss(logits_real, logits_fake):
    d_loss = 0.5 * (ops.mean(ops.softplus(-logits_real)) + ops.mean(ops.softplus(logits_fake)))
    return d_loss


def hinge_d_loss_with_exemplar_weights(logits_real, logits_fake, weights):
    assert weights.shape[0] == logits_real.shape[0] == logits_fake.shape[0]
    loss_real = ops.mean(ops.relu(1.0 - logits_real), dim=[1, 2, 3])
    loss_fake = ops.mean(ops.relu(1.0 + logits_fake), dim=[1, 2, 3])
...
def measure_perplexity(predicted_indices, n_embed):
    ...
    encodings = ops.one_hot(predicted_indices, n_embed).float().reshape(-1, n_embed)
    avg_probs = encodings.mean(0)
    perplexity = (-(avg_probs * ops.log(avg_probs + 1e-10)).sum()).exp()
    cluster_use = ops.sum(avg_probs > 0)
...
class LPIPSWithDiscriminator(nn.Cell):
    def __init__(
        ...
        self.logvar = ms.Parameter(ops.ones(shape=[]) * logvar_init)
    ...
    def calculate_adaptive_weight(self, nll_loss, g_loss, last_layer=None):
        ...
        d_weight = ops.clamp(d_weight, 0.0, 1e4).detach()
    ...
    def construct(
        ...
        rec_loss = ops.abs(inputs - reconstructions)
        ...
        weighted_nll_loss = ops.sum(weighted_nll_loss) / weighted_nll_loss.shape[0]
        nll_loss = ops.sum(nll_loss) / nll_loss.shape[0]
