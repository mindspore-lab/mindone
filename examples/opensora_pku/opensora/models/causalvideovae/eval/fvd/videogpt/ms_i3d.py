...
class Unit3D(nn.Cell):
    def __init__(
        ...
        self.conv3d = nn.Conv3d(
            ...
            stride=self._stride,
            padding=0,  # we always want padding to be 0 here. We will dynamically pad based on input size in construct function
    ...
    def construct(self, x):
        ...
        if npu_config is not None and npu_config.on_npu:
            x = npu_config.run_conv3d(self.conv3d, x, x.dtype)
        else:
            x = self.conv3d(x)
...
class InceptionModule(nn.Cell):
    ...
    def construct(self, x):
        ...
        return mint.cat([b0, b1, b2, b3], dim=1)
