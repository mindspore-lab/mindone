...
from mindnlp.transformers.models.bert import BertConfig,BertLMHeadModel
# from header import *

class TextFcLayer(nn.Cell):
    ...
    def init_Qformer(cls, num_query_token, vision_width, num_hidden_layers=2, cross_attention_freq=1):
        ...
        query_tokens = Parameter(
            ops.zeros(1, num_query_token, encoder_config.hidden_size)
        )
    ...
    def __init__(self, in_dim: int, out_dim: int, num_input_tokens: int = 1, num_output_tokens: int = 1,
        ...
        if mode == 'linear':
            self.model = nn.Dense(in_dim, out_dim)
        elif mode == 'transformer':
            hidden_dim = 512
            self.fc = nn.Dense(in_dim, hidden_dim)
            ...
            self.model = nn.Dense(hidden_dim, out_dim)
            self.query_embs = Parameter(ops.randn(1, num_output_tokens, hidden_dim))
        elif mode == 'qformer':
    ...
    def construct(self, x: ms.Tensor, input_embs: ms.Tensor) -> ms.Tensor:
        outputs = None

        if isinstance(self.model, nn.CellList):
            ...
            outputs = ops.stack(outputs, axis=1)  # (N, T_I_V_A.txt, D)
            ...
            x = x + input_embs
            x = self.fc(x)
            image_atts = ops.ones(x.size()[:-1], dtype=ms.int64)
            # print(x.size())
