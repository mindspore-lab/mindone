...
def mask_token(inputs, tokenizer, mlm_probability, vocab_size=None, special_tokens_mask=None):
    ...
    """
    mlm_probability = ms.Tensor(mlm_probability)
    indices_replaced = ops.bernoulli(inputs.shape, p=mlm_probability).bool()
...
def l2_loss(u: ms.Tensor, v: ms.Tensor) -> ms.Tensor:
  """
  Args:
