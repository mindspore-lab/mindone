...
tokenizer = Emu3Tokenizer.from_pretrained(
    "/home/susan/workspace/checkpoints/BAAI/Emu3-Stage1",
...
eov = tokenizer.encode(visual_token_pattern.format(token_id=args.codebook_size - 1))[0]
...
def getitem(path):
    ...
    if task == "img_gen":
        ...
        vt_prompts = chat_template.format(image_prompt=image_prompt, text_prompt=prompt)  # instruction + input vision & text prompts
...
def to_imgstr(image_tokens):
    image_token_str = [
        [visual_token_pattern.format(token_id=token_id) for token_id in token_row]
