...
def betas_to_sigmas(betas):
    return mint.sqrt(1 - ops.cumprod(1 - betas, dim=0))


def sigmas_to_betas(sigmas):
    square_alphas = 1 - sigmas**2
    betas = 1 - mint.cat([square_alphas[:1], square_alphas[1:] / square_alphas[:-1]])
    return betas


def logsnrs_to_sigmas(logsnrs):
    return mint.sqrt(mint.sigmoid(-logsnrs))


def sigmas_to_logsnrs(sigmas):
    square_sigmas = sigmas**2
    return mint.log(square_sigmas / (1 - square_sigmas))
...
def karras_schedule(n, sigma_min=0.002, sigma_max=80.0, rho=7.0):
    ramp = mint.linspace(1, 0, n)
    min_inv_rho = sigma_min ** (1 / rho)
    max_inv_rho = sigma_max ** (1 / rho)
    sigmas = (max_inv_rho + ramp * (min_inv_rho - max_inv_rho)) ** rho
    sigmas = mint.sqrt(sigmas**2 / (1 + sigmas**2))
    return sigmas
