...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="THUDM/CogVideoX1.5-5b-I2V",
    ...
    parser.add_argument("--image_path", type=str, required=True, help="Image path of the input image")
    parser.add_argument(
...
def infer(args: argparse.Namespace) -> None:
    ...
    prompt = (
        ...
        "The panda's face is expressive, showing concentration and joy as it plays. The background includes a small, "
        "flowing stream and vibrant green foliage, enhancing the peaceful and magical atmosphere of this unique musical performance."
    )
    prompt = prompt if args.prompt is None else args.prompt
    image = load_image(image=args.image_path)
    video = pipe(prompt=prompt, image=image, height=args.height, width=args.width, num_frames=args.frame)[0][0]
