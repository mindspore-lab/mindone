...
class TrainStepForCogVideo(nn.Cell):
    ...
    def construct(self, images, videos, text_input_ids_or_prompt_embeds, image_rotary_emb=None):
        ...
        if not self.args.load_tensors:
            with pynative_no_grad():
                images = images.permute(0, 2, 1, 3, 4).to(self.vae_dtype)
        ...
        images = images.to(dtype=self.weight_dtype)
        ...
        noisy_video = self.scheduler.add_noise(videos, noise, timesteps)
        noisy_model_input = ops.cat([noisy_video, images], axis=2)
        ofs_emb = None if self.ofs_embed_dim is None else ops.full((1,), 2.0, dtype=noisy_model_input.dtype)
        ...
        model_pred = self.scheduler.get_velocity(model_output, noisy_video, timesteps)
