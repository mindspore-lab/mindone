...
from mindone.diffusers.pipelines.stable_diffusion_xl import StableDiffusionXLPipelineOutput
from mindone.diffusers.utils import _get_model_file
...
class PhotoMakerStableDiffusionXLPipeline(StableDiffusionXLPipeline):
    def load_photomaker_adapter(
        ...
        **kwargs,
        ...
        id_encoder = id_encoder.to(dtype=self.unet.dtype)
    ...
    def encode_prompt_with_trigger_word(
        ...
        class_tokens_mask: Optional[ms.Tensor] = None,
        nc_flag: bool = False,
    ):
        ...
        if prompt_embeds is None:
            ...
            for prompt, tokenizer, text_encoder in zip(prompts, tokenizers, text_encoders):
                ...
                clean_input_ids = ms.Tensor(clean_input_ids, dtype=ms.int32).unsqueeze(0)
                class_tokens_mask = ms.Tensor(class_tokens_mask, dtype=ms.bool_).unsqueeze(0)
            ...
            prompt_embeds = ops.cat(prompt_embeds_list, axis=-1)

        prompt_embeds = prompt_embeds.to(dtype=self.text_encoder_2.dtype)
        class_tokens_mask = class_tokens_mask  # TODO: ignoring two-prompt case
    ...
    def __call__(
        ...
        class_tokens_mask: Optional[ms.Tensor] = None,
        prompt_embeds_text_only: Optional[ms.Tensor] = None,
        pooled_prompt_embeds_text_only: Optional[ms.Tensor] = None,
        nc_flag=False,
    ):
        r"""
        Function invoked when calling the pipeline for generation.
        Only the parameters introduced by PhotoMaker are discussed here.
        For explanations of the previous parameters in StableDiffusionXLPipeline, please refer to https://github.com/huggingface\
            /diffusers/blob/v0.25.0/src/diffusers/pipelines/stable_diffusion_xl/pipeline_stable_diffusion_xl.py

        Args:
            input_id_images (`PipelineImageInput`, *optional*):
                Input ID Image to work with PhotoMaker.
            class_tokens_mask (`ms.Tensor`, *optional*):
                Pre-generated class token. When the `prompt_embeds` parameter is provided in advance, it is \
                    necessary to prepare the `class_tokens_mask` beforehand for marking out the position of class word.
            prompt_embeds_text_only (`ms.Tensor`, *optional*):
                Pre-generated text embeddings. Can be used to easily tweak text inputs, *e.g.* prompt weighting. If not
                provided, text embeddings will be generated from `prompt` input argument.
            pooled_prompt_embeds_text_only (`ms.Tensor`, *optional*):
        ...
        if prompt_embeds is not None and class_tokens_mask is None:
            raise ValueError(
                "If `prompt_embeds` are provided, `class_tokens_mask` also have to be passed. Make sure \
                    to generate `class_tokens_mask` from the same tokenizer that was used to generate `prompt_embeds`."
            )
        ...
        if isinstance(prompt, list):
            ...
            for prompt in prompt_arr:
                ...
                if not isinstance(input_id_images[0], ms.Tensor):
                    id_pixel_values = self.id_image_processor(input_id_images, return_tensors="np").pixel_values

                id_pixel_values = id_pixel_values.unsqueeze(0).to(dtype=dtype)  # TODO: multiple prompts
                ...
                if not nc_flag:
                    ...
                    prompt_embeds = prompt_embeds.repeat_interleave(num_images_per_prompt, dim=1)
                    prompt_embeds = prompt_embeds.view(bs_embed * num_images_per_prompt, seq_len, -1)
                    pooled_prompt_embeds = pooled_prompt_embeds.repeat_interleave(num_images_per_prompt, dim=1).view(
            ...
            negative_prompt_embeds = ops.cat(negative_prompt_embeds_arr, axis=0)
            print(negative_prompt_embeds.shape)
            if not nc_flag:
                prompt_embeds = ops.cat(prompt_embeds_arr, axis=0)
                print(prompt_embeds.shape)
                pooled_prompt_embeds = ops.cat(pooled_prompt_embeds_arr, axis=0)
                print(pooled_prompt_embeds.shape)

            prompt_embeds_text_only = ops.cat(prompt_embeds_text_only_arr, axis=0)
            print(prompt_embeds_text_only.shape)
            pooled_prompt_embeds_text_only = ops.cat(pooled_prompt_embeds_text_only_arr, axis=0)
            print(pooled_prompt_embeds_text_only.shape)

            negative_pooled_prompt_embeds = ops.cat(negative_pooled_prompt_embeds_arr, axis=0)
            ...
            add_time_ids = add_time_ids.repeat_interleave(batch_size * num_images_per_prompt, dim=0)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i <= start_merge_step or nc_flag:
                    current_prompt_embeds = ops.cat([negative_prompt_embeds, prompt_embeds_text_only], axis=0)
                    add_text_embeds = ops.cat([negative_pooled_prompt_embeds, pooled_prompt_embeds_text_only], axis=0)
                else:
                    current_prompt_embeds = ops.cat([negative_prompt_embeds, prompt_embeds], axis=0)
                    add_text_embeds = ops.cat([negative_pooled_prompt_embeds, pooled_prompt_embeds], axis=0)
                ...
                # print(add_time_ids.shape)
                # zeros_matrix =
                # global_mask1024 = ops.cat([ops.randn(1, 1024, 1, 1) for random_number])
                # global_mask4096 =
