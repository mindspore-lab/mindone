""" If the extracted raw mesh is large in a way, needs to scale down the color fusion part to avoid oom."""
...
def get_opts():
    ...
    parser.add_argument('--z_range', nargs="+", type=float, default=[-1.0, 1.0],
                        help='x range of the object')
    parser.add_argument('--sigma_threshold', type=float, default=30.0,   # higher threshold sparser, as the harder to be opaque
    ...
    parser.add_argument("--resume", 
                        default="PATH/TO/OUTPUTS/ckpt/step10000.ckpt",
                        help="path to config file")
    parser.add_argument("--debug", action="store_true")
    parser.add_argument("--nviews", default=60, type=int)
    parser.add_argument("--fuse_color", action="store_true", help='with it can fuse color from a textureless mesh directly to generate a colored mesh, when you have a low ram device you may try this, but normally the ram bottleneck is not storing the mesh but the resolution of mesh during coloring (N_grid size).')
...
def render_raw_mesh(_ply_path, renderer):
    ...
    PlyData([PlyElement.describe(vertices_[:, 0], 'vertex'), 
             PlyElement.describe(face, 'face')]).write(f'{_ply_path}_raw.ply')
...
if __name__ == "__main__":
    ...
    OmegaConf.update(cfg, "system.guidance_type", "")  # make guidance cfg empty to avoid loading mview-ldm guidance, since it's not relevant in this script
    system: BaseSystem = threestudio.find(cfg.system_type)(
            cfg.system, 
            resumed=cfg.resume is not None)
    system.set_save_dir(output_path)
    system.renderer.cfg.render_to_mesh = True

    state_dict = ms.load_checkpoint(args.resume)

    # rm the 'net.' in the sd key
    ...
    K = np.array([[dataset.source.focal_length, 0, W/2],
                  [0, dataset.source.focal_length, H/2],
    ...
    rendered_img_path = [os.path.join(rendered_img_dir, i) 
                         for i in sorted(os.listdir(rendered_img_dir), 
                                         key=lambda x: int(x.split('.')[0]))][::fraction_slice_120views]  # 30 views if 4 slices
    ...
    PlyData([PlyElement.describe(vertex_all, 'vertex'), 
             PlyElement.describe(face, 'face')]).write(f'{_ply_path}_color.ply')

    print('Color Mesh Extarction Done!')
