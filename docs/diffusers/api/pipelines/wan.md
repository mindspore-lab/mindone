...
!!! note The preprocessing tools are available in the original Wan-Animate repository. Integration of these preprocessing steps into Diffusers is planned for a future release.
...
import numpy as np
import mindspore as ms
from mindone.diffusers import AutoencoderKLWan, WanAnimatePipeline
...
model_id = "Wan-AI/Wan2.2-Animate-14B-Diffusers"
image_encoder = CLIPVisionModel.from_pretrained(model_id, subfolder="image_encoder", mindspore_dtype=ms.float16)
vae = AutoencoderKLWan.from_pretrained(model_id, subfolder="vae", mindspore_dtype=ms.float32)
pipe = WanAnimatePipeline.from_pretrained(
