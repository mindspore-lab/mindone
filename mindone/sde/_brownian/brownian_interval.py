...
def _randn(size, dtype, seed):
    generator = np.Generator().manual_seed(int(seed))
    return ops.randn(size, dtype=dtype, generator=generator)


def _is_scalar(x):
    return isinstance(x, int) or isinstance(x, float) or (isinstance(x, ms.Tensor) and x.numel() == 1)
...
class BrownianInterval(brownian_base.BaseBrownian, _Interval):
    ...
    def __call__(self, ta, tb=None, return_U=False, return_A=False):
        ...
        if ta == tb:
            W = ops.zeros(self._size, dtype=self._dtype)
            H = None
            A = None
            if self._have_H:
                H = ops.zeros(self._size, dtype=self._dtype)
            if self._have_A:
                size = (*self._size, *self._size[-1:])  # not self._size[-1] as that may not exist
                A = ops.zeros(size, dtype=self._dtype)
        else:
