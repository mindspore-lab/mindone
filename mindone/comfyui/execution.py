...
from comfy_execution.graph import DynamicPrompt, ExecutionBlocker, ExecutionList, get_input_info
...
async def get_output_data(
    prompt_id, unique_id, obj, input_data_all, execution_block_cb=None, pre_execute_cb=None, hidden_inputs=None
):
    ...
    try:
        if unique_id in pending_async_nodes:
            ...
            if lazy_status_present:
                ...
                required_inputs = [
                    x for x in required_inputs if isinstance(x, str) and (x not in input_data_all or x in missing_keys)
                ]
        ...
        if len(output_ui) > 0:
            ...
            if server.client_id is not None:
                server.send_sync(
                    "executed",
                    {"node": unique_id, "display_node": display_node_id, "output": output_ui, "prompt_id": prompt_id},
                    server.client_id,
                )
        if has_subgraph:
            cached_outputs = []
            ...
            while not execution_list.is_empty():
                ...
                if error is not None:
                    self.handle_execution_error(
                        prompt_id, dynamic_prompt.original_prompt, current_outputs, executed, error, ex
                    )
                    break
...
async def validate_inputs(prompt_id, prompt, item, validated):
    ...
    if len(validate_function_inputs) > 0 or validate_has_kwargs:
        ...
        ret = await _async_map_node_over_list(
            prompt_id, unique_id, obj_class, input_filtered, validate_function_name, hidden_inputs=hidden_inputs
        )
