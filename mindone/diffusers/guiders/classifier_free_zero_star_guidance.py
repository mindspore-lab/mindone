...
def cfg_zero_star_scale(cond: ms.Tensor, uncond: ms.Tensor, eps: float = 1e-8) -> ms.Tensor:
    cond_dtype = cond.dtype
    cond = cond.float()
    uncond = uncond.float()
    dot_product = mint.sum(cond * uncond, dim=1, keepdim=True)
    squared_norm = mint.sum(uncond**2, dim=1, keepdim=True) + eps
