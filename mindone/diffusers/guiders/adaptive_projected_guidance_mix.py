...
class MomentumBuffer:
    ...
    def update(self, update_value: ms.Tensor):
    ...
    def __repr__(self) -> str:
        ...
        """
        if isinstance(self.running_average, ms.Tensor):
...
def normalized_guidance(
    ...
    if norm_threshold > 0:
        ones = mint.ones_like(diff)
        diff_norm = mint.norm(diff, p=2, dim=dim, keepdim=True)
        scale_factor = mint.minimum(ones, norm_threshold / diff_norm)
        diff = diff * scale_factor

    v0, v1 = diff.double(), pred_cond.double()
    # `mint.nn.functional.normalize`` can't support `double`
    v1 = mint.nn.functional.normalize(v1.float(), dim=dim).double()
