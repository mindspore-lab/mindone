...
def normalized_guidance(
    pred_cond: ms.Tensor, pred_uncond: ms.Tensor, guidance_scale: float, use_original_formulation: bool = False
) -> ms.Tensor:
    cond_dtype = pred_cond.dtype
    preds = mint.stack([pred_cond, pred_uncond], dim=1).float()
    preds = preds.flatten(2)
    U, S, Vh = mint.linalg.svd(preds, full_matrices=False)
    Vh_modified = Vh.clone()
    Vh_modified[:, 1] = 0

    uncond_flat = pred_uncond.reshape(pred_uncond.size(0), 1, -1).float()
    x_Vh = mint.matmul(uncond_flat, Vh.transpose(-2, -1))
    x_Vh_V = mint.matmul(x_Vh, Vh_modified)
