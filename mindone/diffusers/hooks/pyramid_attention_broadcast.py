...
class PyramidAttentionBroadcastHook(ModelHook):
    ...
    def reset_state(self, module: ms.nn.Cell) -> None:
        self.state.reset()
        return module


def apply_pyramid_attention_broadcast(module: ms.nn.Cell, config: PyramidAttentionBroadcastConfig):
    r"""
    ...
    ```python
    >>> import mindspore as ms
    >>> from mindone.diffusers import CogVideoXPipeline, PyramidAttentionBroadcastConfig, apply_pyramid_attention_broadcast
...
def _apply_pyramid_attention_broadcast_hook(
    ...
    r"""
    Apply [Pyramid Attention Broadcast](https://huggingface.co/papers/2408.12588) to a given ms.nn.Cell.
