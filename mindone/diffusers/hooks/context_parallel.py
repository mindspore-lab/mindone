...
class AllGatherFunction(ms.nn.Cell):
    ...
    def construct(self, tensor):
        ...
        return mint.distributed.all_gather_into_tensor(mint.zeros([*tensor.shape[:self.dim], tensor.shape[self.dim] * self.world_size, *tensor.shape[self.dim+1:]], dtype=tensor.dtype), tensor, group=self.group)
...
class EquipartitionSharder:
    ...
    def shard(cls, tensor: ms.Tensor, dim: int, mesh) -> ms.Tensor:
        ...
        assert tensor.shape[dim] % mint.distributed.get_world_size(mesh.get_group()) == 0, (
        ...
        # return tensor.chunk(mint.distributed.get_world_size(mesh.get_group()), dim=dim)[mesh.get_rank()]

        return tensor.chunk(mint.distributed.get_world_size(mesh.get_group()), dim=dim)[mint.distributed.get_rank(mesh.get_group())]

    @classmethod
    def unshard(cls, tensor: ms.Tensor, dim: int, mesh) -> ms.Tensor:
        tensor = tensor.contiguous()
        tensor = AllGatherFunction(dim, mesh.get_group())(tensor)
        return tensor
