...
from ..utils import get_logger, is_peft_version
...
_SHOULD_DISABLE_PEFT_INPUT_AUTOCAST = is_peft_version(">", "0.14.0")
if _SHOULD_DISABLE_PEFT_INPUT_AUTOCAST:
    from mindone.peft.helpers import disable_input_dtype_casting
...
class LayerwiseCastingHook(ModelHook):
    ...
    def __init__(self, storage_dtype: ms.Type, compute_dtype: ms.Type, non_blocking: bool) -> None:
        ...
        - We can, on our end, use something like accelerate's `send_to_device` but for dtypes. This way, we can ensure
          ...
          hoping to achieve:
            ...
            2. Performing inference without losing information from casting to different precisions. With the current
               ...
               with storage_dtype=ms.float8_e4m3fn and compute_dtype=ms.bfloat16, inputs are cast to
               ms.float8_e4m3fn in the lora layer. We will then upcast back to ms.bfloat16 when we continue the
...
def apply_layerwise_casting(
    ...
    ...     transformer,
    ...     storage_dtype=ms.float16, # mindone.diffusers: float8 not supported
    ...
    Args:
        ...
        storage_dtype (`ms.Type`):
            ...
            **Note:** mindone.diffusers currently does not support `ms.float8_e4m3fn` or other float8 dtypes.
            You may use `ms.float16` or `ms.bfloat16` instead.
        compute_dtype (`ms.Type`):
        ...
        skip_modules_classes (`Tuple[Type[ms.nn.Cell], ...]`, defaults to `None`):
...
def apply_layerwise_casting_hook(
    module: ms.nn.Cell, storage_dtype: ms.Type, compute_dtype: ms.Type, non_blocking: bool
) -> None:
    r"""
