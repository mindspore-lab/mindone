...
def _get_identifiable_transformer_blocks_in_module(module: ms.nn.Cell):
    ...
    for name, submodule in module.cells_and_names():
        ...
        is_modulelist = isinstance(submodule, ms.nn.CellList)
...
def _get_identifiable_attention_layers_in_module(module: ms.nn.Cell):
    attention_layers = []
    for name, submodule in module.cells_and_names():
...
def _get_identifiable_feedforward_layers_in_module(module: ms.nn.Cell):
    feedforward_layers = []
    for name, submodule in module.cells_and_names():
