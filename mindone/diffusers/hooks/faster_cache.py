...
from ..models.layers_compat import unflatten
...
class FasterCacheConfig:
    ...
    Attributes:
        ...
        (`Tuple[str, ...]`, defaults to `("blocks.*attn1", "transformer_blocks.*attn1", "single_transformer_blocks.*attn1")`):
        ...
        temporal_attention_block_identifiers (`Tuple[str, ...]`, defaults to `("temporal_transformer_blocks.*attn1",)`):
            ...
            of these identifiers, FasterCache will be applied to that block. This can either be the full layer names,
            partial layer names, or regex patterns. Matching will always be done using a regex match.
        attention_weight_callback (`Callable[[ms.nn.Cell], float]`, defaults to `None`):
        ...
        low_frequency_weight_callback (`Callable[[ms.nn.Cell], float]`, defaults to `None`):
        ...
        high_frequency_weight_callback (`Callable[[ms.nn.Cell], float]`, defaults to `None`):
        ...
        (`List[str]`, defaults to `("hidden_states", "encoder_hidden_states", "timestep", "attention_mask", "encoder_attention_mask")`):
    ...
    attention_weight_callback: Callable[[ms.nn.Cell], float] = None
    low_frequency_weight_callback: Callable[[ms.nn.Cell], float] = None
    high_frequency_weight_callback: Callable[[ms.nn.Cell], float] = None
...
class FasterCacheDenoiserState:
    ...
    def __init__(self) -> None:
        ...
        self.low_frequency_delta: ms.Tensor = None
        self.high_frequency_delta: ms.Tensor = None
...
class FasterCacheBlockState:
    ...
    def __init__(self) -> None:
        ...
        self.cache: Tuple[ms.Tensor, ms.Tensor] = None
...
class FasterCacheDenoiserHook(ModelHook):
    ...
    def __init__(
        ...
        low_frequency_weight_callback: Callable[[ms.nn.Cell], ms.Tensor],
        high_frequency_weight_callback: Callable[[ms.nn.Cell], ms.Tensor],
    ) -> None:
        super().__init__()
    ...
    @staticmethod
    def _get_cond_input(input: ms.Tensor) -> Tuple[ms.Tensor, ms.Tensor]:
    ...
    def new_construct(self, module: ms.nn.Cell, *args, **kwargs) -> Any:
        ...
        if should_skip_uncond:
            ...
            if is_any_kwarg_uncond:
                ...
                args = tuple([self._get_cond_input(arg) if ops.is_tensor(arg) else arg for arg in args])
                kwargs = {
        ...
        if should_skip_uncond:
            ...
            uncond_states = ms.tensor(np.fft.ifftshift(uncond_freq.numpy()))
            uncond_states = ms.tensor(np.fft.ifft2(uncond_states.numpy()).real)

            if self.tensor_format == "BCFHW" or self.tensor_format == "BFCHW":
                uncond_states = unflatten(uncond_states, 0, (batch_size, -1))
                hidden_states = unflatten(hidden_states, 0, (batch_size, -1))
            if self.tensor_format == "BCFHW":
            ...
            hidden_states = mint.cat([uncond_states, hidden_states], dim=0)
        else:
    ...
    def reset_state(self, module: ms.nn.Cell) -> ms.nn.Cell:
        self.state.reset()
        return module
...
class FasterCacheBlockHook(ModelHook):
    ...
    def __init__(
        ...
        is_guidance_distilled: bool,
        weight_callback: Callable[[ms.nn.Cell], float],
        current_timestep_callback: Callable[[], int],
    ) -> None:
        super().__init__()
    ...
    def _compute_approximated_attention_output(
        self, t_2_output: ms.Tensor, t_output: ms.Tensor, weight: float, batch_size: int
    ) -> ms.Tensor:
    ...
    def new_construct(self, module: ms.nn.Cell, *args, **kwargs) -> Any:
        ...
        if should_skip_attention:
            ...
            if ops.is_tensor(self.state.cache[-1]):
...
def apply_faster_cache(module: ms.nn.Cell, config: FasterCacheConfig) -> None:
    r"""
    Applies [FasterCache](https://huggingface.co/papers/2410.19355) to a given pipeline.

    Args:
        module (`ms.nn.Cell`):
            The mindspore module to apply FasterCache to. Typically, this should be a transformer architecture supported
    ...
    if config.low_frequency_weight_callback is None:
        ...
        def low_frequency_weight_callback(module: ms.nn.Cell) -> float:
            is_within_range = (
    ...
    if config.high_frequency_weight_callback is None:
        ...
        def high_frequency_weight_callback(module: ms.nn.Cell) -> float:
            is_within_range = (
...
def _apply_faster_cache_on_denoiser(module: ms.nn.Cell, config: FasterCacheConfig) -> None:
...
def _split_low_high_freq(x):
    fft = ms.tensor(np.fft.fft2(x.numpy()))
    fft_shifted = ms.tensor(np.fft.fftshift(fft.numpy()))
    height, width = x.shape[-2:]
    radius = min(height, width) // 5

    y_grid, x_grid = mint.meshgrid(mint.arange(height), mint.arange(width))
