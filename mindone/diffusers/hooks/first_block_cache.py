...
class FBCSharedBlockState(BaseState):
    def __init__(self) -> None:
        ...
        self.head_block_output: Union[ms.Tensor, Tuple[ms.Tensor, ...]] = None
        self.head_block_residual: ms.Tensor = None
        self.tail_block_residuals: Union[ms.Tensor, Tuple[ms.Tensor, ...]] = None
...
class FBCHeadBlockHook(ModelHook):
    ...
    def _should_compute_remaining_blocks(self, hidden_states_residual: ms.Tensor) -> bool:
...
def apply_first_block_cache(module: ms.nn.Cell, config: FirstBlockCacheConfig) -> None:
    """
    Applies [First Block
    ...
    Args:
        module (`ms.nn.Cell`):
            The mindspore module to apply FBCache to. Typically, this should be a transformer architecture supported in
    ...
    Example:
        ...
        >>> image = pipe(prompt, generator=np.random.default_rng(42))[0][0]
...
def _apply_fbc_head_block_hook(block: ms.nn.Cell, state_manager: StateManager, threshold: float) -> None:
    registry = HookRegistry.check_if_exists_or_initialize(block)
    hook = FBCHeadBlockHook(state_manager, threshold)
    registry.register_hook(hook, _FBC_LEADER_BLOCK_HOOK)


def _apply_fbc_block_hook(block: ms.nn.Cell, state_manager: StateManager, is_tail: bool = False) -> None:
    registry = HookRegistry.check_if_exists_or_initialize(block)
