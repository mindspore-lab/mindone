...
class ModelHook:
    ...
    def initialize_hook(self, module: ms.nn.Cell) -> ms.nn.Cell:
        r"""
    ...
    def deinitalize_hook(self, module: ms.nn.Cell) -> ms.nn.Cell:
        r"""
    ...
    def detach_hook(self, module: ms.nn.Cell) -> ms.nn.Cell:
        r"""
    ...
    def reset_state(self, module: ms.nn.Cell):
        if self._is_stateful:
            raise NotImplementedError("This hook is stateful and needs to implement the `reset_state` method.")
        return module

    def _set_context(self, module: ms.nn.Cell, name: str) -> None:
...
class HookRegistry:
    def __init__(self, module_ref: ms.nn.Cell) -> None:
        super().__init__()
    ...
    def remove_hook(self, name: str, recurse: bool = True) -> None:
        ...
        if recurse:
            for module_name, module in self._module_ref.cells_and_names():
                if module_name == "":
                    continue
    ...
    def reset_stateful_hooks(self, recurse: bool = True) -> None:
        ...
        if recurse:
            for module_name, module in unwrap_module(self._module_ref).cells_and_names():
                if module_name == "":
                    continue
    ...
    @classmethod
    def check_if_exists_or_initialize(cls, module: ms.nn.Cell) -> "HookRegistry":
