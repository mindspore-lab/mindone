...
def _apply_smoothed_energy_guidance_hook(
    module: ms.nn.Cell, config: SmoothedEnergyGuidanceConfig, blur_sigma: float, name: Optional[str] = None
) -> None:
    ...
    for i, block in enumerate(transformer_blocks):
        ...
        for submodule_name, submodule in block.cells_and_names():
            ...
            for identifier in config._query_proj_identifiers:
                ...
                if query_proj is None or not isinstance(query_proj, mint.nn.Linear):
                    continue
                logger.debug(
...
def _gaussian_blur_2d(query: ms.Tensor, kernel_size: int, sigma: float, sigma_threshold_inf: float) -> ms.Tensor:
    """
    ...
    if is_inf:
        ...
        x = mint.linspace(-kernel_size_half, kernel_size_half, steps=kernel_size)
        pdf = mint.exp(-0.5 * (x / sigma).pow(2))
        kernel1d = pdf / pdf.sum()
        kernel1d = kernel1d.to(query)
        kernel2d = mint.matmul(kernel1d[:, None], kernel1d[None, :])
