...
class Kandinsky3Block(nn.Cell):
    def __init__(self, in_channels, out_channels, time_embed_dim, kernel_size=3, norm_groups=32, up_resolution=None):
        ...
        if up_resolution is not None and up_resolution:
            self.up_sample = nn.Conv2dTranspose(in_channels, in_channels, kernel_size=2, stride=2, has_bias=True)
        else:
        ...
        if up_resolution is not None and not up_resolution:
            self.down_sample = nn.Conv2d(out_channels, out_channels, kernel_size=2, stride=2, has_bias=True)
        else:
...
class Kandinsky3ResNetBlock(nn.Cell):
    def __init__(
        ...
        self.shortcut_up_sample = (
            nn.Conv2dTranspose(in_channels, in_channels, kernel_size=2, stride=2, has_bias=True)
        ...
        self.shortcut_down_sample = (
            nn.Conv2d(out_channels, out_channels, kernel_size=2, stride=2, has_bias=True)
