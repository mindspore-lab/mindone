...
class PatchedLoraProjection(nn.Cell):
    ...
    def _fuse_lora(self, lora_scale=1.0, safe_fusing=False):
        ...
        dtype = self.regular_linear_layer.weight.data.dtype
        ...
        fused_weight = w_orig + (lora_scale * mint.bmm(w_up[None, :], w_down[None, :])[0])

        if safe_fusing and mint.isnan(fused_weight).any().item():
            raise ValueError(
        ...
        self.regular_linear_layer.weight.data = fused_weight.to(dtype=dtype)
    ...
    def _unfuse_lora(self):
        ...
        unfused_weight = fused_weight.float() - (self.lora_scale * mint.bmm(w_up[None, :], w_down[None, :])[0])
        self.regular_linear_layer.weight.data = unfused_weight.to(dtype=dtype)
...
class LoRALinearLayer(nn.Cell):
    ...
    Parameters:
        ...
        dtype (`mindspore.Type`, `optional`, defaults to `None`):
    ...
    def __init__(
        ...
        self.down.weight.data.normal_(std=1 / rank)
        self.up.weight.data.zeros_()
...
class LoRACompatibleConv(mint.nn.Conv2d):
    """
    ...
    def _fuse_lora(self, lora_scale: float = 1.0, safe_fusing: bool = False):
        ...
        fusion = mint.mm(w_up.flatten(start_dim=1), w_down.flatten(start_dim=1))
        ...
        self.weight.data = fused_weight.to(dtype=dtype)
    ...
    def _unfuse_lora(self):
        ...
        dtype = fused_weight.data.dtype

        self.w_up = self.w_up.float()
        self.w_down = self.w_down.float()

        fusion = mint.mm(self.w_up.flatten(start_dim=1), self.w_down.flatten(start_dim=1))
        fusion = fusion.reshape((fused_weight.shape))
        unfused_weight = fused_weight.float() - (self._lora_scale * fusion)
        self.weight.data = unfused_weight.to(dtype=dtype)
...
class LoRACompatibleLinear(mint.nn.Linear):
    """
    ...
    def _unfuse_lora(self):
        ...
        unfused_weight = fused_weight.float() - (self._lora_scale * mint.bmm(w_up[None, :], w_down[None, :])[0])
        self.weight.data = unfused_weight.to(dtype=dtype)
    ...
    def construct(self, hidden_states: ms.Tensor, scale: float = 1.0) -> ms.Tensor:
        if self.lora_layer is None:
            out = super().construct(hidden_states)
            return out
        else:
            out = super().construct(hidden_states) + (scale * self.lora_layer(hidden_states))
            return out
