...
if _CAN_USE_FLASH_ATTN:
    raise RuntimeError(
        f"Flash Attention is not usable."
    )
else:
...
if _CAN_USE_FLASH_ATTN_3:
    raise RuntimeError(
        f"Flash Attention 3 is not usable."
    )
else:
...
if _CAN_USE_SAGE_ATTN:
    raise RuntimeError(
        f"Sage Attention is not usable."
    )
else:
    sageattn = None
...
if _CAN_USE_FLEX_ATTN:
    ...
    # compiled function.
    raise RuntimeError(
        f"Flex Attention is not usable."
    )
else:
    flex_attention = None
...
if _CAN_USE_NPU_ATTN:
    raise RuntimeError(
        f"NPU Fusion Attention is not usable."
    )
else:
...
if _CAN_USE_XFORMERS_ATTN:
    raise RuntimeError(
        f"Xformers Attention is not usable."
    )
else:
    xops = None
...
# def _check_device_cuda(query: ms.Tensor, key: ms.Tensor, value: ms.Tensor, **kwargs) -> None:
...
#     def check_device_cuda(query: ms.Tensor, key: ms.Tensor, value: ms.Tensor, **kwargs) -> None:
...
def _check_attention_backend_requirements(backend: AttentionBackendName) -> None:
    if backend in [AttentionBackendName.FLASH, AttentionBackendName.FLASH_VARLEN]:
        if not _CAN_USE_FLASH_ATTN:
            raise RuntimeError(
                f"Flash Attention backend '{backend.value}' is not usable."
            )

    elif backend in [AttentionBackendName._FLASH_3, AttentionBackendName._FLASH_VARLEN_3]:
        if not _CAN_USE_FLASH_ATTN_3:
            raise RuntimeError(
                f"Flash Attention 3 backend '{backend.value}' is not usable."
            )

    # TODO: add support Hub variant of FA3 varlen later
    elif backend in [AttentionBackendName._FLASH_3_HUB]:
        raise RuntimeError(
            f"Flash Attention 3 Hub backend '{backend.value}' is not usable."
        )
...
#     causal: bool = False,
#     qv: Optional[ms.Tensor] = None,
#     q_descale: Optional[ms.Tensor] = None,
#     k_descale: Optional[ms.Tensor] = None,
#     v_descale: Optional[ms.Tensor] = None,
...
#     return mint.empty_like(q), q.new_empty(lse_shape)
...
#     def __init__(self, dropout_p: float = 0.0, is_causal: bool = False, enable_gqa: bool = False):
#         super().__init__()
...
#     def bprop(self, query, key, value, attn_mask, scale, out, dout):
#         query_t, key_t, value_t = (x.permute(0, 2, 1, 3) for x in (query, key, value))
#         dout_t = dout.permute(0, 2, 1, 3)

#         def forward_fn(query_t, key_t, value_t):
#             out = scaled_dot_product_attention(
#                 query=query_t,
#                 key=key_t,
#                 value=value_t,
...
#         grad_query_t, grad_key_t, grad_value_t = self.grad_fn(
#             forward_fn, inputs=[query_t, key_t, value_t], grad_outputs=dout_t, retain_graph=False  
#         )
...
class NativeAttentionCell(nn.Cell):
    ...
    def bprop(self, query, key, value, attn_mask, dropout_p, is_causal, scale, enable_gqa, return_lse, _save_ctx, _parallel_config, out, dout):
        query_t, key_t, value_t = (x.permute(0, 2, 1, 3) for x in (query, key, value))
...
def _all_to_all_single(x: ms.Tensor, group) -> ms.Tensor:
    ...
    x = x.flatten()
    x = mint.distributed.all_to_all_single(x, None, None, group)
    x = x.reshape(shape)
    # x = _wait_tensor(x)
    return x
...
def permute_tensor(
    ...
    mint.distributed.all_to_all_single(output, tensor,
...
class TemplatedRingAttention(nn.Cell):
    ...
    def construct(
        ...
        kv_buffer = mint.distributed.all_gather_tensor(kv_buffer, gather_dim=0, group=ring_mesh.get_group())
    ...
    def bprop(
        ...
        for i in range(world_size):
            ...
            grad_query_op, grad_key_op, grad_value_op, *_ = ms.grad(self.forward_op, grad_position=(0, 1, 2))(
                                                                query,
                                                                key,
                                                                value,
                                                                attn_mask,
            ...
            if i < world_size - 1:
                ...
                next_grad_kv = permute_tensor(grad_kv_buffer, next_ranks, group=ring_mesh.get_group())
...
#     key_packed = mint.cat(key_valid, dim=0)
#     value_packed = mint.cat(value_valid, dim=0)
...
#     key: ms.Tensor,
#     value: ms.Tensor,
#     attn_mask: Optional[Union[ms.Tensor, "flex_attention.BlockMask"]] = None,
#     is_causal: bool = False,
#     scale: Optional[float] = None,
#     enable_gqa: bool = False,
#     return_lse: bool = False,
...
#         )
#     elif ms.is_tensor(attn_mask):
#         if attn_mask.ndim == 2:
#             attn_mask = attn_mask.view(attn_mask.shape[0], 1, attn_mask.shape[1], 1)
