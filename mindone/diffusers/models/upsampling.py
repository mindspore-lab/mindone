...
from ..utils.mindspore_utils import ms_conv_transpose2d
...
class FirUpsample2D(nn.Cell):
    ...
    def _upsample_2d(
        ...
        if self.use_conv:
            ...
            inverse_conv = ms_conv_transpose2d(
                hidden_states,
                weight,
                stride=stride,
...
class KUpsample2D(nn.Cell):
    ...
    def construct(self, inputs: ms.Tensor) -> ms.Tensor:
        ...
        return ms_conv_transpose2d(inputs, weight, stride=2, padding=self.pad * 2 + 1)
...
def upfirdn2d_native(
    ...
    # out = ops.Conv2D(1, kernel_size=(kernel_h, kernel_w))(out, w.to(out.dtype))
    out = out.reshape(
        -1,
        minor,
