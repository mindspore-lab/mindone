...
class EasyAnimateCausalConv3d(mint.nn.Conv3d):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        ...
        if self.prev_features is None:
            # TODO: when mode is "replicate", F.pad does not support bf16
            if hidden_states.dtype == ms.bfloat16:
            ...
            while i + self.temporal_padding + 1 <= num_frames:
                out = super().construct(hidden_states[:, :, i : i + self.temporal_padding + 1])
                i += self.t_stride
                outputs.append(out)
            return mint.concat(outputs, 2)
        else:
            # Concatenate previous features with the input tensor for continuous temporal processing
            if self.t_stride == 2:
                hidden_states = mint.concat(
                    [self.prev_features[:, :, -(self.temporal_padding - 1) :], hidden_states], dim=2
                )
            else:
                hidden_states = mint.concat([self.prev_features, hidden_states], dim=2)
            hidden_states = hidden_states.to(dtype=dtype)
...
class EasyAnimateResidualBlock3D(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        if in_channels != out_channels:
            self.shortcut = mint.nn.Conv3d(in_channels, out_channels, kernel_size=1)
        else:
...
class EasyAnimateDownsampler3D(nn.Cell):
...
class EasyAnimateUpsampler3D(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class EasyAnimateDownBlock3D(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class EasyAnimateUpBlock3d(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class EasyAnimateMidBlock3d(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        num_layers: int = 1,
...
class EasyAnimateEncoder(nn.Cell):
    ...
    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        ...
        if self.spatial_group_norm:
            ...
            hidden_states = hidden_states.reshape((batch_size, -1) + hidden_states.shape[1:]).permute(0, 2, 1, 3, 4)
        else:
            hidden_states = self.conv_norm_out(hidden_states)
