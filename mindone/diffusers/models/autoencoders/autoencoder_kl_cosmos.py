...
class CosmosPatchEmbed3d(nn.Cell):
    ...
    def _dwt(self, hidden_states: ms.tensor, mode: str = "reflect", rescale=False) -> ms.tensor:
        ...
        hidden_states = mint.functional.pad(hidden_states, pad=(max(0, n - 2), n - 1, n - 2, n - 1, n - 2, n - 1), mode=mode).to(
            dtype
        )
...
class CosmosUnpatcher3d(nn.Cell):
    ...
    def _idwt(self, hidden_states: ms.tensor, rescale: bool = False) -> ms.tensor:
        ...
        hidden_states = (
            conv_transpose3d(xh, hh.unsqueeze(3).unsqueeze(4), groups=g, stride=(2, 1, 1)) + hidden_states
        )
