...
class CosmosCausalConv3d(mint.nn.Conv3d):
    ...
    def construct(self, hidden_states: ms.tensor) -> ms.tensor:
        ...
        hidden_states = mint.cat([hidden_states_prev, hidden_states], dim=2)
        hidden_states = mint.functional.pad(hidden_states, (*self.spatial_pad, 0, 0), mode=self.pad_mode, value=0.0)
        return super().construct(hidden_states)


class CosmosCausalGroupNorm(nn.Cell):
    ...
    def construct(self, hidden_states: ms.tensor) -> ms.tensor:
        if self.num_groups == 1:
            ...
            hidden_states = unflatten(hidden_states, 0, (batch_size, -1)).permute(
                0, 2, 1, 3, 4
            )  # [B * T, C, H, W] -> [B, C, T, H, W]
        else:
            hidden_states = self.norm(hidden_states)
...
class CosmosPatchEmbed3d(nn.Cell):
    def __init__(self, patch_size: int = 1, patch_method: str = "haar") -> None:
        ...
        self.register_buffer("_arange", mint.arange(_WAVELETS[patch_method].shape[0]), persistent=False)

    def _dwt(self, hidden_states: ms.tensor, mode: str = "reflect", rescale=False) -> ms.tensor:
        ...
        hidden_states = mint.functional.pad(
            hidden_states, pad=(max(0, n - 2), n - 1, n - 2, n - 1, n - 2, n - 1), mode=mode
        ).to(dtype)
        xl = mint.functional.conv3d(hidden_states, hl.unsqueeze(3).unsqueeze(4), groups=g, stride=(2, 1, 1))
        xh = mint.functional.conv3d(hidden_states, hh.unsqueeze(3).unsqueeze(4), groups=g, stride=(2, 1, 1))

        # Handles spatial axes
        xll = mint.functional.conv3d(xl, hl.unsqueeze(2).unsqueeze(4), groups=g, stride=(1, 2, 1))
        xlh = mint.functional.conv3d(xl, hh.unsqueeze(2).unsqueeze(4), groups=g, stride=(1, 2, 1))
        xhl = mint.functional.conv3d(xh, hl.unsqueeze(2).unsqueeze(4), groups=g, stride=(1, 2, 1))
        xhh = mint.functional.conv3d(xh, hh.unsqueeze(2).unsqueeze(4), groups=g, stride=(1, 2, 1))

        xlll = mint.functional.conv3d(xll, hl.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2))
        xllh = mint.functional.conv3d(xll, hh.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2))
        xlhl = mint.functional.conv3d(xlh, hl.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2))
        xlhh = mint.functional.conv3d(xlh, hh.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2))
        xhll = mint.functional.conv3d(xhl, hl.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2))
        xhlh = mint.functional.conv3d(xhl, hh.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2))
        xhhl = mint.functional.conv3d(xhh, hl.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2))
        xhhh = mint.functional.conv3d(xhh, hh.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2))

        hidden_states = mint.cat([xlll, xllh, xlhl, xlhh, xhll, xhlh, xhhl, xhhh], dim=1)
        if rescale:
            hidden_states = hidden_states / 8**0.5
        return hidden_states

    def _haar(self, hidden_states: ms.tensor) -> ms.tensor:
        xi, xv = mint.split(hidden_states, [1, hidden_states.shape[2] - 1], dim=2)
        hidden_states = mint.cat([xi.repeat_interleave(self.patch_size, dim=2), xv], dim=2)
        for _ in range(int(math.log2(self.patch_size))):
            hidden_states = self._dwt(hidden_states, rescale=True)
        return hidden_states

    def _arrange(self, hidden_states: ms.tensor) -> ms.tensor:
        xi, xv = mint.split(hidden_states, [1, hidden_states.shape[2] - 1], dim=2)
        hidden_states = mint.cat([xi.repeat_interleave(self.patch_size, dim=2), xv], dim=2)
...
class CosmosUnpatcher3d(nn.Cell):
    def __init__(self, patch_size: int = 1, patch_method: str = "haar"):
        ...
        self.register_buffer(
            "_arange",
            mint.arange(_WAVELETS[patch_method].shape[0]),
            persistent=False,
        )

    def _idwt(self, hidden_states: ms.tensor, rescale: bool = False) -> ms.tensor:
        dtype = hidden_states.dtype
        h = self.wavelets
        ...
        xlll, xllh, xlhl, xlhh, xhll, xhlh, xhhl, xhhh = mint.chunk(hidden_states, 8, dim=1)

        # Handle height transposed convolutions
        # FIXME use mint.functional.conv_transpose3d later
        xll = conv_transpose3d(xlll, hl.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2))
        xll = conv_transpose3d(xllh, hh.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2)) + xll

        xlh = conv_transpose3d(xlhl, hl.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2))
        xlh = conv_transpose3d(xlhh, hh.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2)) + xlh

        xhl = conv_transpose3d(xhll, hl.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2))
        xhl = conv_transpose3d(xhlh, hh.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2)) + xhl

        xhh = conv_transpose3d(xhhl, hl.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2))
        xhh = conv_transpose3d(xhhh, hh.unsqueeze(2).unsqueeze(3), groups=g, stride=(1, 1, 2)) + xhh

        # Handles width transposed convolutions
        xl = conv_transpose3d(xll, hl.unsqueeze(2).unsqueeze(4), groups=g, stride=(1, 2, 1))
        xl = conv_transpose3d(xlh, hh.unsqueeze(2).unsqueeze(4), groups=g, stride=(1, 2, 1)) + xl
        xh = conv_transpose3d(xhl, hl.unsqueeze(2).unsqueeze(4), groups=g, stride=(1, 2, 1))
        xh = conv_transpose3d(xhh, hh.unsqueeze(2).unsqueeze(4), groups=g, stride=(1, 2, 1)) + xh

        # Handles time axis transposed convolutions
        hidden_states = conv_transpose3d(xl, hl.unsqueeze(3).unsqueeze(4), groups=g, stride=(2, 1, 1))
        hidden_states = conv_transpose3d(xh, hh.unsqueeze(3).unsqueeze(4), groups=g, stride=(2, 1, 1)) + hidden_states
    ...
    def _ihaar(self, hidden_states: ms.tensor) -> ms.tensor:
        for _ in range(int(math.log2(self.patch_size))):
            hidden_states = self._idwt(hidden_states, rescale=True)
        hidden_states = hidden_states[:, :, self.patch_size - 1 :, ...]
        return hidden_states

    def _irearrange(self, hidden_states: ms.tensor) -> ms.tensor:
        p = self.patch_size
        hidden_states = unflatten(hidden_states, 1, (-1, p, p, p))
...
class CosmosConvProjection3d(nn.Cell):
...
class CosmosResnetBlock3d(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        if in_channels != out_channels:
            ...
            self.conv_shortcut = mint.nn.Identity()
    ...
    def construct(self, hidden_states: ms.tensor) -> ms.tensor:
        ...
        hidden_states = mint.functional.silu(hidden_states)
...
class CosmosDownsample3d(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        ...
        self.conv1 = mint.nn.Identity()
        self.conv2 = mint.nn.Identity()
    ...
    def construct(self, hidden_states: ms.tensor) -> ms.tensor:
        ...
        if self.spatial_downsample:
            pad = (0, 1, 0, 1, 0, 0)
            hidden_states = mint.functional.pad(hidden_states, pad, mode="constant", value=0)
            conv_out = self.conv1(hidden_states)
            pool_out = mint.functional.avg_pool3d(hidden_states, kernel_size=(1, 2, 2), stride=(1, 2, 2))
            hidden_states = conv_out + pool_out

        if self.temporal_downsample:
            hidden_states = mint.cat([hidden_states[:, :, :1, ...], hidden_states], dim=2)
            conv_out = self.conv2(hidden_states)
            pool_out = mint.functional.avg_pool3d(hidden_states, kernel_size=(2, 1, 1), stride=(2, 1, 1))
            hidden_states = conv_out + pool_out
...
class CosmosUpsample3d(nn.Cell):
    def __init__(
        self,
        in_channels: int,
...
class CosmosCausalAttention(nn.Cell):
    def __init__(
        self,
    ...
    def flash_attention_op(
        ...
        if attn_mask is not None:
            ...
            # mask_input for ops.operations.nn_ops.FlashAttentionScore should be (B N1 S1 S2) or (B 1 S1 S2) or (S1 S2)
    ...
    def construct(self, hidden_states: ms.tensor, attention_mask: Optional[ms.tensor] = None) -> ms.tensor:
...
class CosmosSpatialAttentionProcessor2_0:
    def __call__(
        self, attn: CosmosCausalAttention, hidden_states: ms.tensor, attention_mask: Optional[ms.tensor] = None
    ) -> ms.tensor:
        batch_size, num_channels, num_frames, height, width = hidden_states.shape
        residual = hidden_states
...
class CosmosDownBlock3d(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        for _ in range(num_layers):
            ...
            if use_attention:
                ...
                temp_attentions.append(NoneCell())
        ...
        if use_downsample:
            self.downsamplers = nn.CellList([CosmosDownsample3d(out_channel, spatial_downsample, temporal_downsample)])
...
class CosmosMidBlock3d(nn.Cell):
    ...
    def __init__(
        ...
        for _ in range(num_layers):
            ...
            if use_attention:
                ...
                attention.append(NoneCell())
                temp_attentions.append(NoneCell())

        self.resnets = nn.CellList(resnets)
        self.attentions = nn.CellList(attention)
        self.temp_attentions = nn.CellList(temp_attentions)

        self.upsamplers = None
        if use_upsample:
            self.upsamplers = nn.CellList([CosmosUpsample3d(out_channel, spatial_upsample, temporal_upsample)])
