...
class HunyuanImageResnetBlock(nn.Cell):
    r"""
    ...
    def __init__(self, in_channels: int, out_channels: int, non_linearity: str = "silu") -> None:
        ...
        if in_channels != out_channels:
            self.conv_shortcut = mint.nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0)
        else:
...
class HunyuanImageAttentionBlock(nn.Cell):
    r"""
    ...
    def __init__(self, in_channels: int):
        ...
        self.to_q = mint.nn.Conv2d(in_channels, in_channels, 1)
        self.to_k = mint.nn.Conv2d(in_channels, in_channels, 1)
        self.to_v = mint.nn.Conv2d(in_channels, in_channels, 1)
        self.proj = mint.nn.Conv2d(in_channels, in_channels, 1)
...
class HunyuanImageDownsample(nn.Cell):
    """
    ...
    def __init__(self, in_channels: int, out_channels: int):
        ...
        self.conv = mint.nn.Conv2d(in_channels, out_channels // factor, kernel_size=3, stride=1, padding=1)
...
class HunyuanImageUpsample(nn.Cell):
    """
...
class HunyuanImageMidBlock(nn.Cell):
    """
...
class HunyuanImageEncoder2D(nn.Cell):
    r"""
    ...
    def __init__(
        ...
        if block_out_channels[-1] % (2 * z_channels) != 0:
            raise ValueError(
                f"block_out_channels[-1 has to be divisible by 2 * out_channels, you have block_out_channels = {block_out_channels[-1]} and out_channels = {z_channels}"  # noqa
            )
        ...
        # init block
        self.conv_in = mint.nn.Conv2d(in_channels, block_out_channels[0], kernel_size=3, stride=1, padding=1)
        ...
        self.conv_out = mint.nn.Conv2d(block_out_channels[-1], 2 * z_channels, kernel_size=3, stride=1, padding=1)
...
class HunyuanImageDecoder2D(nn.Cell):
    r"""
    ...
    def __init__(
        ...
        self.conv_in = mint.nn.Conv2d(z_channels, block_out_channels[0], kernel_size=3, stride=1, padding=1)
        ...
        # Output layers
        self.norm_out = mint.nn.GroupNorm(num_groups=32, num_channels=block_out_channels[-1], eps=1e-6, affine=True)
        self.conv_out = mint.nn.Conv2d(block_out_channels[-1], out_channels, kernel_size=3, stride=1, padding=1)
...
class AutoencoderKLHunyuanImage(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def tiled_encode(self, x: ms.Tensor) -> ms.Tensor:
        ...
        Args:
            x (`ms.Tensor`): Input tensor of shape (B, C, T, H, W).
    ...
    def tiled_decode(self, z: ms.Tensor, return_dict: bool = False) -> Union[DecoderOutput, ms.Tensor]:
        ...
        Args:
            z (`ms.Tensor`): Latent tensor of shape (B, C, H, W).
            return_dict (`bool`, *optional*, defaults to `True`):
