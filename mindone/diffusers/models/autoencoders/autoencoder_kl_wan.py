    ...
    def __init__(self, dim: int, mode: str, upsample_out_dim: int = None) -> None:
        ...
        if mode == "upsample2d":
            self.resample = nn.SequentialCell(
                ...
                mint.nn.Conv2d(dim, upsample_out_dim, 3, padding=1),
            )
...
class WanResidualDownBlock(nn.Cell):
...
class WanResidualUpBlock(nn.Cell):
    """
...
class WanUpBlock(nn.Cell):
    ...
    def construct(self, x, feat_cache=None, feat_idx=[0], first_chunk=None):
        """
        Forward pass through the upsampling block.
...
class AutoencoderKLWan(ModelMixin, AutoencoderMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def tiled_decode(self, z: ms.Tensor, return_dict: bool = True) -> Union[DecoderOutput, ms.Tensor]:
        ...
        dec = mint.clamp(dec, min=-1.0, max=1.0)
