...
class DeviceMesh:
    def __init__(self, device_type, mesh_shape, mesh_dim_names):
        ...
        self.mesh = [
            [r + i*dim1 for r in range(dim1)]
            for i in range(dim0)
        ]
        self.groups = {}

        self.groups[mesh_dim_names[0]] = [mint.distributed.new_group(ranks=row) for row in self.mesh]
        self.groups[mesh_dim_names[1]] = [
            mint.distributed.new_group(ranks=[self.mesh[r][c] for r in range(dim0)])
            for c in range(dim1)
        ]
    ...
    def _flatten(self):
        ...
        return [mint.distributed.new_group(ranks=[r]) for r in flat_ranks]
...
def scaled_dot_product_attention(
    ...
    backend: Optional[str] = "flash_attention",
):
    ...
    if (attn_mask is not None and attn_mask.dtype != ms.bool_ and 1.0 in attn_mask) or head_dim > 512 or backend == "math" or enable_gqa:
