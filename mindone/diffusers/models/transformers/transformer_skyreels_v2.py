...
class SkyReelsV2ImageEmbedding(ms.nn.Cell):
...
class SkyReelsV2Timesteps(nn.Cell):
    def __init__(self, num_channels: int, flip_sin_to_cos: bool, output_type: str = "ms"):
        super().__init__()
...
class SkyReelsV2TimeTextImageEmbedding(nn.Cell):
    def __init__(
        self,
        dim: int,
        time_freq_dim: int,
...
class SkyReelsV2RotaryPosEmbed(nn.Cell):
    def __init__(
        ...
        for dim in [t_dim, h_dim, w_dim]:
            freq = get_1d_rotary_pos_embed(
                dim, max_seq_len, theta, use_real=False, repeat_interleave_real=False, freqs_dtype=ms.float32
            )
            freqs.append(freq)
...
class SkyReelsV2TransformerBlock(nn.Cell):
    def __init__(
        self,
        dim: int,
        ffn_dim: int,
    ...
    def construct(
        ...
        shift_msa, scale_msa, gate_msa, c_shift_msa, c_scale_msa, c_gate_msa = None, None, None, None, None, None
        if temb.dim() == 3:
...
class SkyReelsV2Transformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        self.config_num_frame_per_block = self.config.num_frame_per_block
        self.config_inject_sample_info = self.config.inject_sample_info
    ...
    def construct(
        ...
        causal_mask = None
        if self.config_num_frame_per_block > 1:
            block_num = post_patch_num_frames // self.config_num_frame_per_block
            range_tensor = mint.arange(block_num).repeat_interleave(self.config_num_frame_per_block)
        ...
        if self.config_inject_sample_info:
