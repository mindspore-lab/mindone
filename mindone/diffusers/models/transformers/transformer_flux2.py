...
class Flux2FeedForward(nn.Cell):
    def __init__(
        self,
        dim: int,
        ...
        self.linear_in = mint.nn.Linear(dim, inner_dim * 2, bias=bias)
        self.act_fn = Flux2SwiGLU()
        self.linear_out = mint.nn.Linear(inner_dim, dim_out, bias=bias)
...
class Flux2Attention(ms.nn.Cell, AttentionModuleMixin):
...
class Flux2ParallelSelfAttnProcessor:
    ...
    def __call__(
        ...
        qkv, mlp_hidden_states = mint.split(
        ...
        hidden_states = mint.cat([hidden_states, mlp_hidden_states], dim=-1)
        hidden_states = attn.to_out(hidden_states)

        return hidden_states


class Flux2ParallelSelfAttention(ms.nn.Cell, AttentionModuleMixin):
    """
    ...
    def __init__(
        ...
        self.to_qkv_mlp_proj = mint.nn.Linear(
        ...
        self.to_out = mint.nn.Linear(self.inner_dim + self.mlp_hidden_dim, self.out_dim, bias=out_bias)
...
class Flux2SingleTransformerBlock(nn.Cell):
    def __init__(
        self,
        dim: int,
        ...
        self.norm = mint.nn.LayerNorm(dim, elementwise_affine=False, eps=eps)
    ...
    def construct(
        ...
        temb_mod_params: Tuple[ms.Tensor, ms.Tensor, ms.Tensor],
...
class Flux2TransformerBlock(nn.Cell):
    def __init__(
        self,
        dim: int,
        num_attention_heads: int,
        ...
        self.norm1 = mint.nn.LayerNorm(dim, elementwise_affine=False, eps=eps)
        self.norm1_context = mint.nn.LayerNorm(dim, elementwise_affine=False, eps=eps)
        ...
        self.norm2 = mint.nn.LayerNorm(dim, elementwise_affine=False, eps=eps)
        self.ff = Flux2FeedForward(dim=dim, dim_out=dim, mult=mlp_ratio, bias=bias)

        self.norm2_context = mint.nn.LayerNorm(dim, elementwise_affine=False, eps=eps)
    ...
    def construct(
        ...
        temb_mod_params_img: Tuple[Tuple[ms.Tensor, ms.Tensor, ms.Tensor], ...],
        temb_mod_params_txt: Tuple[Tuple[ms.Tensor, ms.Tensor, ms.Tensor], ...],
        image_rotary_emb: Optional[Tuple[ms.Tensor, ms.Tensor]] = None,
        joint_attention_kwargs: Optional[Dict[str, Any]] = None,
    ) -> Tuple[ms.Tensor, ms.Tensor]:
        joint_attention_kwargs = joint_attention_kwargs or {}
...
class Flux2TimestepGuidanceEmbeddings(nn.Cell):
    ...
    def construct(self, timestep: ms.Tensor, guidance: ms.Tensor) -> ms.Tensor:
...
class Flux2Modulation(nn.Cell):
    def __init__(self, dim: int, mod_param_sets: int = 2, bias: bool = False):
        super().__init__()
        self.mod_param_sets = mod_param_sets

        self.linear = mint.nn.Linear(dim, dim * 3 * self.mod_param_sets, bias=bias)
        self.act_fn = mint.nn.SiLU()

    def construct(self, temb: ms.Tensor) -> Tuple[Tuple[ms.Tensor, ms.Tensor, ms.Tensor], ...]:
        mod = self.act_fn(temb)
        mod = self.linear(mod)

        if mod.ndim == 2:
            mod = mod.unsqueeze(1)
        mod_params = mint.chunk(mod, 3 * self.mod_param_sets, dim=-1)
...
class Flux2Transformer2DModel(
    ...
    def __init__(
        ...
        # 4. Input projections
        self.x_embedder = mint.nn.Linear(in_channels, self.inner_dim, bias=False)
        self.context_embedder = mint.nn.Linear(joint_attention_dim, self.inner_dim, bias=False)
    ...
    def construct(
        ...
        concat_rotary_emb = (
            mint.cat([text_rotary_emb[0], image_rotary_emb[0]], dim=0),
            mint.cat([text_rotary_emb[1], image_rotary_emb[1]], dim=0),
        )
