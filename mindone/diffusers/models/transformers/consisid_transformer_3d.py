...
class PerceiverAttention(nn.Cell):
    def __init__(self, dim: int, dim_head: int = 64, heads: int = 8, kv_dim: Optional[int] = None):
        ...
        self.norm1 = mint.nn.LayerNorm(dim if kv_dim is None else kv_dim)
        self.norm2 = mint.nn.LayerNorm(dim)

        self.to_q = mint.nn.Linear(dim, inner_dim, bias=False)
        self.to_kv = mint.nn.Linear(dim if kv_dim is None else kv_dim, inner_dim * 2, bias=False)
        self.to_out = mint.nn.Linear(inner_dim, dim, bias=False)

    def construct(self, image_embeds: ms.Tensor, latents: ms.Tensor) -> ms.Tensor:
        # Apply normalization
        ...
        kv_input = mint.cat((image_embeds, latents), dim=-2)
        key, value = self.to_kv(kv_input).chunk(2, dim=-1)

        # Reshape the tensors for multi-head attention
        query = query.reshape(query.shape[0], -1, self.heads, self.dim_head).swapaxes(1, 2)
        key = key.reshape(key.shape[0], -1, self.heads, self.dim_head).swapaxes(1, 2)
        value = value.reshape(value.shape[0], -1, self.heads, self.dim_head).swapaxes(1, 2)

        # attention
        scale = 1 / math.sqrt(math.sqrt(self.dim_head))
        weight = (query * scale) @ (key * scale).swapaxes(-2, -1)  # More stable with f16 than dividing afterwards
...
class LocalFacialExtractor(nn.Cell):
    def __init__(
        self,
        id_dim: int = 1280,
        vit_dim: int = 1024,
        ...
        self.latents = ms.Parameter(mint.randn(1, num_queries, vit_dim) * scale)
        # Projection layer to map the latent output to the desired dimension
        self.proj_out = ms.Parameter(scale * mint.randn(vit_dim, output_dim))
            ...
            layers.append(
                nn.CellList(
                    [
                        ...
                        nn.SequentialCell(
                            mint.nn.LayerNorm(vit_dim),
                            mint.nn.Linear(vit_dim, vit_dim * ff_mult, bias=False),
                            mint.nn.GELU(),
                            mint.nn.Linear(vit_dim * ff_mult, vit_dim, bias=False),
        ...
        for i in range(num_scale):
            setattr(
                ...
                f"mapping_{i}",
                nn.SequentialCell(
                    mint.nn.Linear(vit_dim, vit_dim),
                    mint.nn.LayerNorm(vit_dim),
                    nn.LeakyReLU(1e-2),  # Explicitly specify 1e-2 to maintain alignment with the torch
                    mint.nn.Linear(vit_dim, vit_dim),
                    mint.nn.LayerNorm(vit_dim),
                    nn.LeakyReLU(1e-2),  # Explicitly specify 1e-2 to maintain alignment with the torch
                    mint.nn.Linear(vit_dim, vit_dim),
        ...
        self.id_embedding_mapping = nn.SequentialCell(
            mint.nn.Linear(id_dim, vit_dim),
            mint.nn.LayerNorm(vit_dim),
            nn.LeakyReLU(1e-2),  # Explicitly specify 1e-2 to maintain alignment with the torch
            mint.nn.Linear(vit_dim, vit_dim),
            mint.nn.LayerNorm(vit_dim),
            nn.LeakyReLU(1e-2),  # Explicitly specify 1e-2 to maintain alignment with the torch
            mint.nn.Linear(vit_dim, vit_dim * num_id_token),
        )

    def construct(self, id_embeds: ms.Tensor, vit_hidden_states: List[ms.Tensor]) -> ms.Tensor:
        # Repeat latent queries for the batch size
        latents = self.latents.tile((id_embeds.shape[0], 1, 1))
...
class PerceiverCrossAttention(nn.Cell):
    ...
    def construct(self, image_embeds: ms.Tensor, hidden_states: ms.Tensor) -> ms.Tensor:
        ...
        scale = 1 / math.sqrt(math.sqrt(self.dim_head))
        weight = (query * scale) @ (key * scale).swapaxes(-2, -1)  # More stable scaling than post-division
        weight = mint.softmax(weight.float(), dim=-1).to(weight.dtype)
