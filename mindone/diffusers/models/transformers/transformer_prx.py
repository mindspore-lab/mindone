...
from mindspore.mint.nn.functional import fold, unfold
...
def get_image_ids(batch_size: int, height: int, width: int, patch_size: int) -> ms.Tensor:
    r"""
    ...
    img_ids = mint.zeros((height // patch_size, width // patch_size, 2))
    img_ids[..., 0] = mint.arange(height // patch_size)[:, None]
    img_ids[..., 1] = mint.arange(width // patch_size)[None, :]
...
class PRXAttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        k = mint.cat((txt_k, img_k), dim=2)  # [B, H, L_txt + L_img, D]
        v = mint.cat((txt_v, img_v), dim=2)  # [B, H, L_txt + L_img, D]
        ...
        if attention_mask is not None:
            ...
            ones_img = mint.ones((bs, l_img), dtype=ms.bool)
            attention_mask = attention_mask.to(dtype=ms.bool)
            joint_mask = mint.cat([attention_mask, ones_img], dim=-1)
        ...
        query = img_q.swapaxes(1, 2)  # [B, L_img, H, D]
        key = k.swapaxes(1, 2)  # [B, L_txt + L_img, H, D]
        value = v.swapaxes(1, 2)  # [B, L_txt + L_img, H, D]
...
class PRXAttention(nn.Cell, AttentionModuleMixin):
    r"""
    ...
    def __init__(
        ...
        self.img_qkv_proj = mint.nn.Linear(query_dim, query_dim * 3, bias=bias)
        ...
        self.txt_kv_proj = mint.nn.Linear(query_dim, query_dim * 2, bias=bias)
        self.norm_added_k = RMSNorm(self.head_dim, eps=eps, elementwise_affine=True)

        self.to_out = nn.CellList([])
        self.to_out.append(mint.nn.Linear(self.inner_dim, query_dim, bias=out_bias))
        self.to_out.append(mint.nn.Dropout(0.0))
...
class PRXEmbedND(nn.Cell):
    ...
    def rope(self, pos: ms.Tensor, dim: int, theta: int) -> ms.Tensor:
        assert dim % 2 == 0
...
class Modulation(nn.Cell):
    ...
    Returns:
        ((`ms.Tensor`, `ms.Tensor`, `ms.Tensor`), (`ms.Tensor`, `ms.Tensor`, `ms.Tensor`)):
            Two tuples `(shift, scale, gate)`.
    """

    def __init__(self, dim: int):
        super().__init__()
        self.lin = mint.nn.Linear(dim, 6 * dim, bias=True)
        self.lin.weight.set_data(initializer(Constant(0), self.lin.weight.shape, self.lin.weight.dtype))
        self.lin.bias.set_data(initializer(Constant(0), self.lin.bias.shape, self.lin.bias.dtype))

    def construct(
        self, vec: ms.Tensor
    ) -> Tuple[Tuple[ms.Tensor, ms.Tensor, ms.Tensor], Tuple[ms.Tensor, ms.Tensor, ms.Tensor]]:
        out = self.lin(mint.nn.functional.silu(vec))[:, None, :].chunk(6, dim=-1)
        return tuple(out[:3]), tuple(out[3:])


class PRXBlock(nn.Cell):
    r"""
    ...
    Attributes:
        img_pre_norm (`mint.nn.LayerNorm`):
        ...
        post_attention_layernorm (`mint.nn.LayerNorm`):
            Normalization applied after attention.
        gate_proj / up_proj / down_proj (`mint.nn.Linear`):
            Feedforward layers forming the gated MLP.
        mlp_act (`mint.nn.GELU`):
    ...
    def __init__(
        ...
        self.img_pre_norm = mint.nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        ...
        # mlp
        self.post_attention_layernorm = mint.nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.gate_proj = mint.nn.Linear(hidden_size, self.mlp_hidden_dim, bias=False)
        self.up_proj = mint.nn.Linear(hidden_size, self.mlp_hidden_dim, bias=False)
        self.down_proj = mint.nn.Linear(self.mlp_hidden_dim, hidden_size, bias=False)
...
class FinalLayer(nn.Cell):
    ...
    def construct(self, x: ms.Tensor, vec: ms.Tensor) -> ms.Tensor:
...
def img2seq(img: ms.Tensor, patch_size: int) -> ms.Tensor:
    r"""
    ...
    """
    return unfold(img, kernel_size=patch_size, stride=patch_size).swapaxes(1, 2)


def seq2img(seq: ms.Tensor, patch_size: int, shape: ms.Tensor) -> ms.Tensor:
    r"""
    ...
    if isinstance(shape, tuple):
        shape = shape[-2:]
    elif isinstance(shape, ms.Tensor):
        shape = (int(shape[0]), int(shape[1]))
    else:
        raise NotImplementedError(f"shape type {type(shape)} not supported")
    return fold(seq.swapaxes(1, 2), shape, kernel_size=patch_size, stride=patch_size)
...
class PRXTransformer2DModel(ModelMixin, ConfigMixin, AttentionMixin):
    ...
    Returns:
        `Transformer2DModelOutput` if `return_dict=True` (default), otherwise a tuple containing:
            - `sample` (`ms.Tensor`): Reconstructed image of shape `(B, C, H, W)`.
    """
    ...
    def __init__(
        ...
        self.img_in = mint.nn.Linear(self.in_channels * self.patch_size**2, self.hidden_size, bias=True)
        self.time_in = MLPEmbedder(in_dim=256, hidden_dim=self.hidden_size)
        self.txt_in = mint.nn.Linear(context_in_dim, self.hidden_size)
    ...
    def _compute_timestep_embedding(self, timestep: ms.Tensor, dtype: ms.Type) -> ms.Tensor:
        return self.time_in(
    ...
    def construct(
        ...
        Returns:
            `Transformer2DModelOutput` if `return_dict=True`, otherwise a tuple:
                ...
                - `sample` (`ms.Tensor`): Output latent image of shape `(B, C, H, W)`.
        """
        ...
        img_ids = get_image_ids(bs, h, w, patch_size=self.patch_size)
