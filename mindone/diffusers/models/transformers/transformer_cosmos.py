...
class CosmosPatchEmbed(nn.Cell):
    def __init__(
        ...
        self.proj = mint.nn.Linear(in_channels * patch_size[0] * patch_size[1] * patch_size[2], out_channels, bias=bias)
...
class CosmosTimestepEmbedding(nn.Cell):
    def __init__(self, in_features: int, out_features: int) -> None:
        super().__init__()
        self.linear_1 = mint.nn.Linear(in_features, out_features, bias=False)
        self.activation = mint.nn.SiLU()
        self.linear_2 = mint.nn.Linear(out_features, 3 * out_features, bias=False)
...
class CosmosEmbedding(nn.Cell):
    ...
    def construct(self, hidden_states: ms.tensor, timestep: ms.tensor) -> ms.tensor:
...
class CosmosAdaLayerNorm(nn.Cell):
    def __init__(self, in_features: int, hidden_features: int) -> None:
        super().__init__()
        self.embedding_dim = in_features

        self.activation = mint.nn.SiLU()
        self.norm = mint.nn.LayerNorm(in_features, elementwise_affine=False, eps=1e-6)
        self.linear_1 = mint.nn.Linear(in_features, hidden_features, bias=False)
        self.linear_2 = mint.nn.Linear(hidden_features, 2 * in_features, bias=False)

    def construct(
        self, hidden_states: ms.tensor, embedded_timestep: ms.tensor, temb: Optional[ms.tensor] = None
    ) -> ms.tensor:
...
class CosmosAdaLayerNormZero(nn.Cell):
    def __init__(self, in_features: int, hidden_features: Optional[int] = None) -> None:
        ...
        if hidden_features is None:
            self.linear_1 = mint.nn.Identity()
        else:
            self.linear_1 = mint.nn.Linear(in_features, hidden_features, bias=False)

        self.linear_2 = mint.nn.Linear(hidden_features, 3 * in_features, bias=False)

    def construct(
        self,
        hidden_states: ms.tensor,
        embedded_timestep: ms.tensor,
...
class CosmosTransformerBlock(nn.Cell):
    def __init__(
        self,
    ...
    def construct(
        ...
        temb: Optional[ms.tensor] = None,
        image_rotary_emb: Optional[ms.tensor] = None,
        extra_pos_emb: Optional[ms.tensor] = None,
        attention_mask: Optional[ms.tensor] = None,
    ) -> ms.tensor:
...
class CosmosRotaryPosEmbed(nn.Cell):
    def __init__(
        self,
        hidden_size: int,
    ...
    def construct(self, hidden_states: ms.tensor, fps: Optional[int] = None) -> Tuple[ms.tensor, ms.tensor]:
        ...
        emb_h = mint.outer(seq[: pe_size[1]], h_spatial_freqs)[None, :, None, :].repeat(pe_size[0], 1, pe_size[2], 1)
        emb_w = mint.outer(seq[: pe_size[2]], w_spatial_freqs)[None, None, :, :].repeat(pe_size[0], pe_size[1], 1, 1)

        # Apply sequence scaling in temporal dimension
        if fps is None:
            # Images
            emb_t = mint.outer(seq[: pe_size[0]], temporal_freqs)
        else:
            # Videos
            emb_t = mint.outer(seq[: pe_size[0]] / fps * self.base_fps, temporal_freqs)

        emb_t = emb_t[:, None, None, :].repeat(1, pe_size[1], pe_size[2], 1)
        freqs = mint.cat([emb_t, emb_h, emb_w] * 2, dim=-1).flatten(0, 2).float()
        cos = mint.cos(freqs)
        sin = mint.sin(freqs)
        return cos, sin


class CosmosLearnablePositionalEmbed(nn.Cell):
    def __init__(
        self,
        hidden_size: int,
    ...
    def construct(self, hidden_states: ms.tensor) -> ms.tensor:
        ...
        norm = mint.linalg.vector_norm(emb, dim=-1, keepdim=True, dtype=ms.float32)
        norm = mint.add(self.eps, norm, alpha=np.sqrt(norm.numel() / emb.numel()))
