...
from ..embeddings import CombinedTimestepTextProjEmbeddings, TimestepEmbedding, Timesteps, get_1d_rotary_pos_embed
...
class HunyuanImageAttnProcessor:
    ...
    def __call__(
        ...
        query = unflatten(query, 2, (attn.heads, -1))  # batch_size, seq_len, heads, head_dim
        ...
        if image_rotary_emb is not None:
            if attn.add_q_proj is None and encoder_hidden_states is not None:
                ...
                key = mint.cat(
                    [
                        self.apply_rotary_emb(
                            key[:, : -encoder_hidden_states.shape[1]], image_rotary_emb, sequence_dim=1
                        ),
        ...
        if attn.add_q_proj is not None and encoder_hidden_states is not None:
            ...
            query = mint.cat([query, encoder_query], dim=1)
            key = mint.cat([key, encoder_key], dim=1)
            value = mint.cat([value, encoder_value], dim=1)
        ...
        # # ms.jit may raise warnings for flatten; reshape is safer here.
...
class HunyuanImagePatchEmbed(nn.Cell):
    def __init__(
        self,
        ...
        if len(patch_size) == 2:
            self.proj = mint.nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
...
class HunyuanImageByT5TextProjection(nn.Cell):
    def __init__(self, in_features: int, hidden_size: int, out_features: int):
        super().__init__()
        self.norm = mint.nn.LayerNorm(in_features)
        self.linear_1 = mint.nn.Linear(in_features, hidden_size)
        self.linear_2 = mint.nn.Linear(hidden_size, hidden_size)
        self.linear_3 = mint.nn.Linear(hidden_size, out_features)
...
class HunyuanImageAdaNorm(nn.Cell):
...
class HunyuanImageCombinedTimeGuidanceEmbedding(nn.Cell):
    def __init__(
        self,
        embedding_dim: int,
    ...
    def construct(
        self,
        timestep: ms.Tensor,
        timestep_r: Optional[ms.Tensor] = None,
...
class HunyuanImageIndividualTokenRefinerBlock(nn.Cell):
    def __init__(
        self,
...
class HunyuanImageIndividualTokenRefiner(nn.Cell):
    def __init__(
        self,
...
# txt_in
class HunyuanImageTokenRefiner(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        num_attention_heads: int,
...
class HunyuanImageRotaryPosEmbed(nn.Cell):
    def __init__(
    ...
    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        ...
        grid = mint.meshgrid(*axes_grids, indexing="ij")  # dim x [H, W]
        grid = mint.stack(grid, dim=0)  # [2, H, W]
...
class HunyuanImageSingleTransformerBlock(nn.Cell):
    def __init__(
        self,
        num_attention_heads: int,
        attention_head_dim: int,
...
class HunyuanImageTransformerBlock(nn.Cell):
    def __init__(
        self,
        num_attention_heads: int,
        attention_head_dim: int,
...
class HunyuanImageTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin):
    ...
    def construct(
        ...
        timestep_r: Optional[ms.Tensor] = None,
        encoder_hidden_states_2: Optional[ms.Tensor] = None,
        encoder_attention_mask_2: Optional[ms.Tensor] = None,
        guidance: Optional[ms.Tensor] = None,
        ...
        if self.context_embedder_2 is not None and encoder_hidden_states_2 is not None:
            ...
            encoder_hidden_states = mint.stack(new_encoder_hidden_states)
            encoder_attention_mask = mint.stack(new_encoder_attention_mask)

        attention_mask = mint.nn.functional.pad(encoder_attention_mask, (hidden_states.shape[1], 0), value=True)
        ...
        # For 5D: [0, 4, 1, 5, 2, 6, 3, 7]
        # -> batch, channels, post_patch_frame, patch_size_frame, post_patch_height, patch_size_height, post_patch_width, patch_size_width
