...
class EasyAnimateLayerNormZero(nn.Cell):
    def __init__(
        self,
        ...
        if norm_type == "layer_norm":
            self.norm = mint.nn.LayerNorm(embedding_dim, elementwise_affine=elementwise_affine, eps=eps)
...
class EasyAnimateRotaryPosEmbed(nn.Cell):
    ...
    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        bs, c, num_frames, grid_height, grid_width = hidden_states.shape
        ...
        image_rotary_emb = get_3d_rotary_pos_embed(
            ...
            temporal_size=hidden_states.shape[2],
            use_real=True,
        )
...
class EasyAnimateAttnProcessor2_0:
    ...
    def __call__(
        ...
        if attn.add_q_proj is not None and encoder_hidden_states is not None:
            ...
            # encoder_query = encoder_query.unflatten(2, (attn.heads, -1)).swapaxes(1, 2)
            encoder_query = encoder_query.reshape(
                encoder_query.shape[:2] + (attn.heads, -1) + encoder_query.shape[3:]
            ).swapaxes(1, 2)
            # encoder_key = encoder_key.unflatten(2, (attn.heads, -1)).swapaxes(1, 2)
            encoder_key = encoder_key.reshape(
                encoder_key.shape[:2] + (attn.heads, -1) + encoder_key.shape[3:]
            ).swapaxes(1, 2)
            # encoder_value = encoder_value.unflatten(2, (attn.heads, -1)).swapaxes(1, 2)
            encoder_value = encoder_value.reshape(
                encoder_value.shape[:2] + (attn.heads, -1) + encoder_value.shape[3:]
            ).swapaxes(1, 2)
        ...
        if image_rotary_emb is not None:
            query[:, :, encoder_hidden_states.shape[1] :] = self.apply_rotary_emb(
                query[:, :, encoder_hidden_states.shape[1] :], image_rotary_emb
            )
            if not attn.is_cross_attention:
                key[:, :, encoder_hidden_states.shape[1] :] = self.apply_rotary_emb(
...
class EasyAnimateTransformerBlock(nn.Cell):
    def __init__(
        self,
        dim: int,
