...
class FluxIPAdapterMixin:
    ...
    def load_ip_adapter(
        ...
        image_encoder_dtype: mindspore.Type = mindspore.float16,
        **kwargs,
    ):
        """
        Parameters:
        ...
        for pretrained_model_name_or_path_or_dict, weight_name, subfolder in zip(
                ...
                if weight_name.endswith(".safetensors"):
                    ...
                    for key, value in load_file(model_file).items():
                        if any(key.startswith(prefix) for prefix in image_proj_keys):
                            ...
                            state_dict["image_proj"][diffusers_name] = value
                            ...
                            state_dict["ip_adapter"][diffusers_name] = value
                else:
                    raise NotImplementedError(
                        f"Only supports deserialization of weights file in safetensors format, but got {model_file}"
                    )
            ...
            if hasattr(self, "image_encoder") and getattr(self, "image_encoder", None) is None:
                if image_encoder_pretrained_model_name_or_path is not None:
                    ...
                    logger.warning(
                        "image_encoder is not loaded since `image_encoder_folder=None` passed. You will not be able to use `ip_adapter_image` when calling the pipeline with IP-Adapter."  # noqa: E501
                        "Use `ip_adapter_image_embeds` to pass pre-generated image embedding instead."
                    )
...
class SD3IPAdapterMixin:
    ...
    def load_ip_adapter(
        self,
        pretrained_model_name_or_path_or_dict: Union[str, Dict[str, mindspore.Tensor]],
