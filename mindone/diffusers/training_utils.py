...
def multinomial(input, num_samples, replacement=True, **kwargs):
    assert isinstance(input, ms.Tensor) and input.ndim in (1, 2), \
        f"argument input should be a MindSpore Tensor with 1 or 2 dim."
    assert replacement or num_samples <= input.shape[-1], \
        f"cannot sample n_sample > prob_dist.size(-1) samples without replacement."
