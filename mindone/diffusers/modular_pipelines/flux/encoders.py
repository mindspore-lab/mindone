...
from ...utils import is_ftfy_available, logging, scale_lora_layers, unscale_lora_layers
...
class FluxVaeEncoderStep(ModularPipelineBlocks):
    ...
    @staticmethod
    # Copied from diffusers.pipelines.stable_diffusion_3.pipeline_stable_diffusion_3_inpaint.
    # StableDiffusion3InpaintPipeline._encode_vae_image with self.vae->vae
    def _encode_vae_image(vae, image: ms.Tensor, generator: np.random.Generator):
        if isinstance(generator, list):
            ...
            image_latents = retrieve_latents(vae, vae.encode(image)[0], generator=generator)
    ...
    def __call__(self, components: FluxModularPipeline, state: PipelineState) -> PipelineState:
        ...
        block_state.image = block_state.image.to(dtype=block_state.dtype)
...
class FluxTextEncoderStep(ModularPipelineBlocks):
    ...
    def _get_t5_prompt_embeds(
        ...
        untruncated_ids = ms.tensor(components.tokenizer_2(prompt, padding="longest", return_tensors="np").input_ids)
        ...
        prompt_embeds = components.text_encoder_2(text_input_ids, output_hidden_states=False)[0]
    ...
    def _get_clip_prompt_embeds(
        ...
        untruncated_ids = components.tokenizer(prompt, padding="longest", return_tensors="np").input_ids
        if untruncated_ids.shape[-1] >= text_input_ids.shape[-1] and not np.array_equal(
            text_input_ids, untruncated_ids
        ):
        ...
        prompt_embeds = components.text_encoder(ms.tensor(text_input_ids), output_hidden_states=False)

        # Use pooled output of CLIPTextModel
        prompt_embeds = prompt_embeds.pooler_output
        prompt_embeds = prompt_embeds.to(dtype=components.text_encoder.dtype)
    ...
    def encode_prompt(
        ...
        if components.text_encoder is not None:
            if isinstance(components, FluxLoraLoaderMixin):
        ...
        dtype = components.text_encoder.dtype if components.text_encoder is not None else ms.bfloat16
