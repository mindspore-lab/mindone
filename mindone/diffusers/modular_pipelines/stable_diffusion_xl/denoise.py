...
class StableDiffusionXLInpaintLoopBeforeDenoiser(ModularPipelineBlocks):
    ...
    def inputs(self) -> List[str]:
        return [
            ...
            InputParam(
                ...
                type_hint=Optional[ms.Tensor],
                description="The masked image latents to use for the denoising process, for inpainting task only. Can be generated in vae_encode or prepare_latent step.",  # noqa
            ),
        ]
    ...
    def __call__(self, components: StableDiffusionXLModularPipeline, block_state: BlockState, i: int, t: int):
        ...
        if components.num_channels_unet == 9:
            block_state.scaled_latents = mint.cat(
    ...
    def inputs(self) -> List[Tuple[str, Any]]:
        return [
            ...
            InputParam(
                "timestep_cond",
                type_hint=Optional[ms.Tensor],
                description="The guidance scale embedding to use for Latent Consistency Models(LCMs). Can be generated in prepare_additional_conditioning step.",  # noqa
            ),
            InputParam(
                ...
                description=(
                    ...
                    "please add `kwargs_type=denoiser_input_fields` to their parameter spec (`OutputParam`) when they are created and added to the pipeline state"  # noqa
                ),
            ),
        ]
        ...
        for guider_state_batch in guider_state:
            ...
            if block_state.guess_mode and not components.guider.is_conditional:
                ...
                if block_state.down_block_res_samples_zeros is None:
                    block_state.down_block_res_samples_zeros = [mint.zeros_like(d) for d in down_block_res_samples]
                if block_state.mid_block_res_sample_zeros is None:
                    block_state.mid_block_res_sample_zeros = mint.zeros_like(mid_block_res_sample)
    ...
    def inputs(self) -> List[Tuple[str, Any]]:
        return [
            ...
            InputParam(
                "image_latents",
                type_hint=Optional[ms.Tensor],
                description="The image latents to use for the denoising process, for inpainting/image-to-image task only. Can be generated in vae_encode or prepare_latent step.",  # noqa
            ),
        ]
        ...
        if components.num_channels_unet == 4:
            ...
            if i < len(block_state.timesteps) - 1:
                ...
                block_state.init_latents_proper = components.scheduler.add_noise(
                    block_state.init_latents_proper, block_state.noise, ms.Tensor([block_state.noise_timestep])
                )
