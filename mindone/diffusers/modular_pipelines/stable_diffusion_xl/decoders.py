...
class StableDiffusionXLDecodeStep(ModularPipelineBlocks):
    ...
    def intermediate_outputs(self) -> List[str]:
        return [
            OutputParam(
                ...
                description="The generated images, can be a PIL.Image.Image, ms.Tensor or a numpy array",
            )
        ]
    ...
    def __call__(self, components, state: PipelineState) -> PipelineState:
        ...
        if not block_state.output_type == "latent":
            ...
            block_state.needs_upcasting = components.vae.dtype == ms.float16 and components.vae.config.force_upcast
            ...
            if block_state.has_latents_mean and block_state.has_latents_std:
                block_state.latents_mean = (
                    ms.tensor(components.vae.config.latents_mean).view(1, 4, 1, 1).to(latents.dtype)
                )
                block_state.latents_std = (
                    ms.tensor(components.vae.config.latents_std).view(1, 4, 1, 1).to(latents.dtype)
                )
                latents = (
            ...
            if block_state.needs_upcasting:
                components.vae.to(dtype=ms.float16)
        else:
