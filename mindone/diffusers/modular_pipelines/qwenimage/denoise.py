...
class QwenImageEditLoopBeforeDenoiser(ModularPipelineBlocks):
    ...
    def __call__(self, components: QwenImageModularPipeline, block_state: BlockState, i: int, t: ms.Tensor):
        ...
        block_state.latent_model_input = ms.mint.cat([block_state.latents, block_state.image_latents], dim=1)
        block_state.timestep = t.expand(block_state.latents.shape[0]).to(block_state.latents.dtype)
        return components, block_state
...
class QwenImageLoopDenoiser(ModularPipelineBlocks):
    ...
    def __call__(self, components: QwenImageModularPipeline, block_state: BlockState, i: int, t: ms.Tensor):
        ...
        pred_cond_norm = ms.mint.norm(guider_output.pred_cond, dim=-1, keepdim=True)
        pred_norm = ms.mint.norm(guider_output.pred, dim=-1, keepdim=True)
...
class QwenImageEditLoopDenoiser(ModularPipelineBlocks):
    ...
    def __call__(self, components: QwenImageModularPipeline, block_state: BlockState, i: int, t: ms.Tensor):
        ...
        pred_norm = ms.mint.norm(pred, dim=-1, keepdim=True)
...
class QwenImageLoopAfterDenoiser(ModularPipelineBlocks):
    ...
    def intermediate_outputs(self) -> List[OutputParam]:
        return [
            OutputParam("latents", type_hint=ms.Tensor, description="The denoised latents."),
        ]
    ...
    def __call__(self, components: QwenImageModularPipeline, block_state: BlockState, i: int, t: ms.Tensor):
        ...
        if i < len(block_state.timesteps) - 1:
            ...
            block_state.init_latents_proper = components.scheduler.scale_noise(
                block_state.init_latents_proper, ms.Tensor([block_state.noise_timestep]), block_state.initial_noise
            )
