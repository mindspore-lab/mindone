...
def _extract_masked_hidden(hidden_states: ms.Tensor, mask: ms.Tensor):
    bool_mask = mask.bool()
...
def get_qwen_prompt_embeds(
    ...
    txt_tokens = tokenizer(
        txt, max_length=tokenizer_max_length + drop_idx, padding=True, truncation=True, return_tensors="np"
    )
...
def get_qwen_prompt_embeds_edit(
    ...
    split_hidden_states = _extract_masked_hidden(hidden_states, ms.tensor(model_inputs.attention_mask))
    split_hidden_states = [e[drop_idx:] for e in split_hidden_states]
    attn_mask_list = [mint.ones(e.shape[0], dtype=ms.int64) for e in split_hidden_states]
    max_seq_len = max([e.shape[0] for e in split_hidden_states])
    prompt_embeds = mint.stack(
...
class QwenImageEditResizeDynamicStep(ModularPipelineBlocks):
    ...
    def inputs(self) -> List[InputParam]:
        return [
            InputParam(
                name=self._image_input_name, required=True, type_hint=ms.Tensor, description="The image to resize"
            ),
        ]
...
class QwenImageEditPlusResizeDynamicStep(QwenImageEditResizeDynamicStep):
    ...
    def __call__(self, components: QwenImageModularPipeline, state: PipelineState):
        ...
        if not isinstance(images, ms.Tensor) and isinstance(images, PIL.Image.Image) and not isinstance(images, list):
            images = [images]

        # TODO (sayakpaul): revisit this when the inputs are `ms.Tensor`s
        condition_images = []
        vae_images = []
        for img in images:
        ...
        return [
            ConfigSpec(
                ...
                default=(
                    ...
                    " explain how the user's text instruction should alter or modify the image. Generate a new image that meets the user's"
                    " requirements while maintaining consistency with the original input where appropriate.<|im_end|>\n<|im_start|>user\n{}"
                    "<|im_end|>\n<|im_start|>assistant\n",
                ),
            ),
...
class QwenImageInpaintProcessImagesInputStep(ModularPipelineBlocks):
    ...
    def description(self) -> str:
        return (
            "Image Preprocess step for inpainting task. This processes the image and mask inputs together. Images can be "
            "resized first using QwenImageEditResizeDynamicStep."
        )
...
class QwenImageEditPlusProcessImagesInputStep(QwenImageProcessImagesInputStep):
    ...
    def description(self) -> str:
        return (
            "Image Preprocess step for QwenImage Edit Plus. Unlike QwenImage Edit, QwenImage Edit Plus doesn't use "
            "the same resized image for further preprocessing."
        )
