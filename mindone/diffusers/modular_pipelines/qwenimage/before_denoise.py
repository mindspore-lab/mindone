...
class QwenImageCreateMaskLatentsStep(ModularPipelineBlocks):
    ...
    def intermediate_outputs(self) -> List[OutputParam]:
        return [
            OutputParam(name="mask", type_hint=ms.Tensor, description="The mask to use for the inpainting process."),
        ]
    ...
    def __call__(self, components: QwenImageModularPipeline, state: PipelineState) -> PipelineState:
        ...
        block_state.mask = ms.mint.nn.functional.interpolate(
        ...
        block_state.mask = block_state.mask.to(dtype=block_state.dtype)
...
class QwenImageSetTimestepsStep(ModularPipelineBlocks):
    ...
    def intermediate_outputs(self) -> List[OutputParam]:
        return [
            OutputParam(
                name="timesteps", type_hint=ms.Tensor, description="The timesteps to use for the denoising process"
            ),
        ]
