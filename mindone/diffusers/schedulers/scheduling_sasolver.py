    ...
    def stochastic_adams_bashforth_update(
        ...
        **kwargs,
        ...
        if self.predict_x0:
            if (
                order == 2
            ):  # if order = 2 we do a modification that does not influence the convergence order similar to unipc.
                # Note: This is used only for few steps sampling.
                # The added term is O(h^3). Empirically we find it will slightly improve the image quality.
                # ODE case
                # gradient_coefficients[0] += (
                #     1.0 * ops.exp(lambda_t) *
                #     (h ** 2 / 2 - (h - 1 + ops.exp(-h))) /
        ...
        **kwargs,
        ...
        if self.predict_x0:
            if (
                ...
                # The added term is O(h^3). Empirically we find it will slightly improve the image quality.
                # ODE case
                # gradient_coefficients[0] += 1.0 * ops.exp(lambda_t) * (h / 2 - (h - 1 + ops.exp(-h)) / h)
