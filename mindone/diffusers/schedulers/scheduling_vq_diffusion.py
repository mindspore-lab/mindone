...
def index_to_log_onehot(x: ms.Tensor, num_classes: int) -> ms.Tensor:
    """
    Convert batch of vector of class indices into batch of log onehot vectors

    Args:
        x (`ms.Tensor` of shape `(batch size, vector length)`):
...
def gumbel_noised(logits: ms.Tensor, generator: Optional[np.random.Generator]) -> ms.Tensor:
    """
