...
class FlowMatchLCMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def set_timesteps(
        ...
        if self.config.invert_sigmas:
            sigmas = 1.0 - sigmas
            timesteps = sigmas * self.config.num_train_timesteps
            sigmas = mint.cat([sigmas, mint.ones((1,))])
        else:
            sigmas = mint.cat([sigmas, mint.zeros((1,))])
    ...
    def step(
        ...
        if self._scale_factors and self._upscale_mode:
            if self._step_index < len(self._scale_factors):
                ...
                x0_pred = mint.nn.functional.interpolate(x0_pred, size=size, mode=self._upscale_mode)

        noise = randn_tensor(x0_pred.shape, generator=generator, dtype=x0_pred.dtype)
