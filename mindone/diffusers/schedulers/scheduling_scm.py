...
class SCMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        if parameterization == "trigflow":
            pred_x0 = mint.cos(s) * sample - mint.sin(s) * model_output
        else:
        ...
        if len(self.timesteps) > 1:
            noise = randn_tensor(model_output.shape, generator=generator) * self.config.sigma_data
            prev_sample = mint.cos(t) * pred_x0 + mint.sin(t) * noise
        else:
            prev_sample = pred_x0
