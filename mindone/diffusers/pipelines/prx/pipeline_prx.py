...
import ftfy
import numpy as np
from transformers import AutoTokenizer, GemmaTokenizerFast, T5TokenizerFast

import mindspore as ms
from mindspore import mint

from mindone.diffusers.image_processor import PixArtImageProcessor
from mindone.diffusers.loaders import FromSingleFileMixin, LoraLoaderMixin, TextualInversionLoaderMixin
from mindone.diffusers.models import AutoencoderDC, AutoencoderKL
from mindone.diffusers.models.transformers.transformer_prx import PRXTransformer2DModel
from mindone.diffusers.pipelines.pipeline_utils import DiffusionPipeline
from mindone.diffusers.pipelines.prx.pipeline_output import PRXPipelineOutput
...
from mindone.transformers.models.t5gemma.modeling_t5gemma import T5GemmaEncoder
...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import mindspore as ms
        >>> from mindone.diffusers import PRXPipeline
        ...
        >>> image = pipe(prompt, num_inference_steps=28, guidance_scale=5.0)[0][0]
...
class PRXPipeline(
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            ) = self._encode_prompt_standard(prompt, do_classifier_free_guidance, negative_prompt)
    ...
    def _tokenize_prompts(self, prompts: List[str]):
        ...
        return ms.tensor(tokens["input_ids"]), ms.tensor(tokens["attention_mask"]).bool()
    ...
    def _encode_prompt_standard(
        ...
        input_ids, attention_mask = self._tokenize_prompts(prompts_to_encode)
    ...
    def __call__(
        ...
        if self.vae is None and output_type not in ["latent", "ms"]:
            raise ValueError(
        ...
        if self.do_classifier_free_guidance:
            ca_embed = mint.cat([uncond_text_embeddings, text_embeddings], dim=0)
            ca_mask = None
            if cross_attn_mask is not None and uncond_cross_attn_mask is not None:
                ca_mask = mint.cat([uncond_cross_attn_mask, cross_attn_mask], dim=0)
        else:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    latents_in = mint.cat([latents, latents], dim=0)
                    # Normalize timestep for the transformer
                    t_cont = (t.float() / self.scheduler.config.num_train_timesteps).view(1).repeat(2)
                else:
                    latents_in = latents
                    # Normalize timestep for the transformer
                    t_cont = (t.float() / self.scheduler.config.num_train_timesteps).view(1)
        ...
        # 8. Post-processing
        if output_type == "latent" or (output_type == "ms" and self.vae is None):
            image = latents
        else:
