    ...
    def process_multi_modal_prompt(self, text, input_images):
        ...
        for i in range(len(prompt_chunks)):
            ...
            if i != len(prompt_chunks) - 1:
                ...
                size = input_images[i].shape[-2] * input_images[i].shape[-1] // 16 // 16
...
class OmniGenCollator:
    ...
    def create_mask(self, attention_mask, num_tokens_for_output_images):
        ...
        for mask in attention_mask:
            ...
            image_mask = mint.zeros(size=(temp_l + 1, img_length))
            temp_mask = mint.cat([temp_mask, image_mask], dim=-1)

            image_mask = mint.ones(size=(img_length, temp_l + img_length + 1))
            temp_mask = mint.cat([temp_mask, image_mask], dim=0)

            if pad_l > 0:
                pad_mask = mint.zeros(size=(temp_l + 1 + img_length, pad_l))
                temp_mask = mint.cat([pad_mask, temp_mask], dim=-1)

                pad_mask = mint.ones(size=(pad_l, seq_len))
                temp_mask = mint.cat([pad_mask, temp_mask], dim=0)

            true_img_length = num_tokens_for_output_images[inx]
            pad_img_length = img_length - true_img_length
            if pad_img_length > 0:
                temp_mask[:, -pad_img_length:] = 0
                temp_padding_imgs = mint.zeros(size=(1, pad_img_length, self.hidden_size))
            else:
            ...
            inx += 1
        return mint.cat(extended_mask, dim=0), padding_images
