...
from ..models.unets.unet_motion_model import CrossAttnDownBlockMotion, DownBlockMotion, UpBlockMotion
        ...
        for motion_module in block.motion_modules:
            ...
            for i in range(num_transformer_blocks):
                if isinstance(motion_module.transformer_blocks[i], FreeNoiseTransformerBlock):
                    ...
                    basic_transfomer_block_state_dict = {k: v for k, v in basic_transfomer_block.parameters_and_names()}
                    ms.load_param_into_net(
                        motion_module.transformer_blocks[i], basic_transfomer_block_state_dict, strict_load=True
                    )
        ...
        for motion_module in block.motion_modules:
            ...
            for i in range(num_transformer_blocks):
                if isinstance(motion_module.transformer_blocks[i], FreeNoiseTransformerBlock):
                    ...
                    free_noise_transfomer_block_state_dict = {
                        k: v for k, v in free_noise_transfomer_block.parameters_and_names()
                    }
                    ms.load_param_into_net(
                        motion_module.transformer_blocks[i], free_noise_transfomer_block_state_dict, strict_load=True
                    )
    ...
    def _prepare_latents_free_noise(
        ...
        if latents is None:
            ...
            if latents.shape[2] == num_frames:
                ...
                raise ValueError(
                    f"You have passed `latents` as a parameter to FreeNoise. The expected number of frames is either {num_frames} or {self._free_noise_context_length}, but found {latents.shape[2]}"
                )
        ...
        if self._free_noise_noise_type == "shuffle_context":
            for i in range(self._free_noise_context_length, num_frames, self._free_noise_context_stride):
                ...
                indices = ms.tensor(list(range(window_start, window_end)))
                shuffled_indices = indices[ops.randperm(window_length)]
