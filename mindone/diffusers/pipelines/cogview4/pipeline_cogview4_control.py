...
EXAMPLE_DOC_STRING = """
    Examples:
        ```python
        >>> import mindspore as ms
        >>> from mindone.diffusers import CogView4ControlPipeline

        >>> pipe = CogView4ControlPipeline.from_pretrained("THUDM/CogView4-6B-Control", mindspore_dtype=ms.bfloat16)
        >>> control_image = load_image(
        ...     "https://huggingface.co/datasets/hf-internal-testing/diffusers-images/resolve/main/sd_controlnet/bird_canny.png"
        ... )
        >>> prompt = "A bird in space"
        >>> image = pipe(prompt, control_image=control_image, height=1024, width=1024, guidance_scale=3.5)[0][0]
...
def retrieve_timesteps(
    ...
    **kwargs,
    ...
    if timesteps is not None and sigmas is not None:
        ...
        scheduler.set_timesteps(timesteps=timesteps, sigmas=sigmas, **kwargs)
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = mint.cat([latents, control_image.to(latents.dtype)], dim=1).to(transformer_dtype)
