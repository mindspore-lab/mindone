...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> image = pipe(prompt, sag_scale=0.75)[0][0]
        ```
"""
...
class CrossAttnStoreProcessor:
    ...
    def __call__(
        ...
        hidden_states = mint.bmm(self.attention_probs, value)
...
class StableDiffusionSAGPipeline(DiffusionPipeline, StableDiffusionMixin, TextualInversionLoaderMixin, IPAdapterMixin):
    ...
    def __call__(
        ...
        if timesteps.dtype not in [ms.int16, ms.int32, ms.int64]:
            raise ValueError(
                f"{self.__class__.__name__} does not support using a scheduler of type {self.scheduler.__class__.__name__}. Please make sure to use one of 'DDIMScheduler, PNDMScheduler, DDPMScheduler, DEISMultistepScheduler, UniPCMultistepScheduler, DPMSolverMultistepScheduler, DPMSolverSinglestepScheduler'."  # noqa: E501
            )
        ...
        def get_map_size(cell, inputs, output):
            nonlocal map_size
    ...
    def sag_masking(self, original_latents, attn_map, map_size, t, eps):
        ...
        attn_mask = mint.nn.functional.interpolate(attn_mask, (latent_h, latent_w))
...
def gaussian_blur_2d(img, kernel_size, sigma):
    ...
    x = mint.linspace(-ksize_half, ksize_half, steps=kernel_size)

    pdf = mint.exp(-0.5 * (x / sigma).pow(2))

    x_kernel = pdf / pdf.sum()
    x_kernel = x_kernel.to(dtype=img.dtype)

    kernel2d = mint.mm(x_kernel[:, None], x_kernel[None, :])
    kernel2d = kernel2d.broadcast_to((img.shape[-3], 1, kernel2d.shape[0], kernel2d.shape[1]))

    padding = [kernel_size // 2, kernel_size // 2, kernel_size // 2, kernel_size // 2]

    img = mint.nn.functional.pad(img, padding, mode="reflect")
    img = mint.nn.functional.conv2d(img, kernel2d, groups=img.shape[-3])
