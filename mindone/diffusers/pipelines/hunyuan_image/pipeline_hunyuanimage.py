...
from mindone.transformers import Qwen2_5_VLForConditionalGeneration, T5EncoderModel
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        ...     "hunyuanvideo-community/HunyuanImage-2.1-Diffusers", mindspore_dtype=ms.bfloat16
        ... )
...
class HunyuanImagePipeline(DiffusionPipeline):
    ...
    def _get_qwen_prompt_embeds(
        ...
        txt_tokens = tokenizer(
            txt, max_length=tokenizer_max_length + drop_idx, padding="max_length", truncation=True, return_tensors="np"
        )
        ...
        encoder_attention_mask = ms.tensor(txt_tokens.attention_mask[:, drop_idx:])
    ...
    def _get_byt5_prompt_embeds(
        ...
        prompt_embeds = text_encoder(
            input_ids=ms.tensor(txt_tokens.input_ids),
            attention_mask=ms.tensor(txt_tokens.attention_mask).float(),
        )[0]

        prompt_embeds = prompt_embeds.to(dtype=dtype)
        encoder_attention_mask = ms.tensor(txt_tokens.attention_mask)
    ...
    def encode_prompt(
        ...
        prompt_embeds_mask_2: Optional[ms.Tensor] = None,
    ):
        r"""
        ...
        Args:
            ...
            prompt_embeds (`ms.Tensor`, *optional*):
                ...
                argument.
            prompt_embeds_mask (`ms.Tensor`, *optional*):
            ...
            prompt_embeds_mask_2 (`ms.Tensor`, *optional*):
        ...
        if prompt_embeds_2 is None:
            ...
            for glyph_text in glyph_texts:
                if glyph_text is None:
                    glyph_text_embeds = mint.zeros((1, self.tokenizer_2_max_length, self.text_encoder_2.config.d_model))
                    glyph_text_embeds_mask = mint.zeros((1, self.tokenizer_2_max_length), dtype=ms.int64)
                else:
            ...
            prompt_embeds_2 = mint.cat(prompt_embeds_2_list, dim=0)
            prompt_embeds_mask_2 = mint.cat(prompt_embeds_mask_2_list, dim=0)
    ...
    def check_inputs(
        ...
        if height % (self.vae_scale_factor * 2) != 0 or width % (self.vae_scale_factor * 2) != 0:
            logger.warning(
                f"`height` and `width` have to be divisible by {self.vae_scale_factor * 2} but are {height} and {width}. Dimensions will be resized accordingly"  # noqa
            )
        ...
        if prompt_embeds is not None and prompt_embeds_mask is None:
            raise ValueError(
                "If `prompt_embeds` are provided, `prompt_embeds_mask` also have to be passed. Make sure to generate `prompt_embeds_mask` from the same text encoder that was used to generate `prompt_embeds`."  # noqa
            )
        if negative_prompt_embeds is not None and negative_prompt_embeds_mask is None:
            raise ValueError(
                "If `negative_prompt_embeds` are provided, `negative_prompt_embeds_mask` also have to be passed. Make sure to generate `negative_prompt_embeds_mask` from the same text encoder that was used to generate `negative_prompt_embeds`."  # noqa
            )
        ...
        if prompt_embeds_2 is not None and prompt_embeds_mask_2 is None:
            raise ValueError(
                "If `prompt_embeds_2` are provided, `prompt_embeds_mask_2` also have to be passed. Make sure to generate `prompt_embeds_mask_2` from the same text encoder that was used to generate `prompt_embeds_2`."  # noqa
            )
        if negative_prompt_embeds_2 is not None and negative_prompt_embeds_mask_2 is None:
            raise ValueError(
                "If `negative_prompt_embeds_2` are provided, `negative_prompt_embeds_mask_2` also have to be passed. Make sure to generate `negative_prompt_embeds_mask_2` from the same text encoder that was used to generate `negative_prompt_embeds_2`."  # noqa
            )
    ...
    def __call__(
        ...
        negative_prompt_embeds_mask_2: Optional[ms.Tensor] = None,
        ...
        Args:
            ...
            negative_prompt_embeds (`ms.Tensor`, *optional*):
                ...
                argument.
            negative_prompt_embeds_mask (`ms.Tensor`, *optional*):
            ...
            negative_prompt_embeds_2 (`ms.Tensor`, *optional*):
                ...
                input argument.
            negative_prompt_embeds_mask_2 (`ms.Tensor`, *optional*):
        ...
        # select guider
        if not mint.all(prompt_embeds_2 == 0) and self.ocr_guider is not None:
        ...
        if self.transformer.config.guidance_embeds:
            guidance = ms.tensor([distilled_guidance_scale] * latents.shape[0], dtype=self.transformer.dtype) * 1000.0
