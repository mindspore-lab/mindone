...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import mindspore as ms
        >>> from mindone.diffusers import HunyuanImageRefinerPipeline

        >>> pipe = HunyuanImageRefinerPipeline.from_pretrained(
        ...     "hunyuanvideo-community/HunyuanImage-2.1-Refiner-Diffusers", mindspore_dtype=ms.bfloat16
        ... )
...
class HunyuanImageRefinerPipeline(DiffusionPipeline):
    ...
    def _reorder_image_tokens(image_latents):
        image_latents = mint.cat((image_latents[:, :, :1], image_latents), dim=2)
    ...
    def _encode_vae_image(self, image: ms.Tensor, generator: np.random.Generator):
        if isinstance(generator, list):
            image_latents = [
                retrieve_latents(
                    self.vae, self.vae.encode(image[i : i + 1])[0], generator=generator[i], sample_mode="sample"
                )
            ...
            image_latents = retrieve_latents(
                self.vae, self.vae.encode(image)[0], generator=generator, sample_mode="sample"
            )
    ...
    def __call__(
        ...
        # 3. process image
        if image is not None and isinstance(image, ms.Tensor) and image.shape[1] == self.latent_channels:
            image_latents = image
        else:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = mint.cat([latents, cond_latents], dim=1).to(self.transformer.dtype)
