"""Adapted from https://github.com/huggingface/diffusers/tree/main/src/diffusers/pipelines/unidiffuser/modeling_uvit.py."""
...
def _no_grad_trunc_normal_(tensor, mean, std, a, b):
    ...
    # [2v-1, 2u-1].
    # todo: unavaliable uniform_
    tensor = tensor.uniform(2 * v - 1, 2 * u - 1)
...
class PatchEmbed(nn.Cell):
    ...
    def __init__(
        ...
        if self.use_pos_embed:
            pos_embed = get_2d_sincos_pos_embed(embed_dim, int(num_patches**0.5), output_type="ms")
            self.register_buffer("pos_embed", mint.unsqueeze(pos_embed.float(), 0), persistent=False)
...
class SkipBlock(nn.Cell):
    def __init__(self, dim: int):
        super().__init__()

        self.skip_linear = mint.nn.Linear(2 * dim, dim)

        # Use mint.nn.LayerNorm for now, following the original code
        self.norm = mint.nn.LayerNorm(dim)

    def construct(self, x, skip):
        x = self.skip_linear(mint.cat([x, skip], dim=-1))
        x = self.norm(x)
...
class UTransformerBlock(nn.Cell):
    r"""
    ...
    def __init__(
        ...
        if self.use_ada_layer_norm:
            ...
            self.norm1 = mint.nn.LayerNorm(dim, elementwise_affine=norm_elementwise_affine)
        ...
        if cross_attention_dim is not None or double_self_attention:
            ...
            self.norm2 = (
                ...
                if self.use_ada_layer_norm
                else mint.nn.LayerNorm(dim, elementwise_affine=norm_elementwise_affine)
            )
        else:
            self.norm2 = None

        # 3. Feed-forward
        self.norm3 = mint.nn.LayerNorm(dim, elementwise_affine=norm_elementwise_affine)
...
# Modified from diffusers.models.attention.BasicTransformerBlock
class UniDiffuserBlock(nn.Cell):
    r"""
...
# Only supports patch-style input and mint.nn.LayerNorm currently
...
class UTransformer2DModel(ModelMixin, ConfigMixin):
    ...
    Parameters:
        ...
        norm_type (`str`, *optional*, defaults to `"layer_norm"`):
            The Layer Normalization implementation to use. Defaults to `mint.nn.LayerNorm`.
    ...
    def __init__(
        ...
        self.transformer_in_blocks = nn.CellList(
            [
                block_cls(
                    inner_dim,
                    num_attention_heads,
        ...
        self.transformer_out_blocks = nn.SequentialCell(
            [
                nn.CellDict(
                    {
                        "skip": SkipBlock(
    ...
    def construct(
        ...
        Args:
            hidden_states ( When discrete, `ms.Tensor` of shape `(batch size, num latent pixels)`.
                When continuous, `ms.Tensor` of shape `(batch size, channel, height, width)`): Input hidden_states
...
class UniDiffuserModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.clip_img_in = mint.nn.Linear(clip_img_dim, self.inner_dim)
        self.text_in = mint.nn.Linear(text_dim, self.inner_dim)
        ...
        self.pos_embed = ms.Parameter(mint.zeros((1, self.num_tokens, self.inner_dim)))
        self.pos_embed_drop = mint.nn.Dropout(p=dropout)
        ...
        if self.use_data_type_embedding:
            self.data_type_token_embedding = mint.nn.Embedding(2, self.inner_dim)
            self.data_type_pos_embed_token = ms.Parameter(mint.zeros((1, 1, self.inner_dim)))
        ...
        self.vae_img_out = mint.nn.Linear(self.inner_dim, patch_dim)
        self.clip_img_out = mint.nn.Linear(self.inner_dim, clip_img_dim)
        self.text_out = mint.nn.Linear(self.inner_dim, text_dim)
    ...
    def construct(
        self,
        latent_image_embeds: ms.Tensor,
        image_embeds: ms.Tensor,
        prompt_embeds: ms.Tensor,
        timestep_img: Union[ms.Tensor, float, int],
        timestep_text: Union[ms.Tensor, float, int],
        data_type: Optional[Union[ms.Tensor, float, int]] = 1,
        encoder_hidden_states=None,
        cross_attention_kwargs=None,
    ):
        """
        Args:
            latent_image_embeds (`ms.Tensor` of shape `(batch size, latent channels, height, width)`):
                Latent image representation from the VAE encoder.
            image_embeds (`ms.Tensor` of shape `(batch size, 1, clip_img_dim)`):
                CLIP-embedded image representation (unsqueezed in the first dimension).
            prompt_embeds (`ms.Tensor` of shape `(batch size, seq_len, text_dim)`):
                CLIP-embedded text representation.
            timestep_img (`ms.int64` or `float` or `int`):
                Current denoising step for the image.
            timestep_text (`ms.int64` or `float` or `int`):
                Current denoising step for the text.
            data_type: (`ms.int32` or `float` or `int`, *optional*, defaults to `1`):
        ...
        if not ms.is_tensor(timestep_img):
            timestep_img = ms.tensor([timestep_img], dtype=ms.int64)

        # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
        timestep_img = timestep_img * mint.ones(batch_size, dtype=timestep_img.dtype)
        ...
        if not ms.is_tensor(timestep_text):
            timestep_text = ms.tensor([timestep_text], dtype=ms.int64)

        # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
        timestep_text = timestep_text * mint.ones(batch_size, dtype=timestep_text.dtype)
        ...
        if self.use_data_type_embedding:
            ...
            if not ms.is_tensor(data_type):
                data_type = ms.tensor([data_type], dtype=ms.int32)

            # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
            data_type = data_type * mint.ones(batch_size, dtype=data_type.dtype)
        ...
        img_vae_out = mint.einsum("nhwpqc->nchpwq", img_vae_out)
        img_vae_out = img_vae_out.reshape(
            shape=(-1, self.out_channels, height * self.patch_size, width * self.patch_size)
        )
