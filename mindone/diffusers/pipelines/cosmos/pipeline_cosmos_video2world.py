...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> video = pipe(video=video, prompt=prompt)[0][0]
        >>> export_to_video(video, "output.mp4", fps=30)
        ```
"""
...
# `AutoencoderKLCosmos` use identity_dist
def retrieve_latents(
    vae, encoder_output: ms.Tensor, generator: Optional[np.random.Generator] = None, sample_mode: str = "sample"
):
    if sample_mode == "sample":
        return vae.identity_dist.sample(encoder_output, generator=generator)
    elif sample_mode == "argmax":
        return vae.identity_dist.mode(encoder_output)
