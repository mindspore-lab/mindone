...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        ...     "SG161222/Realistic_Vision_V6.0_B1_noVAE", motion_adapter=adapter, mindspore_dtype=ms.float16
        ... )
...
class PIAPipelineOutput(BaseOutput):
    ...
    Args:
        frames (`ms.Tensor`, `np.ndarray`, or List[List[PIL.Image.Image]]):
            ...
            shape `(batch_size, num_frames, channels, height, width, MindSpore tensor of shape `(batch_size, num_frames,
...
class PIAPipeline(
    ...
    def prepare_masked_condition(
        ...
        if isinstance(generator, list):
            image_latent = [
                self.vae.diag_gauss_dist.sample(self.vae.encode(image[k : k + 1])[0], generator[k])
                for k in range(batch_size)
            ]
            image_latent = ops.cat(image_latent, axis=0)
        else:
            image_latent = self.vae.diag_gauss_dist.sample(self.vae.encode(image)[0], generator)

        image_latent = image_latent.to(dtype=dtype)
        image_latent = ops.interpolate(image_latent, size=[scaled_height, scaled_width])
        image_latent_padding = image_latent.copy() * self.vae.config.scaling_factor

        mask = ops.zeros((batch_size, 1, num_frames, scaled_height, scaled_width)).to(dtype=dtype)
        mask_coef = prepare_mask_coef_by_statistics(num_frames, 0, motion_scale)
        masked_image = ops.zeros((batch_size, 4, num_frames, scaled_height, scaled_width)).to(dtype=self.unet.dtype)
        for f in range(num_frames):
            mask[:, :, f, :, :] = mask_coef[f]
            masked_image[:, :, f, :, :] = image_latent_padding.copy()

        mask = ops.cat([mask] * 2) if self.do_classifier_free_guidance else mask
        masked_image = ops.cat([masked_image] * 2) if self.do_classifier_free_guidance else masked_image
    ...
    def __call__(
        ...
        with self.progress_bar(total=self._num_timesteps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = ops.cat([latent_model_input, mask, masked_image], axis=1)
