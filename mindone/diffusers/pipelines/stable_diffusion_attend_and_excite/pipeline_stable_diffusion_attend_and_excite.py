...
class AttentionStore:
    ...
    def aggregate_attention(self, from_where: List[str]) -> ms.Tensor:
...
class StableDiffusionAttendAndExcitePipeline(DiffusionPipeline, StableDiffusionMixin, TextualInversionLoaderMixin):
    ...
    def _compute_max_attention_per_index(
        attention_maps: ms.Tensor,
        indices: List[int],
    ) -> List[ms.Tensor]:
        """Computes the maximum attention value for each of the tokens we wish to alter."""
        attention_for_text = attention_maps[:, :, 1:-1]
        attention_for_text *= 100
        attention_for_text = mint.nn.functional.softmax(attention_for_text, dim=-1)
        ...
        for i in indices:
            ...
            input = mint.nn.functional.pad(image.unsqueeze(0).unsqueeze(0), (1, 1, 1, 1), mode="reflect")
    ...
    @staticmethod
    def _compute_loss(max_attention_per_index: List[ms.Tensor]) -> ms.Tensor:
        """Computes the attend-and-excite loss using the maximum attention value for each token."""
        losses = [max(0, 1.0 - curr_max) for curr_max in max_attention_per_index]
        loss = max(losses)
        return loss

    def _update_latent(self, latents: ms.Tensor, grad_cond: ms.Tensor, step_size: float) -> ms.Tensor:
        """Update the latent according to the computed loss."""
        latents = latents - ms.tensor(step_size, dtype=latents.dtype) * grad_cond
        return latents
    ...
    def _perform_iterative_refinement_step(
        ...
        while loss > target_loss:
            ...
            latents = ops.stop_gradient(latents.clone())

            grad_fn = ms.value_and_grad(self.iterative_refinement_step, grad_position=0)
            loss, grad_cond = grad_fn(latents, t, text_embeddings, indices)

            if loss != 0:
                latents = self._update_latent(latents, grad_cond, step_size)
        ...
        latents = ops.stop_gradient(latents.clone())
        _ = self.unet(latents, t, encoder_hidden_states=text_embeddings)[0]
        # self.unet.zero_grad()
    ...
    def iterative_refinement_step(self, latents, t, text_embeddings, indices):
        self.unet(latents, t, encoder_hidden_states=text_embeddings)[0]
        # self.unet.zero_grad()
    ...
    def train_step(self, latent, index, text_embedding, t, cross_attention_kwargs, thresholds, i, step_size):
        self.unet(
            latent,
            t,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for latent, index, text_embedding in zip(latents, indices, text_embeddings):
                    for name in self.unet.attn_processors.keys():
                        self.unet.attn_processors[name].requires_grad = True
                    # Forward pass of denoising with text conditioning
                    latent = latent.unsqueeze(0)
                    text_embedding = text_embedding.unsqueeze(0)
                    grad_fn = ms.value_and_grad(self.train_step, grad_position=0)
                    loss, grad_cond = grad_fn(
                        latent, index, text_embedding, t, cross_attention_kwargs, thresholds, i, step_size
                    )
                    ...
                    if i < max_iter_to_alter:
                        ...
                        logger.info(f"Iteration {i} | Loss: {loss.numpy():0.4f}")

                    updated_latents.append(latent)

                latents = mint.cat(updated_latents, dim=0)
                ...
                for name in self.unet.attn_processors.keys():
                    self.unet.attn_processors[name].requires_grad = False
...
class GaussianSmoothing(nn.Cell):
    """
    Arguments:
    ...
    def __init__(
        ...
        kernel = kernel.tile((channels, *[1] * (kernel.dim() - 1)))

        self.weight = ms.Parameter(kernel, requires_grad=False, name="weight")
        self.groups = channels

        if dim == 1:
            self.conv = ops.conv1d
        elif dim == 2:
            self.conv = mint.nn.functional.conv2d
        elif dim == 3:
            self.conv = mint.nn.functional.conv3d
        else:
