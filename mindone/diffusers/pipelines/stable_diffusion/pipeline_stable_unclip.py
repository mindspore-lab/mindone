...
from mindone.transformers.models.clip.modeling_clip import CLIPTextModelOutput
...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import mindspore as ms
        >>> from mindone.diffusers import StableUnCLIPPipeline

        >>> pipe = StableUnCLIPPipeline.from_pretrained(
        ...     "fusing/stable-unclip-2-1-l", mindspore_dtype=ms.float16
        ... )  # TODO update model path

        >>> prompt = "a photo of an astronaut riding a horse on mars"
        >>> images = pipe(prompt)[0]
