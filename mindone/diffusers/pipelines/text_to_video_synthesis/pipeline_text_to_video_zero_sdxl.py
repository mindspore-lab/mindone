...
from ...utils import BaseOutput, is_invisible_watermark_available, logging, scale_lora_layers, unscale_lora_layers
...
def rearrange_0(tensor, f):
    ...
    tensor = mint.permute(mint.reshape(tensor, (F // f, f, C, H, W)), (0, 2, 1, 3, 4))
    return tensor
...
class CrossFrameAttnProcessor2_0:
    """
    Cross frame attention processor with scaled_dot_product attention.
...
def coords_grid(batch, ht, wd):
    ...
    coords = mint.meshgrid(mint.arange(ht), mint.arange(wd))
    coords = mint.stack(coords[::-1], dim=0).float()
...
def warp_single_latent(latent, reference_flow):
    ...
    coords0 = coords_grid(1, H, W).to(latent.dtype)
    ...
    coords_t0 = mint.nn.functional.interpolate(coords_t0, size=(h, w), mode="bilinear")
    coords_t0 = mint.permute(coords_t0, (0, 2, 3, 1))

    # fp16 is not supported in mint.nn.functional.grid_sample
    if latent.dtype == ms.float16:
        warped = mint.nn.functional.grid_sample(
            latent.float(), coords_t0.float(), mode="nearest", padding_mode="reflection"
        ).half()
    else:
...
def create_motion_field(motion_field_strength_x, motion_field_strength_y, frame_ids, dtype):
    ...
    reference_flow = mint.zeros((seq_length, 2, 512, 512), dtype=dtype)
...
def create_motion_field_and_warp_latents(motion_field_strength_x, motion_field_strength_y, frame_ids, latents):
    ...
    warped_latents = latents.copy()
    for i in range(len(warped_latents)):
        # in torch, a[0] = b (b.shape=[1,x,x,x], a.shape=[x,x,x,x])   output: a[0] can be assigned to b
        # in mindspore a[0] = b  will throw an error
        warped_latents[i] = warp_single_latent(latents[i][None], motion_field[i][None])[0]
...
class TextToVideoZeroSDXLPipeline(
    ...
    # Copied from diffusers.pipelines.stable_diffusion_xl.pipeline_stable_diffusion_xl.StableDiffusionXLPipeline._get_add_time_ids  # noqa: E501
    ...
    def forward_loop(self, x_t0, t0, t1, generator):
        ...
        alpha_vec = mint.prod(self.scheduler.alphas[t0:t1])
        x_t1 = (mint.sqrt(alpha_vec) * x_t0 + mint.sqrt(1 - alpha_vec) * eps).to(x_t0.dtype)
        return x_t1
    ...
    def __call__(
        ...
        x_2k_t1 = self.forward_loop(
            x_t0=x_2k_t0,
            t0=timesteps[-t0 - 1].to(ms.int64),
            t1=timesteps[-t1 - 1].to(ms.int64),
            generator=generator,
        )

        # Perform backward process from time T_1 to 0
        latents = mint.cat([x_1_t1, x_2k_t1])
