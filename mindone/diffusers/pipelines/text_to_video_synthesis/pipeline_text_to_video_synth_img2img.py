...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        ...     "cerspense/zeroscope_v2_576w", mindspore_dtype=ms.float16, revision="refs/pr/46"
        ... )
        >>> pipe.scheduler = DPMSolverMultistepScheduler.from_config(pipe.scheduler.config)

        >>> prompt = "spiderman running in the desert"
        >>> video_frames = pipe(prompt, num_inference_steps=40, height=320, width=576, num_frames=24)[0][0]
        >>> # safe low-res video
        >>> video_path = export_to_video(video_frames, output_video_path="./video_576_spiderman.mp4")

        >>> # load the image-to-image model
        >>> pipe = DiffusionPipeline.from_pretrained(
        ...     "cerspense/zeroscope_v2_XL", mindspore_dtype=ms.float16, revision="refs/pr/34"
        ... )
        >>> pipe.scheduler = DPMSolverMultistepScheduler.from_config(pipe.scheduler.config)
...
class VideoToVideoSDPipeline(
    ...
    def prepare_latents(self, video, timestep, batch_size, dtype, generator=None):
        ...
        if video.shape[1] == 4:
            ...
            if isinstance(generator, list) and len(generator) != batch_size:
                ...
                init_latents = [
                    retrieve_latents(self.vae, self.vae.encode(video[i : i + 1])[0], generator=generator[i])
                    for i in range(batch_size)
                ]
                init_latents = mint.cat(init_latents, dim=0)
            else:
                init_latents = retrieve_latents(self.vae, self.vae.encode(video)[0], generator=generator)
    ...
    def __call__(
        ...
        video: Union[List[np.ndarray], ms.Tensor] = None,
        strength: float = 0.6,
        ...
        Args:
            ...
            video (`List[np.ndarray]` or `ms.Tensor`):
            ...
            output_type (`str`, *optional*, defaults to `"np"`):
                The output format of the generated video. Choose between `ms.Tensor` or `np.array`.
