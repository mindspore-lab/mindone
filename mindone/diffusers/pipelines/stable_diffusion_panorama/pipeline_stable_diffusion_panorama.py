...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import mindspore as ms
        >>> from mindone.diffusers import StableDiffusionPanoramaPipeline, DDIMScheduler

        >>> model_ckpt = "stabilityai/stable-diffusion-2-base"
        >>> scheduler = DDIMScheduler.from_pretrained(model_ckpt, subfolder="scheduler")
        >>> pipe = StableDiffusionPanoramaPipeline.from_pretrained(
        ...     model_ckpt, scheduler=scheduler, mindspore_dtype=ms.float16
        ... )
...
class StableDiffusionPanoramaPipeline(
    ...
    def decode_latents_with_padding(self, latents: ms.Tensor, padding: int = 8) -> ms.Tensor:
        """
        Decode the given latents with padding for circular inference.

        Args:
            latents (ms.Tensor): The input latents to decode.
        ...
        latents = ops.cat((latents_right, latents, latents_left), axis=-1)
        image = self.vae.decode(latents, return_dict=False)[0]
            ...
            for i, t in enumerate(timesteps):
                ...
                for j, batch_view in enumerate(views_batch):
                    ...
                    if circular_padding:
                        ...
                        latents_for_view = ops.cat(latents_for_view)
                    else:
                        latents_for_view = ops.cat(
                            [
                    ...
                    prompt_embeds_input = ops.cat([prompt_embeds] * vb_size)
                    ...
                    latents_denoised_batch = self.scheduler.step(noise_pred, t, latents_for_view, **extra_step_kwargs)[
                        0
                    ]
