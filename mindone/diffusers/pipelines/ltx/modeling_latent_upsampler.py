...
class ResBlock(nn.Cell):
    def __init__(self, channels: int, mid_channels: Optional[int] = None, dims: int = 3):
        ...
        Conv = mint.nn.Conv2d if dims == 2 else mint.nn.Conv3d

        self.conv1 = Conv(channels, mid_channels, kernel_size=3, padding=1)
        self.norm1 = mint.nn.GroupNorm(32, mid_channels)
        self.conv2 = Conv(mid_channels, channels, kernel_size=3, padding=1)
        self.norm2 = mint.nn.GroupNorm(32, channels)
...
class PixelShuffleND(nn.Cell):
    ...
    def construct(self, x):
        if self.dims == 3:
            ...
            return (
                unflatten(x, 1, (-1, *self.upscale_factors[:3]))
            ...
            return (
                unflatten(x, 1, (-1, *self.upscale_factors[:2])).permute(0, 1, 4, 2, 5, 3).flatten(4, 5).flatten(2, 3)
            )
        elif self.dims == 1:
            # temporal: b (c p1) f h w -> b c (f p1) h w
            return unflatten(x, 1, (-1, *self.upscale_factors[:1])).permute(0, 1, 3, 2, 4, 5).flatten(2, 3)
...
class LTXLatentUpsamplerModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        ConvNd = mint.nn.Conv2d if dims == 2 else mint.nn.Conv3d

        self.initial_conv = ConvNd(in_channels, mid_channels, kernel_size=3, padding=1)
        self.initial_norm = mint.nn.GroupNorm(32, mid_channels)
        self.initial_activation = mint.nn.SiLU()

        self.res_blocks = nn.CellList([ResBlock(mid_channels, dims=dims) for _ in range(num_blocks_per_stage)])

        if spatial_upsample and temporal_upsample:
            self.upsampler = nn.SequentialCell(
                mint.nn.Conv3d(mid_channels, 8 * mid_channels, kernel_size=3, padding=1),
                PixelShuffleND(3),
            )
        elif spatial_upsample:
            self.upsampler = nn.SequentialCell(
                mint.nn.Conv2d(mid_channels, 4 * mid_channels, kernel_size=3, padding=1),
                PixelShuffleND(2),
            )
        elif temporal_upsample:
            self.upsampler = nn.SequentialCell(
                mint.nn.Conv3d(mid_channels, 2 * mid_channels, kernel_size=3, padding=1),
                PixelShuffleND(1),
            )
        else:
            raise ValueError("Either spatial_upsample or temporal_upsample must be True")

        self.post_upsample_res_blocks = nn.CellList(
    ...
    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        ...
        if self.dims == 2:
            ...
            hidden_states = unflatten(hidden_states, 0, (batch_size, -1)).permute(0, 2, 1, 3, 4)
        else:
            hidden_states = self.initial_conv(hidden_states)
            hidden_states = self.initial_norm(hidden_states)
            hidden_states = self.initial_activation(hidden_states)
