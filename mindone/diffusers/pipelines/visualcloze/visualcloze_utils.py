...
class VisualClozeProcessor(VaeImageProcessor):
    ...
    def preprocess_image(
    ...
    ) -> Tuple[List[List[ms.Tensor]], List[List[List[int]]], List[int]]:
        """
        ...
        Returns:
            Tuple containing:
            - List[List[ms.Tensor]]: Preprocessed images in tensor format
    ...
    def preprocess_mask(
        ...
        for i, row in enumerate(input_images):
            if i == len(input_images) - 1:  # Query row
                row_masks = [mint.full((1, 1, row[0].shape[2], row[0].shape[3]), fill_value=m) for m in target_position]
            else:  # In-context examples
                row_masks = [mint.full((1, 1, row[0].shape[2], row[0].shape[3]), fill_value=0) for _ in target_position]
    ...
    def preprocess_image_upsampling(
        ...
        image = self.numpy_to_ms(image)  # to pt
        image = self.normalize(image)
    ...
    def preprocess_mask_upsampling(self, input_images: List[List[Image.Image]]) -> List[List[ms.Tensor]]:
        return [[mint.ones((1, 1, input_images[0][0].shape[2], input_images[0][0].shape[3]))]]
