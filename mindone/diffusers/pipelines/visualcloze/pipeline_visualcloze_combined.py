...
class VisualClozePipeline(
    ...
    def __call__(
        ...
        upsampling_mask = []
        upsampling_prompt = []
        upsampling_generator = generator if isinstance(generator, (np.random.Generator,)) else []
        for i in range(len(generation_output[0])):
            n_target_per_sample.append(len(generation_output[0][i]))
            for image in generation_output[0][i]:
