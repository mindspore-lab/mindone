...
class BriaFiboPipeline(DiffusionPipeline):
    ...
    def get_prompt_embeds(
        ...
        text_encoder_device = device if device is not None else ms.device("cpu")
        if not isinstance(text_encoder_device, ms.device):
            text_encoder_device = ms.device(text_encoder_device)

        if all(p == "" for p in prompt):
            input_ids = mint.full((batch_size, 1), bot_token_id, dtype=ms.int64).to(device=text_encoder_device)
            ...
            input_ids = ms.tensor(tokenized.input_ids).to(text_encoder_device)
            attention_mask = ms.tensor(tokenized.attention_mask).to(text_encoder_device)

            if any(p == "" for p in prompt):
                empty_rows = ms.tensor([p == "" for p in prompt], dtype=ms.bool).to(device=text_encoder_device)
    ...
    def pad_embedding(prompt_embeds, max_tokens, attention_mask=None):
        ...
        if attention_mask is None:
            attention_mask = mint.ones((batch_size, seq_len), dtype=prompt_embeds.dtype).to(device=prompt_embeds.device)
        else:
        ...
        if max_tokens > seq_len:
            ...
            padding = mint.zeros((batch_size, pad_length, dim), dtype=prompt_embeds.dtype).to(
                device=prompt_embeds.device
            )
            prompt_embeds = mint.cat([prompt_embeds, padding], dim=1)

            mask_padding = mint.zeros((batch_size, pad_length), dtype=prompt_embeds.dtype).to(
                device=prompt_embeds.device
            )
    ...
    def encode_prompt(
        ...
        text_ids = mint.zeros((prompt_embeds.shape[0], max_tokens, 3)).to(device=device, dtype=dtype)
    ...
    def prepare_latents(
        ...
        latents = randn_tensor(shape, generator=generator, dtype=dtype).to(device=device)
        if do_patching:
