...
class StableDiffusionPipelineSafe(DeprecatedPipelineMixin, DiffusionPipeline, StableDiffusionMixin, IPAdapterMixin):
    ...
    def _encode_prompt(
        ...
        if do_classifier_free_guidance:
            ...
            if enable_safety_guidance:
                ...
                safety_embeddings = self.text_encoder(ms.tensor(safety_concept_input.input_ids))[0]
                ...
                prompt_embeds = mint.cat([negative_prompt_embeds, prompt_embeds, safety_embeddings])
        ...
        if self.safety_checker is not None:
            ...
            if any(has_nsfw_concept):
                logger.warning(
                    ...
                    " instead."
                    f"{'You may look at this images in the `unsafe_images` variable of the output at your own discretion.' if enable_safety_guidance else 'Try again with a different prompt and/or seed.'}"  # noqa: E501
                )
    ...
    def perform_safety_guidance(
        ...
        if enable_safety_guidance:
            if safety_momentum is None:
                safety_momentum = mint.zeros_like(noise_guidance)
            noise_pred_text, noise_pred_uncond = noise_pred_out[0], noise_pred_out[1]
            noise_pred_safety_concept = noise_pred_out[2]

            # Equation 6
            scale = mint.clamp(mint.abs((noise_pred_text - noise_pred_safety_concept)) * sld_guidance_scale, max=1.0)

            # Equation 6
            safety_concept_scale = mint.where(
                (noise_pred_text - noise_pred_safety_concept) >= sld_threshold, mint.zeros_like(scale), scale
            )

            # Equation 4
            noise_guidance_safety = mint.mul((noise_pred_safety_concept - noise_pred_uncond), safety_concept_scale)
    ...
    def __call__(
        ...
        pipeline = StableDiffusionPipelineSafe.from_pretrained(
            "AIML-TUDA/stable-diffusion-safe", mindspore_dtype=ms.float16, revision="refs/pr/6"
        )
        prompt = "the four horsewomen of the apocalypse, painting by tom of finland, gaston bussiere, craig mullins, j. c. leyendecker"
        image = pipeline(prompt=prompt, **SafetyConfig.MEDIUM)[0][0]
        ```
        """
        ...
        if ip_adapter_image is not None:
            ...
            if do_classifier_free_guidance:
                if enable_safety_guidance:
                    image_embeds = mint.cat([negative_image_embeds, image_embeds, image_embeds])
                else:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = (
                    mint.cat([latents] * (3 if enable_safety_guidance else 2))
                    if do_classifier_free_guidance
                    else latents
                )
                ...
                if do_classifier_free_guidance:
                    ...
                    if enable_safety_guidance:
                        ...
                        # Equation 6
                        scale = mint.clamp(
                            mint.abs((noise_pred_text - noise_pred_safety_concept)) * sld_guidance_scale, max=1.0
                        )

                        # Equation 6
                        safety_concept_scale = mint.where(
                            (noise_pred_text - noise_pred_safety_concept) >= sld_threshold,
                            mint.zeros_like(scale),
                            scale,
                        )

                        # Equation 4
                        noise_guidance_safety = mint.mul(
