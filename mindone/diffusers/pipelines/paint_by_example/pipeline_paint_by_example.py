...
def prepare_mask_and_masked_image(image, mask):
    ...
    if isinstance(image, ms.Tensor):
        ...
        image = ms.tensor(image).to(dtype=ms.float32) / 127.5 - 1.0
...
class PaintByExamplePipeline(DiffusionPipeline, StableDiffusionMixin):
    ...
    def __call__(
        self,
        example_image: Union[ms.Tensor, PIL.Image.Image],
        image: Union[ms.Tensor, PIL.Image.Image],
        mask_image: Union[ms.Tensor, PIL.Image.Image],
        ...
        Args:
            example_image (`ms.Tensor` or `PIL.Image.Image` or `List[PIL.Image.Image]`):
                An example image to guide image generation.
            image (`ms.Tensor` or `PIL.Image.Image` or `List[PIL.Image.Image]`):
            ...
            mask_image (`ms.Tensor` or `PIL.Image.Image` or `List[PIL.Image.Image]`):
        ...
        >>> image = pipe(image=init_image, mask_image=mask_image, example_image=example_image)[0][0]
        >>> image
        ```
