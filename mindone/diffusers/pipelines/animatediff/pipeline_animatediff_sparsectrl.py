...
class AnimateDiffSparseControlNetPipeline(
    ...
    def check_image(self, image, prompt, prompt_embeds):
        ...
        if (
            ...
            raise TypeError(
                f"image must be passed and be one of PIL image, numpy array, mindspore tensor, list of PIL images, list of numpy arrays or list of mindspore tensors, but is {type(image)}"
            )
    ...
    def prepare_image(self, image, width, height, dtype):
        ...
        if self.controlnet.use_simplified_condition_embedding:
            ...
            conditioning_frames = (
                retrieve_latents(self.vae, self.vae.encode(controlnet_images)) * self.vae.config.scaling_factor
            )
