...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> prompt = "Photorealistic food photography of a stack of fluffy pancakes on a white plate, "
                    "with maple syrup being poured over them. On top of the pancakes are the words 'BRIA 3.2' in bold, "
                    "yellow, 3D letters. The background is dark and out of focus."
...
class BriaPipeline(DiffusionPipeline):
    ...
    def encode_prompt(
        ...
        text_ids = mint.zeros((batch_size, prompt_embeds.shape[1], 3), dtype=prompt_embeds.dtype)
    ...
    def _get_t5_prompt_embeds(
        ...
        for p in prompt:
            ...
            prompt_embeds = text_encoder(ms.tensor(text_input_ids))[0]

            # Concat zeros to max_sequence
            b, seq_len, dim = prompt_embeds.shape
            if seq_len < max_sequence_length:
                padding = mint.zeros((b, max_sequence_length - seq_len, dim), dtype=prompt_embeds.dtype)
                prompt_embeds = mint.concat([prompt_embeds, padding], dim=1)
            prompt_embeds_list.append(prompt_embeds)

        prompt_embeds = mint.concat(prompt_embeds_list, dim=0)
    ...
    def __call__(
        ...
        if (
            ...
            if isinstance(self.scheduler, DDIMScheduler) or isinstance(self.scheduler, EulerAncestralDiscreteScheduler):
                timesteps, num_inference_steps = retrieve_timesteps(self.scheduler, num_inference_steps, None, None)
            else:
        ...
        if output_type == "latent":
            ...
            latents = (latents.to(dtype=ms.float32) / self.vae.config.scaling_factor) + self.vae.config.shift_factor
