...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import mindspore as ms
        >>> from mindone.diffusers import EasyAnimateInpaintPipeline
        >>> from mindone.diffusers.pipelines.easyanimate.pipeline_easyanimate_inpaint import get_image_to_video_latent
        >>> from mindone.diffusers.utils import export_to_video, load_image

        >>> pipe = EasyAnimateInpaintPipeline.from_pretrained(
        ...     "alibaba-pai/EasyAnimateV5.1-12b-zh-InP-diffusers", mindspore_dtype=ms.bfloat16
        ... )

        >>> prompt = "An astronaut hatching from an egg, on the surface of the moon, the darkness and depth of space realised in the background. High quality, ultrarealistic detail and breath-taking movie-like camera shot." # noqa
...
def get_image_to_video_latent(validation_image_start, validation_image_end, num_frames, sample_size):
    ...
    if validation_image_start is not None:
        ...
        if isinstance(image_start, list):
            start_video = mint.cat(
                [img.unsqueeze(1).unsqueeze(0) for img in image_start],
                dim=2,
            )
            input_video = mint.tile(start_video[:, :, :1], (1, 1, num_frames, 1, 1))
            input_video[:, :, : len(image_start)] = start_video
        else:
            input_video = mint.tile(
        ...
        input_video = mint.zeros([1, 3, num_frames, sample_size[0], sample_size[1]])
        input_video_mask = mint.ones([1, 1, num_frames, sample_size[0], sample_size[1]]) * 255
...
def add_noise_to_reference_video(image, ratio=None, generator=None):
    if ratio is None:
        sigma = mint.normal(mean=-3.0, std=0.5, size=(image.shape[0],))
        sigma = mint.exp(sigma).to(image.dtype)
    else:
        sigma = mint.ones((image.shape[0],)).to(image.dtype) * ratio

    if generator is not None:
        # TODO: mint.randn can't use np.random.Generator
        image_noise = (
            ms.tensor(generator.standard_normal(image.shape), dtype=image.dtype) * sigma[:, None, None, None, None]
        )
    else:
        image_noise = mint.randn_like(image) * sigma[:, None, None, None, None]
    image_noise = mint.where(image == -1, mint.zeros_like(image), image_noise)
...
class EasyAnimateInpaintPipeline(DiffusionPipeline):
    ...
    def prepare_mask_latents(
        ...
        if mask is not None:
            ...
            for i in range(0, mask.shape[0], bs):
                ...
                mask_bs = self.vae.diag_gauss_dist.mode(mask_bs)
                new_mask.append(mask_bs)
            mask = mint.cat(new_mask, dim=0)
        ...
        if masked_image is not None:
            ...
            for i in range(0, masked_image.shape[0], bs):
                ...
                mask_pixel_values_bs = self.vae.diag_gauss_dist.mode(mask_pixel_values_bs)
                new_mask_pixel_values.append(mask_pixel_values_bs)
            masked_image_latents = mint.cat(new_mask_pixel_values, dim=0)
    ...
    def prepare_latents(
        ...
        if return_video_latents or (latents is None and not is_strength_max):
            ...
            for i in range(0, video.shape[0], bs):
                ...
                video_bs = self.vae.diag_gauss_dist.sample(video_bs)
                new_video.append(video_bs)
            video = mint.cat(new_video, dim=0)
    ...
    def __call__(
        ...
        video: Union[ms.Tensor] = None,
        mask_video: Union[ms.Tensor] = None,
        masked_video_latents: Union[ms.Tensor] = None,
        ...
        Examples:
            ...
            generator (`np.random.Generator` or `List[np.random.Generator]`, *optional*):
                A [`np.random.Generator`](https://numpy.org/doc/stable/reference/random/generator.html) for setting
        ...
        if video is not None:
            ...
            init_video = init_video.to(dtype=ms.float32)
        ...
        if mask_video is not None:
            if (mask_video == 255).all():
                mask = mint.zeros_like(latents).to(dtype)
                # Use zero latents if we want to t2v.
                if self.transformer.config.resize_inpaint_mask_directly:
                    mask_latents = mint.zeros_like(latents)[:, :1].to(dtype)
                else:
                    mask_latents = mint.zeros_like(latents).to(dtype)
                masked_video_latents = mint.zeros_like(latents).to(dtype)

                mask_input = mint.cat([mask_latents] * 2) if self.do_classifier_free_guidance else mask_latents
                masked_video_latents_input = (
                    mint.cat([masked_video_latents] * 2) if self.do_classifier_free_guidance else masked_video_latents
                )
                inpaint_latents = mint.cat([mask_input, masked_video_latents_input], dim=1).to(dtype)
            else:
                ...
                mask_condition = mask_condition.to(dtype=ms.float32)
                mask_condition = mask_condition.reshape(batch_size, num_frames, channels, height, width).permute(
                    0, 2, 1, 3, 4
                )

                if num_channels_transformer != num_channels_latents:
                    mask_condition_tile = mint.tile(mask_condition, (1, 3, 1, 1, 1))
                    if masked_video_latents is None:
                        masked_video = (
                            init_video * (mask_condition_tile < 0.5)
                            + mint.ones_like(init_video) * (mask_condition_tile > 0.5) * -1
                        )
                    else:
                    ...
                    if self.transformer.config.resize_inpaint_mask_directly:
                        ...
                        mask_latents = mask_latents.to(dtype) * self.vae.config.scaling_factor
                    else:
                    ...
                    mask_input = mint.cat([mask_latents] * 2) if self.do_classifier_free_guidance else mask_latents
                    masked_video_latents_input = (
                        mint.cat([masked_video_latents] * 2)
                ...
                mask = mint.tile(mask_condition, (1, num_channels_latents, 1, 1, 1))
                mask = F.interpolate(mask, size=latents.shape[-3:], mode="trilinear", align_corners=True).to(dtype)
        else:
            if num_channels_transformer != num_channels_latents:
                mask = mint.zeros_like(latents).to(dtype)
                if self.transformer.config.resize_inpaint_mask_directly:
                ...
                mask = mint.zeros_like(init_video[:, :1])
                mask = mint.tile(mask, (1, num_channels_latents, 1, 1, 1))
                mask = F.interpolate(mask, size=latents.shape[-3:], mode="trilinear", align_corners=True).to(dtype)
