...
def update_forward_signature(model: PeftModel) -> None:
    ...
    current_signature = inspect.signature(model.construct)
    if (
        len(current_signature.parameters) == 2
        and "args" in current_signature.parameters
        and "kwargs" in current_signature.parameters
    ):
        forward = deepcopy(model.construct.__func__)
        update_wrapper(
            forward, type(model.get_base_model()).construct, assigned=("__doc__", "__name__", "__annotations__")
        )
        model.construct = MethodType(forward, model)
...
@contextmanager
def disable_input_dtype_casting(model: nn.Cell, active: bool = True):
    """
