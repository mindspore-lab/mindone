...
from mindone.peft.utils import INCLUDE_LINEAR_LAYERS_SHORTHAND
from mindone.peft.utils.constants import (
    DUMMY_MODEL_CONFIG,
...
from mindone.peft.utils.peft_types import PeftType, TaskType
...
class BaseTuner(nn.Cell, ABC):
    ...
    def _pre_injection_hook(self, model: nn.Cell, config: PeftConfig, adapter_name: str) -> None:
        r"""
    ...
    def _prepare_model(self, peft_config: PeftConfig, model: nn.Cell):
        r"""
    ...
    def _cast_adapter_dtype(self, adapter_name: str, autocast_adapter_dtype: bool = True) -> None:
        ...
        dtypes_to_convert_to_fp32 = {ms.float16, ms.bfloat16}
        ...
        for _, module in self.model.cells_and_names():
            ...
            for _, submodule in module.cells_and_names():
                if not isinstance(submodule, (nn.CellDict, BufferDict)):
                    continue

                if adapter_name not in submodule:
                    continue

                if isinstance(submodule[adapter_name], ms.Parameter):
                    if submodule[adapter_name].dtype in dtypes_to_convert_to_fp32:
                        submodule[adapter_name] = submodule[adapter_name].to(ms.float32)
                    continue

                if isinstance(submodule[adapter_name], ms.Tensor):  # e.g. from a BufferDict
                    if submodule[adapter_name].dtype in dtypes_to_convert_to_fp32:
                        submodule[adapter_name] = submodule[adapter_name].to(ms.float32)
                    continue

                for param in submodule[adapter_name].get_parameters():
    ...
    def _check_merge_allowed(self):
        ...
        example_code = textwrap.dedent(
            ...
            model.lm_head.weight = model.model.embed_tokens.weight.copy()
    ...
    def inject_adapter(self, model: nn.Cell, adapter_name: str, autocast_adapter_dtype: bool = True) -> None:
        r"""
    ...
    @staticmethod
    def get_model_config(model: nn.Cell) -> dict:
        """
    ...
    def _get_tied_target_modules(self, model: nn.Cell) -> list[str]:
        tied_target_modules = []
        model_config = self.get_model_config(model)
...
class BaseTunerLayer(ABC):
    ...
    def _get_available_adapters(self) -> set[str]:
        ...
        for layer_name in self.adapter_layer_names:
            ...
            if not isinstance(module, nn.CellDict):
                continue
...
def _find_minimal_target_modules(
    ...
    Example:
        ```py
        >>> from mindone.peft.tuners.tuners_utils import _find_minimal_target_modules
    ...
    Args:
        target_modules (`Union[list[str], set[str]]`):
            The list of target modules.
        other_module_names (`Union[list[str], set[str]]`):
...
def _maybe_include_all_linear_layers(peft_config: PeftConfig, model: nn.Cell) -> PeftConfig:
    ...
    linear_classes = (mint.nn.Linear, Conv1D)
    linear_names = ("Linear",)
        ...
        if output_emb is not None:
            ...
            for name in SEQ_CLS_HEAD_NAMES:
                ...
                if cls_head is not None:
                    last_module_name = [name for name, module in model.cells_and_names() if module is cls_head][0]
                    module_names_to_exclude.add(last_module_name)
                    break

    # we don't want nested LoRA layers, i.e. LoRA being applied to possibly existing lora_A, lora_B, etc.
    # see 2390
    for prefix, module in model.cells_and_names():
        if isinstance(module, BaseTunerLayer):
            for suffix, child in module.cells_and_names():
                if suffix:
...
def clone_module(module: nn.Cell, share_weights=False):
    ...
    def _share_weights(src: nn.Cell, dst: nn.Cell):
        for name, param in src.parameters_and_names(expand=False):
            ...
            dst.__setattr__(name, param)

    if share_weights:
        for name, submodule in module.cells_and_names():
            _share_weights(submodule, _get_subcell(clone, name))
...
def replicate_layers(model: nn.Cell, layer_map: list[tuple[int, int]]):
    ...
    for start, end in layer_map:
        for i in range(start, end):
            ...
            for _, submodule in new_layers[-1].cells_and_names():
