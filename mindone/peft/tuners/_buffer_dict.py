...
class BufferDict(Cell):
    ...
    will be visible by all Module methods. `nn.BufferDict` is an **ordered** dictionary that respects

    * the order of insertion, and
    * in `nn.BufferDict.update`, the order of the merged `OrderedDict` or another `nn.BufferDict` (the
      argument to `nn.BufferDict.update`).

    Note that `nn.BufferDict.update` with other unordered mapping types (e.g., Python's plain `dict`) does not
    preserve the order of the merged mapping.

    Args:
        buffers (iterable, optional):
            a mapping (dictionary) of (string : `mindspore.Tensor`) or an iterable of key-value pairs of type (string,
            `mindspore.Tensor`)
    ...
    class MyModule(nn.Cell):
        def __init__(self):
            super().__init__()
            self.buffers = nn.BufferDict({"left": ops.randn(5, 10), "right": ops.randn(5, 10)})
    ...
    def __init__(self, buffers=None, persistent: bool = False):
        ...
        Args:
            buffers (`dict`):
                A mapping (dictionary) from string to `mindspore.Tensor`, or an iterable of key-value pairs of type
                (string, `mindspore.Tensor`).
        """
        super().__init__()
    ...
    def update(self, buffers):
        r"""
        Update the `nn.BufferDict` with the key-value pairs from a mapping or an iterable, overwriting existing
        keys.

        Note:
            If `buffers` is an `OrderedDict`, a `nn.BufferDict`, or an iterable of key-value pairs, the order of
    ...
    def extra_repr(self):
        ...
        for k, p in self._buffers.items():
            ...
            parastr = f"Buffer containing: [mindspore.Tensor of size {size_str}]"
