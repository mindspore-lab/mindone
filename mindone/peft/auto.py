...
class _BaseAutoPeftModel:
    ...
    def __init__(self, *args, **kwargs):
        # For consistency with transformers: https://github.com/huggingface/transformers/blob/91d7df58b6537d385e90578dac40204cb550f706/src/transformers/models/auto/auto_factory.py#L400  # noqa: E501
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if cls._target_class is not None:
            ...
            raise ValueError(
                "Cannot use `AutoPeftModel` with a task type, please use a specific class for your task type. (e.g. `AutoPeftModelForCausalLM` for `task_type='CAUSAL_LM'`)"  # noqa: E501
            )
        ...
        if tokenizer_exists and hasattr(base_model, "get_input_embeddings"):
            ...
            try:
                ...
                embedding_size = base_model.get_input_embeddings().embedding_table.shape[0]

            if len(tokenizer) > embedding_size:
                # only resize if the tokenizer has a larger vocab size than there are embeddings
                # FIXME: use `base_model.resize_token_embeddings(len(tokenizer))` when it is available.
                pass
