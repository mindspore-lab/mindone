...
from mindone.peft.config import PeftConfig
from mindone.peft.mapping import PEFT_TYPE_TO_CONFIG_MAPPING, PEFT_TYPE_TO_PREFIX_MAPPING
from mindone.peft.tuners.lora import Conv2d, Linear, LoraConfig, LoraLayer

from .other import _get_subcell, get_pattern_key
...
def _update_scaling(lora_module, adapter_name, scaling=None):
    ...
    if isinstance(lora_module.scaling[adapter_name], ms.Tensor):
        ...
        raise ValueError(
            ...
            f"float or ms.Tensor, got {type(lora_module.scaling[adapter_name])} instead."
        )


def _convert_scalings_to_tensor(model) -> bool:
    """
    Convert the LoRA scaling values into ms.Tensors to prevent recompilation if they change.
    ...
    for _, module in model.cells_and_names():
        ...
        for key, val in scaling.items():
            if isinstance(val, float):
                ...
                scaling[key] = ms.Tensor(val)
            elif not isinstance(val, ms.Tensor):
                raise ValueError(
...
def _get_padded_linear(lora_module: nn.Cell, target_rank: int, is_lora_A: bool) -> mint.nn.Linear:
    """
    ...
    # For LoRA A, the "rank dimension" is weight.shape[0] (out_features).
    # For LoRA B, it is weight.shape[1] (in_features).
    original_rank = weight.shape[0] if is_lora_A else weight.shape[1]
    ...
    new_layer.weight.set_data(padded)
    # Copy bias if present
    if lora_module.bias is not None:
        new_layer.bias.set_data(lora_module.bias)

    return new_layer


def _get_padded_conv2d(lora_module: nn.Cell, target_rank: int, is_lora_A: bool) -> mint.nn.Conv2d:
    """
...
def _pad_lora_weights(model: nn.Cell, target_rank: int) -> bool:
    """
    Pad LoRA weights in a model to a target rank while preserving the original behavior.

    Args:
      model (nn.Cell): The model containing LoRA modules (with lora_A and lora_B).
...
def prepare_model_for_compiled_hotswap(
    ...
    *,
    ...
    config: Optional[Union[LoraConfig, dict[str, LoraConfig]]] = None,
    ...
    - you plan to run the model with JIT and want to avoid re-compilation
    ...
    Example:
        ...
        hotswap_adapter(model, path_adapter_1, adapter_name="default")
...
def hotswap_adapter_from_state_dict(
    model: nn.Cell,
    state_dict: dict[str, ms.Parameter],
    adapter_name: str,
    config: LoraConfig,
    ...
    for key in missing_keys:
        ...
        module = _get_subcell(model, module_name)
        old_val = attrgetter(key)(model)
        old_val.fill_(0.0)
    ...
    for key, new_val in state_dict.items():
        ...
        if old_val.shape == new_val.shape:
            ...
            if old_val.shape[0] > new_val.shape[0]:
                old_val.fill_(0)
                old_val[: new_val.shape[0]] = new_val
            elif old_val.shape[1] > new_val.shape[1]:
                old_val.fill_(0)
                old_val[:, : new_val.shape[1]] = new_val
            else:
                raise ValueError(
...
def hotswap_adapter(model, model_name_or_path, adapter_name, **kwargs):
    ...
    >>> from mindone.peft.utils.hotswap import hotswap_adapter
    ...
    >>> # replace the "default" lora adapter with the new one
    >>> hotswap_adapter(model, "path-adapter-1", adapter_name="default")
    ...
    state_dict = load_peft_weights(model_name_or_path, **kwargs)
