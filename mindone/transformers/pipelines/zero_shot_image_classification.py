...
class ZeroShotImageClassificationPipeline(Pipeline):
    ...
    def preprocess(
        ...
        if self.framework == "ms":
            for k, v in inputs.items():
                inputs[k] = ms.tensor(v).to(self.mindspore_dtype)
    ...
    def postprocess(self, model_outputs):
        ...
        if self.framework == "ms" and "siglip" in self.model.config.model_type:
            probs = mint.sigmoid(logits).squeeze(-1)
            scores = probs.tolist()
            if not isinstance(scores, list):
                scores = [scores]
        elif self.framework == "ms":
            probs = logits.softmax(axis=-1).squeeze(-1)
            scores = probs.tolist()
            if not isinstance(scores, list):
                scores = [scores]
        else:
