"""Adapted from https://github.com/huggingface/transformers/blob/main/src/transformers/pipelines/table_question_answering.py."""
...
class TableQuestionAnsweringPipeline(Pipeline):
    ...
    >>> oracle = pipeline(model="google/tapas-base-finetuned-wtq", revision="refs/pr/2")
    >>> table = {
        ...
        if self.framework == "ms":
            ...
            for index in range(batch_size):
                ...
                if prev_answers is not None:
                    ...
                    token_type_ids_example[:, 3] = ms.from_numpy(model_labels).type(ms.int64)
                ...
                probs = mint.sigmoid(logits)
                probabilities = probs * attention_mask_example.type(ms.float32)
            ...
            logits_batch = mint.cat(tuple(all_logits), 0)

            return (logits_batch,) if not self.aggregate else (logits_batch, mint.cat(tuple(all_aggregations), 0))
    ...
    def preprocess(self, pipeline_input, padding=True, truncation=None):
        ...
        inputs = self.tokenizer(table, query, return_tensors="np", truncation=truncation, padding=padding)
