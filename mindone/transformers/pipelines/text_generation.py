...
from ..utils import ModelOutput, is_mindspore_available
        ...
        if other_outputs:
            if self.framework == "ms":
                for k, v in other_outputs.items():
                    if isinstance(v, ms.Tensor) and v.shape[0] == len(generated_sequence):
