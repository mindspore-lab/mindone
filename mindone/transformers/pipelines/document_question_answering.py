...
import numpy as np
from transformers.utils import ExplicitEnum, add_end_docstrings, is_pytesseract_available, is_vision_available, logging
...
class DocumentQuestionAnsweringPipeline(ChunkPipeline):
    ...
    def preprocess(
        ...
        if input.get("image", None) is not None:
            ...
            if self.image_processor is not None:
                ...
                if self.framework == "ms":
                    image_inputs = image_inputs.to(self.mindspore_dtype)
        ...
        if self.model_type == ModelType.VisionEncoderDecoder:
            ...
            for span_idx in range(num_spans):
                if self.framework == "ms":
                    span_encoding = {k: ms.tensor(v[span_idx : span_idx + 1]) for (k, v) in encoding.items()}
                ...
                if "boxes" not in tokenizer_kwargs:
                    ...
                    if self.framework == "ms":
                        span_encoding["bbox"] = ms.tensor(bbox).unsqueeze(0)
    ...
    def postprocess_extractive_qa(
        ...
        for output in model_outputs:
            ...
            if self.framework == "ms" and output["start_logits"].dtype in (ms.bfloat16, ms.float16):
                output["start_logits"] = output["start_logits"].float()
            if self.framework == "ms" and output["end_logits"].dtype in (ms.bfloat16, ms.float16):
