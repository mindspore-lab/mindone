...
class TokenClassificationPipeline(ChunkPipeline):
    ...
    >>> token_classifier = pipeline("ner" ,model="Jean-Baptiste/camembert-ner", aggregation_strategy="simple")
    ...
    def preprocess(self, sentence, offset_mapping=None, **preprocess_params):
        ...
        for i in range(num_chunks):
            model_inputs = {k: ms.tensor(v[i]).unsqueeze(0) for k, v in inputs.items()}
            if offset_mapping is not None:
    ...
    def postprocess(self, all_outputs, aggregation_strategy=AggregationStrategy.NONE, ignore_labels=None):
        ...
        for model_outputs in all_outputs:
            if self.framework == "ms" and model_outputs["logits"][0].dtype in (ms.bfloat16, ms.float16):
                logits = model_outputs["logits"][0].to(ms.float32).asnumpy()
            else:
                logits = model_outputs["logits"][0].asnumpy()
            ...
            special_tokens_mask = model_outputs["special_tokens_mask"][0].asnumpy()
