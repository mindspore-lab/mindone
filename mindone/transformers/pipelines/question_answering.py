...
class QuestionAnsweringPipeline(ChunkPipeline):
    ...
    def postprocess(
        ...
        for output in model_outputs:
            if self.framework == "ms" and output["start"].dtype == ms.bfloat16:
                start_ = output["start"].to(ms.float32)
                end_ = output["end"].to(ms.float32)
            else:
                start_ = output["start"]
                end_ = output["end"]
            ...
            attention_mask = (
                output["attention_mask"].asnumpy() if output.get("attention_mask", None) is not None else None
            )
