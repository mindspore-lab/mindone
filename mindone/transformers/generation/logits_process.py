...
class LogitsProcessorList(list):
    ...
    def __call__(self, input_ids: ms.int64, scores: ms.Tensor, **kwargs) -> ms.Tensor:
        r"""
        Args:
...
class MinLengthLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, min_length: int, eos_token_id: Union[int, List[int], ms.Tensor], device: str = "cpu"):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        vocab_tensor = ops.arange(scores.shape[-1], device=scores.device)
...
class MinNewTokensLengthLogitsProcessor(LogitsProcessor):
    ...
    def __init__(
        ...
        if not isinstance(eos_token_id, ms.Tensor):
            if isinstance(eos_token_id, int):
                eos_token_id = [eos_token_id]
            eos_token_id = ms.Tensor(eos_token_id, device=device)
...
class RepetitionPenaltyLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        score = ops.gather(scores, input_ids, 1)

        # if score < 0 then repetition penalty has to be multiplied to reduce the token probabilities
        score = ops.where(score < 0, score * self.penalty, score / self.penalty)

        scores_processed = scores.scatter(1, input_ids, score[0])
        return scores_processed
...
class EncoderRepetitionPenaltyLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        score = ops.gather(scores, self.encoder_input_ids, 1)
...
class TopPLogitsWarper(LogitsWarper):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        # FIXME replace scatter with mindspor ops
        # indices_to_remove = sorted_indices_to_remove.scatter(1, sorted_indices, sorted_indices_to_remove)
        # method 1
        # indices_to_remove = sorted_indices_to_remove
        # for i in range(sorted_indices_to_remove.shape[0]):
        #     for j in range(sorted_indices_to_remove.shape[1]):
        #         indices_to_remove[i][sorted_indices[i][j]] = sorted_indices_to_remove[i][j]
        ...
        indices_to_remove = sorted_indices_to_remove.scatter(1, sorted_indices, sorted_indices_to_remove)
        indices_to_remove = indices_to_remove.astype(ms.bool_)
...
class MinPLogitsWarper(LogitsWarper):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        sorted_indices = ops.argsort(scores, descending=True, axis=-1)
        sorted_indices_to_remove = ops.gather(tokens_to_remove, axis=-1, input_indices=sorted_indices)
        # Keep at least min_tokens_to_keep
...
class EtaLogitsWarper(LogitsWarper):
    ...
    def __init__(
        ...
        self.epsilon = ms.Tensor(epsilon, device=device)
        self.filter_value = filter_value
        self.min_tokens_to_keep = min_tokens_to_keep
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        for sequence_ids, sequence_bias in self.sequence_bias.items():
            ...
            matching_rows = ops.eq(
                ...
                ms.Tensor(sequence_ids[:-1], dtype=input_ids.dtype, device=input_ids.device),
            ).prod(dim=1)
...
class InfNanRemoveLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        scores_processed = ops.where(scores == float("inf"), Tensor(np.finfo(scores.dtype).max), scores_processed)
        scores_processed = ops.where(scores == -float("inf"), Tensor(np.finfo(scores.dtype).min), scores_processed)
...
class LogitNormalization(LogitsProcessor, LogitsWarper):
    ...
    >>> print(torch.allclose(torch.sum(torch.exp(outputs.scores[-1])), ms.Tensor((1.000,)), rtol=1e-4))
    False
...
class SuppressTokensAtBeginLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, begin_suppress_tokens, begin_index, device: str = "cpu"):
        self.begin_suppress_tokens = ms.Tensor(list(begin_suppress_tokens), device=device)
        ...
        if self.unconditional_context["first_pass"]:
            ...
            attention_mask = ops.cat(
                [
                    ...
                    ops.ones_like(input_ids[:, -1:], dtype=ms.int64),
                ],
                axis=1,
            )
...
class BarkEosPrioritizerLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, eos_token_id: Union[int, List[int], ms.Tensor], min_eos_p: float, device: str = "cpu"):
        if not isinstance(eos_token_id, ms.Tensor):
            if isinstance(eos_token_id, int):
                eos_token_id = [eos_token_id]
            eos_token_id = ms.Tensor(eos_token_id)
...
class WatermarkLogitsProcessor(LogitsProcessor):
    ...
    def _get_greenlist_ids(self, input_seq: ms.Tensor) -> ms.Tensor:
        ...
        vocab_permutation = ops.randperm(self.vocab_size, generator=self.rng)
