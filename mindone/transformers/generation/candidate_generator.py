...
class AssistedCandidateGenerator(CandidateGenerator):
    ...
    def update_candidate_strategy(self, input_ids: ms.Tensor, scores: ms.Tensor, num_matches: int):
        ...
        # The assistant's confidence threshold is adjusted throughout the speculative iterations to reduce the number of unnecessary draft and target
        # forward passes. The costs are estimated based on the ROC curve, which considers the probability of the draft token and its match with the target.
    ...
    def _update_past_and_masks(self, input_ids: ms.Tensor, remove_from_pkv: int = 0, num_added_tokens: int = 1) -> bool:
...
class AssistedCandidateGeneratorDifferentTokenizers(AssistedCandidateGenerator):
    ...
    def _get_longest_diag_dict(input_matrix, nonzero_idx):
        ...
        Returns:
            dict: A dictionary where the keys are the indices of the non-zero elements and the values are the lengths of the longest diagonal sequences starting
             from those indices.
        """
...
class AssistantToTargetTranslator:
    ...
    def get_target_ids(self, assistant_input_ids, target_input_ids, assistant_candidate_ids: ms.Tensor) -> ms.Tensor:
        """
