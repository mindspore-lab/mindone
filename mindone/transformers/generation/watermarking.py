...
import numpy as np
from transformers.generation.configuration_utils import PretrainedConfig, WatermarkingConfig
from transformers.utils import ModelOutput, logging

import mindspore as ms
from mindspore import mint, nn
from mindspore.mint.nn import BCELoss
...
class WatermarkDetector:
    ...
    ```python
    >>> from transformers import AutoTokenizer, WatermarkDetector, WatermarkingConfig
    ...
    def __init__(
        ...
        self.processor = WatermarkLogitsProcessor(vocab_size=model_config.vocab_size, **watermarking_config)
...
class BayesianWatermarkDetectorModelOutput(ModelOutput):
    ...
    Args:
        ...
        posterior_probabilities (`ms.Tensor` of shape `(1,)`):
            Multiple choice classification loss.
    """

    loss: Optional[ms.Tensor] = None
    posterior_probabilities: Optional[ms.Tensor] = None


class BayesianDetectorWatermarkedLikelihood(nn.Cell):
    ...
    def __init__(self, watermarking_depth: int):
        ...
        super().__init__()
        self.watermarking_depth = watermarking_depth
        self.beta = ms.Parameter(-2.5 + 0.001 * mint.randn(1, 1, watermarking_depth))
        self.delta = ms.Parameter(0.001 * mint.randn(1, 1, self.watermarking_depth, watermarking_depth))

    def _compute_latents(self, g_values: ms.Tensor) -> Tuple[ms.Tensor, ms.Tensor]:
        """Computes the unique token probability distribution given g-values.

        Args:
            g_values (`ms.Tensor` of shape `(batch_size, seq_len, watermarking_depth)`):
                PRF values.
        ...
        x = mint.repeat_interleave(mint.unsqueeze(g_values, dim=-2), self.watermarking_depth, axis=-2)

        # mask all elements above -1 diagonal for autoregressive factorization
        x = mint.tril(x, diagonal=-1)
        ...
        p_two_unique_tokens = mint.sigmoid(logits)
        p_one_unique_token = 1 - p_two_unique_tokens
        return p_one_unique_token, p_two_unique_tokens

    def construct(self, g_values: ms.Tensor) -> ms.Tensor:
...
class BayesianDetectorModel(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.prior = ms.Parameter(ms.Tensor([self.base_rate]))
    ...
    def _compute_posterior(
        self,
        likelihoods_watermarked: ms.Tensor,
        likelihoods_unwatermarked: ms.Tensor,
        mask: ms.Tensor,
        prior: float,
    ) -> ms.Tensor:
        """
        ...
        Args:
            likelihoods_watermarked (`ms.Tensor` of shape `(batch, length, depth)`):
                Likelihoods P(g_values|watermarked) of g-values under watermarked model.
            likelihoods_unwatermarked (`ms.Tensor` of shape `(batch, length, depth)`):
                Likelihoods P(g_values|unwatermarked) of g-values under unwatermarked model.
            mask (`ms.Tensor` of shape `(batch, length)`):
        ...
        prior = mint.clamp(prior, min=1e-5, max=1 - 1e-5)
        log_likelihoods_watermarked = mint.log(mint.clamp(likelihoods_watermarked, min=1e-30, max=float("inf")))
        log_likelihoods_unwatermarked = mint.log(mint.clamp(likelihoods_unwatermarked, min=1e-30, max=float("inf")))
        ...
        relative_surprisal_likelihood = mint.einsum("i...->i", log_odds * mask)

        # Compute the relative surprisal prior
        relative_surprisal_prior = mint.log(prior) - mint.log(1 - prior)
        ...
        return mint.sigmoid(relative_surprisal)
    ...
    def construct(
        ...
        likelihoods_unwatermarked = 0.5 * mint.ones_like(g_values)
        ...
        if labels is not None:
            loss_fct = BCELoss()
            loss_unwweight = mint.sum(self.likelihood_model_watermarked.delta**2)
            loss_weight = loss_unwweight * loss_batch_weight
            loss = loss_fct(mint.clamp(out, 1e-5, 1 - 1e-5), labels) + loss_weight
...
class SynthIDTextWatermarkDetector:
    ...
    ... )
    >>> from mindone.transformers.generation.logits_process import SynthIDTextWatermarkLogitsProcessor
    >>> from mindone.transformers.generation.watermarking import SynthIDTextWatermarkDetector
    ...
    def __call__(self, tokenized_outputs: ms.Tensor):
