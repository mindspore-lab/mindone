...
def translate_gt(gt: ms.Tensor, max_num_bins: int, reg_scale: int, up: ms.Tensor):
    """
    ...
    mask = diffs <= 0
    closest_left_indices = mint.sum(mask, dim=1) - 1

    # Calculate the weights for the interpolation
    indices = closest_left_indices.float()

    weight_right = mint.zeros_like(indices)
    weight_left = mint.zeros_like(indices)
    ...
    left_diffs = mint.abs(gt[valid_idx_mask] - left_values)
    right_diffs = mint.abs(right_values - gt[valid_idx_mask])
...
def bbox2distance(points, bbox, max_num_bins, reg_scale, up, eps=0.1):
    ...
    four_lens = mint.stack([left, top, right, bottom], -1)
...
class DFineLoss(RTDetrLoss):
    ...
    def loss_local(self, outputs, targets, indices, num_boxes, T=5):
        ...
        if "pred_corners" in outputs:
            ...
            ious = mint.diag(
                box_iou(center_to_corners_format(outputs["pred_boxes"][idx]), center_to_corners_format(target_boxes))[0]
            )
            ...
            if mint.equal(pred_corners, target_corners):
                losses["loss_ddf"] = pred_corners.sum() * 0
            else:
                weight_targets_local = outputs["teacher_logits"].sigmoid().max(dim=-1)[0]
                mask = mint.zeros_like(weight_targets_local, dtype=ms.bool_)
                mask[idx] = True
    ...
    **kwargs,
    ...
    if config.auxiliary_loss:
        if denoising_meta_values is not None:
            dn_out_coord, outputs_coord = mint.split(
                outputs_coord.clamp(min=0, max=1), denoising_meta_values["dn_num_split"], dim=2
            )
            dn_out_class, outputs_class = mint.split(outputs_class, denoising_meta_values["dn_num_split"], dim=2)
            dn_out_corners, out_corners = mint.split(predicted_corners, denoising_meta_values["dn_num_split"], dim=2)
            dn_out_refs, out_refs = mint.split(initial_reference_points, denoising_meta_values["dn_num_split"], dim=2)
