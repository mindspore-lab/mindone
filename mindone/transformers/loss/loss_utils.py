...
def fixed_cross_entropy(
    source: ms.Tensor,
    target: ms.Tensor,
    num_items_in_batch: Optional[ms.Tensor] = None,
...
def ForCausalLMLoss(
    ...
    num_items_in_batch: Optional[ms.Tensor] = None,
    ignore_index: int = -100,
    shift_labels: Optional[ms.Tensor] = None,
    **kwargs,
) -> ms.Tensor:
...
def ForSequenceClassificationLoss(labels: ms.Tensor, pooled_logits: ms.Tensor, config, **kwargs) -> ms.Tensor:
    num_labels = config.num_labels
    if config.problem_type is None:
        if num_labels == 1:
            config.problem_type = "regression"
        elif num_labels > 1 and (labels.dtype in (ms.int64, ms.int32)):
...
def ForTokenClassification(logits: ms.Tensor, labels, config, **kwargs):
    # Upcast to float if we need to compute the loss to avoid potential precision issues
