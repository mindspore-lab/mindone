...
class GroundingDinoHungarianMatcher(HungarianMatcher):
    def construct(self, outputs, targets):
        ...
        label_maps = mint.cat([label_map[target["class_labels"]] for label_map, target in zip(label_maps, targets)])
...
class GroundingDinoImageLoss(ImageLoss):
    ...
    def _get_target_classes_one_hot(self, outputs, targets, indices):
        ...
        label_maps = mint.cat(outputs["label_maps"], dim=0)

        idx = self._get_source_permutation_idx(indices)
        target_classes_onehot = mint.zeros_like(logits, dtype=ms.int64)
        target_classes_onehot[idx] = label_maps[class_labels].to(ms.int64)
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes):
        ...
        source_logits = mint.masked_select(source_logits, text_mask)
        target_classes_onehot = mint.masked_select(target_classes_onehot, text_mask)
