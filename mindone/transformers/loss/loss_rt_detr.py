...
class RTDetrHungarianMatcher(nn.Cell):
...
class RTDetrLoss(nn.Cell):
    ...
    def __init__(self, config):
        ...
        empty_weight = mint.ones(config.num_labels + 1)
    ...
    def loss_labels_vfl(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target_classes_original = mint.cat([_target["class_labels"][i] for _target, (_, i) in zip(targets, indices)])
        target_classes = mint.full(src_logits.shape[:2], self.num_classes, dtype=mint.int64)
        target_classes[idx] = target_classes_original
        target = F.one_hot(target_classes, num_classes=self.num_classes + 1)[..., :-1]

        target_score_original = mint.zeros_like(target_classes, dtype=src_logits.dtype)
        target_score_original[idx] = ious.to(target_score_original.dtype)
        target_score = target_score_original.unsqueeze(-1) * target

        pred_score = ops.stop_gradient(F.sigmoid(src_logits))
    ...
    def get_cdn_matched_indices(dn_meta, targets):
        ...
        for i, num_gt in enumerate(num_gts):
            if num_gt > 0:
                gt_idx = mint.arange(num_gt, dtype=ms.int64)
    ...
    def construct(self, outputs, targets):
        ...
        num_boxes = mint.clamp(num_boxes, min=1).item()
