...
class ProcessorMixin(PushToHubMixin):
    ...
    def save_pretrained(self, save_directory, push_to_hub: bool = False, legacy_serialization=True, **kwargs):
        """
    ...
    @deprecate_kwarg(
        "video_load_backend",
        version="4.59",
        additional_message=". This function will use `mindspore` by default, or `pyav` if `mindspore` is not installed.",
    )
