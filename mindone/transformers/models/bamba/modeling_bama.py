    ...
    def __init__(self, config: BambaConfig, batch_size, dtype=ms.float16):
        ...
        for i in range(config.num_hidden_layers):
            if self.layers_block_type[i] == "mamba":
                ...
                self.ssm_states += [
                    mint.zeros(
                        (batch_size, config.mamba_n_heads, config.mamba_d_head, ssm_state_size),
                        dtype=dtype,
                    )
                ]
            else:
...
class BambaRMSNormGated(nn.Cell):
...
class BambaMixer(nn.Cell):
    ...
    def __init__(self, config: BambaConfig, layer_idx: int):
        ...
        self.dt_bias = Parameter(mint.ones(self.num_heads))
        ...
        self.D = Parameter(mint.ones(self.num_heads))
    ...
    def cuda_kernels_forward(
        ...
        if use_precomputed_states:
            ...
            A = A[:, None, ...][:, :, None].broadcast_to((-1, self.head_dim, self.ssm_state_size)).to(dtype=ms.float32)
            dt = dt[:, :, None].broadcast_to((-1, -1, self.head_dim))
            dt_bias = self.dt_bias[:, None, ...].broadcast_to((-1, self.head_dim))
            D = self.D[:, None, ...].broadcast_to((-1, self.head_dim))
            ...
            if self.training and cache_params is None:
                ...
                if self.activation not in ["silu", "swish"]:
                    hidden_states_B_C = self.act(
                        self.conv1d(hidden_states_B_C.swapaxes(1, 2))[..., :seq_len].swapaxes(1, 2)
                    )
                else:
                    hidden_states_B_C = causal_conv1d_fn(
                        x=hidden_states_B_C.swapaxes(1, 2),
...
class BambaDecoderLayer(nn.Cell):
...
class BambaForCausalLM(BambaPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs,
        ...
        >>> from mindone.transformers import BambaForCausalLM
