...
class MobileViTConvLayer(mindspore.nn.Cell):
    def __init__(
        self,
        ...
        self.convolution = mindspore.mint.nn.Conv2d(
        ...
        if use_normalization:
            self.normalization = mindspore.mint.nn.BatchNorm2d(
...
class MobileViTInvertedResidual(mindspore.nn.Cell):
    """
...
class MobileViTMobileNetLayer(mindspore.nn.Cell):
    def __init__(
...
class MobileViTSelfAttention(mindspore.nn.Cell):
    def __init__(self, config: MobileViTConfig, hidden_size: int) -> None:
        ...
        self.query = mindspore.mint.nn.Linear(hidden_size, self.all_head_size, bias=config.qkv_bias)
        self.key = mindspore.mint.nn.Linear(hidden_size, self.all_head_size, bias=config.qkv_bias)
        self.value = mindspore.mint.nn.Linear(hidden_size, self.all_head_size, bias=config.qkv_bias)
...
class MobileViTSelfOutput(mindspore.nn.Cell):
    def __init__(self, config: MobileViTConfig, hidden_size: int) -> None:
        super().__init__()
        self.dense = mindspore.mint.nn.Linear(hidden_size, hidden_size)
...
class MobileViTAttention(mindspore.nn.Cell):
    def __init__(self, config: MobileViTConfig, hidden_size: int) -> None:
        super().__init__()
...
class MobileViTIntermediate(mindspore.nn.Cell):
    def __init__(self, config: MobileViTConfig, hidden_size: int, intermediate_size: int) -> None:
        super().__init__()
        self.dense = mindspore.mint.nn.Linear(hidden_size, intermediate_size)
...
class MobileViTOutput(mindspore.nn.Cell):
    def __init__(self, config: MobileViTConfig, hidden_size: int, intermediate_size: int) -> None:
        super().__init__()
        self.dense = mindspore.mint.nn.Linear(intermediate_size, hidden_size)
        self.dropout = mindspore.mint.nn.Dropout(config.hidden_dropout_prob)
...
class MobileViTTransformerLayer(mindspore.nn.Cell):
    def __init__(self, config: MobileViTConfig, hidden_size: int, intermediate_size: int) -> None:
        super().__init__()
        ...
        self.layernorm_before = mindspore.mint.nn.LayerNorm(hidden_size, eps=config.layer_norm_eps)
        self.layernorm_after = mindspore.mint.nn.LayerNorm(hidden_size, eps=config.layer_norm_eps)
...
class MobileViTTransformer(mindspore.nn.Cell):
...
class MobileViTLayer(mindspore.nn.Cell):
    """
    ...
    def __init__(
        ...
        self.layernorm = mindspore.mint.nn.LayerNorm(hidden_size, eps=config.layer_norm_eps)
    ...
    def unfolding(self, features: mindspore.Tensor) -> tuple[mindspore.Tensor, dict]:
        ...
        if new_width != orig_width or new_height != orig_height:
            ...
            features = mindspore.mint.nn.functional.interpolate(
    ...
    def folding(self, patches: mindspore.Tensor, info_dict: dict) -> mindspore.Tensor:
        patch_width, patch_height = self.patch_width, self.patch_height
        patch_area = int(patch_width * patch_height)
    ...
    def construct(self, features: mindspore.Tensor) -> mindspore.Tensor:
        ...
        features = self.fusion(mindspore.mint.cat((residual, features), dim=1))
        return features


class MobileViTEncoder(mindspore.nn.Cell):
...
class MobileViTModel(MobileViTPreTrainedModel):
    ...
    def construct(
        ...
        if self.expand_output:
            ...
            pooled_output = mindspore.mint.mean(last_hidden_state, dim=[-2, -1], keepdim=False)
        else:
...
class MobileViTForImageClassification(MobileViTPreTrainedModel):
    def __init__(self, config: MobileViTConfig) -> None:
        ...
        self.classifier = (
            mindspore.mint.nn.Linear(config.neck_hidden_sizes[-1], config.num_labels)
...
class MobileViTASPPPooling(mindspore.nn.Cell):
    def __init__(self, config: MobileViTConfig, in_channels: int, out_channels: int) -> None:
        super().__init__()

        self.global_pool = mindspore.mint.nn.AdaptiveAvgPool2d(output_size=1)
...
class MobileViTASPP(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: MobileViTConfig) -> None:
        ...
        self.dropout = mindspore.mint.nn.Dropout(p=config.aspp_dropout_prob)

    def construct(self, features: mindspore.Tensor) -> mindspore.Tensor:
        pyramid = []
        for conv in self.convs:
            pyramid.append(conv(features))
        pyramid = mindspore.mint.cat(pyramid, dim=1)
...
class MobileViTDeepLabV3(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: MobileViTConfig) -> None:
        ...
        self.dropout = mindspore.mint.nn.Dropout2d(config.classifier_dropout_prob)
...
class MobileViTForSemanticSegmentation(MobileViTPreTrainedModel):
    ...
    def construct(
        ...
        if labels is not None:
            ...
            upsampled_logits = mindspore.mint.nn.functional.interpolate(
