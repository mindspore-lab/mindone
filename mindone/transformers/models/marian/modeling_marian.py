...
class MarianSinusoidalPositionalEmbedding(mint.nn.Embedding):
    ...
    def _init_weight(self):
        ...
        out = mint.empty(n_pos, dim, dtype=self.weight.dtype)
        sentinel = dim // 2 if dim % 2 == 0 else (dim // 2) + 1
        out[:, 0:sentinel] = ms.Tensor(np.sin(position_enc[:, 0::2]))
        out[:, sentinel:] = ms.Tensor(np.cos(position_enc[:, 1::2]))
        self.weight = ms.Parameter(out)

    def construct(
        self, input_ids_shape: ops.Size, past_key_values_length: int = 0, position_ids: Optional[ms.Tensor] = None
    ) -> ms.Tensor:
        """`input_ids_shape` is expected to be [bsz x seqlen]."""
        if position_ids is None:
            bsz, seq_len = input_ids_shape[:2]
            position_ids = mint.arange(past_key_values_length, past_key_values_length + seq_len, dtype=ms.int64)
...
class MarianEncoderLayer(nn.Cell):
    ...
    def construct(
        ...
        if hidden_states.dtype == ms.float16 and (mint.isinf(hidden_states).any() or ops.isnan(hidden_states).any()):
...
class MarianPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: Union[mint.nn.Linear, mint.nn.Embedding, MarianSinusoidalPositionalEmbedding]):
...
class MarianEncoder(MarianPreTrainedModel):
    ...
    def __init__(self, config: MarianConfig, embed_tokens: Optional[mint.nn.Embedding] = None):
...
class MarianDecoder(MarianPreTrainedModel):
    ...
    def __init__(self, config: MarianConfig, embed_tokens: Optional[mint.nn.Embedding] = None):
        ...
        if embed_tokens is not None:
            ...
            self.embed_tokens = mint.nn.Embedding(config.decoder_vocab_size, config.d_model, self.padding_idx)

        self.embed_positions = MarianSinusoidalPositionalEmbedding(
            config.max_position_embeddings, config.d_model, self.padding_idx
        )
        self.layers = nn.CellList([MarianDecoderLayer(config, layer_idx=i) for i in range(config.decoder_layers)])
...
class MarianModel(MarianPreTrainedModel):
    ...
    def resize_decoder_token_embeddings(self, new_num_tokens: int) -> mint.nn.Embedding:
        if self.config.share_encoder_decoder_embeddings:
            raise ValueError(
    ...
    def construct(
        ...
        cross_attn_head_mask: Optional[ms.Tensor] = None,
        encoder_outputs: Optional[Union[tuple[ms.Tensor], BaseModelOutput]] = None,
        past_key_values: Optional[tuple[tuple[ms.Tensor]]] = None,
        inputs_embeds: Optional[ms.Tensor] = None,
...
class MarianMTModel(MarianPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: MarianConfig):
        ...
        self.register_buffer("final_logits_bias", mint.zeros((1, target_vocab_size)))
        self.lm_head = mint.nn.Linear(config.d_model, target_vocab_size, bias=False)
    ...
    def _resize_token_embeddings(self, new_num_tokens: int, pad_to_multiple_of=None, *args) -> mint.nn.Embedding:
    ...
    def set_output_embeddings(self, new_embeddings: mint.nn.Embedding):
