...
class VitPoseBackbonePatchEmbeddings(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.projection = mint.nn.Conv2d(num_channels, embed_dim, kernel_size=patch_size, stride=patch_size, padding=2)
...
class VitPoseBackboneEmbeddings(nn.Cell):
    """
...
class VitPoseBackboneSelfAttention(nn.Cell):
    def __init__(self, config: VitPoseBackboneConfig) -> None:
        super().__init__()
    ...
    def construct(
        ...
        if self.config._attn_implementation != "eager":
            if self.config._attn_implementation == "sdpa" and output_attentions:
                logger.warning_once(
                    "does not have `mint.nn.functional.scaled_dot_product_attention` and does not support `output_attentions=True`. Falling back to "
...
class VitPoseBackboneSelfOutput(nn.Cell):
    """
...
class VitPoseBackboneAttention(nn.Cell):
    def __init__(self, config: VitPoseBackboneConfig) -> None:
        super().__init__()
...
class VitPoseBackboneMoeMLP(nn.Cell):
    ...
    def construct(self, hidden_state: ms.Tensor, indices: ms.Tensor) -> ms.Tensor:
        expert_hidden_state = mint.zeros_like(hidden_state[:, :, -self.part_features :])
        ...
        hidden_state = mint.cat([shared_hidden_state, expert_hidden_state], dim=-1)

        return hidden_state


class VitPoseBackboneMLP(nn.Cell):
    def __init__(self, config: VitPoseBackboneConfig) -> None:
        super().__init__()
        in_features = out_features = config.hidden_size
        hidden_features = int(config.hidden_size * config.mlp_ratio)
...
class VitPoseBackboneLayer(nn.Cell):
    def __init__(self, config: VitPoseBackboneConfig) -> None:
        super().__init__()
...
class VitPoseBackboneEncoder(nn.Cell):
    def __init__(self, config: VitPoseBackboneConfig) -> None:
        super().__init__()
        self.config = config
        self.layer = nn.CellList([VitPoseBackboneLayer(config) for _ in range(config.num_hidden_layers)])
...
class VitPoseBackbone(VitPoseBackbonePreTrainedModel, BackboneMixin):
    ...
    def construct(
        ...
        ```python
        >>> from transformers import VitPoseBackboneConfig
        >>> from mindone.transformers import VitPoseBackbone
        >>> import mindspore as ms

        >>> config = VitPoseBackboneConfig(out_indices=[-1])
        >>> model = VitPoseBackbone(config)

        >>> pixel_values = ms.ops.randn(1, 3, 256, 192)
