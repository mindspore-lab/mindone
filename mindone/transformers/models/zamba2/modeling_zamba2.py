...
class Zamba2RMSNormGated(nn.Cell):
    ...
    def construct(self, hidden_states, gate=None):
        ...
        *prefix_dims, last_dim = hidden_states.shape
        ...
        hidden_states_group = hidden_states_group * mint.rsqrt(variance + self.variance_epsilon)
        hidden_states = hidden_states_group.view(*prefix_dims, group_count * self.group_size)
        return self.weight * hidden_states.to(input_dtype)


class Zamba2RMSNorm(nn.Cell):
...
class Zamba2HybridDynamicCache(Cache):
    ...
    def __init__(
        self, config: Zamba2Config, batch_size: int, dtype: ms.dtype = ms.float16, device: Optional[str] = None
    ):
        self.dtype = dtype
        ...
        for i in range(config.num_hidden_layers):
            ...
            self.ssm_states[i] = mint.zeros(
                (batch_size, self.n_mamba_heads, config.mamba_headdim, self.ssm_state_size), dtype=dtype
            )
...
class Zamba2RotaryEmbedding(nn.Cell):
    def __init__(
        self,
        config: Zamba2Config,
        device=None,
...
class Zamba2Attention(nn.Cell):
    ...
    attn_weights = mint.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.head_dim) with
    attn_weights = mint.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.head_dim/2)
    ...
    def __init__(
        ...
        self.q_proj = mint.nn.Linear(
            config.attention_hidden_size, config.num_attention_heads * self.head_dim, bias=False
        )
        self.k_proj = mint.nn.Linear(
            config.attention_hidden_size, config.num_key_value_heads * self.head_dim, bias=False
        )
        ...
        if config.use_shared_attention_adapter:
            self.linear_q_adapter_list = nn.CellList([])
            self.linear_k_adapter_list = nn.CellList([])
            self.linear_v_adapter_list = nn.CellList([])

            for i in range(self.num_fwd_mem_blocks):
                if i % config.num_mem_blocks == block_id:
                    linear_q_adapter = nn.SequentialCell(
                        mint.nn.Linear(self.attention_hidden_size, self.config.adapter_rank, bias=False),
                        mint.nn.Linear(self.config.adapter_rank, self.attention_hidden_size, bias=False),
                    )
                    linear_k_adapter = nn.SequentialCell(
                        mint.nn.Linear(self.attention_hidden_size, self.config.adapter_rank, bias=False),
                        mint.nn.Linear(self.config.adapter_rank, self.attention_hidden_size, bias=False),
                    )
                    linear_v_adapter = nn.SequentialCell(
                        mint.nn.Linear(self.attention_hidden_size, self.config.adapter_rank, bias=False),
                        mint.nn.Linear(self.config.adapter_rank, self.attention_hidden_size, bias=False),
                    )
                else:
                    linear_q_adapter = mint.nn.Identity()
                    linear_k_adapter = mint.nn.Identity()
                    linear_v_adapter = mint.nn.Identity()
...
class Zamba2MambaMixer(nn.Cell):
    """
    ...
    def __init__(self, config: Zamba2Config, layer_idx: Optional[int] = None):
        ...
        self.out_proj = mint.nn.Linear(self.intermediate_size, self.hidden_size, bias=config.add_bias_linear)
    ...
    def mindspore_forward(
        ...
        if cache_params is not None and cache_params.has_previous_state:
            ...
            if attention_mask is not None and not mint.all(attention_mask == 1):
        ...
        if cache_params is not None:
            ...
            if cache_params.has_previous_state:
                ...
                conv_state = mint.roll(conv_state, shifts=-1, dims=-1)
                ...
                hidden_states = self.act(hidden_states).to(dtype)[
                    :, None, ...
                ...
                cache_params.conv_states[self.layer_idx].copy_(conv_state)
                hidden_states = self.act(self.conv1d(hidden_states).transpose(1, 2))[
                    :, :seq_len, :
            ...
            ssm_state = mint.zeros((batch_size, self.num_heads, self.head_dim, self.ssm_state_size), dtype=dtype)
        ...
        if cache_params is not None and cache_params.has_previous_state:
            ...
            dt = mint.clamp(dt, self.time_step_min)  # , self.time_step_max)
            ...
            dt = mint.clamp(dt, self.time_step_min)
    ...
    def construct(
        ...
        return self.mindspore_forward(hidden_states, cache_params, attention_mask)


class Zamba2MLP(nn.Cell):
    def __init__(self, config: Zamba2Config, num_fwd_mem_blocks=None, block_id: Optional[int] = None):
        ...
        is tied, un-tied adapter modules (formally same as LoRA, but used in the base model) are added to the up and gate projectors to
        increase expressivity with a small memory overhead.
        """
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        ...
        self.gate_up_proj = mint.nn.Linear(self.hidden_size, 2 * self.intermediate_size, bias=config.add_bias_linear)
        self.down_proj = mint.nn.Linear(self.intermediate_size, self.hidden_size, bias=config.add_bias_linear)
        self.act_fn = ACT2FN[config.hidden_act]

        self.gate_up_proj_adapter_list = nn.CellList([])
        for i in range(self.num_fwd_mem_blocks):
            if i % config.num_mem_blocks == block_id:
                gate_up_proj_adapter = nn.SequentialCell(
                    mint.nn.Linear(self.config.hidden_size, self.config.adapter_rank, bias=False),
                    mint.nn.Linear(self.config.adapter_rank, 2 * self.intermediate_size, bias=False),
                )
            else:
                gate_up_proj_adapter = mint.nn.Identity()
    ...
    def construct(self, hidden_state, layer_idx=None):
...
class Zamba2AttentionDecoderLayer(nn.Cell):
    ...
    def construct(
        self,
        hidden_states: ms.Tensor,
        original_hidden_states: ms.Tensor,
        layer_idx: int,
        attention_mask: Optional[ms.Tensor] = None,
        past_key_value: Optional[Zamba2HybridDynamicCache] = None,
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        """
        Args:
            hidden_states (`ms.Tensor`): output of previous Mamba layer of shape `(batch, seq_len, embed_dim)`
            original_hidden_states (`ms.Tensor`): word embedding output of shape `(batch, seq_len, embed_dim)`.
        ...
        """
        hidden_states = mint.concat([hidden_states, original_hidden_states], dim=-1)
...
class Zamba2MambaDecoderLayer(nn.Cell):
    ...
    def construct(
        self,
        hidden_states: ms.Tensor,
        original_hidden_states: Optional[ms.Tensor] = None,
        layer_idx: Optional[int] = None,
        attention_mask: Optional[ms.Tensor] = None,
        causal_mask: Optional[ms.Tensor] = None,
        past_key_value: Optional[Zamba2HybridDynamicCache] = None,
        output_attentions: Optional[bool] = False,
        use_cache: Optional[bool] = False,
        cache_position: Optional[ms.Tensor] = None,
        transformer_hidden_states: Optional[ms.Tensor] = None,
        **kwargs,
    ) -> Tuple[ms.Tensor, Optional[Tuple[ms.Tensor, ms.Tensor]]]:
        """
        Args:
...
class Zamba2HybridLayer(nn.Cell):
    def __init__(
    ...
    def construct(
        ...
        Args:
            hidden_states (`ms.Tensor`): input to the layer of shape `(batch, seq_len, embed_dim)`
            original_hidden_states (`ms.Tensor`): word embedding output that will be concatenated with
...
class Zamba2PreTrainedModel(PreTrainedModel):
    ...
    _supports_cache_class = False  # Note: only supports Zamba2HybridDynamicCache
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (mint.nn.Linear, mint.nn.Conv1d)):
            ...
            dt = mint.exp(
                mint.rand(self.config.n_mamba_heads)
            ...
            A = mint.arange(1, module.num_heads + 1)
            module.A_log.data.copy_(mint.log(A))
...
class Zamba2Model(Zamba2PreTrainedModel):
    ...
    def __init__(self, config: Zamba2Config):
        ...
        for i in range(config.num_hidden_layers):
            if config.layers_block_type[i] == "mamba":
                ...
                linear_layers.append(mint.nn.Linear(self.config.hidden_size, self.config.hidden_size, bias=False))
                mamba_layers.append(Zamba2MambaDecoderLayer(config, layer_idx=i))
    ...
    def construct(
        ...
        original_hidden_states = mint.clone(inputs_embeds)
        ...
        if use_cache and past_key_values is None:
            ...
            past_key_values = Zamba2HybridDynamicCache(self.config, batch_size, dtype=self.dtype)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if not empty_past_kv:
            ...
            past_key_values = Zamba2HybridDynamicCache(self.config, input_ids.shape[0], dtype=self.dtype)
