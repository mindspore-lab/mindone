...
SPEECH_ENCODER_DECODER_INPUTS_DOCSTRING = r"""
    Args:
        inputs (`ms.Tensor` of shape `(batch_size, sequence_length)` or `(batch_size, sequence_length, feature_dim)`, *optional*):
        ...
        output_hidden_states (`bool`, *optional*):
            ...
            more detail.
        input_values (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            Float values of input raw speech waveform. Values can be obtained by loading a *.flac* or *.wav* audio file
            into an array of type *List[float]* or a *numpy.ndarray*, *e.g.* via the soundfile library (*pip install
            soundfile*). To prepare the array into *input_values*, the [`Wav2Vec2Processor`] should be used for padding
            and conversion into a tensor of type *ms.Tensor*. See [`Wav2Vec2Processor.__call__`] for details.
        input_features (`ms.Tensor` of shape `(batch_size, sequence_length, feature_size)`, *optional*):
            ...
            into a tensor of type `ms.Tensor`. See [`~Speech2TextFeatureExtractor.__call__`]
        return_dict (`bool`, *optional*):
            If set to `True`, the model will return a [`~utils.Seq2SeqLMOutput`] instead of a plain tuple.
        kwargs (*optional*): Remaining dictionary of keyword arguments. Keyword arguments come in two flavors:

            - Without a prefix which will be input as `**encoder_kwargs` for the encoder construct function.
            - With a *decoder_* prefix which will be input as `**decoder_kwargs` for the decoder construct function.
"""
