...
class DeiTEmbeddings(nn.Cell):
    ...
    def __init__(self, config: DeiTConfig, use_mask_token: bool = False) -> None:
        ...
        self.distillation_token = ms.Parameter(mint.zeros((1, 1, config.hidden_size)), name="distillation_token")
        self.mask_token = (
            ms.Parameter(mint.zeros((1, 1, config.hidden_size)), name="mask_token") if use_mask_token else None
        )
        self.patch_embeddings = DeiTPatchEmbeddings(config)
        num_patches = self.patch_embeddings.num_patches
        self.position_embeddings = ms.Parameter(
            mint.zeros((1, num_patches + 2, config.hidden_size)), name="position_embeddings"
        )
    ...
    def interpolate_pos_encoding(self, embeddings: ms.Tensor, height: int, width: int) -> ms.Tensor:
        ...
        return mint.cat((class_and_dist_pos_embed, patch_pos_embed), dim=1)
    ...
    def construct(
        ...
        embeddings = mint.cat((cls_tokens, distillation_tokens, embeddings), dim=1)
...
class DeiTForMaskedImageModeling(DeiTPreTrainedModel):
    ...
    def construct(
        ...
        >>> from mindone.transformers import DeiTForMaskedImageModeling
        >>> import mindspore as ms
        ...
        >>> image_processor = AutoImageProcessor.from_pretrained("facebook/deit-base-distilled-patch16-224", revision="refs/pr/2")
        >>> model = DeiTForMaskedImageModeling.from_pretrained("facebook/deit-base-distilled-patch16-224", revision="refs/pr/2")
        ...
        if bool_masked_pos is not None:
            ...
            mask = (
                # bool input is not supported in tensor.repeat_interleave
                bool_masked_pos.int()
...
class DeiTForImageClassification(DeiTPreTrainedModel):
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoImageProcessor
        >>> from mindone.transformers import DeiTForImageClassification
        >>> import mindspore as ms
        >>> from PIL import Image
        >>> import requests
        ...
        >>> image_processor = AutoImageProcessor.from_pretrained("facebook/deit-base-distilled-patch16-224", revision="refs/pr/2")
        >>> model = DeiTForImageClassification.from_pretrained("facebook/deit-base-distilled-patch16-224", revision="refs/pr/2")
...
class DeiTForImageClassificationWithTeacherOutput(ModelOutput):
    ...
    distillation_logits: Optional[ms.Tensor] = None
