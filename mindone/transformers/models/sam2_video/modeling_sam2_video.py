...
class Sam2VideoInferenceSession:
    ...
    def __init__(
        ...
        self.processed_frames = dict(enumerate(video.to(dtype=dtype))) if video is not None else None
    ...
    def add_mask_inputs(self, obj_idx: int, frame_idx: int, inputs: ms.Tensor):
        ...
        self.mask_inputs_per_obj[obj_idx][frame_idx] = inputs.to(dtype=self.dtype, non_blocking=True)
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, Sam2VideoModel):
            if module.no_memory_positional_encoding is not None:
                module.no_memory_positional_encoding.set_data(
                    initializer(
                        Zero(), module.no_memory_positional_encoding.shape, module.no_memory_positional_encoding.dtype
                    )
                )
            if module.memory_temporal_positional_encoding is not None:
                module.memory_temporal_positional_encoding.set_data(
                    initializer(
                        Zero(),
                        module.memory_temporal_positional_encoding.shape,
                        module.memory_temporal_positional_encoding.dtype,
                    )
                )
            if module.no_object_pointer is not None:
                module.no_object_pointer.set_data(
                    initializer(Zero(), module.no_object_pointer.shape, module.no_object_pointer.dtype)
                )
            if module.occlusion_spatial_embedding_parameter is not None:
                module.occlusion_spatial_embedding_parameter.set_data(
                    initializer(
                        Zero(),
                        module.occlusion_spatial_embedding_parameter.shape,
                        module.occlusion_spatial_embedding_parameter.dtype,
                    )
                )
...
class Sam2VideoMemoryFuser(nn.Cell):
    def __init__(self, config: Sam2VideoConfig):
        super().__init__()
        self.layers = nn.CellList([Sam2VideoMemoryFuserCXBlock(config) for _ in range(config.memory_fuser_num_layers)])
...
class Sam2VideoModel(Sam2VideoPreTrainedModel):
    ...
    def __init__(self, config: Sam2VideoConfig):
        ...
        self.no_memory_positional_encoding = ms.Parameter(mint.zeros((1, 1, config.vision_config.fpn_hidden_size)))
        ...
        self.memory_temporal_positional_encoding = ms.Parameter(mint.zeros((self.num_maskmem, 1, 1, self.mem_dim)))
    ...
    def _get_object_pointers(
        ...
        for t_diff_offset in range(1, max_object_pointers_to_use):
            ...
            # check if the output is already stored without using get_output to avoid unnecessary memory transfers between CPU and GPU
            out_data = inference_session.output_dict_per_obj[obj_idx]["non_cond_frame_outputs"].get(ref_frame_idx, None)
    ...
    def _process_object_pointers(
        ...
        if self.config.enable_temporal_pos_encoding_for_object_pointers:
            ...
            normalized_temporal_diffs = ms.tensor(temporal_offsets, dtype=ms.float32) / max_temporal_diff
    ...
    def _prepare_memory_conditioned_features(
        ...
        if self.num_maskmem == 0:
            ...
            current_feature_map = current_vision_features.permute(1, 2, 0).view(batch_size, num_channels, height, width)
    ...
    def _run_single_frame_inference(
        ...
        if point_inputs is not None and mask_inputs is not None:
            raise ValueError("point_inputs and mask_inputs should not appear as input simultaneously on the same frame")
