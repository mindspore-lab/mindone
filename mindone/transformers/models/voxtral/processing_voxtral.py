...
class VoxtralProcessor(ProcessorMixin):
    ...
    def _retreive_input_features(self, audio, max_source_positions, **kwargs):
        """
        Handles specific logic of Voxtral expected input features: audio arrays should be padded to next multiple of 480000 (duration is a multiple of 30s),
        see VoxtralProcessorKwargs' default audio_kwargs.
        ...
        **kwargs: Unpack[AllKwargsForChatTemplate],
        ...
        if kwargs.get("continue_final_message", False):
            if kwargs.get("add_generation_prompt", False):
                raise ValueError(
                    "continue_final_message and add_generation_prompt are not compatible. Use continue_final_message when you want the model to continue "
                    "the final message, and add_generation_prompt when you want to add a header that will prompt it to start a new assistant message instead."
                )
        ...
        if any(key in kwargs for key in mm_load_kwargs):
            ...
            logger.warning(
                f"{overlapping_keys[0] if len(overlapping_keys) == 1 else ', '.join(overlapping_keys)} load multimodal data "
                f"kwarg{'s' if len(overlapping_keys) > 1 else ''} {'have' if len(overlapping_keys) > 1 else 'has'} been passed to the processor,"
                f" but {'they are' if len(overlapping_keys) > 1 else 'it is'} not supported for VoxtralProcessor since it relies on mistral_common directly. "
                f"{'They' if len(overlapping_keys) > 1 else 'It'} will be ignored."
            )
        ...
        if return_tensors != "np":
            raise ValueError(f"{self.__class__.__name__} only supports `return_tensors='np'`.")
    ...
    def __call__(
        ...
        **kwargs: Unpack[VoxtralProcessorKwargs],
        ...
        if any(self.audio_token in t for t in text):
            raise ValueError(
                f"{self.audio_token} is present in the provided text which "
                f"is not supported by VoxtralProcessor. Please use the `apply_chat_template` method instead."
            )
        ...
        **kwargs: Unpack[VoxtralProcessorKwargs],
        ...
        if is_list_of_audio:
            if sampling_rate is None:
                logger.warning_once(
                    f"You've provided audio without specifying the sampling rate. "
                    f"It will be assumed to be {audio_kwargs['sampling_rate']}, which can result in silent errors."
                )
            elif sampling_rate != audio_kwargs["sampling_rate"]:
                raise ValueError(
                    f"The sampling rate of the audio ({sampling_rate}) does not match the sampling rate of the processor ({audio_kwargs['sampling_rate']}). "
                    f"Please provide resampled the audio to the expected sampling rate."
                )
    ...
    def apply_transcrition_request(self, *args, **kwargs):
        ...
        """
        warnings.warn(
            "`apply_transcrition_request` is deprecated due to a typo and "
            "will be removed in a future release. Please use `apply_transcription_request` instead.",
            FutureWarning,
        )
