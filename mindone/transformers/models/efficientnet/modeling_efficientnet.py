...
"""MindSpore EfficientNet model."""

import math
from typing import Optional, Tuple, Union

from transformers.models.efficientnet.configuration_efficientnet import EfficientNetConfig
...
class EfficientNetEmbeddings(nn.Cell):
    r"""
...
class EfficientNetDepthwiseConv2d(mint.nn.Conv2d):
    def __init__(
        self,
        in_channels,
        depth_multiplier=1,
...
class EfficientNetExpansionLayer(nn.Cell):
    r"""
...
class EfficientNetDepthwiseLayer(nn.Cell):
    r"""
...
class EfficientNetSqueezeExciteLayer(nn.Cell):
    r"""
...
class EfficientNetFinalBlockLayer(nn.Cell):
    r"""
...
class EfficientNetBlock(nn.Cell):
    r"""
...
class EfficientNetEncoder(nn.Cell):
    r"""
    ...
    def __init__(self, config: EfficientNetConfig):
        ...
        self.top_bn = mint.nn.BatchNorm2d(
...
class EfficientNetForImageClassification(EfficientNetPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = (
            mint.nn.Linear(config.hidden_dim, self.num_labels) if self.num_labels > 0 else mint.nn.Identity()
        )
    ...
    def construct(
        ...
        Examples:
        ```python
        >>> from mindone.transformers import AutoImageProcessor, EfficientNetForImageClassification
        ...
        >>> image_processor = AutoImageProcessor.from_pretrained("google/efficientnet-b7", revision="refs/pr/2")
        >>> model = EfficientNetForImageClassification.from_pretrained("google/efficientnet-b7", revision="refs/pr/2")
