...
class Data2VecTextEncoder(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.CellList([Data2VecTextLayer(config) for _ in range(config.num_hidden_layers)])
...
class Data2VecTextModel(Data2VecTextPreTrainedModel):
    ...
    # Copied from transformers.models.clap.modeling_clap.ClapTextModel.construct
    def construct(
        self,
...
class Data2VecTextLMHead(nn.Cell):
    ...
    def _tie_weights(self):
        ...
        if self.decoder.bias.device == "meta":
            self.decoder.bias = self.bias
        else:
...
class Data2VecTextForQuestionAnswering(Data2VecTextPreTrainedModel):
    ...
    def construct(
        ...
        if start_positions is not None and end_positions is not None:
            # If we are on multi-NPU, split add a dimension
