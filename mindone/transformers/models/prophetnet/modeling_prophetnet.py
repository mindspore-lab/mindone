...
def softmax(hidden_state, dim, onnx_trace=False):
    if onnx_trace:
        return mindspore.mint.nn.functional.softmax(hidden_state.float(), dim=dim)
    else:
        return mindspore.mint.nn.functional.softmax(hidden_state, dim=dim, dtype=mindspore.float32)
...
def ngram_attention_bias(sequence_length, ngram, dtype):
    ...
    """
    left_block = mindspore.mint.ones((ngram, sequence_length, sequence_length), dtype=dtype) * dtype_to_min(dtype)
    right_block = left_block.clone()
    # create bias
    for stream_idx in range(ngram):
        right_block[stream_idx].fill_diagonal_(0, wrap=False)
        left_block[stream_idx] = mindspore.mint.triu(left_block[stream_idx], diagonal=-stream_idx + 1)

    left_block[:, :, 0] = 0
    return mindspore.mint.cat([left_block, right_block], dim=2)
...
def compute_relative_buckets(num_buckets, max_distance, relative_positions, is_bidirectional=False):
    ...
    if is_bidirectional:
        ...
        rel_positions_bucket = (
            rel_positions_bucket
            + mindspore.mint.lt(inv_relative_positions, mindspore.mint.zeros_like(inv_relative_positions)).int()
            * num_buckets
        )
        inv_relative_positions = mindspore.mint.abs(inv_relative_positions)
    else:
        inv_relative_positions = mindspore.mint.max(
            inv_relative_positions, mindspore.mint.zeros_like(inv_relative_positions)
        )

    max_exact = num_buckets // 2
    is_small = mindspore.mint.lt(inv_relative_positions, max_exact)
    val_if_large = max_exact + mindspore.mint.log(inv_relative_positions.float() / max_exact) / math.log(
        max_distance / max_exact
    ) * (num_buckets - max_exact)
    val_if_large = mindspore.mint.min(val_if_large, mindspore.mint.ones_like(val_if_large) * (num_buckets - 1)).int()
    rel_positions_bucket = rel_positions_bucket + mindspore.mint.where(
...
def compute_all_stream_relative_buckets(num_buckets, max_distance, position_ids):
    ...
    # predicting stream
    predicting_stream_relative_positions = mindspore.mint.cat((position_ids - 1, position_ids), dim=-1).unsqueeze(1)
...
class ProphetNetSeq2SeqLMOutput(ModelOutput):
    ...
    Args:
        ...
        decoder_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or
            when `config.output_hidden_states=True`): Tuple of `ms.Tensor` (one for the output of the embeddings + one for the output of each layer) of
        ...
        decoder_ngram_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or
            when `config.output_hidden_states=True`): Tuple of `ms.Tensor` (one for the output of the embeddings + one for the output of each layer) of
        ...
        decoder_ngram_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is
            passed or when `config.output_attentions=True`): Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_attn_heads,
            decoder_sequence_length, decoder_sequence_length)`.
...
class ProphetNetSeq2SeqModelOutput(ModelOutput):
    ...
    Args:
        ...
        decoder_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            Attentions weights of the decoder, after the attention softmax, used to compute the weighted average in the
            self-attention heads.
        decoder_ngram_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when
            `config.output_attentions=True`): Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_attn_heads,
            decoder_sequence_length, decoder_sequence_length)`.

            Attentions weights of the predict stream of the decoder, after the attention softmax, used to compute the
            weighted average in the
        cross_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or
            when `config.output_attentions=True`): Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_attn_heads,
            encoder_sequence_length, decoder_sequence_length)`.
        ...
        encoder_last_hidden_state (`ms.Tensor` of shape `(batch_size, encoder_sequence_length, hidden_size)`, *optional*):
            Sequence of hidden-states at the output of the last layer of the encoder of the model.
        encoder_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when
            `config.output_hidden_states=True`): Tuple of `ms.Tensor` (one for the output of the embeddings + one for the output of each layer) of
            shape `(batch_size, encoder_sequence_length, hidden_size)`.
...
class ProphetNetDecoderModelOutput(ModelOutput):
    ...
    Args:
        ...
        hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            ...
            Hidden-states of main stream of the decoder at the output of each layer plus the initial embedding outputs.
        ngram_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or
            when `config.output_hidden_states=True`): Tuple of `ms.Tensor` (one for the output of the embeddings + one for the output of each layer) of
            shape `(batch_size, ngram * decoder_sequence_length, hidden_size)`.
...
class ProphetNetDecoderLMOutput(ModelOutput):
    ...
    Args:
        ...
        hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            ...
            Hidden-states of main stream of the decoder at the output of each layer plus the initial embedding outputs.
        ngram_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when
            `config.output_hidden_states=True`): Tuple of `ms.Tensor` (one for the output of the embeddings + one for the output of each layer) of
            shape `(batch_size, ngram * decoder_sequence_length, hidden_size)`.
...
class ProphetNetPositionalEmbeddings(mindspore.mint.nn.Embedding):
    """
    ...
    def construct(self, inputs_shape, attention_mask=None, past_key_values=None, position_ids=None):
        ...
        if position_ids is None:
            if past_key_values is not None:
                ...
                position_ids = mindspore.mint.ones(
                    (1, 1),
...
class ProphetNetAttention(mindspore.nn.Cell):
    ...
    def __init__(
        ...
        self.key_proj = mindspore.mint.nn.Linear(hidden_size, hidden_size)
        self.value_proj = mindspore.mint.nn.Linear(hidden_size, hidden_size)
        self.query_proj = mindspore.mint.nn.Linear(hidden_size, hidden_size)

        self.out_proj = mindspore.mint.nn.Linear(hidden_size, hidden_size)
    ...
    def construct(
        ...
        attn_weights = mindspore.mint.einsum("bsij,bsjk->bsik", query_states, key_states.transpose(2, 3))
        ...
        attn_probs = mindspore.mint.nn.functional.dropout(
            attn_weights,
            p=self.attention_dropout,
            training=self.training,
        )
        attn_output = mindspore.mint.einsum("bsij,bsjk->bsik", attn_probs, value_states)
        ...
        attn_output = mindspore.mint.nn.functional.dropout(attn_output, p=self.dropout, training=self.training)
        return attn_output, attn_weights_reshaped, past_key_value


class ProphetNetFeedForward(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: ProphetNetConfig, ffn_dim: int):
        ...
        self.intermediate = mindspore.mint.nn.Linear(config.hidden_size, ffn_dim)
        self.output = mindspore.mint.nn.Linear(ffn_dim, config.hidden_size)
...
class ProphetNetNgramSelfAttention(mindspore.nn.Cell):
    def __init__(self, config: ProphetNetConfig):
        ...
        self.key_proj = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
        self.value_proj = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
        self.query_proj = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)

        # out projection
        self.out_proj = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)

        # rel position embeddings
        self.relative_pos_embeddings = mindspore.mint.nn.Linear(
    ...
    def construct(
        ...
        if past_key_value is not None:
            ...
            main_key_states = mindspore.mint.cat((prev_main_key_states, main_key_states), dim=2)
            prev_main_value_states = past_key_value[1]
            main_value_states = mindspore.mint.cat((prev_main_value_states, main_value_states), dim=2)
        ...
        main_attn_weights = mindspore.mint.einsum("bntc,bncs->bnts", main_query_states, main_key_states.transpose(2, 3))
        ...
        main_attn_probs = mindspore.mint.nn.functional.dropout(
        ...
        # -> [batch_size, number_heads, sequence_length, head_dimesion]
        main_attn_output = mindspore.mint.einsum("bntc,bncs->bnts", main_attn_probs, main_value_states)
        ...
        predict_query_states = mindspore.mint.stack(predict_query_states_list, 1).view(
        ...
        predict_key_states = mindspore.mint.stack(
            [mindspore.mint.cat([main_key_states, key], 2) for key in predict_key_states_list], 1
        )

        # [batch_size, sequence_length, ngram, hidden_size]
        predict_hidden_states = mindspore.mint.stack(hidden_states_predict_list, dim=2)

        # [batch_size, number_heads, ngram, 2*sequence_length, head_dimesion]
        predict_value_states = mindspore.mint.cat(
            [mindspore.mint.cat([main_value_states, v_p], 2).unsqueeze(2) for v_p in predict_value_states_list], 2
        )
        ...
        predict_attn_weights = mindspore.mint.einsum("bnhtc,bnhsc->bnhts", predict_query_states, predict_key_states)
        ...
        predict_attn_probs = mindspore.mint.nn.functional.dropout(
        ...
        # x [batch_size, ngram, number_heads, 2*sequence_length, head_dimesion]
        # -> [batch_size, ngram, number_heads, sequence_length, head_dimesion]
        predict_attn_output = mindspore.mint.einsum(
        ...
        attn_output = mindspore.mint.cat([main_attn_output, predict_attn_output], 1).view(batch_size, -1, hidden_size)
    ...
    def get_main_relative_pos_embeddings(
        ...
        if main_relative_position_buckets is None:
            ...
            relative_positions = (
                mindspore.mint.arange(1, attn_weights.shape[-1] + 1)
                .unsqueeze(0)
                .unsqueeze(0)
        ...
        main_relative_pos_embeddings = mindspore.mint.gather(
    ...
    def get_predict_relative_pos_embeddings(
        ...
        if predict_relative_position_buckets is None:
            ...
            relative_positions = (
                mindspore.mint.arange(0, key_sequence_length)
                .unsqueeze(0)
                .unsqueeze(0)
                .repeat(batch_size, sequence_length, 1)
            )
        ...
        predict_relative_pos_embeddings = mindspore.mint.gather(
...
class ProphetNetEncoderLayer(mindspore.nn.Cell):
    """
...
class ProphetNetDecoderLayer(mindspore.nn.Cell):
    """
...
class ProphetNetEncoder(ProphetNetPreTrainedModel):
    r"""
    word_embeddings  (`ms.mint.nn.Embeddings` of shape `(config.vocab_size, config.hidden_size)`, *optional*):
        The word embedding parameters. This can be used to initialize [`ProphetNetEncoder`] with pre-defined word
        embeddings instead of randomly initialized word embeddings.
    """

    def __init__(self, config: ProphetNetConfig, word_embeddings: mindspore.mint.nn.Embedding = None):
        super().__init__(config)

        self.word_embeddings = (
            word_embeddings
            if word_embeddings is not None
            else mindspore.mint.nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=config.pad_token_id)
        )
        self.position_embeddings = ProphetNetPositionalEmbeddings(config)
        self.embeddings_layer_norm = LayerNorm(config.hidden_size)

        self.layers = mindspore.nn.CellList([ProphetNetEncoderLayer(config) for _ in range(config.num_encoder_layers)])
    ...
    def construct(
        ...
        >>> from mindone.transformers import ProphetNetEncoder
...
class ProphetNetDecoder(ProphetNetPreTrainedModel):
    ...
    def __init__(self, config: ProphetNetConfig, word_embeddings: Optional[mindspore.mint.nn.Embedding] = None):
        super().__init__(config)
        ...
        self.ngram_embeddings = mindspore.mint.nn.Embedding(self.ngram, config.hidden_size, None)
        self.layers = mindspore.nn.CellList([ProphetNetDecoderLayer(config) for _ in range(config.num_decoder_layers)])
        self.embeddings_layer_norm = LayerNorm(config.hidden_size)
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import ProphetNetDecoder
        >>> import mindspore as ms
        ...
        hidden_states = mindspore.mint.cat([hidden_states] + ngram_hidden_states, 1)
    ...
    def compute_buffered_relative_buckets(self, position_ids):
        ...
        position_ids = mindspore.mint.arange(1, self.max_target_positions).repeat(1, 1)
        ...
        predict_relative_buckets = mindspore.mint.cat(
            [
    ...
    def prepare_attention_mask(self, hidden_states, attention_mask):
        ...
        causal_mask = mindspore.ops.full(
            ...
            dtype_to_min(hidden_states.dtype),
            dtype=hidden_states.dtype,
        )
        causal_mask = mindspore.mint.triu(causal_mask, 1)
        ...
        if attention_mask is not None:
            extended_attention_mask = (1.0 - attention_mask[:, None, None, :]) * dtype_to_min(self.dtype)
    ...
    def prepare_predict_attention_mask(self, hidden_states, attention_mask):
        ...
        # get causal mask
        predict_causal_mask = ngram_attention_bias(self.max_target_positions, self.ngram, hidden_states.dtype)
        predict_causal_mask = mindspore.mint.cat(
            [
        ...
        # add usual attention mask
        if attention_mask is not None:
            extended_attention_mask = (1.0 - attention_mask[:, None, None, None, :]) * dtype_to_min(self.dtype)
            ...
            extended_attention_mask = mindspore.mint.cat(
                [extended_attention_mask, mindspore.mint.zeros_like(extended_attention_mask)], dim=-1
            )
...
class ProphetNetModel(ProphetNetPreTrainedModel):
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import ProphetNetModel
        >>> import mmindspore as ms
        >>> tokenizer = AutoTokenizer.from_pretrained("microsoft/prophetnet-large-uncased")
        ...
        >>> outputs = model(input_ids=ms.tensor(input_ids), decoder_input_ids=ms.tensor(decoder_input_ids))
...
class ProphetNetForConditionalGeneration(ProphetNetPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import ProphetNetForConditionalGeneration
        >>> import mindspore as ms
        >>> tokenizer = AutoTokenizer.from_pretrained("microsoft/prophetnet-large-uncased")
    ...
    def _compute_loss(self, logits, labels, ignore_index=-100):
        ...
        lprobs = mindspore.mint.nn.functional.log_softmax(
            logits.view(-1, logits.shape[-1]),
            dim=-1,
            dtype=mindspore.float32,
        )

        loss = mindspore.mint.nn.functional.nll_loss(lprobs, expend_targets.view(-1), reduction="mean")
...
class ProphetNetForCausalLM(ProphetNetPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import ProphetNetForCausalLM
        >>> import mindspore as ms
        ...
        >>> from mindone.transformers import EncoderDecoderModel
        >>> import mindspore as ms
        ...
        ... )
        >>> input_ids = tokenizer_enc(ARTICLE, return_tensors="np").input_ids
        >>> labels = tokenizer_dec(
        ...     "us rejects charges against its ambassador in bolivia", return_tensors="np"
        ... ).input_ids
        >>> outputs = model(input_ids=ms.tensor(input_ids), decoder_input_ids=ms.tensor(labels[:, :-1]), labels=ms.tensor(labels[:, 1:]))
