    ...
    def post_process_object_detection(
        ...
        if target_sizes is not None:
            ...
            if isinstance(target_sizes, list):
                img_h, img_w = ms.tensor(target_sizes).unbind(1)
            else:
        ...
        if use_focal_loss:
            ...
            boxes = boxes.gather(dim=1, index=index.unsqueeze(-1).tile((1, 1, boxes.shape[-1])))
        else:
            scores = mint.nn.functional.softmax(out_logits)[:, :, :-1]
            scores, labels = scores.max(dim=-1)
            if scores.shape[1] > num_top_queries:
                scores, index = mint.topk(scores, num_top_queries, dim=-1)
                labels = mint.gather(labels, dim=1, index=index)
                boxes = mint.gather(boxes, dim=1, index=index.unsqueeze(-1).tile((1, 1, boxes.shape[-1])))
