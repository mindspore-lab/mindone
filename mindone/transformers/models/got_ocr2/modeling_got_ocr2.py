...
class GotOcr2MLPBlock(ms.nn.Cell):
...
class GotOcr2VisionAttention(ms.nn.Cell):
...
class GotOcr2VisionLayer(nn.Cell):
    def __init__(self, config, window_size):
        super().__init__()
    ...
    def window_partition(self, hidden_states: ms.Tensor, window_size: int) -> tuple[ms.Tensor, tuple[int, int]]:
        ...
        hidden_states = mint.nn.functional.pad(hidden_states, (0, 0, 0, pad_w, 0, pad_h))
...
class GotOcr2PatchEmbeddings(ms.nn.Cell):
    """
...
class GotOcr2LayerNorm(ms.nn.Cell):
...
class GotOcr2VisionNeck(ms.nn.Cell):
...
class GotOcr2VisionEncoder(GotOcr2PreTrainedModel):
    ...
    def construct(
        self, pixel_values: Optional[ms.Tensor] = None,
...
class GotOcr2MultiModalProjector(ms.nn.Cell):
    def __init__(self, config: GotOcr2Config):
        ...
        self.conv_upsampler1 = mint.nn.Conv2d(
            vision_output_channels, vision_output_channels * 2, kernel_size=3, stride=2, padding=1, bias=False
        )
        self.conv_upsampler2 = mint.nn.Conv2d(
            vision_output_channels * 2, language_hidden_size, kernel_size=3, stride=2, padding=1, bias=False
        )
        self.multimodal_projector = mint.nn.Linear(language_hidden_size, language_hidden_size)

    def construct(self, vision_embeddings: ms.Tensor) -> ms.Tensor:
...
class GotOcr2ForConditionalGeneration(GotOcr2PreTrainedModel, GenerationMixin):
    ...
    def get_output_embeddings(self) -> ms.nn.Cell:
        return self.lm_head
