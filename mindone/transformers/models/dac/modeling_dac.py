...
class DacOutput(ModelOutput):
    ...
    Args:
        ...
        audio_values (`ms.Tensor` of shape `(batch_size, input_length)`):
            Reconstructed audio data.
        quantized_representation (`ms.Tensor` of shape `(batch_size, dimension, time_steps)`):
            Quantized continuous representation of input.
        audio_codes (`ms.Tensor` of shape `(batch_size, num_codebooks, time_steps)`):
            Codebook indices for each codebook (quantized discrete representation of input).
        projected_latents (`ms.Tensor` of shape `(batch_size, num_codebooks * dimension, time_steps)`):
            Projected latents (continuous representation of input before quantization).
    """

    loss: Tensor = None
    audio_values: Tensor = None
    quantized_representation: Tensor = None
    audio_codes: Tensor = None
    projected_latents: Tensor = None
...
class DacEncoderOutput(ModelOutput):
    ...
    Args:
        loss (`ms.Tensor`):
            Loss from the encoder model, comprising the weighted combination of the commitment and codebook losses.
        quantized_representation (`ms.Tensor` of shape `(batch_size, dimension, time_steps)`, *optional*):
            Quantized continuous representation of input.
        audio_codes (`ms.Tensor` of shape `(batch_size, num_codebooks, time_steps)`, *optional*):
            Codebook indices for each codebook (quantized discrete representation of input).
        projected_latents (`ms.Tensor` of shape `(batch_size, num_codebooks * dimension, time_steps)`, *optional*):
            Projected latents (continuous representation of input before quantization).
    """
...
class DacDecoderOutput(ModelOutput):
    """
    Args:
        audio_values (`ms.Tensor`  of shape `(batch_size, input_length)`, *optional*):
...
class Snake1d(nn.Cell):
    ...
    def __init__(self, hidden_dim):
        super().__init__()
        self.alpha = ms.Parameter(mint.ones((1, hidden_dim, 1)))

    def construct(self, hidden_states):
        shape = hidden_states.shape
        hidden_states = hidden_states.reshape(shape[0], shape[1], -1)
        hidden_states = hidden_states + (self.alpha + 1e-9).reciprocal() * mint.sin(self.alpha * hidden_states).pow(2)
        hidden_states = hidden_states.reshape(shape)
        return hidden_states


class DacVectorQuantize(nn.Cell):
    """
    ...
    def construct(self, hidden_state):
        ...
        Args:
            hidden_state (`ms.Tensor` of shape `(batch_size, dimension, time_steps)`):
                Input tensor.

        Returns:
            quantized_representation (`ms.Tensor`of shape `(batch_size, dimension, time_steps)`):
                Quantized continuous representation of input.
            commitment_loss (`ms.Tensor`of shape `(1)`):
                Commitment loss to train encoder to predict vectors closer to codebook entries.
            codebook_loss (`ms.Tensor`of shape `(1)`):
                Codebook loss to update the codebook.
            audio_codes (`ms.Tensor` of shape `(batch_size, time_steps)`):
                Codebook indices for each codebook, quantized discrete representation of input.
            projected_latents (ms.Tensor of shape `(batch_size, num_codebooks * dimension, time_steps)`):
                Projected latents (continuous representation of input before quantization).
        """
        ...
        commitment_loss = mint.nn.functional.mse_loss(
            projected_latents, ops.stop_gradient(quantized_representation), reduction="mean"
        )
        codebook_loss = mint.nn.functional.mse_loss(
            quantized_representation, ops.stop_gradient(projected_latents), reduction="mean"
        )
        # noop in forward pass, straight-through gradient estimator in backward pass
        quantized_representation = projected_latents + ops.stop_gradient((quantized_representation - projected_latents))
    ...
    def decode_latents(self, hidden_states):
        ...
        encodings = mint.nn.functional.normalize(encodings)
        codebook = mint.nn.functional.normalize(codebook)
...
class DacResidualVectorQuantize(nn.Cell):
    """
    ...
    def __init__(self, config: DacConfig):
        ...
        self.quantizers = nn.CellList([DacVectorQuantize(config) for i in range(config.n_codebooks)])
        self.quantizer_dropout = quantizer_dropout

    def construct(self, hidden_state, n_quantizers: int = None):
        """
        ...
        if self.training:
            n_quantizers = mint.ones((hidden_state.shape[0],)) * self.n_codebooks + 1
            dropout = mint.randint(1, self.n_codebooks + 1, (hidden_state.shape[0],))
        ...
        for i, quantizer in enumerate(self.quantizers):
            ...
            mask = mint.full((hidden_state.shape[0],), fill_value=i) < n_quantizers
        ...
        audio_codes = mint.stack(audio_codes, dim=1)
        projected_latents = mint.cat(projected_latents, dim=1)

        return quantized_representation, audio_codes, projected_latents, commitment_loss, codebook_loss

    def from_codes(self, audio_codes: Tensor):
        """
        ...
        Returns:
            quantized_representation (`ms.Tensor`):
                Quantized continuous representation of input.
            projected_latents (`ms.Tensor`):
        ...
        return quantized_representation, mint.cat(projected_latents, dim=1), audio_codes

    def from_latents(self, latents: Tensor):
        """Reconstructs the quantized representation from unquantized latents.

        Args:
            latents (`ms.Tensor` of shape `(batch_size, total_latent_dimension, time_steps)`):
                Continuous representation of input after projection.

        Returns:
            quantized_representation (`ms.Tensor` of shape `(batch_size, dimension, time_steps)`):
                Quantized representation of the full-projected space.
            quantized_latents (`ms.Tensor` of shape `(batch_size, dimension, time_steps)`):
                Quantized representation of the latent space (continuous representation before quantization).
        """
        quantized_representation = 0
        quantized_latents = []
        codes = []
        codebook_dims_tensor = Tensor([0] + [q.codebook_dim for q in self.quantizers])
        dims = mint.cumsum(codebook_dims_tensor, dim=0)
        ...
        return quantized_representation, mint.cat(quantized_latents, dim=1)


class DacDecoder(nn.Cell):
    """DAC Decoder"""
...
class DacModel(DacPreTrainedModel):
    ...
    def encode(
        ...
        Args:
            input_values (`ms.Tensor of shape `(batch_size, 1, time_steps)`):
    ...
    def decode(
        self,
        quantized_representation: Optional[Tensor] = None,
    ...
    def construct(
        ...
        >>> from mindone.transformers.models.dac import DacModel
        ...
        >>> inputs = processor(raw_audio=audio_sample, sampling_rate=processor.sampling_rate, return_tensors="np")
