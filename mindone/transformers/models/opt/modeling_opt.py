...
class OPTLearnedPositionalEmbedding(mint.nn.Embedding):
    ...
    def construct(
        ...
        if position_ids is None:
            position_ids = mint.cumsum(attention_mask, dim=1)
...
class OPTAttention(nn.Cell):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.k_proj = mint.nn.Linear(self.embed_dim, self.embed_dim, bias=self.enable_bias)
        self.v_proj = mint.nn.Linear(self.embed_dim, self.embed_dim, bias=self.enable_bias)
        self.q_proj = mint.nn.Linear(self.embed_dim, self.embed_dim, bias=self.enable_bias)
        self.out_proj = mint.nn.Linear(self.embed_dim, self.embed_dim, bias=self.enable_bias)
    ...
    def construct(
        ...
        query_states = query_states.view(bsz, -1, self.num_heads, self.head_dim).swapaxes(1, 2)

        key_states = self.k_proj(hidden_states)
        value_states = self.v_proj(hidden_states)
        key_states = key_states.view(bsz, -1, self.num_heads, self.head_dim).swapaxes(1, 2)
        value_states = value_states.view(bsz, -1, self.num_heads, self.head_dim).swapaxes(1, 2)
        ...
        attn_weights = mint.matmul(query_states, key_states.swapaxes(3, 2))
...
class OptFlashAttention2(OPTAttention):
    ...
    def construct(
        ...
        """Input shape: Batch x Time x Channel"""
        bsz, query_length, _ = hidden_states.shape
        ...
        if attention_mask is None and self.is_causal and query_states.shape[-2] > 1:
            attention_mask = mint.tril(mint.ones((query_states.shape[-2], key_states.shape[-2]), dtype=ms.bool_))
...
class OPTSdpaAttention(OPTAttention):
    ...
    def construct(
        ...
        # in SDPA to support both mindspore.jit's dynamic shapes and full graph options. An inline conditional prevents dynamic shapes from compiling.
...
class OPTDecoderLayer(nn.Cell):
    def __init__(self, config: OPTConfig, layer_idx: int = None):
        ...
        self.fc1 = mint.nn.Linear(self.embed_dim, config.ffn_dim, bias=config.enable_bias)
        self.fc2 = mint.nn.Linear(config.ffn_dim, self.embed_dim, bias=config.enable_bias)
    ...
    def construct(
        ...
        Args:
            ...
            layer_head_mask (`ms.Tensor`, *optional*): mask for attention heads in a given layer of size
...
class OPTDecoder(OPTPreTrainedModel):
    ...
    def __init__(self, config: OPTConfig):
        ...
        self.embed_tokens = mint.nn.Embedding(config.vocab_size, config.word_embed_proj_dim, self.padding_idx)
        self.embed_positions = OPTLearnedPositionalEmbedding(config.max_position_embeddings, config.hidden_size)

        if config.word_embed_proj_dim != config.hidden_size:
            self.project_out = mint.nn.Linear(config.hidden_size, config.word_embed_proj_dim, bias=False)
        else:
            self.project_out = None

        if config.word_embed_proj_dim != config.hidden_size:
            self.project_in = mint.nn.Linear(config.word_embed_proj_dim, config.hidden_size, bias=False)
        else:
        ...
        self.layers = nn.CellList([OPTDecoderLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)])
    ...
    def construct(
        ...
        if attention_mask is None:
            ...
            attention_mask = mint.ones((inputs_embeds.shape[0], seq_length))
...
class OPTForCausalLM(OPTPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.lm_head = mint.nn.Linear(config.word_embed_proj_dim, config.vocab_size, bias=False)
    ...
    def construct(
        ...
        **kwargs,
        ...
        >>> from mindone.transformers import OPTForCausalLM
        >>> import mindspore as ms
...
class OPTForSequenceClassification(OPTPreTrainedModel):
    def __init__(self, config: OPTConfig):
        ...
        self.model = OPTModel(config)
        self.score = mint.nn.Linear(config.word_embed_proj_dim, self.num_labels, bias=False)
...
class OPTForQuestionAnswering(OPTPreTrainedModel):
    def __init__(self, config: OPTConfig):
        super().__init__(config)
        self.model = OPTModel(config)
        self.qa_outputs = mint.nn.Linear(config.word_embed_proj_dim, 2)
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import OPTForQuestionAnswering
        >>> import mindspore as ms

        >>> ms.manual_seed(4)  # doctest: +IGNORE_RESULT
        >>> tokenizer = AutoTokenizer.from_pretrained("facebook/opt-350m")
