...
M4T_TEXT_TO_UNITS_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            [What are input IDs?](../glossary#input-ids)
        char_input_ids (`ms.Tensor` of shape `(batch_size, char_sequence_length)`):
        ...
        char_count_per_id (`ms.Tensor` of shape `(batch_size, sequence_length)`):
...
class SeamlessM4Tv2ConformerFeatureProjection(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.dropout = mint.nn.Dropout(config.speech_encoder_dropout)
...
class SeamlessM4Tv2ConformerFeedForward(nn.Cell):
    def __init__(self, config, act_fn=None, dropout=None):
        ...
        self.intermediate_dropout = mint.nn.Dropout(dropout)
        ...
        self.output_dropout = mint.nn.Dropout(dropout)
...
class SeamlessM4Tv2ConformerConvolutionModule(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.depthwise_layer_norm = mint.nn.LayerNorm(config.hidden_size)
    ...
    def construct(self, hidden_states, attention_mask=None):
        ...
        # FIXME: ms.nn.Conv1d.kernel_size is (1, kernel_size) but torch.nn.Conv1d.kernel_size is (kernel_size,)
        # hidden_states = mint.nn.functional.pad(hidden_states, (self.depthwise_conv.kernel_size[0] - 1, 0))
        hidden_states = mint.nn.functional.pad(hidden_states, (self.depthwise_conv.kernel_size[1] - 1, 0))
...
class SeamlessM4Tv2ConformerSelfAttention(nn.Cell):
    ...
    def __init__(self, config, use_position_embeddings=True):
        ...
        if self.position_embeddings_type == "relative_key":
            ...
            self.distance_embedding = mint.nn.Embedding(num_positions, self.head_size)
    ...
    def construct(
        ...
        attn_weights = mint.matmul(query, key.transpose(-2, -1)) / math.sqrt(self.head_size)
        ...
        if self.position_embeddings_type == "relative_key":
            ...
            distance = mint.clamp(distance, -self.left_max_position_embeddings, self.right_max_position_embeddings)

            positional_embedding = self.distance_embedding(distance + self.left_max_position_embeddings)
            positional_embedding = positional_embedding.to(dtype=query.dtype)  # fp16 compatibility

            relative_position_attn_weights = mint.einsum("bhld,lrd->bhlr", query, positional_embedding)
...
class SeamlessM4Tv2ConformerEncoderLayer(nn.Cell):
    ...
    # Wav2Vec2ConformerEncoderLayer.__init__ with Wav2Vec2->SeamlessM4Tv2,
    # attention_dropout->speech_encoder_dropout, torch.nn->nn
    def __init__(self, config):
        super().__init__()
        ...
        self.self_attn_dropout = mint.nn.Dropout(dropout)
...
class SeamlessM4Tv2ConformerEncoder(nn.Cell):
    ...
    def _apply_chunk_attention(self, attention_mask, hidden_states):
        ...
        chunk_indices = mint.arange(
            sequence_len,
        )
        chunk_indices = mint.div(chunk_indices, self.config.speech_encoder_chunk_size).long()

        start_indices = mint.full_like(chunk_indices, 0)
...
class SeamlessM4Tv2ScaledWordEmbedding(mint.nn.Embedding):
    """
...
# Copied from transformers.models.nllb_moe.modeling_nllb_moe.NllbMoeDenseActDense
# with NllbMoe->SeamlessM4Tv2,DenseActDense->FeedForwardNetwork, d_model->hidden_size
...
class SeamlessM4Tv2EncoderLayer(nn.Cell):
    def __init__(self, config: SeamlessM4Tv2Config, encoder_ffn_dim=None, encoder_attention_heads=None):
        ...
        self.attn_dropout = mint.nn.Dropout(config.dropout)
        ...
        self.ffn_dropout = mint.nn.Dropout(config.activation_dropout)
...
class SeamlessM4Tv2TextToUnitDecoderLayer(nn.Cell):
    def __init__(self, config: SeamlessM4Tv2Config, decoder_ffn_dim=None, decoder_attention_heads=None):
        ...
        self.conv1 = nn.Conv1d(self.embed_dim, self.embed_dim, kernel_size=7, stride=1, pad_mode="same", has_bias=True)
        self.activation_fn = ACT2FN[config.activation_function]
        self.conv2 = nn.Conv1d(self.embed_dim, self.embed_dim, kernel_size=7, stride=1, pad_mode="same", has_bias=True)

        self.conv_layer_norm = mint.nn.LayerNorm(config.hidden_size)
        self.conv_dropout = mint.nn.Dropout(self.dropout)
...
class SeamlessM4Tv2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, mint.nn.Linear):
            ...
            module.projection.weight.data.uniform_(from_=-k, to=k)
            module.projection.bias.data.uniform_(from_=-k, to=k)
        elif isinstance(module, (mint.nn.LayerNorm, mint.nn.GroupNorm)):
            module.bias.data.zero_()
            module.weight.data.fill_(1.0)
        elif isinstance(module, (nn.Conv1d, nn.Conv1dTranspose)):
            ...
            if module.bias is not None:
                ...
                module.bias.data.uniform_(from_=-k, to=k)
    ...
    def _get_char_input_ids(self, input_ids, subwords_batch, char_count_per_id, pad_token_id=0, unk_token_id=1):
        ...
        Args:
            ...
            unk_token_id (`int`, *optional*, defaults to 1):
                The id of the _unknown_ text token. Associated to a subword of length 1.
        Returns:
            `ms.Tensor`: Tensor of shape `(batch_size, char_sequence_length)` containing the id of each character.
        """
        ...
        for batch_id in range(batch_size):
            ...
            for subword_idx, subword in zip(subword_indices, subwords):
                ...
                char_seqs[batch_id, total : total + char_seq_len] = ms.tensor(char_ids).to(char_seqs.dtype)
                total += char_seq_len
        return char_seqs
    ...
    def _hard_upsample(self, hidden_states, durations):
        ...
        Args:
            hidden_states (`ms.Tensor` of shape `(batch_size, sequence_length, *)`, *optional*):
                The sequence to repeat, where `*` is any number of sequence-specific dimensions including none.
            durations (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                Indicates how many times to repeat time segments.
        """
        if hidden_states.shape[0] == 1:
            hidden_states = mint.repeat_interleave(hidden_states, durations.view(-1), dim=1)
        else:
            ...
            hidden_states = [
                mint.repeat_interleave(hidden_state, duration, dim=0)
...
class SeamlessM4Tv2TextToUnitDecoder(SeamlessM4Tv2PreTrainedModel):
    def __init__(
        ...
        if embed_tokens is not None:
            ...
            self.embed_tokens = mint.nn.Embedding(self.vocab_size, config.hidden_size, self.padding_idx)

        self.embed_char = mint.nn.Embedding(config.char_vocab_size, config.hidden_size)
        ...
        self.pos_emb_alpha_char = ms.Parameter(mint.ones(1))
        self.pos_emb_alpha = ms.Parameter(mint.ones(1))
    ...
    def construct(
        self,
        char_input_ids: ms.Tensor = None,
        char_count_per_id: ms.Tensor = None,
        ...
        Args:
            char_input_ids (`ms.Tensor` of shape `(batch_size, char_sequence_length)`):
                Character indices. The correspondence between characters and indices can be found in `char_to_id`, a
                dictionary in the generation configuration.
            char_count_per_id (`ms.Tensor` of shape `(batch_size, encoder_sequence_length)`):
...
@add_start_docstrings(
    """
    Transformer bare text-to-unit encoder-decoder. The encoder is a [`SeamlessM4Tv2Encoder`]
    without embeddings and the decoder is a [`SeamlessM4Tv2TextToUnitDecoder`].
    """,
    SEAMLESS_M4T_V2_START_DOCSTRING,
    """
...
class SeamlessM4Tv2TextToUnitModel(SeamlessM4Tv2PreTrainedModel):
    # Copied from transformers.models.seamless_m4t.modeling_seamless_m4t.
    # SeamlessM4TTextToUnitModel.__init__ with SeamlessM4T->SeamlessM4Tv2, Decoder->TextToUnitDecoder
    def __init__(
        self,
        config: SeamlessM4Tv2Config,
...
class SeamlessM4Tv2VariancePredictor(nn.Cell):
    def __init__(self, embed_dim, hidden_dim, kernel_size, var_pred_dropout):
        ...
        self.ln2 = mint.nn.LayerNorm(hidden_dim)
        self.proj = mint.nn.Linear(hidden_dim, 1)

    def construct(self, hidden_states: Tensor, padding_mask: Tensor = None) -> Tensor:
...
class SeamlessM4Tv2CodeHifiGan(PreTrainedModel):
    ...
    def construct(self, input_ids: ms.Tensor, speaker_id: ms.Tensor, lang_id: ms.Tensor) -> Tuple[ms.Tensor]:
        """
        Args:
            input_ids (`ms.Tensor` of shape `(batch_size, sequence_length)`):
                Indices of input sequence tokens in the vocabulary.
    ...
    def _init_weights(self, module):
        """Initialize the weights."""
        if isinstance(module, (mint.nn.Linear, nn.Conv1d, nn.Conv1dTranspose)):
    ...
    def remove_weight_norm(self):
        self.hifi_gan.conv_pre.remove_weight_norm()
        for layer in self.hifi_gan.upsampler:
            layer.remove_weight_norm()
        for layer in self.hifi_gan.resblocks:
            layer.remove_weight_norm()
        self.hifi_gan.conv_post.remove_weight_norm()
...
# Copied from transformers.models.seamless_m4t.modeling_seamless_m4t.SeamlessM4TForTextToText with
# SeamlessM4T->SeamlessM4Tv2,SeamlessM4Tv2Tokenizer->SeamlessM4TTokenizer, SeamlessM4Tv2Processor->SeamlessM4TProcessor
...
class SeamlessM4Tv2ForTextToSpeech(SeamlessM4Tv2PreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
    ) -> Union[ms.Tensor, SeamlessM4Tv2GenerationOutput]:
        """
        ...
        if attention_mask is not None:
            ...
            attention_mask = mint.repeat_interleave(attention_mask, num_return_sequences, dim=0)
        encoder_hidden_states = text_generation_output.encoder_hidden_states[-1]

        # repeat attention mask alongside batch dimension
        encoder_hidden_states = mint.repeat_interleave(encoder_hidden_states, num_return_sequences, dim=0)
        ...
        t2u_char_count_per_id = mint.cat([pad_zero, t2u_char_count_per_id, pad_zero], dim=1)
        ...
        if (temperature is None or temperature == 1.0) or not kwargs_speech.get("do_sample", False):
            ...
            # apply softmax
            probs = mint.nn.functional.softmax(t2u_logits, dim=-1)
            ...
            unit_ids = mint.multinomial(probs, num_samples=1).view(t2u_logits.shape[0], -1)

        output_unit_ids = ops.stop_gradient(unit_ids).clone()
        ...
        speaker_id = ms.tensor([[speaker_id]] * len(unit_ids))
...
@add_start_docstrings(
    ...
    """
        current_modality (`str`, *optional*, defaults to `"text"`):
            ...
            This will be updated automatically according to the modality passed to the forward and
            generate passes (`input_ids` for text and `input_features` for audio).
    """,
)
