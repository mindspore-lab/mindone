...
def remove_keypoints_from_borders(
    keypoints: ms.Tensor, scores: ms.Tensor, border: int, height: int, width: int
...
def top_k_keypoints(keypoints: ms.Tensor, scores: ms.Tensor, k: int) -> Tuple[ms.Tensor, ms.Tensor]:
    """Keeps the k keypoints with highest score"""
    if k >= len(keypoints):
        return keypoints, scores
    scores, indices = mint.topk(scores, k, dim=0)
    return keypoints[indices], scores


def simple_nms(scores: ms.Tensor, nms_radius: int) -> ms.Tensor:
    ...
    for _ in range(2):
        ...
        supp_scores = mint.where(supp_mask, zeros, scores)
        new_max_mask = supp_scores == max_pool(supp_scores)
        max_mask = max_mask | (new_max_mask & (~supp_mask))
    return mint.where(max_mask, scores, zeros)
...
class SuperPointKeypointDescriptionOutput(ModelOutput):
    ...
    scores: Optional[ms.Tensor] = None
    descriptors: Optional[ms.Tensor] = None
    mask: Optional[ms.Tensor] = None
    hidden_states: Optional[Tuple[ms.Tensor]] = None


class SuperPointConvBlock(ms.nn.Cell):
    def __init__(
        self, config: SuperPointConfig, in_channels: int, out_channels: int, add_pooling: bool = False
    ) -> None:
        super().__init__()
        self.conv_a = mint.nn.Conv2d(
            in_channels,
            out_channels,
            kernel_size=3,
            stride=1,
...
class SuperPointEncoder(ms.nn.Cell):
    """
    ...
    def __init__(self, config: SuperPointConfig) -> None:
        ...
        self.conv_blocks = ms.nn.CellList(conv_blocks)
...
class SuperPointInterestPointDecoder(ms.nn.Cell):
    """
    ...
    def _get_pixel_scores(self, encoded: ms.Tensor) -> ms.Tensor:
        ...
        scores = mint.nn.functional.softmax(scores, 1)[:, :-1]
    ...
    def _extract_keypoints(self, scores: ms.Tensor) -> Tuple[ms.Tensor, ms.Tensor]:
        """
        ...
        keypoints = mint.nonzero(scores[0] > self.keypoint_threshold)
...
class SuperPointDescriptorDecoder(ms.nn.Cell):
    """
    ...
    def __init__(self, config: SuperPointConfig) -> None:
        ...
        self.conv_descriptor_b = mint.nn.Conv2d(
            config.decoder_hidden_size,
    ...
    def construct(self, encoded: ms.Tensor, keypoints: ms.Tensor) -> ms.Tensor:
        ...
        descriptors = mint.transpose(descriptors, 0, 1)

        return descriptors

    @staticmethod
    def _sample_descriptors(keypoints, descriptors, scale: int = 8) -> ms.Tensor:
        """Interpolate descriptors at keypoint locations"""
        batch_size, num_channels, height, width = descriptors.shape
        keypoints = keypoints - scale / 2 + 0.5
        divisor = ms.Tensor([[(width * scale - scale / 2 - 0.5), (height * scale - scale / 2 - 0.5)]])
...
class SuperPointForKeypointDetection(SuperPointPreTrainedModel):
    ...
    def construct(
        ...
        mask = mint.zeros((batch_size, maximum_num_keypoints), dtype=ms.int32)
        ...
        keypoints = keypoints / ms.Tensor([width, height], )
