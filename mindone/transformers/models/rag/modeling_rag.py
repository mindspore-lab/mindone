...
class RetrievAugLMMarginOutput(ModelOutput):
    ...
    Args:
        ...
        doc_scores (`ms.Tensor` of shape `(batch_size, config.n_docs)`):
        ...
        retrieved_doc_embeds (`ms.Tensor` of shape `(batch_size, config.n_docs, hidden_size)`, *optional*, returned when *output_retrieved=True*):
            Embedded documents retrieved by the retriever. Is used with `question_encoder_last_hidden_state` to compute
            the `doc_scores`.
        retrieved_doc_ids (`ms.Tensor` of shape `(batch_size, config.n_docs)`, *optional*, returned when *output_retrieved=True*):
            The indexes of the embedded documents retrieved by the retriever.
        context_input_ids (`ms.Tensor` of shape `(batch_size * config.n_docs, config.max_combined_length)`, *optional*, returned when *output_retrieved=True*):  # noqa: E501
            Input ids post-processed from the retrieved documents and the question encoder input_ids by the retriever.
        context_attention_mask (`ms.Tensor` of shape `(batch_size * config.n_docs, config.max_combined_length)`, *optional*, returned when *output_retrieved=True*):  # noqa: E501
            Attention mask post-processed from the retrieved documents and the question encoder `input_ids` by the
            retriever.
        question_encoder_last_hidden_state (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
            Sequence of hidden states at the output of the last layer of the question encoder pooled output of the
            model.
        question_enc_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):  # noqa: E501
        ...
        question_enc_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):  # noqa: E501
            Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
            sequence_length)`.

            Attentions weights of the question encoder, after the attention softmax, used to compute the weighted
            average in the self-attention heads.
        generator_enc_last_hidden_state (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
            Sequence of hidden-states at the output of the last layer of the generator encoder of the model.
        generator_enc_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):  # noqa: E501
            Tuple of `ms.Tensor` (one for the output of the embeddings and one for the output of each layer) of
            shape `(batch_size, sequence_length, hidden_size)`.

            Hidden states of the generator encoder at the output of each layer plus the initial embedding outputs.
        generator_enc_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):  # noqa: E501
            Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
            sequence_length)`.

            Attentions weights of the generator encoder, after the attention softmax, used to compute the weighted
            average in the self-attention heads.
        generator_dec_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):  # noqa: E501
            Tuple of `ms.Tensor` (one for the output of the embeddings and one for the output of each layer) of
            shape `(batch_size, sequence_length, hidden_size)`.

            Hidden states of the generator decoder at the output of each layer plus the initial embedding outputs.
        generator_dec_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):  # noqa: E501
            Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
            sequence_length)`.

            Attentions weights of the generator decoder, after the attention softmax, used to compute the weighted
            average in the self-attention heads.
        generator_cross_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):  # noqa: E501
            Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
            sequence_length)`.
    ...
    doc_scores: Optional[ms.Tensor] = None
    past_key_values: Optional[List[ms.Tensor]] = None
    retrieved_doc_embeds: Optional[ms.Tensor] = None
    retrieved_doc_ids: Optional[ms.Tensor] = None
    context_input_ids: Optional[ms.Tensor] = None
    context_attention_mask: Optional[ms.Tensor] = None
    question_encoder_last_hidden_state: Optional[ms.Tensor] = None
    question_enc_hidden_states: Optional[Tuple[ms.Tensor, ...]] = None
    question_enc_attentions: Optional[Tuple[ms.Tensor, ...]] = None
    generator_enc_last_hidden_state: Optional[ms.Tensor] = None
    generator_enc_hidden_states: Optional[Tuple[ms.Tensor, ...]] = None
    generator_enc_attentions: Optional[Tuple[ms.Tensor, ...]] = None
    generator_dec_hidden_states: Optional[Tuple[ms.Tensor, ...]] = None
    generator_dec_attentions: Optional[Tuple[ms.Tensor, ...]] = None
    generator_cross_attentions: Optional[Tuple[ms.Tensor, ...]] = None
...
class RetrievAugLMOutput(ModelOutput):
    ...
    Args:
        ...
        doc_scores (`ms.Tensor` of shape `(batch_size, config.n_docs)`):
            Score between each retrieved document embeddings (see `retrieved_doc_embeds`) and
            `question_encoder_last_hidden_state`.
        past_key_values (`List[ms.Tensor]`, *optional*, returned when `use_cache=True` is passed or when `config.use_cache=True`):  # noqa: E501
            List of `ms.Tensor` of length `config.n_layers`, with each tensor of shape `(2, batch_size,
            num_heads, sequence_length, embed_size_per_head)`).

            Contains precomputed hidden-states (key and values in the attention blocks) of the decoder that can be used
            (see `past_key_values` input) to speed up sequential decoding.
        retrieved_doc_embeds (`ms.Tensor` of shape `(batch_size, config.n_docs, hidden_size)`, *optional*, returned when *output_retrieved=True*):  # noqa: E501
            Embedded documents retrieved by the retriever. Is used with `question_encoder_last_hidden_state` to compute
            the `doc_scores`.
        retrieved_doc_ids (`ms.Tensor` of shape `(batch_size, config.n_docs)`, *optional*, returned when *output_retrieved=True*):
            The indexes of the embedded documents retrieved by the retriever.
...
class RagPreTrainedModel(PreTrainedModel):
    ...
    def from_pretrained_question_encoder_generator(
        ...
        **kwargs,
        ...
        ```python
        >>> from mindone.transformers import RagModel
...
RAG_FORWARD_INPUTS_DOCSTRING = r"""
    Args:
        ...
        decoder_attention_mask (`ms.Tensor` of shape `(batch_size,  target_sequence_length)`, *optional*):
            ...
            be used by default.
        past_key_values (`tuple(tuple(ms.Tensor))`):
        ...
        context_input_ids (`ms.Tensor` of shape `(batch_size * config.n_docs, config.max_combined_length)`, *optional*, returned when *output_retrieved=True*):
...
class RagModel(RagPreTrainedModel):
    ...
    def construct(
    ...
    ) -> Union[Tuple[ms.Tensor], RetrievAugLMOutput]:
        r"""
        Returns:
        ...
        >>> tokenizer = AutoTokenizer.from_pretrained("facebook/rag-token-base", revision="refs/pr/1")
        >>> retriever = RagRetriever.from_pretrained(
        ...     "facebook/rag-token-base", index_name="exact", use_dummy_dataset=True, revision="refs/pr/1"
        ... )
        ...
        if encoder_outputs is None:
            if has_to_retrieve:
                ...
                retriever_outputs = self.retriever(
                    input_ids,
                    ops.stop_gradient(question_encoder_last_hidden_state).to(dtype=ms.float32).numpy(),
                    prefix=self.generator.config.prefix,
                    n_docs=n_docs,
                    return_tensors="np",
                )
                retriever_outputs = {k: ms.tensor(v) for k, v in retriever_outputs.items()}
                ...
                if self.context_encoder_training:
                    ...
                    doc_scores = mint.bmm(
                        question_encoder_last_hidden_state.unsqueeze(1), retrieved_doc_embeds.swapaxes(1, 2)
                    ).squeeze(1)
...
class RagSequenceForGeneration(RagPreTrainedModel):
    ...
    def construct(
        ...
        **kwargs,  # needs kwargs for generation
        ...
        reduce_loss (`bool`, *optional*):
            Only relevant if `labels` is passed. If `True`, the NLL loss is reduced using the `ms.Tensor.sum`
            operation.
        ...
        >>> retriever = RagRetriever.from_pretrained(
        ...     "facebook/rag-sequence-nq", index_name="exact", use_dummy_dataset=True, revision="refs/pr/1"
        ... )
        >>> # initialize with RagRetriever to do everything in one forward call
        >>> model = RagSequenceForGeneration.from_pretrained(
        ...     "facebook/rag-token-nq", retriever=retriever, revision="refs/pr/5"
        ... )

        >>> inputs = tokenizer("How many people live in Paris?", return_tensors="np")
        >>> targets = tokenizer(text_target="In Paris, there are 10 million people.", return_tensors="np")
        >>> input_ids = ms.tensor(inputs["input_ids"])
        >>> labels = ms.tensor(targets["input_ids"])
        >>> outputs = model(input_ids=input_ids, labels=labels)

        >>> # or use retriever separately
        >>> model = RagSequenceForGeneration.from_pretrained(
        ...     "facebook/rag-sequence-nq", use_dummy_dataset=True, revision="refs/pr/1"
        ... )
        >>> # 1. Encode
        >>> question_hidden_states = model.question_encoder(input_ids)[0]
        >>> # 2. Retrieve
        >>> docs_dict = retriever(input_ids.numpy(), question_hidden_states.detach().numpy(), return_tensors="np")
        >>> doc_scores = mint.bmm(
        ...     question_hidden_states.unsqueeze(1), ms.tensor(docs_dict["retrieved_doc_embeds"]).float().swapaxes(1, 2)
        ... ).squeeze(1)
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        Return:
            `ms.Tensor` of shape `(batch_size * num_return_sequences, sequence_length)`: The generated
        ...
        if self.retriever is not None and context_input_ids is None:
            ...
            context_input_ids = self.retriever(
                input_ids,
                ops.stop_gradient(question_hidden_states).to(dtype=ms.float32).numpy(),
                prefix=self.generator.config.prefix,
                n_docs=n_docs,
                return_tensors="np",
            )["context_input_ids"]
            context_input_ids = ms.tensor(context_input_ids)
        ...
        for index in range(batch_size):
            ...
            if do_deduplication:
                ...
                output_sequences = mint.stack(list({str(k.tolist()): k for k in output_sequences}.values()))
            ...
            if input_ids is not None:
                new_input_ids = input_ids[index : index + 1].tile((num_candidates, 1))
                ...
                individual_input_ids = generator_input_ids.tile((num_candidates, 1))  # (num_candidates*n_docs, max_len)

                individual_attention_mask = context_attention_mask[index * n_docs : (index + 1) * n_docs]
                individual_attention_mask = individual_attention_mask.tile((num_candidates, 1))

                individual_doc_scores = doc_scores[index : (index + 1), :]  # doc_scores.shape = [batch, n_docs]
                individual_doc_scores = individual_doc_scores.tile((num_candidates, 1))  # [num_candidates, n_docs]
    ...
    def get_nll(
        ...
        # shift tokens left
        target = mint.cat([target[:, 1:], target.new(target.shape[0], 1).fill_(self.config.generator.pad_token_id)], 1)
        ...
        seq_logprobs = mint.nn.functional.log_softmax(seq_logits, dim=-1).view(
            seq_logits.shape[0] // n_docs, n_docs, -1, seq_logits.shape[-1]
        )  # batch_size x n_docs x tgt_len x #vocab_size
        doc_logprobs = mint.nn.functional.log_softmax(doc_scores, dim=1).unsqueeze(-1).unsqueeze(-1)

        # RAG-sequence marginalization
        first_token_scores = seq_logprobs[:, :, :1, :]
        second_token_scores = seq_logprobs[:, :, 1:2, :]
        remainder = seq_logprobs[:, :, 2:, :]
        rag_logprobs = mint.cat([first_token_scores, second_token_scores + doc_logprobs, remainder], dim=2)

        # calculate loss
        target = target.unsqueeze(1).unsqueeze(-1).tile((1, n_docs, 1, 1))
...
class RagTokenForGeneration(RagPreTrainedModel):
    ...
    def marginalize(self, seq_logits, doc_scores, n_docs=None):
        ...
        doc_logprobs = mint.nn.functional.log_softmax(doc_scores, dim=1)
        log_prob_sum = seq_logprobs + doc_logprobs.unsqueeze(-1).unsqueeze(-1)
        return mint.logsumexp(log_prob_sum, dim=1)
    ...
    def construct(
        ...
        **kwargs,  # needs kwargs for generation
        ...
        do_marginalize (`bool`, *optional*):
            ...
            `ms.mint.nn.functional.log_softmax`.
        reduce_loss (`bool`, *optional*):
            Only relevant if `labels` is passed. If `True`, the NLL loss is reduced using the `ms.Tensor.sum`
            operation.
        kwargs (`Dict[str, any]`, *optional*, defaults to `{}`):
        ...
        ```python
        >>> from transformers import AutoTokenizer, RagRetriever
        >>> from mindone.transformers import RagTokenForGeneration
        >>> import mindspore as ms
        >>> from mindspore import mint, ops

        >>> tokenizer = AutoTokenizer.from_pretrained("facebook/rag-token-nq", revision="refs/pr/5")
        >>> retriever = RagRetriever.from_pretrained(
        ...     "facebook/rag-token-nq", index_name="exact", use_dummy_dataset=True, revision="refs/pr/5"
        ... )
        >>> # initialize with RagRetriever to do everything in one forward call
        ...
        >>> # or use retriever separately
        >>> model = RagTokenForGeneration.from_pretrained(
        ...     "facebook/rag-token-nq", use_dummy_dataset=True, revision="refs/pr/5"
        ... )
        >>> # 1. Encode
        >>> question_hidden_states = model.question_encoder(input_ids)[0]
        >>> # 2. Retrieve
        >>> docs_dict = retriever(
        ...     input_ids.numpy(), ops.stop_gradient(question_hidden_states).numpy(), return_tensors="np"
        ... )
        >>> docs_dict = {k: ms.tensor(v) for k, v in docs_dict.items()}
        >>> doc_scores = mint.bmm(
        ...     question_hidden_states.unsqueeze(1), docs_dict["retrieved_doc_embeds"].float().swapaxes(1, 2)
        ... ).squeeze(1)
        >>> # 3. Forward to generator
        >>> outputs = model(
        ...     context_input_ids=docs_dict["context_input_ids"],
    ...
    def generate(
        ...
        prefix_allowed_tokens_fn: Callable[[int, ms.Tensor], List[int]] = None,
        ...
        **kwargs,
        ...
        if self.retriever is not None and context_input_ids is None:
            ...
            out = {k: ms.tensor(v) for k, v in out.items()}
            ...
            # compute doc_scores
            doc_scores = mint.bmm(question_hidden_states.unsqueeze(1), retrieved_doc_embeds.swapaxes(1, 2)).squeeze(1)
        ...
        input_ids = mint.full(
            (batch_size * generation_config.num_beams, 1), generation_config.decoder_start_token_id, dtype=ms.int64
        )
