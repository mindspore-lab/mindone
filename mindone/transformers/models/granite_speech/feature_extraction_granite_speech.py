...
import mindspore.dataset.audio as ms_audio
...
class GraniteSpeechFeatureExtractor(FeatureExtractionMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mel_filters = ms_audio.MelSpectrogram(**self.melspec_kwargs)
    ...
    def __call__(
        ...
        speech_inputs["input_features_mask"] = mint.arange(max(audio_embed_sizes)).view(1, -1) < ms.tensor(
            audio_embed_sizes
        ).view(-1, 1)
        return BatchFeature(data=speech_inputs)

    def _extract_mel_spectrograms(self, audio):
        """
        ...
        with ms._no_grad():
            ...
            mel = self.mel_filters(audio.astype(np.float32))
            mel = ms.from_numpy(mel)
            logmel = mel.transpose(-1, -2).clip(min=1e-10).log10()
            mx = mint.amax(logmel, dim=(-2, -1), keepdim=True)
            logmel = mint.maximum(logmel, mx - 8.0).div_(4).add_(1)
    ...
    def _get_audios_and_audio_lengths(self, audios) -> Sequence["ms.Tensor", Sequence[int]]:
        """
        ...
        if isinstance(audios, np.ndarray):
            audios = ms.from_numpy(audios)
        elif isinstance(audios, Sequence) and isinstance(audios[0], np.ndarray):
            audios = [ms.from_numpy(arr) for arr in audios]

        if isinstance(audios, ms.Tensor):
            if audios.ndim == 1:
                audios = audios.unsqueeze(0)
            if not ops.is_floating_point(audios):
            ...
            # dataset.audio.MelSpectrogram only supports numpy input
            # return audios, lengths
            return audios.numpy(), lengths

        elif isinstance(audios, Sequence) and isinstance(audios[0], ms.Tensor):
            if not ops.is_floating_point(audios[0]):
                raise ValueError("Invalid audio provided. Audio should be a floating point between 0 and 1")
            lengths = [audio.shape[-1] for audio in audios]
            padding = [max(length) - length for length in lengths]

            audios = [audio.view(1, -1) for audio in audios]
            padded = [mint.nn.functional.pad(audio, (0, pad)) for audio, pad in zip(audios, padding)]
            audios = mint.cat(padded, dim=0)
            # dataset.audio.MelSpectrogram only supports numpy input
            # return audios, lengths
            return audios.numpy(), lengths
