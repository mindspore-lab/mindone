...
from mindspore.common.initializer import HeUniform, XavierUniform, initializer

from mindone.utils import WeightNorm as weight_norm
...
class FastSpeech2ConformerModelOutput(ModelOutput):
    ...
    Args:
        ...
        decoder_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            self-attention heads.
        duration_outputs (`ms.Tensor` of shape `(batch_size, max_text_length + 1)`, *optional*):
            Outputs of the duration predictor.
        pitch_outputs (`ms.Tensor` of shape `(batch_size, max_text_length + 1, 1)`, *optional*):
            Outputs of the pitch predictor.
        energy_outputs (`ms.Tensor` of shape `(batch_size, max_text_length + 1, 1)`, *optional*):
    ...
    duration_outputs: ms.Tensor = None
    pitch_outputs: ms.Tensor = None
    energy_outputs: ms.Tensor = None
...
class FastSpeech2ConformerWithHifiGanOutput(FastSpeech2ConformerModelOutput):
    ...
    Args:
        waveform (`ms.Tensor` of shape `(batch_size, audio_length)`):
...
def length_regulator(encoded_embeddings, duration_labels, speaking_speed=1.0):
    ...
    Args:
        encoded_embeddings (`ms.Tensor` of shape `(batch_size, max_text_length, embedding_dim)`):
            Batch of sequences of char or phoneme embeddings.
        duration_labels (`ms.Tensor` of shape `(batch_size, time)`):
    ...
    for i, (encoded_embedding, target_duration) in enumerate(zip(encoded_embeddings, duration_labels)):
        repeated = mint.repeat_interleave(encoded_embedding, target_duration, dim=0)
...
class FastSpeech2ConformerDurationPredictor(nn.Cell):
    ...
    def __init__(self, config: FastSpeech2ConformerConfig):
        ...
        self.linear = mint.nn.Linear(config.duration_predictor_channels, 1)

    def construct(self, encoder_hidden_states):
        """
        Args:
            hidden_states (`ms.Tensor` of shape `(batch_size, max_text_length, input_dim)`):
                Batch of input sequences.
            padding_masks (`ms.Tensor` of shape `(batch_size, max_text_length)`, *optional*):
                Batch of masks indicating padded part.

        Returns:
            `ms.Tensor`: Batch of predicted durations in log domain `(batch_size, max_text_length)`.
        ...
        if not self.training:
            ...
            hidden_states = mint.clamp(mint.round(hidden_states.exp() - self.log_domain_offset), min=0).long()
...
class FastSpeech2ConformerBatchNormConvLayer(nn.Cell):
    def __init__(self, config, layer_id=0):
        ...
        self.batch_norm = mint.nn.BatchNorm1d(out_conv_dim)
        ...
        self.dropout = mint.nn.Dropout(config.speech_decoder_postnet_dropout)
...
class FastSpeech2ConformerSpeechDecoderPostnet(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.feat_out = mint.nn.Linear(config.hidden_size, config.num_mel_bins * config.reduction_factor)
...
class FastSpeech2ConformerPredictorLayer(nn.Cell):
    def __init__(self, input_channels, num_chans, kernel_size, dropout_rate):
        ...
        self.layer_norm = mint.nn.LayerNorm(num_chans)
...
class FastSpeech2ConformerVariancePredictor(nn.Cell):
    def __init__(
        ...
        for idx in range(num_layers):
            ...
            conv_layers.append(layer)
        self.conv_layers = nn.CellList(conv_layers)
        self.linear = mint.nn.Linear(num_chans, 1)
    ...
    def construct(self, encoder_hidden_states, padding_masks=None):
        ...
        Args:
            encoder_hidden_states (`ms.Tensor` of shape `(batch_size, max_text_length, input_dim)`):
                Batch of input sequences.
            padding_masks (`ms.Tensor` of shape `(batch_size, max_text_length)`, *optional*):
                Batch of masks indicating padded part.
...
class FastSpeech2ConformerAttention(nn.Cell):
    ...
    def __init__(self, config: FastSpeech2ConformerConfig, module_config):
        ...
        self.linear_q = mint.nn.Linear(self.hidden_size, self.hidden_size)
        self.linear_k = mint.nn.Linear(self.hidden_size, self.hidden_size)
        self.linear_v = mint.nn.Linear(self.hidden_size, self.hidden_size)
        self.linear_out = mint.nn.Linear(self.hidden_size, self.hidden_size)
        self.dropout = mint.nn.Dropout(p=module_config["attention_dropout_rate"])

        # linear transformation for positional encoding
        self.linear_pos = mint.nn.Linear(self.hidden_size, self.hidden_size, bias=False)
        # these two learnable bias are used in matrix c and matrix d
        # as described in https://arxiv.org/abs/1901.02860 Section 3.3
        self.pos_bias_u = ms.Parameter(mint.zeros((self.num_heads, self.head_dim)))
        self.pos_bias_v = ms.Parameter(mint.zeros((self.num_heads, self.head_dim)))

    def shift_relative_position_tensor(self, pos_tensor):
        """
        Args:
            pos_tensor (ms.Tensor of shape (batch_size, head, time1, 2*time1-1)): Input tensor.
        """
        zero_pad = mint.zeros((*pos_tensor.shape[:3], 1), dtype=pos_tensor.dtype)
        pos_tensor_padded = mint.cat([zero_pad, pos_tensor], dim=-1)
    ...
    def construct(
        ...
        pos_emb: Optional[ms.Tensor] = None,
        ...
        Args:
            hidden_states (`ms.Tensor` of shape `(batch, time2, size)`): Values of the hidden states
            attention_mask (`ms.Tensor` of shape `(batch, time1, time2)`): Mask tensor.
            pos_emb (`ms.Tensor` of shape `(batch, 2*time1-1, size)`): Positional embedding tensor.
            output_attentions (`bool`, *optional*):
                Whether or not to return the attentions tensors of all attention layers. See `attentions` under
                returned tensors for more detail.
        Returns:
            `ms.Tensor`: Output tensor of shape `(batch, time1, d_model)`.
        """
        ...
        matrix_ac = mint.matmul(query_with_bias_u, key_states.permute(0, 2, 3, 1))

        # compute matrix b and matrix d
        # (batch_size, head, time1, 2*time1-1)
        matrix_bd = mint.matmul(query_with_bias_v, pos_encoding.permute(0, 2, 3, 1))
        ...
        if attention_mask is not None:
            ...
            min_value = float(dtype_to_min(scores.dtype))
            scores = scores.masked_fill(attention_mask, min_value)
            attn_weights = mint.softmax(scores, dim=-1).masked_fill(attention_mask, 0.0)
        else:
            attn_weights = mint.softmax(scores, dim=-1)

        attn_weights = self.dropout(attn_weights)
        attn_output = mint.matmul(attn_weights, value_states.transpose(1, 2))
...
class FastSpeech2ConformerConvolutionModule(nn.Cell):
    def __init__(self, config: FastSpeech2ConformerConfig, module_config):
        ...
        self.pointwise_conv1 = nn.Conv1d(
            channels, 2 * channels, kernel_size=1, stride=1, pad_mode="pad", padding=0, has_bias=True
        )
        ...
        self.pointwise_conv2 = nn.Conv1d(
            channels, channels, kernel_size=1, stride=1, pad_mode="pad", padding=0, has_bias=True
        )
    ...
    def construct(self, hidden_states):
        ...
        hidden_states = hidden_states * mint.sigmoid(hidden_states)
...
class FastSpeech2ConformerEncoderLayer(nn.Cell):
    def __init__(self, config: FastSpeech2ConformerConfig, module_config):
        ...
        if self.macaron_style:
            ...
            self.ff_macaron_layer_norm = mint.nn.LayerNorm(config.hidden_size)
            self.ff_scale = 0.5
        else:
            self.ff_scale = 1.0
        ...
        if self.use_cnn_module:
            ...
            self.conv_layer_norm = mint.nn.LayerNorm(config.hidden_size)
            self.final_layer_norm = mint.nn.LayerNorm(config.hidden_size)

        self.ff_layer_norm = mint.nn.LayerNorm(config.hidden_size)

        self.self_attn_layer_norm = mint.nn.LayerNorm(config.hidden_size)

        self.dropout = mint.nn.Dropout(module_config["dropout_rate"])
        self.size = config.hidden_size
        self.normalize_before = module_config["normalize_before"]
        self.concat_after = module_config["concat_after"]
        if self.concat_after:
            self.concat_linear = mint.nn.Linear(config.hidden_size + config.hidden_size, config.hidden_size)
    ...
    def construct(
        ...
        Args:
            hidden_states (`ms.Tensor` of shape `(batch, time, size)`): Input tensor.
            pos_emb (`ms.Tensor` of shape `(1, time, size)`): Positional embeddings tensor.
            attention_mask (`ms.Tensor` of shape `(batch, time)`): Attention mask tensor for the input.
            output_attentions (`bool`, *optional*):
                Whether or not to return the attentions tensors of all attention layers. See `attentions` under
                returned tensors for more detail.
        Returns:
            `ms.Tensor`: Output tensor of shape `(batch, time, size)`.
        ...
        if self.concat_after:
            x_concat = mint.cat((hidden_states, attention_output), dim=-1)
...
class FastSpeech2ConformerMultiLayeredConv1d(nn.Cell):
    ...
    def construct(self, hidden_states):
        ...
        Args:
            hidden_states (ms.Tensor): Batch of input tensors (batch_size, time, input_channels).

        Returns:
            ms.Tensor: Batch of output tensors (batch_size, time, hidden_channels).
        """
        hidden_states = hidden_states.transpose(-1, 1)
...
class FastSpeech2ConformerRelPositionalEncoding(nn.Cell):
    ...
    def __init__(self, config: FastSpeech2ConformerConfig, module_config):
        ...
        self.dropout = mint.nn.Dropout(p=module_config["positional_dropout_rate"])
        self.pos_enc = None
        self.max_len = 5000
        self.extend_pos_enc(ms.tensor(0.0).expand((1, self.max_len)))
    ...
    def extend_pos_enc(self, x):
        ...
        pos_enc_positive = mint.zeros((x.shape[1], self.embed_dim))
        pos_enc_negative = mint.zeros((x.shape[1], self.embed_dim))
        position = mint.arange(0, x.shape[1], dtype=ms.int64).float().unsqueeze(1)
        div_term = mint.exp(
            mint.arange(0, self.embed_dim, 2, dtype=ms.int64).float() * -(math.log(10000.0) / self.embed_dim)
        )
        pos_enc_positive[:, 0::2] = mint.sin(position * div_term)
        pos_enc_positive[:, 1::2] = mint.cos(position * div_term)
        pos_enc_negative[:, 0::2] = mint.sin(-1 * position * div_term)
        pos_enc_negative[:, 1::2] = mint.cos(-1 * position * div_term)

        # Reserve the order of positive indices and concat both positive and
        # negative indices. This is used to support the shifting trick
        # as in https://arxiv.org/abs/1901.02860
        pos_enc_positive = mint.flip(pos_enc_positive, [0]).unsqueeze(0)
        pos_enc_negative = pos_enc_negative[1:].unsqueeze(0)
        pos_enc = mint.cat([pos_enc_positive, pos_enc_negative], dim=1)
        self.pos_enc = pos_enc.to(dtype=x.dtype)

    def construct(self, feature_representation):
        """
        Args:
            feature_representation (`ms.Tensor` of shape (batch_size, time, `*`)):
                Input tensor.

        Returns:
            `ms.Tensor`: Encoded tensor (batch_size, time, `*`).
        """
...
class FastSpeech2ConformerLoss(nn.Cell):
    def __init__(self, config: FastSpeech2ConformerConfig):
        ...
        self.l1_criterion = mint.nn.L1Loss(reduction=reduction)
        self.mse_criterion = mint.nn.MSELoss(reduction=reduction)
        self.duration_criterion = mint.nn.MSELoss(reduction=reduction)
        self.log_domain_offset = 1.0
    ...
    def construct(
        ...
        """
        Args:
            outputs_after_postnet (`ms.Tensor` of shape `(batch_size, max_spectrogram_length, num_mel_bins)`):
                Batch of outputs after postnet.
            outputs_before_postnet (`ms.Tensor` of shape `(batch_size, max_spectrogram_length, num_mel_bins)`):
                Batch of outputs before postnet.
            duration_outputs (`ms.Tensor` of shape `(batch_size, max_text_length)`):
                Batch of outputs of duration predictor.
            pitch_outputs (`ms.Tensor` of shape `(batch_size, max_text_length, 1)`):
                Batch of outputs of pitch predictor.
            energy_outputs (`ms.Tensor` of shape `(batch_size, max_text_length, 1)`):
                Batch of outputs of energy predictor.
            spectrogram_labels (`ms.Tensor` of shape `(batch_size, max_spectrogram_length, num_mel_bins)`):
                Batch of target features.
            duration_labels (`ms.Tensor` of shape `(batch_size, max_text_length)`): Batch of durations.
            pitch_labels (`ms.Tensor` of shape `(batch_size, max_text_length, 1)`):
                Batch of target token-averaged pitch.
            energy_labels (`ms.Tensor` of shape `(batch_size, max_text_length, 1)`):
                Batch of target token-averaged energy.
            duration_mask (`ms.Tensor`):
                Mask used to discern which values the duration loss should be calculated for.
            spectrogram_mask (`ms.Tensor`):
                Mask used to discern which values the spectrogam loss should be calculated for.

        Returns:
            `tuple(ms.Tensor)`: Tuple of tensors containing, in order, the L1 loss value, duration predictor
        ...
        duration_labels = mint.log(duration_labels.float() + self.log_domain_offset)
        ...
        if self.use_weighted_masking:
            spectrogram_mask = mint.nn.functional.pad(
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, mint.nn.LayerNorm):
            ...
            if module.bias is not None:
                # key = math.sqrt(module.group / (module.in_channels * module.kernel_size[0]))
                key = math.sqrt(module.group / (module.in_channels * module.kernel_size[1]))
                module.bias.data.uniform_(from_=-key, to=key)
...
class FastSpeech2ConformerModel(FastSpeech2ConformerPreTrainedModel):
    ...
    def __init__(self, config: FastSpeech2ConformerConfig):
        ...
        if self.multilingual_model:
            self.language_id_embedding = mint.nn.Embedding(config.num_languages, self.hidden_size)

        self.multispeaker_model = config.num_speakers is not None and config.num_speakers > 1
        if self.multispeaker_model:
            self.speaker_id_embedding = mint.nn.Embedding(config.num_speakers, config.hidden_size)

        self.speaker_embed_dim = config.speaker_embed_dim
        if self.speaker_embed_dim:
            self.projection = mint.nn.Linear(config.hidden_size + self.speaker_embed_dim, config.hidden_size)
    ...
    def construct(
        ...
        input_ids: ms.Tensor,
        attention_mask: Optional[ms.Tensor] = None,
        spectrogram_labels: Optional[ms.Tensor] = None,
        duration_labels: Optional[ms.Tensor] = None,
        pitch_labels: Optional[ms.Tensor] = None,
        energy_labels: Optional[ms.Tensor] = None,
        speaker_ids: Optional[ms.Tensor] = None,
        lang_ids: Optional[ms.Tensor] = None,
        speaker_embedding: Optional[ms.Tensor] = None,
        return_dict: Optional[bool] = None,
        ...
        Args:
            ...
            attention_mask (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*, defaults to `None`):
                Mask to avoid performing convolution and attention on padding token indices. Mask values selected in
                `[0, 1]`: 0 for tokens that are **masked**, 1 for tokens that are **not masked**.
            spectrogram_labels (`ms.Tensor` of shape `(batch_size, max_spectrogram_length, num_mel_bins)`, *optional*, defaults to `None`):
                Batch of padded target features.
            duration_labels (`ms.Tensor` of shape `(batch_size, sequence_length + 1)`, *optional*, defaults to `None`):
                Batch of padded durations.
            pitch_labels (`ms.Tensor` of shape `(batch_size, sequence_length + 1, 1)`, *optional*, defaults to `None`):
                Batch of padded token-averaged pitch.
            energy_labels (`ms.Tensor` of shape `(batch_size, sequence_length + 1, 1)`, *optional*, defaults to `None`):
                Batch of padded token-averaged energy.
            speaker_ids (`ms.Tensor` of shape `(batch_size, 1)`, *optional*, defaults to `None`):
                Speaker ids used to condition features of speech output by the model.
            lang_ids (`ms.Tensor` of shape `(batch_size, 1)`, *optional*, defaults to `None`):
                Language ids used to condition features of speech output by the model.
            speaker_embedding (`ms.Tensor` of shape `(batch_size, embedding_dim)`, *optional*, defaults to `None`):
        ...
        >>> inputs = tokenizer("some text to convert to speech", return_tensors="np")
        ...
        if attention_mask is None:
            attention_mask = mint.ones(input_ids.shape)
        ...
        if self.speaker_embed_dim is not None and speaker_embedding is not None:
            embeddings_expanded = (
                mint.nn.functional.normalize(speaker_embedding).unsqueeze(1).expand((-1, hidden_states.shape[1], -1))
            )
            hidden_states = self.projection(mint.cat([hidden_states, embeddings_expanded], dim=-1))

        # forward duration predictor and variance predictors
        duration_mask = ~attention_mask.bool()

        if self.stop_gradient_from_pitch_predictor:
            pitch_predictions = self.pitch_predictor(ops.stop_gradient(hidden_states), duration_mask.unsqueeze(-1))
        else:
            pitch_predictions = self.pitch_predictor(hidden_states, duration_mask.unsqueeze(-1))

        if self.stop_gradient_from_energy_predictor:
            energy_predictions = self.energy_predictor(ops.stop_gradient(hidden_states), duration_mask.unsqueeze(-1))
        else:
...
class FastSpeech2ConformerHifiGan(PreTrainedModel):
    ...
    def __init__(self, config: FastSpeech2ConformerHifiGanConfig):
        ...
        # self.register_buffer("mean", mint.zeros(config.model_in_dim))
        # self.register_buffer("scale", mint.ones(config.model_in_dim))
        self.mean = ms.Parameter(mint.zeros(config.model_in_dim), name="mean")
        self.scale = ms.Parameter(mint.ones(config.model_in_dim), name="scale")
    ...
    def construct(self, spectrogram: ms.Tensor) -> ms.Tensor:
        ...
        for i in range(self.num_upsamples):
            hidden_states = mint.nn.functional.leaky_relu(hidden_states, self.config.leaky_relu_slope)
...
class FastSpeech2ConformerWithHifiGan(PreTrainedModel):
    ...
    def construct(
        ...
        ```python
        >>> from mindone.transformers import FastSpeech2ConformerWithHifiGan
        >>> from transformers import FastSpeech2ConformerTokenizer
        >>> import mindspore as ms
