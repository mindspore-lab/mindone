...
class DPRContextEncoderOutput(ModelOutput):
    ...
    Args:
        pooler_output (`ms.Tensor` of shape `(batch_size, embeddings_size)`):
...
class DPRReaderOutput(ModelOutput):
    ...
    Args:
        start_logits (`ms.Tensor` of shape `(n_passages, sequence_length)`):
            Logits of the start index of the span for each passage.
        end_logits (`ms.Tensor` of shape `(n_passages, sequence_length)`):
            Logits of the end index of the span for each passage.
        relevance_logits (`ms.Tensor` of shape `(n_passages, )`):
    ...
    start_logits: ms.Tensor
    end_logits: Optional[ms.Tensor] = None
    relevance_logits: Optional[ms.Tensor] = None
    hidden_states: Optional[tuple[ms.Tensor, ...]] = None
    attentions: Optional[tuple[ms.Tensor, ...]] = None
...
class DPREncoder(DPRPreTrainedModel):
    ...
    def __init__(self, config: DPRConfig):
        ...
        if self.projection_dim > 0:
            self.encode_proj = mint.nn.Linear(self.bert_model.config.hidden_size, config.projection_dim)
...
class DPRSpanPredictor(DPRPreTrainedModel):
    ...
    def __init__(self, config: DPRConfig):
        ...
        self.qa_outputs = mint.nn.Linear(self.encoder.embeddings_size, 2)
        self.qa_classifier = mint.nn.Linear(self.encoder.embeddings_size, 1)
        # Initialize weights and apply final processing
        self.post_init()
...
class DPRContextEncoder(DPRPretrainedContextEncoder):
    ...
    def construct(
        ...
        >>> from mindone.transformers import DPRContextEncoder
        >>> import mindspore as ms

        >>> tokenizer = DPRContextEncoderTokenizer.from_pretrained("facebook/dpr-ctx_encoder-single-nq-base")
        >>> model = DPRContextEncoder.from_pretrained("facebook/dpr-ctx_encoder-single-nq-base")
        >>> input_ids = tokenizer("Hello, is my dog cute ?", return_tensors="np")["input_ids"]
        >>> embeddings = model(ms.tensor(input_ids)).pooler_output
        ```"""
        ...
        if attention_mask is None:
            attention_mask = mint.ones(input_shape) if input_ids is None else (input_ids != self.config.pad_token_id)
...
class DPRQuestionEncoder(DPRPretrainedQuestionEncoder):
    ...
    def construct(
        ...
        >>> from mindone.transformers import DPRQuestionEncoder
        >>> import mindspore as ms
...
class DPRReader(DPRPretrainedReader):
    ...
    def construct(
        ...
        r"""
        input_ids (`tuple[ms.Tensor]` of shapes `(n_passages, sequence_length)`):
            ...
            [What are input IDs?](../glossary#input-ids)
        inputs_embeds (`ms.Tensor` of shape `(n_passages, sequence_length, hidden_size)`, *optional*):
        ...
        >>> from mindone.transformers import DPRReader
        >>> import mindspore as ms
        >>> import numpy as np
        ...
        ...     return_tensors="np",
        ... )
        >>> for key, value in encoded_inputs.items():  # by default input numpy array or list
        >>>     if isinstance(value, np.ndarray):
        >>>         encoded_inputs[key] = ms.Tensor(value)
        >>>     elif isinstance(value, list):
        >>>         encoded_inputs[key] = ms.Tensor(value)
        >>>     if encoded_inputs[key].dtype == ms.int64:
        >>>         encoded_inputs[key] = encoded_inputs[key].to(ms.int32)
