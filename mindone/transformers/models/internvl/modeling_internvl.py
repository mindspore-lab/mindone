        ...
        if isinstance(module, InternVLVisionEmbeddings):
            ...
            if module.position_embeddings is not None:
                module.position_embeddings.set_data(
                    initializer(Constant(0.0), module.position_embeddings.shape, module.position_embeddings.dtype)
                )
            ...
            module.lambda_2.set_data(
                initializer(Constant(self.config.layer_scale_init_value), module.lambda_2.shape, module.lambda_2.dtype)
            )
...
class InternVLVisionEmbeddings(nn.Cell):
    ...
    def interpolate_pos_encoding(self, embeddings: ms.Tensor, height: int, width: int) -> ms.Tensor:
        ...
        # always interpolate to ensure the exported model works for dynamic input shapes
    ...
    def construct(
        ...
        if bool_masked_pos is not None:
            ...
            w = bool_masked_pos.unsqueeze(-1).astype(mask_tokens.dtype)
...
class InternVLVisionLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config: InternVLVisionConfig) -> None:
        ...
        self.lambda_1 = Parameter(init_values * mint.ones(config.hidden_size), requires_grad=True)
        self.lambda_2 = Parameter(init_values * mint.ones(config.hidden_size), requires_grad=True)
        self.dropout = mint.nn.Dropout(config.hidden_dropout_prob)
...
class InternVLVisionEncoder(nn.Cell):
    def __init__(self, config: InternVLVisionConfig) -> None:
        super().__init__()
        self.config = config
        self.layer = nn.CellList([InternVLVisionLayer(config) for i in range(config.num_hidden_layers)])
...
class InternVLModel(InternVLPreTrainedModel):
    ...
    def get_image_features(
        ...
        **kwargs,
        ...
        Returns:
            vision_features (`mindspore.Tensor`): Image feature tensor of shape `(num_images, image_length, embed_dim)`.
        """
    ...
    def construct(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if pixel_values is not None:
            ...
            special_image_mask = special_image_mask.unsqueeze(-1).broadcast_to(inputs_embeds.shape)
    ...
    def pixel_shuffle(self, vision_features: ms.Tensor, scale_factor: float = 0.5):
        ...
        Args:
            vision_features (`mindspore.Tensor`):
...
class InternVLForConditionalGeneration(InternVLPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> inputs = processor.apply_chat_template(messages, add_generation_prompt=True, tokenize=True, return_dict=True, return_tensors="np")
        >>> generate_ids = model.generate(**{k: ms.Tensor(v) for k, v in inputs.items()}, max_new_tokens=200)
