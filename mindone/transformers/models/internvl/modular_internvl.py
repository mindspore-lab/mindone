...
import numpy as np
from transformers import InternVLConfig, InternVLVisionConfig, JanusVisionConfig
...
# FIXME: seperate this file after Janus implementation
# adapted from https://github.com/huggingface/transformers/blob/main/src/transformers/models/janus/modeling_janus.py
...
class InternVLVisionEmbeddings(nn.Module):
    ...
    def __init__(self, config: InternVLVisionConfig) -> None:
        ...
        if config.use_absolute_position_embeddings:
            self.position_embeddings = nn.Parameter(mint.zeros(1, num_patches + 1, config.hidden_size))
        else:
...
class InternVLVisionLayer(nn.Cell):
    ...
    def __init__(self, config: InternVLVisionConfig) -> None:
        ...
        self.lambda_1 = nn.Parameter(init_values * mint.ones(config.hidden_size), requires_grad=True)
        self.lambda_2 = nn.Parameter(init_values * mint.ones(config.hidden_size), requires_grad=True)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
