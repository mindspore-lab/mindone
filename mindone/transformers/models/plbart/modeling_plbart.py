...
class PLBartScaledWordEmbedding(mint.nn.Embedding):
    """
...
class PLBartLearnedPositionalEmbedding(mint.nn.Embedding):
    """
    ...
    def construct(self, input_ids: ms.Tensor, past_key_values_length: int = 0, position_ids: ms.Tensor = None):
    ...
    def construct(
        ...
        **kwargs,
        ...
        if is_cross_attention and past_key_value is not None and is_updated:
            ...
            key_states = key_states.view(*kv_input_shape).swapaxes(1, 2)
            value_states = value_states.view(*kv_input_shape).swapaxes(1, 2)
...
class PLBartEncoderLayer(nn.Cell):
...
class PLBartEncoder(PLBartPreTrainedModel):
    ...
    def __init__(self, config: PLBartConfig, embed_tokens: Optional[mint.nn.Embedding] = None):
        ...
        self.layers = nn.CellList([PLBartEncoderLayer(config, layer_idx=i) for i in range(config.encoder_layers)])
...
class PLBartDecoderLayer(nn.Cell):
    def __init__(self, config: PLBartConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.embed_dim = config.d_model
...
class PLBartDecoder(PLBartPreTrainedModel):
    ...
    def __init__(self, config: PLBartConfig, embed_tokens: Optional[mint.nn.Embedding] = None):
        ...
        self.layers = nn.CellList([PLBartDecoderLayer(config, layer_idx=i) for i in range(config.decoder_layers)])
...
class PLBartModel(PLBartPreTrainedModel):
    ...
    def construct(
        ...
        decoder_attention_mask (:
            obj:*ms.Tensor* of shape `(batch_size, target_sequence_length)`, *optional*):
            Default behavior:
            generate a tensor that ignores pad tokens in `decoder_input_ids`. Causal mask will also be used by default.
        cross_attn_head_mask (:
            obj:*ms.Tensor* of shape `(decoder_layers, decoder_attention_heads)`, *optional*):
            Mask to nullify
...
class PLBartForConditionalGeneration(PLBartPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        >>> from mindone.transformers import PLBartForConditionalGeneration
        >>> import mindspore as ms
...
class PLBartClassificationHead(nn.Cell):
...
class PLBartForCausalLM(PLBartPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import PLBartForCausalLM
        >>> import numpy as np
