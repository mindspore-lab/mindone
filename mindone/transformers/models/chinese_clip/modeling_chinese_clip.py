...
def chinese_clip_loss(similarity: ms.Tensor) -> ms.Tensor:
...
class ChineseCLIPTextEmbeddings(nn.Cell):
...
class ChineseCLIPVisionEmbeddings(nn.Cell):
...
class ChineseCLIPTextSelfAttention(nn.Cell):
    ...
    def construct(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in ChineseCLIPTextModel construct() function)
...
class ChineseCLIPTextSelfOutput(nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class ChineseCLIPTextAttention(nn.Cell):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
...
class ChineseCLIPVisionAttention(nn.Cell):
...
class ChineseCLIPTextIntermediate(nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class ChineseCLIPTextOutput(nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class ChineseCLIPVisionMLP(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class ChineseCLIPTextLayer(nn.Cell):
    def __init__(self, config):
        super().__init__()
    ...
    def construct(
        ...
        layer_output = apply_chunking_to_forward(
            self.feed_construct_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attention_output
        )
    ...
    def feed_construct_chunk(self, attention_output):
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output)
        return layer_output


class ChineseCLIPVisionLayer(nn.Cell):
...
class ChineseCLIPTextPooler(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.dense = mint.nn.Linear(config.hidden_size, config.hidden_size)
...
class ChineseCLIPPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, ChineseCLIPVisionEmbeddings):
            ...
            normal_(module.word_embeddings.weight, mean=0.0, std=self.config.initializer_range)
            normal_(module.position_embeddings.weight, mean=0.0, std=self.config.initializer_range)
            normal_(module.token_type_embeddings.weight, mean=0.0, std=self.config.initializer_range)
            for embedding in [module.word_embeddings, module.position_embeddings, module.token_type_embeddings]:
                if embedding.padding_idx is not None:
                    embedding.weight[embedding.padding_idx] = 0
...
class ChineseCLIPTextEncoder(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.CellList([ChineseCLIPTextLayer(config) for _ in range(config.num_hidden_layers)])
...
class ChineseCLIPVisionEncoder(nn.Cell):
    """
    ...
    def __init__(self, config: ChineseCLIPConfig):
        super().__init__()
        self.config = config
        self.layers = nn.CellList([ChineseCLIPVisionLayer(config) for _ in range(config.num_hidden_layers)])
        self.gradient_checkpointing = False
...
class ChineseCLIPVisionTransformer(nn.Cell):
    def __init__(self, config: ChineseCLIPVisionConfig):
        super().__init__()
        self.config = config
...
class ChineseCLIPVisionModel(ChineseCLIPPreTrainedModel):
    ...
    def construct(
        ...
        >>> import requests
        >>> from mindone.transformers import AutoProcessor, ChineseCLIPVisionModel

        >>> model = ChineseCLIPVisionModel.from_pretrained("OFA-Sys/chinese-clip-vit-base-patch16", revision="refs/pr/1")
        >>> processor = AutoProcessor.from_pretrained("OFA-Sys/chinese-clip-vit-base-patch16", revision="refs/pr/1")
...
class ChineseCLIPModel(ChineseCLIPPreTrainedModel):
    ...
    def get_text_features(
        ...
        >>> from mindone.transformers import ChineseCLIPModel

        >>> model = ChineseCLIPModel.from_pretrained("OFA-Sys/chinese-clip-vit-base-patch16", revision="refs/pr/1")
        >>> tokenizer = AutoTokenizer.from_pretrained("OFA-Sys/chinese-clip-vit-base-patch16", revision="refs/pr/1")

        >>> inputs = tokenizer(["杰尼龟", "妙蛙种子", "小火龙", "皮卡丘"], padding=True, return_tensors="np")
        >>> inputs = {k: ms.Tensor(v) for k, v in inputs.items()}
        >>> text_features = model.get_text_features(**inputs)
        >>> text_features = text_features / mint.norm(text_features, p=2, dim=-1, keepdim=True)
        ```"""
    ...
    def get_image_features(
        ...
        >>> import requests
        >>> import mindspore.mint as mint
        >>> from mindone.transformers import AutoProcessor, ChineseCLIPModel
        ...
        >>> image_features = image_features / mint.norm(image_features, p=2, dim=-1, keepdim=True)
        ```"""
        # Use CHINESE_CLIP model's config for some fields (if specified) instead of those of vision & text components.
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
    ...
    def construct(
        ...
        >>> inputs = processor(text=["杰尼龟", "妙蛙种子", "小火龙", "皮卡丘"], images=image, return_tensors="ms", padding=True)
