...
from transformers.models.pvt.configuration_pvt import PvtConfig
...
class PvtPatchEmbeddings(nn.Cell):
    ...
    def __init__(
        ...
        self.position_embeddings = ms.Parameter(
            mint.randn(1, num_patches + 1 if cls_token else num_patches, hidden_size), name="position_embeddings"
        )
        self.cls_token = ms.Parameter(mint.zeros((1, 1, hidden_size))) if cls_token else None
        self.projection = mint.nn.Conv2d(num_channels, hidden_size, kernel_size=stride, stride=patch_size)
    ...
    def interpolate_pos_encoding(self, embeddings: ms.Tensor, height: int, width: int) -> ms.Tensor:
        ...
        if embeddings.dtype == ms.bfloat16:
            interpolated_embeddings = mint.nn.functional.interpolate(
                embeddings.half(), size=(height, width), mode="bilinear"
            ).to(ms.bfloat16)
        else:
            interpolated_embeddings = mint.nn.functional.interpolate(embeddings, size=(height, width), mode="bilinear")
    ...
    def construct(self, pixel_values: ms.Tensor) -> Tuple[ms.Tensor, int, int]:
        ...
        *_, height, width = patch_embed.shape
        patch_embed = patch_embed.flatten(2).swapaxes(1, 2)
        embeddings = self.layer_norm(patch_embed)
        if self.cls_token is not None:
            cls_token = self.cls_token.broadcast_to((batch_size, -1, -1))
            embeddings = mint.cat((cls_token, embeddings), dim=1)
            position_embeddings = self.interpolate_pos_encoding(self.position_embeddings[:, 1:], height, width)
            position_embeddings = mint.cat((self.position_embeddings[:, :1], position_embeddings), dim=1)
        else:
...
class PvtEfficientSelfAttention(nn.Cell):
    ...
    def __init__(self, config: PvtConfig, hidden_size: int, num_attention_heads: int, sequences_reduction_ratio: float):
        ...
        if sequences_reduction_ratio > 1:
            self.sequence_reduction = mint.nn.Conv2d(
...
class PvtLayer(nn.Cell):
    def __init__(
        ...
        self.drop_path = PvtDropPath(drop_path) if drop_path > 0.0 else mint.nn.Identity()
...
class PvtEncoder(nn.Cell):
    def __init__(self, config: PvtConfig):
        ...
        # Layer norms
        self.layer_norm = mint.nn.LayerNorm(config.hidden_sizes[-1], eps=config.layer_norm_eps)
