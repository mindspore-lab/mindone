...
from transformers.models.roc_bert.configuration_roc_bert import RoCBertConfig
...
_TOKEN_CLASS_EXPECTED_OUTPUT = ["S-EVENT", "S-FAC", "I-ORDINAL", "I-ORDINAL", "E-ORG", "E-LANGUAGE", "E-ORG",
                                "E-ORG", "E-ORG", "E-ORG", "I-EVENT", "S-TIME", "S-TIME", "E-LANGUAGE", "S-TIME",
                                "E-DATE", "I-ORDINAL", "E-QUANTITY", "E-LANGUAGE", "S-TIME", "B-ORDINAL",
                                "S-PRODUCT", "E-LANGUAGE", "E-LANGUAGE", "E-ORG", "E-LOC", "S-TIME", "I-ORDINAL",
                                "S-FAC", "O", "S-GPE", "I-EVENT", "S-GPE", "E-LANGUAGE", "E-ORG", "S-EVENT",
                                "S-FAC", "S-FAC", "S-FAC", "E-ORG", "S-FAC", "E-ORG", "S-GPE"]  # fmt: skip
...
class RoCBertEmbeddings(nn.Cell):
    ...
    def __init__(self, config):
        ...
        if config.concat_input:
            ...
            if self.enable_shape:
                ...
                input_dim += shape_dim
            self.map_inputs_layer = mint.nn.Linear(input_dim, config.hidden_size)
        else:
        ...
        self.register_buffer(
            "token_type_ids",
            mint.zeros(self.position_ids.shape, dtype=ms.int32),
            persistent=False,
        )
    ...
    def construct(
        ...
        if self.map_inputs_layer is None:
            ...
            denominator = 1
            embedding_in = mint.clone(embeddings)
            ...
            embedding_in = mint.clone(inputs_embeds)
            if self.enable_shape:
                if input_shape_ids is None:
                    input_shape_ids = mint.zeros(input_shape, dtype=ms.int32)
                embedding_shape = self.shape_embed(input_shape_ids)
                embedding_in = mint.cat((embedding_in, embedding_shape), -1)
            if self.enable_pronunciation:
                if input_pronunciation_ids is None:
                    input_pronunciation_ids = mint.zeros(input_shape, dtype=ms.int32)
                embedding_pronunciation = self.pronunciation_embed(input_pronunciation_ids)
                embedding_in = mint.cat((embedding_in, embedding_pronunciation), -1)
...
class RoCBertEncoder(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.CellList([RoCBertLayer(config) for _ in range(config.num_hidden_layers)])
...
ROC_BERT_INPUTS_DOCSTRING = r"""
    Args:
        ...
        input_shape_ids (`ms.Tensor` of shape `({0})`):
        ...
        input_pronunciation_ids (`ms.Tensor` of shape `({0})`):
...
class RoCBertModel(RoCBertPreTrainedModel):
    ...
    def construct(
        ...
        input_shape_ids: Optional[ms.Tensor] = None,
        input_pronunciation_ids: Optional[ms.Tensor] = None,
        attention_mask: Optional[ms.Tensor] = None,
...
class RoCBertForPreTraining(RoCBertPreTrainedModel):
    ...
    def construct(
        ...
        attention_mask: Optional[ms.Tensor] = None,
        token_type_ids: Optional[ms.Tensor] = None,
        attack_input_ids: Optional[ms.Tensor] = None,
        attack_input_shape_ids: Optional[ms.Tensor] = None,
        attack_input_pronunciation_ids: Optional[ms.Tensor] = None,
        attack_attention_mask: Optional[ms.Tensor] = None,
        attack_token_type_ids: Optional[ms.Tensor] = None,
        position_ids: Optional[ms.Tensor] = None,
        head_mask: Optional[ms.Tensor] = None,
        inputs_embeds: Optional[ms.Tensor] = None,
        labels_input_ids: Optional[ms.Tensor] = None,
        labels_input_shape_ids: Optional[ms.Tensor] = None,
        labels_input_pronunciation_ids: Optional[ms.Tensor] = None,
        labels_attention_mask: Optional[ms.Tensor] = None,
        labels_token_type_ids: Optional[ms.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        **kwargs,
        ...
        r"""
            attack_input_ids (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                attack sample ids for computing the contrastive loss. Indices should be in `[-100, 0, ...,
                config.vocab_size]` (see `input_ids` docstring) Tokens with indices set to `-100` are ignored (masked),
                the loss is only computed for the tokens with labels in `[0, ..., config.vocab_size]`
            attack_input_shape_ids (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                attack sample shape ids for computing the contrastive loss. Indices should be in `[-100, 0, ...,
                config.vocab_size]` (see `input_ids` docstring) Tokens with indices set to `-100` are ignored (masked),
                the loss is only computed for the tokens with labels in `[0, ..., config.vocab_size]`
            attack_input_pronunciation_ids (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            labels_input_ids (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                target ids for computing the contrastive loss and masked_lm_loss . Indices should be in `[-100, 0, ...,
                config.vocab_size]` (see `input_ids` docstring) Tokens with indices set to `-100` are ignored (masked),
                the loss is only computed for the tokens with labels in `[0, ..., config.vocab_size]`
            labels_input_shape_ids (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                ...
                (masked), the loss is only computed for the tokens with labels in `[0, ..., config.vocab_size]`
            labels_input_pronunciation_ids (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
        ...
        if labels_input_ids is not None:
            ...
            if attack_input_ids is not None:
                ...
                target_inputs = mint.clone(labels_input_ids)
                ...
                pooled_output_norm = mint.nn.functional.normalize(pooled_output, dim=-1)
                labels_pooled_output_norm = mint.nn.functional.normalize(labels_pooled_output, dim=-1)
                attack_pooled_output_norm = mint.nn.functional.normalize(attack_pooled_output, dim=-1)

                sim_matrix = mint.matmul(pooled_output_norm, attack_pooled_output_norm.T)  # batch_size * hidden_dim
                sim_matrix_target = mint.matmul(labels_pooled_output_norm, attack_pooled_output_norm.T)
                batch_labels = ms.tensor(list(range(batch_size)))
                contrastive_loss = (
...
class RoCBertForMaskedLM(RoCBertPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        if input_shape_ids is not None:
            input_shape_ids = mint.cat([input_shape_ids, dummy_token], dim=1)
        if input_pronunciation_ids is not None:
            input_pronunciation_ids = mint.cat([input_pronunciation_ids, dummy_token], dim=1)
...
class RoCBertForMultipleChoice(RoCBertPreTrainedModel):
    ...
    def construct(
        ...
        input_shape_ids = input_shape_ids.view(-1, input_shape_ids.shape[-1]) if input_shape_ids is not None else None
