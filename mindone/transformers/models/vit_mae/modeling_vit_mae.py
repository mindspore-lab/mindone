...
class ViTMAEModelOutput(ModelOutput):
    ...
    ids_restore: Optional[ms.Tensor] = None
...
class ViTMAEEmbeddings(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.position_embeddings = ms.Parameter(
            mint.zeros((1, self.num_patches + 1, config.hidden_size)), requires_grad=False, name="position_embeddings"
        )
    ...
    def random_masking(self, sequence, noise=None):
        ...
        if noise is None:
            noise = mint.rand(batch_size, seq_length)  # noise in [0, 1]
        ...
        sequence_unmasked = mint.gather(sequence, dim=1, index=ids_keep.unsqueeze(-1).tile((1, 1, dim)))

        # generate the binary mask: 0 is keep, 1 is remove
        mask = mint.ones([batch_size, seq_length])
        mask[:, :len_keep] = 0
        # unshuffle to get the binary mask
        mask = mint.gather(mask, dim=1, index=ids_restore)
...
class ViTMAEDecoder(nn.Cell):
    def __init__(self, config, num_patches):
        super().__init__()
        self.decoder_embed = mint.nn.Linear(config.hidden_size, config.decoder_hidden_size, bias=True)
        self.mask_token = ms.Parameter(mint.zeros((1, 1, config.decoder_hidden_size)), name="mask_token")
        self.decoder_pos_embed = ms.Parameter(
            mint.zeros((1, num_patches + 1, config.decoder_hidden_size)), requires_grad=False, name="decoder_pos_embed"
        )  # fixed sin-cos embedding
        ...
        self.decoder_pred = mint.nn.Linear(
    ...
    def interpolate_pos_encoding(self, embeddings: ms.Tensor) -> ms.Tensor:
        """
    ...
    def construct(
        ...
        x_ = mint.cat([x[:, 1:, :], mask_tokens], dim=1)  # no cls token
        # unshuffle
        x_ = mint.gather(x_, dim=1, index=ids_restore.unsqueeze(-1).tile((1, 1, x.shape[2])))
        x = mint.cat([x[:, :1, :], x_], dim=1)  # append cls token
        # add pos embed
        if interpolate_pos_encoding:
...
class ViTMAEForPreTraining(ViTMAEPreTrainedModel):
    ...
    def patchify(self, pixel_values, interpolate_pos_encoding: bool = False):
        ...
        patchified_pixel_values = mint.einsum("nchpwq->nhwpqc", patchified_pixel_values)
    ...
    def unpatchify(self, patchified_pixel_values, original_image_size: Optional[Tuple[int, int]] = None):
        ...
        if num_patches_h * num_patches_w != patchified_pixel_values.shape[1]:
            raise ValueError(
                f"The number of patches in the patchified pixel values {patchified_pixel_values.shape[1]}, does not match the number of patches on original image {num_patches_h}*{num_patches_w}"  # noqa: E501
            )
        ...
        patchified_pixel_values = mint.einsum("nhwpqc->nchpwq", patchified_pixel_values)
