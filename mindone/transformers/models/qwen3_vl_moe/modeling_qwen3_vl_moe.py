...
class Qwen3VLMoeTextRMSNorm(nn.Cell):
...
class Qwen3VLMoeTextRouter(nn.Cell):
    ...
    def construct(self, hidden_states, router_logits):
        ...
        router_weights = mint.zeros_like(router_logits).scatter_(1, router_indices, routing_weights)
        return router_weights, router_logits, router_indices


class Qwen3VLMoeTextExperts(nn.Cell):
    def __init__(self, config):
        super().__init__()
    ...
    def construct(self, hidden_states: ms.Tensor, routing_weights: ms.Tensor, router_indices: ms.Tensor) -> ms.Tensor:
        ...
        Args:
            ...
            routing_weights (ms.Tensor): (batch_size * token_num, num_experts)
            router_indices (ms.Tensor): (batch_size * token_num, top_k)
        Returns:
            ms.Tensor
        """
...
class Qwen3VLMoeTextSparseMoeBlock(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.num_experts = config.num_experts
...
class Qwen3VLMoeTextAttention(nn.Cell):
...
class Qwen3VLMoeTextMLP(nn.Cell):
...
class Qwen3VLMoeVisionPatchEmbed(nn.Cell):
...
class Qwen3VLMoeVisionRotaryEmbedding(nn.Cell):
...
class Qwen3VLMoeVisionAttention(nn.Cell):
...
class Qwen3VLMoeVisionModel(Qwen3VLMoePreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.blocks = nn.CellList([Qwen3VLMoeVisionBlock(config) for _ in range(config.depth)])
...
class Qwen3VLMoeTextRotaryEmbedding(nn.Cell):
    inv_freq: ms.Tensor  # fix linting for `register_buffer`
