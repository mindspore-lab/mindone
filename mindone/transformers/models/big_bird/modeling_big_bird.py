...
class BigBirdBlockSparseAttention(nn.Cell):
    ...
    def bigbird_block_sparse_attention(
        ...
        # attention is calculated separately for q[0], q[1], q[2:-2], q[-2], q[-1] in order to
        # use special trick of shifting tokens (for calculating sliding attention)
        ...
        # [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, -1] x
        # [bsz, n_heads, from_seq_len//from_block_size-4, n_rand_blocks*to_block_size, -1]
        ...
        # [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, n_rand_blocks*to_block_size] x
        # [bsz, n_heads, from_seq_len//from_block_size-4, n_rand_blocks*to_block_size, -1]
        context_layer += self.torch_bmm_nd(
        ...
        )  # [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, to_block_size] x
        # [bsz, n_heads, to_block_size, -1] ==> [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, -1]
        context_layer += mint.einsum(
            "bhlqk,bhkd->bhlqd", attn_weights[:, :, :, :, -to_block_size:], blocked_value_matrix[:, :, -1]
        )  # [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, to_block_size]
        # x [bsz, n_heads, to_block_size, -1] ==> [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, -1]
    ...
    def construct(
        ...
        **kwargs,  # NOOP kwargs, for now
        ...
        encoder_attention_mask (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            - 1 for tokens that are **not masked**,
            - 0 for tokens that are **masked**.
        past_key_values (`tuple(tuple(ms.Tensor))` of length `config.n_layers` with each tuple having
