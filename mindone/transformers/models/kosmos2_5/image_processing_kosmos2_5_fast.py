...
def torch_extract_patches(image_tensor, patch_height, patch_width):
    ...
    """
    patches = mindspore.mint.nn.functional.unfold(image_tensor, (patch_height, patch_width), stride=(patch_height, patch_width))
...
class Kosmos2_5ImageProcessorFast(BaseImageProcessorFast):
    ...
    def normalize(
        ...
        **kwargs,
        ...
        if image.dtype == mindspore.uint8:
            image = image.to(dtype=mindspore.float32)

        # take mean across the whole `image` except the batch dim (= 0).
        dim = list(range(1, image.ndim))
        mean = mindspore.mint.mean(image, dim=dim)
        std = mindspore.mint.std(image, dim=dim)
        # num_elements in a single image
        num_elements = mindspore.Tensor(mindspore.ops.numel(image[0]))
        adjusted_stddev = mindspore.mint.max(std, 1.0 / mindspore.mint.sqrt(num_elements))

        # change `image` from [batch_size, n_channels, width, height] to [n_channels, batch_size, width, height]
        image = mindspore.mint.transpose(image, 0, 1)
        ...
        normalized_image = mindspore.mint.transpose(image, 0, 1)
    ...
    def extract_flattened_patches(
        ...
        image = mindspore.mint.nn.functional.interpolate(
            image,
        ...
        # [rows * columns, 1]
        row_ids = (
            mindspore.mint.arange(rows, )
            .reshape([rows, 1])
            .repeat(1, columns)
            .reshape([rows * columns, 1])
        )
        col_ids = (
            mindspore.mint.arange(columns, )
            .reshape([1, columns])
            .repeat(rows, 1)
            .reshape([rows * columns, 1])
        )
        ...
        row_ids = row_ids.unsqueeze(0).repeat(batch_size, 1, 1).to(mindspore.float32)
        col_ids = col_ids.unsqueeze(0).repeat(batch_size, 1, 1).to(mindspore.float32)

        # [rows * columns, 2 + patch_height * patch_width * image_channels]
        result = mindspore.mint.cat([row_ids, col_ids, patches], -1)

        # [batch_size, max_patches, 2 + patch_height * patch_width * image_channels]
        result = mindspore.mint.nn.functional.pad(result, [0, 0, 0, max_patches - (rows * columns)]).float()
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        for shape, stacked_images in grouped_images.items():
            ...
            n_of_stacked_images = stacked_images.shape[0]
            ...
            attention_masks.extend(list((patches.sum(axis=-1) != 0).to(dtype=mindspore.float32)))
        ...
        encoded_outputs = BatchFeature(
            data={
                "flattened_patches": mindspore.mint.stack(flattened_patches, dim=0) if return_tensors else flattened_patches,
                "attention_mask": mindspore.mint.stack(attention_masks, dim=0) if return_tensors else attention_masks,
                "width": width,
                "height": height,
                "rows": rows,
                "cols": cols,
