...
from ...utils import ModelOutput, TransformersKwargs, can_return_tuple, logging
...
KOSMOS2_5_VISION_INPUTS_DOCSTRING = r"""
    Args:
        flattened_patches (`ms.Tensor` of shape `(batch_size, max_patches, 2 + patch_height * patch_width * image_channels)`):
...
KOSMOS2_5_INPUTS_DOCSTRING = r"""
    Args:
        ...
        flattened_patches (`ms.Tensor` of shape `(batch_size, max_patches, 2 + patch_height * patch_width * image_channels)`):
            Flattened patches of the images. `flattened_patches` can be obtained using [`AutoImageProcessor`]. See
            [`Kosmos2_5ImageProcessor.__call__`] for details.
        width (`ms.Tensor` of shape `(batch_size,)`):
        ...
        height (`ms.Tensor` of shape `(batch_size,)`):
...
class Kosmos2_5ModelOutput(ModelOutput):
    ...
    width: Optional[mindspore.Tensor] = None
    height: Optional[mindspore.Tensor] = None
...
class Kosmos2_5LayerNorm(mindspore.nn.Cell):
...
class Kosmos2_5VisionEmbeddings(mindspore.nn.Cell):
    def __init__(self, config: Kosmos2_5VisionConfig) -> None:
        ...
        self.row_embedder = mindspore.mint.nn.Embedding(config.max_num_patches, config.hidden_size)
        self.column_embedder = mindspore.mint.nn.Embedding(config.max_num_patches, config.hidden_size)

        self.dropout = mindspore.mint.nn.Dropout(config.dropout_rate, inplace=False)
...
# Copied from transformers.models.t5.modeling_t5.T5DenseGatedActDense with T5DenseGatedActDense->Pix2StructVisionMlp,
# T5Config->Pix2StructVisionConfig,config.d_model->config.hidden_size,dropout_rate->dropout_rate
class Kosmos2_5VisionMlp(mindspore.nn.Cell):
    def __init__(self, config: Kosmos2_5VisionConfig):
        super().__init__()
        self.wi_0 = mindspore.mint.nn.Linear(config.hidden_size, config.intermediate_size, bias=False)
        self.wi_1 = mindspore.mint.nn.Linear(config.hidden_size, config.intermediate_size, bias=False)
        self.wo = mindspore.mint.nn.Linear(config.intermediate_size, config.hidden_size, bias=False)
...
class Kosmos2_5VisionAttention(mindspore.nn.Cell):
...
class Kosmos2_5VisionEncoder(mindspore.nn.Cell):
    def __init__(self, config: Kosmos2_5VisionConfig) -> None:
        super().__init__()
        self.config = config
        self.layer = mindspore.nn.CellList([Kosmos2_5VisionLayer(config) for _ in range(config.num_hidden_layers)])
        self.gradient_checkpointing = False

    def _prepare_attention_mask(self, attention_mask, input_shape, inputs_embeds):
        if self.config._attn_implementation == "flash_attention_2":
            # Diff with transformers.
...
class Kosmos2_5TextSinusoidalPositionalEmbedding(mindspore.nn.Cell):
...
class Kosmos2_5TextFFN(mindspore.nn.Cell):
...
class Kosmos2_5TextAttention(mindspore.nn.Cell):
    ...
    def construct(
        self,
        hidden_states: mindspore.Tensor,  # text part
        encoder_hidden_states: Optional[mindspore.Tensor] = None,  # image part
        attention_mask: Optional[mindspore.Tensor] = None,
...
class Kosmos2_5TextTransformer(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: Kosmos2_5TextConfig):
        ...
        self.embed_tokens = mindspore.mint.nn.Embedding(
        ...
        # Ignore copy
        self.segment_emb = mindspore.mint.nn.Embedding(2, config.embed_dim)
        self.layers = mindspore.nn.CellList(
            [Kosmos2_5TextBlock(config, layer_idx) for layer_idx in range(config.layers)]
        )
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if image_embeds is not None:
            inputs_embeds[image_embeds_position_mask == 1] = image_embeds.view(-1, image_embeds.shape[-1])
        ...
        if image_embeds_position_mask is not None:
            ...
            bsz, seq_len, dim = positions.shape
...
class Kosmos2_5ImageToTextProjection(mindspore.nn.Cell):
    ...
    def __init__(self, config: Kosmos2_5Config):
        ...
        self.latent_query = mindspore.Parameter(
            mindspore.mint.randn(config.latent_query_num, config.text_config.embed_dim)
        )
...
class Kosmos2_5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, (mindspore.mint.nn.LayerNorm, Kosmos2_5LayerNorm)):
...
class Kosmos2_5Model(Kosmos2_5PreTrainedModel):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> from mindone.transformers import AutoProcessor, Kosmos2_5Model
        ...
        if image_embeds is None:
            if flattened_patches is not None:
                ...
                # normalized features
                image_embeds = mindspore.mint.nn.functional.normalize(vision_model_output.last_hidden_state, dim=-1)
...
class Kosmos2_5TextForCausalLM(Kosmos2_5PreTrainedModel):
    ...
    def __init__(self, config: Kosmos2_5TextConfig):
        ...
        self.model = Kosmos2_5TextTransformer(config)
        self.lm_head = mindspore.mint.nn.Linear(
...
class Kosmos2_5ForConditionalGeneration(Kosmos2_5PreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> import requests
        >>> import mindspore as ms
        >>> from mindone.transformers import AutoProcessor, Kosmos2_5ForConditionalGeneration

        >>> repo = "microsoft/kosmos-2.5"
        >>> dtype = ms.bfloat16
        >>> model = Kosmos2_5ForConditionalGeneration.from_pretrained(repo, dtype=dtype)
