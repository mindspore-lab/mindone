...
class Wav2Vec2ConformerNoLayerNormConvLayer(mindspore.nn.Cell):
...
class Wav2Vec2ConformerLayerNormConvLayer(mindspore.nn.Cell):
    def __init__(self, config, layer_id=0):
        super().__init__()
        self.in_conv_dim = config.conv_dim[layer_id - 1] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
...
class Wav2Vec2ConformerGroupNormConvLayer(mindspore.nn.Cell):
    def __init__(self, config, layer_id=0):
        super().__init__()
        self.in_conv_dim = config.conv_dim[layer_id - 1] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
...
class Wav2Vec2ConformerPositionalConvEmbedding(mindspore.nn.Cell):
...
class Wav2Vec2ConformerRotaryPositionalEmbedding(mindspore.nn.Cell):
...
class Wav2Vec2ConformerRelPositionalEmbedding(mindspore.nn.Cell):
...
class Wav2Vec2ConformerSamePadLayer(mindspore.nn.Cell):
...
class Wav2Vec2ConformerFeatureEncoder(mindspore.nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.conv_layers = mindspore.nn.CellList(conv_layers)
...
class Wav2Vec2ConformerFeatureProjection(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.layer_norm = mindspore.mint.nn.LayerNorm(config.conv_dim[-1], eps=config.layer_norm_eps)
        self.projection = mindspore.mint.nn.Linear(config.conv_dim[-1], config.hidden_size)
...
class Wav2Vec2ConformerFeedForward(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.intermediate_dropout = mindspore.mint.nn.Dropout(config.activation_dropout)

        self.intermediate_dense = mindspore.mint.nn.Linear(config.hidden_size, config.intermediate_size)
        if isinstance(config.hidden_act, str):
            self.intermediate_act_fn = ACT2FN[config.hidden_act]
        else:
            self.intermediate_act_fn = config.hidden_act

        self.output_dense = mindspore.mint.nn.Linear(config.intermediate_size, config.hidden_size)
...
class Wav2Vec2ConformerConvolutionModule(mindspore.nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.batch_norm = mindspore.mint.nn.BatchNorm1d(config.hidden_size)
...
class Wav2Vec2ConformerSelfAttention(mindspore.nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.linear_q = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
        self.linear_k = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
        self.linear_v = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
        self.linear_out = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)

        self.dropout = mindspore.mint.nn.Dropout(p=config.attention_dropout)

        if self.position_embeddings_type == "relative":
            # linear transformation for positional encoding
            self.linear_pos = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size, bias=False)
...
class Wav2Vec2ConformerEncoderLayer(mindspore.nn.Cell):
    ...
    def __init__(self, config):
        ...
        # Feed-forward 1
        self.ffn1_layer_norm = mindspore.mint.nn.LayerNorm(embed_dim)
        self.ffn1 = Wav2Vec2ConformerFeedForward(config)

        # Self-Attention
        self.self_attn_layer_norm = mindspore.mint.nn.LayerNorm(embed_dim)
        ...
        # Feed-forward 2
        self.ffn2_layer_norm = mindspore.mint.nn.LayerNorm(embed_dim)
        self.ffn2 = Wav2Vec2ConformerFeedForward(config)
        self.final_layer_norm = mindspore.mint.nn.LayerNorm(embed_dim)
...
class Wav2Vec2ConformerEncoder(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class Wav2Vec2ConformerGumbelVectorQuantizer(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        self.weight_proj = mindspore.mint.nn.Linear(config.conv_dim[-1], self.num_groups * self.num_vars)
    ...
    def _compute_perplexity(probs, mask=None):
        if mask is not None:
            ...
            probs = mindspore.mint.where(mask_extended, probs, mindspore.mint.zeros_like(probs))
    ...
    def construct(self, hidden_states, mask_time_indices=None):
        ...
        if self.training:
            ...
            codevector_probs = mindspore.ops.gumbel_softmax(
                hidden_states.float(), tau=self.temperature, hard=True
            ).type_as(hidden_states)

            # compute perplexity
            codevector_soft_dist = mindspore.mint.softmax(
...
class Wav2Vec2ConformerAdapter(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        ...
        if config.output_hidden_size != config.hidden_size:
            ...
            self.proj_layer_norm = mindspore.mint.nn.LayerNorm(config.output_hidden_size)
        else:
...
class Wav2Vec2ConformerAdapterLayer(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.conv = nn.Conv1d(
    ...
    def construct(self, hidden_states):
        hidden_states = self.conv(hidden_states)
        hidden_states = mindspore.mint.nn.functional.glu(hidden_states, dim=1)
...
class Wav2Vec2ConformerPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        # gumbel softmax requires special init
            ...
            normal_(
                module.conv.weight_norm_cell.weight,
                mean=0,
                std=2 * math.sqrt(1 / (module.conv.kernel_size[0] * module.conv.weight_norm_cell.in_channels)),
            )
            if module.conv.weight_norm_cell.bias:
    ...
    def _get_feat_extract_output_lengths(
        ...
        def _conv_out_length(input_length, kernel_size, stride):
            ...
            return mindspore.mint.div(input_length - kernel_size, stride, rounding_mode="floor") + 1
...
class Wav2Vec2ConformerForPreTraining(Wav2Vec2ConformerPreTrainedModel):
    ...
    def __init__(self, config: Wav2Vec2ConformerConfig):
        ...
        self.dropout_features = mindspore.mint.nn.Dropout(config.feat_quantizer_dropout)

        self.quantizer = Wav2Vec2ConformerGumbelVectorQuantizer(config)

        self.project_hid = mindspore.mint.nn.Linear(config.hidden_size, config.proj_codevector_dim)
        self.project_q = mindspore.mint.nn.Linear(config.codevector_dim, config.proj_codevector_dim)
    ...
    def compute_contrastive_logits(
        ...
        """
        target_features = mindspore.mint.cat([target_features, negative_features], dim=0)

        logits = mindspore.ops.cosine_similarity(predicted_features.float(), target_features.float(), dim=-1).type_as(
            target_features
        )
    ...
    def construct(
        ...
        if sampled_negative_indices is not None:
            ...
            contrastive_loss = mindspore.mint.nn.functional.cross_entropy(logits.float(), target, reduction="sum")
