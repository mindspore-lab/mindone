...
class HeliumRMSNorm(nn.Cell):
    ...
    def construct(self, hidden_states):
        ...
        return (self.weight.to(ms.float32) * hidden_states).to(input_dtype)

    def extra_repr(self):
        return f"{tuple(self.weight.shape)}, eps={self.variance_epsilon}"


class HeliumRotaryEmbedding(nn.Cell):
    def __init__(
        self,
        config: HeliumConfig,
    ):
...
class HeliumAttention(nn.Cell):
    ...
    def __init__(self, config: HeliumConfig, layer_idx: Optional[int] = None):
        ...
        self.o_proj = mint.nn.Linear(config.hidden_size, config.hidden_size, bias=False)
...
class HeliumDecoderLayer(nn.Cell):
    def __init__(self, config: HeliumConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.hidden_size = config.hidden_size
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        **kwargs,
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            min_dtype = _DTYPE_2_MIN[dtype]
            # FIXME: mint.full raise "TypeError: Can not convert Tensor(shape=[], dtype=BFloat16, value=-3.38953e+38) to number" in mindspore 2.6.0 and 2.7.0rc1
