...
def build_alibi_tensor(attention_mask: ms.Tensor, num_heads: int, dtype: ms.dtype) -> ms.Tensor:
    ...
    base = ms.Tensor(
        2 ** (-(2 ** -(math.log2(closest_power_of_2) - 3))),  dtype=ms.float32
    )
    powers = mint.arange(1, 1 + closest_power_of_2,  dtype=ms.int32)
    slopes = mint.pow(base, powers)

    if closest_power_of_2 != num_heads:
        extra_base = ms.Tensor(
            2 ** (-(2 ** -(math.log2(2 * closest_power_of_2) - 3))),  dtype=ms.float32
        )
        ...
        if alibi is None:
            if self._use_sdpa and not output_attentions:
                ...
                if attention_mask is not None:
                    ...
                    attention_mask = mint.ones((query_layer.shape[-2], key_layer.shape[-2]), dtype=ms.bool_).tril(diagonal=0)
...
class FalconModel(FalconPreTrainedModel):
    ...
    def construct(
        ...
        if self.use_alibi:
            mask = (
                mint.ones(
                    (batch_size, seq_length + past_key_values_length),  dtype=ms.int64
                )
