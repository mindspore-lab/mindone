...
class Qwen2MoeRMSNorm(mindspore.nn.Cell):
...
class Qwen2MoeMLP(mindspore.nn.Cell):
...
class Qwen2MoeAttention(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: Qwen2MoeConfig, layer_idx: Optional[int] = None):
        ...
        self.k_proj = mindspore.mint.nn.Linear(
            self.hidden_size, self.num_key_value_heads * self.head_dim, bias=self.config.qkv_bias
        )
...
class Qwen2MoeFlashAttention2(Qwen2MoeAttention):
    ...
    def construct(
        ...
        if attention_mask is not None:
            attention_mask = -attention_mask.to(mindspore.bool_)
...
class Qwen2MoeSdpaAttention(Qwen2MoeAttention):
    ...
    def construct(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "Qwen2MoeModel is using Qwen2MoeSdpaAttention, but `scaled_dot_product_attention` does not support `output_attentions=True`. "
...
class Qwen2MoeForCausalLM(Qwen2MoePreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **loss_kwargs,
        ...
        >>> from mindone.transformers import Qwen2MoeForCausalLM
...
class Qwen2MoeForTokenClassification(Qwen2MoePreTrainedModel):
    def __init__(self, config):
        ...
        self.dropout = mindspore.mint.nn.Dropout(p=classifier_dropout)
        self.score = mindspore.mint.nn.Linear(config.hidden_size, config.num_labels)
