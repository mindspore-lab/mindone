...
FSMT_INPUTS_DOCSTRING = r"""
    Args:
        ...
        encoder_outputs (`Tuple(ms.Tensor)`, *optional*):
            Tuple consists of (`last_hidden_state`, *optional*: `hidden_states`, *optional*: `attentions`)
            `last_hidden_state` of shape `(batch_size, sequence_length, hidden_size)` is a sequence of hidden-states at
            the output of the last layer of the encoder. Used in the cross-attention of the decoder.
        past_key_values (`Tuple(ms.Tensor)` of length `config.n_layers` with each tuple having 4 tensors of
...
def _prepare_fsmt_decoder_inputs(
    ...
    causal_mask = triu_onnx(fill_with_neg_inf(mint.zeros((tgt_len, tgt_len), dtype=causal_mask_dtype)), 1)
...
class PretrainedFSMTModel(PreTrainedModel):
    ...
    _supports_dynamic_input = True  # Static cache has no attribute "num_key_calue_heads"
...
def _make_linear_from_emb(emb):
    ...
    lin_layer = mint.nn.Linear(vocab_size, emb_size, bias=False)
...
class EncoderLayer(nn.Cell):
    ...
    def construct(self, x, encoder_padding_mask, layer_head_mask, output_attentions=False):
        """
        Args:
            x (`ms.Tensor`): input to the layer of shape *(seq_len, batch, embed_dim)*
            encoder_padding_mask (`ms.Tensor`): binary ByteTensor of shape
        ...
        x = mint.nn.functional.dropout(x, p=self.activation_dropout, training=self.training)
        x = self.fc2(x)
        x = mint.nn.functional.dropout(x, p=self.dropout, training=self.training)
        x = residual + x
...
class FSMTEncoder(nn.Cell):
    ...
    def construct(
        ...
        """
        Args:
            input_ids (`ms.Tensor`): tokens in the source language of shape
                *(batch, src_len)*
            attention_mask (`ms.Tensor`): indicating which indices are padding tokens
        ...
        Returns:
            BaseModelOutput or Tuple comprised of:
                ...
                - **x** (`ms.Tensor`): the last encoder layer's output of shape *(src_len, batch, embed_dim)*
                - **encoder_states** (`Tuple(ms.Tensor)`): all intermediate hidden states of shape *(src_len,
                  batch, embed_dim)*. Only populated if *output_hidden_states:* is True.
                - **all_attentions** (`Tuple(ms.Tensor)`): Attention weights for each layer.
...
class FSMTDecoder(nn.Cell):
    ...
    def __init__(self, config: FSMTConfig, embed_tokens: mint.nn.Embedding):
        super().__init__()
        self.dropout = config.dropout
    ...
    def construct(
        ...
        encoder_padding_mask: ms.Tensor,
        decoder_padding_mask: ms.Tensor,
        decoder_causal_mask: ms.Tensor,
        head_mask: Optional[ms.Tensor] = None,
        ...
        Args:
            input_ids (`ms.Tensor` of shape `(batch, tgt_len)`):
            ...
            cross_attn_head_mask (`ms.Tensor` of shape `(num_layers, num_heads)`, *optional*):
...
class Attention(nn.Cell):
    ...
    def construct(
        ...
        layer_state: Optional[Dict[str, Optional[ms.Tensor]]] = None,
        ...
        src_len = k.shape[1]
        attn_weights = mint.bmm(q, k.transpose(1, 2))
        ...
        if key_padding_mask is not None:  # don't attend to padding symbols
            ...
            attn_weights = attn_weights.masked_fill(reshaped, dtype_to_min(attn_weights.dtype))
            attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
        ...
        attn_output = mint.bmm(attn_probs, v)
    ...
    def _use_saved_state(self, k, v, saved_state, key_padding_mask, static_kv, bsz):
        ...
        if "prev_value" in saved_state:
            ...
            if static_kv:
                ...
                assert v is not None
                v = mint.cat([prev_value, v], dim=1)
        assert k is not None and v is not None
        prev_key_padding_mask: Optional[ms.Tensor] = saved_state.get("prev_key_padding_mask", None)
        if prev_key_padding_mask is not None:
            if static_kv:
                new_key_padding_mask = prev_key_padding_mask
            else:
                new_key_padding_mask = mint.cat([prev_key_padding_mask, key_padding_mask], dim=1)
        else:
...
def fill_with_neg_inf(t):
    ...
    return ops.fill(t.float().dtype, t.shape, dtype_to_min(t.dtype)).type_as(t)
...
class FSMTModel(PretrainedFSMTModel):
    ...
    def __init__(self, config: FSMTConfig):
        ...
        encoder_embed_tokens = mint.nn.Embedding(config.src_vocab_size, config.d_model, padding_idx)
        decoder_embed_tokens = mint.nn.Embedding(config.tgt_vocab_size, config.d_model, padding_idx)
...
class SinusoidalPositionalEmbedding(mint.nn.Embedding):
    """
    ...
    def make_weight(self, num_positions, embedding_dim, padding_idx):
        ...
        if not hasattr(self, "weight"):
            ...
            self.weight = ms.Parameter(weight)
        # self.weight.detach_()  # Todo mindspore does't support detach_
    ...
    def make_positions(tensor, padding_idx: int):
        ...
        return (mint.cumsum(mask, dim=1).type_as(mask) * mask).long() + padding_idx
