...
class Lfm2VlMultiModalProjector(mindspore.nn.Cell):
    def __init__(self, config: Lfm2VlConfig):
        super().__init__()
        in_channels = config.vision_config.hidden_size * (config.downsample_factor**2)
        self.factor = config.downsample_factor
        self.layer_norm = mindspore.mint.nn.LayerNorm(in_channels)
    ...
    def construct(self, image_features: mindspore.Tensor):
    ...
    def pixel_unshuffle(self, hidden_states: mindspore.Tensor):
...
class Lfm2VlModel(Lfm2VlPreTrainedModel):
    ...
    def get_image_features(
        ...
        pixel_attention_mask: mindspore.Tensor,
        **kwargs,
        ...
        Args:
            ...
            spatial_shapes (`ms.Tensor` of shape `(batch_size, 2)`):
                The spatial shapes of the input images.
            pixel_attention_mask (`ms.Tensor` of shape `(batch_size, height, width)`):
                The pixel attention mask of the input images.
        Returns:
            image_features (`list[ms.Tensor]`): Image feature tensor of shape `(num_images, image_length, embed_dim)`).
        """
        ...
        for img_idx in range(image_outputs.shape[0]):
            ...
            # TODO: Remove cast tensor->int(tensor).
            feature = feature.reshape(1, int(feature_org_h), int(feature_org_w), -1)
    ...
    def get_placeholder_mask(
        self, input_ids: mindspore.Tensor, inputs_embeds: mindspore.Tensor, image_features: mindspore.Tensor
    ):
        """
    ...
    def construct(
        ...
        spatial_shapes: Optional[mindspore.Tensor] = None,
        pixel_attention_mask: Optional[mindspore.Tensor] = None,
        past_key_values: Optional[Cache] = None,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        r"""
        spatial_shapes (`ms.Tensor` of shape `(batch_size, 2)`, *optional*):
            The spatial shapes of the input images.
        pixel_attention_mask (`ms.Tensor` of shape `(batch_size, height, width)`, *optional*):
            The pixel attention mask of the input images.
        """
        ...
        if pixel_values is not None:
            ...
            image_features = mindspore.mint.cat(image_features, dim=0).to(inputs_embeds.dtype)
...
class Lfm2VlForConditionalGeneration(Lfm2VlPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        r"""
        pixel_values (`ms.Tensor` of shape `(batch_size, channels, height, width)`, *optional*):
        ...
        >>> from mindone.transformers import AutoProcessor, AutoModelForImageTextToText
        ...
        'This image depicts a vibrant street scene in what appears to be a Chinatown or similar cultural area. The focal point is a large red stop
        sign with white lettering, mounted on a pole.'
        ```"""
        outputs = self.model(
            input_ids=input_ids,
            pixel_values=pixel_values,
