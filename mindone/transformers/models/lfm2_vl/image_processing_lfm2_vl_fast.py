...
def resize(image, size, interpolation, antialias=None):
    image = image.squeeze(0)
    height, width = size
    resize = vision.Resize((height, width), interpolation=interpolation)
    # image ms.tensor-->numpy-->PIL
    image = image.permute(1, 2, 0).asnumpy()
    image = (image * 255).clip(0, 255).astype(np.uint8)
    image = Image.fromarray(image)
    return ms.tensor(np.array(resize(image))).permute(2, 0, 1).unsqueeze(0)
...
def convert_image_to_patches(images: "ms.tensor", patch_size: int) -> "ms.tensor":
    """
...
def pad_along_first_dim(images: "ms.tensor", target_length: int, pad_value: int = 0) -> tuple["ms.tensor", "ms.tensor"]:
    """
    Pad the array along the first dimension.
    """
    current_length = images.shape[1]
    padding_length = target_length - current_length
    pixel_mask = mint.ones((target_length,), dtype=ms.int32)
    if padding_length > 0:
        paddings = (0, 0, 0, padding_length, 0, 0)
        images = mint.nn.functional.pad(images, paddings, mode="constant", value=pad_value)
...
class Lfm2VlImageProcessorFast(BaseImageProcessorFast):
    ...
    def crop_image_to_patches(
        ...
        **kwargs,
        ...
        resized_image = resize(image, size=(target_height, target_width), interpolation=interpolation)
        # split the image into patches
        resized_image = unfold(resized_image, 2, size=tile_size, step=tile_size)
        resized_image = unfold(resized_image, 3, size=tile_size, step=tile_size)
        processed_images = (
            resized_image.contiguous()
        ...
        if use_thumbnail and grid_width * grid_height != 1:
            ...
            # FIXME: Different with torchvision resize, which has inout `antialias`
            thumbnail_image = resize(image, size=thumbnail_size, interpolation=interpolation, antialias=antialias)
    ...
    def _is_image_too_large(
        ...
        return (
            h_bar * w_bar > max_image_tokens * encoder_patch_size**2 * downsample_factor**2 * max_pixels_tolerance
        )
    ...
    def resize_and_split(
        ...
        if is_image_large and do_image_splitting:
            ...
            images = resize(images, size=(new_height, new_width), interpolation=interpolation)
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        data = {"pixel_values": mint.cat([mint.stack(images) for images in processed_images])}
        ...
        if do_pad:
            ...
            processed_masks = mint.cat([mint.stack(masks) for masks in processed_masks])
            processed_spatial_shapes = mint.cat(
                [ms.tensor(spatial_shape) for spatial_shape in processed_spatial_shapes]
            )
