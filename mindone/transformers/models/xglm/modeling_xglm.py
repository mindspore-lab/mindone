...
XGLM_INPUTS_DOCSTRING = r"""
    Args:
        ...
        encoder_attention_mask (`mindspore.Tensor` of shape `(batch_size, encoder_sequence_length)`, *optional*):
            ...
            [What are attention masks?](../glossary#attention-mask)
        head_mask (`mindspore.Tensor` of shape `(num_layers, attention_heads)`, *optional*):
            Mask to nullify selected heads of the attention modules. Mask values selected in `[0, 1]`:

            - 1 indicates the head is **not masked**,
            - 0 indicates the head is **masked**.

        cross_attn_head_mask (`mindspore.Tensor` of shape `(num_layers, attention_heads)`, *optional*):
...
class XGLMScaledWordEmbedding(mindspore.mint.nn.Embedding):
    """
...
class XGLMSinusoidalPositionalEmbedding(mindspore.nn.Cell):
    ...
    def construct(self, position_ids: Optional[mindspore.Tensor] = None, past_key_values_length: int = 0):
        bsz, seq_len = position_ids.shape
        position_ids += self.offset

        # Expand embeddings if needed. `position_ids.max()` is NOT used to keep mindspore.mint.fx compatibility.
...
class XGLMAttention(mindspore.nn.Cell):
    ...
    def construct(
        ...
        if attention_mask is not None:
            ...
            attn_weights = mindspore.mint.max(attn_weights, mindspore.Tensor(dtype_to_min(attn_weights.dtype)))
...
class XGLMDecoderLayer(mindspore.nn.Cell):
    def __init__(self, config: XGLMConfig):
        ...
        self.fc1 = mindspore.mint.nn.Linear(self.embed_dim, config.ffn_dim)
        self.fc2 = mindspore.mint.nn.Linear(config.ffn_dim, self.embed_dim)
...
class XGLMModel(XGLMPreTrainedModel):
    ...
    def __init__(self, config: XGLMConfig, embed_tokens: Optional[mindspore.mint.nn.Embedding] = None):
        ...
        self.layers = mindspore.nn.CellList([XGLMDecoderLayer(config) for _ in range(config.num_layers)])
