...
def _prepare_aspect_ratio_attention_mask(
    ...
    attention_mask = attention_mask @ attention_mask.transpose(-1, -2) * dtype_to_min(dtype)
...
class MllamaVisionAttention(nn.Cell):
    ...
    def construct(
        ...
        attn_weights = mint.matmul(query, key.transpose(2, 3)) / math.sqrt(self.head_dim)
...
class MllamaTextCrossSdpaAttention(MllamaTextCrossAttention):
    ...
    def construct(
        ...
        # in SDPA to support both compile's dynamic shapes and full graph options. An inline conditional prevents dynamic shapes from compiling.
...
class MllamaVisionModel(MllamaPreTrainedModel):
    ...
    def apply_class_embedding(self, hidden_state: ms.Tensor) -> ms.Tensor:
        ...
        class_embedding = self.class_embedding.broadcast_to((batch_size, 1, hidden_size))
