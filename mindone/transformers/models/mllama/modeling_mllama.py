...
def _prepare_cross_attention_mask(
    cross_attention_mask: ms.Tensor,
    ...
    cross_attention_mask = inverted_cross_attn_mask.masked_fill(
        inverted_cross_attn_mask.to(ms.bool_), dtype_to_min(dtype)
    )

    # apply full-row bias, which return 4D tensor of shape [B, H, S1, 1] where value is 0 if the a full row in cross attn mask's
    # last dimension contains negative infinity values, otherwise it's 1
    negative_inf_value = dtype_to_min(dtype)
...
def _prepare_aspect_ratio_attention_mask(
    ...
    attention_mask = attention_mask @ attention_mask.swapaxes(-1, -2) * dtype_to_min(dtype)
...
class MllamaVisionAttention(nn.Cell):
    def __init__(self, config: MllamaVisionConfig):
        ...
        self.q_proj = mint.nn.Linear(self.embed_dim, self.num_heads * self.head_dim, bias=False)
        self.k_proj = mint.nn.Linear(self.embed_dim, self.num_heads * self.head_dim, bias=False)
        self.v_proj = mint.nn.Linear(self.embed_dim, self.num_heads * self.head_dim, bias=False)
        self.o_proj = mint.nn.Linear(self.num_heads * self.head_dim, self.embed_dim, bias=False)
    ...
    def construct(
        ...
        query = query.view(batch_size, q_seq_len, self.num_heads, self.head_dim).swapaxes(1, 2)
        key = key.view(batch_size, kv_seq_len, self.num_heads, self.head_dim).swapaxes(1, 2)
        value = value.view(batch_size, kv_seq_len, self.num_heads, self.head_dim).swapaxes(1, 2)

        attn_weights = mint.matmul(query, key.swapaxes(2, 3)) / math.sqrt(self.head_dim)
...
class MllamaVisionEncoderLayer(nn.Cell):
    def __init__(self, config: MllamaVisionConfig, is_gated: bool = False):
        ...
        self.input_layernorm = mint.nn.LayerNorm(self.hidden_size, eps=config.norm_eps)
...
class MllamaTextRMSNorm(nn.Cell):
...
class MllamaTextCrossAttention(nn.Cell):
    ...
    def construct(
        self,
        hidden_states: ms.Tensor,
        cross_attention_states: Optional[ms.Tensor] = None,
        ...
        if cross_attention_states is not None:
            ...
            key_states = key_states.view(bsz, -1, self.num_key_value_heads, self.head_dim).swapaxes(1, 2)
            value_states = value_states.view(bsz, -1, self.num_key_value_heads, self.head_dim).swapaxes(1, 2)
...
class MllamaTextCrossSdpaAttention(MllamaTextCrossAttention):
    """
    Mllama attention module using scaled_dot_product_attention. This module inherits from
...
class MllamaTextSelfAttention(nn.Cell):
...
class MllamaSelfAttentionDecoderLayer(nn.Cell):
    def __init__(self, config: MllamaTextConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
    ...
    def construct(
        ...
        hidden_states: ms.Tensor,
        cross_attention_states: Optional[ms.Tensor] = None,
        cross_attention_mask: Optional[ms.Tensor] = None,
        attention_mask: Optional[ms.Tensor] = None,
        full_text_row_masked_out_mask: Optional[Tuple[ms.Tensor, ms.Tensor]] = None,
...
class MllamaCrossAttentionDecoderLayer(nn.Cell):
    ...
    def construct(
        self,
        hidden_states: ms.Tensor,
        cross_attention_states: ms.Tensor,
        cross_attention_mask: ms.Tensor,
        attention_mask: ms.Tensor,
        full_text_row_masked_out_mask: Tuple[ms.Tensor, ms.Tensor],
        position_ids: Optional[ms.Tensor] = None,
        past_key_value: Optional[Cache] = None,
        output_attentions: Optional[bool] = False,
        use_cache: Optional[bool] = False,
...
class MllamaRotaryEmbedding(nn.Cell):
    ...
    def construct(self, x, position_ids):
        if "dynamic" in self.rope_type:
            self._dynamic_frequency_update(position_ids=x)
...
class MllamaPreTrainedModel(MSPreTrainedModel):
...
MLLAMA_VISION_INPUTS_DOCSTRING = r"""
    Args:
        ...
        aspect_ratio_mask (`ms.Tensor` of shape `(batch_size, max_num_images, max_num_tiles)`, *optional*):
        ...
        aspect_ratio_ids (`ms.Tensor` of shape `(batch_size, max_num_images)`, *optional*):
...
MLLAMA_TEXT_INPUTS_DOCSTRING = r"""
    Args:
        ...
        cross_attention_mask (`ms.Tensor` of shape `(batch_size, seq_length, max_num_images, max_num_tiles)`, *optional*):
        ...
        cross_attention_states (`ms.Tensor`, *optional*):
...
class MllamaVisionModel(MllamaPreTrainedModel):
    ...
    def __init__(self, config: MllamaVisionConfig):
        ...
        # layer norms
        self.layernorm_pre = mint.nn.LayerNorm(self.hidden_size)
        self.layernorm_post = mint.nn.LayerNorm(self.hidden_size)
    ...
    def apply_class_embedding(self, hidden_state: ms.Tensor) -> ms.Tensor:
        batch_size, _, hidden_size = hidden_state.shape
        class_embedding = self.class_embedding.expand((batch_size, 1, hidden_size))
        hidden_state = mint.cat([class_embedding, hidden_state], dim=1)
        return hidden_state
    ...
    def construct(
        ...
        >>> import requests
        >>> from mindone.transformers import MllamaVisionModel
        ...
        mindspore.Size([1, 1, 4, 1025, 7680])
        ```
        """
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
        ...
        hidden_state = patch_embeds.flatten(2).swapaxes(1, 2)
        ...
        intermediate_hidden_states = mint.stack(all_intermediate_hidden_states, dim=-1)
        ...
        hidden_state = mint.cat([hidden_state, intermediate_hidden_states], dim=-1)
...
class MllamaTextModel(MllamaPreTrainedModel):
    ...
    def __init__(self, config: MllamaTextConfig):
        ...
        self.embed_tokens = mint.nn.Embedding(config.vocab_size + 8, config.hidden_size, self.padding_idx)
    ...
    def construct(
        ...
        >>> from mindone.transformers import MllamaTextModel
        >>> import mindspore as ms
        ...
        >>> inputs = tokenizer(text=text, return_tensors="np")
        >>> for key, value in inputs.items():
        >>>     inputs[key] = ms.tensor(value)
...
class MllamaForCausalLM(MllamaPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.lm_head = mint.nn.Linear(self.text_config.hidden_size, self.vocab_size, bias=False)
    ...
    def construct(
        ...
        **loss_kwargs,
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import MllamaForCausalLM
        >>> import mindspore as ms
...
class MllamaForConditionalGeneration(MllamaPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: MllamaConfig):
        ...
        self.multi_modal_projector = mint.nn.Linear(
    ...
    def construct(
        ...
        aspect_ratio_mask: Optional[ms.Tensor] = None,
        aspect_ratio_ids: Optional[ms.Tensor] = None,
        attention_mask: Optional[ms.Tensor] = None,
        cross_attention_mask: Optional[ms.Tensor] = None,
        cross_attention_states: Optional[ms.Tensor] = None,
        position_ids: Optional[ms.Tensor] = None,
    ...
    def _update_model_kwargs_for_generation(self, outputs, model_kwargs, is_encoder_decoder, **kwargs):
        ...
        if cross_attention_mask_prev is not None:
            model_kwargs["cross_attention_mask"] = mint.cat(
