...
from ...mindspore_adapter import MultiheadAttention, dtype_to_max
...
class Mask2FormerPixelDecoderOutput(ModelOutput):
    ...
    Args:
        multi_scale_features (`tuple(ms.Tensor)`):
            Tuple of multi-scale features of scales [1/8, 1/16, 1/32] and shape `(batch_size, num_channels, height,
            width)`from the Multi-Scale Deformable Attenntion based Pixel Decoder.
        mask_features (`ms.Tensor`):
            Tensor of shape `(batch_size, num_channels, height, width)`, 1/4 scale features from the last Pixel Decoder
            Layer.
        attentions (`tuple(ms.Tensor)`, *optional*):
            Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
            sequence_length)`. Attentions weights from pixel decoder. Returned when `output_attentions=True` is passed
            or when `config.output_attentions=True`
    """

    multi_scale_features: Tuple[ms.Tensor] = None
    mask_features: Optional[ms.Tensor] = None
...
class Mask2FormerMaskedAttentionDecoderOutput(BaseModelOutputWithCrossAttentions):
    ...
    Args:
        ...
        hidden_states (`tuple(ms.Tensor)`, *optional*):
        ...
        masks_queries_logits (`tuple(ms.Tensor)` of shape `(batch_size, num_queries, height, width)`):
            Tuple of mask predictions from all layers of the transformer decoder.
        intermediate_hidden_states (`tuple(ms.Tensor)` of shape `(num_queries, 1, hidden_size)`):
    ...
    attentions: Optional[ms.Tensor] = None
    masks_queries_logits: Tuple[ms.Tensor] = None
    intermediate_hidden_states: Tuple[ms.Tensor] = None
...
class Mask2FormerPixelLevelModuleOutput(ModelOutput):
    ...
    Args:
        encoder_last_hidden_state (`ms.Tensor`):
            Last hidden states (final feature map of shape `(batch_size, num_channels, height, width)`) of the last
            stage of the encoder.
        encoder_hidden_states (`tuple(ms.Tensor)`, *optional*):
            Tuple of `ms.Tensor` of shape `(batch_size, num_channels, height, width)`. Hidden states (also
        ...
        decoder_hidden_states (`tuple(ms.Tensor)`):
            Tuple of `ms.Tensor` of shape `(batch_size, num_channels, height, width)`. Hidden states (also
    ...
    decoder_hidden_states: Tuple[ms.Tensor] = None
...
class Mask2FormerModelOutput(ModelOutput):
    ...
    Args:
        encoder_last_hidden_state (`ms.Tensor` of shape `(batch_size, num_channels, height, width)`, *optional*):
        ...
        pixel_decoder_last_hidden_state (`ms.Tensor` of shape `(batch_size, num_channels, height, width)`, *optional*):
        ...
        transformer_decoder_last_hidden_state (`tuple(ms.Tensor)`):
            Final output of the transformer decoder `(batch_size, sequence_length, hidden_size)`.
        transformer_decoder_hidden_states (`tuple(ms.Tensor)`, *optional*):
            Tuple of `ms.Tensor` (one for the output of the embeddings + one for the output of each stage) of
            shape `(batch_size, sequence_length, hidden_size)`. Hidden-states (also called feature maps) of the
            transformer decoder at the output of each stage. Returned when `output_hidden_states=True` is passed.
        transformer_decoder_intermediate_states (`tuple(ms.Tensor)` of shape `(num_queries, 1, hidden_size)`):
            Intermediate decoder activations, i.e. the output of each decoder layer, each of them gone through a
            layernorm.
        masks_queries_logits (`tuple(ms.Tensor)` of shape `(batch_size, num_queries, height, width)`)
            Mask Predictions from each layer in the transformer decoder.
        attentions (`tuple(tuple(ms.Tensor))`, *optional*, returned when `output_attentions=True` is passed):
            Tuple of `tuple(ms.Tensor)` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
    ...
    transformer_decoder_intermediate_states: Tuple[ms.Tensor] = None
    masks_queries_logits: Tuple[ms.Tensor] = None
    attentions: Optional[Tuple[ms.Tensor]] = None
...
class Mask2FormerForUniversalSegmentationOutput(ModelOutput):
    ...
    Args:
        ...
        masks_queries_logits (`ms.Tensor`):
            ...
            query.
        auxiliary_logits (`List[Dict(str, ms.Tensor)]`, *optional*):
    ...
    auxiliary_logits: Optional[List[Dict[str, ms.Tensor]]] = None
    encoder_last_hidden_state: Optional[ms.Tensor] = None
    pixel_decoder_last_hidden_state: Optional[ms.Tensor] = None
    transformer_decoder_last_hidden_state: Optional[ms.Tensor] = None
    encoder_hidden_states: Optional[Tuple[ms.Tensor]] = None
    pixel_decoder_hidden_states: Optional[Tuple[ms.Tensor]] = None
    transformer_decoder_hidden_states: Optional[ms.Tensor] = None
    attentions: Optional[Tuple[ms.Tensor]] = None
...
def sample_point(input_features: ms.Tensor, point_coordinates: ms.Tensor, add_dim=False, **kwargs) -> ms.Tensor:
    """
    A wrapper around `mint.nn.functional.grid_sample` to support 3D point_coordinates tensors.

    Args:
        input_features (`ms.Tensor` of shape (batch_size, channels, height, width)):
            A tensor that contains features map on a height * width grid
        point_coordinates (`ms.Tensor` of shape (batch_size, num_points, 2) or (batch_size, grid_height, grid_width,:
        2)):
    ...
    Returns:
        point_features (`ms.Tensor` of shape (batch_size, channels, num_points) or (batch_size, channels,
...
def dice_loss(inputs: ms.Tensor, labels: ms.Tensor, num_masks: int) -> ms.Tensor:
    r"""
...
def sigmoid_cross_entropy_loss(inputs: ms.Tensor, labels: ms.Tensor, num_masks: int) -> ms.Tensor:
    r"""
    Args:
        inputs (`ms.Tensor`):
    ...
    Returns:
        loss (`ms.Tensor`): The computed loss.
    """
...
def pair_wise_dice_loss(inputs: ms.Tensor, labels: ms.Tensor) -> ms.Tensor:
    """
...
def pair_wise_sigmoid_cross_entropy_loss(inputs: ms.Tensor, labels: ms.Tensor) -> ms.Tensor:
    r"""
    ...
    Returns:
        loss (`ms.Tensor`): The computed loss between each pairs.
    """
    ...
    loss_pos = mint.matmul(cross_entropy_loss_pos / height_and_width, labels.T)
    loss_neg = mint.matmul(cross_entropy_loss_neg / height_and_width, (1 - labels).T)
...
class Mask2FormerHungarianMatcher(nn.Cell):
    ...
    def construct(
        ...
        for i in range(batch_size):
            ...
            # Compute the classification cost. Contrary to the loss, we don't use the NLL, but approximate it in 1 - proba[target class]. The 1 is a constant that doesn't change the matching, it can be ommitted.  # noqa: E501
            ...
            point_coordinates = mint.rand(1, self.num_points, 2)
            ...
            cost_matrix = mint.minimum(cost_matrix, ms.tensor(1e10))
...
class Mask2FormerLoss(nn.Cell):
    ...
    def _pad_images_to_max_in_batch(self, tensors: List[ms.Tensor]) -> Tuple[ms.Tensor, ms.Tensor]:
    ...
    def loss_labels(
        self, class_queries_logits: ms.Tensor, class_labels: List[ms.Tensor], indices: Tuple[np.array]
    ) -> Dict[str, ms.Tensor]:
        ...
        target_classes_o = mint.cat(
            [target[j] for target, (_, j) in zip(class_labels, indices)]
        )  # shape of (batch_size, num_queries)
        target_classes = ops.full((batch_size, num_queries), fill_value=self.num_labels, dtype=ms.int64)
    ...
    def loss_masks(
        ...
        Returns:
            losses (`Dict[str, Tensor]`): A dict of `ms.Tensor` containing two keys:
    ...
    def calculate_uncertainty(self, logits: ms.Tensor) -> ms.Tensor:
        """
        ...
        Returns:
            scores (`ms.Tensor`): A tensor of shape (R, 1, ...) that contains uncertainty scores with the most
            uncertain locations having the highest uncertainty score.
        """
        uncertainty_scores = -(mint.abs(logits))
    ...
    def construct(
        ...
        mask_labels: List[ms.Tensor],
        class_labels: List[ms.Tensor],
        auxiliary_predictions: Optional[Dict[str, ms.Tensor]] = None,
    ) -> Dict[str, ms.Tensor]:
        """
        ...
        Returns:
            losses (`Dict[str, Tensor]`): A dict of `ms.Tensor` containing three keys:
            - **loss_cross_entropy** -- The loss computed using cross entropy on the predicted and ground truth labels.
...
def multi_scale_deformable_attention(
    value: ms.Tensor,
    value_spatial_shapes: Union[ms.Tensor, List[Tuple]],
    ...
    for level_id, (height, width) in enumerate(value_spatial_shapes):
        ...
        # -> batch_size*num_heads, hidden_dim, height, width
        value_l_ = (
            value_list[level_id].flatten(2).swapaxes(1, 2).reshape(batch_size * num_heads, hidden_dim, height, width)
        )
...
class Mask2FormerPixelDecoderEncoderMultiscaleDeformableAttention(nn.Cell):
    ...
    def __init__(self, embed_dim: int, num_heads: int, n_levels: int, n_points: int):
        ...
        self.sampling_offsets = mint.nn.Linear(embed_dim, num_heads * n_levels * n_points * 2)
        self.attention_weights = mint.nn.Linear(embed_dim, num_heads * n_levels * n_points)
        self.value_proj = mint.nn.Linear(embed_dim, embed_dim)
        self.output_proj = mint.nn.Linear(embed_dim, embed_dim)
    ...
    def construct(
        ...
        if reference_points.shape[-1] == 2:
            offset_normalizer = ms.tensor([[shape[1], shape[0]] for shape in spatial_shapes_list], dtype=ms.int64)
            sampling_locations = (
...
class Mask2FormerPixelDecoderEncoderLayer(nn.Cell):
    def __init__(self, config: Mask2FormerConfig):
        ...
        self.activation_fn = mint.nn.functional.relu
        self.activation_dropout = config.dropout
        self.fc1 = mint.nn.Linear(self.embed_dim, config.encoder_feedforward_dim)
        self.fc2 = mint.nn.Linear(config.encoder_feedforward_dim, self.embed_dim)
...
class Mask2FormerPixelDecoderEncoderOnly(nn.Cell):
    ...
    def __init__(self, config: Mask2FormerConfig):
        ...
        self.config = config
        self.dropout = config.dropout
        self.layers = nn.CellList([Mask2FormerPixelDecoderEncoderLayer(config) for _ in range(config.encoder_layers)])

    @staticmethod
    def get_reference_points(spatial_shapes_list, valid_ratios):
        """
    ...
    def construct(
        ...
        reference_points = self.get_reference_points(spatial_shapes_list, valid_ratios)
...
class Mask2FormerPixelDecoder(nn.Cell):
    def __init__(self, config: Mask2FormerConfig, feature_channels):
        ...
        self.level_embed = ms.Parameter(mint.rand(self.num_feature_levels, feature_dim), name="level_embed")
        ...
        if self.num_feature_levels > 1:
            ...
            for in_channels in transformer_in_channels[::-1]:
                input_projections_list.append(
                    nn.SequentialCell(
                        mint.nn.Conv2d(in_channels, feature_dim, kernel_size=1),
                        mint.nn.GroupNorm(32, feature_dim),
                    )
                )
            ...
            self.input_projections = nn.CellList(
                [
                    nn.SequentialCell(
                        mint.nn.Conv2d(transformer_in_channels[-1], feature_dim, kernel_size=1),
                        mint.nn.GroupNorm(32, feature_dim),
                    )
                ]
            )
        ...
        self.mask_projection = mint.nn.Conv2d(feature_dim, mask_dim, kernel_size=1, stride=1, padding=0)
        ...
        for idx, in_channels in enumerate(self.feature_channels[: self.num_fpn_levels]):
            lateral_conv = nn.SequentialCell(
                mint.nn.Conv2d(in_channels, feature_dim, kernel_size=1, bias=False),
                mint.nn.GroupNorm(32, feature_dim),
            )

            output_conv = nn.SequentialCell(
                mint.nn.Conv2d(feature_dim, feature_dim, kernel_size=3, stride=1, padding=1, bias=False),
                mint.nn.GroupNorm(32, feature_dim),
                mint.nn.ReLU(),
            )
    ...
    def get_valid_ratio(self, mask, dtype=ms.float32):
        ...
        valid_ratio = mint.stack([valid_ratio_width, valid_ratio_heigth], -1)
        return valid_ratio
    ...
    def construct(
        ...
        masks = [mint.zeros((x.shape[0], x.shape[2], x.shape[3]), dtype=ms.bool_) for x in input_embeds]

        # Prepare encoder inputs (by flattening)
        spatial_shapes_list = [(embed.shape[2], embed.shape[3]) for embed in input_embeds]
        input_embeds_flat = mint.cat([embed.flatten(2).swapaxes(1, 2) for embed in input_embeds], 1)
        spatial_shapes = ms.tensor(spatial_shapes_list, dtype=ms.int64)
        masks_flat = mint.cat([mask.flatten(1) for mask in masks], 1)

        position_embeddings = [embed.flatten(2).swapaxes(1, 2) for embed in position_embeddings]
        level_pos_embed_flat = [x + self.level_embed[i].view(1, 1, -1) for i, x in enumerate(position_embeddings)]
        level_pos_embed_flat = mint.cat(level_pos_embed_flat, 1)

        level_start_index = mint.cat(
            (spatial_shapes.new_zeros((1,)), spatial_shapes.prod(1).int().cumsum(0).long()[:-1])
        )
        valid_ratios = mint.stack([self.get_valid_ratio(mask, dtype=input_embeds_flat.dtype) for mask in masks], 1)
        ...
        encoder_output = mint.split(last_hidden_state, split_sizes, dim=1)

        # Compute final features
        outputs = [
            x.swapaxes(1, 2).view(batch_size, -1, spatial_shapes_list[i][0], spatial_shapes_list[i][1])
        ...
        for idx, feature in enumerate(features[: self.num_fpn_levels][::-1]):
            ...
            if current_fpn.dtype == ms.bfloat16:
                out = current_fpn + mint.nn.functional.interpolate(
                    outputs[-1].float(), size=current_fpn.shape[-2:], mode="bilinear", align_corners=False
                ).to(ms.bfloat16)
            else:
                out = current_fpn + mint.nn.functional.interpolate(
...
class Mask2FormerPixelLevelModule(nn.Cell):
    ...
    def construct(
        self, pixel_values: ms.Tensor, output_hidden_states: bool = False
...
class Mask2FormerAttention(nn.Cell):
    ...
    def construct(
        ...
        key_value_position_embeddings: Optional[ms.Tensor] = None,
        output_attentions: bool = False,
...
class Mask2FormerMaskedAttentionDecoderLayer(nn.Cell):
    ...
    def __init__(self, config: Mask2FormerConfig):
        ...
        self.self_attn_layer_norm = mint.nn.LayerNorm(self.embed_dim)
        self.cross_attn = MultiheadAttention(self.embed_dim, self.config.num_attention_heads, self.config.dropout)
        self.cross_attn_layer_norm = mint.nn.LayerNorm(self.embed_dim)
        self.fc1 = mint.nn.Linear(self.embed_dim, self.config.dim_feedforward)
        self.fc2 = mint.nn.Linear(self.config.dim_feedforward, self.embed_dim)
        self.final_layer_norm = mint.nn.LayerNorm(self.embed_dim)

    def with_pos_embed(self, tensor, pos: Optional[ms.Tensor]):
...
class Mask2FormerMaskedAttentionDecoder(nn.Cell):
    ...
    def __init__(self, config: Mask2FormerConfig):
        ...
        self.layernorm = mint.nn.LayerNorm(config.hidden_dim)
    ...
    def construct(
        ...
        multi_stage_positional_embeddings: Optional[ms.Tensor] = None,
        pixel_embeddings: Optional[ms.Tensor] = None,
        encoder_hidden_states: Optional[ms.Tensor] = None,
        query_position_embeddings: Optional[ms.Tensor] = None,
        ...
        r"""
        Args:
            inputs_embeds (`ms.Tensor` of shape `(num_queries, batch_size, hidden_size)`):
                The query embeddings that are passed into the decoder.
            multi_stage_positional_embeddings (`ms.Tensor` of shape `(height*width, batch_size, num_channels)`):
                Position embeddings that are added to the keys in each cross(masked)-attention layer.
            pixel_embeddings (`ms.Tensor`):
                Tensor of shape `(batch_size, num_channels, height, width)`, 1/4 scale features from the last Pixel
                Decoder.
            query_position_embeddings (`ms.Tensor` of shape `(num_queries, batch_size, hidden_size)`):
                , *optional*): Position embeddings that are added to the queries and keys in each self-attention layer.
            encoder_hidden_states (`ms.Tensor` of shape `(batch_size, encoder_sequence_length, hidden_size)`):
                Sequence of hidden-states at the output of the last layer of the encoder. Used in the
                cross(masked)-attention of the decoder.
            feature_size_list (`List[Tuple[int]]`):
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # Multiply the attention mask instead of indexing
...
class Mask2FormerMLPPredictionHead(nn.Cell):
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int, num_layers: int = 3):
        ...
        for i, (in_dim, out_dim) in enumerate(zip(in_dims, out_dims)):
            ...
            # Provide backwards compatibility from when the class inherited from nn.SequentialCell
            # In nn.SequentialCell subclasses, the name given to the layer is its index in the sequence.
...
class Mask2FormerMaskPredictor(nn.Cell):
    def __init__(self, hidden_size: int, num_heads: int, mask_feature_size: ms.Tensor):
        """
    ...
    def construct(
        self, outputs: ms.Tensor, pixel_embeddings: ms.Tensor, attention_mask_target_size: Optional[int] = None
    ):
        mask_embeddings = self.mask_embedder(outputs.swapaxes(0, 1))

        outputs_mask = mint.einsum("bqc, bchw -> bqhw", mask_embeddings, pixel_embeddings)

        # TODO: there is a precision issue in `mint.nn.functional.interpolate` if input.dtype = fp16 or bf16
        attention_mask = mint.nn.functional.interpolate(
            outputs_mask.float(), size=attention_mask_target_size, mode="bilinear", align_corners=False
        )

        attention_mask = attention_mask.sigmoid().flatten(2).unsqueeze(1).tile((1, self.num_heads, 1, 1))
...
class Mask2FormerTransformerModule(nn.Cell):
    ...
    def __init__(self, in_features: int, config: Mask2FormerConfig):
        ...
        self.queries_embedder = mint.nn.Embedding(config.num_queries, hidden_dim)
        self.queries_features = mint.nn.Embedding(config.num_queries, hidden_dim)
        self.input_projections = []

        for _ in range(self.num_feature_levels):
            if in_features != hidden_dim or config.enforce_input_projection:
                self.input_projections.append(mint.nn.Conv2d(in_features, hidden_dim, kernel_size=1))
            else:
                self.input_projections.append(nn.SequentialCell())

        self.decoder = Mask2FormerMaskedAttentionDecoder(config=config)
        self.level_embed = mint.nn.Embedding(self.num_feature_levels, hidden_dim)

    def construct(
        self,
        multi_scale_features: List[ms.Tensor],
        mask_features: ms.Tensor,
...
class Mask2FormerModel(Mask2FormerPreTrainedModel):
    ...
    def construct(
        ...
        >>> from mindone.transformers import Mask2FormerModel
...
class Mask2FormerForUniversalSegmentation(Mask2FormerPreTrainedModel):
    ...
    def __init__(self, config: Mask2FormerConfig):
        ...
        self.class_predictor = mint.nn.Linear(config.hidden_dim, config.num_labels + 1)
    ...
    def get_loss_dict(
        ...
        mask_labels: ms.Tensor,
        class_labels: ms.Tensor,
        auxiliary_predictions: Dict[str, ms.Tensor],
    ) -> Dict[str, ms.Tensor]:
        loss_dict: Dict[str, ms.Tensor] = self.criterion(
    ...
    def get_loss(self, loss_dict: Dict[str, ms.Tensor]) -> ms.Tensor:
        return sum(loss_dict.values())

    def get_auxiliary_logits(self, classes: ms.Tensor, output_masks: ms.Tensor):
        auxiliary_logits: List[Dict(str, ms.Tensor)] = []
    ...
    def construct(
        self,
        pixel_values: ms.Tensor,
        mask_labels: Optional[List[ms.Tensor]] = None,
        class_labels: Optional[List[ms.Tensor]] = None,
        pixel_mask: Optional[ms.Tensor] = None,
        output_hidden_states: Optional[bool] = None,
        ...
        ```python
        >>> from transformers import AutoImageProcessor
        >>> from mindone.transformers import Mask2FormerForUniversalSegmentation
        >>> from PIL import Image
        >>> import requests
        >>> import mindspore as ms
        ...
        ...     "facebook/mask2former-swin-small-cityscapes-panoptic", revision="refs/pr/2"
        ... )
        >>> model = Mask2FormerForUniversalSegmentation.from_pretrained(
        ...     "facebook/mask2former-swin-small-cityscapes-panoptic", revision="refs/pr/2"
        ... )
