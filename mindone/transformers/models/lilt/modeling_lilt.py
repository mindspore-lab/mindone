...
class LiltTextEmbeddings(ms.nn.Cell):
...
class LiltLayoutEmbeddings(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
        ...
        self.x_position_embeddings = mint.nn.Embedding(config.max_2d_position_embeddings, config.hidden_size // 6)
        self.y_position_embeddings = mint.nn.Embedding(config.max_2d_position_embeddings, config.hidden_size // 6)
        self.h_position_embeddings = mint.nn.Embedding(config.max_2d_position_embeddings, config.hidden_size // 6)
        self.w_position_embeddings = mint.nn.Embedding(config.max_2d_position_embeddings, config.hidden_size // 6)

        self.padding_idx = config.pad_token_id
        self.box_position_embeddings = mint.nn.Embedding(
            config.max_position_embeddings,
            config.hidden_size // config.channel_shrink_ratio,
            padding_idx=self.padding_idx,
        )
        self.box_linear_embeddings = mint.nn.Linear(
            in_features=config.hidden_size, out_features=config.hidden_size // config.channel_shrink_ratio
        )
        self.LayerNorm = mint.nn.LayerNorm(config.hidden_size // config.channel_shrink_ratio, eps=config.layer_norm_eps)
        self.dropout = mint.nn.Dropout(config.hidden_dropout_prob)
...
class LiltSelfAttention(ms.nn.Cell):
    def __init__(self, config, position_embedding_type=None):
        ...
        self.layout_query = mint.nn.Linear(
            config.hidden_size // config.channel_shrink_ratio, self.all_head_size // config.channel_shrink_ratio
        )
        self.layout_key = mint.nn.Linear(
            config.hidden_size // config.channel_shrink_ratio, self.all_head_size // config.channel_shrink_ratio
        )
        self.layout_value = mint.nn.Linear(
            config.hidden_size // config.channel_shrink_ratio, self.all_head_size // config.channel_shrink_ratio
        )
    ...
    def transpose_for_scores(self, x, r=1):
        new_x_shape = x.shape[:-1] + (self.num_attention_heads, self.attention_head_size // r)
    ...
    def construct(
        ...
        layout_attention_scores = mint.matmul(layout_query_layer, layout_key_layer.transpose(-1, -2))
        ...
        layout_attention_probs = mint.nn.Softmax(dim=-1)(layout_attention_scores)
        ...
        layout_context_layer = mint.matmul(layout_attention_probs, layout_value_layer)

        layout_context_layer = layout_context_layer.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape = layout_context_layer.shape[:-2] + (self.all_head_size // self.channel_shrink_ratio,)
...
class LiltSelfOutput(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class LiltAttention(ms.nn.Cell):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
...
class LiltIntermediate(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class LiltEncoder(ms.nn.Cell):
    # Copied from transformers.models.bert.modeling_bert.BertEncoder.__init__ with Bert->Lilt
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = ms.nn.CellList([LiltLayer(config) for _ in range(config.num_hidden_layers)])
...
class LiltModel(LiltPreTrainedModel):
    ...
    def construct(
        ...
        if bbox is None:
            bbox = mint.zeros(input_shape + (4,), dtype=ms.int64)
...
class LiltClassificationHead(ms.nn.Cell):
