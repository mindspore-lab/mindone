...
class ZambaHybridDynamicCache(DynamicCache):
    ...
    def __init__(self, config, batch_size, dtype=ms.float16):
        ...
        for i in range(config.num_hidden_layers):
            self.conv_states += [mint.zeros((batch_size, self.intermediate_size, self.conv_kernel_size), dtype=dtype)]
            cache_shape = (
                batch_size,
                self.n_mamba_heads,
            ...
            self.ssm_states += [mint.zeros(cache_shape, dtype=dtype)]
...
class ZambaAttention(nn.Cell):
    ...
    attn_weights = torch.matmul(query_states, key_states.swapaxes(2, 3)) / math.sqrt(self.head_dim) with
    attn_weights = torch.matmul(query_states, key_states.swapaxes(2, 3)) / math.sqrt(self.head_dim/2)
    """
...
class ZambaMambaMixer(nn.Cell):
    ...
    def __init__(self, config: ZambaConfig, layer_idx):
        ...
        self.x_proj_weight = Parameter(
            (
                mint.zeros(
                    (self.n_mamba_heads, self.time_step_rank + self.ssm_state_size * 2, self.mamba_head_dim),
                )
            )
        )
        # time step projection (discretization)
        self.dt_proj_weight = Parameter(
            (mint.zeros((self.n_mamba_heads, self.mamba_head_dim, self.time_step_rank)) - 0.5)
            * 2
            / self.time_step_rank**0.5
        )
        self.dt_proj_bias = Parameter(mint.zeros((self.n_mamba_heads, self.mamba_head_dim)))
    ...
    def cuda_kernels_forward(
        self, hidden_states: ms.Tensor, cache_params: ZambaHybridDynamicCache = None, attention_mask=None
    ):
        ...
        projected_states = self.in_proj(hidden_states).swapaxes(1, 2)
        ...
        gate = gate.squeeze(2)
        gate = gate.reshape(batch_size, self.n_mamba_heads, -1, seq_len).swapaxes(0, 1)
        ...
        hidden_states = hidden_states.reshape(-1, self.n_mamba_heads, self.mamba_head_dim, seq_len).swapaxes(0, 1)
        ssm_parameters = (self.x_proj_weight[:, None, :, :] @ hidden_states).swapaxes(-1, -2)

        time_step, B, C = mint.split(
            ssm_parameters, [self.time_step_rank, self.ssm_state_size, self.ssm_state_size], dim=-1
        )

        discrete_time_step = self.dt_proj_weight[:, None] @ time_step.swapaxes(-1, -2)

        A = -mint.exp(self.A_log.float())

        # 3.c perform the recurrence y ← SSM(A, B, C)(x)
        time_proj_bias = self.dt_proj_bias.float() if self.dt_proj_bias is not None else None
        scan_outputs = mint.empty((batch_size, 0, seq_len), dtype=hidden_states.dtype)
        ...
        if use_precomputed_states:
            for n in range(self.n_mamba_heads):
                ...
                scan_outputs = mint.cat((scan_outputs, scan_outputs_), dim=1)

        else:
            ssm_state = mint.empty(
            ...
            for n in range(self.n_mamba_heads):
                ...
                scan_outputs = mint.cat((scan_outputs, scan_outputs_), dim=1).contiguous()
                ssm_state = mint.cat((ssm_state, ssm_state_.unsqueeze(1)), dim=1)
            if ssm_state is not None and cache_params is not None:
                cache_params.ssm_states[self.layer_idx].copy_(ssm_state)

        # 4. Final linear projection
        contextualized_states = self.out_proj(scan_outputs.swapaxes(1, 2))
    ...
    def slow_forward(self, input_states, cache_params: ZambaHybridDynamicCache = None, attention_mask=None):
        ...
        # 1. Gated linear projection
        projected_states = self.in_proj(input_states).swapaxes(1, 2)
        ...
        discrete_time_step = (self.dt_proj_weight[:, None] @ time_step.swapaxes(-1, -2)) + self.dt_proj_bias[
            :, None, :, None
        ]
        ...
        A = -mint.exp(self.A_log.float())
        discrete_A = mint.exp(A[:, None, :, None, :] * discrete_time_step[:, :, :, :, None])
        discrete_B = discrete_time_step[:, :, :, :, None] * B[:, :, None, :, :].float()
        deltaB_u = discrete_B * hidden_states[:, :, :, :, None].float()
        # 3.c perform the recurrence y ← SSM(A, B, C)(x)
        scan_outputs = []
        for i in range(seq_len):
            ssm_state = discrete_A[:, :, :, i, :].swapaxes(0, 1) * ssm_state + deltaB_u[:, :, :, i, :].swapaxes(0, 1)
            scan_output = mint.matmul(ssm_state.swapaxes(0, 1).to(dtype), C[:, :, i, :].unsqueeze(-1))
            scan_outputs.append(scan_output[:, :, :, 0])
        scan_output = mint.stack(scan_outputs, dim=-1)
        ...
        contextualized_states = self.out_proj(
            scan_output.swapaxes(0, 1).reshape(batch_size, -1, seq_len).swapaxes(1, 2)
        )
        return contextualized_states

    def construct(self, hidden_states, cache_params: ZambaHybridDynamicCache = None, attention_mask=None):
...
class ZambaAttentionDecoderLayer(nn.Cell):
    ...
    def construct(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        """
        hidden_states = mint.cat([hidden_states, original_hidden_states], dim=-1)
...
class ZambaMambaDecoderLayer(nn.Cell):
    def __init__(self, config: ZambaConfig, layer_idx: int):
        super().__init__()
...
class ZambaHybridLayer(nn.Cell):
    def __init__(
        self, shared_transf: ZambaAttentionDecoderLayer, linear: mint.nn.Linear, mamba: ZambaMambaDecoderLayer
    ):
        super().__init__()
...
ZAMBA_START_DOCSTRING = r"""
    ...
    This model is also a MindSpore [ms.nn.Cell](https://www.mindspore.cn/) subclass.
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if not empty_past_kv:
            ...
            past_key_values = ZambaHybridDynamicCache(self.config, input_ids.shape[0], dtype=self.dtype)
