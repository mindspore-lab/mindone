...
class TimesformerEmbeddings(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.cls_token = ms.Parameter(mint.zeros((1, 1, embed_dim)), name="cls_token")
        self.position_embeddings = ms.Parameter(
            mint.zeros((1, self.num_patches + 1, embed_dim)), name="position_embeddings"
        )
        self.pos_drop = mint.nn.Dropout(p=drop_rate)
        if attention_type != "space_only":
            self.time_embeddings = ms.Parameter(mint.zeros((1, num_frames, embed_dim)), name="time_embeddings")
            self.time_drop = mint.nn.Dropout(p=drop_rate)
    ...
    def construct(self, pixel_values):
        ...
        if self.attention_type != "space_only":
            ...
            if num_frames != self.time_embeddings.shape[1]:
                ...
                new_time_embeddings = mint.nn.functional.interpolate(time_embeddings, size=(num_frames), mode="nearest")
...
class TimesformerSelfAttention(nn.Cell):
    def __init__(self, config: TimesformerConfig):
        ...
        self.qkv = mint.nn.Linear(config.hidden_size, config.hidden_size * 3, bias=qkv_bias)
        self.attn_drop = mint.nn.Dropout(attention_dropout_prob)
...
class TimesformerLayer(nn.Cell):
    def __init__(self, config: TimesformerConfig, layer_index: int) -> None:
        ...
        drop_path_rates = [
            x.item() for x in mint.linspace(0, config.drop_path_rate, config.num_hidden_layers)
        ]  # stochastic depth decay rule
        drop_path_rate = drop_path_rates[layer_index]

        self.drop_path = TimeSformerDropPath(drop_path_rate) if drop_path_rate > 0.0 else mint.nn.Identity()
        ...
        if self.attention_type == "divided_space_time":
            self.temporal_layernorm = mint.nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
            self.temporal_attention = TimeSformerAttention(config)
            self.temporal_dense = mint.nn.Linear(config.hidden_size, config.hidden_size)

    def construct(self, hidden_states: ms.Tensor, output_attentions: bool = False):
        ...
        if self.attention_type in ["space_only", "joint_space_time"]:
            ...
            spatial_embedding = mint.cat((cls_token, spatial_embedding), 1)
            ...
            cls_token = mint.mean(cls_token, 1, True)  # averaging for every frame
            ...
            # Mlp
            hidden_states = mint.cat((init_cls_token, hidden_states), 1) + mint.cat((cls_token, residual), 1)
            layer_output = self.layernorm_after(hidden_states)
            layer_output = self.intermediate(layer_output)
            layer_output = self.output(layer_output)
            layer_output = hidden_states + self.drop_path(layer_output)
...
class TimesformerModel(TimesformerPreTrainedModel):
    ...
    def construct(
        ...
        >>> from mindone.transformers import TimesformerModel
        ...
        >>> model = TimesformerModel.from_pretrained("facebook/timesformer-base-finetuned-k400", revision="refs/pr/5")
...
class TimesformerForVideoClassification(TimesformerPreTrainedModel):
    ...
    def construct(
        ...
        >>> from transformers import AutoImageProcessor
        >>> from mindone.transformers import TimesformerForVideoClassification
        >>> from huggingface_hub import hf_hub_download
        ...
        ...     "facebook/timesformer-base-finetuned-k400", revision="refs/pr/5"
        ... )
