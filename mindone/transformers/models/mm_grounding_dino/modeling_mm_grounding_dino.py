...
class MMGroundingDinoContrastiveEmbedding(ms.nn.Cell):
    ...
    def construct(
        ...
        res = res + self.bias
        res = ops.masked_fill(res, ~text_token_mask[:, None, :], float("-inf"))

        # padding to max_text_len
        new_res = mint.full((*res.shape[:-1], self.max_text_len), float("-inf"))
...
class MMGroundingDinoLearnedPositionEmbedding(ms.nn.Cell):
    """
...
class MMGroundingDinoMultiscaleDeformableAttention(ms.nn.Cell):
    """
...
class MMGroundingDinoBiMultiHeadAttention(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class MMGroundingDinoDropPath(ms.nn.Cell):
    """Drop paths (Stochastic Depth) per sample (when applied in main path of residual blocks)."""
...
class MMGroundingDinoFusionLayer(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
        ...
        self.drop_path = MMGroundingDinoDropPath(drop_path) if drop_path > 0.0 else mint.nn.Identity()
        init_values = 1e-4
...
class MMGroundingDinoFrozenBatchNorm2d(ms.nn.Cell):
    """
...
def replace_batch_norm(model):
    r"""
    Recursively replace all `mindspore.nn.BatchNorm2d` with `MMGroundingDinoFrozenBatchNorm2d`.
...
class MMGroundingDinoConvEncoder(ms.nn.Cell):
    """
...
class MMGroundingDinoConvModel(ms.nn.Cell):
    """
...
class MMGroundingDinoMultiheadAttention(ms.nn.Cell):
...
class MMGroundingDinoTextEnhancerLayer(ms.nn.Cell):
    ...
    def construct(
        ...
        Args:
            ...
            attention_masks (`ms.BoolTensor`, *optional*):
        ...
        if attention_masks.dim() == 3 and attention_masks.shape[0] == hidden_states.shape[0]:
            ...
            attention_masks = (1.0 - attention_masks) * dtype_to_min(dtype)
...
class MMGroundingDinoDeformableLayer(ms.nn.Cell):
...
class MMGroundingDinoEncoderLayer(ms.nn.Cell):
...
class MMGroundingDinoEncoder(MMGroundingDinoPreTrainedModel):
    ...
    def __init__(self, config: MMGroundingDinoConfig):
        ...
        self.dropout = config.dropout
        self.layers = ms.nn.CellList([MMGroundingDinoEncoderLayer(config) for _ in range(config.encoder_layers)])
...
class MMGroundingDinoDecoderLayer(ms.nn.Cell):
    def __init__(self, config: MMGroundingDinoConfig):
        super().__init__()
        self.embed_dim = config.d_model
...
class MMGroundingDinoDecoder(MMGroundingDinoPreTrainedModel):
    ...
    def __init__(self, config: MMGroundingDinoConfig):
        ...
        self.layers = ms.nn.CellList([MMGroundingDinoDecoderLayer(config) for _ in range(config.decoder_layers)])
    ...
    def construct(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                # MindSpore equivalent - direct call without checkpoint
            ...
            if self.bbox_embed is not None:
                ...
                num_coordinates = reference_points.shape[-1]
                if num_coordinates == 4:
                    # MindSpore equivalent of torch.special.logit
                    new_reference_points = tmp + ops.log(reference_points / (1 - reference_points + 1e-5))
...
class MMGroundingDinoSinePositionEmbedding(ms.nn.Cell):
    """
...
class MMGroundingDinoMLPPredictionHead(ms.nn.Cell):
    """
...
def build_label_maps(logits: ms.Tensor, input_ids: ms.Tensor) -> tuple[ms.Tensor]:
    ...
    for label_group in label_groups:
        ...
        label_map = mint.nn.functional.pad(label_map, (0, max_seq_len - label_map.shape[1]), value=0)
...
class MMGroundingDinoForObjectDetection(MMGroundingDinoPreTrainedModel):
    ...
    def construct(
        ...
        for level in range(num_levels):
            ...
            reference = ops.log(reference / (1 - reference + 1e-5))
