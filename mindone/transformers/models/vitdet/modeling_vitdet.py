...
class VitDetEmbeddings(nn.Cell):
    ...
    def get_absolute_positions(self, abs_pos_embeddings, has_cls_token, height, width):
        ...
        if size != height or size != width:
            ...
            new_abs_pos_embeddings = mint.nn.functional.interpolate(
...
def add_decomposed_relative_positions(attn, queries, rel_pos_h, rel_pos_w, q_size, k_size):
    ...
    relative_height = mint.einsum("bhwc,hkc->bhwk", r_q, relative_height)
    relative_weight = mint.einsum("bhwc,wkc->bhwk", r_q, relative_width)
...
class VitDetAttention(nn.Cell):
    ...
    def __init__(self, config, input_size=None):
        ...
        self.qkv = mint.nn.Linear(dim, dim * 3, bias=config.qkv_bias)
    ...
    def construct(self, hidden_state, output_attentions=False):
        ...
        attention_scores = mint.matmul(queries * self.scale, keys.transpose(-2, -1))
...
class VitDetResBottleneckBlock(nn.Cell):
    ...
    def __init__(self, config, in_channels, out_channels, bottleneck_channels):
        ...
        """
        super().__init__()
        self.conv1 = mint.nn.Conv2d(in_channels, bottleneck_channels, 1, bias=False)
        self.norm1 = VitDetLayerNorm(bottleneck_channels)
        self.act1 = ACT2FN[config.hidden_act]

        self.conv2 = mint.nn.Conv2d(bottleneck_channels, bottleneck_channels, 3, padding=1, bias=False)
        self.norm2 = VitDetLayerNorm(bottleneck_channels)
        self.act2 = ACT2FN[config.hidden_act]

        self.conv3 = mint.nn.Conv2d(bottleneck_channels, out_channels, 1, bias=False)
...
class VitDetMlp(nn.Cell):
    def __init__(self, config, in_features: int, hidden_features: int) -> None:
        ...
        self.fc2 = mint.nn.Linear(hidden_features, in_features)
        self.drop = mint.nn.Dropout(config.dropout_prob)
...
class VitDetEncoder(nn.Cell):
    def __init__(self, config: VitDetConfig) -> None:
        ...
        drop_path_rate = [x.item() for x in mint.linspace(0, config.drop_path_rate, depth)]
...
def caffe2_msra_fill(module: nn.Cell) -> None:
    """
