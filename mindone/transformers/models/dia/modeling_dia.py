...
class DiaEncoder(DiaPreTrainedModel):
    def __init__(self, config: DiaEncoderConfig):
        ...
        self.layers = nn.CellList([DiaEncoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
...
class DiaDecoder(DiaPreTrainedModel):
    ...
    def __init__(self, config: DiaDecoderConfig):
        ...
        self.rotary_embeddings = DiaRotaryEmbedding(config)
        self.layers = nn.CellList([DiaDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
        self.norm = DiaRMSNorm(config.hidden_size, eps=config.norm_eps)
    ...
    def construct(
        ...
        **kwargs,
        ...
        if decoder_input_ids is None:
            decoder_input_ids = mint.full(size=(bsz, 1, channels), fill_value=self.config.bos_token_id)
        # Ensure 3D
