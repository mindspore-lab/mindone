...
from models.blip_2.modeling_blip_2 import (
...
class BertEmbeddings(nn.Cell):
    ...
    def __init__(self, config):
        ...
        # 先注释掉 The 'BertEmbeddings' object has no attribute 'register_buffer'.
        # self.register_buffer(
        #     "position_ids", ops.arange(config.max_position_embeddings).expand((1, -1))
        # )
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            if type(encoder_attention_mask) == list:
                ...
                encoder_attention_mask = ops.ones(encoder_hidden_shape, device=device)
...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        attention_mask = ops.cat([query_mask, attention_mask], dim=-1)
...
encoder_config.encoder_hidden_size = 768
# insert cross-attention layer every other block
encoder_config.add_cross_attention = True
encoder_config.cross_attention_frequency = 2
encoder_config.query_length = 32

def bert():
    # model = BertLMHeadModel(config=encoder_config)
    # model = BertEmbeddings(config=encoder_config)
    # model = BertEncoder(config=encoder_config)
    # model = BertPooler(config=encoder_config)
    model = BertOnlyMLMHead(config=encoder_config)
    return model
