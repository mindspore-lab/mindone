...
"""MindSpore IdeficsVision model: a copy of CLIPVisionModel using a simpler config object"""
...
class IdeficsVisionEmbeddings(nn.Cell):
    ...
    def interpolate_pos_encoding(self, embeddings: ms.Tensor, height: int, width: int) -> ms.Tensor:
        ...
        fp32_upcasting = patch_pos_embed.dtype == ms.bfloat16
        if fp32_upcasting:
            logger.warning_once(
                "Upcasting patch_pos_embed to fp32 for interpolation since `upsample_bicubic2d_out_frame` in mint.nn.functional.interpolate "
                "is not implemented for 'ms.bfloat16' dtype. This will result in a slight overhead."
            )
            patch_pos_embed = patch_pos_embed.to(ms.float32)
        ...
        if fp32_upcasting:
            patch_pos_embed = patch_pos_embed.to(ms.bfloat16)
...
class IdeficsVisionAttention(nn.Cell):
...
class IdeficsVisionMLP(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class IdeficsVisionEncoderLayer(nn.Cell):
    def __init__(self, config: IdeficsVisionConfig):
        super().__init__()
        self.embed_dim = config.hidden_size
...
class IdeficsVisionEncoder(nn.Cell):
    """
    ...
    def __init__(self, config: IdeficsVisionConfig):
        super().__init__()
        self.config = config
        self.layers = nn.CellList([IdeficsVisionEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class IdeficsVisionTransformer(nn.Cell):
    def __init__(self, config: IdeficsVisionConfig):
        super().__init__()
        self.config = config
