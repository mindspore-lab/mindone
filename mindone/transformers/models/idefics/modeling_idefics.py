...
def freeze_model(model, module_exceptions=[]):
    mapping = {
        ...
        "Linear": mint.nn.Linear,
        "Embedding": mint.nn.Embedding,
    }
    module_exceptions_mapped = [mapping[m] for m in module_exceptions]
    for module in model.cells():
        requires_grad = False
        if module_exceptions and any(isinstance(module, t) for t in module_exceptions_mapped):
            requires_grad = True  # Explicitely setting it to true to avoid any mistakes
        for param in module.get_parameters():
            param.requires_grad = requires_grad
    return model


class IdeficsDecoupledEmbedding(mint.nn.Embedding):
    """
    Implements a decoupling of parameters to allow freezing (or not) a subset of the embeddings. In practise, the
    regular `weight` can be trained or frozen (i.e. `partially_freeze=True`), and if `num_additional_embeddings` > 0,
    then it will create `num_additional_embeddings` additional parameters that are always trained. If
    `num_additional_embeddings=0`, then the module defaults back to the regular behavior of `mint.nn.Embedding`.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Note: there are a lot of other parameters to initialize a standard `mint.nn.Embedding` such as `padding_idx`,
    ...
    def construct(self, input_ids):
        ...
        additional_vocab_indices = mint.where(input_ids >= self.num_embeddings)
...
class IdeficsDecoupledLinear(mint.nn.Linear):
    """
    Implements a decoupling of parameters to allow freezing (or not) a subset of the parameters. In practise, the
    regular `weight` can be trained or frozen (i.e. `partially_freeze=True`), and if `out_additional_features` > 0,
    then it will create `out_additional_features * in_features` additional parameters that are always trained. If
    `out_additional_features=0`, then the module defaults back to the regular behavior of `mint.nn.Linear`.
    """
    ...
    def __init__(
        ...
        parameters (if any) will be trainable. If False, default to the regular behavior of mint.nn.Linear.
        """
        ...
        if out_additional_features > 0:
            self.additional_fc = mint.nn.Linear(
    ...
    def construct(self, input: ms.Tensor) -> ms.Tensor:
        ...
        if self.out_additional_features > 0:
            ...
            output = mint.cat((output, additional_features), -1)

        return output

    def extra_repr(self) -> str:
        """Overwriting `mint.nn.Linear.extra_repr` to include new parameters."""
...
class IdeficsEmbedding(nn.Cell):
    ...
    def _set_cos_sin_cache(self, seq_len, dtype):
        ...
        freqs = mint.einsum("i,j->ij", t, self.inv_freq)
...
class IdeficsAttention(nn.Cell):
    ...
    def __init__(
        ...
        if self.is_cross_attention:
            ...
            self.k_proj = mint.nn.Linear(kv_input_dim, num_heads * self.head_dim, bias=False)
...
class IdeficsDecoderLayer(nn.Cell):
...
class IdeficsGatedCrossAttentionLayer(nn.Cell):
    def __init__(self, config: IdeficsConfig, layer_idx: int = None):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.act_cross_attn = mint.nn.Tanh()
        self.act_dense = mint.nn.Tanh()

        if config.alpha_initializer == "zeros":
            if config.alpha_type == "vector":
                self.alpha_cross_attn = ms.Parameter(mint.zeros((1, 1, self.hidden_size)))
                self.alpha_dense = ms.Parameter(mint.zeros((1, 1, self.hidden_size)))
            elif config.alpha_type == "float":
                self.alpha_cross_attn = ms.Parameter(mint.zeros((1,)))
                self.alpha_dense = ms.Parameter(mint.zeros((1,)))
            else:
                raise ValueError(f"Unknown value for `alpha_type` ({config.alpha_type})")

        elif config.alpha_initializer == "ones":
            if config.alpha_type == "vector":
                self.alpha_cross_attn = ms.Parameter(mint.ones((1, 1, self.hidden_size)))
                self.alpha_dense = ms.Parameter(mint.ones((1, 1, self.hidden_size)))
            elif config.alpha_type == "float":
                self.alpha_cross_attn = ms.Parameter(mint.ones((1,)))
                self.alpha_dense = ms.Parameter(mint.ones((1,)))
            else:
                raise ValueError(f"Unknown value for `alpha_type` ({config.alpha_type})")

        elif config.alpha_initializer in {"normal", "gaussian", "random"}:
            if config.alpha_type == "vector":
                self.alpha_cross_attn = ms.Parameter(
                    mint.normal(mean=0.0, std=config.alphas_initializer_range, size=(1, 1, self.hidden_size))
                )
                self.alpha_dense = ms.Parameter(
                    mint.normal(mean=0.0, std=config.alphas_initializer_range, size=(1, 1, self.hidden_size))
                )
            elif config.alpha_type == "float":
                self.alpha_cross_attn = ms.Parameter(
                    mint.normal(mean=0.0, std=config.alphas_initializer_range, size=(1))
                )
                self.alpha_dense = ms.Parameter(mint.normal(mean=0.0, std=config.alphas_initializer_range, size=(1)))
            else:
                raise ValueError(f"Unknown value for `alpha_type` ({config.alpha_type})")
    ...
    def construct(
        ...
        cross_attention_gate: Optional[ms.Tensor] = None,
        output_attentions: Optional[bool] = False,
        use_cache: Optional[bool] = False,
        past_key_value: Optional[Tuple[ms.Tensor]] = None,
    ) -> Tuple[ms.Tensor, Optional[Tuple[ms.Tensor, ms.Tensor]]]:
        ...
        Args:
            ...
            attention_mask (`ms.Tensor`, *optional*): attention mask of size
                `(batch, 1, tgt_len, src_len)` where padding elements are indicated by very large negative values.
            image_attention_mask (`ms.Tensor`, *optional*): image attention mask of size
                `(batch, 1, tgt_len, src_len)` where padding elements are indicated by very large negative values.
            cross_attention_gate (`ms.Tensor`, *optional*):
        ...
        hidden_states = mint.nn.functional.dropout(hidden_states, p=self.config, training=self.training)
...
class IdeficsModel(IdeficsPreTrainedModel):
    ...
    def __init__(self, config: IdeficsConfig):
        ...
        self.gated_cross_attn_layers = nn.CellList(
    ...
    def construct(
        ...
        image_encoder_embeddings: Optional[ms.Tensor] = None,
        perceiver_embeddings: Optional[ms.Tensor] = None,
        image_attention_mask: Optional[ms.Tensor] = None,
        ...
        image_attention_mask = image_attention_mask.tile((1, 1, 1, image_seq_len))
        ...
        if image_hidden_states is not None:
            ...
            if image_attention_mask is None:
                image_attention_mask = mint.ones(image_hidden_shape)
        ...
        **kwargs,
        ...
        if "image_attention_mask" in model_kwargs:
            ...
            if model_kwargs.get("use_cache", True):
                ...
                model_kwargs["image_attention_mask"] = mint.cat([image_attention_mask, last_mask], dim=1)
