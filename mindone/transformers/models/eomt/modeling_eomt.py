...
import numpy as np
from transformers.models.eomt.configuration_eomt import EomtConfig
...
from ...utils import ModelOutput, can_return_tuple, is_scipy_available, requires_backends
...
class EomtForUniversalSegmentationOutput(ModelOutput):
    ...
    patch_offsets: Optional[list[ms.Tensor]] = None
...
def sample_point(input_features: ms.Tensor, point_coordinates: ms.Tensor, add_dim=False, **kwargs) -> ms.Tensor:
    ...
    # use mint.nn.functional.grid_sample to get features for points in `point_coordinates` via bilinear interpolation
    point_features = mint.nn.functional.grid_sample(input_features.float(), 2.0 * point_coordinates - 1.0, **kwargs).to(
...
class EomtHungarianMatcher(nn.Cell):
    ...
    def construct(
        ...
        for i in range(batch_size):
            ...
            # Compute the classification cost. Contrary to the loss, we don't use the NLL, but approximate it in 1 - proba[target class]. The 1 is a constant that doesn't change the matching, it can be omitted.  # noqa: E501
...
class EomtLoss(nn.Cell):
    ...
    def sample_points_using_uncertainty(
        ...
        if num_random_points > 0:
            point_coordinates = mint.cat([point_coordinates, mint.rand(num_boxes, num_random_points, 2)], dim=1)
...
class EomtPatchEmbeddings(nn.Cell):
    """
...
class EomtEmbeddings(nn.Cell):
    ...
    def __init__(self, config: EomtConfig) -> None:
        ...
        self.cls_token = ms.Parameter(mint.randn(1, 1, config.hidden_size), name="cls_token")
        self.register_tokens = ms.Parameter(
            mint.zeros((1, config.num_register_tokens, config.hidden_size)), name="register_tokens"
        )
        ...
        self.position_embeddings = mint.nn.Embedding(num_patches, config.hidden_size)
        self.position_ids = mint.arange(num_patches).broadcast_to((1, -1))
    ...
    def construct(self, pixel_values: ms.Tensor) -> ms.Tensor:
        ...
        register_tokens = self.register_tokens.broadcast_to((batch_size, -1, -1))

        embeddings = embeddings + self.position_embeddings(self.position_ids)
        embeddings = mint.cat([cls_tokens, register_tokens, embeddings], dim=1)
...
class EomtLayerScale(nn.Cell):
    def __init__(self, config) -> None:
        super().__init__()
        self.lambda1 = ms.Parameter(config.layerscale_value * mint.ones(config.hidden_size), name="lambda1")
...
class EomtLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config: EomtConfig) -> None:
        ...
        self.drop_path = EomtDropPath(config.drop_path_rate) if config.drop_path_rate > 0.0 else mint.nn.Identity()
...
class EomtLayerNorm2d(mint.nn.LayerNorm):
...
class EomtScaleLayer(nn.Cell):
    def __init__(self, config: EomtConfig):
        super().__init__()
        hidden_size = config.hidden_size
        self.conv1 = mint.nn.ConvTranspose2d(hidden_size, hidden_size, kernel_size=2, stride=2)
        self.activation = ACT2FN[config.hidden_act]
...
class EomtScaleBlock(nn.Cell):
    def __init__(self, config: EomtConfig):
        super().__init__()
        self.num_blocks = config.num_upscale_blocks
        self.block = nn.CellList([EomtScaleLayer(config) for _ in range(self.num_blocks)])
...
class EomtMaskHead(nn.Cell):
    def __init__(self, config: EomtConfig):
        super().__init__()

        hidden_size = config.hidden_size
        self.fc1 = mint.nn.Linear(hidden_size, hidden_size)
        self.fc2 = mint.nn.Linear(hidden_size, hidden_size)
        self.fc3 = mint.nn.Linear(hidden_size, hidden_size)
        self.activation = ACT2FN[config.hidden_act]
...
class EomtForUniversalSegmentation(EomtPreTrainedModel):
    ...
    def __init__(self, config: EomtConfig) -> None:
        ...
        self.query = mint.nn.Embedding(config.num_queries, config.hidden_size)
        self.layers = nn.CellList([EomtLayer(config) for _ in range(config.num_hidden_layers)])

        self.upscale_block = EomtScaleBlock(config)
        self.mask_head = EomtMaskHead(config)

        self.class_predictor = mint.nn.Linear(config.hidden_size, config.num_labels + 1)
        ...
        self.attn_mask_probs = ms.Parameter(mint.ones(config.num_blocks), requires_grad=False, name="attn_mask_probs")
    ...
    def construct(
        ...
        for idx, layer_module in enumerate(self.layers):
            ...
            if idx == self.num_hidden_layers - self.config.num_blocks:
                query = self.query.weight[None, :, :].broadcast_to((hidden_states.shape[0], -1, -1))
                hidden_states = mint.cat((query, hidden_states), dim=1)
            ...
            if idx >= self.num_hidden_layers - self.config.num_blocks and (
                ...
                attention_mask = mint.ones(
                    (hidden_states.shape[0], hidden_states.shape[1], hidden_states.shape[1]), dtype=ms.bool_
                )
                ...
                interpolated_logits = interpolated_logits.view(
                    interpolated_logits.shape[0], interpolated_logits.shape[1], -1
                )
                ...
                attention_mask = attention_mask[:, None, ...].broadcast_to(
    ...
    def predict(self, logits: ms.Tensor):
        ...
        prefix_tokens = prefix_tokens.swapaxes(1, 2)
        ...
        mask_logits = mint.einsum("bqc, bchw -> bqhw", query_tokens, prefix_tokens)

        return mask_logits, class_logits

    @staticmethod
    def _disable_attention_mask(attn_mask, prob, num_query_tokens, encoder_start_tokens):
        if prob < 1:
            # Generate random queries to disable based on the probs
            random_queries = mint.rand(attn_mask.shape[0], num_query_tokens) > prob
