...
from ...utils import TensorType, filter_out_non_signature_kwargs, is_mindspore_available
...
def reorder_patches_and_offsets(
    patches: list[ms.Tensor], offsets: list[list[int]]
) -> tuple[list[ms.Tensor], list[list[int]]]:
...
class EomtImageProcessorFast(BaseImageProcessorFast):
    ...
    def _split_image(self, images: ms.Tensor, size: dict, image_indices: int) -> tuple[list, list]:
        ...
        for i in range(num_patches):
            ...
            for batch_idx, single in enumerate(mint.unbind(batch_patch, dim=0)):
                patches.append(single)
                patch_offsets.append([image_indices[batch_idx], start, end])

        return patches, patch_offsets

    def _pad(self, images: ms.Tensor, size: dict) -> ms.Tensor:
        ...
        padded_images = mint.nn.functional.pad(images, padding, mode="constant", value=0.0)
        return padded_images

    def preprocess(
        self,
        images: ImageInput,
        segmentation_maps: Optional[list[ms.Tensor]] = None,
    ...
    def _preprocess_image_like_inputs(
        ...
        **kwargs: Unpack[EomtImageProcessorFastKwargs],
        ...
        if segmentation_maps is not None:
            ...
            segmentation_maps_kwargs.update(
                {
                    ...
                    "interpolation": pil_mindspore_interpolation_mapping[PILImageResampling.NEAREST],
                }
            )
            ...
            processed_segmentation_maps = processed_segmentation_maps.squeeze(1).to(ms.int64)
            ...
            for idx, segmentation_map in enumerate(processed_segmentation_maps):
                ...
                mask_labels.append(ms.Tensor.from_numpy(masks))
                class_labels.append(ms.Tensor.from_numpy(classes))
        ...
        if patch_offsets:
            outputs["patch_offsets"] = [ms.tensor(offsets) for offsets in patch_offsets]
        ...
        **kwargs,
        ...
        for shape, stacked_images in grouped_images.items():
            if do_resize:
                ...
                stacked_images_updated = mint.stack(stacked_images_updated)
        ...
        processed_images = mint.stack(images, dim=0) if return_tensors else images

        return processed_images, patch_offsets

    def merge_image_patches(
        self,
        segmentation_logits: ms.Tensor,
        ...
        for image_size in target_sizes:
            ...
            aggregated_logits.append(mint.zeros((num_classes, height, width)))
            patch_counts.append(mint.zeros((num_classes, height, width)))
    ...
    def post_process_semantic_segmentation(
        ...
        masks_queries_logits = mint.nn.functional.interpolate(
        ...
        masks_classes = class_queries_logits.softmax(axis=-1)[..., :-1]
        masks_probs = masks_queries_logits.sigmoid()  # [batch_size, num_queries, height, width]

        segmentation_logits = mint.einsum("bqc, bqhw -> bchw", masks_classes, masks_probs)
    ...
    def post_process_panoptic_segmentation(
        ...
        pred_scores_batch, pred_labels_batch = class_queries_logits.softmax(axis=-1).max(-1)
        ...
        for i in range(batch_size):
            ...
            if mask_probs.shape[0] <= 0:
                ...
                segmentation = mint.zeros((height, width)) - 1
    ...
    def post_process_instance_segmentation(
        ...
        for i in range(batch_size):
            ...
            # Remove the null class `[..., :-1]`
            scores, pred_classes = mask_class.softmax(axis=-1)[..., :-1].max(-1)
            ...
            segmentation = mint.zeros(target_sizes[i]) - 1
            ...
            for j in range(num_queries):
                ...
                if not mint.all(pred_masks[j] == 0) and score >= threshold:
