...
from transformers.models.cvt.configuration_cvt import CvtConfig
...
class BaseModelOutputWithCLSToken(ModelOutput):
    ...
    Args:
        ...
        cls_token_value (`ms.Tensor` of shape `(batch_size, 1, hidden_size)`):
    ...
    cls_token_value: ms.Tensor = None
...
class CvtConvEmbeddings(nn.Cell):
    ...
    def __init__(self, patch_size, num_channels, embed_dim, stride, padding):
        ...
        self.projection = mint.nn.Conv2d(
            num_channels, embed_dim, kernel_size=patch_size, stride=stride, padding=padding
        )
        self.normalization = mint.nn.LayerNorm(embed_dim)
...
class CvtSelfAttentionConvProjection(nn.Cell):
    def __init__(self, embed_dim, kernel_size, padding, stride):
        ...
        self.normalization = mint.nn.BatchNorm2d(embed_dim)
...
class CvtSelfAttention(nn.Cell):
    def __init__(
        ...
        **kwargs,
        ...
        self.projection_query = mint.nn.Linear(embed_dim, embed_dim, bias=qkv_bias)
        self.projection_key = mint.nn.Linear(embed_dim, embed_dim, bias=qkv_bias)
        self.projection_value = mint.nn.Linear(embed_dim, embed_dim, bias=qkv_bias)

        self.dropout = mint.nn.Dropout(attention_drop_rate)
    ...
    def construct(self, hidden_state, height, width):
        if self.with_cls_token:
            cls_token, hidden_state = mint.split(hidden_state, [1, height * width], 1)
        ...
        if self.with_cls_token:
            query = mint.cat((cls_token, query), dim=1)
            key = mint.cat((cls_token, key), dim=1)
            value = mint.cat((cls_token, value), dim=1)
        ...
        attention_score = mint.einsum("bhlk,bhtk->bhlt", query, key) * self.scale
        attention_probs = mint.nn.functional.softmax(attention_score, dim=-1)
        attention_probs = self.dropout(attention_probs)

        context = mint.einsum("bhlt,bhtv->bhlv", attention_probs, value)
...
class CvtSelfOutput(nn.Cell):
    ...
    def __init__(self, embed_dim, drop_rate):
        super().__init__()
        self.dense = mint.nn.Linear(embed_dim, embed_dim)
        self.dropout = mint.nn.Dropout(drop_rate)
...
class CvtIntermediate(nn.Cell):
    def __init__(self, embed_dim, mlp_ratio):
        super().__init__()
        self.dense = mint.nn.Linear(embed_dim, int(embed_dim * mlp_ratio))
...
class CvtOutput(nn.Cell):
    def __init__(self, embed_dim, mlp_ratio, drop_rate):
        super().__init__()
        self.dense = mint.nn.Linear(int(embed_dim * mlp_ratio), embed_dim)
        self.dropout = mint.nn.Dropout(drop_rate)
...
class CvtLayer(nn.Cell):
    ...
    def __init__(
        ...
        self.drop_path = CvtDropPath(drop_prob=drop_path_rate) if drop_path_rate > 0.0 else mint.nn.Identity()
        self.layernorm_before = mint.nn.LayerNorm(embed_dim)
        self.layernorm_after = mint.nn.LayerNorm(embed_dim)
...
class CvtStage(nn.Cell):
    def __init__(self, config, stage):
        ...
        if self.config.cls_token[self.stage]:
            self.cls_token = ms.Parameter(mint.randn(1, 1, self.config.embed_dim[-1]))
        ...
        drop_path_rates = [x.item() for x in mint.linspace(0, config.drop_path_rate[self.stage], config.depth[stage])]
    ...
    def construct(self, hidden_state):
        ...
        if self.config.cls_token[self.stage]:
            ...
            hidden_state = mint.cat((cls_token, hidden_state), dim=1)
...
class CvtForImageClassification(CvtPreTrainedModel):
    def __init__(self, config):
        ...
        self.layernorm = mint.nn.LayerNorm(config.embed_dim[-1])
        # Classifier head
        self.classifier = (
            mint.nn.Linear(config.embed_dim[-1], config.num_labels) if config.num_labels > 0 else mint.nn.Identity()
        )
    ...
    def construct(
        ...
        ```python
        >>> from mindone.transformers import AutoImageProcessor, CvtForImageClassification
