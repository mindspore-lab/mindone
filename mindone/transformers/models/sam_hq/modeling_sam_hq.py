...
class SamHQVisionEncoderOutput(ModelOutput):
    ...
    intermediate_embeddings (`list(ms.Tensor)`, *optional*):
...
class SamHQMMaskDecoderOutputs(ModelOutput):
    r"""
    masks (`ms.Tensor` of shape `(batch_size, num_prompts, num_masks, height, width)`):
        The predicted masks for the input image. The masks are of shape `(batch_size, num_prompts, num_masks, height, width)`.
    iou_scores (`ms.Tensor` of shape `(batch_size, num_prompts, num_masks)`):
        The predicted IoU scores for each mask. The scores are of shape `(batch_size, num_prompts, num_masks)`.
    mask_decoder_attentions (`ms.Tensor`, *optional*):
        The attention weights from the mask decoder, if `output_attentions=True` was passed during the construct pass.
        This is specific to SAM-HQ and not present in base SAM.
    """

    masks: ms.Tensor
    iou_scores: Optional[ms.Tensor] = None
    mask_decoder_attentions: Optional[ms.Tensor] = None
...
class SamHQImageSegmentationOutput(ModelOutput):
    r"""
    iou_scores (`ms.Tensor` of shape `(batch_size, num_masks)`):
    ...
    vision_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
        ...
        heads.
    mask_decoder_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
        Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
        sequence_length)`.
...
class SamHQVisionAttention(nn.Cell):
    ...
    def construct(self, hidden_states: ms.Tensor, output_attentions=None) -> tuple[ms.Tensor, ms.Tensor]:
...
class SamHQVisionSdpaAttention(SamHQVisionAttention):
    ...
    def construct(self, hidden_states: ms.Tensor, output_attentions=False) -> ms.Tensor:
        if output_attentions:
            logger.warning_once(
                "`SamHQVisionSdpaAttention` is used but `scaled_dot_product_attention` does not support "
        ...
        attn_output = scaled_dot_product_attention(query, key, value, attn_mask=attn_bias)
...
class SamHQVisionLayer(GradientCheckpointingLayer):
    ...
    def window_partition(self, hidden_states: ms.Tensor, window_size: int) -> tuple[ms.Tensor, tuple[int, int]]:
        """
        Args:
    ...
    def window_unpartition(
        self, windows: ms.Tensor, window_size: int, padding_shape: tuple[int, int], original_shape: tuple[int, int]
    ) -> ms.Tensor:
        """
        Args:
...
class SamHQPatchEmbeddings(nn.Cell):
    """
...
class SamHQVisionEncoder(SamHQPreTrainedModel):
    ...
    @check_model_inputs
    def construct(
        self, pixel_values: Optional[ms.Tensor] = None, **kwargs: Unpack[TransformersKwargs]
...
class SamHQLayerNorm(mint.nn.LayerNorm):
...
class SamHQAttention(nn.Cell):
    ...
    def __init__(self, config, downsample_rate=None):
        ...
        self.out_proj = mint.nn.Linear(self.internal_dim, self.hidden_size)
...
class SamHQTwoWayAttentionBlock(nn.Cell):
    def __init__(self, config, attention_downsample_rate: int = 2, skip_first_layer_pe: bool = False):
        ...
        self.layer_norm1 = mint.nn.LayerNorm(self.hidden_size, eps=self.layer_norm_eps)

        self.cross_attn_token_to_image = SamHQAttention(config, downsample_rate=attention_downsample_rate)
        self.layer_norm2 = mint.nn.LayerNorm(self.hidden_size, eps=self.layer_norm_eps)

        self.mlp = SamHQMLPBlock(config)
        self.layer_norm3 = mint.nn.LayerNorm(self.hidden_size, eps=self.layer_norm_eps)

        self.layer_norm4 = mint.nn.LayerNorm(self.hidden_size, eps=self.layer_norm_eps)
...
class SamHQTwoWayTransformer(nn.Cell):
...
class SamHQMaskDecoder(nn.Cell):
    def __init__(self, config: SamHQMaskDecoderConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.hq_token = mint.nn.Embedding(1, self.hidden_size)
    ...
    def construct(
        ...
        Args:
            ...
            image_positional_embedding (`ms.Tensor`):
            ...
            intermediate_embeddings (`ms.Tensor`):
        ...
        Returns:
            `Tuple[ms.Tensor, ms.Tensor, Optional[ms.Tensor]]`: A tuple of tensors containing:
        ...
        output_tokens = mint.cat([self.iou_token.weight, self.mask_tokens.weight, self.hq_token.weight], dim=0)
        ...
        if hq_features.shape[0] == 1:
            hq_features = hq_features.tile((batch_size * point_batch_size, 1, 1, 1))
        ...
        masks = mint.cat([masks_sam, masks_hq], dim=2)
        ...
        if multimask_output:
            ...
            iou_pred_sorted, sort_indices = mint.sort(iou_pred, dim=2, descending=True)
            # Reorder the masks according to sorted scores
            masks_sam = masks[:, :, mask_slice, :, :]
            masks_sam = mint.gather(
                masks_sam,
                2,
                sort_indices[..., None, None].broadcast_to((-1, -1, -1, masks_sam.shape[3], masks_sam.shape[4])),
            )
...
class SamHQPositionalEmbedding(nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class SamHQMaskEmbedding(nn.Cell):
...
class SamHQPromptEncoder(nn.Cell):
    def __init__(self, config: SamHQConfig):
        ...
        self.point_embed = nn.CellList(
            [mint.nn.Embedding(1, config.hidden_size) for i in range(config.num_point_embeddings)]
        )
        self.hidden_size = config.hidden_size
    ...
    def _embed_points(self, points: ms.Tensor, labels: ms.Tensor, pad: bool) -> ms.Tensor:
        ...
        # This is required for the ONNX export. The dtype need to be explicitly
        # specified as otherwise torch.onnx.export interprets as double
        point_embedding = mint.where(labels[..., None] != -10, point_embedding, mint.zeros_like(point_embedding))
...
class SamHQModel(SamHQPreTrainedModel):
    ...
    def get_image_wide_positional_embeddings(self):
        ...
        grid = mint.ones((size, size), dtype=target_dtype)
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> import requests
        >>> import mindspore as ms
        >>> from mindone.transformers import AutoModel, AutoProcessor

        >>> model = AutoModel.from_pretrained("syscv-community/sam-hq-vit-base")
        >>> processor = AutoProcessor.from_pretrained("syscv-community/sam-hq-vit-base")
