...
class FlavaModelOutput(ModelOutput):
    ...
    Args:
        image_embeddings (`ms.Tensor` of shape `(batch_size, output_dim)`, *optional*, returned when `pixel_values` are present):
            The image embeddings which are basically the pooled output of [`FlavaImageModel`].
        image_output (`BaseModelOutputWithPooling`, *optional*, returned when `pixel_values` are present):
            The output of the [`FlavaImageModel`].
        text_embeddings (`ms.Tensor` of shape `(batch_size, output_dim)`, *optional*, returned when `input_ids` are present):
            The text embeddings which are basically the pooled output of [`FlavaTextModel`].
        text_output (`BaseModelOutputWithPooling`, *optional*, returned when `input_ids` are present):
            The output of the [`FlavaTextModel`].
        multimodal_embeddings (`ms.Tensor` of shape `(batch_size, output_dim)`, *optional*, returned when `input_ids` and `pixel_values` are present and `skip_multimodal_encoder` is `None` or `False`):
    ...
    text_embeddings: Optional[ms.Tensor] = None
    text_output: Optional[BaseModelOutputWithPooling] = None
    multimodal_embeddings: Optional[ms.Tensor] = None
...
class FlavaLosses(ModelOutput):
    ...
    Args:
        mim (`ms.Tensor` of shape `(1,)`, *optional*, returned when `mim_labels` and `pixel_values` are present, `input_ids_masked` is absent and `mim_weight` > 0.:
            Masked Image Modeling loss as used in BeIT calculated only for unimodal image data.
        mlm (`ms.Tensor` of shape `(1,)`, *optional*, returned when `mlm_labels` and `input_ids_masked` are present, `pixel_values` is absent and `mlm_weight` > 0.:
            Masked Language Modeling loss as used in BERT calculated only for unimodal text data.
        itm (`ms.Tensor` of shape `(1,)`, *optional*, returned when `itm_labels`, `input_ids_masked`, `pixel_values` are present and `itm_weight` > 0.:
            Image Text Matching (ITM) loss calculated for paired image-text data. Note that ITM loss is calculated on
            masked pairs in FLAVA.
        global_contrastive (`ms.Tensor` of shape `(1,)`, *optional*, returned when `input_ids` and `pixel_values` are present and `global_contrastive_weight` > 0.:
            Contrastive loss for image-text similarity similar to CLIP but calculated globally for paired image-text
            data. This is calculated on unmasked images and texts.
        mmm_image (`ms.Tensor` of shape `(1,)`, *optional*, returned when `mim_labels`, `pixel_values` and `input_ids_masked` are present and `mmm_image_weight` > 0.:
            Masked Multimodal Modeling loss's image component calculated on paired image-text data.
        mmm_text (`ms.Tensor` of shape `(1,)`, *optional*, returned when `mlm_labels`, `pixel_values` and `input_ids_masked` are present and `mmm_text_weight` > 0.:
            Masked Multimodal Modeling loss's text component calculated on paired image-text data.
    """  # noqa: E501

    mim: Optional[ms.Tensor] = None
    mlm: Optional[ms.Tensor] = None
    itm: Optional[ms.Tensor] = None
    global_contrastive: Optional[ms.Tensor] = None
    mmm_image: Optional[ms.Tensor] = None
    mmm_text: Optional[ms.Tensor] = None
...
class FlavaForPreTrainingOutput(ModelOutput):
    ...
    Args:
        loss (`ms.Tensor`, *optional*, returned when `return_loss` is True):
        ...
        text_output (`BaseModelOutputWithPooling`, *optional*, returned when `input_ids` are present):
            The output of the [`FlavaTextModel`].
        multimodal_embeddings (`ms.Tensor` of shape `(batch_size, output_dim)`, *optional*, returned when `input_ids` and `pixel_values` are present and `skip_unmasked_multimodal_encoder` is `None` or `False`):
            The multimodal embeddings which are basically the pooled output of [`FlavaTextModel`].
        ...
        image_masked_embeddings (`ms.Tensor` of shape `(batch_size, output_dim)`, *optional*, returned when `pixel_values` are present):
        ...
        text_masked_embeddings (`ms.Tensor` of shape `(batch_size, output_dim)`, *optional*, returned when `input_ids_masked` are present):
            The text embeddings which are basically the pooled output of [`FlavaTextModel`].
        text_masked_output (`BaseModelOutputWithPooling`, *optional*, returned when `input_ids_masked` are present):
            The output of the [`FlavaTextModel`].
        multimodal_masked_embeddings (`ms.Tensor` of shape `(batch_size, output_dim)`, *optional*, returned when `input_ids` and `pixel_values` are present):
            The multimodal embeddings which are basically the pooled output of [`FlavaTextModel`].
        multimodal_masked_output (`BaseModelOutputWithPooling`, *optional*, returned when `input_ids_masked` and `pixel_values` are present):
            The output of the [`FlavaMultimodalModel`].

        mim_logits (`ms.Tensor` of shape `(batch_size, num_image_patches, image_vocab_size)` or of shape `(total_masked_patches, image_vocab_size)` , *optional*, returned when `pixel_values` are present and `input_ids_masked` are not):
                The logits for MIM unimodal loss. Uses `book_masked_pos` to get masked patches. The flattened output is
                returned when `bool_masked_pos` has some of the patches masked.
        mlm_logits (`ms.Tensor` of shape `(batch_size, text_seq_length, text_vocab_size)` or of shape `(total_masked_seq_length, text_vocab_size)`, *optional*, returned when `input_ids_masked` are present and `pixel_values` are not):
                The logits for MLM unimodal loss. The flattened output is returned when `input_ids_masked` has some of
                the tokens masked.
        itm_logits (`ms.Tensor` of shape `(batch_size, 2)`, *optional*, returned when `input_ids_masked` and `pixel_values` are present):
                The logits for ITM loss. Note that ITM loss is calculated on masked pairs in FLAVA.
        mmm_image_logits (`ms.Tensor` of shape `(batch_size, num_image_patches, image_vocab_size)` or of shape`(total_masked_patches, image_vocab_size)`, *optional*, returned when `pixel_values` and `input_ids_masked` are present):
                The logits for MMM image multimodal loss. Uses `book_masked_pos` to get masked patches. The flattened
                output is returned when `bool_masked_pos` has some of the patches masked.
        mmm_text_logits (`ms.Tensor` of shape `(batch_size, text_seq_length, text_vocab_size)` or of shape `(`(total_masked_seq_length, text_vocab_size)`), *optional*, returned when `pixel_values` and `input_ids_masked` are present):
                The logits for MMM text multimodal loss. The flattened output is returned when `input_ids_masked` has
                some of the tokens masked.
        contrastive_logits_per_image (`ms.Tensor` of shape `(image_batch_size, text_batch_size)`):
        ...
        contrastive_logits_per_text (`ms.Tensor` of shape `(text_batch_size, image_batch_size)`):
    ...
    multimodal_embeddings: Optional[ms.Tensor] = None
    multimodal_output: Optional[BaseModelOutputWithPooling] = None
    image_masked_embeddings: Optional[ms.Tensor] = None
    image_masked_output: Optional[BaseModelOutputWithPooling] = None
    text_masked_embeddings: Optional[ms.Tensor] = None
    text_masked_output: Optional[BaseModelOutputWithPooling] = None
    multimodal_masked_embeddings: Optional[ms.Tensor] = None
    multimodal_masked_output: Optional[BaseModelOutputWithPooling] = None
    mim_logits: Optional[ms.Tensor] = None
    mlm_logits: Optional[ms.Tensor] = None
    itm_logits: Optional[ms.Tensor] = None
    contrastive_logits_per_image: Optional[ms.Tensor] = None
    contrastive_logits_per_text: Optional[ms.Tensor] = None
    mmm_image_logits: Optional[ms.Tensor] = None
    mmm_text_logits: Optional[ms.Tensor] = None
...
# https://github.com/rwightman/mindspore-image-models/blob/master/timm/models/image_transformer.py
...
class FlavaIntermediate(nn.Cell):
    ...
    # Copied from transformers.models.vit.modeling_vit.ViTIntermediate.construct
...
class FlavaOutput(nn.Cell):
    ...
    # Copied from transformers.models.vit.modeling_vit.ViTOutput.construct
    def construct(self, hidden_states: ms.Tensor, input_tensor: ms.Tensor) -> ms.Tensor:
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
...
class FlavaMultimodalModel(FlavaPreTrainedModel):
    ...
    def construct(
        ...
        if self.use_cls_token:
            cls_tokens = self.cls_token.expand((batch_size, -1, -1))
            hidden_states = mint.cat((cls_tokens, hidden_states), dim=1)
            seq_length += 1
        ...
        # and head_mask is converted to shape [num_hidden_layers x batch x num_heads x seq_length x seq_length]
        head_mask = self.get_head_mask(head_mask, self.config.num_hidden_layers)
        extended_attention_mask: ms.Tensor = self.get_extended_attention_mask(attention_mask, (batch_size, seq_length))
...
class FlavaModel(FlavaPreTrainedModel):
    ...
    def __init__(self, config: FlavaConfig):
        ...
        self.image_projection = mint.nn.Linear(self.image_hidden_size, self.projection_dim)
        self.text_projection = mint.nn.Linear(self.text_hidden_size, self.projection_dim)
        self.logit_scale = ms.Parameter(ms.tensor(self.config.logit_scale_init_value))

        self.image_to_mm_projection = mint.nn.Linear(self.image_hidden_size, self.mm_hidden_size)
        self.text_to_mm_projection = mint.nn.Linear(self.text_hidden_size, self.mm_hidden_size)
    ...
    def construct(
        ...
        bool_masked_pos: Optional[ms.Tensor] = None,
        position_ids: Optional[ms.Tensor] = None,
        image_attention_mask: Optional[ms.Tensor] = None,
        ...
        if image_mm_projection is not None and text_mm_projection is not None and not skip_multimodal_encoder:
            if attention_mask is not None:
                ...
                if self.multimodal_model.use_cls_token:
                    seq_len += 1
                attention_mask_image = mint.ones((batch_size, seq_len))
                attention_multimodal = mint.cat([attention_mask_image.to(attention_mask.dtype), attention_mask], dim=1)
            else:
                attention_multimodal = None
            multimodal_input = mint.cat([image_mm_projection, text_mm_projection], dim=1)
...
class FlavaImageCodebookResPath(nn.Cell):
    def __init__(self, in_size: int, out_size: int, **kwargs):
        ...
        path = OrderedDict()
        path["relu_1"] = mint.nn.ReLU()
        path["conv_1"] = mint.nn.Conv2d(in_size, hid_size, kernel_size=3, padding=1)
        path["relu_2"] = mint.nn.ReLU()
        path["conv_2"] = mint.nn.Conv2d(hid_size, hid_size, kernel_size=3, padding=1)
        path["relu_3"] = mint.nn.ReLU()
        path["conv_3"] = mint.nn.Conv2d(hid_size, hid_size, kernel_size=3, padding=1)
        path["relu_4"] = mint.nn.ReLU()
        path["conv_4"] = mint.nn.Conv2d(hid_size, out_size, kernel_size=1, padding=0)
...
class FlavaImageCodebookBlock(nn.Cell):
    def __init__(self, in_size: int, out_size: int, num_layers: int, **kwargs):
        ...
        if in_size != out_size:
            self.id_path = mint.nn.Conv2d(in_size, out_size, kernel_size=1, padding=0)
        else:
            self.id_path = mint.nn.Identity()
...
class FlavaImageCodebook(FlavaPreTrainedModel):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        output_blocks["relu"] = mint.nn.ReLU()
        output_blocks["conv"] = mint.nn.Conv2d(8 * self.hidden_size, self.vocab_size, kernel_size=1, padding=0)

        blocks = OrderedDict()
        blocks["input"] = mint.nn.Conv2d(self.input_channels, 1 * self.hidden_size, kernel_size=7, padding=3)
    ...
    def get_codebook_indices(self, pixel_values: ms.Tensor) -> ms.Tensor:
        """
        Args:
        ...
        >>> from PIL import Image
        >>> import requests
        >>> from mindone.transformers import AutoImageProcessor, FlavaImageCodebook

        >>> model = FlavaImageCodebook.from_pretrained("facebook/flava-image-codebook", revision="refs/pr/1")
        >>> image_processor = AutoImageProcessor.from_pretrained("facebook/flava-full", revision="refs/pr/6")
        ...
        return mint.argmax(z_logits, dim=1)

    def get_codebook_probs(self, pixel_values: ms.Tensor) -> ms.Tensor:
        z_logits = self.blocks(pixel_values)
        return mint.nn.Softmax(dim=1)(z_logits)
...
class FlavaGlobalContrastiveHead(nn.Cell):
    ...
    def construct(self, image_embeddings, text_embeddings, logit_scale):
        temperature = mint.exp(logit_scale)
        if not mint.distributed.is_available() or not mint.distributed.is_initialized():
            labels = mint.arange(image_embeddings.shape[0])
            ...
            if self.global_backprop_contrastive:
                ...
                image_embeddings_all = [mint.zeros_like(text_embeddings) for _ in range(world_size)]
                text_embeddings_all = [mint.zeros_like(image_embeddings) for _ in range(world_size)]
                mint.distributed.all_gather(image_embeddings_all, image_embeddings)
                mint.distributed.all_gather(text_embeddings_all, text_embeddings)

            labels = local_batch_size * mint.distributed.get_rank() + mint.arange(local_batch_size)

        image_embeddings_all = mint.cat(image_embeddings_all)
        text_embeddings_all = mint.cat(text_embeddings_all)

        logits_per_image = mint.matmul(image_embeddings, text_embeddings_all.transpose(0, 1)) * temperature
        logits_per_text = mint.matmul(text_embeddings, image_embeddings_all.transpose(0, 1)) * temperature
...
class FlavaForPreTraining(FlavaPreTrainedModel):
    ...
    def _resize_to_2d(self, x: ms.Tensor):
        if x.ndim > 2:
    ...
    def construct(
        self,
        input_ids: Optional[ms.Tensor] = None,
        input_ids_masked: Optional[ms.Tensor] = None,
        pixel_values: Optional[ms.Tensor] = None,
        codebook_pixel_values: Optional[ms.Tensor] = None,
        attention_mask: Optional[ms.Tensor] = None,
        token_type_ids: Optional[ms.Tensor] = None,
        bool_masked_pos: Optional[ms.Tensor] = None,
        position_ids: Optional[ms.Tensor] = None,
        ...
        mlm_labels: Optional[ms.Tensor] = None,
        mim_labels: Optional[ms.Tensor] = None,
        itm_labels: Optional[ms.Tensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: bool = True,
        return_dict: Optional[bool] = None,
        return_loss: Optional[bool] = None,
    ) -> Union[Tuple[ms.Tensor], FlavaForPreTrainingOutput]:
        """
        Examples:
        ```python
        >>> from PIL import Image
        >>> import requests
        >>> from mindone.transformers import FlavaForPreTraining, AutoProcessor
        ...
        >>> output = model(**inputs, return_loss=False)
        ```
        ...
        if self.mim_weight > 0 and image_masked_embeddings is not None and multimodal_masked_embeddings is None:
            ...
            if mim_labels is not None:
                ...
                if return_loss:
                    mim_loss = mint.nn.functional.cross_entropy(
        ...
        if self.mlm_weight > 0 and text_masked_embeddings is not None and multimodal_masked_embeddings is None:
            ...
            if mlm_labels is not None:
                ...
                if return_loss:
                    mlm_loss = mint.nn.functional.cross_entropy(
        ...
        if self.itm_weight > 0 and multimodal_masked_embeddings is not None:
            ...
            if itm_labels is not None:
                ...
                if return_loss:
                    itm_loss = mint.nn.functional.cross_entropy(itm_logits, itm_labels)
        ...
        if multimodal_masked_embeddings is not None and self.mmm_image_weight > 0:
            ...
            if mim_labels is not None:
                ...
                if return_loss:
                    mmm_image_loss = mint.nn.functional.cross_entropy(
        ...
        if multimodal_masked_embeddings is not None and self.mmm_text_weight > 0:
            ...
            if mlm_labels is not None:
                ...
                if return_loss:
                    mmm_text_loss = mint.nn.functional.cross_entropy(
        ...
        if image_embeddings is not None and text_embeddings is not None and self.global_contrastive_weight > 0:
            ...
            text_embedding = mint.nn.functional.normalize(text_embedding, dim=-1)

            image_embedding = self.flava.image_projection(image_embeddings[:, 0, :])
            image_embedding = mint.nn.functional.normalize(image_embedding, dim=-1)
            ...
            if return_loss:
                gc_loss_image = mint.nn.functional.cross_entropy(logits_per_image, gc_labels)
                gc_loss_text = mint.nn.functional.cross_entropy(logits_per_text, gc_labels)
