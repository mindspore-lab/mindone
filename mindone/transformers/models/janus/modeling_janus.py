...
class JanusVQVAEOutput(ModelOutput):
    ...
    embedding_loss (`mindspore.Tensor`):
        Embedding loss.
    """

    decoded_pixel_values: Optional[ms.Tensor] = None
    embedding_loss: ms.Tensor = None
...
class JanusVisionEmbeddings(nn.Cell):
    ...
    def construct(self, pixel_values: ms.Tensor, interpolate_pos_encoding: bool = False) -> ms.Tensor:
        ...
        patch_embeds = self.patch_embedding(pixel_values.astype(target_dtype))  # shape = [*, width, grid, grid]
...
def eager_attention_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    attn_weights = mint.matmul(query, mint.transpose(key_states, 2, 3)) * scaling
    if attention_mask is not None:
        causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
        attn_weights = attn_weights + causal_mask

    attn_weights = mint.nn.functional.softmax(attn_weights, dim=-1, dtype=ms.float32).astype(query.dtype)
...
class JanusVisionAttention(nn.Cell):
    ...
    def __init__(self, config: JanusVisionConfig):
        ...
        self.q_norm = mint.nn.LayerNorm(self.embed_dim) if qk_norm else mint.nn.Identity()
        self.k_norm = mint.nn.LayerNorm(self.embed_dim) if qk_norm else mint.nn.Identity()
...
class JanusVisionMLP(nn.Cell):
    def __init__(self, config: JanusVisionConfig):
        super().__init__()
        self.config = config
        self.intermediate_size = int(config.hidden_size * config.mlp_ratio)
        self.activation_fn = ACT2FN[config.hidden_act]  # Gelu act
        self.fc1 = mint.nn.Linear(config.hidden_size, self.intermediate_size)
        self.fc2 = mint.nn.Linear(self.intermediate_size, config.hidden_size)
        self.dropout1 = mint.nn.Dropout(config.hidden_dropout_rate)
        self.dropout2 = mint.nn.Dropout(config.hidden_dropout_rate)
...
class JanusVisionEncoder(nn.Cell):
    """
    ...
    def __init__(self, config: JanusVisionConfig):
        super().__init__()
        self.config = config
        self.layers = nn.CellList([JanusVisionEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class JanusVisionAlignerMLP(nn.Cell):
    def __init__(self, config: JanusVisionConfig):
        super().__init__()

        self.fc1 = mint.nn.Linear(config.hidden_size, config.projection_dim)
        self.hidden_layers = nn.CellList(
            [mint.nn.Linear(config.projection_dim, config.projection_dim) for _ in range(1, config.depth)]
        )
...
class JanusVQVAEVectorQuantizer(nn.Cell):
    """
    ...
    def construct(self, hidden_state: ms.Tensor):
        ...
        distances = (
            ...
            - 2 * mint.einsum("bd,dn->bn", hidden_state_flattened, mint.transpose(self.embedding.weight, 0, 1))
        )
        ...
        loss = mint.mean((hidden_state_quant.clone() - hidden_state) ** 2) + self.beta * mint.mean(
            (hidden_state_quant - hidden_state.clone()) ** 2
        )

        # preserve gradients
        hidden_state_quant = hidden_state + (hidden_state_quant - hidden_state).clone()
    ...
    def get_codebook_entry(self, image_tokens: ms.Tensor) -> ms.Tensor:
        ...
        hidden_state_quant = mint.nn.functional.normalize(hidden_state_quant, p=2, dim=-1)
...
class JanusVQVAEResnetBlock(nn.Cell):
    def __init__(
        self,
        config,
        in_channels,
...
class JanusVQVAEAttnBlock(nn.Cell):
    ...
    def construct(self, hidden_states):
        ...
        attn_weights = mint.nn.functional.softmax(attn_weights, dim=2)
...
class JanusVQVAEConvDownsample(nn.Cell):
    def __init__(self, in_channels):
        super().__init__()
...
class JanusVQVAEConvUpsample(nn.Cell):
    def __init__(self, in_channels):
        super().__init__()
...
class JanusVQVAEMidBlock(nn.Cell):
...
class JanusVQVAEEncoder(nn.Cell):
...
class JanusVQVAEDecoder(nn.Cell):
    def __init__(self, config):
        super().__init__()
        ...
        # z to block_in
        self.conv_in = mint.nn.Conv2d(latent_channels, block_in, kernel_size=3, stride=1, padding=1)
    ...
    def construct(self, hidden_state: ms.Tensor) -> ms.Tensor:
        ...
        hidden_state *= mint.sigmoid(hidden_state)
...
class JanusVQVAE(JanusPreTrainedModel):
    ...
    def __init__(self, config: JanusVQVAEConfig):
        ...
        self.set_train(False)  # Janus's VQ model is frozen
    ...
    def decode(self, image_tokens: ms.Tensor) -> ms.Tensor:
        """
        Decodes quantized token IDs into pixel values.
        Args:
            image_tokens (mindspore.Tensor): Batch of token IDs.
        Returns:
            pixel_values (`mindspore.Tensor` of shape `(batch_size, num_channels, image_size, image_size)`):
...
class JanusVQVAEAlignerMLP(nn.Cell):
    def __init__(self, config: JanusVQVAEConfig):
        super().__init__()

        self.fc1 = mint.nn.Linear(config.embed_dim, config.projection_dim)
        self.hidden_layers = nn.CellList(
            [mint.nn.Linear(config.projection_dim, config.projection_dim) for _ in range(1, config.num_hidden_layers)]
        )
        self.activation_fn = ACT2FN[config.hidden_act]
...
class JanusVQVAEHead(nn.Cell):
    """Head used for sampling tokens in image generation, replacing the usual lm head."""

    def __init__(self, config: JanusVQVAEConfig):
        super().__init__()
        self.proj_out = mint.nn.Linear(config.image_token_embed_dim, config.projection_dim)
        self.activation_fn = ACT2FN[config.hidden_act]
        self.vision_head = mint.nn.Linear(config.projection_dim, config.num_embeddings)
...
class JanusForConditionalGeneration(JanusPreTrainedModel, GenerationMixin):
    ...
    def prepare_embeddings_for_image_generation(self, inputs: ms.Tensor) -> ms.Tensor:
    ...
    def decode_image_tokens(self, image_tokens: ms.Tensor):
        """
        Decodes generated image tokens from language model to continuous pixel values
        with VQGAN module via upsampling.
        Args:
            image_tokens (`mindspore.Tensor` of shape `(batch_size, num_of_tokens)`):
    ...
    def generate(
        ...
        **kwargs,
        ...
        unconditional_tokens = mint.where(
            mask,
            mint.full_like(input_tokens[batch_size:, :], generation_config.pad_token_id),
            input_tokens[batch_size:, :],
        )
        input_tokens = mint.cat([input_tokens[:batch_size], unconditional_tokens], dim=0)

        inputs_embeds = self.get_input_embeddings()(input_tokens)

        model_kwargs = self._get_initial_cache_position(seq_len, model_kwargs)
        ...
        generated_tokens = mint.zeros((batch_size, num_image_tokens), dtype=dtype)
        ...
        for i in range(num_image_tokens):
            ...
            # Use input_tokens for logits processor
            next_token_scores = logits_processor(input_tokens, scores)

            # Sample next token.
            if generation_config.do_sample:
                probs = mint.softmax(next_token_scores, dim=-1)
                next_token = mint.multinomial(probs, num_samples=1).squeeze(-1)
            else:
                next_token = mint.argmax(next_token_scores, dim=-1)

            generated_tokens[:, i] = next_token

            # Prepare embeddings for the next step.
            next_token = mint.cat([next_token, next_token])
        ...
        if return_dict_in_generate:
            if output_scores:
                raw_scores += (scores,)
            if output_logits:
                raw_logits += (hidden_state.astype(ms.float32),)
            if output_attentions:
