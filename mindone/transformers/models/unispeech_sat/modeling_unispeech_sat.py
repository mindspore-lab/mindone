...
class UniSpeechSatForPreTrainingOutput(ModelOutput):
    ...
    Args:
        loss (*optional*, returned when model is in train mode, `ms.Tensor` of shape `(1,)`):
...
class UniSpeechSatGumbelVectorQuantizer(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.codevectors = Parameter(
            mint.empty((1, self.num_groups * self.num_vars, config.codevector_dim // self.num_groups))
        )
        self.weight_proj = mint.nn.Linear(config.hidden_size, self.num_groups * self.num_vars)
    ...
    def construct(self, hidden_states):
        ...
        if self.training:
            ...
            codevector_probs = ops.gumbel_softmax(hidden_states.float(), tau=self.temperature, hard=True).type_as(
                hidden_states
            )
...
class UniSpeechSatPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, UniSpeechSatGumbelVectorQuantizer):
            ...
            module.codevectors.set_data(initializer(Uniform(), module.codevectors.shape, module.codevectors.dtype))
            ...
            module.conv.weight_norm_cell.weight.set_data(
                initializer(
                    Normal(
                        ...
                        sigma=2
                        * math.sqrt(
                            1 / (module.conv.weight_norm_cell.kernel_size[0] * module.conv.weight_norm_cell.in_channels)
                        ),
                    ),
            ...
            module.projection.weight.set_data(
                initializer(Uniform(k), module.projection.weight.shape, module.projection.weight.dtype)
            )
            module.projection.bias.set_data(
                initializer(Uniform(k), module.projection.bias.shape, module.projection.bias.dtype)
            )
...
class UniSpeechSatForPreTraining(UniSpeechSatPreTrainedModel):
    def __init__(self, config: UniSpeechSatConfig):
        ...
        self.speaker_proj = mint.nn.Linear(config.hidden_size, config.codevector_dim)
        self.label_embeddings_concat = Parameter(mint.empty((config.num_clusters, config.codevector_dim)))
        self.label_embeddings_concat.data.zero_()

        self.layer_norm_for_extract = mint.nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class UniSpeechSatForCTC(UniSpeechSatPreTrainedModel):
    ...
    def construct(
        ...
        >>> from mindone.transformers.models.unispeech_sat import UniSpeechSatForCTC
...
class UniSpeechSatForSequenceClassification(UniSpeechSatPreTrainedModel):
    ...
    def construct(
        ...
        >>> from mindone.transformers.models.unispeech_sat import UniSpeechSatForSequenceClassification
        >>> from datasets import load_dataset
        >>> import mindspore as ms
...
# Copied from transformers.models.wav2vec2.modeling_wav2vec2.Wav2Vec2ForAudioFrameClassification
# with Wav2Vec2->UniSpeechSat, wav2vec2->unispeech_sat, WAV_2_VEC_2->UNISPEECH_SAT
class UniSpeechSatForAudioFrameClassification(UniSpeechSatPreTrainedModel):
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoFeatureExtractor
        >>> from mindone.transformers.models.unispeech_sat import UniSpeechSatForAudioFrameClassification
        >>> from datasets import load_dataset
        >>> import mindspore as ms
        ...
        >>> probabilities = ms.mint.sigmoid(logits[0])
...
class UniSpeechSatForXVector(UniSpeechSatPreTrainedModel):
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoFeatureExtractor
        >>> from mindone.transformers.models.unispeech_sat import UniSpeechSatForXVector
        >>> from datasets import load_dataset
        >>> import mindspore as ms
        ...
        ...     [d["array"] for d in dataset[:2]["audio"]], sampling_rate=sampling_rate, return_tensors="np", padding=True
        ... )
        >>> inputs = {k: ms.tensor(v) for k, v in inputs.items()}

        >>> embeddings = model(**inputs).embeddings

        >>> embeddings = ms.mint.nn.functional.normalize(embeddings, dim=-1)

        >>> # the resulting embeddings can be used for cosine similarity-based retrieval
        >>> similarity = ms.ops.cosine_similarity(embeddings[0], embeddings[1], dim=-1)
