...
def _mask_to_rle(input_mask: "ms.Tensor"):
    """
...
def _batched_mask_to_box(masks: "ms.Tensor"):
    ...
    in_height_coords = in_height * mint.arange(height, device=in_height.device)[None, :]
    ...
    in_width_coords = in_width * mint.arange(width, device=in_width.device)[None, :]
...
def _is_box_near_crop_edge(boxes, crop_box, orig_box, atol=20.0):
    ...
    offset = ms.tensor([[left, top, left, top]], device=boxes.device)
...
def _generate_crop_boxes(
    ...
    points_per_crop = mint.stack(point_grid_per_crop)
    points_per_crop = points_per_crop.unsqueeze(0).permute(0, 2, 1, 3)
    cropped_images = mint.stack(cropped_images)

    input_labels = mint.ones_like(points_per_crop[:, :, :, 0], dtype=ms.int64)
...
def _build_point_grid(n_per_side: int) -> ms.Tensor:
    """Generates a 2D grid of points evenly spaced in [0,1]x[0,1]."""
    offset = 1 / (2 * n_per_side)
    points_one_side = mint.linspace(offset, 1 - offset, n_per_side)
    points_x = mint.tile(points_one_side[None, :], (n_per_side, 1))
    points_y = mint.tile(points_one_side[:, None], (1, n_per_side))
    points = mint.stack([points_x, points_y], dim=-1).reshape(-1, 2)
    return points
...
def _generate_crop_images(
    ...
    for i, crop_box in enumerate(crop_boxes):
        ...
        points_scale = ms.tensor(cropped_im_size).flip(dims=(0,)).unsqueeze(0)
...
def _normalize_coordinates(
    target_size: int, coords: ms.Tensor, original_size: tuple[int, int], is_bounding_box=False
) -> ms.Tensor:
    """
...
def _rle_to_mask(rle: dict[str, Any]) -> ms.Tensor:
    """Compute a binary mask from an uncompressed RLE."""
    height, width = rle["size"]
    mask = mint.empty(height * width, dtype=bool)
    idx = 0
    parity = False
...
class Sam2ImageProcessorFast(BaseImageProcessorFast):
    ...
    def _preprocess_image_like_inputs(
        ...
        **kwargs: Unpack[Sam2FastImageProcessorKwargs],
        ...
        if segmentation_maps is not None:
            ...
            data["labels"] = processed_segmentation_maps.squeeze(1).to(ms.int64)
    ...
    def filter_masks(
        ...
        keep_mask = mint.ones(batch_size, dtype=ms.bool, device=masks.device)
    ...
    def post_process_masks(
        ...
        Args:
            masks (`Union[ms.Tensor, List[ms.Tensor], np.ndarray, List[np.ndarray]]`):
    ...
    def _apply_non_overlapping_constraints(self, pred_masks: ms.Tensor) -> ms.Tensor:
        """
        ...
        batch_obj_inds = mint.arange(batch_size, device=device)[:, None, None, None]
