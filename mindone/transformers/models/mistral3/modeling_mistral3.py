...
class Mistral3PatchMerger(nn.Cell):
    """
    ...
    def __init__(self, config: Mistral3Config):
        ...
        self.merging_layer = mint.nn.Linear(hidden_size * self.spatial_merge_size**2, hidden_size, bias=False)

    def construct(self, image_features: ms.Tensor, image_sizes: ms.Tensor) -> ms.Tensor:
        image_sizes = [
        ...
        permuted_tensor = []
        for image_index, image_tokens in enumerate(image_features.split(tokens_per_image[0].item())):
            # Reshape image_tokens into a 2D grid
            h, w = image_sizes[image_index]
            image_grid = image_tokens.view(h.item(), w.item(), d).permute(2, 0, 1).unsqueeze(0)
            grid = mint.nn.functional.unfold(
                image_grid, kernel_size=self.spatial_merge_size, stride=self.spatial_merge_size
            )
            grid = grid.view(d * self.spatial_merge_size**2, -1).t()
            permuted_tensor.append(grid)

        image_features = mint.cat(permuted_tensor, dim=0)
        image_features = self.merging_layer(image_features)
        return image_features


class Mistral3MultiModalProjector(nn.Cell):
    def __init__(self, config: Mistral3Config):
        super().__init__()
    ...
    def construct(self, image_features: ms.Tensor, image_sizes: ms.Tensor):
        ...
        **kwargs,
        ...
        Args:
            ...
            vision_feature_layer (`Union[int, list[int]]`, *optional*):
                ...
                vision features.
            image_sizes (`ms.Tensor`, *optional*):
        ...
        if isinstance(vision_feature_layer, int):
            selected_image_feature = image_outputs[1][vision_feature_layer]
        else:
            hs_pool = [image_outputs[1][layer_idx] for layer_idx in vision_feature_layer]
        ...
        image_features = mint.split(image_features.squeeze(0), [tensor.item() for tensor in split_sizes])
        return image_features
    ...
    def construct(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if pixel_values is not None:
            ...
            image_features = mint.cat(image_features, dim=0).to(inputs_embeds.dtype)
...
class Mistral3ForConditionalGeneration(Mistral3PreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> processor = AutoProcessor.from_pretrained("mistralai/Mistral-Small-3.1-24B-Instruct-2503", use_fast=False)
