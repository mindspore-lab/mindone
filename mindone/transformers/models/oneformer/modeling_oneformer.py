...
def sample_point(input_features: ms.Tensor, point_coordinates: ms.Tensor, add_dim=False, **kwargs) -> ms.Tensor:
    ...
    point_features = mint.nn.functional.grid_sample(
        input_features.to(ms.float32), (2.0 * point_coordinates - 1.0).to(ms.float32), **kwargs
...
class OneFormerHungarianMatcher(ms.nn.Cell):
    def __init__(
...
class OneFormerLoss(ms.nn.Cell):
    def __init__(
        self,
        num_classes: int,
        ...
        empty_weight = mint.ones(self.num_classes + 1)
        ...
        if self.contrastive_temperature is not None:
            self.logit_scale = ms.Parameter(ms.Tensor(np.log(1 / contrastive_temperature)))
    ...
    def loss_contrastive(self, contrastive_queries_logits: Tensor, text_queries: Tensor):
        ...
        image_queries = mint.nn.functional.normalize(image_queries.flatten(1), dim=-1)
        text_queries = mint.nn.functional.normalize(text_queries.flatten(1), dim=-1)

        logit_scale = mint.clamp(self.logit_scale.exp(), max=100)

        logits_per_text = mint.matmul(text_queries, image_queries.t()) * logit_scale
...
class OneFormerTransformerDecoderOutput(BaseModelOutput):
    ...
    contrastive_logits: Optional[ms.Tensor] = None
    prediction_masks: ms.Tensor = None
    prediction_class: ms.Tensor = None
    auxiliary_predictions: Optional[Tuple[Dict[str, ms.Tensor]]] = None
...
class OneFormerPixelLevelModuleOutput(ModelOutput):
    ...
    encoder_features: List[ms.Tensor] = None
    decoder_features: List[ms.Tensor] = None
    decoder_last_feature: ms.Tensor = None
...
class OneFormerModelOutput(ModelOutput):
    ...
    transformer_decoder_object_queries: ms.Tensor = None
    transformer_decoder_contrastive_queries: Optional[ms.Tensor] = None
    transformer_decoder_mask_predictions: ms.Tensor = None
    transformer_decoder_class_predictions: ms.Tensor = None
    transformer_decoder_auxiliary_predictions: Optional[Tuple[Dict[str, ms.Tensor]]] = None
    text_queries: Optional[ms.Tensor] = None
    task_token: ms.Tensor = None
    attentions: Optional[Tuple[ms.Tensor]] = None
...
class OneFormerForUniversalSegmentationOutput(ModelOutput):
    ...
    class_queries_logits: ms.Tensor = None
    masks_queries_logits: ms.Tensor = None
    auxiliary_predictions: List[Dict[str, ms.Tensor]] = None
    encoder_hidden_states: Optional[Tuple[ms.Tensor]] = None
    pixel_decoder_hidden_states: Optional[List[ms.Tensor]] = None
    transformer_decoder_hidden_states: Optional[ms.Tensor] = None
    transformer_decoder_object_queries: ms.Tensor = None
    transformer_decoder_contrastive_queries: Optional[ms.Tensor] = None
    transformer_decoder_mask_predictions: ms.Tensor = None
    transformer_decoder_class_predictions: ms.Tensor = None
    transformer_decoder_auxiliary_predictions: Optional[List[Dict[str, ms.Tensor]]] = None
    text_queries: Optional[ms.Tensor] = None
    task_token: ms.Tensor = None
...
class OneFormerPixelDecoderFrozenBatchNorm2d(ms.nn.Cell):
    """
...
class OneFormerPixelDecoderEncoderMultiscaleDeformableAttention(ms.nn.Cell):
    """
...
class OneFormerPixelDecoderEncoderLayer(ms.nn.Cell):
    ...
    def construct(
        ...
        hidden_states = mint.nn.functional.dropout(hidden_states, p=self.dropout, training=self.is_training)
        hidden_states = residual + hidden_states
        hidden_states = self.self_attn_layer_norm(hidden_states)

        residual = hidden_states
        hidden_states = self.activation_fn(self.fc1(hidden_states))
        hidden_states = mint.nn.functional.dropout(hidden_states, p=self.activation_dropout, training=self.is_training)
...
class OneFormerPixelDecoderEncoderOnly(ms.nn.Cell):
    """
...
class OneFormerPixelDecoder(ms.nn.Cell):
    def __init__(self, config: OneFormerConfig, feature_channels):
            ...
            for in_channels in transformer_in_channels[::-1]:
                input_projections_list.append(
                    ms.nn.SequentialCell(
                        mint.nn.Conv2d(in_channels, config.conv_dim, kernel_size=1),
                        mint.nn.GroupNorm(32, config.conv_dim),
                    )
                )
            self.input_projections = ms.nn.CellList(input_projections_list)
        else:
            self.input_projections = ms.nn.CellList(
                [
                    ms.nn.SequentialCell(
                        mint.nn.Conv2d(transformer_in_channels[-1], config.conv_dim, kernel_size=1),
                        mint.nn.GroupNorm(32, config.conv_dim),
                    )
                ]
            )
        ...
        self.mask_projection = mint.nn.Conv2d(
            config.conv_dim,
            config.mask_dim,
            kernel_size=1,
            stride=1,
        ...
        for idx, in_channels in enumerate(self.feature_channels[: self.num_fpn_levels]):
            lateral_conv = ms.nn.SequentialCell(
                mint.nn.Conv2d(
                    in_channels,
                    config.conv_dim,
                    kernel_size=1,
            ...
            output_conv = ms.nn.SequentialCell(
                mint.nn.Conv2d(
                    config.conv_dim,
                    config.conv_dim,
                    kernel_size=3,
    ...
    def construct(
        ...
        masks = [mint.zeros((x.shape[0], x.shape[2], x.shape[3]), dtype=ms.bool_) for x in sources]
        ...
        valid_ratios = mint.stack([self.get_valid_ratio(m, dtype=source_flatten.dtype) for m in masks], 1)
...
class OneFormerPixelLevelModule(ms.nn.Cell):
    def __init__(self, config: OneFormerConfig):
        """
...
class OneFormerAttention(ms.nn.Cell):
    """
...
class OneFormerTransformerDecoderSelfAttentionLayer(ms.nn.Cell):
    def __init__(
...
class OneFormerTransformerDecoderCrossAttentionLayer(ms.nn.Cell):
    def __init__(
        self, embed_dim, num_heads, dropout=0.0, activation="relu", normalize_before=False, layer_norm_eps=1e-05
    ):
        super().__init__()
...
class OneFormerTransformerDecoderFFNLayer(ms.nn.Cell):
    def __init__(
        self,
        d_model,
        ...
        self.dropout = mint.nn.Dropout(dropout)
        self.linear2 = mint.nn.Linear(dim_feedforward, d_model)

        self.norm = mint.nn.LayerNorm(d_model, eps=layer_norm_eps)
...
class OneFormerMLPPredictionHead(ms.nn.Cell):
...
class OneFormerTransformerDecoderLayer(ms.nn.Cell):
    def __init__(self, config: OneFormerConfig):
        super().__init__()
    ...
    def construct(
        ...
        index: int,
        output: ms.Tensor,
        multi_stage_features: List[ms.Tensor],
        multi_stage_positional_embeddings: List[ms.Tensor],
        attention_mask: Optional[ms.Tensor] = None,
        query_embeddings: Optional[ms.Tensor] = None,
        ...
        attention_mask[mint.where(attention_mask.sum(-1) == attention_mask.shape[-1])] = False
...
class OneFormerTransformerDecoderQueryTransformerDecoder(ms.nn.Cell):
    ...
    def construct(
        ...
        if self.return_intermediate:
            return mint.stack(intermediate)

        return output.unsqueeze(0)


class OneFormerTransformerDecoderQueryTransformerDecoderLayer(ms.nn.Cell):
    def __init__(
        self,
        d_model,
        nhead,
        ...
        self.norm1 = mint.nn.LayerNorm(d_model, eps=layer_norm_eps)
        self.norm2 = mint.nn.LayerNorm(d_model, eps=layer_norm_eps)
        self.norm3 = mint.nn.LayerNorm(d_model, eps=layer_norm_eps)
        self.dropout1 = mint.nn.Dropout(dropout)
        self.dropout2 = mint.nn.Dropout(dropout)
        self.dropout3 = mint.nn.Dropout(dropout)
...
class OneFormerTransformerDecoderQueryTransformer(ms.nn.Cell):
    def __init__(
        self,
        d_model=512,
        nhead=8,
        ...
        decoder_norm = mint.nn.LayerNorm(d_model, eps=layer_norm_eps)
    ...
    def construct(self, src, mask, query_embed, pos_embed, task_token=None):
        ...
        if task_token is None:
            queries = mint.zeros_like(query_embed)
        else:
...
class OneFormerTransformerDecoder(ms.nn.Cell):
    """
    Transformer decoder
    """
    ...
    def __init__(self, in_channels: int, config: OneFormerConfig):
        ...
        self.decoder_norm = mint.nn.LayerNorm(config.hidden_dim, eps=config.layer_norm_eps)
        ...
        self.query_input_projection = mint.nn.Conv2d(in_channels, config.hidden_dim, kernel_size=1)

        self.class_embed = mint.nn.Linear(config.hidden_dim, config.num_labels + 1)
    ...
    def construct(
        ...
        queries = mint.cat([object_queries, task_token], dim=0)
...
class OneFormerTransformerModule(ms.nn.Cell):
    """
    ...
    def __init__(self, in_features: int, config: OneFormerConfig):
        ...
        for _ in range(self.num_feature_levels):
            if in_features != hidden_dim or config.enforce_input_proj:
                ...
                self.input_projections.append(ms.nn.SequentialCell())
...
class OneFormerSinePositionEmbedding(ms.nn.Cell):
    """
...
class PredictionBlock(ms.nn.Cell):
    def __init__(self, in_dim: int, out_dim: int, activation: ms.nn.Cell) -> None:
        super().__init__()
...
class OneFormerTextMapperAttention(ms.nn.Cell):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.0, proj_drop=0.0):
        ...
        self.q_proj = mint.nn.Linear(dim, dim, bias=qkv_bias)
        self.k_proj = mint.nn.Linear(dim, dim, bias=qkv_bias)
        self.v_proj = mint.nn.Linear(dim, dim, bias=qkv_bias)
    ...
    def construct(self, q, k, v):
        ...
        attn = mint.einsum("bnkc,bmkc->bknm", q, k) * self.scale

        attn = attn.softmax(dim=-1)

        output = mint.einsum("bknm,bmkc->bnkc", attn, v).reshape(batch_size, q_sequence_length, num_channels)
...
class OneFormerTextTransformerDecoderLayer(ms.nn.Cell):
    def __init__(
        self,
        d_model,
        nhead,
...
class OneFormerTextContextDecoder(ms.nn.Cell):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        self.memory_proj = ms.nn.SequentialCell(
            mint.nn.LayerNorm(visual_dim, eps=layer_norm_eps),
            mint.nn.Linear(visual_dim, transformer_width),
            mint.nn.LayerNorm(transformer_width, eps=layer_norm_eps),
        )

        self.text_proj = ms.nn.SequentialCell(
            mint.nn.LayerNorm(visual_dim, eps=layer_norm_eps),
            mint.nn.Linear(visual_dim, transformer_width),
        )

        self.decoder = ms.nn.CellList(
            [
        ...
        self.out_proj = ms.nn.SequentialCell(
            mint.nn.LayerNorm(transformer_width, eps=layer_norm_eps), mint.nn.Linear(transformer_width, visual_dim)
        )
...
class OneFormerTextMLP(ms.nn.Cell):
    def __init__(
        self,
...
class OneFormerTextTransformer(ms.nn.Cell):
    def __init__(
        self,
        width: int,
        layers: int,
...
class OneFormerTextEncoder(ms.nn.Cell):
    def __init__(
        self,
        context_length: int,
        width: int,
        ...
        self.positional_embedding = ms.Parameter(mint.empty(self.context_length, width))
        self.ln_final = mint.nn.LayerNorm(width, eps=layer_norm_eps)
        self.token_embedding = mint.nn.Embedding(vocab_size, width)
    ...
    def construct(self, text):
        ...
        hidden_state = hidden_state[mint.arange(hidden_state.shape[0]), text.argmax(dim=-1)]

        return hidden_state


class OneFormerTextMapper(ms.nn.Cell):
    def __init__(self, config: OneFormerConfig):
        super().__init__()
        ...
        if config.text_encoder_n_ctx > 0:
            self.prompt_ctx = mint.nn.Embedding(
    ...
    def encode_text(self, text):
        ...
        if squeeze_dim:
            ...
            if self.prompt_ctx is not None:
                ...
                text_queries = mint.cat([text_queries, text_queries_ctx], dim=1)

        return text_queries


class OneFormerTaskModel(ms.nn.Cell):
    def __init__(self, config: OneFormerConfig):
        super().__init__()
...
ONEFORMER_INPUTS_DOCSTRING = r"""
    Args:
        ...
        task_inputs (`mindspore.Tensor` of shape `(batch_size, sequence_length)`):
...
class OneFormerPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: ms.nn.Cell):
        xavier_std = self.config.init_xavier_std
        std = self.config.init_std
        if isinstance(module, OneFormerTransformerModule):
            if module.input_projections is not None:
                for input_projection in module.input_projections:
                    if not isinstance(input_projection, ms.nn.SequentialCell):
                        xavier_uniform_(input_projection.weight, gain=xavier_std)
                        constant_(input_projection.bias, 0)
        elif isinstance(module, OneFormerTransformerDecoder):
            xavier_uniform_(module.query_input_projection.weight, gain=xavier_std)
            constant_(module.query_input_projection.bias, 0)
            ...
            thetas = mint.arange(module.n_heads, dtype=ms.int64).float() * (2.0 * math.pi / module.n_heads)
            ...
            for submodule in module.cells():
                ...
                elif isinstance(submodule, mint.nn.LayerNorm):
                    constant_(submodule.bias, 0)
                    constant_(submodule.weight, 1.0)
            ...
            for layer in module.layers:
                normal_(layer.self_attn.in_proj_weight, std=attn_std)
                normal_(layer.self_attn.out_proj.weight, std=proj_std)
                normal_(layer.mlp.fc1.weight, std=fc_std)
                normal_(layer.mlp.fc2.weight, std=proj_std)
        elif isinstance(module, OneFormerTextEncoder):
            normal_(module.token_embedding.weight, std=0.02)
            normal_(module.positional_embedding, std=0.01)
...
class OneFormerModel(OneFormerPreTrainedModel):
    ...
    def construct(
        ...
        >>> import requests
        >>> from mindone.transformers import OneFormerProcessor, OneFormerModel
...
class OneFormerForUniversalSegmentation(OneFormerPreTrainedModel):
    ...
    def construct(
        ...
        ```python
        >>> from mindone.transformers import OneFormerProcessor, OneFormerForUniversalSegmentation
        >>> from PIL import Image
        >>> import requests
        >>> import mindspore as ms
