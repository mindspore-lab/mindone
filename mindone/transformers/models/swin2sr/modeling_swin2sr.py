...
from ...mindspore_adapter import LeakyReLU, ReLU
...
class Swin2SRPatchEmbeddings(nn.Cell):
    def __init__(self, config, normalize_patches=True):
        ...
        self.projection = mint.nn.Conv2d(num_channels, config.embed_dim, kernel_size=patch_size, stride=patch_size)
        self.layernorm = mint.nn.LayerNorm(config.embed_dim) if normalize_patches else None

    def construct(self, embeddings: Optional[ms.Tensor]) -> Tuple[ms.Tensor, Tuple[int]]:
...
class Swin2SRSelfAttention(nn.Cell):
    def __init__(self, config, dim, num_heads, window_size, pretrained_window_size=[0, 0]):
        ...
        relative_coords_table = (
            mint.stack(mint.meshgrid(relative_coords_h, relative_coords_w, indexing="ij"))
            .permute(1, 2, 0)
            .contiguous()
            .unsqueeze(0)
        ...
        self.relative_coords_table = ms.Parameter(relative_coords_table, name="relative_coords_table")
...
class Swin2SRLayer(nn.Cell):
    def __init__(
        ...
        self.drop_path = Swin2SRDropPath(drop_path_rate) if drop_path_rate > 0.0 else mint.nn.Identity()
...
class Swin2SRStage(nn.Cell):
    ...
    def __init__(self, config, dim, input_resolution, depth, num_heads, drop_path, pretrained_window_size=0):
        ...
        if config.resi_connection == "1conv":
            self.conv = mint.nn.Conv2d(dim, dim, 3, 1, 1)
        elif config.resi_connection == "3conv":
            # to save parameters and memory
            self.conv = nn.SequentialCell(
                mint.nn.Conv2d(dim, dim // 4, 3, 1, 1),
                LeakyReLU(negative_slope=0.2, inplace=True),
                mint.nn.Conv2d(dim // 4, dim // 4, 1, 1, 0),
                LeakyReLU(negative_slope=0.2, inplace=True),
                mint.nn.Conv2d(dim // 4, dim, 3, 1, 1),
            )
    ...
    def construct(
        ...
        # `ms.jit` needs to define the variable `layer_outputs`
        layer_outputs = None
        height, width = input_dimensions
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (mint.nn.Linear, mint.nn.Conv2d)):
            module.weight.set_data(
                initializer(
                    TruncatedNormal(sigma=self.config.initializer_range), module.weight.shape, module.weight.dtype
                ),
            )
...
class Swin2SRModel(Swin2SRPreTrainedModel):
    def __init__(self, config):
        ...
        if config.num_channels == 3 and config.num_channels_out == 3:
            mean = ms.tensor([0.4488, 0.4371, 0.4040]).view(1, 3, 1, 1)
        else:
            mean = mint.zeros((1, 1, 1, 1))
        # self.register_buffer("mean", mean, persistent=False)
        self.mean = ms.Parameter(mean, name="mean")

        self.img_range = config.img_range

        self.first_convolution = mint.nn.Conv2d(config.num_channels, config.embed_dim, 3, 1, 1)
        ...
        self.layernorm = mint.nn.LayerNorm(config.embed_dim, eps=config.layer_norm_eps)
        self.patch_unembed = Swin2SRPatchUnEmbeddings(config)
        self.conv_after_body = mint.nn.Conv2d(config.embed_dim, config.embed_dim, 3, 1, 1)
    ...
    def pad_and_normalize(self, pixel_values):
        ...
        pixel_values = mint.nn.functional.pad(pixel_values, (0, modulo_pad_width, 0, modulo_pad_height), "reflect")
    ...
    def __init__(self, scale, num_features):
        ...
        if (scale & (scale - 1)) == 0:
            ...
            for i in range(int(math.log(scale, 2))):
                self.insert_child_to_cell(f"convolution_{i}", mint.nn.Conv2d(num_features, 4 * num_features, 3, 1, 1))
                self.insert_child_to_cell(f"pixelshuffle_{i}", nn.PixelShuffle(2))
        elif scale == 3:
            self.convolution = mint.nn.Conv2d(num_features, 9 * num_features, 3, 1, 1)
...
class UpsampleOneStep(nn.Cell):
    ...
    def __init__(self, scale, in_channels, out_channels):
        ...
        self.conv = mint.nn.Conv2d(in_channels, (scale**2) * out_channels, 3, 1, 1)
...
class PixelShuffleUpsampler(nn.Cell):
    def __init__(self, config, num_features):
        super().__init__()
        self.conv_before_upsample = mint.nn.Conv2d(config.embed_dim, num_features, 3, 1, 1)
        self.activation = LeakyReLU(inplace=True)
        self.upsample = Upsample(config.upscale, num_features)
        self.final_convolution = mint.nn.Conv2d(num_features, config.num_channels_out, 3, 1, 1)
...
class NearestConvUpsampler(nn.Cell):
    def __init__(self, config, num_features):
        ...
        self.conv_before_upsample = mint.nn.Conv2d(config.embed_dim, num_features, 3, 1, 1)
        self.activation = LeakyReLU(inplace=True)
        self.conv_up1 = mint.nn.Conv2d(num_features, num_features, 3, 1, 1)
        self.conv_up2 = mint.nn.Conv2d(num_features, num_features, 3, 1, 1)
        self.conv_hr = mint.nn.Conv2d(num_features, num_features, 3, 1, 1)
        self.final_convolution = mint.nn.Conv2d(num_features, config.num_channels_out, 3, 1, 1)
    ...
    def construct(self, sequence_output):
        ...
        sequence_output = self.lrelu(
            self.conv_up1(mint.nn.functional.interpolate(sequence_output, scale_factor=2, mode="nearest"))
        )
        sequence_output = self.lrelu(
            self.conv_up2(mint.nn.functional.interpolate(sequence_output, scale_factor=2, mode="nearest"))
        )
...
class PixelShuffleAuxUpsampler(nn.Cell):
    def __init__(self, config, num_features):
        ...
        self.conv_bicubic = mint.nn.Conv2d(config.num_channels, num_features, 3, 1, 1)
        self.conv_before_upsample = mint.nn.Conv2d(config.embed_dim, num_features, 3, 1, 1)
        self.activation = LeakyReLU(inplace=True)
        self.conv_aux = mint.nn.Conv2d(num_features, config.num_channels, 3, 1, 1)
        self.conv_after_aux = nn.SequentialCell(mint.nn.Conv2d(3, num_features, 3, 1, 1), LeakyReLU(inplace=True))
        self.upsample = Upsample(config.upscale, num_features)
        self.final_convolution = mint.nn.Conv2d(num_features, config.num_channels_out, 3, 1, 1)
...
class Swin2SRForImageSuperResolution(Swin2SRPreTrainedModel):
    def __init__(self, config):
        ...
        if self.upsampler == "pixelshuffle":
            ...
            self.final_convolution = mint.nn.Conv2d(config.embed_dim, config.num_channels_out, 3, 1, 1)
    ...
    def construct(
        ...
        Example:
         ...
         >>> from mindone.transformers import Swin2SRForImageSuperResolution
         ...
         >>> output = outputs.reconstruction.squeeze().float().clamp_(0, 1).numpy()
        ...
        if self.config.upsampler == "pixelshuffle_aux":
            bicubic = mint.nn.functional.interpolate(
                pixel_values,
