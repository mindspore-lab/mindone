...
class SamLayerNorm(nn.Cell):
    ...
    def construct(self, x: ms.Tensor) -> ms.Tensor:
        if self.data_format == "channels_last":
            x = mint.mint.nn.functional.layer_norm(x, self.normalized_shape, self.weight, self.bias, self.eps)
...
class SamMaskDecoder(nn.Cell):
    ...
    def construct(
        ...
        output_attentions: Optional[bool] = None,
        attention_similarity: ms.Tensor = None,
        target_embedding: ms.Tensor = None,
        ...
        output_tokens = mint.cat([self.iou_token.weight, self.mask_tokens.weight], dim=0)
...
class SamPromptEncoder(nn.Cell):
    ...
    def _embed_points(self, points: ms.Tensor, labels: ms.Tensor, pad: bool) -> ms.Tensor:
        ...
        # This is required for the ONNX export. The dtype needs to be explicitely
        # specificed as otherwise torch.onnx.export interprets as double
        point_embedding = mint.where(
            labels[..., None] != -10,
            point_embedding,
            ms.tensor(0.0, dtype=point_embedding.dtype),
        )
    ...
    def construct(
        ...
        if sparse_embeddings is None:
            sparse_embeddings = mint.zeros((batch_size, 1, 1, self.hidden_size))
...
class SamVisionAttention(nn.Cell):
    ...
    def construct(self, hidden_states: ms.Tensor, output_attentions=False) -> ms.Tensor:
        ...
        attn_weights = F.softmax(attn_weights, dtype=ms.float32, dim=-1).to(query.dtype)
...
SAM_START_DOCSTRING = r"""
    ...
    This model is also a MindSpore [mindspore.nn.Cell](https://www.mindspore.cn/docs/en/r2.6.0/api_python/nn/mindspore.nn.Cell.html) subclass.
