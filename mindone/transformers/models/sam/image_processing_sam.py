...
def _compute_stability_score_ms(masks: "ms.Tensor", mask_threshold: float, stability_score_offset: int):
    ...
    intersections = (
        (masks > (mask_threshold + stability_score_offset)).sum(-1, dtype=ms.int32).sum(-1, dtype=ms.int32)
    )
...
def nms(boxes: ms.Tensor, scores: ms.Tensor, iou_threshold: float) -> Tensor:
    """
