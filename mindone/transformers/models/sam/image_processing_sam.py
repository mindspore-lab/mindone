...
class SamImageProcessor(BaseImageProcessor):
    ...
    def post_process_masks(
        ...
        """
        if return_tensors != "ms" and return_tensors != "np":
            raise ValueError("return_tensors must be either 'ms' or 'np'")
        output_masks = self._post_process_masks_ms(
            masks=masks,
            original_sizes=original_sizes,
            reshaped_input_sizes=reshaped_input_sizes,
        ...
        if return_tensors == "np":
            output_masks = output_masks.numpy()
        return output_masks
    ...
    def _post_process_masks_ms(
        ...
        for i, original_size in enumerate(original_sizes):
            if isinstance(masks[i], np.ndarray):
                ...
                raise ValueError("Input masks should be a list of `ms.Tensor` or a list of `np.ndarray`")
    ...
    def post_process_for_mask_generation(self, all_masks, all_scores, all_boxes, crops_nms_thresh, return_tensors="ms"):
        ...
        Args:
            all_masks (`Union[List[ms.Tensor]]`):
                List of all predicted segmentation masks
            all_scores (`Union[List[ms.Tensor]]`):
                List of all predicted iou scores
            all_boxes (`Union[List[ms.Tensor]]`):
    ...
    def filter_masks(
        ...
        Args:
            masks (`Union[ms.Tensor]`):
                Input masks.
            iou_scores (`Union[ms.Tensor]`):
                List of IoU scores.
        ...
        if return_tensors != "ms" and return_tensors != "np":
            raise ValueError("return_tensors must be either 'ms' or 'np'")
        output_masks = self._filter_masks_ms(
            masks=masks,
