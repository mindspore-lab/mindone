...
class SamImageProcessor(BaseImageProcessor):
    ...
    def preprocess(
        ...
        Args:
            ...
            return_tensors (`str` or `TensorType`, *optional*):
                The type of tensors to return. Can be one of:
                    ...
                    - `TensorType.MINDSPORE` or `'ms'`: Return a batch of type `ms.Tensor`.
        ...
        if not valid_images(images):
            raise ValueError("Invalid image type. Must be of type PIL.Image.Image, numpy.ndarray, " "or ms.Tensor.")
        ...
        if segmentation_maps is not None:
            ...
            if not valid_images(segmentation_maps):
                raise ValueError(
                    "Invalid segmentation map type. Must be of type PIL.Image.Image, numpy.ndarray, " "or ms.Tensor."
                )
        validate_preprocess_arguments(
            do_rescale=do_rescale,
            rescale_factor=rescale_factor,
    ...
    def post_process_masks(
        ...
        Args:
            masks (`Union[List[ms.Tensor], List[np.ndarray], List[tf.Tensor]]`):
                Batched masks from the mask_decoder in (batch_size, num_channels, height, width) format.
            original_sizes (`Union[ms.Tensor, tf.Tensor, List[Tuple[int,int]]]`):
                The original sizes of each image before it was resized to the model's expected input shape, in (height,
                width) format.
            reshaped_input_sizes (`Union[ms.Tensor, tf.Tensor, List[Tuple[int,int]]]`):
            ...
            return_tensors (`str`, *optional*, defaults to `"ms"`):
                If `"ms"`, return MindSpore tensors. If `"np"`, return np.ndarray.
        Returns:
            (`Union[ms.Tensor, np.ndarray]`): Batched masks in batch_size, num_channels, height, width) format, where
        ...
        if return_tensors == "ms":
            ...
            raise ValueError("return_tensors must be either 'ms' or 'np'")
    ...
    def _post_process_masks_ms(
        ...
        Args:
            ...
            original_sizes (`Union[ms.Tensor, List[Tuple[int,int]]]`):
                The original sizes of each image before it was resized to the model's expected input shape, in (height,
                width) format.
            reshaped_input_sizes (`Union[ms.Tensor, List[Tuple[int,int]]]`):
                The size of each image as it is fed to the model, in (height, width) format. Used to remove padding.
            mask_threshold (`float`, *optional*, defaults to 0.0):
                The threshold to use for binarizing the masks.
            binarize (`bool`, *optional*, defaults to `True`):
        ...
        if isinstance(original_sizes, (ms.Tensor, np.ndarray)):
            original_sizes = original_sizes.tolist()
        if isinstance(reshaped_input_sizes, (ms.Tensor, np.ndarray)):
            reshaped_input_sizes = reshaped_input_sizes.tolist()
        output_masks = []
        for i, original_size in enumerate(original_sizes):
            if isinstance(masks[i], np.ndarray):
                masks[i] = ms.Tensor(masks[i])
            elif not isinstance(masks[i], ms.Tensor):
                raise ValueError("Input masks should be a list of `ms.tensors` or a list of `np.ndarray`")
    ...
    def post_process_for_mask_generation(self, all_masks, all_scores, all_boxes, crops_nms_thresh, return_tensors="ms"):
        """
        Post processes mask that are generated by calling the Non Maximum Suppression algorithm on the predicted masks.

        Args:
            all_masks (`Union[List[ms.Tensor], List[tf.Tensor]]`):
                List of all predicted segmentation masks
            all_scores (`Union[List[ms.Tensor], List[tf.Tensor]]`):
                List of all predicted iou scores
            all_boxes (`Union[List[ms.Tensor], List[tf.Tensor]]`):
                List of all bounding boxes of the predicted masks
            crops_nms_thresh (`float`):
                Threshold for NMS (Non Maximum Suppression) algorithm.
            return_tensors (`str`, *optional*, defaults to `ms`):
                If `ms`, returns `ms.Tensor`. If `np`, returns `np.ndarray`.
        """
        if return_tensors == "ms":
            return _postprocess_for_mg(all_masks, all_scores, all_boxes, crops_nms_thresh)
        else:
            raise ValueError("return_tensors must be either 'ms'")
    ...
    def generate_crop_boxes(
        ...
        if return_tensors == "ms":
            crop_boxes = ms.tensor(crop_boxes)
            points_per_crop = ms.tensor(points_per_crop)
    ...
    def filter_masks(
        ...
        Args:
            masks (`Union[ms.Tensor, tf.Tensor]`):
                Input masks.
            iou_scores (`Union[ms.Tensor, tf.Tensor]`):
                List of IoU scores.
    ...
    def _filter_masks_ms(
        ...
        keep_mask = mint.ones(batch_size, dtype=ms.bool_)
        ...
        if stability_score_thresh > 0.0:
            stability_scores = _compute_stability_score_ms(masks, mask_threshold, stability_score_offset)
        ...
        masks = _mask_to_rle_mindspore(masks)

        return masks, scores, converted_boxes


def _compute_stability_score_ms(masks: "ms.Tensor", mask_threshold: float, stability_score_offset: int):
    # One mask is always contained inside the other.
    # Save memory by preventing unnecesary cast to ms.int64
    intersections = (masks > (mask_threshold + stability_score_offset)).sum(-1, dtype=ms.int32).sum(-1, dtype=ms.int32)
    unions = (masks > (mask_threshold - stability_score_offset)).sum(-1, dtype=ms.int32).sum(-1, dtype=ms.int32)
...
def _generate_crop_boxes(
    ...
    Args:
        image (Union[`numpy.ndarray`, `PIL.Image`, `ms.Tensor`]):
...
def _is_box_near_crop_edge(boxes, crop_box, orig_box, atol=20.0):
    ...
    offset = ms.tensor([[left, top, left, top]])
    # Check if boxes has a channel dimension
    if len(boxes.shape) == 3:
        offset = offset.unsqueeze(1)
    boxes = (boxes + offset).float()

    near_crop_edge = mint.isclose(boxes, crop_box_ms[None, :], atol=atol, rtol=0)
    near_image_edge = mint.isclose(boxes, orig_box_ms[None, :], atol=atol, rtol=0)
    near_crop_edge = mint.logical_and(near_crop_edge, ~near_image_edge)
    return mint.any(near_crop_edge, dim=1)


def _batched_mask_to_box(masks: "ms.Tensor"):
    """
    ...
    in_height_coords = in_height * mint.arange(height)[None, :]
    ...
    in_width_coords = in_width * mint.arange(width)[None, :]
...
def _mask_to_rle_mindspore(input_mask: "ms.Tensor"):
    """
...
def _postprocess_for_mg(rle_masks, iou_scores, mask_boxes, amg_crops_nms_thresh=0.7):
    ...
    Args:
            ...
            iou_scores (`ms.Tensor` of shape (nb_masks, 1)):
                iou_scores predicted by the model
            mask_boxes (`ms.Tensor`):
    ...
    keep_by_nms = batched_nms(
        ...
        scores=iou_scores,
        idxs=mint.zeros(mask_boxes.shape[0]),
...
def batched_nms(
    ...
    # FIXME: to determine a reasonable threshold in MindSpore
