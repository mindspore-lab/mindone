...
class MBartOnnxConfig(OnnxSeq2SeqConfigWithPast):
    ...
    def _generate_dummy_inputs_for_default_and_seq2seq_lm(
        ...
        if self.use_past:
            ...
            common_inputs["decoder_attention_mask"] = mindspore.mint.cat(
                [common_inputs["decoder_attention_mask"], mindspore.mint.ones(batch, decoder_past_length)], dim=1
            )
            ...
            for _ in range(min_num_layers):
                common_inputs["past_key_values"].append(
                    (
                        mindspore.mint.zeros(decoder_shape),
                        mindspore.mint.zeros(decoder_shape),
                        mindspore.mint.zeros(encoder_shape),
                        mindspore.mint.zeros(encoder_shape),
                    )
                )
            # TODO: test this.
            ...
            for _ in range(min_num_layers, max_num_layers):
                common_inputs["past_key_values"].append((mindspore.mint.zeros(shape), mindspore.mint.zeros(shape)))
        return common_inputs
    ...
    def _generate_dummy_inputs_for_causal_lm(
        ...
        if self.use_past:
            ...
            common_inputs["attention_mask"] = mindspore.mint.cat(
                [common_inputs["attention_mask"], mindspore.mint.ones(batch, past_key_values_length, dtype=mask_dtype)], dim=1
            )
            common_inputs["past_key_values"] = [
                (mindspore.mint.zeros(past_shape), mindspore.mint.zeros(past_shape)) for _ in range(num_encoder_layers)
            ]
        return common_inputs
