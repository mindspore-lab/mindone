...
class MBartLearnedPositionalEmbedding(mindspore.mint.nn.Embedding):
    """
    ...
    def construct(self, input_ids: mindspore.Tensor, past_key_values_length: int = 0, position_ids: mindspore.Tensor = None):
...
class MBartScaledWordEmbedding(mindspore.mint.nn.Embedding):
    """
...
class MBartAttention(mindspore.nn.Cell):
...
class MBartEncoderLayer(mindspore.nn.Cell):
...
class MBartDecoderLayer(mindspore.nn.Cell):
...
class MBartClassificationHead(mindspore.nn.Cell):
...
class MBartEncoder(MBartPreTrainedModel):
    ...
    def __init__(self, config: MBartConfig, embed_tokens: Optional[mindspore.mint.nn.Embedding] = None):
        ...
        self.layers = mindspore.nn.CellList([MBartEncoderLayer(config) for _ in range(config.encoder_layers)])
        self.config = config
...
class MBartDecoder(MBartPreTrainedModel):
    ...
    def __init__(self, config: MBartConfig, embed_tokens: Optional[mindspore.mint.nn.Embedding] = None):
        ...
        self.layers = mindspore.nn.CellList([MBartDecoderLayer(config, layer_idx=i) for i in range(config.decoder_layers)])
        self.config = config
...
class MBartModel(MBartPreTrainedModel):
    ...
    def construct(
        ...
        return_dict: Optional[bool] = None,
        cache_position: Optional[mindspore.Tensor] = None,
    ) -> Union[Seq2SeqModelOutput, Tuple[mindspore.Tensor]]:
        r"""
...
class MBartForConditionalGeneration(MBartPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: MBartConfig):
        ...
        self.register_buffer("final_logits_bias", mindspore.mint.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = mindspore.mint.nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
