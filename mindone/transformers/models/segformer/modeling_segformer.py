...
class SegformerLayer(nn.Cell):
    ...
    def __init__(self, config, hidden_size, num_attention_heads, drop_path, sequence_reduction_ratio, mlp_ratio):
        ...
        self.drop_path = SegformerDropPath(drop_path) if drop_path > 0.0 else mint.nn.Identity()
...
class SegformerPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, (mint.nn.LayerNorm, mint.nn.BatchNorm2d)):
            zeros_(module.bias)
            ones_(module.weight)
...
class SegformerForImageClassification(SegformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # Classifier head
        self.classifier = mint.nn.Linear(config.hidden_sizes[-1], config.num_labels)
...
class SegformerMLP(nn.Cell):
    ...
    def __init__(self, config: SegformerConfig, input_dim):
        super().__init__()
        self.proj = mint.nn.Linear(input_dim, config.decoder_hidden_size)
...
class SegformerDecodeHead(SegformerPreTrainedModel):
    def __init__(self, config):
        ...
        self.linear_fuse = mint.nn.Conv2d(
            in_channels=config.decoder_hidden_size * config.num_encoder_blocks,
            out_channels=config.decoder_hidden_size,
            kernel_size=1,
            bias=False,
        )
        self.batch_norm = mint.nn.BatchNorm2d(config.decoder_hidden_size)
        self.activation = mint.nn.ReLU()

        self.dropout = mint.nn.Dropout(config.classifier_dropout_prob)
        self.classifier = mint.nn.Conv2d(config.decoder_hidden_size, config.num_labels, kernel_size=1)
    ...
    def construct(self, encoder_hidden_states: ms.Tensor) -> ms.Tensor:
        ...
        hidden_states = self.linear_fuse(mint.cat(all_hidden_states[::-1], dim=1))
