...
class Olmo2Attention(nn.Cell):
    ...
    def construct(
        ...
        **kwargs,
        ...
        query_states = query_states.view(hidden_shape).swapaxes(1, 2)
        key_states = key_states.view(hidden_shape).swapaxes(1, 2)
        value_states = value_states.view(hidden_shape).swapaxes(1, 2)
...
class Olmo2DecoderLayer(nn.Cell):
...
class Olmo2RotaryEmbedding(nn.Cell):
    ...
    # FIXME: _dynamic_frequency_update() is not supported in static graph
    def construct(self, x, position_ids):
        if "dynamic" in self.rope_type:
            logger.warning("MindSpore graph mode does not supported dynamic frequency update.")
...
OLMO2_INPUTS_DOCSTRING = r"""
    Args:
        ...
        past_key_values (`Cache` or `tuple(tuple(Tensor))`, *optional*):
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        **kwargs,
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            min_dtype = dtype_to_min(dtype).float().item()  # limitation of `mint.full`, remove when fixed
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        r"""
            ...
            logits_to_keep (`int` or `Tensor`, *optional*):
                If an `int`, compute logits for the last `logits_to_keep` tokens. If `0`, calculate logits for all
                `input_ids` (special case). Only last token logits are needed for generation, and calculating them only for that
                token can save memory, which becomes pretty significant for long sequences or large vocabulary size.
                If a `Tensor`, must be 1D corresponding to the indices to keep in the sequence length dimension.
        ...
        >>> from mindone.transformers import Olmo2ForCausalLM

        >>> model = Olmo2ForCausalLM.from_pretrained("allenai/OLMo-2-0425-1B")
        ...
        >>> # Generate
        >>> generate_ids = model.generate(mindspore.Tensor.from_numpy(inputs.input_ids))
        >>> tokenizer.batch_decode(generate_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
        "Hey, are you conscious? Can you talk to me? Can you hear me? Can you see me? Can you feel me? Can you smell me?"
        ```"""
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
