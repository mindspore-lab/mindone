...
class BaseLukeModelOutputWithPooling(BaseModelOutputWithPooling):
    ...
    Args:
        ...
        entity_last_hidden_state (`ms.Tensor` of shape `(batch_size, entity_length, hidden_size)`):
        ...
        entity_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when
...
class BaseLukeModelOutput(BaseModelOutput):
    ...
    Args:
        ...
        entity_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or
...
class LukeMaskedLMOutput(ModelOutput):
    ...
    Args:
        ...
        mlm_loss (`ms.Tensor` of shape `(1,)`, *optional*, returned when `labels` is provided):
            Masked language modeling (MLM) loss.
        mep_loss (`ms.Tensor` of shape `(1,)`, *optional*, returned when `labels` is provided):
        ...
        entity_logits (`ms.Tensor` of shape `(batch_size, sequence_length, config.vocab_size)`):
...
class EntitySpanClassificationOutput(ModelOutput):
    ...
    Args:
        loss (`ms.Tensor` of shape `(1,)`, *optional*, returned when `labels` is provided):
            Classification loss.
        logits (`ms.Tensor` of shape `(batch_size, entity_length, config.num_labels)`):
            Classification scores (before SoftMax).
        hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple of `ms.Tensor` (one for the output of the embeddings + one for the output of each layer) of
            shape `(batch_size, sequence_length, hidden_size)`. Hidden-states of the model at the output of each layer
...
class LukeEmbeddings(mindspore.nn.Cell):
    """
...
class LukeEntityEmbeddings(mindspore.nn.Cell):
    def __init__(self, config: LukeConfig):
        super().__init__()
        self.config = config

        self.entity_embeddings = mindspore.mint.nn.Embedding(
            config.entity_vocab_size, config.entity_emb_size, padding_idx=0
        )
        if config.entity_emb_size != config.hidden_size:
            self.entity_embedding_dense = mindspore.mint.nn.Linear(
    ...
    def construct(
        ...
        if token_type_ids is None:
            token_type_ids = mindspore.mint.zeros_like(entity_ids)
        ...
        position_embeddings = mindspore.mint.sum(position_embeddings, dim=-2)
...
class LukeSelfAttention(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        ...
        if self.use_entity_aware_attention:
            self.w2e_query = mindspore.mint.nn.Linear(config.hidden_size, self.all_head_size)
            self.e2w_query = mindspore.mint.nn.Linear(config.hidden_size, self.all_head_size)
            self.e2e_query = mindspore.mint.nn.Linear(config.hidden_size, self.all_head_size)
    ...
    def construct(
        ...
        if entity_hidden_states is None:
            ...
            concat_hidden_states = mindspore.mint.cat([word_hidden_states, entity_hidden_states], dim=1)
        ...
        if self.use_entity_aware_attention and entity_hidden_states is not None:
            ...
            w2w_attention_scores = mindspore.mint.matmul(w2w_query_layer, w2w_key_layer.transpose(-1, -2))
            w2e_attention_scores = mindspore.mint.matmul(w2e_query_layer, w2e_key_layer.transpose(-1, -2))
            e2w_attention_scores = mindspore.mint.matmul(e2w_query_layer, e2w_key_layer.transpose(-1, -2))
            e2e_attention_scores = mindspore.mint.matmul(e2e_query_layer, e2e_key_layer.transpose(-1, -2))

            # combine attention scores to create the final attention score matrix
            word_attention_scores = mindspore.mint.cat([w2w_attention_scores, w2e_attention_scores], dim=3)
            entity_attention_scores = mindspore.mint.cat([e2w_attention_scores, e2e_attention_scores], dim=3)
            attention_scores = mindspore.mint.cat([word_attention_scores, entity_attention_scores], dim=2)
        ...
        context_layer = mindspore.mint.matmul(attention_probs, value_layer)
...
class LukeSelfOutput(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class LukeAttention(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
    ...
    def construct(
        ...
        if entity_hidden_states is None:
            ...
            concat_self_outputs = mindspore.mint.cat(self_outputs[:2], dim=1)
            concat_hidden_states = mindspore.mint.cat([word_hidden_states, entity_hidden_states], dim=1)
...
class LukeIntermediate(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class LukeOutput(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class LukeLayer(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
    ...
    def construct(
        ...
        if entity_hidden_states is None:
            ...
            concat_attention_output = mindspore.mint.cat(self_attention_outputs[:2], dim=1)
...
class LukeEncoder(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = mindspore.nn.CellList([LukeLayer(config) for _ in range(config.num_hidden_layers)])
...
class LukePooler(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.dense = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
...
class EntityPredictionHeadTransform(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.dense = mindspore.mint.nn.Linear(config.hidden_size, config.entity_emb_size)
        if isinstance(config.hidden_act, str):
            self.transform_act_fn = ACT2FN[config.hidden_act]
        else:
            self.transform_act_fn = config.hidden_act
        self.LayerNorm = mindspore.mint.nn.LayerNorm(config.entity_emb_size, eps=config.layer_norm_eps)
...
class EntityPredictionHead(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.transform = EntityPredictionHeadTransform(config)
        self.decoder = mindspore.mint.nn.Linear(config.entity_emb_size, config.entity_vocab_size, bias=False)
        self.bias = mindspore.Parameter(mindspore.mint.zeros(config.entity_vocab_size))
...
class LukePreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: mindspore.nn.Cell):
...
LUKE_INPUTS_DOCSTRING = r"""
    Args:
        ...
        entity_ids (`ms.Tensor` of shape `(batch_size, entity_length)`):
            Indices of entity tokens in the entity vocabulary.

            Indices can be obtained using [`AutoTokenizer`]. See [`PreTrainedTokenizer.encode`] and
            [`PreTrainedTokenizer.__call__`] for details.

        entity_attention_mask (`ms.Tensor` of shape `(batch_size, entity_length)`, *optional*):
            Mask to avoid performing attention on padding entity token indices. Mask values selected in `[0, 1]`:

            - 1 for entity tokens that are **not masked**,
            - 0 for entity tokens that are **masked**.

        entity_token_type_ids (`ms.Tensor` of shape `(batch_size, entity_length)`, *optional*):
        ...
        entity_position_ids (`ms.Tensor` of shape `(batch_size, entity_length, max_mention_length)`, *optional*):
...
class LukeModel(LukePreTrainedModel):
    ...
    def construct(
        ...
        >>> from mindone.transformers import LukeModel
        ...
        >>> encoding = tokenizer(text, entity_spans=entity_spans, add_prefix_space=True, return_tensors="np")
        >>> for k,v in encoding.items():
        >>>    encoding[k] = ms.tensor(v)
        ...
        >>> encoding = tokenizer(
        ...     text, entities=entities, entity_spans=entity_spans, add_prefix_space=True, return_tensors="np"
        ... )
        >>> for k,v in encoding.items():
        >>>    encoding[k] = ms.tensor(v)
        >>> outputs = model(**encoding)
        ...
        if entity_ids is not None:
            ...
            if entity_attention_mask is None:
                entity_attention_mask = mindspore.mint.ones(
                    (batch_size, entity_seq_length),
                )
            if entity_token_type_ids is None:
                entity_token_type_ids = mindspore.mint.zeros(
                    (batch_size, entity_seq_length),
                    dtype=mindspore.int64,
                )
    ...
    def get_extended_attention_mask(
        ...
        Arguments:
            word_attention_mask (`ms.Tensor`):
                Attention mask for word tokens with ones indicating tokens to attend to, zeros for tokens to ignore.
            entity_attention_mask (`ms.Tensor`, *optional*):
        ...
        if entity_attention_mask is not None:
            attention_mask = mindspore.mint.cat([attention_mask, entity_attention_mask], dim=-1)
...
class LukeLMHead(mindspore.nn.Cell):
...
class LukeForMaskedLM(LukePreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.loss_fn = mindspore.mint.nn.CrossEntropyLoss()
    ...
    def construct(
        ...
        entity_labels (`ms.Tensor` of shape `(batch_size, entity_length)`, *optional*):
            Labels for computing the masked language modeling loss. Indices should be in `[-100, 0, ...,
            config.vocab_size]` (see `input_ids` docstring) Tokens with indices set to `-100` are ignored (masked), the
            loss is only computed for the tokens with labels in `[0, ..., config.vocab_size]`
...
class LukeForEntityClassification(LukePreTrainedModel):
    ...
    def construct(
        ...
        r"""
        labels (`ms.Tensor` of shape `(batch_size,)` or `(batch_size, num_labels)`, *optional*):
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import LukeForEntityClassification
        >>> import mindspore as ms
        ...
        >>> text = "Beyoncé lives in Los Angeles."
        >>> entity_spans = [(0, 7)]  # character-based entity span corresponding to "Beyoncé"
        >>> inputs = tokenizer(text, entity_spans=entity_spans, return_tensors="np")
        ...
        if labels is not None:
            ...
            # move labels to correct device to enable model parallelism
            if labels.ndim == 1:
                loss = mindspore.mint.nn.functional.cross_entropy(logits, labels)
            else:
                loss = mindspore.mint.nn.functional.binary_cross_entropy_with_logits(
...
class LukeForEntityPairClassification(LukePreTrainedModel):
    def __init__(self, config):
        ...
        self.num_labels = config.num_labels
        self.dropout = mindspore.mint.nn.Dropout(config.hidden_dropout_prob)
        self.classifier = mindspore.mint.nn.Linear(config.hidden_size * 2, config.num_labels, False)
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import LukeForEntityPairClassification
        >>> import mindspore as ms
        ...
        feature_vector = mindspore.mint.cat(
...
class LukeForEntitySpanClassification(LukePreTrainedModel):
    def __init__(self, config):
        ...
        self.num_labels = config.num_labels
        self.dropout = mindspore.mint.nn.Dropout(config.hidden_dropout_prob)
        self.classifier = mindspore.mint.nn.Linear(config.hidden_size * 3, config.num_labels)
    ...
    def construct(
        ...
        r"""
        entity_start_positions (`ms.Tensor`):
            The start positions of entities in the word token sequence.

        entity_end_positions (`ms.Tensor`):
            The end positions of entities in the word token sequence.

        labels (`ms.Tensor` of shape `(batch_size, entity_length)` or `(batch_size, entity_length, num_labels)`, *optional*):
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import LukeForEntitySpanClassification
        >>> import mindspore as ms
        ...
        start_states = mindspore.mint.gather(outputs.last_hidden_state, -2, entity_start_positions)

        entity_end_positions = entity_end_positions.unsqueeze(-1).expand((-1, -1, hidden_size))
        end_states = mindspore.mint.gather(outputs.last_hidden_state, -2, entity_end_positions)

        feature_vector = mindspore.mint.cat([start_states, end_states, outputs.entity_last_hidden_state], dim=2)
        ...
        if labels is not None:
            ...
            # cross entropy is used otherwise.
            if labels.ndim == 2:
                loss = mindspore.mint.nn.functional.cross_entropy(logits.view(-1, self.num_labels), labels.view(-1))
            else:
                loss = mindspore.mint.nn.functional.binary_cross_entropy_with_logits(
                    logits.view(-1), labels.view(-1).type_as(logits)
                )
...
class LukeForSequenceClassification(LukePreTrainedModel):
    def __init__(self, config):
        ...
        self.num_labels = config.num_labels
        self.luke = LukeModel(config)
        self.dropout = mindspore.mint.nn.Dropout(
