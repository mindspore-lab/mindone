...
class WavLMNoLayerNormConvLayer(mindspore.nn.Cell):
...
class WavLMLayerNormConvLayer(mindspore.nn.Cell):
    def __init__(self, config, layer_id=0):
        super().__init__()
        self.in_conv_dim = config.conv_dim[layer_id - 1] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
...
class WavLMGroupNormConvLayer(mindspore.nn.Cell):
    def __init__(self, config, layer_id=0):
        super().__init__()
        self.in_conv_dim = config.conv_dim[layer_id - 1] if layer_id > 0 else 1
        self.out_conv_dim = config.conv_dim[layer_id]
...
class WavLMPositionalConvEmbedding(mindspore.nn.Cell):
...
class WavLMSamePadLayer(mindspore.nn.Cell):
...
class WavLMFeatureEncoder(mindspore.nn.Cell):
...
class WavLMFeatureProjection(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class WavLMAttention(mindspore.nn.Cell):
    ...
    def __init__(
        ...
        self.k_proj = mindspore.mint.nn.Linear(embed_dim, embed_dim)
        self.v_proj = mindspore.mint.nn.Linear(embed_dim, embed_dim)
        self.q_proj = mindspore.mint.nn.Linear(embed_dim, embed_dim)
        self.out_proj = mindspore.mint.nn.Linear(embed_dim, embed_dim)

        self.num_buckets = num_buckets
        self.max_distance = max_distance

        self.gru_rel_pos_const = mindspore.Parameter(mindspore.mint.ones((1, self.num_heads, 1, 1)))
        self.gru_rel_pos_linear = mindspore.mint.nn.Linear(self.head_dim, 8)

        if has_relative_position_bias:
            self.rel_attn_embed = mindspore.mint.nn.Embedding(self.num_buckets, self.num_heads)
    ...
    def construct(
        ...
        gate_a, gate_b = mindspore.mint.sigmoid(relative_position_proj).chunk(2, dim=-1)
        ...
        attn_output, attn_weights = self.ms_multi_head_self_attention(
            hidden_states, attention_mask, gated_position_bias, output_attentions
        )

        return attn_output, attn_weights, position_bias

    def ms_multi_head_self_attention(
        self,
        hidden_states: mindspore.Tensor,
        ...
        """simple wrapper around ms's multi_head_attention_forward function"""
        ...
        attn_output, attn_weights = multi_head_attention_forward(
            ...
            self.embed_dim,
            self.num_heads,
            mindspore.mint.empty([0]),
            mindspore.mint.cat((self.q_proj.bias, self.k_proj.bias, self.v_proj.bias)),
            bias_k,
            bias_v,
            add_zero_attn,
            self.dropout,
            ...
            self.training,
            key_padding_mask,
            # output_attentions, mindspore don't support
            gated_position_bias,
    ...
    def _relative_positions_bucket(self, relative_positions: mindspore.Tensor) -> mindspore.Tensor:
        ...
        relative_positions = mindspore.mint.abs(relative_positions)

        max_exact = num_buckets // 2
        is_small = relative_positions < max_exact

        relative_positions_if_large = mindspore.mint.log(relative_positions.float() / max_exact)
        ...
        relative_buckets += mindspore.mint.where(is_small, relative_positions, relative_position_if_large)
        return relative_buckets


# Copied from transformers.models.wav2vec2.modeling_wav2vec2.Wav2Vec2FeedForward with Wav2Vec2->WavLM
class WavLMFeedForward(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class WavLMEncoderLayer(mindspore.nn.Cell):
    def __init__(self, config: WavLMConfig, has_relative_position_bias: bool = True):
        ...
        self.final_layer_norm = mindspore.mint.nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class WavLMEncoderLayerStableLayerNorm(mindspore.nn.Cell):
    def __init__(self, config: WavLMConfig, has_relative_position_bias: bool = True):
        super().__init__()
        self.attention = WavLMAttention(
            embed_dim=config.hidden_size,
...
class WavLMEncoder(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class WavLMEncoderStableLayerNorm(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.pos_conv_embed = WavLMPositionalConvEmbedding(config)
...
class WavLMGumbelVectorQuantizer(mindspore.nn.Cell):
    """
    ...
    def construct(self, hidden_states):
        ...
        if self.training:
            ...
            codevector_probs = mindspore.ops.gumbel_softmax(hidden_states.float(), tau=self.temperature, hard=True)
...
class WavLMAdapter(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        ...
        self.layers = mindspore.nn.CellList(WavLMAdapterLayer(config) for _ in range(config.num_adapter_layers))
...
class WavLMAdapterLayer(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.conv = nn.Conv1d(
...
class WavLMPreTrainedModel(MSPreTrainedModel):
    """
...
class WavLMForCTC(WavLMPreTrainedModel):
    ...
    def tie_weights(self):
        ...
        # [`MSPreTrainedModel`]. While slightly hacky, WavLM never has to tie input and output embeddings, so that it is
