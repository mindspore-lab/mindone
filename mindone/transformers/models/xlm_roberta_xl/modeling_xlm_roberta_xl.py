...
class XLMRobertaXLSelfAttention(nn.Cell):
    ...
    def construct(
        ...
        if self.position_embedding_type == "relative_key" or self.position_embedding_type == "relative_key_query":
            ...
            if use_cache:
                position_ids_l = mint.tensor(key_length - 1, dtype=ms.int64).view(-1, 1)
            else:
                position_ids_l = mint.arange(query_length, dtype=ms.int64, device=hidden_states.device).view(-1, 1)
            position_ids_r = mint.arange(key_length, dtype=ms.int64, device=hidden_states.device).view(1, -1)
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in XLMRobertaXLSelfAttention construct() function)
...
class XLMRobertaXLAttention(nn.Cell):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self_attn_layer_norm = mint.nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class XLMRobertaXLForCausalLM(XLMRobertaXLPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs,
        ...
        ```python
        >>> from mindone.transformers import AutoTokenizer, RobertaForCausalLM, RobertaConfig
        >>> import torch
    ...
    def construct(
        ...
        if labels is not None:
            ...
            if attention_mask is not None:
                ...
                active_labels = mint.where(
                    active_loss, labels.view(-1), ms.tensor(loss_fct.ignore_index).type_as(labels)
                )
                loss = loss_fct(active_logits, active_labels.int())
            else:
                loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1).int())
