...
# Similar to `transformers.models.clip.modeling_clip.CLIPVisionTransformer`
# but without docstring for `forward`
class Kosmos2VisionTransformer(nn.Cell):
    # Copied from transformers.models.altclip.modeling_altclip.AltCLIPVisionTransformer.__init__ with AltCLIPVision->Kosmos2Vision,ALTCLIP_VISION
    # ->KOSMOS2_VISION,AltCLIP->Kosmos2Vision
    def __init__(self, config: Kosmos2VisionConfig):
        super().__init__()
        self.config = config
...
class Kosmos2TextTransformer(nn.Cell):
    ...
    def forward_embedding(
        ...
        if image_embeds is not None:
            inputs_embeds[img_input_mask.to(dtype=ms.bool_)] = image_embeds.view(-1, image_embeds.shape[-1])
...
class Kosmos2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, Kosmos2VisionEmbeddings):
            ...
            module.q_proj.weight.set_data(
                initializer(Normal(sigma=std), module.q_proj.weight.shape, module.q_proj.weight.dtype)
            )
            module.k_proj.weight.set_data(
                initializer(Normal(sigma=std), module.k_proj.weight.shape, module.k_proj.weight.dtype)
            )
            module.v_proj.weight.set_data(
                initializer(Normal(sigma=std), module.v_proj.weight.shape, module.v_proj.weight.dtype)
            )
            module.out_proj.weight.set_data(
                initializer(Normal(sigma=std), module.out_proj.weight.shape, module.out_proj.weight.dtype)
            )
            if module.q_proj.bias is not None:
                module.q_proj.bias.data.zero_()
            if module.k_proj.bias is not None:
            ...
            module.fc1.weight.set_data(initializer(Normal(sigma=std), module.fc1.weight.shape, module.fc1.weight.dtype))
            module.fc2.weight.set_data(initializer(Normal(sigma=std), module.fc2.weight.shape, module.fc2.weight.dtype))
            if module.fc1.bias is not None:
                module.fc1.bias.data.zero_()
            if module.fc2.bias is not None:
                module.fc2.bias.data.zero_()
            ...
            module.lm_head.weight.set_data(
                initializer(Normal(sigma=std), module.lm_head.weight.shape, module.lm_head.weight.dtype)
            )
            if module.lm_head.bias is not None:
                module.lm_head.bias.data.zero_()
        elif isinstance(module, Kosmos2ImageToTextProjection):
            module.dense.weight.set_data(
                initializer(Normal(sigma=std), module.dense.weight.shape, module.dense.weight.dtype)
            )
...
class Kosmos2VisionModel(Kosmos2PreTrainedModel):
    ...
    # Copied from transformers.models.clip.modeling_clip.CLIPVisionModel.get_input_embeddings with CLIP_VISION->KOSMOS2_VISION,CLIP
    # ->Kosmos2,self.vision_model->self.model
            ...
            image_embeds_position_mask = mint.cat(
                (
                    ...
                    mint.zeros(
                        ...
                        dtype=image_embeds_position_mask.dtype,
                    ),
                ),
                dim=1,
            )
...
class Kosmos2ForConditionalGeneration(Kosmos2PreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        >>> processed_text
        '<grounding> An image of<phrase> a snowman</phrase><object><patch_index_0044><patch_index_0863></object> warming himself by<phrase>
        a fire</phrase><object><patch_index_0005><patch_index_0911></object>.'
