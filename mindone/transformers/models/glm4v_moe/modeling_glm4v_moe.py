...
class Glm4vMoeRMSNorm(mindspore.nn.Cell):
...
def apply_multimodal_rotary_pos_emb(q, k, cos, sin, mrope_section, unsqueeze_dim=1):
    ...
    cos = mindspore.mint.cat([m[i % 3] for i, m in enumerate(cos.split(mrope_section, dim=-1))], dim=-1).unsqueeze(
        unsqueeze_dim
    )
    sin = mindspore.mint.cat([m[i % 3] for i, m in enumerate(sin.split(mrope_section, dim=-1))], dim=-1).unsqueeze(
        unsqueeze_dim
    )
...
class Glm4vMoeTextAttention(mindspore.nn.Cell):
...
class Glm4vMoeTextTopkRouter(mindspore.nn.Cell):
    def __init__(self, config: Glm4vMoeTextConfig):
        ...
        self.register_buffer(
            "e_score_correction_bias", mindspore.mint.zeros((self.n_routed_experts), dtype=mindspore.float32)
        )
...
class Glm4vMoeTextMoE(mindspore.nn.Cell):
    """
...
class Glm4vMoeTextMLP(mindspore.nn.Cell):
...
class Glm4vMoeTextRMSNorm(mindspore.nn.Cell):
    def __init__(self, hidden_size, eps=1e-6):
        """
...
class Glm4vMoeisionMlp(mindspore.nn.Cell):
    def __init__(self, config, bias: bool = False):
        ...
        self.gate_proj = mindspore.mint.nn.Linear(self.hidden_size, self.intermediate_size, bias=bias)
        self.up_proj = mindspore.mint.nn.Linear(self.hidden_size, self.intermediate_size, bias=bias)
        self.down_proj = mindspore.mint.nn.Linear(self.intermediate_size, self.hidden_size, bias=bias)
        self.act_fn = ACT2FN[config.hidden_act]
...
class Glm4vMoeVisionPatchEmbed(mindspore.nn.Cell):
    def __init__(self, config: Glm4vMoeVisionConfig) -> None:
        ...
        self.proj = mindspore.mint.nn.Conv3d(
            self.in_channels, self.embed_dim, kernel_size=kernel_size, stride=kernel_size
        )
...
class Glm4vMoeVisionRotaryEmbedding(mindspore.nn.Cell):
    inv_freq: mindspore.Tensor  # fix linting for `register_buffer`

    def __init__(self, dim: int, theta: float = 10000.0) -> None:
        super().__init__()
        inv_freq = 1.0 / (theta ** (mindspore.mint.arange(0, dim, 2, dtype=mindspore.float32) / dim))
        self.register_buffer("inv_freq", inv_freq, persistent=False)

    def construct(self, seqlen: int) -> mindspore.Tensor:
        seq = mindspore.mint.arange(seqlen, dtype=self.inv_freq.dtype)
        freqs = mindspore.mint.outer(seq, self.inv_freq)
        return freqs


class Glm4vMoeVisionPatchMerger(mindspore.nn.Cell):
    def __init__(self, dim: int, context_dim: int, hidden_act: str, bias: bool = False) -> None:
        super().__init__()
        self.proj = mindspore.mint.nn.Linear(dim, dim, bias=bias)
        self.post_projection_norm = LayerNorm(dim)
        self.gate_proj = mindspore.mint.nn.Linear(dim, context_dim, bias=bias)
        self.up_proj = mindspore.mint.nn.Linear(dim, context_dim, bias=bias)
        self.down_proj = mindspore.mint.nn.Linear(context_dim, dim, bias=bias)
        self.act1 = mindspore.mint.nn.GELU()
...
class Glm4vMoeVisionEmbeddings(mindspore.nn.Cell):
    def __init__(self, config: Glm4vMoeVisionConfig):
        ...
        self.register_buffer(
            "position_ids", mindspore.mint.arange(self.num_positions).expand((1, -1)), persistent=False
        )

    def construct(self, embeddings, lengths, image_shapes, h_coords, w_coords) -> mindspore.Tensor:
        """
        Forward pass with integrated position encoding adaptation using 2D interpolation.

        Args:
            embeddings: Input embeddings tensor
            lengths (mindspore.Tensor): Sequence lengths for each image in the batch.
            image_shapes (mindspore.Tensor): Tensor of shape [batch_size, 3] representing the image shapes (t, h, w).
            h_coords (mindspore.Tensor): Tensor of shape [total_seq] representing the h coordinate for each patch.
            w_coords (mindspore.Tensor): Tensor of shape [total_seq] representing the w coordinate for each patch.

        Returns:
            mindspore.Tensor: Embeddings with adapted position encoding added.
        """
        ...
        h_coords, w_coords = h_coords, w_coords

        # Handle empty sequence case
        if total_seq == 0:
            adapted_pos_embed = mindspore.mint.empty(0, hidden_size, dtype=pos_embed_weight.dtype)
        else:
            # Convert inputs to tensors if needed
            if isinstance(lengths, list):
                lengths = mindspore.Tensor(lengths, dtype=mindspore.int64)
            if not isinstance(image_shapes, mindspore.Tensor):
                image_shapes = mindspore.Tensor(image_shapes, dtype=mindspore.int64)
            ...
            pos_embed_2d = (
                ...
                .permute(2, 0, 1)
                .unsqueeze(0)
                .to(dtype=mindspore.float32)
            )

            # Calculate target dimensions for each patch
            target_h = mindspore.mint.cat([image_shapes[i, 1].repeat(int(lengths[i])) for i in range(len(lengths))]).to(
                dtype=mindspore.float32
            )
            target_w = mindspore.mint.cat([image_shapes[i, 2].repeat(int(lengths[i])) for i in range(len(lengths))]).to(
                dtype=mindspore.float32
            )

            # Normalize coordinates to [-1, 1] range for grid_sample
            h_coords = h_coords.to(dtype=mindspore.float32)
            w_coords = w_coords.to(dtype=mindspore.float32)
            norm_w = ((w_coords + 0.5) / target_w) * 2 - 1
            norm_h = ((h_coords + 0.5) / target_h) * 2 - 1

            # Create sampling grid
            grid = mindspore.mint.stack((norm_w, norm_h), dim=-1).unsqueeze(0).unsqueeze(2)

            # Perform bicubic interpolation
            interpolated_embed_fp32 = mindspore.mint.nn.functional.grid_sample(
...
class Glm4vMoeVisionAttention(mindspore.nn.Cell):
    def __init__(self, config: Glm4vMoeVisionConfig) -> None:
        super().__init__()
        ...
        self.qkv = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size * 3, bias=config.attention_bias)
        self.proj = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size, bias=False)
        self.scaling = self.head_dim**-0.5
        self.config = config
        self.attention_dropout = config.attention_dropout
    ...
    def construct(
        ...
        **kwargs,
        ...
        if self.config._attn_implementation == "flash_attention_2":
            ...
            splits = [
                mindspore.mint.split(tensor, lengths.tolist(), dim=2)
            ...
            attn_output = mindspore.mint.cat(attn_outputs, dim=1)
...
class Glm4vMoeTextRotaryEmbedding(mindspore.nn.Cell):
    inv_freq: mindspore.Tensor  # fix linting for `register_buffer`
...
class Glm4vMoeVisionModel(Glm4vMoePreTrainedModel):
    ...
    def __init__(self, config) -> None:
        ...
        self.blocks = mindspore.nn.CellList([Glm4vMoeVisionBlock(config) for _ in range(config.depth)])
        ...
        self.downsample = mindspore.mint.nn.Conv2d(
    ...
    def rot_pos_emb(self, grid_thw):
        ...
        for t, h, w in grid_thw:
            ...
            hpos_ids = mindspore.mint.arange(h).unsqueeze(1).expand((-1, w))
            ...
            wpos_ids = mindspore.mint.arange(w).unsqueeze(0).expand((h, -1))
            ...
            pos_ids.append(mindspore.mint.stack([hpos_ids, wpos_ids], dim=-1).repeat(t, 1))
        pos_ids = mindspore.mint.cat(pos_ids, dim=0)
        max_grid_size = grid_thw[:, 1:].max()
        rotary_pos_emb_full = self.rotary_pos_emb(int(max_grid_size))
        rotary_pos_emb = rotary_pos_emb_full[pos_ids].flatten(1)
        return rotary_pos_emb, pos_ids

    def construct(self, hidden_states: mindspore.Tensor, grid_thw: mindspore.Tensor) -> mindspore.Tensor:
        """
        Args:
            hidden_states (`mindspore.Tensor` of shape `(seq_len, hidden_size)`):
                The final hidden states of the model.
            grid_thw (`mindspore.Tensor` of shape `(num_images_or_videos, 3)`):
                The temporal, height and width of feature shape of each image in LLM.

        Returns:
            `mindspore.Tensor`: hidden_states.
        """
        hidden_states = self.patch_embed(hidden_states)
        hidden_states = self.post_conv_layernorm(hidden_states)

        rotary_pos_emb, image_type_ids = self.rot_pos_emb(grid_thw)
        emb = mindspore.mint.cat((rotary_pos_emb, rotary_pos_emb), dim=-1)
        ...
        cu_seqlens = mindspore.mint.nn.functional.pad(cu_seqlens, (1, 0), value=0)
...
class Glm4vMoeModel(Glm4vMoePreTrainedModel):
    ...
    def get_rope_index(
        ...
        Explanation:
            ...
            Examples:
                ...
                tokens_per_second: This is a crucial parameter. It dictates how many "time-steps"
                or "temporal tokens" are conceptually packed into a one-second interval of the video.
                In this case, we have 25 tokens per second. So each second of the video will be represented with 25 separate time points.
        ...
        if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            ...
            if attention_mask is None:
                attention_mask = mindspore.mint.ones_like(total_input_ids)
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for modality_type, start_idx, end_idx in input_type_group:
                    ...
                    if modality_type == "image":
                        ...
                        t_index = (
                            mindspore.mint.arange(llm_grid_t)
                            .view(-1, 1)
                            .expand((-1, llm_grid_h * llm_grid_w))
                            .flatten()
                        )
                        h_index = (
                            mindspore.mint.arange(llm_grid_h)
                            .view(1, -1, 1)
                            .expand((llm_grid_t, -1, llm_grid_w))
                            .flatten()
                        )
                        w_index = (
                            mindspore.mint.arange(llm_grid_w)
                            .view(1, 1, -1)
                        ...
                        llm_pos_ids_list.append(mindspore.mint.arange(text_len).view(1, -1).expand((3, -1)) + st_idx)

                        video_frame_num = 1

                llm_positions = mindspore.mint.cat(llm_pos_ids_list, dim=1).reshape(3, -1)
                position_ids[..., i, attention_mask[i] == 1] = llm_positions
                mrope_position_deltas.append(llm_positions.max() + 1 - len(total_input_ids[i]))
            mrope_position_deltas = mindspore.Tensor(
                mrope_position_deltas,
            ).unsqueeze(1)
            ...
            if attention_mask is not None:
                ...
                mrope_position_deltas = mindspore.mint.zeros(
                    [input_ids.shape[0], 1],
                    dtype=input_ids.dtype,
                )
    ...
    def get_video_features(
        self, pixel_values_videos: mindspore.Tensor, video_grid_thw: Optional[mindspore.Tensor] = None
    ):
        """
        Encodes videos into continuous embeddings that can be forwarded to the language model.

        Args:
            pixel_values_videos (`mindspore.Tensor` of shape `(batch_size, num_channels, image_size, image_size)`):
        ...
        for t, h, w in video_grid_thw:
            repeated_row = mindspore.Tensor([1, h.item(), w.item()]).unsqueeze(0).repeat(t, 1)
            temp_frames_hw.append(repeated_row)
        flattened_video_grid_thw = mindspore.mint.cat(temp_frames_hw, dim=0)
        video_embeds = self.visual(pixel_values_videos, grid_thw=flattened_video_grid_thw)
        split_sizes = (video_grid_thw.prod(-1) // self.visual.spatial_merge_size**2).tolist()
        video_embeds = mindspore.mint.split(video_embeds, split_sizes)
        return video_embeds

    def get_image_features(self, pixel_values: mindspore.Tensor, image_grid_thw: Optional[mindspore.Tensor] = None):
        """
        ...
        image_embeds = mindspore.mint.split(image_embeds, split_sizes)
        return image_embeds
    ...
    def get_placeholder_mask(
        ...
        image_features: Optional[mindspore.Tensor] = None,
        video_features: Optional[mindspore.Tensor] = None,
    ):
        """
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            ...
            image_embeds = mindspore.mint.cat(image_embeds, dim=0).to(inputs_embeds.dtype)
            image_mask, _ = self.get_placeholder_mask(input_ids, inputs_embeds, image_features=image_embeds)
            inputs_embeds = inputs_embeds.masked_scatter(image_mask, image_embeds)

        if pixel_values_videos is not None:
            video_embeds = self.get_video_features(pixel_values_videos, video_grid_thw)
            video_embeds = mindspore.mint.cat(video_embeds, dim=0).to(inputs_embeds.dtype)
        ...
        if position_ids is None:
            ...
            if attention_mask_tensor is not None and attention_mask_tensor.ndim == 4:
                attention_mask_tensor = mindspore.ops.diagonal(attention_mask_tensor[:, 0], dim1=1, dim2=2)
            ...
            prefill_compiled_stage = (input_ids is not None and input_ids.shape[1] != 1) or (
...
class Glm4vMoeForConditionalGeneration(Glm4vMoePreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> from mindone.transformers import Glm4vMoeForConditionalGeneration
    ...
    def _get_image_nums_and_video_nums(
        ...
        Returns:
            image_nums (`mindspore.Tensor` of shape `(batch_size, num_images_sample)`)
            video_nums (`mindspore.Tensor` of shape `(batch_size, num_videos_sample)`)
        """
        ...
        video_level = mindspore.mint.cumsum(is_video_start.int() - is_video_end.int(), dim=1)
        ...
        **model_kwargs,
        ...
        def _expand_dict_for_generation_visual(dict_to_expand):
            ...
            def _repeat_interleave_samples(x, lengths, repeat_times):
                samples = mindspore.mint.split(x, lengths)
                repeat_args = [repeat_times] + [1] * (x.dim() - 1)
                result = mindspore.mint.cat([sample.repeat(*repeat_args) for sample in samples], dim=0)
                return result
            ...
            for key in dict_to_expand:
                if key == "pixel_values":
                    ...
                    samples = mindspore.mint.split(video_grid_thw, list(video_nums))
                    lengths = [mindspore.mint.prod(sample, dim=1).sum() for sample in samples]
                    dict_to_expand[key] = _repeat_interleave_samples(
                        dict_to_expand[key], lengths=lengths, repeat_times=expand_size
                    )
