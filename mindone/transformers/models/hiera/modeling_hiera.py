...
class HieraEncoderOutput(ModelOutput):
    ...
    Args:
        ...
        hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple of `ms.FloatTensor` (one for the output of the embeddings + one for the output of each stage) of
...
class HieraModelOutput(ModelOutput):
    ...
    Args:
        ...
        bool_masked_pos (`ms.Tensor` of shape `(batch_size, sequence_length)`):
    ...
    bool_masked_pos: ms.Tensor = None
    ids_restore: ms.Tensor = None
    hidden_states: Optional[Tuple[ms.Tensor, ...]] = None
    attentions: Optional[Tuple[ms.Tensor, ...]] = None
    reshaped_hidden_states: Optional[Tuple[ms.Tensor, ...]] = None
...
class HieraForImageClassificationOutput(ImageClassifierOutput):
    ...
    Args:
        loss (`ms.Tensor` of shape `(1,)`, `optional`):
        ...
        attentions (`tuple(ms.Tensor)`, `optional`):
            ...
            Attentions weights after the attention softmax, used to compute the weighted average in the self-attention
            heads.
        reshaped_hidden_states (`tuple(ms.Tensor)`, `optional`):
            Tuple of `ms.Tensor` (one for the output of the embeddings + one for the output of each stage) of
            shape `(batch_size, height, width, hidden_size)`. These are the reshaped and re-rolled hidden states of the model.
...
class HieraPatchEmbeddings(nn.Cell):
    """
    ...
    def __init__(self, config, is_mae: bool = False):
        ...
        # mint.nn.Conv2d need tuple input, not list like pytorch
    ...
    def masked_conv(self, pixel_values: ms.Tensor, bool_masked_pos: Optional[ms.Tensor] = None) -> ms.Tensor:
        ...
        bool_masked_pos = mint.nn.functional.interpolate(bool_masked_pos.float(), size=target_size)
    ...
    def random_masking(self, pixel_values: ms.Tensor, noise: Optional[ms.Tensor] = None) -> Tuple[ms.Tensor, ms.Tensor]:
        ...
        Args:
            embeddings (`ms.Tensor` of shape `(batch_size, sequence_length, embed_dim)`)
            noise (`ms.Tensor` of shape `(batch_size, num_mask_units)`, *optional*) which is
        ...
        if noise is None:
            noise = mint.rand(batch_size, num_windows)

        # Sort noise for each sample
        ids_shuffle = mint.argsort(noise, dim=1)
        # ascend: small is keep, large is remove
        ids_restore = mint.argsort(ids_shuffle, dim=1)

        # Generate the binary bool_masked_pos: 1 is *keep*, 0 is *remove*
        # Note this is opposite to original MAE
        bool_masked_pos = mint.zeros([batch_size, num_windows])
        bool_masked_pos[:, :len_keep] = 1
        # Unshuffle to get the binary bool_masked_pos
        bool_masked_pos = mint.gather(bool_masked_pos, dim=1, index=ids_restore).bool()
    ...
    def construct(
        ...
        embeddings = ops.swapaxes(embeddings, 2, 1)

        return embeddings, bool_masked_pos, ids_restore


class HieraEmbeddings(nn.Cell):
    """
    ...
    def __init__(self, config: HieraConfig, is_mae: bool = False) -> None:
        ...
        self.position_embeddings = ms.Parameter(mint.zeros((1, self.num_tokens, config.embed_dim)))

    def interpolate_pos_encoding(
        self, embeddings: ms.Tensor, pos_embeds: ms.Tensor, height: int, width: int
    ) -> ms.Tensor:
        """
        ...
        pos_embeds = mint.nn.functional.interpolate(
            pos_embeds,
    ...
    def get_position_embedding(
        self, embeddings: ms.Tensor, height: int, width: int, interpolate_pos_encoding: bool
    ) -> ms.Tensor:
        return (
...
class HieraMaskUnitAttention(nn.Cell):
    """
    ...
    def __init__(
        ...
        self.qkv = mint.nn.Linear(hidden_size, 3 * hidden_size_output)
        self.proj = mint.nn.Linear(hidden_size_output, hidden_size_output)
    ...
    def construct(
        ...
        if self.query_stride > 1:
            ...
            query = query.max(dim=3)[0]

        attn_weights = (query * self.scale) @ ops.swapaxes(key, -1, -2)
        ...
        attn_output = ops.swapaxes(attn_output, 1, 3).reshape(batch_size, -1, self.hidden_size_output)
...
class HieraMlp(nn.Cell):
    def __init__(self, config, dim: int) -> None:
        super().__init__()
        self.activation_fn = ACT2FN[config.hidden_act]
        self.fc1 = mint.nn.Linear(dim, int(dim * config.mlp_ratio))
        self.fc2 = mint.nn.Linear(int(dim * config.mlp_ratio), dim)
...
class HieraLayer(nn.Cell):
    def __init__(
        ...
        self.layernorm_before = mint.nn.LayerNorm(hidden_size, eps=config.layer_norm_eps)
        ...
        self.layernorm_after = mint.nn.LayerNorm(hidden_size_output, eps=config.layer_norm_eps)
        self.mlp = HieraMlp(config, hidden_size_output)

        self.drop_path = HieraDropPath(drop_path) if drop_path > 0 else mint.nn.Identity()
        if hidden_size != hidden_size_output:
            self.proj = mint.nn.Linear(hidden_size, hidden_size_output)
...
def undo_windowing(hidden_states: ms.Tensor, shape: List[int], mask_unit_shape: List[int]) -> ms.Tensor:
    """
    Restore spatial organization by undoing windowed organization of mask units.

    Args:
        hidden_states (`ms.Tensor`): The hidden states tensor of shape `[batch_size, num_mask_unit_height*num_mask_unit_width, hidden_size]`.
...
class HieraEncoder(nn.Cell):
    ...
    def reroll(
        self, hidden_states: ms.Tensor, stage_idx: int, bool_masked_pos: Optional[ms.Tensor] = None
    ) -> ms.Tensor:
        """
...
def unroll(
    hidden_states: ms.Tensor, image_shape: Tuple[int, int], patch_stride: Tuple[int, int], schedule: List[List[int]]
) -> ms.Tensor:
    """
    ...
    for strides in schedule:
        ...
        interleaved_dims = []
        for pair in zip(current_size, strides):
            interleaved_dims.extend(pair)
        # add batch_size and hidden_size to new_shape
        new_shape = [batch_size] + interleaved_dims + [hidden_size]
        hidden_states = hidden_states.reshape(new_shape)
...
class HieraPreTrainedModel(MSPreTrainedModel):
    """
...
class HieraPooler(nn.Cell):
    def __init__(self, config: HieraConfig):
        super().__init__()
        num_features = int(config.embed_dim * config.embed_dim_multiplier ** (len(config.depths) - 1))
        self.layernorm = mint.nn.LayerNorm(num_features, eps=config.layer_norm_eps)
        self.pooler = mint.nn.AdaptiveAvgPool1d(1)
...
class HieraDecoder(nn.Cell):
    def __init__(self, config: HieraConfig):
        ...
        self.decoder_embeddings = mint.nn.Linear(num_features, config.decoder_hidden_size)

        self.mask_token = ms.Parameter(mint.zeros((1, 1, config.decoder_hidden_size)))

        self.decoder_position_embeddings = ms.Parameter(
            mint.zeros((1, math.prod(self.tokens_spatial_shape_final), config.decoder_hidden_size))
        )
        ...
        self.decoder_norm = mint.nn.LayerNorm(config.decoder_hidden_size, eps=config.layer_norm_eps)
        ...
        self.decoder_pred = mint.nn.Linear(config.decoder_hidden_size, pred_dim)
...
class HieraMultiScaleHead(nn.Cell):
    def __init__(self, config: HieraConfig):
        super().__init__()
        self.mask_unit_spatial_shape_final = [
            i // s ** (config.num_query_pool) for i, s in zip(config.masked_unit_size, config.query_stride)
        ...
        self.multi_scale_fusion_heads = nn.CellList()
        ...
        self.multi_scale_fusion_heads.append(mint.nn.Identity())

    def apply_fusion_head(self, head: nn.Cell, hidden_states: ms.Tensor) -> ms.Tensor:
    ...
    def construct(self, feature_maps: List[ms.Tensor]) -> ms.Tensor:
        # Multi-scale fusion
        hidden_states = 0.0
...
class HieraForPreTraining(HieraPreTrainedModel):
    def __init__(self, config: HieraConfig) -> None:
        ...
        self.encoder_norm = mint.nn.LayerNorm(self.hiera.num_features, eps=config.layer_norm_eps)
    ...
    def get_pixel_label_2d(self, pixel_values: ms.Tensor, bool_masked_pos: ms.Tensor) -> ms.Tensor:
        # bool_masked_pos (boolean tensor): True means *masked*
        size = self.pred_stride
        label = mint.nn.functional.unfold(pixel_values, kernel_size=size, stride=size)
    ...
    def forward_loss(self, pixel_values: ms.Tensor, logits: ms.Tensor, bool_masked_pos: ms.Tensor):
    ...
    def construct(
        ...
        Examples:
        ```python
        >>> from mindone.transformers import AutoImageProcessor, HieraForPreTraining
        ...
        >>> outputs = model(tensor(inputs.pixel_values))
...
class HieraForImageClassification(HieraPreTrainedModel):
    def __init__(self, config: HieraConfig) -> None:
        ...
        # Classifier head
        self.classifier = (
            mint.nn.Linear(self.hiera.num_features, config.num_labels) if config.num_labels > 0 else nn.Identity()
        )
    ...
    def construct(
        ...
        r"""
        labels (`msTensor` of shape `(batch_size,)`, *optional*):
