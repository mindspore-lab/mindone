...
class ConvNextV2GRN(nn.Cell):
    ...
    def __init__(self, dim: int):
        super().__init__()
        self.weight = ms.Parameter(mint.zeros((1, 1, 1, dim)))
        self.bias = ms.Parameter(mint.zeros((1, 1, 1, dim)))

    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        # Compute and normalize global spatial feature maps
        global_features = mint.norm(hidden_states, p=2, dim=(1, 2), keepdim=True)
...
class ConvNextV2Layer(nn.Cell):
    ...
    def __init__(self, config, dim, drop_path=0):
        super().__init__()
        # depthwise conv
        self.dwconv = mint.nn.Conv2d(dim, dim, kernel_size=7, padding=3, groups=dim)
        self.layernorm = ConvNextV2LayerNorm(dim, eps=1e-6)
        # pointwise/1x1 convs, implemented with linear layers
        self.pwconv1 = mint.nn.Linear(dim, 4 * dim)
        self.act = ACT2FN[config.hidden_act]
        self.grn = ConvNextV2GRN(4 * dim)
        self.pwconv2 = mint.nn.Linear(4 * dim, dim)
        self.drop_path = ConvNextV2DropPath(drop_path) if drop_path > 0.0 else mint.nn.Identity()
