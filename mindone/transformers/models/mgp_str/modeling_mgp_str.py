...
class MgpstrDropPath(ms.nn.Cell):
    """Drop paths (Stochastic Depth) per sample (when applied in main path of residual blocks)."""
...
class MgpstrModelOutput(ModelOutput):
    ...
    Args:
        logits (`tuple(mindspore.Tensor)` of shape `(batch_size, config.num_character_labels)`):
            Tuple of `mindspore.Tensor` (one for the output of character of shape `(batch_size,
        ...
        attentions (`tuple(mindspore.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `mindspore.Tensor` (one for each layer) of shape `(batch_size, config.max_token_length,
            sequence_length, sequence_length)`.

            Attentions weights after the attention softmax, used to compute the weighted average in the self-attention
            heads.
        a3_attentions (`tuple(mindspore.Tensor)`, *optional*, returned when `output_a3_attentions=True` is passed or when `config.output_a3_attentions=True`):
            Tuple of `mindspore.Tensor` (one for the attention of character, + one for the attention of bpe`, + one
...
class MgpstrEmbeddings(ms.nn.Cell):
    ...
    def __init__(self, config: MgpstrConfig):
        ...
        self.pos_embed = ms.Parameter(mint.zeros((1, self.num_patches + self.num_tokens, config.hidden_size)))
        self.pos_drop = mint.nn.Dropout(p=config.drop_rate)
...
class MgpstrMlp(ms.nn.Cell):
    """MLP as used in Vision Transformer, MLP-Mixer and related networks"""

    def __init__(self, config: MgpstrConfig, hidden_features):
        super().__init__()
        hidden_features = hidden_features or config.hidden_size
        self.fc1 = mint.nn.Linear(config.hidden_size, hidden_features)
        self.act = mint.nn.GELU()
        self.fc2 = mint.nn.Linear(hidden_features, config.hidden_size)
        self.drop = mint.nn.Dropout(config.drop_rate)
...
class MgpstrAttention(ms.nn.Cell):
    def __init__(self, config: MgpstrConfig):
        super().__init__()
        ...
        self.attn_drop = mint.nn.Dropout(config.attn_drop_rate)
        self.proj = mint.nn.Linear(config.hidden_size, config.hidden_size)
        self.proj_drop = mint.nn.Dropout(config.drop_rate)
...
class MgpstrLayer(ms.nn.Cell):
    def __init__(self, config: MgpstrConfig, drop_path=None):
        ...
        self.drop_path = MgpstrDropPath(drop_path) if drop_path is not None else mint.nn.Identity()
...
class MgpstrA3Module(ms.nn.Cell):
    def __init__(self, config: MgpstrConfig):
        super().__init__()
        self.token_norm = mint.nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.tokenLearner = ms.nn.SequentialCell(
            mint.nn.Conv2d(config.hidden_size, config.hidden_size, kernel_size=(1, 1), stride=1, groups=8, bias=False),
            mint.nn.Conv2d(config.hidden_size, config.max_token_length, kernel_size=(1, 1), stride=1, bias=False),
        )
        self.feat = mint.nn.Conv2d(
    ...
    def construct(self, hidden_states):
        ...
        attentions = mint.nn.functional.softmax(selected, dim=-1)

        feat = self.feat(hidden_states)
        feat = feat.flatten(2).transpose(1, 2)
        feat = mint.einsum("...si,...id->...sd", attentions, feat)
...
class MgpstrPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: Union[mint.nn.Linear, mint.nn.Conv2d, mint.nn.LayerNorm]) -> None:
        ...
        if isinstance(module, MgpstrEmbeddings):
            trunc_normal_(module.pos_embed, mean=0.0, std=self.config.initializer_range)
            ...
            module.weight.data = trunc_normal_(module.weight.data, mean=0.0, std=self.config.initializer_range)
...
class MgpstrForSceneTextRecognition(MgpstrPreTrainedModel):
    ...
    def __init__(self, config: MgpstrConfig) -> None:
        ...
        self.char_head = mint.nn.Linear(config.hidden_size, config.num_character_labels)
        self.bpe_head = mint.nn.Linear(config.hidden_size, config.num_bpe_labels)
        self.wp_head = mint.nn.Linear(config.hidden_size, config.num_wordpiece_labels)
