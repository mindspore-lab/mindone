...
class YolosEmbeddings(nn.Cell):
    ...
    def __init__(self, config: YolosConfig) -> None:
        ...
        self.detection_tokens = ms.Parameter(mint.zeros((1, config.num_detection_tokens, config.hidden_size)))
        self.patch_embeddings = YolosPatchEmbeddings(config)
        num_patches = self.patch_embeddings.num_patches
        self.position_embeddings = ms.Parameter(
            mint.zeros((1, num_patches + config.num_detection_tokens + 1, config.hidden_size))
        )
    ...
    def construct(self, pixel_values: ms.Tensor) -> ms.Tensor:
        ...
        embeddings = mint.cat((cls_tokens, embeddings, detection_tokens), dim=1)
...
class InterpolateInitialPositionEmbeddings(nn.Cell):
    ...
    def construct(self, pos_embed, img_size=(800, 1344)) -> ms.Tensor:
        ...
        scale_pos_embed = mint.cat((cls_pos_embed, patch_pos_embed, det_pos_embed), dim=1)
...
class InterpolateMidPositionEmbeddings(nn.Cell):
    ...
    def construct(self, pos_embed, img_size=(800, 1344)) -> ms.Tensor:
        ...
        scale_pos_embed = mint.cat((cls_pos_embed, patch_pos_embed, det_pos_embed), dim=2)
        return scale_pos_embed
    ...
    def construct(
        ...
        if self.config._attn_implementation != "eager":
            if self.config._attn_implementation == "sdpa" and output_attentions:
                logger.warning_once(
                    "`sdpa` attention does not support `output_attentions=True`. Falling back to eager attention."
...
class YolosAttention(nn.Cell):
    ...
    def construct(
        self, hidden_states: ms.Tensor, head_mask: Optional[ms.Tensor] = None, output_attentions: bool = False
    ) -> Union[tuple[ms.Tensor, ms.Tensor], tuple[ms.Tensor]]:
...
class YolosEncoder(nn.Cell):
    def __init__(self, config: YolosConfig) -> None:
        ...
        self.mid_position_embeddings = (
            ms.Parameter(mint.zeros((config.num_hidden_layers - 1, 1, seq_length, config.hidden_size)))
...
class YolosPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: Union[nn.Linear, mint.nn.Conv2d, mint.nn.LayerNorm]) -> None:
...
class YolosForObjectDetection(YolosPreTrainedModel):
    ...
    def construct(
        ...
        labels (`list[dict]` of len `(batch_size,)`, *optional*):
            ...
            batch respectively). The class labels themselves should be a `ms.Tensor` of len `(number of bounding
            boxes in the image,)` and the boxes a `ms.Tensor` of shape `(number of bounding boxes in the image,
            4)`.
        ...
        >>> from mindone.transformers import AutoImageProcessor, AutoModelForObjectDetection
        ...
        ...     )
        Detected remote with confidence 0.991 at location [46.47, 72.78, 178.98, 119.3]
