...
class TrOCRLearnedPositionalEmbedding(mint.nn.Embedding):
    """
...
class TrOCRScaledWordEmbedding(mint.nn.Embedding):
    """
...
class TrOCRSinusoidalPositionalEmbedding(nn.Cell):
    ...
    def __init__(self, num_positions: int, embedding_dim: int, padding_idx: Optional[int] = None):
        ...
        # self.register_buffer("_float_tensor", mint.empty(1))
...
class TrOCRAttention(nn.Cell):
    ...
    def __init__(
        ...
        self.k_proj = mint.nn.Linear(self.kdim, embed_dim, bias=bias)
        self.v_proj = mint.nn.Linear(self.vdim, embed_dim, bias=bias)
...
class TrOCRDecoder(TrOCRPreTrainedModel):
    ...
    def __init__(self, config: TrOCRConfig):
        ...
        if config.layernorm_embedding:
            self.layernorm_embedding = mint.nn.LayerNorm(config.hidden_size)
        else:
...
class TrOCRForCausalLM(TrOCRPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.output_projection = mint.nn.Linear(config.hidden_size, config.vocab_size, bias=False)
    ...
    def construct(
        ...
        >>> labels = processor.tokenizer(text, return_tensors="np").input_ids
