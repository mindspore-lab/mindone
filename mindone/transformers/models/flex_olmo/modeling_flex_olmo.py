...
class FlexOlmoRMSNorm(mindspore.nn.Cell):
...
class FlexOlmoRotaryEmbedding(mindspore.nn.Cell):
...
class FlexOlmoMLP(mindspore.nn.Cell):
...
class FlexOlmoAttention(mindspore.nn.Cell):
...
class FlexOlmoSparseMoeBlock(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.num_experts = config.num_experts
        self.top_k = config.num_experts_per_tok
        self.norm_topk_prob = config.norm_topk_prob
        self.gate = mindspore.mint.nn.Linear(config.hidden_size, self.num_experts, bias=False)
        self.experts = mindspore.nn.CellList([FlexOlmoMLP(config) for _ in range(self.num_experts)])
...
def load_balancing_loss_func(
    ...
    rank = routing_weights.shape[1]
    overall_loss = mindspore.mint.sum(
...
class FlexOlmoForCausalLM(FlexOlmoPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs,
        ...
        >>> from mindone.transformers import FlexOlmoForCausalLM
