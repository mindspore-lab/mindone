...
def split_to_tiles(images: ms.Tensor, num_tiles_height: int, num_tiles_width: int) -> ms.Tensor:
...
@lru_cache(maxsize=1)
def find_supported_resolutions(max_num_chunks: int, patch_size: SizeDict) -> ms.Tensor:
    """
    ...
    Returns:
        ms.Tensor: List of possible resolutions as tuples (height, width).
...
def get_best_fit(
    image_size: tuple[int, int],
    possible_resolutions: ms.Tensor,
    ...
    Args:
        ...
        possible_resolutions (ms.Tensor): A tensor of shape (N, 2) where each
    ...
    Example:
        >>> image_size = (200, 300)
        >>> possible_resolutions = ms.tensor([[224, 672],
        ...                                     [672, 224],
        ...                                     [224, 448],
        ...                                     [448, 224],
        ...                                     [224, 224]])
    ...
    scales = mint.where(scale_h > scale_w, scale_w, scale_h)

    # filter only scales that allow upscaling
    upscaling_options = scales[scales >= 1]
    if len(upscaling_options) > 0:
        if resize_to_max_canvas:
            selected_scale = mint.max(upscaling_options)
        else:
            selected_scale = mint.min(upscaling_options)
    else:
        # no upscaling possible,
        # get the minimum downscaling (max scale for scales<1)
        downscaling_options = scales[scales < 1]
        selected_scale = mint.max(downscaling_options)
...
class Llama4ImageProcessorFast(BaseImageProcessorFast):
    ...
    def rescale_and_normalize(
        ...
        Override to rescale and normalize the images in mindspore.bfloat16 as in the original implementation
        """
        if do_rescale and do_normalize:
            images = images.to(dtype=ms.bfloat16) * rescale_factor
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        possible_resolutions = ms.tensor(possible_resolutions)
        ...
        for shape, stacked_images in grouped_images.items():
            ...
            if ratio_h * ratio_w > 1:
                ...
                grouped_processed_images[shape] = mint.cat([processed_images, global_tiles.unsqueeze(1)], dim=1)
        ...
        processed_images = mint.cat(processed_images, dim=0) if return_tensors else processed_images
        aspect_ratios = mint.stack(aspect_ratios_list, dim=0) if return_tensors else aspect_ratios_list
        return BatchFeature(
