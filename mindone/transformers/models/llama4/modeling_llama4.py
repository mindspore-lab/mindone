...
class Llama4TextExperts(nn.Cell):
...
class Llama4TextMLP(nn.Cell):
    def __init__(self, config, intermediate_size=None):
        ...
        self.config = config
        self.gate_proj = mint.nn.Linear(config.hidden_size, intermediate_size, bias=False)
        self.up_proj = mint.nn.Linear(config.hidden_size, intermediate_size, bias=False)
        self.down_proj = mint.nn.Linear(intermediate_size, config.hidden_size, bias=False)
...
class Llama4TextL2Norm(nn.Cell):
...
class Llama4TextRMSNorm(nn.Cell):
...
class Llama4Router(mint.nn.Linear):
...
class Llama4TextRotaryEmbedding(nn.Cell):
...
def apply_rotary_emb(
    ...
    xq_out = ops.view_as_real(xq_ * freqs_cis[:, :, None, :]).flatten(3)
    xk_out = ops.view_as_real(xk_ * freqs_cis[:, :, None, :]).flatten(3)
...
def vision_eager_attention_forward(
    ...
    **kwargs,
    ...
    attn_weights = mint.matmul(query, key_states.transpose(2, 3)) * module.head_dim**-0.5
    if attention_mask is not None:
        causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
        attn_weights = attn_weights + causal_mask
...
class Llama4TextAttention(nn.Cell):
    ...
    def construct(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.attn_temperature_tuning and not self.use_rope:
            attn_scales = (
                mint.log1p(mint.floor((cache_position.float() + 1.0) / self.floor_scale)) * self.attn_scale + 1.0
            )
...
class Llama4ForCausalLM(Llama4PreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> from mindone.transformers import Llama4ForCausalLM
...
class Llama4VisionMLP2(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.intermediate_size = config.intermediate_size
        self.fc1 = mint.nn.Linear(self.intermediate_size, config.projector_input_dim, bias=False)
        self.fc2 = mint.nn.Linear(config.projector_output_dim, config.projector_output_dim, bias=False)
        self.activation_fn = mint.nn.GELU()  # ACT2FN[config.hidden_act]
...
class Llama4MultiModalProjector(nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class Llama4VisionPixelShuffleMLP(nn.Cell):
    def __init__(self, config):
        super().__init__()
    ...
    def construct(self, encoded_patches: ms.Tensor) -> ms.Tensor:
        encoded_patches = pixel_shuffle(encoded_patches, self.pixel_shuffle_ratio)
        return self.mlp(encoded_patches)


# TODO there is a different RoPE for vision encoder, defined as below
def reshape_for_broadcast(freqs_ci: ms.Tensor, query: ms.Tensor):
    ndim = query.ndim
...
def vision_apply_rotary_emb(
    ...
    query_out = ops.view_as_real(query_ * freqs_ci).flatten(3)
    key_out = ops.view_as_real(key_ * freqs_ci).flatten(3)
    return query_out.type_as(query), key_out.type_as(key)  # but this drops to 8e-3


class Llama4VisionAttention(nn.Cell):
    def __init__(self, config: Llama4VisionConfig):
        ...
        self.q_proj = mint.nn.Linear(self.embed_dim, self.num_heads * self.head_dim, bias=True)
        self.k_proj = mint.nn.Linear(self.embed_dim, self.num_heads * self.head_dim, bias=True)
        self.v_proj = mint.nn.Linear(self.embed_dim, self.num_heads * self.head_dim, bias=True)
        self.o_proj = mint.nn.Linear(self.num_heads * self.head_dim, self.embed_dim, bias=True)
...
class Llama4VisionMLP(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.activation_fn = mint.nn.GELU()  # ACT2FN[config.hidden_act]
        self.fc1 = mint.nn.Linear(config.hidden_size, config.intermediate_size, bias=True)
        self.fc2 = mint.nn.Linear(config.intermediate_size, config.hidden_size, bias=True)
...
class Llama4VisionEncoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Llama4VisionConfig):
        ...
        self.input_layernorm = mint.nn.LayerNorm(config.hidden_size)
        self.post_attention_layernorm = mint.nn.LayerNorm(config.hidden_size)
...
class Llama4VisionEncoder(nn.Cell):
    """
    ...
    def __init__(self, config: Llama4VisionConfig):
        super().__init__()
        self.config = config
        self.layers = nn.CellList([Llama4VisionEncoderLayer(config) for _ in range(config.num_hidden_layers)])
        self.gradient_checkpointing = False
        self.config = config
    ...
    def construct(
        self,
        hidden_states: ms.Tensor,
        freqs_ci: ms.Tensor,  # TODO move this to an attribute instead of keeping it around
        attention_mask: Optional[ms.Tensor] = None,
        output_attentions: Optional[bool] = None,
...
class Llama4UnfoldConvolution(nn.Cell):
    def __init__(self, config):
        ...
        self.unfold = mint.nn.Unfold(kernel_size=kernel_size, stride=config.patch_size)
...
class Llama4VisionRotaryEmbedding(nn.Cell):
    def __init__(self, config):
        super().__init__()
        idx = config.image_size // config.patch_size
        img_idx = mint.arange(idx**2, dtype=ms.int32).reshape(idx**2, 1)
        img_idx = mint.cat([img_idx, img_idx[:1]], dim=0)
        img_idx[-1, -1] = -2  # ID_CLS_TOKEN
        frequencies_x = img_idx % idx  # get the coordinates of the 2d matrix along x
        frequencies_y = img_idx // idx  # get the coordinates of the 2d matrix along y
        freq_dim = config.hidden_size // config.num_attention_heads // 2
        rope_freq = 1.0 / (config.rope_theta ** (mint.arange(0, freq_dim, 2)[: (freq_dim // 2)].float() / freq_dim))
        ...
        freqs = mint.cat([freqs_x, freqs_y], dim=-1).float().contiguous()[..., ::2]
        freqs = freqs.masked_fill(img_idx.reshape(-1, 1, 1) < 0, 0)
        freq_cis = view_as_complex(mint.stack([mint.cos(freqs), mint.sin(freqs)], dim=-1))
...
class Llama4VisionModel(Llama4PreTrainedModel):
    ...
    def construct(
        ...
        hidden_state = mint.cat([hidden_state, class_embedding], dim=1)
        num_patches += 1
...
class Llama4ForConditionalGeneration(Llama4PreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> import mindspore as ms
        >>> from transformers import AutoProcessor
        >>> from mindone.transformers import LlavaForConditionalGeneration
