...
class OlmoLayerNorm(nn.Cell):
    ...
    def construct(self, hidden_states: Tensor) -> Tensor:
        ...
        return F.layer_norm(hidden_states.to(dtype=ms.float32), self.normalized_shape, None, None, eps=1e-5).to(
            orig_dtype
        )
...
class OlmoForCausalLM(OlmoPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> from mindone.transformers import OlmoForCausalLM
        ...
        >>> # Generate
        >>> generate_ids = model.generate(mindspore.Tensor.from_numpy(inputs.input_ids), max_length=40)
        >>> tokenizer.batch_decode(generate_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
        "Hey, are you conscious? Can you talk to me?\nI’m not sure if you’re conscious of this, but I’m not a fan of the word “conversation.”"
        ```"""
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
