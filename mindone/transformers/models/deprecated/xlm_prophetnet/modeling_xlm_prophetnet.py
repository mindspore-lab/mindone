...
from transformers.models.deprecated.xlm_prophetnet.configuration_xlm_prophetnet import XLMProphetNetConfig
...
XLM_PROPHETNET_START_DOCSTRING = r"""
    ...
    This model is a Mindspore [mindspore.nn.Cell](https://pytorch.org/docs/stable/nn.html#torch.nn.Cell) sub-class. Use
...
def softmax(hidden_state, dim, onnx_trace=False):
    if onnx_trace:
        return mint.nn.functional.softmax(hidden_state.float(), dim=dim)
    else:
        return mint.nn.functional.softmax(hidden_state, dim=dim, dtype=ms.float32)
...
def ngram_attention_bias(sequence_length, ngram, dtype):
    ...
    """
    left_block = mint.ones((ngram, sequence_length, sequence_length), dtype=dtype) * dtype_to_min(dtype)
    ...
    left_block[:, :, 0] = 0
    return mint.cat([left_block, right_block], dim=2)
...
def compute_relative_buckets(num_buckets, max_distance, relative_positions, is_bidirectional=False):
    ...
    if is_bidirectional:
        ...
        rel_positions_bucket = (
            rel_positions_bucket
            + mint.lt(inv_relative_positions, mint.zeros_like(inv_relative_positions)).int() * num_buckets
        )
        inv_relative_positions = mint.abs(inv_relative_positions)
    else:
        inv_relative_positions = mint.max(inv_relative_positions, mint.zeros_like(inv_relative_positions))

    max_exact = num_buckets // 2
    is_small = mint.lt(inv_relative_positions, max_exact)
    val_if_large = max_exact + mint.log(inv_relative_positions.float() / max_exact) / math.log(
        max_distance / max_exact
    ) * (num_buckets - max_exact)
    val_if_large = mint.min(val_if_large, mint.ones_like(val_if_large) * (num_buckets - 1)).int()
    rel_positions_bucket = rel_positions_bucket + mint.where(is_small, inv_relative_positions.int(), val_if_large)
...
def compute_all_stream_relative_buckets(num_buckets, max_distance, position_ids):
    ...
    # predicting stream
    predicting_stream_relative_positions = mint.cat((position_ids - 1, position_ids), dim=-1).unsqueeze(1)
...
class XLMProphetNetSeq2SeqLMOutput(ModelOutput):
    ...
    Args:
        ...
        decoder_ngram_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed
...
class XLMProphetNetPreTrainedModel(PreTrainedModel):
    ...
    def _shift_right(self, input_ids):
        ...
        assert mint.all(shifted_input_ids >= 0).item(), "Verify that `shifted_input_ids` has only positive values"

        return shifted_input_ids


class XLMProphetNetPositionalEmbeddings(mint.nn.Embedding):
    """
    ...
    def construct(self, inputs_shape, attention_mask=None, past_key_values=None, position_ids=None):
        ...
        if position_ids is None:
            if past_key_values is not None:
                ...
                position_ids = mint.ones((1, 1), dtype=ms.int32) * (int(self.padding_idx + num_input_ids))
            else:
                if attention_mask is None:
                    attention_mask = mint.ones(inputs_shape, dtype=ms.int32)

                # retrieve position_ids from input_ids / attention_mask
                position_ids = (
                    mint.cumsum(attention_mask, dim=1).type_as(attention_mask) * attention_mask
...
class XLMProphetNetAttention(nn.Cell):
    ...
    def __init__(
        ...
        self.key_proj = mint.nn.Linear(hidden_size, hidden_size)
        self.value_proj = mint.nn.Linear(hidden_size, hidden_size)
        self.query_proj = mint.nn.Linear(hidden_size, hidden_size)
    ...
    def construct(
        ...
        attn_weights = mint.einsum("bsij,bsjk->bsik", query_states, key_states.transpose(2, 3))
        ...
        attn_output = mint.einsum("bsij,bsjk->bsik", attn_probs, value_states)
        ...
        attn_output = mint.nn.functional.dropout(attn_output, p=self.dropout, training=self.training)
...
class XLMProphetNetFeedForward(nn.Cell):
    ...
    def __init__(self, config: XLMProphetNetConfig, ffn_dim: int):
        ...
        self.intermediate = mint.nn.Linear(config.hidden_size, ffn_dim)
        self.output = mint.nn.Linear(ffn_dim, config.hidden_size)
...
class XLMProphetNetNgramSelfAttention(nn.Cell):
    def __init__(self, config: XLMProphetNetConfig):
        ...
        self.key_proj = mint.nn.Linear(config.hidden_size, config.hidden_size)
        self.value_proj = mint.nn.Linear(config.hidden_size, config.hidden_size)
        self.query_proj = mint.nn.Linear(config.hidden_size, config.hidden_size)

        # out projection
        self.out_proj = mint.nn.Linear(config.hidden_size, config.hidden_size)

        # rel position embeddings
        self.relative_pos_embeddings = mint.nn.Linear(config.hidden_size, self.num_buckets * self.num_attn_heads)
    ...
    def construct(
        ...
        if past_key_value is not None:
            ...
            main_key_states = mint.cat((prev_main_key_states, main_key_states), dim=2)
            prev_main_value_states = past_key_value[1]
            main_value_states = mint.cat((prev_main_value_states, main_value_states), dim=2)
        ...
        main_attn_weights = mint.einsum("bntc,bncs->bnts", main_query_states, main_key_states.transpose(2, 3))
        ...
        main_attn_probs = mint.nn.functional.dropout(main_attn_probs, p=self.attention_dropout, training=self.training)
        # project to attn_output
        # [batch_size, number_heads, sequence_length, sequence_length]
        # x [batch_size, number_heads, sequence_length, head_dimesion]
        # -> [batch_size, number_heads, sequence_length, head_dimesion]
        main_attn_output = mint.einsum("bntc,bncs->bnts", main_attn_probs, main_value_states)
        ...
        predict_query_states = mint.stack(predict_query_states_list, 1).view(
        ...
        predict_key_states = mint.stack([mint.cat([main_key_states, key], 2) for key in predict_key_states_list], 1)

        # [batch_size, sequence_length, ngram, hidden_size]
        predict_hidden_states = mint.stack(hidden_states_predict_list, dim=2)

        # [batch_size, number_heads, ngram, 2*sequence_length, head_dimesion]
        predict_value_states = mint.cat(
            [mint.cat([main_value_states, v_p], 2).unsqueeze(2) for v_p in predict_value_states_list], 2
        )
        ...
        predict_attn_weights = mint.einsum("bnhtc,bnhsc->bnhts", predict_query_states, predict_key_states)
        ...
        predict_attn_probs = mint.nn.functional.dropout(
        ...
        # x [batch_size, ngram, number_heads, 2*sequence_length, head_dimesion]
        # -> [batch_size, ngram, number_heads, sequence_length, head_dimesion]
        predict_attn_output = mint.einsum(
        ...
        attn_output = mint.cat([main_attn_output, predict_attn_output], 1).view(batch_size, -1, hidden_size)
    ...
    def get_main_relative_pos_embeddings(
        ...
        if main_relative_position_buckets is None:
            ...
            relative_positions = (
                mint.arange(1, attn_weights.shape[-1] + 1)
                .unsqueeze(0)
                .unsqueeze(0)
        ...
        main_relative_pos_embeddings = mint.gather(rel_pos_embeddings, dim=1, index=main_relative_position_buckets)
    ...
    def get_predict_relative_pos_embeddings(
        ...
        if predict_relative_position_buckets is None:
            ...
            relative_positions = (
                mint.arange(0, key_sequence_length).unsqueeze(0).unsqueeze(0).repeat(batch_size, sequence_length, 1)
            )
        ...
        predict_relative_pos_embeddings = mint.gather(
...
class XLMProphetNetEncoder(XLMProphetNetPreTrainedModel):
    r"""
    word_embeddings  (`mint.mint.nn.Embeddings` of shape `(config.vocab_size, config.hidden_size)`, *optional*):
        The word embedding parameters. This can be used to initialize [`XLMProphetNetEncoder`] with pre-defined word
        embeddings instead of randomly initialized word embeddings.
    """

    def __init__(self, config: XLMProphetNetConfig, word_embeddings: mint.nn.Embedding = None):
        super().__init__(config)

        self.word_embeddings = (
            word_embeddings
            if word_embeddings is not None
            else mint.nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=config.pad_token_id)
        )
    ...
    def construct(
        ...
        >>> from mindone.transformers import XLMProphetNetEncoder
        >>> import mindspore as ms
        >>> import numpy as np
        ...
        hidden_states = mint.nn.functional.dropout(hidden_states, p=self.config.dropout, training=self.training)
...
class XLMProphetNetDecoder(XLMProphetNetPreTrainedModel):
    ...
    def __init__(self, config: XLMProphetNetConfig, word_embeddings: Optional[mint.nn.Embedding] = None):
        super().__init__(config)
        ...
        self.ngram_embeddings = mint.nn.Embedding(self.ngram, config.hidden_size, None)
    ...
    def construct(
        ...
        hidden_states = mint.cat([hidden_states] + ngram_hidden_states, 1)
    ...
    def compute_buffered_relative_buckets(self, position_ids):
        ...
        position_ids = mint.arange(1, self.max_target_positions).repeat(1, 1)
        ...
        predict_relative_buckets = mint.cat(
            [
    ...
    def prepare_attention_mask(self, hidden_states, attention_mask):
        ...
        causal_mask = mint.triu(causal_mask, 1)
    ...
    def prepare_predict_attention_mask(self, hidden_states, attention_mask):
        ...
        if attention_mask is not None:
            ...
            extended_attention_mask = mint.cat(
                [extended_attention_mask, mint.zeros_like(extended_attention_mask)], dim=-1
            )
...
class XLMProphetNetModel(XLMProphetNetPreTrainedModel):
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import XLMProphetNetModel
        >>> import mindspore as ms
...
class XLMProphetNetForConditionalGeneration(XLMProphetNetPreTrainedModel):
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import XLMProphetNetForConditionalGeneration
        >>> import mindspore as ms
    ...
    def _compute_loss(self, logits, labels, ignore_index=-100):
        ...
        lprobs = mint.nn.functional.log_softmax(
            logits.view(-1, logits.shape[-1]),
            dim=-1,
            dtype=ms.float32,
        )

        loss = mint.nn.functional.nll_loss(lprobs, expend_targets.view(-1), reduction="mean")
...
class XLMProphetNetForCausalLM(XLMProphetNetPreTrainedModel):
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import XLMProphetNetForCausalLM
        >>> import mindspore as ms
        >>> import numpy as np
    ...
    def _compute_loss(self, logits, labels, ignore_index=-100):
        ...
        logits = logits.transpose(0, 1).contiguous()
        lprobs = mint.nn.functional.Log_softmax(dim=-1)(
            logits.view(-1, logits.shape[-1]),
            dtype=ms.float32,
        )
