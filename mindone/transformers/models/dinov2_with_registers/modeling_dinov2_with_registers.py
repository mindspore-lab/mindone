...
class Dinov2WithRegistersPatchEmbeddings(nn.Cell):
    """
...
class Dinov2WithRegistersEmbeddings(nn.Cell):
    """
    ...
    def __init__(self, config: Dinov2WithRegistersConfig) -> None:
        ...
        self.mask_token = ms.Parameter(mint.zeros((1, config.hidden_size)), name="mask_token")
        ...
        self.position_embeddings = ms.Parameter(
            mint.randn(1, num_patches + 1, config.hidden_size), name="position_embeddings"
        )
    ...
    def interpolate_pos_encoding(self, embeddings: ms.Tensor, height: int, width: int) -> ms.Tensor:
        ...
        # FIXME: The absence of `antialias` parameter in MindSpore's `interpolate` operator may lead to significant accuracy degradation.  # noqa: E501
        patch_pos_embed = mint.nn.functional.interpolate(
            patch_pos_embed.to(dtype=ms.float32),
            size=(mindspore_int(height), mindspore_int(width)),  # Explicit size instead of scale_factor
            mode="bicubic",
            align_corners=False,
            # antialias=True,
    ...
    def construct(self, pixel_values: ms.Tensor, bool_masked_pos: Optional[ms.Tensor] = None) -> ms.Tensor:
        ...
        # add register tokens
        embeddings = mint.cat(
            (embeddings[:, :1], self.register_tokens.broadcast_to((embeddings.shape[0], -1, -1)), embeddings[:, 1:]),
            dim=1,
        )
...
class Dinov2WithRegistersSelfAttention(nn.Cell):
    def __init__(self, config: Dinov2WithRegistersConfig) -> None:
        super().__init__()
...
class Dinov2WithRegistersSelfOutput(nn.Cell):
    """
...
class Dinov2WithRegistersAttention(nn.Cell):
    def __init__(self, config: Dinov2WithRegistersConfig) -> None:
        super().__init__()
...
class Dinov2WithRegistersLayerScale(nn.Cell):
...
class Dinov2WithRegistersDropPath(nn.Cell):
    """Drop paths (Stochastic Depth) per sample (when applied in main path of residual blocks)."""
...
class Dinov2WithRegistersMLP(nn.Cell):
    def __init__(self, config) -> None:
        super().__init__()
...
class Dinov2WithRegistersSwiGLUFFN(nn.Cell):
    def __init__(self, config) -> None:
        super().__init__()
        in_features = out_features = config.hidden_size
        hidden_features = int(config.hidden_size * config.mlp_ratio)
...
class Dinov2WithRegistersLayer(nn.Cell):
    ...
    def __init__(self, config: Dinov2WithRegistersConfig) -> None:
        ...
        self.drop_path = (
            Dinov2WithRegistersDropPath(config.drop_path_rate) if config.drop_path_rate > 0.0 else mint.nn.Identity()
        )
...
class Dinov2WithRegistersEncoder(nn.Cell):
    def __init__(self, config: Dinov2WithRegistersConfig) -> None:
        super().__init__()
        self.config = config
        self.layer = nn.CellList([Dinov2WithRegistersLayer(config) for _ in range(config.num_hidden_layers)])
