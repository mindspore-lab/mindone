...
class UniSpeechGumbelVectorQuantizer(nn.Cell):
    ...
    def construct(self, hidden_states):
        ...
        if self.training:
            ...
            codevector_probs = hidden_states.new_zeros(hidden_states.shape).scatter(-1, codevector_idx.view(-1, 1), 1.0)
...
class UniSpeechForPreTraining(UniSpeechPreTrainedModel):
    def __init__(self, config: UniSpeechConfig):
        ...
        self.project_hid = mint.nn.Linear(config.proj_codevector_dim, config.hidden_size)

        self.ctc_proj = mint.nn.Linear(config.hidden_size, config.num_ctc_classes)
    ...
    def construct(
        ...
        prob_replace_matrix = mint.empty(transformer_features.shape[0], transformer_features.shape[1]).fill_(
            self.config.replace_prob
        )
        prob_replace_matrix = prob_replace_matrix.transpose(0, 1)
        sampled_replace_matrix = mint.bernoulli(prob_replace_matrix).bool()
...
class UniSpeechForCTC(UniSpeechPreTrainedModel):
    ...
    def construct(
        ...
        >>> from mindone.transformers.models.unispeech import UniSpeechForCTC
...
class UniSpeechForSequenceClassification(UniSpeechPreTrainedModel):
    ...
    def construct(
        ...
        >>> from datasets import load_dataset
        >>> from mindone.transformers.models.unispeech import UniSpeechForSequenceClassification
        >>> import mindspore as ms
