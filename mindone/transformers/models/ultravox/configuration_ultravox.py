    ...
    def __init__(
        ...
        **kwargs,
        ...
        if text_model_id is not None:
            self.text_config: transformers.LlamaConfig = transformers.AutoConfig.from_pretrained(text_model_id)
        else:
            text_config = text_config or {}
            self.text_config = transformers.CONFIG_MAPPING[text_config.get("model_type", "llama")](**text_config)

        if audio_model_id is not None:
            self.audio_config: transformers.PretrainedConfig = transformers.AutoConfig.from_pretrained(audio_model_id)
        else:
