...
class DataCollatorForSeq2SeqWithAudio(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features, *args, **kwargs):
        ...
        audio_token_start_idx = [x for f in features for x in f.pop("audio_token_start_idx", [])]
        ...
        batch["audio_token_start_idx"] = mint.stack(audio_token_start_idx)
        batch["audio_lens"] = mint.stack(audio_lens)
        batch["audio_token_len"] = mint.stack(audio_token_len)

        # Pad the last dimension of all audio_values to the same length, with 0s on the right.
        if audio_values:
            max_len = max([x.shape[-1] for x in audio_values])
            batch["audio_values"] = mint.stack([F.pad(x, (0, max_len - x.shape[-1])) for x in audio_values])
            if self.tokenizer.padding_side == "left":
                input_ids_lens = ms.Tensor([f["input_ids"].shape[-1] for f in features])
                displacement = batch["input_ids"].shape[-1] - input_ids_lens
                displacement = displacement.repeat_interleave(batch["audio_batch_size"].squeeze(-1))
                batch["audio_token_start_idx"] += displacement.to(batch["audio_token_start_idx"].device)
        return batch


class UltravoxProcessor(ProcessorMixin):
    """
    ...
    def __init__(
        ...
        assert tokenizer.eos_token is not None, "The tokenizer has no EOS token. Cannot recover."
    ...
    def _chunk_and_pad_audio(
        self,
        audio_values: ms.Tensor,
        audio_lens: ms.Tensor,
        ...
        Args:
            audio_values (ms.Tensor): A tensor of audio values (e.g., in B, D, T format).
            audio_lens (ms.Tensor): A tensor of audio lengths.
        Returns:
        ...
        """
        chunked_audio_values: List[ms.Tensor] = []
        ...
        for i in range(audio_values.shape[0]):  # iterate over the batch
            ...
            for offset in range(0, audio_lens[i], context_size):
                ...
                chunked_audio_lens.append(min(int(audio_lens[i].item()) - offset, context_size))
                is_continuation_list.append(is_continuation)

        data = {
            "audio_values": mint.stack(chunked_audio_values, dim=0),
            "audio_lens": ms.Tensor(chunked_audio_lens, dtype=ms.int64),
            "audio_is_continuation": ms.Tensor(is_continuation_list, dtype=ms.bool_),
            "audio_batch_size": ms.Tensor([len(chunked_audio_values)]),
        }
        if include_audio_num_chunks:
            data["audio_num_chunks"] = ms.Tensor(num_chunks, dtype=ms.int64)
        return data

    def __call__(
        self,
        text: Optional[str] = None,
        audio: Optional[Union[np.ndarray, ms.Tensor]] = None,
        audios: Optional[Union[List[Union[np.ndarray, ms.Tensor]], Union[np.ndarray, ms.Tensor]]] = None,
        ...
        **kwargs,
        ...
        Args:
            ...
            audio (`np.ndarray`, `ms.Tensor`, `List[np.ndarray]`, `List[ms.Tensor]`):
                The audio to be prepared. Audio can be a single-channel (1-dimensional) NumPy array or PyTorch tensor.
            audios (`np.ndarray`, `ms.Tensor`, `List[np.ndarray]`, `List[ms.Tensor]`):
        ...
        if len(audios) > 0:
            audios = [x.numpy() if isinstance(x, ms.Tensor) else x for x in audios]
            ...
            audios = [
                (np.pad(x, (0, 2 * hop_length - len(x)), mode="constant") if len(x) < 2 * hop_length else x)
                for x in audios
            ]

            # Main audio processing. The processor is model-specific.
            x: BatchFeature = self.audio_processor(
                audios,
            ...
            data.update(
                self._chunk_and_pad_audio(
                    audio_values=ms.tensor(x.input_features if "input_features" in x else x.input_values),
                    audio_lens=ms.tensor(x.attention_mask).sum(-1),
                    include_audio_num_chunks=include_audio_num_chunks,
                )
            )

            audio_is_continuation = data.pop("audio_is_continuation")
            data["audio_token_len"] = mint.ceil(data["audio_lens"] / (self.encoder_ds_factor * self.stack_factor)).to(
                dtype=ms.int32
            )
        ...
        if text is not None:
            ...
            tokenized_parts = self.tokenizer(
                text.split("<|audio|>"),  # The placeholder isn't part of the vocabulary, so split the text around it.
            ...
            if placeholder_index != len(split_input_ids) - 1:
                raise ValueError(f"Text contains too many audio placeholders. (Expected {len(audios)} placeholders)")
            input_ids.extend(split_input_ids[placeholder_index])

            if "audio_token_len" in data:
                data["audio_token_start_idx"] = ms.tensor(audio_token_start_idx)
