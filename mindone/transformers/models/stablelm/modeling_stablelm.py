...
class StableLmLayerNormPerHead(nn.Cell):
    def __init__(self, dim, num_heads, eps=1e-5, bias=False):
        ...
        self.norms = nn.CellList([mint.nn.LayerNorm(dim, eps=eps, bias=bias) for _ in range(self.num_heads)])
    ...
    def construct(self, hidden_states: ms.Tensor):
        ...
        states_per_heads = mint.split(hidden_states, 1, dim=1)
        # Normalize and merge the heads back together
        return mint.cat([norm(hidden_states) for norm, hidden_states in zip(self.norms, states_per_heads)], dim=1)
...
class StableLmAttention(nn.Cell):
    ...
    def __init__(self, config: StableLmConfig, layer_idx: Optional[int] = None):
        ...
        self.q_proj = mint.nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=config.use_qkv_bias)
        self.k_proj = mint.nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.use_qkv_bias)
        self.v_proj = mint.nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.use_qkv_bias)
        self.o_proj = mint.nn.Linear(self.hidden_size, self.hidden_size, bias=False)
    ...
    def construct(
        ...
        attn_weights = mint.nn.functional.softmax(attn_weights, dtype=ms.float32, dim=-1).to(query_states.dtype)
...
class StableLmFlashAttention2(StableLmAttention):
    ...
    def construct(
        ...
        **kwargs,
        ...
        elif self.is_causal and query_states.shape[-2] > 1:
...
class StableLmForSequenceClassification(StableLmPreTrainedModel):
    ...
    def construct(
        ...
        if labels is not None:
            ...
            loss = ForSequenceClassificationLoss(labels=labels, pooled_logits=pooled_logits, config=self.config)
        if not return_dict:
...
class StableLmForTokenClassification(StableLmPreTrainedModel):
    ...
    def construct(
        ...
        if labels is not None:
            ...
            loss = ForTokenClassification(logits, labels, self.config)
