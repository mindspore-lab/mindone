...
class StableLmAttention(nn.Cell):
    ...
    def __init__(self, config: StableLmConfig, layer_idx: Optional[int] = None):
        ...
        self.k_proj = mint.nn.Linear(
            self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.use_qkv_bias
        )
...
class StableLmSdpaAttention(StableLmAttention):
    def construct(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "StableLmModel is using StableLmSdpaAttention, but `scaled_dot_product_attention` does not support "
