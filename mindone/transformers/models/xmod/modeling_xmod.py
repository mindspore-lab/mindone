...
from transformers.models.xmod.configuration_xmod import XmodConfig
...
class XmodAdapter(nn.Cell):
    def __init__(self, config):
        ...
        self.dense1 = mint.nn.Linear(config.hidden_size, self.bottleneck_size)
        self.dense2 = mint.nn.Linear(self.bottleneck_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
...
class XmodOutput(nn.Cell):
    def __init__(self, config):
        ...
        if config.adapter_layer_norm:
            self.adapter_layer_norm = mint.nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        else:
    ...
    def construct(self, hidden_states: ms.Tensor, input_tensor: ms.Tensor, lang_ids: ms.Tensor) -> ms.Tensor:
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = hidden_states + input_tensor
    ...
    def lang_adapter(self, lang_ids: ms.Tensor, hidden_states: ms.Tensor):
        # Process subsequent samples with the same lang_id in parallel
        lang_ids, lang_lengths = mint.unique_consecutive(lang_ids, return_counts=True)
        ...
        split_hidden_states = mint.split(hidden_states, lang_lengths.tolist(), 0)
        lang_wise_outputs = []
        ...
        hidden_states = mint.cat(lang_wise_outputs, 0)
...
class XmodModel(XmodPreTrainedModel):
    ...
    def construct(
        ...
        if lang_ids is None:
            ...
            lang_ids = default_lang_id * mint.ones(batch_size)
...
class XmodForCausalLM(XmodPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs,
        ...
        past_key_values (`tuple(tuple(torch.FloatTensor))` of length `config.n_layers` with each tuple having 4 tensors of shape `(batch_size, num_heads, sequence_length - 1, embed_size_per_head)`):  # noqa: E501
            Contains precomputed key and value hidden states of the attention blocks. Can be used to speed up decoding.
        ...
        >>> from mindone.transformers import XmodForCausalLM
        >>> import mindspore as ms

        >>> tokenizer = AutoTokenizer.from_pretrained("FacebookAI/xlm-roberta-base")
        >>> config = AutoConfig.from_pretrained("facebook/xmod-base", revision="refs/pr/3")
        >>> config.is_decoder = True
        >>> model = XmodForCausalLM.from_pretrained("facebook/xmod-base", config=config, revision="refs/pr/3")
...
class XmodForMultipleChoice(XmodPreTrainedModel):
    ...
    def construct(
        ...
        flat_lang_ids = lang_ids.tile((input_ids.shape[0] * input_ids.shape[1])) if lang_ids is not None else None
