...
class BitGroupNormActivation(nn.GroupNorm):
    ...
    def construct(self, hidden_state):
        self._check_input_dim(ops.functional.shape(hidden_state), self.cls_name)
        self._check_dtype(hidden_state.dtype, [ms.float16, ms.float32], self.cls_name)
...
def pad(input, pad, mode='constant', value=0):
    ...
    if mode == 'replicate':
        if top > 0:
            output[:, :, :top, left:left + width] = input[:, :, 0:1, :].broadcast_to((batch_size, channels, top, width))
        if bottom > 0:
        ...
        if top > 0:
            output[:, :, :top, left:left + width] = input[:, :, 1:top + 1, :].flip(dims=[2]).broadcast_to(
                (batch_size, channels, top, width))
        if bottom > 0:
            output[:, :, top + height:, left:left + width] = input[:, :, -bottom - 1:-1, :].flip(dims=[2]).broadcast_to(
                (batch_size, channels, bottom, width))
        if left > 0:
            output[:, :, :, :left] = output[:, :, :, left + 1:2 * left + 1].flip(dims=[3]).broadcast_to(
                (batch_size, channels, padded_height, left))
        if right > 0:
