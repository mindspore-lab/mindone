...
class EfficientLoFTRRotaryEmbedding(mindspore.nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig):
        ...
        i_indices = mindspore.mint.ones((embed_height, embed_width)).cumsum(0).float().unsqueeze(-1)
        j_indices = mindspore.mint.ones((embed_height, embed_width)).cumsum(1).float().unsqueeze(-1)

        emb = mindspore.mint.zeros((1, embed_height, embed_width, self.config.hidden_size // 2))
