...
# from ...mindspore_utils import compile_compatible_method_lru_cache
from ...utils import ModelOutput, TransformersKwargs, can_return_tuple, mindspore_int
...
class KeypointMatchingOutput(ModelOutput):
    r"""
    matches (`ms.Tensor` of shape `(batch_size, 2, num_matches)`):
        Index of keypoint matched in the other image.
    matching_scores (`ms.Tensor` of shape `(batch_size, 2, num_matches)`):
        Scores of predicted matches.
    keypoints (`ms.Tensor` of shape `(batch_size, num_keypoints, 2)`):
        Absolute (x, y) coordinates of predicted keypoints in a given image.
    hidden_states (`tuple[ms.Tensor, ...]`, *optional*):
        Tuple of `ms.Tensor` (one for the output of each stage) of shape `(batch_size, 2, num_channels,
        num_keypoints)`, returned when `output_hidden_states=True` is passed or when
        `config.output_hidden_states=True`)
    attentions (`tuple[ms.Tensor, ...]`, *optional*):
        Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, 2, num_heads, num_keypoints,
...
def compute_embeddings(
    inv_freq: mindspore.Tensor, embed_height: int, embed_width: int, hidden_size: int
) -> mindspore.Tensor:
    i_indices = mindspore.mint.ones((embed_height, embed_width), dtype=inv_freq.dtype)
    j_indices = mindspore.mint.ones((embed_height, embed_width), dtype=inv_freq.dtype)
    i_indices = i_indices.cumsum(0).unsqueeze(-1)
    j_indices = j_indices.cumsum(1).unsqueeze(-1)

    emb = mindspore.mint.zeros((1, embed_height, embed_width, hidden_size // 2), dtype=inv_freq.dtype)
...
class EfficientLoFTRRotaryEmbedding(mindspore.nn.Cell):
    ...
    def __init__(self, config: EfficientLoFTRConfig):
        ...
        inv_freq, _ = self.rope_init_fn(self.config)
    ...
    @mindspore._no_grad()
    def construct(
        self, x: mindspore.Tensor, position_ids: Optional[tuple[mindspore.Tensor, mindspore.Tensor]] = None
...
class EfficientLoFTRConvNormLayer(mindspore.nn.Cell):
    def __init__(self, config, in_channels, out_channels, kernel_size, stride, padding=None, activation=None):
        ...
        self.norm = mindspore.mint.nn.BatchNorm2d(out_channels, config.batch_norm_eps)
        self.activation = mindspore.mint.nn.Identity() if activation is None else ACT2FN[activation]
...
class EfficientLoFTRRepVGGBlock(GradientCheckpointingLayer):
    ...
    def __init__(self, config: EfficientLoFTRConfig, stage_idx: int, block_idx: int):
        ...
        self.identity = (
            mindspore.mint.nn.BatchNorm2d(in_channels) if in_channels == out_channels and stride == 1 else None
        )
        self.activation = mindspore.mint.nn.Identity() if activation is None else ACT2FN[activation]
...
class EfficientLoFTRRepVGGStage(mindspore.nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig, stage_idx: int):
        super().__init__()
        self.blocks = mindspore.nn.CellList([])
...
class EfficientLoFTRepVGG(mindspore.nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig):
        super().__init__()
    ...
    def construct(self, hidden_states: mindspore.Tensor) -> list[mindspore.Tensor]:
        outputs = []
...
class EfficientLoFTRAggregationLayer(mindspore.nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig):
        super().__init__()

        hidden_size = config.hidden_size

        self.q_aggregation = mindspore.mint.nn.Conv2d(
            hidden_size,
            hidden_size,
        ...
        self.kv_aggregation = mint.nn.MaxPool2d(
            kernel_size=config.kv_aggregation_kernel_size, stride=config.kv_aggregation_stride
        )
        self.norm = mindspore.mint.nn.LayerNorm(hidden_size)
...
def rotate_half(x):
    ...
    x1 = x[..., ::2]
    x2 = x[..., 1::2]
    rot_x = mindspore.mint.stack([-x2, x1], dim=-1).flatten(-2)
    return rot_x
...
class EfficientLoFTRAttention(mindspore.nn.Cell):
...
class EfficientLoFTRMLP(mindspore.nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig):
        super().__init__()
        hidden_size = config.hidden_size
        intermediate_size = config.intermediate_size
        self.fc1 = mindspore.mint.nn.Linear(hidden_size * 2, intermediate_size, bias=False)
        self.activation = ACT2FN[config.mlp_activation_function]
        self.fc2 = mindspore.mint.nn.Linear(intermediate_size, hidden_size, bias=False)
        self.layer_norm = mindspore.mint.nn.LayerNorm(hidden_size)
...
class EfficientLoFTRAggregatedAttention(mindspore.nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig, layer_idx: int):
        super().__init__()
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        size = (aggregated_h * self.q_aggregation_kernel_size, aggregated_w * self.q_aggregation_kernel_size)
        attn_output = mindspore.mint.nn.functional.interpolate(
            attn_output, size=size, mode="bilinear", align_corners=False
        )
        intermediate_states = mindspore.mint.cat([hidden_states, attn_output], dim=1)
...
class EfficientLoFTRLocalFeatureTransformerLayer(GradientCheckpointingLayer):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        hidden_states = mindspore.mint.stack((features_0, features_1), dim=1)
        ###

        return hidden_states


class EfficientLoFTRLocalFeatureTransformer(mindspore.nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig):
        super().__init__()
...
class EfficientLoFTROutConvBlock(mindspore.nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig, hidden_size: int, intermediate_size: int):
        super().__init__()

        self.out_conv1 = mindspore.mint.nn.Conv2d(
            hidden_size, intermediate_size, kernel_size=1, stride=1, padding=0, bias=False
        )
        self.out_conv2 = mindspore.mint.nn.Conv2d(
            intermediate_size, intermediate_size, kernel_size=3, stride=1, padding=1, bias=False
        )
        self.batch_norm = mindspore.mint.nn.BatchNorm2d(intermediate_size)
        self.activation = ACT2FN[config.mlp_activation_function]
        self.out_conv3 = mindspore.mint.nn.Conv2d(
            intermediate_size, hidden_size, kernel_size=3, stride=1, padding=1, bias=False
        )

    def construct(self, hidden_states: mindspore.Tensor, residual_states: mindspore.Tensor) -> mindspore.Tensor:
        ...
        # TODO: MindSpore does not allow using the parameter combination of scale_factor and bilinear at the same time.
        size = [i * 2 for i in residual_states.shape[2:]]
        residual_states = mindspore.mint.nn.functional.interpolate(
            residual_states, size=size, mode="bilinear", align_corners=False
        )
        return residual_states


class EfficientLoFTRFineFusionLayer(mindspore.nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig):
        super().__init__()
    ...
    def forward_pyramid(
        self,
        hidden_states: mindspore.Tensor,
        residual_states: list[mindspore.Tensor],
    ) -> mindspore.Tensor:
        hidden_states = self.out_conv(hidden_states)
        # TODO: MindSpore does not allow using the parameter combination of scale_factor and bilinear at the same time.
        size = [i * 2 for i in hidden_states.shape[2:]]
        hidden_states = mindspore.mint.nn.functional.interpolate(
            hidden_states, size=size, mode="bilinear", align_corners=False
        )
    ...
    def construct(
        self,
        coarse_features: mindspore.Tensor,
        residual_features: list[mindspore.Tensor],
    ) -> tuple[mindspore.Tensor, mindspore.Tensor]:
        """
        ...
        fine_features_0 = mindspore.mint.nn.functional.unfold(
        ...
        fine_features_1 = mindspore.mint.nn.functional.unfold(
...
class EfficientLoFTRPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: mindspore.nn.Cell) -> None:
        ...
        if isinstance(
            module, (mindspore.mint.nn.Linear, mindspore.mint.nn.Conv2d, nn.Conv1d, mindspore.mint.nn.BatchNorm2d)
        ):
    ...
    def extract_one_channel_pixel_values(self, pixel_values: mindspore.Tensor) -> mindspore.Tensor:
        ...
        Args:
            pixel_values: ms.Tensor of shape (batch_size, 3, height, width)

        Returns:
            pixel_values: ms.Tensor of shape (batch_size, 1, height, width)
...
class EfficientLoFTRModel(EfficientLoFTRPreTrainedModel):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> url = \
        "https://github.com/magicleap/SuperGluePretrainedNetwork/blob/master/assets/phototourism_sample_images/london_bridge_78916675_4568141288.jpg?raw=true"
        >>> image1 = Image.open(requests.get(url, stream=True).raw)
        >>> url = \
        "https://github.com/magicleap/SuperGluePretrainedNetwork/blob/master/assets/phototourism_sample_images/london_bridge_19481797_2295892421.jpg?raw=true"
...
def mask_border(tensor: mindspore.Tensor, border_margin: int, value: Union[bool, float, int]) -> mindspore.Tensor:
    """
    Mask a tensor border with a given value

    Args:
        tensor (`ms.Tensor` of shape `(batch_size, height_0, width_0, height_1, width_1)`):
            The tensor to mask
        border_margin (`int`) :
...
def create_meshgrid(
    height: Union[int, mindspore.Tensor],
    width: Union[int, mindspore.Tensor],
    ...
    normalized to be in the range :math:`[-1,1]` to be consistent with the mindspore
    function :py:func:`mint.nn.functional.grid_sample`.
    ...
    Args:
        ...
        normalized_coordinates (`bool`):
            ...
            MindSpore function :py:func:`mint.nn.functional.grid_sample`.
        dtype (`ms.dtype`):
            The data type of the generated grid.

    Return:
        grid (`ms.Tensor` of shape `(1, height, width, 2)`):
            The grid tensor.
    ...
    """
    xs = mindspore.mint.linspace(0, width - 1, width, dtype=dtype)
    ys = mindspore.mint.linspace(0, height - 1, height, dtype=dtype)
    if normalized_coordinates:
        xs = (xs / (width - 1) - 0.5) * 2
        ys = (ys / (height - 1) - 0.5) * 2
    grid = mindspore.mint.stack(mindspore.mint.meshgrid(ys, xs, indexing="ij"), dim=-1)
    grid = grid.permute(1, 0, 2).unsqueeze(0)
    return grid


def spatial_expectation2d(input: mindspore.Tensor, normalized_coordinates: bool = True) -> mindspore.Tensor:
    r"""
    ...
    Args:
        input (`ms.Tensor` of shape `(batch_size, embed_dim, height, width)`):
    ...
    Returns:
        output (`ms.Tensor` of shape `(batch_size, embed_dim, 2)`)
    ...
    expected_y = mindspore.mint.sum(pos_y * input_flat, -1, keepdim=True)
    expected_x = mindspore.mint.sum(pos_x * input_flat, -1, keepdim=True)

    output = mindspore.mint.cat([expected_x, expected_y], -1)
...
class EfficientLoFTRForKeypointMatching(EfficientLoFTRPreTrainedModel):
    ...
    def _get_matches_from_scores(self, scores: mindspore.Tensor) -> tuple[mindspore.Tensor, mindspore.Tensor]:
        """
        ...
        Args:
            scores (`ms.Tensor` of shape `(batch_size, height_0, width_0, height_1, width_1)`):
                Scores of keypoints

        Returns:
            matched_indices (`ms.Tensor` of shape `(2, num_matches)`):
                Indices representing which pixel in the first image matches which pixel in the second image
            matching_scores (`ms.Tensor` of shape `(num_matches,)`):
        ...
        max_0 = scores.max(2, keepdim=True)[0]
        max_1 = scores.max(1, keepdim=True)[0]
        ...
        matching_indices = mindspore.mint.cat([max_indices_0, max_indices_1]).reshape(batch_size, 2, -1)
        matching_scores = mindspore.mint.stack([matching_scores_0, matching_scores_1], dim=1)

        # For the keypoints not meeting the threshold score, set the indices to -1 which corresponds to no matches found
        matching_indices = mindspore.mint.where(matching_scores > 0, matching_indices, -1)

        return matching_indices, matching_scores

    def _coarse_matching(
        self, coarse_features: mindspore.Tensor, coarse_scale: float
        ...
        * (image_width / 8 elements)) from the first image to the second image.
        ...
        Args:
            coarse_features (`ms.Tensor` of shape `(batch_size, 2, hidden_size, coarse_height, coarse_width)`):
                Coarse features
            coarse_scale (`float`): Scale between the image size and the coarse size

        Returns:
            keypoints (`ms.Tensor` of shape `(batch_size, 2, num_matches, 2)`):
                Keypoints coordinates.
            matching_scores (`ms.Tensor` of shape `(batch_size, 2, num_matches)`):
                The confidence matching score of each keypoint.
            matched_indices (`ms.Tensor` of shape `(batch_size, 2, num_matches)`):
        ...
        if self.config.coarse_matching_skip_softmax:
            ...
            confidence = mindspore.mint.nn.functional.softmax(similarity, 1) * mindspore.mint.nn.functional.softmax(
                similarity, 2
            )
        ...
        keypoints = mindspore.mint.stack([matched_indices % width, matched_indices // width], dim=-1) * coarse_scale

        return keypoints, matching_scores, matched_indices

    def _get_first_stage_fine_matching(
        self,
        fine_confidence: mindspore.Tensor,
        coarse_matched_keypoints: mindspore.Tensor,
        ...
        Args:
            fine_confidence (`ms.Tensor` of shape `(num_matches, fine_window_size, fine_window_size)`):
                First stage confidence of matching fine features between the first and the second image
            coarse_matched_keypoints (`ms.Tensor` of shape `(2, num_matches, 2)`):
        ...
        delta_0 = mindspore.mint.gather(grid, 1, indices_0.unsqueeze(-1).expand((-1, -1, -1, 2))).squeeze(2)
        delta_1 = mindspore.mint.gather(grid, 1, indices_1.unsqueeze(-1).expand((-1, -1, -1, 2))).squeeze(2)

        fine_matches_0 = coarse_matched_keypoints[:, 0] + delta_0 * fine_scale
        fine_matches_1 = coarse_matched_keypoints[:, 1] + delta_1 * fine_scale

        indices = mindspore.mint.stack([indices_0, indices_1], dim=1)
        fine_matches = mindspore.mint.stack([fine_matches_0, fine_matches_1], dim=1)
    ...
    def _get_second_stage_fine_matching(
        ...
        Args:
            indices (`ms.Tensor` of shape `(batch_size, 2, num_keypoints)`):
        ...
        batch_indices = mindspore.mint.arange(
            batch_size,
        ).reshape(batch_size, 1, 1, 1)
        matches_indices = mindspore.mint.arange(
            num_keypoints,
        ).reshape(1, num_keypoints, 1, 1)
        indices_0 = indices_0[..., None]
        indices_1_i = indices_1_i[..., None]
        indices_1_j = indices_1_j[..., None]

        delta = create_meshgrid(
            3,
            3,
        ...
        fine_confidence = mindspore.mint.nn.functional.softmax(
    ...
    def _fine_matching(
        self,
        fine_features_0: mindspore.Tensor,
        fine_features_1: mindspore.Tensor,
        coarse_matched_keypoints: mindspore.Tensor,
        fine_scale: float,
    ) -> mindspore.Tensor:
        """
        ...
        Args:
            fine_features_0 (`ms.Tensor` of shape `(num_matches, fine_kernel_size ** 2, fine_kernel_size ** 2)`):
                Fine features from the first image
            fine_features_1 (`ms.Tensor` of shape `(num_matches, (fine_kernel_size + 2) ** 2, (fine_kernel_size + 2)
            ** 2)`):
        ...
        Returns:
            fine_coordinates (`ms.Tensor` of shape `(2, num_matches, 2)`):
        ...
        split_fine_features_0 = mindspore.mint.split(fine_features_0, fine_embed_dim - fine_matching_slice_dim, -1)
        split_fine_features_1 = mindspore.mint.split(fine_features_1, fine_embed_dim - fine_matching_slice_dim, -1)
        ...
        fine_confidence = mindspore.mint.nn.functional.softmax(
            fine_confidence, 1
        ) * mindspore.mint.nn.functional.softmax(fine_confidence, 2)
        fine_confidence = fine_confidence.reshape(
            batch_size, num_keypoints, fine_window_size, fine_kernel_size + 2, fine_kernel_size + 2
        )
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        batch_indices = mindspore.mint.arange(batch_size)[..., None]
        ...
        fine_height = mindspore_int(coarse_height * coarse_scale)
