...
def compute_embeddings(inv_freq: ms.Tensor, embed_height: int, embed_width: int, hidden_size: int) -> ms.Tensor:
    i_indices = mint.ones((embed_height, embed_width), dtype=inv_freq.dtype)
    j_indices = mint.ones((embed_height, embed_width), dtype=inv_freq.dtype)
    i_indices = i_indices.cumsum(0).unsqueeze(-1)
    j_indices = j_indices.cumsum(1).unsqueeze(-1)

    emb = mint.zeros((1, embed_height, embed_width, hidden_size // 2), dtype=inv_freq.dtype)
...
class EfficientLoFTRRotaryEmbedding(nn.Cell):
    ...
    def __init__(self, config: EfficientLoFTRConfig):
        ...
        inv_freq_expanded = inv_freq[None, None, None, :].float().broadcast_to((1, 1, 1, -1))

        self.inv_freq = ms.Parameter(inv_freq_expanded, requires_grad=False, name="inv_freq")

    def construct(
        self, x: ms.Tensor, position_ids: Optional[tuple[ms.Tensor, ms.Tensor]] = None
        ...
        emb = compute_embeddings(self.inv_freq, embed_height, embed_width, self.config.hidden_size).float()
        sin = emb.sin()
        cos = emb.cos()
...
class EfficientLoFTRConvNormLayer(nn.Cell):
...
class EfficientLoFTRRepVGGBlock(GradientCheckpointingLayer):
    ...
    def __init__(self, config: EfficientLoFTRConfig, stage_idx: int, block_idx: int):
        ...
        self.identity = mint.nn.BatchNorm2d(in_channels) if in_channels == out_channels and stride == 1 else None
        self.activation = mint.nn.Identity() if activation is None else ACT2FN[activation]
...
class EfficientLoFTRRepVGGStage(nn.Cell):
...
class EfficientLoFTRepVGG(nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig):
        super().__init__()
    ...
    def construct(self, hidden_states: ms.Tensor) -> list[ms.Tensor]:
        outputs = []
...
class EfficientLoFTRAggregationLayer(nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig):
        super().__init__()

        hidden_size = config.hidden_size

        self.q_aggregation = mint.nn.Conv2d(
            hidden_size,
            hidden_size,
        ...
        self.norm = mint.nn.LayerNorm(hidden_size)
    ...
    def construct(
        ...
        if kv_states.dtype == ms.bfloat16:
            kv_states = self.kv_aggregation(kv_states.float()).to(ms.bfloat16)
        else:
...
class EfficientLoFTRAttention(nn.Cell):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        value_states = self.v_proj(current_states).view(batch_size, seq_len, -1, self.head_dim).swapaxes(1, 2)

        if position_embeddings is not None:
            cos, sin = position_embeddings
            query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, unsqueeze_dim=2)

        query_states = query_states.view(batch_size, seq_len, -1, self.head_dim).swapaxes(1, 2)
        key_states = key_states.view(batch_size, seq_len, -1, self.head_dim).swapaxes(1, 2)
...
class EfficientLoFTRMLP(nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig):
        super().__init__()
        hidden_size = config.hidden_size
        intermediate_size = config.intermediate_size
        self.fc1 = mint.nn.Linear(hidden_size * 2, intermediate_size, bias=False)
        self.activation = ACT2FN[config.mlp_activation_function]
        self.fc2 = mint.nn.Linear(intermediate_size, hidden_size, bias=False)
...
class EfficientLoFTRAggregatedAttention(nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig, layer_idx: int):
        super().__init__()
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        attn_output = mint.nn.functional.interpolate(
            attn_output,
            scale_factor=self.q_aggregation_kernel_size,
            mode="bilinear",
            align_corners=False,
            recompute_scale_factor=True,
        )
        intermediate_states = mint.cat([hidden_states, attn_output], dim=1)
...
class EfficientLoFTRLocalFeatureTransformerLayer(GradientCheckpointingLayer):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        hidden_states = mint.stack((features_0, features_1), dim=1)
        ###

        return hidden_states


class EfficientLoFTRLocalFeatureTransformer(nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig):
        super().__init__()
...
class EfficientLoFTROutConvBlock(nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig, hidden_size: int, intermediate_size: int):
        super().__init__()

        self.out_conv1 = mint.nn.Conv2d(hidden_size, intermediate_size, kernel_size=1, stride=1, padding=0, bias=False)
        self.out_conv2 = mint.nn.Conv2d(
            intermediate_size, intermediate_size, kernel_size=3, stride=1, padding=1, bias=False
        )
        self.batch_norm = mint.nn.BatchNorm2d(intermediate_size)
        self.activation = ACT2CLS[config.mlp_activation_function]()
        self.out_conv3 = mint.nn.Conv2d(intermediate_size, hidden_size, kernel_size=3, stride=1, padding=1, bias=False)

    def construct(self, hidden_states: ms.Tensor, residual_states: ms.Tensor) -> ms.Tensor:
        ...
        residual_states = mint.nn.functional.interpolate(
            residual_states, scale_factor=2.0, mode="bilinear", align_corners=False
        )
        return residual_states


class EfficientLoFTRFineFusionLayer(nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig):
        super().__init__()
        ...
        self.out_conv_layers = nn.CellList(self.out_conv_layers)

    def forward_pyramid(
        self,
        hidden_states: ms.Tensor,
        residual_states: list[ms.Tensor],
    ) -> ms.Tensor:
    ...
    def construct(
        self,
        coarse_features: ms.Tensor,
        residual_features: list[ms.Tensor],
    ) -> tuple[ms.Tensor, ms.Tensor]:
        """
        ...
        fine_features_0 = mint.nn.functional.unfold(
        ...
        fine_features_1 = mint.nn.functional.unfold(
...
class EfficientLoFTRPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: nn.Cell) -> None:
    ...
    def extract_one_channel_pixel_values(self, pixel_values: ms.Tensor) -> ms.Tensor:
        """
...
class EfficientLoFTRModel(EfficientLoFTRPreTrainedModel):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> url = "https://github.com/magicleap/SuperGluePretrainedNetwork/blob/master/assets/phototourism_sample_images/london_bridge_78916675_4568141288.jpg?raw=true"  # noqa: E501
        >>> image1 = Image.open(requests.get(url, stream=True).raw)
        >>> url = "https://github.com/magicleap/SuperGluePretrainedNetwork/blob/master/assets/phototourism_sample_images/london_bridge_19481797_2295892421.jpg?raw=true"  # noqa: E501
        ...
        cos = cos.broadcast_to((batch_size * 2, -1, -1, -1)).reshape(batch_size * 2, -1, coarse_embed_dim)
        sin = sin.broadcast_to((batch_size * 2, -1, -1, -1)).reshape(batch_size * 2, -1, coarse_embed_dim)
...
def mask_border(tensor: ms.Tensor, border_margin: int, value: Union[bool, float, int]) -> ms.Tensor:
    """
...
def create_meshgrid(
    height: Union[int, ms.Tensor],
    width: Union[int, ms.Tensor],
    ...
    """
    xs = mint.linspace(0, width - 1, width, dtype=dtype)
    ys = mint.linspace(0, height - 1, height, dtype=dtype)
    if normalized_coordinates:
        xs = (xs / (width - 1) - 0.5) * 2
        ys = (ys / (height - 1) - 0.5) * 2
    grid = mint.stack(mint.meshgrid(ys, xs, indexing="ij"), dim=-1)
    grid = grid.permute(1, 0, 2).unsqueeze(0)
    return grid


def spatial_expectation2d(input: ms.Tensor, normalized_coordinates: bool = True) -> ms.Tensor:
    r"""
    ...
    expected_y = mint.sum(pos_y * input_flat, -1, keepdim=True)
    expected_x = mint.sum(pos_x * input_flat, -1, keepdim=True)

    output = mint.cat([expected_x, expected_y], -1)
...
class EfficientLoFTRForKeypointMatching(EfficientLoFTRPreTrainedModel):
    ...
    def _get_matches_from_scores(self, scores: ms.Tensor) -> tuple[ms.Tensor, ms.Tensor]:
        """
        ...
        Note:
            ...
            However, we keep it in the modeling code for consistency with other keypoint matching models.
        ...
        matching_indices = mint.cat([max_indices_0, max_indices_1]).reshape(batch_size, 2, -1)
        matching_scores = mint.stack([matching_scores_0, matching_scores_1], dim=1)

        # For the keypoints not meeting the threshold score, set the indices to -1 which corresponds to no matches found
        matching_indices = mint.where(matching_scores > 0, matching_indices, -1)

        return matching_indices, matching_scores

    def _coarse_matching(
        self, coarse_features: ms.Tensor, coarse_scale: float
        ...
        * (image_width / 8 elements)) from the first image to the second image.
        ...
        similarity = coarse_features_0 @ coarse_features_1.swapaxes(-1, -2)
        similarity = similarity / self.config.coarse_matching_temperature

        if self.config.coarse_matching_skip_softmax:
            confidence = similarity
        else:
            confidence = mint.nn.functional.softmax(similarity, 1) * mint.nn.functional.softmax(similarity, 2)
        ...
        keypoints = mint.stack([matched_indices % width, matched_indices // width], dim=-1) * coarse_scale

        return keypoints, matching_scores, matched_indices

    def _get_first_stage_fine_matching(
        self,
        fine_confidence: ms.Tensor,
        coarse_matched_keypoints: ms.Tensor,
        ...
        values, indices = mint.max(fine_confidence, dim=-1)
        ...
        grid = grid.reshape(1, 1, -1, 2).broadcast_to((batch_size, num_keypoints, -1, -1))
        delta_0 = mint.gather(grid, 1, indices_0.unsqueeze(-1).broadcast_to((-1, -1, -1, 2))).squeeze(2)
        delta_1 = mint.gather(grid, 1, indices_1.unsqueeze(-1).broadcast_to((-1, -1, -1, 2))).squeeze(2)

        fine_matches_0 = coarse_matched_keypoints[:, 0] + delta_0 * fine_scale
        fine_matches_1 = coarse_matched_keypoints[:, 1] + delta_1 * fine_scale

        indices = mint.stack([indices_0, indices_1], dim=1)
        fine_matches = mint.stack([fine_matches_0, fine_matches_1], dim=1)
    ...
    def _get_second_stage_fine_matching(
        ...
        batch_indices = mint.arange(batch_size).reshape(batch_size, 1, 1, 1)
        matches_indices = mint.arange(num_keypoints).reshape(1, num_keypoints, 1, 1)
        indices_0 = indices_0[..., None]
        indices_1_i = indices_1_i[..., None]
        indices_1_j = indices_1_j[..., None]

        delta = create_meshgrid(3, 3, normalized_coordinates=True).to(ms.int64)
        delta = delta[None, ...]
        ...
        fine_confidence = mint.nn.functional.softmax(
    ...
    def _fine_matching(
        self,
        fine_features_0: ms.Tensor,
        fine_features_1: ms.Tensor,
        coarse_matched_keypoints: ms.Tensor,
        fine_scale: float,
    ) -> ms.Tensor:
        """
        ...
        split_fine_features_0 = mint.split(fine_features_0, fine_embed_dim - fine_matching_slice_dim, -1)
        split_fine_features_1 = mint.split(fine_features_1, fine_embed_dim - fine_matching_slice_dim, -1)
        ...
        fine_confidence = fine_features_0 @ fine_features_1.swapaxes(-1, -2)
        fine_confidence = mint.nn.functional.softmax(fine_confidence, 1) * mint.nn.functional.softmax(
            fine_confidence, 2
        )
        fine_confidence = fine_confidence.reshape(
            batch_size, num_keypoints, fine_window_size, fine_kernel_size + 2, fine_kernel_size + 2
        ...
        second_stage_fine_confidence = fine_features_0 @ fine_features_1.swapaxes(-1, -2)
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        batch_indices = mint.arange(batch_size)[..., None]
