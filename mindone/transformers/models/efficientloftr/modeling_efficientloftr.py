...
class EfficientLoFTRRotaryEmbedding(nn.Cell):
    def __init__(self, config: EfficientLoFTRConfig):
        ...
        i_indices = mint.ones((embed_height, embed_width)).cumsum(0).float().unsqueeze(-1)
        j_indices = mint.ones((embed_height, embed_width)).cumsum(1).float().unsqueeze(-1)

        emb = mint.zeros((1, embed_height, embed_width, self.config.hidden_size // 2))
