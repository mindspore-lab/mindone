...
class RoFormerSinusoidalPositionalEmbedding(mindspore.mint.nn.Embedding):
    ...
    def _init_weight(self):
        ...
        out = mindspore.mint.empty(n_pos, dim, dtype=self.weight.dtype)
        sentinel = dim // 2 if dim % 2 == 0 else (dim // 2) + 1
        out[:, 0:sentinel] = mindspore.Tensor(np.sin(position_enc[:, 0::2]))
        out[:, sentinel:] = mindspore.Tensor(np.cos(position_enc[:, 1::2]))
        self.weight = mindspore.Parameter(out)

    def construct(self, input_ids_shape: mindspore.ops.Size, past_key_values_length: int = 0) -> mindspore.Tensor:
...
class RoFormerEmbeddings(mindspore.nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.token_type_embeddings = mindspore.mint.nn.Embedding(config.type_vocab_size, config.embedding_size)

        # self.LayerNorm is not snake-cased to stick with TensorFlow model variable name and be able to load
        # any TensorFlow checkpoint file
        self.LayerNorm = mindspore.mint.nn.LayerNorm(config.embedding_size, eps=config.layer_norm_eps)
...
class RoFormerSelfAttention(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
    ...
    def apply_rotary_position_embeddings(sinusoidal_pos, query_layer, key_layer, value_layer=None):
        ...
        sin_pos = mindspore.mint.stack([sin, sin], dim=-1).reshape_as(sinusoidal_pos)
        # cos [θ0,θ1,θ2......θd/2-1] -> cos_pos [θ0,θ0,θ1,θ1,θ2,θ2......θd/2-1,θd/2-1]
        cos_pos = mindspore.mint.stack([cos, cos], dim=-1).reshape_as(sinusoidal_pos)
        # rotate_half_query_layer [-q1,q0,-q3,q2......,-qd-1,qd-2]
        rotate_half_query_layer = mindspore.mint.stack(
            [-query_layer[..., 1::2], query_layer[..., ::2]], dim=-1
        ).reshape_as(query_layer)
        query_layer = query_layer * cos_pos + rotate_half_query_layer * sin_pos
        # rotate_half_key_layer [-k1,k0,-k3,k2......,-kd-1,kd-2]
        rotate_half_key_layer = mindspore.mint.stack([-key_layer[..., 1::2], key_layer[..., ::2]], dim=-1).reshape_as(
            key_layer
        )
        key_layer = key_layer * cos_pos + rotate_half_key_layer * sin_pos
        if value_layer is not None:
            # rotate_half_value_layer [-v1,v0,-v3,v2......,-vd-1,vd-2]
            rotate_half_value_layer = mindspore.mint.stack(
...
class RoFormerSelfOutput(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class RoFormerAttention(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class RoFormerIntermediate(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class RoFormerOutput(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class RoFormerLayer(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class RoFormerEncoder(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        ...
        self.layer = mindspore.nn.CellList([RoFormerLayer(config) for _ in range(config.num_hidden_layers)])
...
class RoFormerPredictionHeadTransform(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.dense = mindspore.mint.nn.Linear(config.hidden_size, config.embedding_size)
        if isinstance(config.hidden_act, str):
...
class RoFormerLMPredictionHead(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        ...
        self.decoder = mindspore.mint.nn.Linear(config.embedding_size, config.vocab_size, bias=False)
...
class RoFormerOnlyMLMHead(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class RoFormerModel(RoFormerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.embedding_size != config.hidden_size:
            self.embeddings_project = mindspore.mint.nn.Linear(config.embedding_size, config.hidden_size)
    ...
    ) -> Union[BaseModelOutputWithPastAndCrossAttentions, Tuple[mindspore.Tensor]]:
        r"""
...
class RoFormerForMaskedLM(RoFormerPreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, input_ids, attention_mask=None, **model_kwargs):
        ...
        dummy_token = mindspore.ops.full(
            (effective_batch_size, 1),
            self.config.pad_token_id,
            dtype=mindspore.int64,
        )
        input_ids = mindspore.mint.cat([input_ids, dummy_token], dim=1)
...
class RoFormerForCausalLM(RoFormerPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs,
        ...
        ```python
        >>> from transformers import AutoTokenizer, RoFormerConfig
        >>> from mindone.transformers import RoFormerForCausalLM
        ...
        >>> inputs = tokenizer("今天天气非常好。", return_tensors="np")
...
class RoFormerClassificationHead(mindspore.nn.Cell):
