...
UDOP_START_DOCSTRING = r"""
    ...
    This model is also a MindSpore [mindspore.nn.Cell](https://www.mindspore.cn/docs/en/master/api_python/nn/mindspore.nn.Cell.html#mindspore.nn.Cell) subclass.
...
UDOP_INPUTS_DOCSTRING = r"""
    Args:
        ...
        bbox (`mindspore.Tensor` of shape `({0}, 4)`, *optional*):
        ...
        visual_bbox (`mindspore.Tensor` of shape `(batch_size, patch_sequence_length, 4)`, *optional*):
...
def get_visual_bbox(image_size=224, patch_size=16):
    ...
    visual_bbox_x = mint.arange(0, 1.0 * (image_feature_pool_shape[1] + 1), 1.0)
    visual_bbox_x /= image_feature_pool_shape[1]

    visual_bbox_y = mint.arange(0, 1.0 * (image_feature_pool_shape[0] + 1), 1.0)
    visual_bbox_y /= image_feature_pool_shape[0]

    visual_bbox_input = mint.stack(
        [
...
def combine_image_text_embeddings(
    ...
    ocr_points_x = mint.clamp(
        mint.floor((bbox[:, :, 0] + bbox[:, :, 2]) / 2.0 * sequence_length).long(), 0, sequence_length - 1
    )
    ocr_points_y = (
        mint.clamp(mint.floor((bbox[:, :, 1] + bbox[:, :, 3]) / 2.0 * sequence_length).long(), 0, sequence_length - 1)
        * sequence_length
    )
    ocr_points = ocr_points_x + ocr_points_y
    # make sure bounding boxes are of type float to calculate means
    bbox = bbox.to(ms.float32)
    target_seg = (bbox.mean(-1) == 0.0) | (bbox.mean(-1) == 1.0)
    repeated_vision_embeds = mint.gather(
    ...
    inputs_vision_patches = mint.stack(
        [pad_sequence(item, max_len, mint.zeros_like(image_embeddings[0, 0])) for item in input_vision_patches]
    )
    visual_bbox = mint.stack([pad_sequence(item, max_len, mint.zeros_like(bbox[0, 0])) for item in visual_bbox])
    if attention_mask is not None:
        visual_attention_mask = mint.stack(
            [pad_sequence(item, max_len, mint.zeros_like(attention_mask[0, 0])) for item in visual_attention_mask]
        )

    inputs_embeds = mint.cat([inputs_embeds, inputs_vision_patches], 1)
    bbox = mint.cat([bbox, visual_bbox], 1)
    if attention_mask is not None:
        attention_mask = mint.cat([attention_mask, visual_attention_mask], 1)
    return inputs_embeds, bbox, attention_mask


class UdopPatchEmbeddings(ms.nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.proj = mint.nn.Conv2d(num_channels, hidden_size, kernel_size=patch_size, stride=patch_size)
...
class UdopLayerNorm(ms.nn.Cell):
...
class UdopDenseActDense(ms.nn.Cell):
...
class UdopDenseGatedActDense(ms.nn.Cell):
    def __init__(self, config: UdopConfig):
        super().__init__()
...
class UdopAttention(ms.nn.Cell):
    def __init__(
        self,
        config: UdopConfig,
...
class UdopLayerSelfAttention(ms.nn.Cell):
...
class UdopLayerCrossAttention(ms.nn.Cell):
    ...
    def construct(
        ...
        if hidden_states.dtype == ms.float16:
            clamp_value = mint.where(
                ...
                _DTYPE_2_MAX[hidden_states.dtype],
            )
...
class UdopCellEmbeddings(ms.nn.Cell):
    def __init__(self, max_2d_position_embeddings=501, hidden_size=1024):
        ...
        self.x_position_embeddings = mint.nn.Embedding(max_2d_position_embeddings, hidden_size)
        self.y_position_embeddings = mint.nn.Embedding(max_2d_position_embeddings, hidden_size)

    def construct(self, bbox):
        bbox = mint.clamp(bbox, 0.0, 1.0)
...
class RelativePositionBiasBase(ms.nn.Cell, ABC):
    """
    ...
    def get_relative_position(self, positions):
        ...
        return relative_position.to(ms.int64)
    ...
    def construct(self, attention_mask: Optional[Tensor] = None, bbox: Optional[Dict[str, Any]] = None) -> Tensor:
        ...
        if self.expand and self.prefix_bucket:
            new_bias = mint.nn.Embedding(self.relative_attention_num_buckets + 2, self.num_heads)
...
class RelativePositionBiasAggregated(ms.nn.Cell):
    def __init__(self, modules: Sequence[RelativePositionBiasBase]):
        ...
        """
        super().__init__()
        self.biases = ms.nn.CellList(modules)
...
class UdopStack(UdopPreTrainedModel):
    ...
    def construct(
        ...
        elif input_ids is not None and ms.ops.numel(input_ids) > 0:
            input_shape = input_ids.shape
            input_ids = input_ids.view(-1, input_shape[-1])
        elif inputs_embeds is None and input_ids is not None and ms.ops.numel(input_ids) == 0:
            input_ids = ms.ops.full((4, 1024), self.config.pad_token_id, dtype=input_ids.dtype)
            attention_mask = mint.zeros((4, 1024), dtype=input_ids.dtype)
            bbox = mint.zeros((4, 1024, 4), dtype=input_ids.dtype)
            input_shape = input_ids.shape
            position_bias = mint.zeros_like(self.get_extended_attention_mask(attention_mask, input_shape))
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        **kwargs,
        ...
        Args:
            ...
            device (`mindspore device context`):
...
class UdopModel(UdopPreTrainedModel):
    ...
    def construct(
        ...
        >>> inputs = processor(image, words, boxes=boxes, return_tensors="np")

        >>> # convert numpy arrays to MindSpore tensors
...
class UdopEncoderModel(UdopPreTrainedModel):
    ...
    def construct(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[ms.Tensor], BaseModelOutputWithAttentionMask]:
        r"""
        Returns:
