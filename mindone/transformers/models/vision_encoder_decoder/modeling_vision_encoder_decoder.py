...
class VisionEncoderDecoderModel(PreTrainedModel, GenerationMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        >>> model = VisionEncoderDecoderModel.from_pretrained("ydshieh/vit-gpt2-coco-en", revision="refs/pr/4")

        >>> url = "http://images.cocodataset.org/val2017/000000039769.jpg"
        >>> img = Image.open(requests.get(url, stream=True).raw)
        >>> pixel_values = ms.tensor(image_processor(images=img, return_tensors="np").pixel_values)  # Batch size 1
    ...
    def construct(
        ...
        **kwargs,
        ...
        >>> pixel_values = ms.tensor(processor(image, return_tensors="np").pixel_values)
        >>> text = "hello world"
        >>> labels = ms.tensor(processor.tokenizer(text, return_tensors="np").input_ids)
