...
class DetrFrozenBatchNorm2d(nn.Cell):
    ...
    Copy-paste from mindsporevision.misc.ops with added eps before rqsrt, without which any other models than
...
def replace_batch_norm(model, meta=False):
    r"""
    Recursively replace all `mint.nn.BatchNorm2d` with `DetrFrozenBatchNorm2d`.
...
class DetrConvEncoder(nn.Cell):
    ...
    mint.nn.BatchNorm2d layers are replaced by DetrFrozenBatchNorm2d as defined above.
...
class DetrLearnedPositionEmbedding(nn.Cell):
    ...
    def construct(self, pixel_values, pixel_mask=None):
        ...
        width_values = mint.arange(width, device=pixel_values.device)
        height_values = mint.arange(height, device=pixel_values.device)
...
class DetrPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, DetrMHAttentionMap):
            zeros_(module.k_linear.bias)
            zeros_(module.q_linear.bias)
            xavier_uniform_(module.k_linear.weight, gain=xavier_std)
            xavier_uniform_(module.q_linear.weight, gain=xavier_std)
...
class DetrEncoder(DetrPreTrainedModel):
    ...
    - object_queries are added to the construct pass.
...
class DetrDecoder(DetrPreTrainedModel):
    ...
    - object_queries and query_position_embeddings are added to the construct pass.
...
class DetrModel(DetrPreTrainedModel):
    ...
    def construct(
    ...
    ) -> Union[Tuple[ms.Tensor], DetrModelOutput]:
        r"""
        Returns:
        ...
        >>> from mindone.transformers import DetrModel
        >>> import mindspore as ms
        ...
        if pixel_mask is None:
            pixel_mask = mint.ones(((batch_size, height, width)), device=device)
...
class DetrForObjectDetection(DetrPreTrainedModel):
    ...
    def construct(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[ms.Tensor], DetrObjectDetectionOutput]:
        r"""
        ...
        ```python
        >>> from transformers import AutoImageProcessor
        >>> from mindone.transformers import DetrForObjectDetection
        >>> import mindspore as ms
        >>> from PIL import Image
        >>> import requests
...
class DetrForSegmentation(DetrPreTrainedModel):
    ...
    def construct(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[ms.Tensor], DetrSegmentationOutput]:
        r"""
        labels (`List[Dict]` of len `(batch_size,)`, *optional*):
        ...
        Examples:
        Note that to use safetensors checkpoint, need to download model.safetensors from
        https://huggingface.co/facebook/detr-resnet-50-panoptic/tree/12df956224e66b0faed42e288f43704ddab668ce
        ```python
        >>> import io
        >>> import requests
        >>> from PIL import Image
...
class DetrMHAttentionMap(nn.Cell):
    ...
    def construct(self, q, k, mask: Optional[Tensor] = None):
        ...
        if mask is not None:
            weights = weights.masked_fill(mask.unsqueeze(1).unsqueeze(1), dtype_to_min(weights.dtype).item())
