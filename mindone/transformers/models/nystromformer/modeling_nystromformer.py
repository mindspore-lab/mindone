...
from transformers.models.nystromformer.configuration_nystromformer import NystromformerConfig
...
class NystromformerEmbeddings(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.position_embeddings = mint.nn.Embedding(config.max_position_embeddings + 2, config.hidden_size)
        ...
        self.register_buffer(
            "position_ids", mint.arange(config.max_position_embeddings).broadcast_to((1, -1)) + 2, persistent=False
        )
...
class NystromformerSelfAttention(nn.Cell):
    ...
    def iterative_inv(self, mat, n_iter=6):
        identity = mint.eye(mat.shape[-1])
        key = mat

        # The entries of key are positive and ||key||_{\infty} = 1 due to softmax
        if self.init_option == "original":
            # This original implementation is more conservative to compute coefficient of Z_0.
            value = 1 / mint.max(mint.sum(key, dim=-2)) * key.swapaxes(-1, -2)
        else:
            # This is the exact coefficient computation, 1 / ||key||_1, of initialization of Z_0, leading to faster convergence.
            value = 1 / mint.max(mint.sum(key, dim=-2), dim=-1).values[:, :, None, None] * key.swapaxes(-1, -2)

        for _ in range(n_iter):
            key_value = mint.matmul(key, value)
            value = mint.matmul(
                0.25 * value,
                13 * identity
                - mint.matmul(key_value, 15 * identity - mint.matmul(key_value, 7 * identity - key_value)),
            )
        return value
    ...
    def construct(self, hidden_states, attention_mask=None, output_attentions=False):
        ...
        if self.num_landmarks == self.seq_len:
            ...
            kernel_1 = mint.nn.functional.softmax(mint.matmul(query_layer, k_landmarks.swapaxes(-1, -2)), dim=-1)
            kernel_2 = mint.nn.functional.softmax(mint.matmul(q_landmarks, k_landmarks.swapaxes(-1, -2)), dim=-1)

            attention_scores = mint.matmul(q_landmarks, key_layer.swapaxes(-1, -2))

            if attention_mask is not None:
                # Apply the attention mask is (precomputed for all layers in NystromformerModel forward() function)
                attention_scores = attention_scores + attention_mask

            kernel_3 = mint.nn.functional.softmax(attention_scores, dim=-1)
            attention_probs = mint.matmul(kernel_1, self.iterative_inv(kernel_2))
            new_value_layer = mint.matmul(kernel_3, value_layer)
            context_layer = mint.matmul(attention_probs, new_value_layer)
...
class NystromformerLMPredictionHead(nn.Cell):
    def __init__(self, config):
        ...
        self.bias = Parameter(mint.zeros(config.vocab_size))
...
NYSTROMFORMER_START_DOCSTRING = r"""
    This model is a Mindspore [mindspore.nn.Cell](https://www.mindspore.cn/) sub-class. Use
...
class NystromformerForMultipleChoice(NystromformerPreTrainedModel):
    def __init__(self, config):
        ...
        self.nystromformer = NystromformerModel(config)
        self.pre_classifier = mint.nn.Linear(config.hidden_size, config.hidden_size)
