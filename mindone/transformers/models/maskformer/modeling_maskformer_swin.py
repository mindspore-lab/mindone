...
class MaskFormerSwinEmbeddings(nn.Cell):
    """
    ...
    def construct(self, pixel_values, interpolate_pos_encoding):
...
class MaskFormerSwinPatchEmbeddings(nn.Cell):
    """
...
class MaskFormerSwinPatchMerging(nn.Cell):
    """
    Patch Merging Layer.
...
class MaskFormerSwinDropPath(nn.Cell):
    """Drop paths (Stochastic Depth) per sample (when applied in main path of residual blocks)."""
...
class MaskFormerSwinSelfAttention(nn.Cell):
...
class MaskFormerSwinSelfOutput(nn.Cell):
...
class MaskFormerSwinAttention(nn.Cell):
    def __init__(self, config, dim, num_heads, window_size):
        super().__init__()
...
class MaskFormerSwinIntermediate(nn.Cell):
    def __init__(self, config, dim):
        super().__init__()
...
class MaskFormerSwinOutput(nn.Cell):
    def __init__(self, config, dim):
        super().__init__()
...
class MaskFormerSwinLayer(nn.Cell):
    def __init__(self, config, dim, input_resolution, num_heads, drop_path_rate=0.0, shift_size=0):
        ...
        self.drop_path = MaskFormerSwinDropPath(drop_path_rate) if drop_path_rate > 0.0 else mint.nn.Identity()
    ...
    def get_attn_mask(self, input_resolution):
        if self.shift_size > 0:
            ...
            img_mask = mint.zeros((1, height, width, 1))
            height_slices = (
    ...
    def construct(self, hidden_states, input_dimensions, head_mask=None, output_attentions=False):
        height, width = input_dimensions
...
class MaskFormerSwinStage(nn.Cell):
...
class MaskFormerSwinEncoder(nn.Cell):
...
class MaskFormerSwinBackbone(MaskFormerSwinPreTrainedModel, BackboneMixin):
    ...
    def __init__(self, config: MaskFormerSwinConfig):
        ...
        self.hidden_states_norms = nn.CellList(
            [mint.nn.LayerNorm(num_channels) for num_channels in self.num_features[1:]]
        )
