...
from ....models.utils import constant_, xavier_uniform_
...
class EncodecConv1d(nn.Cell):
    ...
    def __init__(
        ...
        self.conv = nn.Conv1d(
            in_channels, out_channels, kernel_size, stride, dilation=dilation, has_bias=True, pad_mode="valid"
        )
        ...
        stride = Tensor(self.conv.stride[1], dtype=mindspore.int64)
        dilation = self.conv.dilation[1]
    ...
    def _pad1d(hidden_states: Tensor, paddings: Tuple[int, int], mode: str = "zero", value: float = 0.0):
        ...
        if isinstance(paddings[0], Tensor):
            paddings = [t.tolist() for t in paddings]
...
class EncodecModel(EncodecPreTrainedModel):
    ...
    def construct(
        ...
        >>> from mindone.transformers.models.encodec import EncodecModel
