...
from ...mindspore_adapter import dtype_to_min, unfold
...
def scatter_reduce(input, dim, index, src, reduce, include_self=True, out=None):
    ...
    if reduce == "sum":
        ...
        if not include_self:
            src_max = mint.full_like(input, -float("inf"))
            updated_mask = mint.zeros_like(input, dtype=ms.bool_)
        else:
            src_max = output.clone()

        for i in range(src.shape[dim]):
            src_slice = src.select(dim, i)
            idx_slice = index.select(dim, i)
            temp = mint.full_like(src_max, -float("inf"))

            if dim == 0:
                temp.scatter_(dim, idx_slice.unsqueeze(0), src_slice.unsqueeze(0))
            elif dim == 1:
                temp.scatter_(dim, idx_slice.unsqueeze(0), src_slice.unsqueeze(0))
            else:
                temp.scatter_(dim, idx_slice, src_slice)

            src_max = mint.max(src_max, temp)

            if not include_self:
                mask_update = mint.zeros_like(updated_mask, dtype=ms.float32)

                if dim == 0:
                    mask_update.scatter_(
                        dim, idx_slice.unsqueeze(0), mint.ones_like(src_slice, dtype=ms.float32).unsqueeze(0)
                    )
                elif dim == 1:
                    mask_update.scatter_(
                        dim, idx_slice.unsqueeze(0), mint.ones_like(src_slice, dtype=ms.float32).unsqueeze(0)
                    ...
                    mask_update.scatter_(dim, idx_slice, mint.ones_like(src_slice, dtype=ms.float32))

                updated_mask |= mask_update.bool()

        if not include_self:
            output = mint.where(updated_mask, src_max, input)
        else:
            output = src_max
    elif reduce == "amin":
        if not include_self:
            output = mint.full_like(input, float("inf"))
            updated_mask = mint.zeros_like(input, dtype=ms.bool_)

        for i in range(src.shape[dim]):
            src_slice = src.select(dim, i)
            idx_slice = index.select(dim, i)
            temp = mint.full_like(output, float("inf"))
...
class BltRotaryEmbedding(nn.Cell):
...
class BltSelfAttention(nn.Cell):
...
class BltCrossAttention(nn.Cell):
...
class BltLocalEncoder(BltPreTrainedModel):
    ...
    def __init__(self, config: BltLocalEncoderConfig):
        ...
        self.patch_embedding_projection = mint.nn.Linear(
        ...
        self.cross_attn_layers = nn.CellList(self.cross_attn_layers)
    ...
    def construct(
        ...
        patch_embeds: Optional[ms.Tensor] = None,
        attention_mask: Optional[ms.Tensor] = None,
        position_ids: Optional[ms.Tensor] = None,
        past_key_values: Optional[Cache] = None,
        cache_position: Optional[ms.Tensor] = None,
        ...
        patch_ids: Optional[ms.Tensor] = None,
        **kwargs: Unpack[TransformersKwargs],
    ):
        ...
        if position_ids is None:
            position_ids = mint.arange(inputs_embeds.shape[1]).unsqueeze(0).broadcast_to((batch_size, -1))
    ...
    def patch_reduce(self, hidden_states, max_num_patches, patch_ids):
        ...
        patch_ids = patch_ids.unsqueeze(-1).broadcast_to((-1, -1, hidden_states.shape[-1]))

        reduced_embeddings = mint.zeros((batch_size, max_num_patches, embedding_dim), dtype=hidden_states.dtype)

        reduced_embeddings = scatter_reduce(
            reduced_embeddings,
            src=hidden_states,
            dim=1,
            index=patch_ids,
...
class BltGlobalTransformer(BltPreTrainedModel):
    ...
    def __init__(self, config: BltGlobalTransformerConfig):
        ...
        # Create token embedding projection (use mint.nn.Identity() when no projection needed)
        if getattr(config, "encoder_cross_output_size", None) is not None:
            self.token_embedding_projection = mint.nn.Linear(
                config.encoder_cross_output_size, config.hidden_size, bias=False
            )
        else:
            self.token_embedding_projection = mint.nn.Identity()
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        hidden_states = F.dropout(hidden_states, p=self.config.dropout, training=self.training)
        if position_ids is None:
            position_ids = mint.arange(input_embeds.shape[1]).unsqueeze(0).broadcast_to((batch_size, -1))
        position_embeddings = self.rotary_emb(hidden_states, position_ids)
        for i, layer in enumerate(self.layers):
            hidden_states = layer(
                hidden_states,
...
def process_patch_lengths(patch_lengths: ms.Tensor, max_patch_length: Optional[int]) -> ms.Tensor:
    """
    ...
    Args:
        patch_lengths (ms.Tensor): [batch_size, num_patches] tensor of patch lengths.
        max_patch_length (int, optional): Maximum allowed length per patch.

    Returns:
        ms.Tensor: [batch_size, max_len] tensor of split and padded patch lengths.
    """
    ...
    padded = mint.zeros((batch_size, max_len), dtype=patch_lengths.dtype)

    for i, splits in enumerate(processed):
        if splits:
            padded[i, : len(splits)] = ms.tensor(splits, dtype=patch_lengths.dtype)
...
class BltPatcher(BltPreTrainedModel):
    ...
    def __init__(self, config: BltPatcherConfig):
        ...
        self.layers = nn.CellList(self.layers)
        self.embed_tokens = mint.nn.Embedding(self.config.vocab_size, self.config.hidden_size)
        self.norm = BltRMSNorm(self.config.hidden_size, eps=self.config.rms_norm_eps)
        self.lm_head = mint.nn.Linear(self.config.hidden_size, self.config.vocab_size, bias=False)
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        prediction_entropies = -mint.sum(probs * log_probs, dim=-1)
        ...
        if patch_size is not None:
            ...
            patch_lengths = mint.ones((batch_size, sequence_length), dtype=inputs_embeds.dtype)
    ...
    def patch_lengths_from_entropies(
        ...
        init_tokens = ms.tensor([0, 1], dtype=ms.int64).unsqueeze(0).tile((batch_size, 1))
        ...
        token_indices = mint.arange(seq_len).unsqueeze(0).broadcast_to((batch_size, -1))
        sentinel = mint.full_like(token_indices, seq_len)
        padded_indices = mint.cat([token_indices, sentinel], dim=1)

        # Pad mask with inverse to align sentinel correctly
        padded_mask = mint.cat([patch_mask, ~patch_mask], dim=1)
        ...
        patch_start_ids = mint.cat((init_tokens, patch_starts + offset), dim=1)

        # Compute patch end positions by shifting start positions
        last_token = mint.full_like(patch_start_ids[:, :1], sequence_length - 1)
        patch_ends = mint.cat((patch_start_ids[:, 1:] - 1, last_token), dim=1)
...
def rolling_polynomial_hash(token_tensor, prime: int = 1000000007):
    ...
    Args:
        token_tensor (ms.Tensor): [batch_size, seq_len, group_size] containing token IDs to hash
        prime (int): Prime number used as the base for the polynomial hash.

    Returns:
        ms.Tensor: Hash values of shape [batch_size, seq_len] where each value
                     represents the hash of the corresponding token group

    Example:
        >>> tokens = ms.tensor([[1, 2, 3], [4, 5, 6]])
        >>> hashes = rolling_polynomial_hash(tokens, prime=31)
        >>> # hash[0] = 1*31^0 + 2*31^1 + 3*31^2
        >>> # hash[1] = 4*31^0 + 5*31^1 + 6*31^2
    """
    prime_tensor = ms.tensor(prime, dtype=ms.int64)
    powers = mint.arange(token_tensor.shape[-1])
    prime_powers = prime_tensor**powers
    return mint.sum(token_tensor * prime_powers, dim=-1)


def byte_group_hash_function(token_ids: ms.Tensor, group_size: int = 2, prime: int = 1000000007, max_hash: int = 30000):
    """Hash token groups and map to range [0, max_hash]."""
    batch_size, seq_len = token_ids.shape
    # Add padding for sliding window
    padding = mint.zeros((batch_size, group_size - 1), dtype=ms.int64)
    padded_tokens = mint.cat([padding.to(token_ids.dtype), token_ids], dim=1)

    # Create sliding windows and compute hashes
    windows = unfold(padded_tokens, 1, group_size, 1)
...
def compute_hash_embeddings(
    local_encoder_tokens: ms.Tensor,
    local_encoder,
    encoder_hash_tok_embedding: mint.nn.Embedding,
...
def _prepare_patch_cross_attention_mask(
    ...
    Args:
        patch_ids (ms.Tensor): Tensor of shape [batch_size, seq_len] containing patch ids.
        ...
        dtype (ms.Type): Data type for the output mask.
    ...
    if patches_as_queries:
        ...
        q_patch_ids = (
            mint.arange(num_patches).unsqueeze(0).unsqueeze(-1).broadcast_to((batch_size, num_patches, seq_len))
        )
        kv_patch_ids = patch_ids.unsqueeze(1).broadcast_to((batch_size, num_patches, seq_len))
    else:
        q_len = sequence_length
        kv_len = num_patches * cross_attn_k
        # Create sequence-to-patch mapping
        q_patch_ids = patch_ids.unsqueeze(-1).broadcast_to((batch_size, seq_len, num_patches))
        kv_patch_ids = (
            mint.arange(num_patches).unsqueeze(0).unsqueeze(0).broadcast_to((batch_size, seq_len, num_patches))
        )
    ...
    cross_attention_mask = cross_attention_mask.float().repeat_interleave(cross_attn_k, dim=repeat_dim).bool()
...
class BltModel(BltPreTrainedModel):
    def __init__(self, config: BltConfig):
        ...
        self.encoder_hash_tok_embedding = mint.nn.Embedding(total_vocab_size, config.encoder_config.hidden_size)
        if self.config.patch_in_forward:
            self.patcher = BltPatcher(config.patcher_config)
            self.patcher.set_train(False)
            for param in self.patcher.get_parameters():
    ...
    def construct(
        self,
        input_ids: Optional[ms.Tensor] = None,
        patch_lengths: Optional[ms.Tensor] = None,
        attention_mask: Optional[ms.Tensor] = None,
        position_ids: Optional[ms.Tensor] = None,
        past_key_values: Optional[Cache] = None,
        inputs_embeds: Optional[ms.Tensor] = None,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if patch_lengths is None:
            if self.config.patching_mode == "entropy" and self.patcher is not None:
                ...
                patch_lengths = process_patch_lengths(
                    mint.ones((batch_size, sequence_length + 1), dtype=dtype), self.config.max_patch_length
                )
        patch_ids = self._patch_ids_from_lengths(patch_lengths, sequence_length)
        if cache_position is None:
            past_seen_tokens = past_key_values.get_seq_length() if past_key_values is not None else 0
            cache_position = mint.arange(past_seen_tokens, past_seen_tokens + encoder_embeds.shape[1])
        if position_ids is None:
            position_ids = cache_position.unsqueeze(0)
        ...
        global_cache_position = mint.arange(0, encoder_cross_states.shape[1])
    ...
    def _patch_ids_from_lengths(self, patch_lengths: ms.Tensor, seq_len: int) -> ms.Tensor:
        batch_size = patch_lengths.shape[0]
        patch_starts = mint.cat(
            [
                mint.zeros((batch_size, 1), dtype=patch_lengths.dtype),
                patch_lengths.cumsum(dim=-1)[:, :-1],
            ],
            dim=-1,
        )
        token_positions = mint.arange(seq_len)
...
class BltForCausalLM(BltPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: BltConfig):
        ...
        self.lm_head = mint.nn.Linear(config.decoder_config.hidden_size, config.vocab_size, bias=False)
    ...
    def construct(
        ...
        attention_mask: Optional[ms.Tensor] = None,
        position_ids: Optional[ms.Tensor] = None,
        cross_attention_states: Optional[ms.Tensor] = None,  # Keep for compatibility
        cross_attention_mask: Optional[ms.Tensor] = None,
        full_text_row_masked_out_mask: Optional[tuple[ms.Tensor, ms.Tensor]] = None,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        full_text_row_masked_out_mask (`tuple[ms.Tensor, ms.Tensor]`, *optional*):
        ...
        >>> from mindone.transformers import BltForCausalLM
        >>> import mindspore as ms
