...
def load_balancing_loss_func(router_probs: mindspore.Tensor, expert_indices: mindspore.Tensor) -> float:
    ...
    expert_mask = mindspore.mint.nn.functional.one_hot(expert_indices, num_experts)

    # For a given token, determine if it was routed to a given expert.
    expert_mask = mindspore.mint.max(expert_mask, axis=-2).values
...
class NllbMoeScaledWordEmbedding(mindspore.mint.nn.Embedding):
    """
...
class NllbMoeSinusoidalPositionalEmbedding(mindspore.nn.Cell):
...
class NllbMoeTop2Router(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: NllbMoeConfig):
        ...
        self.classifier = mindspore.mint.nn.Linear(config.hidden_size, self.num_experts, bias=config.router_bias)
    ...
    def route_tokens(
        ...
        router_probs = mindspore.mint.nn.functional.softmax(router_logits, dim=-1, dtype=self.dtype).to(input_dtype)
        top_1_expert_index = mindspore.mint.argmax(router_probs, dim=-1)
        top_1_mask = mindspore.mint.nn.functional.one_hot(top_1_expert_index, num_classes=self.num_experts)
        ...
        top_2_expert_index = mindspore.mint.argmax(logits_except_top_1, dim=-1)
        ...
        if self.second_expert_policy == "random":
            top_2_max_probs = (router_probs * top_2_mask).sum(dim=1)
            sampled = (2 * top_2_max_probs) > mindspore.mint.rand_like(top_2_max_probs.float())
        ...
        if self.batch_prioritized_routing:
            ...
            sorted_cumsum1 = (mindspore.mint.cumsum(sorted_top_1_mask, dim=0) - 1) * sorted_top_1_mask
            ...
            sorted_cumsum2 = (mindspore.mint.cumsum(sorted_top_2_mask, dim=0) - 1) * sorted_top_2_mask
            locations2 = sorted_cumsum2[importance_scores.argsort(dim=0).argsort(dim=0)]
            # Update 2nd's location by accounting for locations of 1st
            locations2 += mindspore.mint.sum(top_1_mask, dim=0, keepdim=True)

        else:
            locations1 = mindspore.mint.cumsum(top_1_mask, dim=0) - 1
            locations2 = mindspore.mint.cumsum(top_2_mask, dim=0) - 1
            # Update 2nd's location by accounting for locations of 1st
            locations2 += mindspore.mint.sum(top_1_mask, dim=0, keepdim=True)
        ...
        top_1_mask = top_1_mask * mindspore.mint.lt(locations1, self.expert_capacity)
    ...
    def construct(self, hidden_states: mindspore.Tensor, padding_mask: Optional[mindspore.Tensor] = None) -> tuple:
        r"""
...
class NllbMoeDenseActDense(mindspore.nn.Cell):
    def __init__(self, config: NllbMoeConfig, ffn_dim: int):
        super().__init__()
        self.fc1 = mindspore.mint.nn.Linear(config.d_model, ffn_dim)
        self.fc2 = mindspore.mint.nn.Linear(ffn_dim, config.d_model)
        self.dropout = mindspore.mint.nn.Dropout(config.activation_dropout)
...
class NllbMoeEncoderLayer(mindspore.nn.Cell):  # TODO: GradientCheckpointingLayer
    def __init__(self, config: NllbMoeConfig, is_sparse: bool = False):
        ...
        self.attn_dropout = mindspore.mint.nn.Dropout(config.dropout)
        ...
        self.ff_layer_norm = mindspore.mint.nn.LayerNorm(config.d_model)
...
class NllbMoeDecoderLayer(mindspore.nn.Cell):  # TODO: GradientCheckpointingLayer
    def __init__(self, config: NllbMoeConfig, is_sparse: bool = False):
        super().__init__()
        self.embed_dim = config.d_model
        self.is_sparse = is_sparse
        ...
        self.cross_attention_layer_norm = mindspore.mint.nn.LayerNorm(self.embed_dim)
        if not self.is_sparse:
...
class NllbMoeEncoder(NllbMoePreTrainedModel):
    ...
    def __init__(self, config: NllbMoeConfig, embed_tokens: Optional[mindspore.mint.nn.Embedding] = None):
    ...
    def _update_cross_attn_mask(
        ...
        if encoder_hidden_states is not None and encoder_attention_mask is not None:
            if self.config._attn_implementation == "flash_attention_2":
                ...
                # if isinstance(encoder_attention_mask, mindspore.Tensor):
...
class NllbMoeForConditionalGeneration(NllbMoePreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: NllbMoeConfig):
        ...
        self.lm_head = mindspore.mint.nn.Linear(config.d_model, config.vocab_size, bias=False)
