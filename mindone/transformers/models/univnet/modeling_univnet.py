...
class UnivNetKernelPredictorResidualBlock(nn.Cell):
    ...
    def __init__(
        ...
        self.conv1 = mint.nn.Conv1d(self.channels, self.channels, self.kernel_size, padding=padding, bias=True)
        self.conv2 = mint.nn.Conv1d(self.channels, self.channels, self.kernel_size, padding=padding, bias=True)
    ...
    def remove_weight_norm(self):
        WeightNorm.remove_weight_norm(self.conv1)
        WeightNorm.remove_weight_norm(self.conv2)
...
class UnivNetKernelPredictor(nn.Cell):
    ...
    def __init__(
        ...
        self.input_conv = mint.nn.Conv1d(self.resnet_in_channels, self.resnet_hidden_channels, 5, padding=2, bias=True)

        self.resblocks = nn.CellList([UnivNetKernelPredictorResidualBlock(config) for _ in range(self.num_blocks)])

        self.kernel_conv = mint.nn.Conv1d(
            self.resnet_hidden_channels, self.kernel_channels, self.resnet_kernel_size, padding=padding, bias=True
        )
        self.bias_conv = mint.nn.Conv1d(
            self.resnet_hidden_channels, self.bias_channels, self.resnet_kernel_size, padding=padding, bias=True
        )

    def construct(self, spectrogram: Tensor):
        """
    ...
    def remove_weight_norm(self):
        WeightNorm.remove_weight_norm(self.input_conv)
        for layer in self.resblocks:
            layer.remove_weight_norm()
        WeightNorm.remove_weight_norm(self.kernel_conv)
        WeightNorm.remove_weight_norm(self.bias_conv)
...
class UnivNetLvcResidualBlock(nn.Cell):
    ...
    def construct(self, hidden_states, kernel, bias, hop_size=256):
        ...
        hidden_states = mint.sigmoid(hidden_states[:, : self.hidden_channels, :]) * mint.tanh(
    ...
    def location_variable_convolution(
        ...
        hidden_states = mint.nn.functional.pad(hidden_states, (padding, padding), "constant", 0)
        # (batch, in_channels, kernel_length, hop_size + 2*padding)
        # hidden_states = hidden_states.unfold(2, hop_size + 2 * padding, hop_size)
        hidden_states = unfold(hidden_states, 2, hop_size + 2 * padding, hop_size)

        if hop_size < dilation:
            hidden_states = mint.nn.functional.pad(hidden_states, (0, dilation), "constant", 0)
        # (batch, in_channels, kernel_length, (hop_size + 2*padding)/dilation, dilation)
        # hidden_states = hidden_states.unfold(3, dilation, dilation)
        hidden_states = unfold(hidden_states, 3, dilation, dilation)
        ...
        hidden_states = unfold(hidden_states, 4, kernel_size, 1)

        # Apply local convolution kernel to hidden_states.
        output_hidden_states = mint.einsum("bildsk,biokl->bolsd", hidden_states, kernel)
    ...
    def remove_weight_norm(self):
        WeightNorm.remove_weight_norm(self.conv)
...
class UnivNetLvcBlock(nn.Cell):
    ...
    def __init__(
        ...
        self.convt_pre = ConvTranspose1d(
            self.hidden_channels,
            self.hidden_channels,
            2 * self.stride,
            stride=self.stride,
    ...
    def construct(self, hidden_states: Tensor, spectrogram: Tensor):
    ...
    def remove_weight_norm(self):
        WeightNorm.remove_weight_norm(self.convt_pre)
...
class UnivNetModel(PreTrainedModel):
    ...
    def __init__(self, config: UnivNetConfig):
        ...
        self.conv_post = mint.nn.Conv1d(config.model_hidden_channels, 1, 7, padding=3, padding_mode="reflect")
    ...
    def construct(
        ...
        noise_sequence: Optional[Tensor] = None,
        padding_mask: Optional[Tensor] = None,
        generator: Optional[mindspore.Generator] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[Tensor], UnivNetModelOutput]:
        r"""
        Returns:
        ...
        Example:
         ...
         >>> from mindone.transformers.models.univnet import UnivNetModel
        ...
        if noise_sequence is not None:
            ...
            noise_sequence = mint.randn(noise_sequence_shape, generator=generator, dtype=input_features.dtype)
        ...
        if padding_mask is not None:
            ...
            waveform_lengths = mint.sum(padding_mask, dim=1)
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (mint.nn.Linear, mint.nn.Conv1d, ConvTranspose1d)):
    ...
    def remove_weight_norm(self):
        WeightNorm.remove_weight_norm(self.conv_pre)
        for layer in self.resblocks:
            layer.remove_weight_norm()
        WeightNorm.remove_weight_norm(self.conv_post)
