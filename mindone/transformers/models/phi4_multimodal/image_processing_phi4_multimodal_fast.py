    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        for image in images:
            ...
            global_attention_mask = mindspore.mint.ones((1, mask_size, mask_size))
            ...
            hd_image_reshape = mindspore.mint.cat([global_image.unsqueeze(0), hd_image_reshape], dim=0)
            hd_attention_mask_reshape = mindspore.mint.cat([global_attention_mask, attention_mask_reshape], dim=0)
            ...
            max_crops = hd_image_reshape.shape[0]
        max_crops = max([img.shape[0] for img in images_transformed])
        images_transformed = [self.pad_to_max_num_crops(im, max_crops) for im in images_transformed]
        images_transformed = mindspore.mint.stack(images_transformed, dim=0)
        masks_transformed = [self.pad_mask_to_max_num_crops(mask, max_crops) for mask in masks_transformed]
        masks_transformed = mindspore.mint.stack(masks_transformed, dim=0)
        image_sizes = mindspore.Tensor(image_sizes, dtype=mindspore.int64)
