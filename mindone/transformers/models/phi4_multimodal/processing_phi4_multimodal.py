...
class Phi4MMImageProcessor(BaseImageProcessor):
    ...
    def pad_to_max_num_crops(self, images, max_crops=5):
        ...
        if B < max_crops:
            pad = mint.zeros((max_crops - B, 3, H, W), dtype=images.dtype)
            images = mint.cat([images, pad], dim=0)
        return images

    def pad_mask_to_max_num_crops(self, masks, max_crops=5):
        B, H, W = masks.shape
        if B < max_crops:
            pad = mint.ones((max_crops - B, H, W), dtype=masks.dtype)
            masks = mint.cat([masks, pad], dim=0)
        return masks
    ...
    def preprocess(
        ...
        mask_transformed = mint.stack(mask_transformed, dim=0)

        returned_input_image_embeds = image_transformed
        returned_image_sizes = ms.Tensor(shapes, dtype=ms.int64)
...
AudioInput = Tuple[Union[np.ndarray, ms.Tensor], int]
...
class Phi4MMAudioFeatureExtractor(SequenceFeatureExtractor):
    ...
    def __call__(
        ...
        for audio_data, sample_rate in audios:
            ...
            returned_input_audio_embeds.append(ms.Tensor(audio_embeds))
...
class Phi4MMProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        inputs["input_mode"] = ms.Tensor([input_mode.value], dtype=ms.int64)
    ...
    def _convert_images_audios_text_to_inputs(
        ...
        if len(images) > 0:
            ...
            image_attention_mask = ms.Tensor([])
            num_img_tokens = []
        ...
        if len(audios) > 0:
            ...
            input_audio_embeds = ms.Tensor([])
            audio_embed_sizes = ms.Tensor([])
        ...
        lengths = ms.Tensor([len(input_ids) for input_ids in new_input_ids_list])
        max_len = lengths.max()
