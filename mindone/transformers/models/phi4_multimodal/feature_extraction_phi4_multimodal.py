...
class Phi4MultimodalFeatureExtractor(SequenceFeatureExtractor):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if isinstance(raw_speech, np.ndarray):
            raw_speech = mindspore.Tensor(raw_speech)
        elif isinstance(raw_speech, (list, tuple)) and isinstance(raw_speech[0], np.ndarray):
            raw_speech = [mindspore.Tensor(speech) for speech in raw_speech]

        is_batched_torch = isinstance(raw_speech, mindspore.Tensor) and len(raw_speech.shape) > 1
        ...
        if is_batched_torch or is_batched_sequence:
            raw_speech = [speech[:, None].to(mindspore.float32) for speech in raw_speech]
        else:
            raw_speech = [raw_speech[:, None].to(mindspore.float32)]
    ...
    def _torch_extract_fbank_features(
        ...
        frames_prev = mindspore.mint.roll(frames, 1, dims=-1)
        ...
        spec = mindspore.mint.abs(S)
        spec_power = spec**2

        # apply triangular mel filter bank
        mel_filters = mindspore.Tensor.from_numpy(self.mel_filters).to(device, mindspore.float32)
        log_spec = mindspore.mint.clamp(spec_power @ mel_filters, min=1.0)
        log_spec = mindspore.mint.log(log_spec)
