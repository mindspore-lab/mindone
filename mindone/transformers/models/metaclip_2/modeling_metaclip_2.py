...
from transformers.models.metaclip_2.configuration_metaclip_2 import MetaClip2Config, MetaClip2TextConfig, MetaClip2VisionConfig


class MetaClip2TextEmbeddings(mindspore.nn.Cell):
...
class MetaClip2VisionEmbeddings(mindspore.nn.Cell):
...
class MetaClip2Attention(mindspore.nn.Cell):
...
class MetaClip2MLP(mindspore.nn.Cell):
...
class MetaClip2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, mindspore.mint.nn.Linear) and module.bias is not None:
            module.bias.data.zero_()
...
class MetaClip2Encoder(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: MetaClip2Config):
        super().__init__()
        self.config = config
        self.layers = mindspore.nn.CellList([MetaClip2EncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class MetaClip2TextTransformer(mindspore.nn.Cell):
    def __init__(self, config: MetaClip2TextConfig):
        super().__init__()
        self.config = config
        embed_dim = config.hidden_size
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        pooled_output = last_hidden_state[
            mindspore.mint.arange(last_hidden_state.shape[0], ),
            (input_ids.to(dtype=mindspore.int32, ) == self.eos_token_id).int().argmax(dim=-1),
        ]
...
class MetaClip2TextModelWithProjection(MetaClip2PreTrainedModel):
    ...
    def __init__(self, config: MetaClip2TextConfig):
        ...
        self.text_projection = mindspore.mint.nn.Linear(config.hidden_size, config.projection_dim, bias=False)
...
def contrastive_loss(logits: mindspore.Tensor) -> mindspore.Tensor:
    return mindspore.mint.nn.functional.cross_entropy(logits, mindspore.mint.arange(len(logits), ))


def metaclip_2_loss(similarity: mindspore.Tensor) -> mindspore.Tensor:
...
class MetaClip2Model(MetaClip2PreTrainedModel):
    ...
    def construct(
        ...
        logits_per_text = mindspore.mint.matmul(text_embeds, image_embeds.t())
...
class MetaClip2VisionTransformer(mindspore.nn.Cell):
    def __init__(self, config: MetaClip2VisionConfig):
        super().__init__()
        self.config = config
        embed_dim = config.hidden_size
...
class MetaClip2VisionModelWithProjection(MetaClip2PreTrainedModel):
    ...
    def __init__(self, config: MetaClip2VisionConfig):
        ...
        self.visual_projection = mindspore.mint.nn.Linear(config.hidden_size, config.projection_dim, bias=False)
...
class MetaClip2ForImageClassification(MetaClip2PreTrainedModel):
    ...
    def __init__(self, config: MetaClip2Config) -> None:
        ...
        # Classifier head
        self.classifier = (
            mindspore.mint.nn.Linear(config.vision_config.hidden_size, config.num_labels) if config.num_labels > 0 else mindspore.mint.nn.Identity()
        )
    ...
    def construct(
        ...
        sequence_output = mindspore.mint.mean(sequence_output[:, 1:, :], dim=1)
        # apply classifier
