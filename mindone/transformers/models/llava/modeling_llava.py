...
class LlavaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, cell):
        ...
        if hasattr(cell, "class_embedding"):
            cell.class_embedding.set_data(
                init.initializer(
                    init.Normal(mean=0.0, sigma=std), cell.class_embedding.shape, cell.class_embedding.dtype
                )
            )
    ...
    def get_image_features(
        ...
        **kwargs,
        ...
        if "image_sizes" in kwargs:
            ...
            image_features = mint.split(image_features.squeeze(0), split_sizes)
        else:
...
class LlavaForConditionalGeneration(LlavaPreTrainedModel):
    ...
    def _merge_input_ids_with_image_features(self, image_features, inputs_embeds, input_ids, attention_mask, labels):
        ...
        batch_indices, non_image_indices = mint.where(input_ids != self.config.image_token_index)
        ...
        batch_indices, pad_indices = mint.where(input_ids == self.pad_token_id)
    ...
    def construct(
        ...
        if inputs_embeds is None:
            ...
            # generation with cache
                ...
                attention_mask = mint.cat((extended_attention_mask, attention_mask[:, -target_length:]), dim=1)
                position_ids = mint.sum(attention_mask, dim=1).unsqueeze(-1) - 1
