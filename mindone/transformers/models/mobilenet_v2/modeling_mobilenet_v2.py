...
def apply_tf_padding(features: mindspore.Tensor, conv_layer: mindspore.mint.nn.Conv2d) -> mindspore.Tensor:
    ...
    return mindspore.mint.nn.functional.pad(features, padding, "constant", 0.0)


class MobileNetV2ConvLayer(mindspore.nn.Cell):
    def __init__(
        self,
...
class MobileNetV2InvertedResidual(mindspore.nn.Cell):
    def __init__(
...
class MobileNetV2Stem(mindspore.nn.Cell):
...
class MobileNetV2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: Union[mindspore.mint.nn.Linear, mindspore.mint.nn.Conv2d]) -> None:
        """Initialize the weights"""
        if isinstance(module, (mindspore.mint.nn.Linear, mindspore.mint.nn.Conv2d)):
            module.weight.data.normal_(mean=0.0, std=self.config.initializer_range)
            if module.bias is not None:
                module.bias.data.zero_()
        elif isinstance(module, mindspore.mint.nn.BatchNorm2d):
            module.bias.data.zero_()
...
class MobileNetV2Model(MobileNetV2PreTrainedModel):
    def __init__(self, config: MobileNetV2Config, add_pooling_layer: bool = True):
        ...
        self.pooler = mindspore.mint.nn.AdaptiveAvgPool2d((1, 1)) if add_pooling_layer else None
    ...
    def construct(
        ...
        if self.pooler is not None:
            pooled_output = mindspore.mint.flatten(self.pooler(last_hidden_state), start_dim=1)
        else:
            pooled_output = None
...
class MobileNetV2DeepLabV3Plus(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: MobileNetV2Config) -> None:
        ...
        self.avg_pool = mindspore.mint.nn.AdaptiveAvgPool2d(output_size=1)
    ...
    def construct(self, features: mindspore.Tensor) -> mindspore.Tensor:
        ...
        features_pool = mindspore.mint.nn.functional.interpolate(
            features_pool, size=spatial_size, mode="bilinear", align_corners=True
        )

        features_aspp = self.conv_aspp(features)

        features = mindspore.mint.cat([features_pool, features_aspp], dim=1)
