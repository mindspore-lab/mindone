...
class JetMoeParallelExperts(mindspore.nn.Cell):
    def __init__(self, num_experts: int, input_size: int, output_size: int) -> None:
        ...
        """
        super().__init__()
        self.weight = mindspore.Parameter(mindspore.mint.empty(num_experts, output_size, input_size))
    ...
    def construct(self, inputs, expert_size):
        ...
        for i in range(self.num_experts):
            output_list.append(mindspore.mint.nn.functional.linear(input_list[i], self.weight[i]))
        results = mindspore.mint.cat(output_list, dim=0)
        return results


class JetMoeTopKGating(mindspore.nn.Cell):
    def __init__(self, input_size: int, num_experts: int, top_k: int):
        ...
        self.layer = mindspore.mint.nn.Linear(input_size, num_experts, bias=False)

    def construct(self, hidden_states):
        # compute the top_k routing decision
        logits = self.layer(hidden_states).float()  # [batch_size x seq_len, num_experts]
        top_k_logits, top_k_indices = logits.topk(self.top_k, dim=1)  # [num_tokens, top_k]
        top_k_gates = mindspore.mint.softmax(top_k_logits, dim=1).type_as(hidden_states)  # [num_tokens, top_k]

        # compute number of input given to each expert
        zeros = mindspore.mint.zeros(
...
class JetMoeMoE(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: JetMoeConfig):
        ...
        self.bias = mindspore.Parameter(mindspore.mint.empty(self.input_size))
...
class JetMoeMoA(mindspore.nn.Cell):
    """
...
class JetMoeRMSNorm(mindspore.nn.Cell):
...
class JetMoeRotaryEmbedding(mindspore.nn.Cell):
    def __init__(self, config: JetMoeConfig):
        super().__init__()
    ...
    def _dynamic_frequency_update(self, position_ids):
        ...
        if seq_len > self.max_seq_len_cached:  # growth
            ...
            self.register_buffer("inv_freq", inv_freq, persistent=False)  # Keep register_buffer
            self.max_seq_len_cached = seq_len

        if seq_len < self.original_max_seq_len and self.max_seq_len_cached > self.original_max_seq_len:  # reset
            self.original_inv_freq = self.original_inv_freq
            self.register_buffer("inv_freq", self.original_inv_freq, persistent=False)  # Keep register_buffer
...
class JetMoeAttention(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: JetMoeConfig, layer_idx: Optional[int] = None):
        ...
        self.kv_proj = mindspore.mint.nn.Linear(config.hidden_size, self.kv_projection_size * 2, bias=False)
...
class JetMoeSdpaAttention(JetMoeAttention):
    """
    JetMoe attention module using scaled_dot_product_attention. This module inherits from
    ...
    def construct(
        ...
        mindspore.Tensor, Optional[mindspore.Tensor], Optional[Tuple[mindspore.Tensor]], Optional[mindspore.Tensor]
    ]:
        if output_attentions:
            # TODO: Improve this warning with e.g. `model.config.attn_implementation = "manual"` once this is implemented.
            logger.warning_once(
                "JetMoeModel is using JetMoeSdpaAttention, but `scaled_dot_product_attention` does not support \
                ...
                'but specifying the manual implementation will be required from Transformers version v5.0.0 \
                    onwards. This warning can be removed using the argument `attn_implementation="eager"` when loading the model.'
            )
...
class JetMoeFlashAttention2(JetMoeAttention):
    ...
    def construct(
        ...
        Args:
            ...
            cache_position (Optional[mindspore.Tensor]): Position of the cache.

        Returns:
            Union[Tuple[mindspore.Tensor, Tuple[mindspore.Tensor]], Optional[Tuple[...]]]: Tuple containing outputs.
        """
...
class JetMoeBlock(mindspore.nn.Cell):
    def __init__(self, config: JetMoeConfig, layer_idx: Optional[int] = None):
        """
...
JETMOE_START_DOCSTRING = r"""
    This model is a MindSpore nn.Cell implementation. Use it as a regular MindSpore Cell and refer to the MindSpore
...
class JetMoeModel(JetMoePreTrainedModel):
    ...
    def _update_causal_mask(
        ...
        if self.config._attn_implementation == "flex_attention":
            raise NotImplementedError("Flex Attention is not supported for JetMoe.")
    ...
    def construct(
        ...
        **kwargs,
        ...
        if output_router_logits:
            ...
            if labels is not None:
                loss += self.aux_loss_coef * aux_loss  # make sure to reside in the same device
