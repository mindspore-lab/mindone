...
class SplinterEmbeddings(mindspore.nn.Cell):
...
class SplinterSelfAttention(mindspore.nn.Cell):
...
class SplinterSelfOutput(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class SplinterAttention(mindspore.nn.Cell):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
...
class SplinterIntermediate(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class SplinterOutput(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class SplinterLayer(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class SplinterEncoder(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = mindspore.nn.CellList([SplinterLayer(config) for _ in range(config.num_hidden_layers)])
...
class SplinterFullyConnectedLayer(mindspore.nn.Cell):
    def __init__(self, input_dim, output_dim, hidden_act="gelu"):
        ...
        self.dense = mindspore.mint.nn.Linear(self.input_dim, self.output_dim)
        self.act_fn = ACT2FN[hidden_act]
        self.LayerNorm = mindspore.mint.nn.LayerNorm(self.output_dim)
...
class QuestionAwareSpanSelectionHead(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        self.start_classifier = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size, bias=False)
        self.end_classifier = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size, bias=False)
    ...
    def construct(self, inputs, positions):
        ...
        gathered_reps = mindspore.mint.gather(inputs, dim=1, index=index)  # [batch_size, num_positions, dim]
        ...
        start_logits = mindspore.mint.matmul(hidden_states, start_reps)

        hidden_states = self.end_classifier(query_end_reps)
        end_reps = end_reps.permute(0, 2, 1)
        end_logits = mindspore.mint.matmul(hidden_states, end_reps)
...
class SplinterForQuestionAnswering(SplinterPreTrainedModel):
    ...
    def construct(
        ...
        end_positions (`ms.Tensor` of shape `(batch_size,)`, *optional*):
            ...
            Positions are clamped to the length of the sequence (`sequence_length`). Position outside of the sequence
            are not taken into account for computing the loss.
        question_positions (`ms.Tensor` of shape `(batch_size, num_questions)`, *optional*):
        ...
        if question_positions is None:
            if input_ids is not None:
                question_position_for_each_example = mindspore.mint.argmax(
                    (mindspore.mint.eq(input_ids, self.question_token_id)).int(), dim=-1
                )
            else:
...
class SplinterForPreTrainingOutput(ModelOutput):
    ...
    Args:
        loss (`ms.Tensor` of shape `(1,)`, *optional*, returned when start and end positions are provided):
            Total span extraction loss is the sum of a Cross-Entropy for the start and end positions.
        start_logits (`ms.Tensor` of shape `(batch_size, num_questions, sequence_length)`):
            Span-start scores (before SoftMax).
        end_logits (`ms.Tensor` of shape `(batch_size, num_questions, sequence_length)`):
...
class SplinterForPreTraining(SplinterPreTrainedModel):
    ...
    def construct(
        ...
        r"""
        start_positions (`ms.Tensor` of shape `(batch_size, num_questions)`, *optional*):
            Labels for position (index) of the start of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`). Position outside of the sequence
            are not taken into account for computing the loss.
        end_positions (`ms.Tensor` of shape `(batch_size, num_questions)`, *optional*):
            Labels for position (index) of the end of the labelled span for computing the token classification loss.
            Positions are clamped to the length of the sequence (`sequence_length`). Position outside of the sequence
            are not taken into account for computing the loss.
        question_positions (`ms.Tensor` of shape `(batch_size, num_questions)`, *optional*):
    ...
    def _prepare_question_positions(self, input_ids: mindspore.Tensor) -> mindspore.Tensor:
        rows, flat_positions = mindspore.mint.where(input_ids == self.config.question_token_id)
        num_questions = mindspore.mint.bincount(rows)
        positions = mindspore.ops.full(
