...
class MusicgenSinusoidalPositionalEmbedding(mindspore.nn.Cell):
...
class MusicgenAttention(mindspore.nn.Cell):
...
class MusicgenDecoderLayer(mindspore.nn.Cell):
...
class MusicgenPreTrainedModel(MSPreTrainedModel):
    """
...
class MusicgenDecoder(MusicgenPreTrainedModel):
    ...
    def __init__(self, config: MusicgenDecoderConfig):
        ...
        self.layers = mindspore.nn.CellList([MusicgenDecoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class MusicgenForConditionalGeneration(MSPreTrainedModel, GenerationMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        ```python
        >>> from mindone.transformers import MusicgenForConditionalGeneration
            ...
            text_encoder_pretrained_model_name_or_path (`str`, *optional*):
                Information necessary to initiate the text encoder. Can be either:
                    ...
                    - A path to a *directory* containing model weights saved using
                      [`~MSPreTrainedModel.save_pretrained`], e.g., `./my_model_directory/`.
    ...
    def _prepare_text_encoder_kwargs_for_generation(
        ...
        if guidance_scale is not None and guidance_scale > 1:
            last_hidden_state = mindspore.mint.cat(
                [last_hidden_state, mindspore.mint.zeros_like(last_hidden_state)], dim=0
            )
            if "attention_mask" in model_kwargs:
                model_kwargs["attention_mask"] = mindspore.mint.cat(
                    [model_kwargs["attention_mask"], mindspore.mint.zeros_like(model_kwargs["attention_mask"])], dim=0
                )
    ...
    def _prepare_audio_encoder_kwargs_for_generation(
        ...
        if self.decoder.config.audio_channels == 1:
            ...
            if audio_scales_left != [None] or audio_scales_right != [None]:
                audio_scales = mindspore.mint.stack([audio_scales_left, audio_scales_right], dim=1)
            else:
    ...
    def get_unconditional_inputs(self, num_samples=1):
        ...
        ```"""
        last_hidden_state = mindspore.mint.zeros(
            (num_samples, 1, self.config.text_encoder.hidden_size), dtype=self.dtype
        )

        attention_mask = mindspore.mint.zeros((num_samples, 1), dtype=mindspore.int64)
