...
class DbrxSdpaAttention(DbrxAttention):
    """
    Dbrx attention module using scaled_dot_product_attention. This module inherits from
    ...
    def construct(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "DbrxModel is using DbrxSdpaAttention, but `scaled_dot_product_attention` does not support `output_attentions=True`."
                "Falling back to the manual attention implementation, but specifying the manual implementation will be required from Transformers version "
                'v5.0.0 onwards. This warning can be removed using the argument `attn_implementation="eager"` when loading the model.'
            )
        ...
        # in SDPA to support both ms.jit's dynamic shapes and full graph options. An inline conditional prevents dynamic shapes from compiling.
...
class DbrxExperts(nn.Cell):
    ...
    def construct(self, x: ms.Tensor, weights: ms.Tensor, top_weights: ms.Tensor, top_experts: ms.Tensor) -> ms.Tensor:
