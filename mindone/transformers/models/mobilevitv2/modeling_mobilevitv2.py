...
class MobileViTV2ConvLayer(mindspore.nn.Cell):
    def __init__(
        self,
...
class MobileViTV2InvertedResidual(mindspore.nn.Cell):
    """
...
class MobileViTV2MobileNetLayer(mindspore.nn.Cell):
    def __init__(
...
class MobileViTV2LinearSelfAttention(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: MobileViTV2Config, embed_dim: int) -> None:
        ...
        self.attn_dropout = mindspore.mint.nn.Dropout(p=config.attn_dropout)
    ...
    def construct(self, hidden_states: mindspore.Tensor) -> mindspore.Tensor:
        ...
        query, key, value = mindspore.mint.split(qkv, split_size_or_sections=[1, self.embed_dim, self.embed_dim], dim=1)

        # apply softmax along num_patches dimension
        context_scores = mindspore.mint.nn.functional.softmax(query, dim=-1)
...
class MobileViTV2FFN(mindspore.nn.Cell):
    def __init__(
        self,
        config: MobileViTV2Config,
        embed_dim: int,
        ...
        self.dropout1 = mindspore.mint.nn.Dropout(ffn_dropout)
        ...
        self.dropout2 = mindspore.mint.nn.Dropout(ffn_dropout)
...
class MobileViTV2TransformerLayer(mindspore.nn.Cell):
    def __init__(
        self,
        config: MobileViTV2Config,
        embed_dim: int,
        ...
        super().__init__()
        self.layernorm_before = mindspore.mint.nn.GroupNorm(
        ...
        self.dropout1 = mindspore.mint.nn.Dropout(p=dropout)
        self.layernorm_after = mindspore.mint.nn.GroupNorm(
            num_groups=1, num_channels=embed_dim, eps=config.layer_norm_eps
        )
...
class MobileViTV2Transformer(mindspore.nn.Cell):
...
class MobileViTV2Layer(mindspore.nn.Cell):
    """
    ...
    def __init__(
        ...
        self.layernorm = mindspore.mint.nn.GroupNorm(
    ...
    def unfolding(self, feature_map: mindspore.Tensor) -> tuple[mindspore.Tensor, tuple[int, int]]:
        batch_size, in_channels, img_height, img_width = feature_map.shape
        patches = mindspore.mint.nn.functional.unfold(
            feature_map,
    ...
    def folding(self, patches: mindspore.Tensor, output_size: tuple[int, int]) -> mindspore.Tensor:
        batch_size, in_dim, patch_size, n_patches = patches.shape
        patches = patches.reshape(batch_size, in_dim * patch_size, n_patches)

        feature_map = mindspore.mint.nn.functional.fold(
            patches,
...
class MobileViTV2Encoder(mindspore.nn.Cell):
...
class MobileViTV2ForImageClassification(MobileViTV2PreTrainedModel):
    def __init__(self, config: MobileViTV2Config) -> None:
        ...
        # Classifier head
        self.classifier = (
            mindspore.mint.nn.Linear(in_features=out_channels, out_features=config.num_labels)
...
class MobileViTV2ASPPPooling(mindspore.nn.Cell):
...
class MobileViTV2ASPP(mindspore.nn.Cell):
    """
...
class MobileViTV2DeepLabV3(mindspore.nn.Cell):
    """
