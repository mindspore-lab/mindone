...
class ZoeDepthDepthEstimatorOutput(ModelOutput):
    ...
    Args:
        ...
        domain_logits (`ms.Tensor` of shape `(batch_size, num_domains)`):
    ...
    domain_logits: ms.Tensor = None
    hidden_states: Optional[Tuple[ms.Tensor, ...]] = None
    attentions: Optional[Tuple[ms.Tensor, ...]] = None


class ZoeDepthReassembleStage(nn.Cell):
    """
    ...
    def construct(self, hidden_states: List[ms.Tensor], patch_height, patch_width) -> List[ms.Tensor]:
        ...
        if self.readout_type == "project":
            ...
            hidden_states = mint.cat((hidden_states, readout), -1)
...
class ZoeDepthReassembleLayer(nn.Cell):
    def __init__(self, config, channels, factor):
        super().__init__()
        # projection
        hidden_size = config.backbone_hidden_size
        self.projection = mint.nn.Conv2d(in_channels=hidden_size, out_channels=channels, kernel_size=1)

        # up/down sampling depending on factor
        if factor > 1:
            self.resize = mint.nn.ConvTranspose2d(channels, channels, kernel_size=factor, stride=factor, padding=0)
        elif factor == 1:
            self.resize = mint.nn.Identity()
        elif factor < 1:
            # so should downsample
            self.resize = mint.nn.Conv2d(channels, channels, kernel_size=3, stride=int(1 / factor), padding=1)
...
class ZoeDepthFeatureFusionStage(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.layers = nn.CellList()
...
class ZoeDepthPreActResidualLayer(nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        if self.use_batch_norm:
            self.batch_norm1 = mint.nn.BatchNorm2d(config.fusion_hidden_size, eps=config.batch_norm_eps)
            self.batch_norm2 = mint.nn.BatchNorm2d(config.fusion_hidden_size, eps=config.batch_norm_eps)
...
class ZoeDepthFeatureFusionLayer(nn.Cell):
    ...
    def __init__(self, config, align_corners=True):
        ...
        self.projection = mint.nn.Conv2d(config.fusion_hidden_size, config.fusion_hidden_size, kernel_size=1, bias=True)
...
class ZoeDepthRelativeDepthEstimationHead(nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        if config.add_projection:
            self.projection = mint.nn.Conv2d(256, 256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))

        features = config.fusion_hidden_size
        self.conv1 = mint.nn.Conv2d(features, features // 2, kernel_size=3, stride=1, padding=1)
        self.conv2 = mint.nn.Conv2d(features // 2, config.num_relative_features, kernel_size=3, stride=1, padding=1)
        self.conv3 = mint.nn.Conv2d(config.num_relative_features, 1, kernel_size=1, stride=1, padding=0)
...
def log_binom(n, k, eps=1e-7):
    ...
    n = n + eps
    k = k + eps
    return n * mint.log(n) - k * mint.log(k) - (n - k) * mint.log(n - k + eps)


class LogBinomialSoftmax(nn.Cell):
    def __init__(self, n_classes=256, act=mint.softmax):
    ...
    def construct(self, probabilities, temperature=1.0, eps=1e-4):
        """Compute the log binomial distribution for probabilities.

        Args:
            probabilities (`ms.Tensor` of shape `(batch_size, num_channels, height, width)`):
                Tensor containing probabilities of each class.
            temperature (`float` or `ms.Tensor` of shape `(batch_size, num_channels, height, width)`, *optional*, defaults to 1):
        ...
        one_minus_probabilities = mint.clamp(1 - probabilities, eps, 1)
        probabilities = mint.clamp(probabilities, eps, 1)
        y = (
            log_binom(self.k_minus_1, self.k_idx)
            + self.k_idx * mint.log(probabilities)
            + (self.k_minus_1 - self.k_idx) * mint.log(one_minus_probabilities)
        )
...
class ZoeDepthConditionalLogBinomialSoftmax(nn.Cell):
    def __init__(
        ...
        self.mlp = nn.SequentialCell(
            mint.nn.Conv2d(in_features + condition_dim, bottleneck, kernel_size=1, stride=1, padding=0),
            mint.nn.GELU(),
        ...
        self.log_binomial_transform = LogBinomialSoftmax(n_classes, act=mint.softmax)

    def construct(self, main_feature, condition_feature):
        """
        Args:
            main_feature (`ms.Tensor` of shape `(batch_size, num_channels, height, width)`):
                Main feature.
            condition_feature (ms.Tensor of shape `(batch_size, num_channels, height, width)`):
                Condition feature.
        ...
        """
        probabilities_and_temperature = self.mlp(mint.concat((main_feature, condition_feature), dim=1))
        probabilities_and_temperature = mint.nn.functional.softplus(probabilities_and_temperature)
...
class ZoeDepthSeedBinRegressor(nn.Cell):
    ...
    def construct(self, x):
        ...
        if self.bin_centers_type == "normed":
            ...
            bin_widths = mint.nn.functional.pad(bin_widths, (0, 0, 0, 0, 1, 0), mode="constant", value=self.min_depth)
            # shape (batch_size, num_channels, height, width)
            bin_edges = mint.cumsum(bin_widths, dim=1)
...
def inv_attractor(dx, alpha: float = 300, gamma: int = 2):
    ...
    Returns:
        ms.Tensor: Delta shifts - dc; New bin centers = Old bin centers + dc
    """
    return dx.div(1 + alpha * dx.pow(gamma))


class ZoeDepthAttractorLayer(nn.Cell):
    def __init__(
        self,
        config,
        n_bins,
        ...
        self.act1 = mint.nn.ReLU()
        self.conv2 = mint.nn.Conv2d(mlp_dim, n_attractors * 2, 1, 1, 0)  # x2 for linear norm
        self.act2 = mint.nn.ReLU()

    def construct(self, x, prev_bin, prev_bin_embedding=None, interpolate=True):
        """
        ...
        Args:
            x (`ms.Tensor` of shape `(batch_size, num_channels, height, width)`):
                Feature block.
            prev_bin (`ms.Tensor` of shape `(batch_size, prev_number_of_bins, height, width)`):
                Previous bin centers normed.
            prev_bin_embedding (`ms.Tensor`, *optional*):
        ...
        if prev_bin_embedding is not None:
            if interpolate:
                prev_bin_embedding = mint.nn.functional.interpolate(
        ...
        bin_centers = mint.nn.functional.interpolate(prev_bin, (height, width), mode="bilinear", align_corners=True)

        # note: only attractor_type = "exp" is supported here, since no checkpoints were released with other attractor types

        if not self.memory_efficient:
            func = {"mean": mint.mean, "sum": mint.sum}[self.kind]
            ...
            delta_c = mint.zeros_like(bin_centers)
        ...
        bin_centers, _ = mint.sort(bin_centers, dim=1)
        bin_centers = mint.clip(bin_centers, self.min_depth, self.max_depth)
        return bin_new_centers, bin_centers


class ZoeDepthAttractorLayerUnnormed(nn.Cell):
    def __init__(
        self,
        config,
        n_bins,
    ...
    def construct(self, x, prev_bin, prev_bin_embedding=None, interpolate=True):
        ...
        Args:
            x (`ms.Tensor` of shape (batch_size, num_channels, height, width)`):
                Feature block.
            prev_bin (`ms.Tensor` of shape (batch_size, prev_num_bins, height, width)`):
                Previous bin centers normed.
            prev_bin_embedding (`ms.Tensor`, *optional*):
                Optional previous bin embeddings.
            interpolate (`bool`, *optional*, defaults to `True`):
...
class ZoeDepthProjector(nn.Cell):
...
class ZoeDepthMultiheadAttention(nn.Cell):
...
class ZoeDepthTransformerEncoderLayer(nn.Cell):
    def __init__(self, config, dropout=0.1, activation="relu"):
        ...
        self.linear1 = mint.nn.Linear(hidden_size, intermediate_size)
        self.dropout = mint.nn.Dropout(dropout)
        self.linear2 = mint.nn.Linear(intermediate_size, hidden_size)

        self.norm1 = mint.nn.LayerNorm(hidden_size)
        self.norm2 = mint.nn.LayerNorm(hidden_size)
    ...
    def construct(
        self,
        src,
        src_mask: Optional[ms.Tensor] = None,
    ):
        queries = keys = src
...
class ZoeDepthPatchTransformerEncoder(nn.Cell):
    def __init__(self, config):
    ...
    def construct(self, x):
        ...
        Args:
            x (ms.Tensor - NCHW): Input feature tensor

        Returns:
            ms.Tensor - Transformer output embeddings of shape (batch_size, sequence_length, embedding_dim)
        """
        ...
        embeddings = embeddings + self.positional_encoding_1d(
            batch_size, sequence_length, embedding_dim, dtype=embeddings.dtype
        )
...
class ZoeDepthMLPClassifier(nn.Cell):
    def __init__(self, in_features, out_features) -> None:
        super().__init__()

        hidden_features = in_features
        self.linear1 = mint.nn.Linear(in_features, hidden_features)
        self.activation = mint.nn.ReLU()
        self.linear2 = mint.nn.Linear(hidden_features, out_features)
...
class ZoeDepthMultipleMetricDepthEstimationHeads(nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        self.conv2 = mint.nn.Conv2d(bottleneck_features, bottleneck_features, kernel_size=1, stride=1, padding=0)
        ...
        self.seed_bin_regressors = nn.CellDict(
            {
        ...
        self.conditional_log_binomial = nn.CellDict(
            {
    ...
    def construct(self, outconv_activation, bottleneck, feature_blocks, relative_depth):
        ...
        domain_vote = mint.softmax(domain_logits.sum(dim=0, keepdim=True), dim=-1)

        # Get the path
        names = [configuration["name"] for configuration in self.bin_configurations]
        bin_configurations_name = names[mint.argmax(domain_vote, dim=-1).squeeze().item()]
...
class ZoeDepthMetricDepthEstimationHead(nn.Cell):
    def __init__(self, config):
        super().__init__()
    ...
    def construct(self, outconv_activation, bottleneck, feature_blocks, relative_depth):
        ...
        for projector, attractor, feature in zip(self.projectors, self.attractors, feature_blocks):
            ...
            bin, bin_centers = attractor(bin_embedding, prev_bin, prev_bin_embedding, interpolate=True)
            prev_bin = bin.copy()
            prev_bin_embedding = bin_embedding.copy()
        ...
        relative_conditioning = mint.nn.functional.interpolate(
        ...
        # Now depth value is Sum px * cx , where cx are bin_centers from the last bin tensor
        bin_centers = mint.nn.functional.interpolate(bin_centers, x.shape[-2:], mode="bilinear", align_corners=True)
        out = mint.sum(x * bin_centers, dim=1, keepdim=True)
...
class ZoeDepthForDepthEstimation(ZoeDepthPreTrainedModel):
    ...
    def construct(
        ...
        >>> from mindone.transformers import ZoeDepthForDepthEstimation
        >>> import mindspore as ms
        >>> import numpy as np
