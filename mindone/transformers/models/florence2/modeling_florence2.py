...
from transformers import Florence2Config, Florence2VisionConfig
...
class Florence2VisionDropPath(nn.Cell):
    """Drop paths (Stochastic Depth) per sample (when applied in main path of residual blocks)."""
...
class Florence2VisionLearnedAbsolutePositionEmbedding2D(nn.Cell):
    """
...
class Florence2VisionPositionalEmbeddingCosine1D(nn.Cell):
    """
    ...
    def __init__(self, config: Florence2Config):
        ...
        pos_idx_to_embed = mint.empty((self.max_seq_len, self.embed_dim))
    ...
    def get_sinusoid_embeddings(max_positions: int, embed_dim: int):
        ...
        emb = mint.arange(max_positions, dtype=mindspore.float32).unsqueeze(1) * emb.unsqueeze(0)
        return mint.sin(emb), mint.cos(emb)

    def construct(self, seq_embeds: Tensor) -> Tensor:
        len_seq = seq_embeds.shape[1]
...
class Florence2VisionMLP(nn.Cell):
    def __init__(self, config: Florence2VisionConfig, stage_idx: int):
        super().__init__()
        self.config = config
        self.activation_fn = ACT2FN[config.activation_function]
        self.fc1 = mint.nn.Linear(config.embed_dim[stage_idx], int(config.embed_dim[stage_idx] * config.mlp_ratio))
        self.fc2 = mint.nn.Linear(int(config.embed_dim[stage_idx] * config.mlp_ratio), config.embed_dim[stage_idx])
...
class Florence2VisionConvEmbed(nn.Cell):
    ...
    def __init__(self, config: Florence2VisionConfig, stage_idx: int):
        ...
        self.norm = mint.nn.LayerNorm(dim_norm)
...
class Florence2VisionChannelAttention(nn.Cell):
    def __init__(self, config: Florence2VisionConfig, stage_idx: int):
        super().__init__()
        self.config = config
        self.dim = config.embed_dim[stage_idx]
        self.groups = config.num_groups[stage_idx]
        self.qkv = mint.nn.Linear(self.dim, self.dim * 3, bias=config.qkv_bias)
...
class Florence2VisionChannelBlock(nn.Cell):
    def __init__(
        self,
        ...
        self.norm1 = mint.nn.LayerNorm(config.embed_dim[stage_idx])
        self.channel_attn = Florence2VisionChannelAttention(config=config, stage_idx=stage_idx)
        self.drop_path1 = Florence2VisionDropPath(drop_path_rate) if drop_path_rate > 0.0 else mint.nn.Identity()
        ...
        self.norm2 = mint.nn.LayerNorm(config.embed_dim[stage_idx])
        self.ffn = Florence2VisionMLP(config=config, stage_idx=stage_idx)
        self.drop_path2 = Florence2VisionDropPath(drop_path_rate) if drop_path_rate > 0.0 else mint.nn.Identity()
...
class Florence2VisionWindowAttention(nn.Cell):
    def __init__(self, config: Florence2VisionConfig, stage_idx: int):
        super().__init__()
        self.config = config
        self.dim = config.embed_dim[stage_idx]
    ...
    def construct(self, hidden_states: Tensor):
        ...
        hidden_states = mint.nn.functional.pad(hidden_states, (0, 0, pad_left, pad_right, pad_top, pad_bottom))
...
class Florence2VisionSpatialBlock(nn.Cell):
    def __init__(
        self,
        config: Florence2VisionConfig,
        stage_idx: int,
...
class Florence2VisionBlock(nn.Cell):
    def __init__(
        self,
        config: Florence2VisionConfig,
        stage_idx: int,
...
class Florence2VisionBackbone(Florence2VisionPreTrainedModel):
    def __init__(self, config: Florence2VisionConfig):
        ...
        dpr = [x.item() for x in mint.linspace(0, config.drop_path_rate, sum(config.depths) * 2)]
        depth_offset = 0
...
class Florence2MultiModalProjector(nn.Cell):
    def __init__(self, config: Florence2Config):
        super().__init__()
        self.vision_embedding_dim = config.vision_config.embed_dim[-1]
        self.vision_projection_dim = config.vision_config.projection_dim
        self.image_projection = mint.nn.Linear(self.vision_embedding_dim, self.vision_projection_dim, bias=False)
        self.image_proj_norm = mint.nn.LayerNorm(self.vision_projection_dim)
    ...
    def construct(self, image_features):
        ...
        temporal_features = self.visual_temporal_embed(position_features[:, :1, :]).to(position_features.dtype)
        ...
        image_features = mint.cat([spatial_image_features, temporal_image_features], dim=1)
...
class Florence2Seq2SeqModelOutput(Seq2SeqModelOutput):
    ...
    image_hidden_states (`ms.Tensor`, *optional*):
        A `ms.Tensor` of size `(batch_size, num_image_tokens, hidden_size)`.
...
class Florence2Model(Florence2PreTrainedModel):
    ...
    def get_image_features(self, pixel_values: Tensor, **kwargs):
        """
    ...
    def get_placeholder_mask(self, input_ids: Tensor, inputs_embeds: Tensor, image_features: Tensor):
        """
        ...
        if input_ids is None:
            special_image_mask = inputs_embeds == self.get_input_embeddings()(
                Tensor(self.config.image_token_id, dtype=mindspore.long)
            )
    ...
    def construct(
        ...
        if decoder_input_ids is None:
            ...
            decoder_input_ids = mint.ones((inputs_embeds.shape[0], 1), dtype=mindspore.long)
