...
class EvollaSaProtEmbeddings(ms.nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        self.register_buffer(
            "position_ids",
            mint.arange(config.max_position_embeddings).unsqueeze(0).broadcast_to((1, -1)),
            persistent=False,
        )
...
class EvollaSaProtRotaryEmbedding(ms.nn.Cell):
    """
...
class EvollaSaProtSelfAttention(ms.nn.Cell):
    ...
    def construct(
        ...
        context_layer = mint.matmul(attention_probs.to(value_layer.dtype), value_layer)
...
class EvollaSaProtSelfOutput(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class EvollaSaProtAttention(ms.nn.Cell):
...
def gelu(x):
    ...
    """
    return x * 0.5 * (1.0 + mint.erf(x / math.sqrt(2.0)))


class EvollaSaProtIntermediate(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class EvollaSaProtOutput(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class EvollaSaProtEncoder(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = ms.nn.CellList([EvollaSaProtLayer(config) for _ in range(config.num_hidden_layers)])
        self.emb_layer_norm_after = mint.nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class EvollaSaProtPooler(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.dense = mint.nn.Linear(config.hidden_size, config.hidden_size)
...
class EvollaSaProtProteinEncoder(EvollaSaProtPreTrainedModel):
    ...
    def get_extended_attention_mask(
        ...
        extended_attention_mask = (1.0 - extended_attention_mask) * _DTYPE_2_MIN[dtype]
        return extended_attention_mask


class EvollaSequenceCompressorAttention(ms.nn.Cell):
    def __init__(self, dim, dim_head=64, heads=8):
        ...
        self.norm_media = mint.nn.LayerNorm(dim)
        self.norm_latents = mint.nn.LayerNorm(dim)
    ...
    def construct(self, x, latents, mask):
        """
        Args:
            x (mindspore.Tensor): image features
                shape (b, n1, D)
            latent (mindspore.Tensor): latent features
...
class EvollaFeedForward(ms.nn.Cell):
    def __init__(self, dim, mult=4):
        super().__init__()
        inner_dim = int(dim * mult)

        self.norm = mint.nn.LayerNorm(dim)
        self.fc1 = mint.nn.Linear(dim, inner_dim, bias=False)
        self.activation = mint.nn.GELU()
        self.fc2 = mint.nn.Linear(inner_dim, dim, bias=False)

    def construct(self, x):
        return self.fc2(self.activation(self.fc1(self.norm(x))))


class EvollaSequenceCompressorResampler(ms.nn.Cell):
    def __init__(self, config: EvollaConfig):
        ...
        self.latents = ms.Parameter(mint.randn(self.num_latents, protein_repr_dim), requires_grad=True)
        ...
        self.norm = mint.nn.LayerNorm(config.hidden_size)
        self.protein_projector = mint.nn.Linear(protein_repr_dim, config.hidden_size)

    def construct(self, embeds, mask):
        b = embeds.shape[0]
...
class EvollaProteinEncoderModelOutput(ModelOutput):
    sequence_compressor_output: ms.Tensor = None
    last_hidden_state: Optional[ms.Tensor] = None
    hidden_states: Optional[tuple[ms.Tensor, ...]] = None
    attentions: Optional[tuple[ms.Tensor, ...]] = None


class EvollaProteinEncoder(ms.nn.Cell):
    def __init__(self, config: EvollaConfig):
        super().__init__()
    ...
    @can_return_tuple
    def construct(self, input_ids: ms.Tensor, attention_mask: ms.Tensor, **kwargs):
...
class EvollaSequenceAlignerCrossAttention(ms.nn.Cell):
    def __init__(
        self,
        config,
        ...
        self.query = mint.nn.Linear(self.hidden_size, self.all_head_size)
        if protein_encoder_dim is not None:
            self.key_protein = mint.nn.Linear(protein_encoder_dim, self.all_head_size)
            self.value_protein = mint.nn.Linear(protein_encoder_dim, self.all_head_size)
        else:
            self.key_protein = None
            self.value_protein = None

        if structure_encoder_dim is not None:
            self.key_structure = mint.nn.Linear(structure_encoder_dim, self.all_head_size)
            self.value_structure = mint.nn.Linear(structure_encoder_dim, self.all_head_size)
        else:
            self.key_structure = None
            self.value_structure = None

        if msa_encoder_dim is not None:
            self.key_msa = mint.nn.Linear(msa_encoder_dim, self.all_head_size)
            self.value_msa = mint.nn.Linear(msa_encoder_dim, self.all_head_size)
        else:
            self.key_msa = None
            self.value_msa = None
        ...
        self.dropout = mint.nn.Dropout(attention_probs_dropout_prob)

        self.out_proj = mint.nn.Linear(self.hidden_size, self.hidden_size, bias=enable_bias)

        self.ff = EvollaFeedForward(self.hidden_size, ffn_mult)
        self.gate_attention = ms.Parameter(ms.Tensor([0.0]))
        self.gate_ffw = ms.Parameter(ms.Tensor([0.0]))
    ...
    def cross_attention(
        ...
        kv_attn_mask = mint.cat(kv_attn_mask, dim=1)
        ...
        new_key_layer_shape = key_layer.shape[:-1] + (
            self.num_attention_heads,
            self.attention_head_size,
        )
        key_layer = key_layer.view(*new_key_layer_shape).permute(0, 2, 1, 3)

        new_value_layer_shape = value_layer.shape[:-1] + (
            self.num_attention_heads,
            self.attention_head_size,
        )
        ...
        context_layer = mint.matmul(attention_probs, value_layer)  # [bs, numheads, querylength, dim/numheads]
    ...
    def construct(
        ...
        if (
            ...
            # tanh gate
            hidden_states = mint.tanh(self.gate_attention) * hidden_states

            hidden_states = residual + hidden_states  # input_query

            residual = hidden_states
            hidden_states = self.ff(hidden_states) * mint.tanh(self.gate_ffw)
...
class EvollaRotaryEmbedding(ms.nn.Cell):
    def __init__(self, config: EvollaConfig):
        super().__init__()
    ...
    def construct(self, x, position_ids):
        inv_freq_expanded = (
            ...
            .astype(ms.float32)
            .broadcast_to((position_ids.shape[0], self.inv_freq.shape[0], 1))
        )
        position_ids_expanded = position_ids[:, None, :].float()
        # Force float32 calculation without torch.autocast
        freqs = (inv_freq_expanded @ position_ids_expanded.astype(ms.float32)).transpose(1, 2)
        emb = mint.cat((freqs, freqs), dim=-1)
...
def repeat_kv(hidden_states: ms.Tensor, n_rep: int) -> ms.Tensor:
    """
    This is the equivalent of mindspore-style repeat_interleave along dim=1. The hidden states go from (batch,
...
class EvollaAttention(ms.nn.Cell):
...
class EvollaDecoderLayer(ms.nn.Cell):
    ...
    def construct(
        ...
        cache_position: Optional[ms.Tensor] = None,
        protein_kv_states: Optional[ms.Tensor] = None,
        structure_kv_states: Optional[ms.Tensor] = None,
        msa_kv_states: Optional[ms.Tensor] = None,
        protein_batch_mask: Optional[ms.Tensor] = None,
        structure_batch_mask: Optional[ms.Tensor] = None,
        msa_batch_mask: Optional[ms.Tensor] = None,
        query_attn_mask: Optional[ms.Tensor] = None,
        **kwargs,
    ) -> tuple[ms.Tensor]:
        residual = hidden_states
...
class EvollaModel(EvollaPreTrainedModel):
    ...
    def construct(
        ...
        cache_position: Optional[ms.Tensor] = None,
        protein_input_ids: Optional[ms.Tensor] = None,
        protein_attention_mask: Optional[ms.Tensor] = None,
        structure_feats: Optional[ms.Tensor] = None,
        msa_feats: Optional[ms.Tensor] = None,
        structure_batch_mask: Optional[ms.Tensor] = None,
        msa_batch_mask: Optional[ms.Tensor] = None,
        **kwargs,
    ) -> Union[tuple, BaseModelOutputWithPast]:
        r"""
        protein_input_ids (mindspore.Tensor):
            The input IDs for the protein sequence in structure-aware tokens. Should be of shape `(batch_size, protein_seq_length)` and type `mindspore.Tensor`.
        ...
        if protein_input_ids is not None and protein_attention_mask is not None:
            ...
            protein_batch_mask = ms.Tensor(
                [True] * protein_input_ids.shape[0],
            )
...
class EvollaForProteinText2Text(EvollaPreTrainedModel, GenerationMixin):
    def __init__(self, config):
        ...
        self.vocab_size = config.vocab_size
        self.lm_head = mint.nn.Linear(config.hidden_size, self.vocab_size, bias=False)
    ...
    def construct(
        self,
        input_ids: ms.Tensor = None,  # text input ids
        attention_mask: Optional[ms.Tensor] = None,  # text attention mask
        inputs_embeds: Optional[ms.Tensor] = None,  # text input embeddings
        labels: Optional[ms.Tensor] = None,
        protein_input_ids: ms.Tensor = None,
        protein_attention_mask: Optional[ms.Tensor] = None,
        use_cache: Optional[bool] = None,
        **kwargs,
    ):
        r"""
        protein_input_ids (mindspore.Tensor):
            The input IDs for the protein sequence. Should be of shape `(batch_size, protein_seq_length)` and type `mindspore.Tensor`.
        protein_attention_mask (mindspore.Tensor):
            The attention mask for the protein sequence. Should be of shape `(batch_size, protein_seq_length)` and type `mindspore.Tensor`.
