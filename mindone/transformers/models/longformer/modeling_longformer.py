...
class LongformerBaseModelOutput(ModelOutput):
    ...
    Args:
        ...
        attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length, x +
        ...
        global_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length, x)`,
    ...
    global_attentions: Optional[Tuple[ms.Tensor, ...]] = None
...
def _compute_global_attention_mask(input_ids, sep_token_id, before_sep_token=True):
    ...
    attention_mask = mint.arange(input_ids.shape[1])
    if before_sep_token is True:
        attention_mask = (attention_mask.expand_as(input_ids) < question_end_index).to(ms.bool_)
    else:
        # last token is separation token and should not be counted and in the middle are two separation tokens
        attention_mask = (attention_mask.expand_as(input_ids) > (question_end_index + 1)).to(ms.bool_) * (
...
class LongformerEmbeddings(nn.Cell):
    ...
    def create_position_ids_from_inputs_embeds(self, inputs_embeds):
        ...
        Args:
            inputs_embeds: ms.Tensor inputs_embeds:
...
class LongformerSelfAttention(nn.Cell):
    def __init__(self, config, layer_id):
        ...
        self.query = mint.nn.Linear(config.hidden_size, self.embed_dim)
        self.key = mint.nn.Linear(config.hidden_size, self.embed_dim)
        self.value = mint.nn.Linear(config.hidden_size, self.embed_dim)

        # separate projection layers for tokens with global attention
        self.query_global = mint.nn.Linear(config.hidden_size, self.embed_dim)
        self.key_global = mint.nn.Linear(config.hidden_size, self.embed_dim)
        self.value_global = mint.nn.Linear(config.hidden_size, self.embed_dim)
    ...
    def construct(
        ...
        *attention_window* happens in [`LongformerModel.forward`] to avoid redoing the padding on each layer.
        ...
        float_mask = remove_from_windowed_attention_mask.type_as(query_vectors).masked_fill(
            remove_from_windowed_attention_mask, dtype_to_min(query_vectors.dtype)
        )
        ...
        if is_global_attn:
            ...
            attn_scores = mint.cat((global_key_attn_scores, attn_scores), dim=-1)

            # free memory
            del global_key_attn_scores

        attn_probs = mint.nn.functional.softmax(
            attn_scores, dim=-1, dtype=ms.float32
        ...
        # apply dropout
        attn_probs = mint.nn.functional.dropout(attn_probs, p=self.dropout, training=self.training)
        ...
        attn_output = attn_output.swapaxes(0, 1).reshape(seq_len, batch_size, embed_dim).contiguous()
    ...
    def _pad_and_transpose_last_two_dims(hidden_states_padded, padding):
        ...
        hidden_states_padded = mint.nn.functional.pad(
    ...
    def _pad_and_diagonalize(chunked_hidden_states):
        ...
        chunked_hidden_states = mint.nn.functional.pad(
    ...
    def _chunk(hidden_states, window_overlap, onnx_export: bool = False):
        ...
        if not onnx_export:
            ...
            num_chunks = 2 * mint.div(seq_len, (window_overlap * 2), rounding_mode="trunc").item() - 1

            hidden_states = hidden_states.permute(0, 2, 1).unsqueeze(-1)  # [batch_size,hidden_dim,seqlen,1]
            hidden_states = mint.nn.functional.unfold(
                hidden_states,
        ...
        chunk_size = [
            ...
            mint.div(hidden_states.shape[1], window_overlap, rounding_mode="trunc") - 1,
            window_overlap * 2,
            hidden_states.shape[2],
        ]

        overlapping_chunks = mint.empty(chunk_size)
    ...
    @staticmethod
    def _mask_invalid_locations(input_tensor, affected_seq_len) -> ms.Tensor:
        beginning_mask_2d = input_tensor.new_ones((affected_seq_len, affected_seq_len + 1)).tril().flip(dims=[0])
        beginning_mask = beginning_mask_2d[None, :, None, :]
        ending_mask = beginning_mask.flip(dims=(1, 3))
        beginning_input = input_tensor[:, :affected_seq_len, :, : affected_seq_len + 1]
        beginning_mask = beginning_mask.broadcast_to(beginning_input.shape)
        input_tensor[:, :affected_seq_len, :, : affected_seq_len + 1] = mint.full_like(
    ...
    def _sliding_chunks_query_key_matmul(self, query: ms.Tensor, key: ms.Tensor, window_overlap: int):
        """
        ...
        chunks_count = mint.div(seq_len, window_overlap, rounding_mode="trunc") - 1
        ...
        # diagonal_chunked_attention_scores = mint.einsum("bcxd,bcyd->bcxy", (query, key))  # multiply
        diagonal_chunked_attention_scores = mint.matmul(query, key.swapaxes(2, 3))
    ...
    def _sliding_chunks_matmul_attn_probs_value(self, attn_probs: ms.Tensor, value: ms.Tensor, window_overlap: int):
        """
        ...
        chunks_count = (mint.div(seq_len, window_overlap, rounding_mode="trunc") - 1).item()
        # group batch_size and num_heads dimensions into one, then chunk seq_len into chunks of size 2 window overlap

        chunked_attn_probs = attn_probs.swapaxes(1, 2).reshape(
            batch_size * num_heads,
            mint.div(seq_len, window_overlap, rounding_mode="trunc").item(),
            window_overlap,
            2 * window_overlap + 1,
        )
        ...
        padded_value = mint.nn.functional.pad(value, (0, 0, window_overlap, window_overlap), value=-1)
        ...
        # avoid using `as_strided` because it's not supported in MindSpore. Use `unfold` instead
        chunked_value = padded_value.permute(0, 2, 1).unsqueeze(-1)  # [batch_size * num_heads, hidden_dim, seq_len, 1]
        chunked_value = mint.nn.functional.unfold(
            chunked_value,
        ...
        # context = mint.einsum("bcwd,bcdh->bcwh", (chunked_attn_probs, chunked_value))
        context = mint.matmul(chunked_attn_probs, chunked_value)
    ...
    def _get_global_attn_indices(is_index_global_attn):
        ...
        # helper variable
        is_local_index_global_attn = mint.arange(
            max_num_global_attn_indices.item()
    ...
    def _concat_with_global_key_attn_probs(
        ...
        # attn_probs_from_global_key = mint.einsum("blhd,bshd->blhs", (query_vectors, key_vectors_only_global))
        query_transposed = query_vectors.swapaxes(1, 2)
        key_transposed = key_vectors_only_global.swapaxes(1, 2).swapaxes(2, 3)
        attn_probs_from_global_key = mint.matmul(query_transposed, key_transposed).swapaxes(1, 2)
        ...
        ] = dtype_to_min(attn_probs_from_global_key.dtype)
        attn_probs_from_global_key = attn_probs_from_global_key.swapaxes(1, 3)
    ...
    def _compute_attn_output_with_global_indices(
        ...
        attn_probs_only_global = attn_probs.narrow(-1, 0, max_num_global_attn_indices.item())
        ...
        # attn = mint.einsum('blhs,bshd->blhd', (selected_attn_probs, selected_v))
        # compute attn output only global
        attn_output_only_global = mint.matmul(
            attn_probs_only_global.swapaxes(1, 2).clone(), value_vectors_only_global.swapaxes(1, 2).clone()
        ).swapaxes(1, 2)

        # reshape attn probs
        attn_probs_without_global = attn_probs.narrow(
            -1, max_num_global_attn_indices.item(), attn_probs.shape[-1] - max_num_global_attn_indices.item()
        ).contiguous()
    ...
    def _compute_global_attn_output_from_hidden(
        ...
        global_query_vectors_only_global = (
            ...
            .view(max_num_global_attn_indices.item(), batch_size * self.num_heads, self.head_dim)
            .swapaxes(0, 1)
        )  # (batch_size * self.num_heads, max_num_global_attn_indices, head_dim)
        global_key_vectors = (
            global_key_vectors.contiguous().view(-1, batch_size * self.num_heads, self.head_dim).swapaxes(0, 1)
        )  # batch_size * self.num_heads, seq_len, head_dim)
        global_value_vectors = (
            global_value_vectors.contiguous().view(-1, batch_size * self.num_heads, self.head_dim).swapaxes(0, 1)
        )  # batch_size * self.num_heads, seq_len, head_dim)

        # compute attn scores
        global_attn_scores = mint.bmm(global_query_vectors_only_global, global_key_vectors.swapaxes(1, 2))
        ...
        global_attn_scores = global_attn_scores.view(
            batch_size, self.num_heads, max_num_global_attn_indices.item(), seq_len
        )
        ...
        global_attn_scores[
            is_local_index_no_global_attn_nonzero[0], is_local_index_no_global_attn_nonzero[1], :, :
        ] = dtype_to_min(global_attn_scores.dtype)
        global_attn_scores = global_attn_scores.swapaxes(1, 2)
        ...
        global_attn_probs_float = mint.nn.functional.softmax(
            global_attn_scores, dim=-1, dtype=ms.float32
        )  # use fp32 for numerical stability
        ...
        global_attn_probs = mint.nn.functional.dropout(
            global_attn_probs_float.type_as(global_attn_scores), p=self.dropout, training=self.training
        )

        # global attn output
        global_attn_output = mint.bmm(global_attn_probs, global_value_vectors)
        ...
        global_attn_output = global_attn_output.view(
            batch_size, self.num_heads, max_num_global_attn_indices.item(), self.head_dim
        )
...
class LongformerEncoder(nn.Cell):
    ...
    def construct(
        ...
        for idx, layer_module in enumerate(self.layer):
            ...
            if output_attentions:
                # bzs x seq_len x num_attn_heads x (num_global_attn + attention_window_len + 1) => bzs x num_attn_heads x seq_len x (num_global_attn + attention_window_len + 1)  # noqa: E501
...
LONGFORMER_START_DOCSTRING = r"""
    ...
    MindSpore [ms.nn.Cell](https://www.mindspore.cn/docs/zh-CN/master/api_python/nn/mindspore.nn.Cell.html) subclass.
...
LONGFORMER_INPUTS_DOCSTRING = r"""
    Args:
        ...
        global_attention_mask (`ms.Tensor` of shape `({0})`, *optional*):
        ...
        head_mask (`ms.Tensor` of shape `(num_layers, num_heads)`, *optional*):
...
class LongformerModel(LongformerPreTrainedModel):
    ...
    def _merge_to_attention_mask(self, attention_mask: ms.Tensor, global_attention_mask: ms.Tensor):
    ...
    def construct(
        ...
        global_attention_mask: Optional[ms.Tensor] = None,
        ...
        >>> from mindone.transformers import LongformerModel

        >>> model = LongformerModel.from_pretrained("allenai/longformer-base-4096", revision="refs/pr/7")
        >>> tokenizer = AutoTokenizer.from_pretrained("allenai/longformer-base-4096", revision="refs/pr/7")

        >>> SAMPLE_TEXT = " ".join(["Hello world! "] * 1000)  # long input document
        >>> input_ids = ms.tensor(tokenizer.encode(SAMPLE_TEXT)).unsqueeze(0)  # batch of size 1

        >>> attention_mask = mint.ones(input_ids.shape, dtype=ms.int64)  # initialize to local attention
        >>> global_attention_mask = mint.zeros(
        ...     input_ids.shape, dtype=ms.int64
        ...
        extended_attention_mask: ms.Tensor = self.get_extended_attention_mask(attention_mask, input_shape)[:, 0, 0, :]
...
class LongformerForMaskedLM(LongformerPreTrainedModel):
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import LongformerForMaskedLM
        >>> import mindspore as ms

        >>> tokenizer = AutoTokenizer.from_pretrained("allenai/longformer-base-4096", revision="refs/pr/7")
        >>> model = LongformerForMaskedLM.from_pretrained("allenai/longformer-base-4096", revision="refs/pr/7")
        ```
        ...
        >>> probs = logits[0, masked_index].softmax(axis=0)
...
class LongformerForSequenceClassification(LongformerPreTrainedModel):
    ...
    def construct(
        ...
        if global_attention_mask is None:
            ...
            global_attention_mask = mint.zeros_like(input_ids)
...
class LongformerForQuestionAnswering(LongformerPreTrainedModel):
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import LongformerForQuestionAnswering
        >>> import mindspore as ms
        >>> from mindspore import mint

        >>> tokenizer = AutoTokenizer.from_pretrained(
        ...     "allenai/longformer-large-4096-finetuned-triviaqa", revision="refs/pr/3"
        ... )
        ...
        >>> input_ids = ms.tensor(encoding["input_ids"])

        >>> # default is local attention everywhere
        >>> # the forward method will automatically set global attention on question tokens
        >>> attention_mask = ms.tensor(encoding["attention_mask"])
        ...
        >>> answer_tokens = all_tokens[mint.argmax(start_logits) : mint.argmax(end_logits) + 1]
...
class LongformerForMultipleChoice(LongformerPreTrainedModel):
    ...
    def construct(
        ...
        if global_attention_mask is None and input_ids is not None:
            ...
            global_attention_mask = mint.stack(
                [
