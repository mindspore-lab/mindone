...
def repeat_kv(hidden_states: mint.Tensor, n_rep: int) -> mint.Tensor:
    """
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    Args:
        q (`mint.Tensor`): The query tensor.
        k (`mint.Tensor`): The key tensor.
        cos (`mint.Tensor`): The cosine part of the rotary embedding.
        sin (`mint.Tensor`): The sine part of the rotary embedding.
        position_ids (`mint.Tensor`, *optional*):
    ...
    Returns:
        `tuple(mint.Tensor)` comprising of the query and key tensors rotated using the Rotary Position Embedding.
    """
    dtype = q.dtype
    q = q.float()
    k = k.float()
...
class Cohere2Attention(nn.Cell):
    ...
    def construct(
        self,
        hidden_states: mint.Tensor,
        position_embeddings: Tuple[mint.Tensor, mint.Tensor],
        attention_mask: Optional[mint.Tensor],
...
class Cohere2DecoderLayer(nn.Cell):
    ...
    def construct(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
    ) -> Tuple[mint.Tensor, Optional[Tuple[mint.Tensor, mint.Tensor]]]:
        """
        Args:
            hidden_states (`mint.Tensor`): input to the layer of shape `(batch, seq_len, embed_dim)`
            position_embeddings (`Tuple[mint.Tensor, mint.Tensor]`):
                Tuple containing the cosine and sine positional embeddings of shape `(batch_size, seq_len, head_dim)`,
                with `head_dim` being the embedding dimension of each attention head.
            attention_mask (`mint.Tensor`, *optional*):
                attention mask of size `(batch_size, sequence_length)` if flash attention is used or `(batch_size, 1,
                query_sequence_length, key_sequence_length)` if default attention is used.
            past_key_value (`Tuple(mint.Tensor)`, *optional*): cached past key and value projection states
            ...
            use_cache (`bool`, *optional*):
                ...
                (see `past_key_values`).
            cache_position (`mint.Tensor` of shape `(sequence_length)`, *optional*):
...
COHERE2_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`mint.Tensor` of shape `(batch_size, sequence_length)`):
        ...
        attention_mask (`mint.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
        ...
        position_ids (`mint.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            Indices of positions of each input sequence tokens in the position embeddings. Selected in the range `[0,
            config.n_positions - 1]`.

            [What are position IDs?](../glossary#position-ids)
        past_key_values (`Cache` or `tuple(tuple(mint.Tensor))`, *optional*):
            Pre-computed hidden-states (key and values in the self-attention blocks and in the cross-attention
            blocks) that can be used to speed up sequential decoding. This typically consists in the `past_key_values`
            returned by the model at a previous stage of decoding, when `use_cache=True` or `config.use_cache=True`.

            Two formats are allowed:
            - a [`~cache_utils.CCache`] instance, see our
            [kv cache guide](https://huggingface.co/docs/transformers/en/kv_cache);
            - Tuple of `tuple(mint.Tensor)` of length `config.n_layers`, with each tuple having 2 tensors of
        ...
        inputs_embeds (`mint.Tensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
        ...
        cache_position (`mint.LLongTensor` of shape `(sequence_length)`, *optional*):
...
class Cohere2Model(Cohere2PreTrainedModel):
    ...
    def construct(
        ...
        attention_mask: Optional[mint.Tensor] = None,
        position_ids: Optional[mint.Tensor] = None,
        past_key_values: Optional[HybridCache] = None,
        inputs_embeds: Optional[mint.Tensor] = None,
...
class Cohere2ForCausalLM(Cohere2PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        attention_mask: Optional[mint.Tensor] = None,
        position_ids: Optional[mint.Tensor] = None,
        past_key_values: Optional[Union[Cache, List[mint.Tensor]]] = None,
        inputs_embeds: Optional[mint.Tensor] = None,
        labels: Optional[mint.Tensor] = None,
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        r"""
            ...
            logits_to_keep (`int` or `mint.Tensor`, *optional*):
                If an `int`, compute logits for the last `logits_to_keep` tokens. If `0`, calculate logits for all
                `input_ids` (special case). Only last token logits are needed for generation, and calculating them only for that
                token can save memory, which becomes pretty significant for long sequences or large vocabulary size.
                If a `mint.Tensor`, must be 1D corresponding to the indices to keep in the sequence length dimension.
        ...
        >> inputs = mint.Tensor(tokenizer(prompt, return_tensors="np"))
