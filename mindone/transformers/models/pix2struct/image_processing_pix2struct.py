...
class Pix2StructImageProcessor(BaseImageProcessor):
    ...
    def extract_flattened_patches(
        ...
        **kwargs,
        ...
        # [rows * columns, 1]
        row_ids = mindspore.mint.arange(rows).reshape([rows, 1]).repeat(1, columns).reshape([rows * columns, 1])
        col_ids = mindspore.mint.arange(columns).reshape([1, columns]).repeat(rows, 1).reshape([rows * columns, 1])
