...
class Pix2StructVisionAttention(nn.Cell):
    ...
    def construct(
        ...
        if position_bias is None:
            position_bias = mint.zeros((1, self.n_heads, seq_length, seq_length), dtype=scores.dtype)
            ...
            if attention_mask.dim() == 2:
                ...
                attention_mask = mint.ones((batch_size, seq_length), dtype=position_bias.dtype)
                position_bias = position_bias + attention_mask
...
# Copied from transformers.models.t5.modeling_t5.T5DenseGatedActDense with T5DenseGatedActDense->Pix2StructVisionMlp,T5Config
# ->Pix2StructVisionConfig,config.d_model->config.hidden_size,dropout_rate->dropout_rate
...
# Copied from transformers.models.t5.modeling_t5.T5LayerSelfAttention with
# T5LayerNorm->Pix2StructLayerNorm,T5Attention->Pix2StructTextAttention,T5LayerSelfAttention->Pix2StructTextLayerSelfAttention,self.SelfAttention,
...
# Copied from transformers.models.t5.modeling_t5.T5LayerCrossAttention with T5LayerNorm->Pix2StructLayerNorm,T5Attention
# ->Pix2StructTextAttention,T5LayerCrossAttention->Pix2StructTextLayerCrossAttention,self.EncDecAttention->self.attention,config.d_model->config.hidden_size
    ...
    def _reorder_cache(self, past_key_values, beam_idx):
        ...
        for layer_past_states in past_key_values:
            ...
            if reordered_layer_past_states[0].shape != layer_past_states[0].shape:
                raise ValueError(
                    f"reordered_layer_past_states[0] shape {reordered_layer_past_states[0].shape} and layer_past_states[0] shape "
                    f"{layer_past_states[0].shape} mismatched"
                )
            if len(reordered_layer_past_states) != len(layer_past_states):
                raise ValueError(
                    f"length of reordered_layer_past_states {len(reordered_layer_past_states)} and length of layer_past_states "
                    f"{len(layer_past_states)} mismatched"
                )
