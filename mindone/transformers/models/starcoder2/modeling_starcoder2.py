...
class Starcoder2MLP(nn.Cell):
    def __init__(self, config: Starcoder2Config):
        ...
        self.c_fc = mint.nn.Linear(embed_dim, config.intermediate_size, bias=config.use_bias)
        self.c_proj = mint.nn.Linear(config.intermediate_size, embed_dim, bias=config.use_bias)
    ...
    def construct(self, hidden_states: Optional[Tuple[ms.Tensor]]) -> ms.Tensor:
        ...
        hidden_states = mint.nn.functional.dropout(hidden_states, p=self.residual_dropout, training=self.training)
...
class Starcoder2Attention(nn.Cell):
    ...
    def construct(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        attn_output = mint.nn.functional.dropout(
    ...
    def construct(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        attn_output, _ = flash_attention_forward(
            ...
            dropout=0.0 if not self.training else self.attention_dropout,
            scaling=self.scaling,
            sliding_window=None,  # Todo: support sliding_window
            **kwargs,
        )
...
class Starcoder2DecoderLayer(nn.Cell):
    def __init__(self, config: Starcoder2Config, layer_idx: int):
        ...
        self.input_layernorm = mint.nn.LayerNorm(config.hidden_size, eps=config.norm_epsilon)
        self.post_attention_layernorm = mint.nn.LayerNorm(config.hidden_size, eps=config.norm_epsilon)
...
class Starcoder2Model(Starcoder2PreTrainedModel):
    ...
    def __init__(self, config: Starcoder2Config):
        ...
        self.norm = mint.nn.LayerNorm(config.hidden_size, eps=config.norm_epsilon)
