...
class InstructBlipVisionEmbeddings(nn.Cell):
...
class InstructBlipAttention(nn.Cell):
...
class InstructBlipEncoderLayer(nn.Cell):
...
class InstructBlipEncoder(nn.Cell):
    """
    ...
    def __init__(self, config: InstructBlipConfig):
        super().__init__()
        self.config = config
        self.layers = nn.CellList([InstructBlipEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class InstructBlipQFormerMultiHeadAttention(nn.Cell):
...
class InstructBlipQFormerSelfOutput(nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class InstructBlipQFormerAttention(nn.Cell):
    def __init__(self, config, is_cross_attention=False):
        super().__init__()
...
class InstructBlipQFormerIntermediate(nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class InstructBlipQFormerOutput(nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class InstructBlipQFormerLayer(nn.Cell):
...
class InstructBlipQFormerEncoder(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class InstructBlipQFormerEmbeddings(nn.Cell):
...
class InstructBlipForConditionalGeneration(InstructBlipPreTrainedModel, GenerationMixin):
    ...
    def _preprocess_accelerate(self):
        ...
        if (
            ...
            and ms.device_context.ascend.device_count() > 1
        ):
    ...
    def construct(
        ...
        The unusual aspect of this image is that a man is ironing clothes on the back of a yellow SUV, which is parked in the middle of a busy city street.
        This is an unconventional approach to ironing clothes, as it requires the man to balance himself and his ironing equipment on top of the vehicle while
        navigating through traffic. Additionally, the presence of taxis and other vehicles in the scene further emphasizes the unusual nature of this situation.
        ```"""
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        ...
        image_attention_mask = mint.ones(
        ...
        query_attention_mask = mint.ones(
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        inputs_embeds = inputs_embeds.to(language_model_inputs.dtype)
