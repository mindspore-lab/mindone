...
class PegasusXScaledWordEmbedding(mint.nn.Embedding):
    """
...
class PegasusXSinusoidalPositionalEmbedding(nn.Cell):
    ...
    @ms._no_grad()
    def construct(self, input_embeds: ms.Tensor, past_key_values_length: int = 0) -> ms.Tensor:
        """`input_ids_shape` is expected to be [bsz x seqlen]."""
        batch_size, seq_len = input_embeds.shape[:2]
        positions = mint.arange(
            past_key_values_length, past_key_values_length + seq_len, dtype=ms.int64, )[:, None]
        pe = mint.zeros((seq_len, self.embed_dim), dtype=input_embeds.dtype)
        half_d_feature = self.embed_dim // 2
        div_term = mint.exp(
            mint.arange(half_d_feature, dtype=ms.int64).type_as(input_embeds)
            * -(np.log(float(self.max_scale)) / (half_d_feature - 1))
        )
        pe[:, :half_d_feature] = mint.sin(positions * div_term)
        pe[:, half_d_feature:] = mint.cos(positions * div_term)
...
class PegasusXGlobalLocalAttention(nn.Cell):
    ...
    def __init__(
        ...
        self.v_proj = mint.nn.Linear(embed_dim, embed_dim, bias=False)
        self.q_proj = mint.nn.Linear(embed_dim, embed_dim, bias=False)
        self.out_proj = mint.nn.Linear(embed_dim, embed_dim, bias=False)
    ...
    def construct(
        self,
        token_hidden_states: ms.Tensor,
        global_hidden_states: ms.Tensor,
        attention_mask: Optional[ms.Tensor] = None,
        output_attentions: bool = False,
    ) -> Tuple[ms.Tensor, ms.Tensor, Optional[ms.Tensor]]:
        """Input shape: Batch x Time x Channel"""
        dim = DimensionInfo(
    ...
    def compute_global_attention_representations(
        ...
        Args:
            global_q (`ms.Tensor`) of shape [batch_size, num_heads, global_len, dim_per_head]:
                query vectors from global tokens
            global_k (`ms.Tensor`) of shape [batch_size, num_heads, global_len, dim_per_head]:
                key vectors from global tokens
            global_v (`ms.Tensor`) of shape [batch_size, num_heads, global_len, dim_per_head]:
                value vectors from global tokens
            local_k (`ms.Tensor`) of shape [batch_size, num_heads, padded_seq_len, dim_per_head]:
                key vectors from local tokens
            local_v (`ms.Tensor`) of shape [batch_size, num_heads, padded_seq_len, dim_per_head]:
                value vectors from local tokens
            mask (`ms.Tensor`) of shape [batch_size, padded_seq_len]: attention mask
        ...
        global_and_local_k = mint.cat([global_k, local_k], dim=2)
        # [batch_size, num_heads, global_len+padded_seq_len, dim_per_head]
        global_and_local_v = mint.cat([global_v, local_v], dim=2)

        # [batch_size, global_len+padded_seq_len]
        extended_mask = mint.nn.functional.pad(mask, pad=(dim.global_len, 0), value=0)

        # [batch_size, num_heads, global_len, global_len+padded_seq_len]
        attn_weights = mint.einsum("BHGF,BHXF->BHGX", global_q, global_and_local_k)
        attn_weights = attn_weights + extended_mask[:, None, None, :]
        attn_probs = mint.nn.functional.softmax(attn_weights, dim=-1)
        attn_probs = mint.nn.functional.dropout(attn_probs, p=self.dropout, training=self.training)

        # [batch_size, num_heads, global_len, F]
        attn_output = mint.einsum("BHGX,BHXF->BHGF", attn_probs, global_and_local_v)
    ...
    def compute_local_attention_representations(
        ...
        Args:
            ...
            global_v (`ms.Tensor`) of shape [batch_size, num_heads, global_len, dim_per_head]:
                value vectors from global tokens
            local_q (`ms.Tensor`) of shape [batch_size, num_heads, padded_seq_len, dim_per_head]:
        ...
        extended_mask = mint.nn.functional.pad(
        ...
        blocked_local2global = mint.einsum("BHNKF,BHGF->BHNKG", blocked_local_q, global_k)
        # [batch_size, num_heads, num_blocks, block_size, block_size]
        blocked_local2local = mint.einsum("BHNKF,BHNXF->BHNKX", blocked_local_q, blocked_local_k)

        # [batch_size, num_heads, num_blocks, block_size, global_len+block_size]
        attn_weights = mint.cat([blocked_local2global, blocked_local2local], dim=-1)
        attn_weights = attn_weights + extended_mask[:, None, :, None, :]
        attn_probs = mint.nn.functional.softmax(attn_weights, dim=-1)
        attn_probs = mint.nn.functional.dropout(attn_probs, p=self.dropout, training=self.training)
        ...
        # [batch_size, num_heads, num_blocks, block_size, dim_per_head]
        local2global_attn_output = mint.einsum("BHNKG,BHGF->BHNKF", local2global_attn_probs, global_v)
        # [batch_size, num_heads, num_blocks, block_size, dim_per_head]
        local2local_attn_output = mint.einsum("BHNKX,BHNXF->BHNKF", local2local_attn_probs, blocked_local_v)
        # [batch_size, num_heads, num_blocks, block_size, dim_per_head]
...
class PegasusXEncoderLayer(nn.Cell):
    def __init__(self, stagger_blocks_this_layer: bool, config: PegasusXConfig):
        ...
        self.global_self_attn_layer_norm = mint.nn.LayerNorm(self.embed_dim)
    ...
    def construct(
        ...
        Args:
            ...
            global_hidden_states (`ms.Tensor`): global token hidden states
        ...
        global_hidden_states = mint.nn.functional.dropout(global_hidden_states, p=self.dropout, training=self.training)
        ...
        global_hidden_states = mint.nn.functional.dropout(
    ...
    def pad_local_tokens(cls, hidden_states, attention_mask, block_size):
        ...
        mask_min_value = dtype_to_min(hidden_states.dtype)
        padded_hidden_states = mint.nn.functional.pad(
            hidden_states,
...
PEGASUS_X_START_DOCSTRING = r"""
    ...
    This model is also a MindSpore [nn.Cell](https://www.mindspore.cn/docs/zh-CN/master/api_python/nn/nn.Cell.html) subclass.
...
PEGASUS_X_GENERATION_EXAMPLE = r"""
    ...
    >>> from mindone.transformers import PegasusXForConditionalGeneration
    >>> import mindspore as ms
...
class PegasusXEncoder(PegasusXPreTrainedModel):
    ...
    def __init__(self, config: PegasusXConfig, embed_tokens: Optional[mint.nn.Embedding] = None):
        ...
        self.embed_global = mint.nn.Embedding(config.num_global_tokens, embed_dim)
    ...
    def construct(
        ...
        if attention_mask is None:
            attention_mask = mint.ones(*input_shape, dtype=inputs_embeds.dtype, )
        ...
        global_hidden_states = self.embed_global(
            mint.arange(self.config.num_global_tokens, )[None].expand((batch_size, -1))
        )
...
class PegasusXModel(PegasusXPreTrainedModel):
    ...
    def get_position_embeddings(self) -> Tuple[mint.nn.Embedding]:
        """
        Returns the position embeddings matrix
        """
    ...
    def construct(
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import PegasusModel
        >>> import mindspore as ms

        >>> tokenizer = AutoTokenizer.from_pretrained("google/pegasus-x-large")
        >>> model = PegasusModel.from_pretrained("google/pegasus-x-large")

        >>> inputs = tokenizer("Studies have been shown that owning a dog is good for you", return_tensors="np")
        >>> for k, v in inputs.items():
        ...     inputs[k] = ms.tensor(v)
        >>> decoder_inputs = tokenizer("Studies show that", return_tensors="np")
        >>> for k, v in decoder_inputs.items():
        ...     decoder_inputs[k] = ms.tensor(v)
