...
def soft_cap(values: ms.Tensor, cap_value: Optional[Union[float, ms.Tensor]] = None) -> ms.Tensor:
    """
    ...
    if cap_value is None:
        return values
    return cap_value * mint.tanh(values / cap_value)
...
def mlstm_chunkwise_recurrent_fw_C(
    ...
    for key in range(0, num_chunks):
        ...
        scaGbar_k = mint.exp(scaG_k + scaM_inter_k - scaM_inter_k_next)[:, :, None]
...
def mlstm_chunkwise_parallel_fw_H(
    ...
    matF_logsig_mask_chunk = mint.where(ltr, matF_logsig_chunk, -float("inf"))

    matLogD_chunk = matF_logsig_mask_chunk + vecI[:, :, :, None, :]

    # max_state intra
    vecMintra_k = mint.max(matLogD_chunk, dim=-1, keepdim=False).values

    # max_state combined
    vecM_b_inter = vecB + scaMinter_k_states[:, :, :, None]
    vecM_k_combine = mint.maximum(vecM_b_inter, vecMintra_k)

    vecM_k_combine = vecM_k_combine[:, :, :, :, None]
    vecM_b_inter = vecM_b_inter[:, :, :, :, None]

    matLogD_stabilized_chunk = matLogD_chunk - vecM_k_combine
    matD_chunk = mint.exp(matLogD_stabilized_chunk)
    ...
    vecBbar = mint.exp(vecM_b_inter - vecM_k_combine)
    ...
    vecDenom_max_common = mint.maximum(mint.abs(vecDenom_l_common), mint.exp(-vecM_k_combine))
...
def mlstm_chunkwise_fw(
    ...
    ms.Tensor,
    ms.Tensor,
    Optional[tuple[ms.Tensor, ms.Tensor, ms.Tensor]],
    Optional[tuple[ms.Tensor, ms.Tensor, ms.Tensor]],
]:
...
def mlstm_recurrent_step_native(
    ...
    **kwargs,
    ...
    # gates
    scaF_log = mint.nn.functional.logsigmoid(fgate)

    # update rule
    scaM_state_new = mint.max(scaF_log + scaM_old, igate)

    scaF_act = mint.exp(scaF_log + scaM_old - scaM_state_new)
    scaI_act = mint.exp(igate - scaM_state_new)
    ...
    max_val = mint.exp(-scaM_state_new)
    h_denom = (mint.maximum(qn_dotproduct.abs(), max_val) + eps).to(dtype=dtype_state)
    h = h_num / h_denom
...
def mlstm_recurrent_sequence_native(
    ...
    **kwargs,
    ...
    matH = mint.stack(vecH_list, dim=-2)
...
class xLSTMRMSNorm(ms.nn.Cell):
    ...
    def _apply_weight_bias(self, x: ms.Tensor) -> ms.Tensor:
    ...
    def _rms_normalize(self, x: ms.Tensor) -> ms.Tensor:
        # apply rms norm over the last dimension, i.e. HD dimension
        in_dtype = x.dtype
        if self.force_float32_reductions:
            x = x.float()
        x = x * mint.rsqrt(x.pow(2).mean(dim=-1, keepdim=True) + self.eps)
...
class xLSTMMultiHeadLayerNorm(ms.nn.Cell):
    ...
    def __init__(
        ...
        if use_weight:
            self.weight = ms.Parameter(mint.ones(self.num_features))
        else:
            self.weight = None

        if use_bias:
            self.bias = ms.Parameter(mint.zeros(self.num_features))
        else:
            self.bias = None
    ...
    def _layer_normalize(self, x: ms.Tensor) -> ms.Tensor:
...
class xLSTMFeedForward(ms.nn.Cell):
    def __init__(self, config: xLSTMConfig):
        super().__init__()
        self.config = config
        ...
        if self.config.weight_mode == "single":
            self.proj_up_gate = mint.nn.Linear(
                in_features=config.hidden_size,
                out_features=self.up_proj_dim,
                bias=self.config.use_bias,
            )
            self.proj_up = mint.nn.Linear(
                in_features=config.hidden_size,
                out_features=self.up_proj_dim,
                bias=self.config.use_bias,
            )
        elif self.config.weight_mode == "fused":
            self.proj_up_gate_z = mint.nn.Linear(
                in_features=config.hidden_size,
                out_features=2 * self.up_proj_dim,
                bias=self.config.use_bias,
            )

        self.proj_down = mint.nn.Linear(
...
class xLSTMLayer(ms.nn.Cell):
    def __init__(self, config: xLSTMConfig):
        ...
        if self.config.weight_mode == "single":
            self.q = mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=self.qk_dim,
                bias=self.config.use_bias,
            )
            self.k = mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=self.qk_dim,
                bias=self.config.use_bias,
            )
            self.v = mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=self.v_dim,
                bias=self.config.use_bias,
            )

            self.ogate_preact = mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=self.v_dim,
                bias=self.config.use_bias,
            )
            self.igate_preact = mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=self.config.num_heads,
                bias=True,
            )
            self.fgate_preact = mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=self.config.num_heads,
                bias=True,
            )
        elif self.config.weight_mode == "fused":
            self.qkv_opreact = mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=2 * self.qk_dim + 2 * self.v_dim,
                bias=self.config.use_bias,
            )
            self.ifgate_preact = mint.nn.Linear(
                in_features=self.config.hidden_size,
                out_features=2 * self.config.num_heads,
                bias=True,
            )

        self.ogate_act_fn = mint.nn.Sigmoid()
...
class xLSTMCache:
    ...
    def reset(self):
        self.rnn_state = {
            layer: (
                mint.zeros_like(self.rnn_state[layer][0]),
                mint.zeros_like(self.rnn_state[layer][1]),
                mint.zeros_like(self.rnn_state[layer][2]),
            )
...
class xLSTMOutput(ModelOutput):
    ...
    last_hidden_state: Optional[ms.Tensor]
...
class xLSTMModel(xLSTMPreTrainedModel):
    def __init__(self, config):
        ...
        self.embeddings = mint.nn.Embedding(config.vocab_size, config.embedding_dim)
        self.blocks = ms.nn.CellList([xLSTMBlock(config) for _ in range(config.num_blocks)])
