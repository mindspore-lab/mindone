...
class FunnelEmbeddings(nn.Cell):
    def __init__(self, config: FunnelConfig) -> None:
        ...
        self.layer_norm = mint.nn.LayerNorm(config.d_model, eps=config.layer_norm_eps)
        self.dropout = mint.nn.Dropout(config.hidden_dropout)

    def construct(self, input_ids: Optional[ms.Tensor] = None, inputs_embeds: Optional[ms.Tensor] = None) -> ms.Tensor:
...
class FunnelAttentionStructure(nn.Cell):
    ...
    def __init__(self, config: FunnelConfig) -> None:
        super().__init__()
        self.config = config
        self.sin_dropout = mint.nn.Dropout(config.hidden_dropout)
        self.cos_dropout = mint.nn.Dropout(config.hidden_dropout)
    ...
    def init_attention_inputs(
        ...
        cls_mask = (
            mint.nn.functional.pad(inputs_embeds.new_ones([seq_len - 1, seq_len - 1]), (1, 0, 1, 0))
            if self.config.separate_cls
            else None
        )
        return (position_embeds, token_type_mat, attention_mask, cls_mask)

    def token_type_ids_to_mat(self, token_type_ids: ms.Tensor) -> ms.Tensor:
    ...
    def get_position_embeds(self, seq_len: int, dtype: ms.Type) -> Union[Tuple[ms.Tensor], List[List[ms.Tensor]]]:
        """
        ...
        if self.config.attention_type == "factorized":
            ...
            pos_seq = mint.arange(0, seq_len, 1.0, dtype=ms.int64).to(dtype)
            freq_seq = mint.arange(0, d_model // 2, 1.0, dtype=ms.int64).to(dtype)
            inv_freq = 1 / (10000 ** (freq_seq / (d_model // 2)))
            sinusoid = pos_seq[:, None] * inv_freq[None]
            sin_embed = mint.sin(sinusoid)
            sin_embed_d = self.sin_dropout(sin_embed)
            cos_embed = mint.cos(sinusoid)
            cos_embed_d = self.cos_dropout(cos_embed)
            # This is different from the formula on the paper...
            phi = mint.cat([sin_embed_d, sin_embed_d], dim=-1)
            psi = mint.cat([cos_embed, sin_embed], dim=-1)
            pi = mint.cat([cos_embed_d, cos_embed_d], dim=-1)
            omega = mint.cat([-sin_embed, cos_embed], dim=-1)
            ...
            rel_pos_id = mint.arange(-seq_len * 2, seq_len * 2, 1.0, dtype=ms.int64).to(dtype)
            zero_offset = seq_len * 2
            sinusoid = rel_pos_id[:, None] * inv_freq[None]
            sin_embed = self.sin_dropout(mint.sin(sinusoid))
            cos_embed = self.cos_dropout(mint.cos(sinusoid))
            pos_embed = mint.cat([sin_embed, cos_embed], dim=-1)

            pos = mint.arange(0, seq_len, dtype=ms.int64).to(dtype)
            pooled_pos = pos
            ...
            for block_index in range(0, self.config.num_blocks):
                ...
                if block_index == 0:
                    ...
                    position_embeds_pooling = mint.gather(pos_embed, 0, rel_pos)
    ...
    def stride_pool_pos(self, pos_id: ms.Tensor, block_index: int):
        """
        ...
        if self.config.separate_cls:
            ...
            cls_pos = ms.tensor([-(2**block_index) + 1], dtype=pos_id.dtype)
            pooled_pos_id = pos_id[1:-1] if self.config.truncate_seq else pos_id[1:]
            return mint.cat([cls_pos, pooled_pos_id[::2]], 0)
        else:
            return pos_id[::2]

    def relative_pos(self, pos: ms.Tensor, stride: int, pooled_pos=None, shift: int = 1) -> ms.Tensor:
        """
    ...
    def stride_pool(
        self,
        tensor: Union[ms.Tensor, Tuple[ms.Tensor], List[ms.Tensor]],
    ...
    def pool_tensor(
        self, tensor: Union[ms.Tensor, Tuple[ms.Tensor], List[ms.Tensor]], mode: str = "mean", stride: int = 2
    ) -> ms.Tensor:
        ...
        if self.config.separate_cls:
            ...
            tensor = mint.cat([tensor[:, :1], suffix], dim=1)
        ...
        if mode == "mean":
            tensor = mint.nn.functional.avg_pool2d(tensor, stride, stride=stride, ceil_mode=True)
        elif mode == "max":
            tensor = mint.nn.functional.max_pool2d(tensor, stride, stride=stride, ceil_mode=True)
        elif mode == "min":
            tensor = -mint.nn.functional.max_pool2d(-tensor, stride, stride=stride, ceil_mode=True)
        else:
    ...
    def pre_attention_pooling(self, output, attention_inputs: Tuple[ms.Tensor]) -> Tuple[ms.Tensor, Tuple[ms.Tensor]]:
    ...
    def post_attention_pooling(self, attention_inputs: Tuple[ms.Tensor]) -> Tuple[ms.Tensor]:
...
def _relative_shift_gather(positional_attn: ms.Tensor, context_len: int, shift: int) -> ms.Tensor:
    ...
    # idxs = context_len + mint.arange(0, context_len).unsqueeze(0) - mint.arange(0, seq_len).unsqueeze(1)
    # # matrix of context_len + i-j
    # return positional_attn.gather(3, idxs.broadcast_to((batch_size, n_head, context_len, context_len)))

    positional_attn = mint.reshape(positional_attn, [batch_size, n_head, max_rel_len, seq_len])
    positional_attn = positional_attn[:, :, shift:, :]
    positional_attn = mint.reshape(positional_attn, [batch_size, n_head, seq_len, max_rel_len - shift])
...
class FunnelRelMultiheadAttention(nn.Cell):
    def __init__(self, config: FunnelConfig, block_index: int) -> None:
        ...
        self.hidden_dropout = mint.nn.Dropout(config.hidden_dropout)
        self.attention_dropout = mint.nn.Dropout(config.attention_dropout)

        self.q_head = mint.nn.Linear(d_model, n_head * d_head, bias=False)
        self.k_head = mint.nn.Linear(d_model, n_head * d_head)
        self.v_head = mint.nn.Linear(d_model, n_head * d_head)

        self.r_w_bias = ms.Parameter(mint.zeros([n_head, d_head]))
        self.r_r_bias = ms.Parameter(mint.zeros([n_head, d_head]))
        self.r_kernel = ms.Parameter(mint.zeros([d_model, n_head, d_head]))
        self.r_s_bias = ms.Parameter(mint.zeros([n_head, d_head]))
        self.seg_embed = ms.Parameter(mint.zeros([2, n_head, d_head]))

        self.post_proj = mint.nn.Linear(n_head * d_head, d_model)
        self.layer_norm = mint.nn.LayerNorm(d_model, eps=config.layer_norm_eps)
    ...
    def relative_positional_attention(self, position_embeds, q_head, context_len, cls_mask=None):
        ...
        if self.config.attention_type == "factorized":
            ...
            q_r_attention = mint.einsum("binh,dnh->bind", q_head + u, w_r)
            q_r_attention_1 = q_r_attention * phi[:, None]
            q_r_attention_2 = q_r_attention * pi[:, None]

            # Shape batch_size x n_head x seq_len x context_len
            positional_attn = mint.einsum("bind,jd->bnij", q_r_attention_1, psi) + mint.einsum(
            ...
            r_head = mint.einsum("td,dnh->tnh", r, w_r)
            # Shape batch_size x n_head x seq_len x max_rel_len
            positional_attn = mint.einsum("binh,tnh->bnit", q_head + v, r_head)
            # Shape batch_size x n_head x seq_len x context_len
    ...
    def relative_token_type_attention(self, token_type_mat, q_head, cls_mask=None):
        ...
        token_type_bias = mint.einsum("bind,snd->bnis", q_head + r_s_bias, self.seg_embed)
        # Shape batch_size x n_head x seq_len x context_len
        token_type_mat = token_type_mat[:, None].broadcast_to((batch_size, q_head.shape[2], seq_len, context_len))
        # Shapes batch_size x n_head x seq_len
        diff_token_type, same_token_type = mint.split(token_type_bias, 1, dim=-1)
        # Shape batch_size x n_head x seq_len x context_len
        token_type_attn = mint.where(
            token_type_mat,
            same_token_type.broadcast_to(token_type_mat.shape),
            diff_token_type.broadcast_to(token_type_mat.shape),
        )
    ...
    def construct(
        ...
        key: ms.Tensor,
        value: ms.Tensor,
        attention_inputs: Tuple[ms.Tensor],
        ...
        content_score = mint.einsum("bind,bjnd->bnij", q_head + r_w_bias, k_head)
        ...
        attn_prob = mint.softmax(attn_score, dim=-1, dtype=dtype)
        attn_prob = self.attention_dropout(attn_prob)

        # attention output, shape batch_size x seq_len x n_head x d_head
        attn_vec = mint.einsum("bnij,bjnd->bind", attn_prob, v_head)
...
class FunnelPositionwiseFFN(nn.Cell):
    def __init__(self, config: FunnelConfig) -> None:
        super().__init__()
        self.linear_1 = mint.nn.Linear(config.d_model, config.d_inner)
        self.activation_function = ACT2FN[config.hidden_act]
        self.activation_dropout = mint.nn.Dropout(config.activation_dropout)
        self.linear_2 = mint.nn.Linear(config.d_inner, config.d_model)
        self.dropout = mint.nn.Dropout(config.hidden_dropout)
        self.layer_norm = mint.nn.LayerNorm(config.d_model, config.layer_norm_eps)

    def construct(self, hidden: ms.Tensor) -> ms.Tensor:
...
def upsample(
    x: ms.Tensor, stride: int, target_len: int, separate_cls: bool = True, truncate_seq: bool = False
) -> ms.Tensor:
    """
    ...
    if separate_cls:
        cls = x[:, :1]
        x = x[:, 1:]
    output = mint.repeat_interleave(x, repeats=stride, dim=1)
    if separate_cls:
        if truncate_seq:
            output = mint.nn.functional.pad(output, (0, 0, 0, stride - 1, 0, 0))
        output = output[:, : target_len - 1]
        output = mint.cat([cls, output], dim=1)
    else:
...
class FunnelDecoder(nn.Cell):
    ...
    def construct(
        self,
        final_hidden: ms.Tensor,
        first_block_hidden: ms.Tensor,
        attention_mask: Optional[ms.Tensor] = None,
        token_type_ids: Optional[ms.Tensor] = None,
        output_attentions: bool = False,
        output_hidden_states: bool = False,
...
class FunnelDiscriminatorPredictions(nn.Cell):
    ...
    def __init__(self, config: FunnelConfig) -> None:
        ...
        self.dense_prediction = mint.nn.Linear(config.d_model, 1)

    def construct(self, discriminator_hidden_states: ms.Tensor) -> ms.Tensor:
...
class FunnelPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if classname.find("Linear") != -1:
            ...
            module.r_w_bias.data.uniform_(to=self.config.initializer_range)
            module.r_r_bias.data.uniform_(to=self.config.initializer_range)
            module.r_kernel.data.uniform_(to=self.config.initializer_range)
            module.r_s_bias.data.uniform_(to=self.config.initializer_range)
            module.seg_embed.data.uniform_(to=self.config.initializer_range)
        elif classname == "FunnelEmbeddings":
            std = 1.0 if self.config.initializer_std is None else self.config.initializer_std
            module.word_embeddings.weight.data.normal_(std=std)
...
class FunnelClassificationHead(nn.Cell):
    def __init__(self, config: FunnelConfig, n_labels: int) -> None:
        super().__init__()
        self.linear_hidden = mint.nn.Linear(config.d_model, config.d_model)
        self.dropout = mint.nn.Dropout(config.hidden_dropout)
        self.linear_out = mint.nn.Linear(config.d_model, n_labels)

    def construct(self, hidden: ms.Tensor) -> ms.Tensor:
        hidden = self.linear_hidden(hidden)
        hidden = mint.tanh(hidden)
...
class FunnelBaseModel(FunnelPreTrainedModel):
    ...
    def set_input_embeddings(self, new_embeddings: mint.nn.Embedding) -> None:
...
class FunnelForPreTraining(FunnelPreTrainedModel):
    ...
    def construct(
        ...
        >>> from mindone.transformers import FunnelForPreTraining
        >>> import mindspore as ms
...
class FunnelForMaskedLM(FunnelPreTrainedModel):
    ...
    def __init__(self, config: FunnelConfig) -> None:
        ...
        self.funnel = FunnelModel(config)
        self.lm_head = mint.nn.Linear(config.d_model, config.vocab_size)
    ...
    def set_output_embeddings(self, new_embeddings: mint.nn.Embedding) -> None:
