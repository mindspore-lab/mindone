...
class FalconH1RMSNormGated(mindspore.nn.Cell):
    ...
    def construct(self, hidden_states, gate=None):
        ...
        if not self.norm_before_gate and gate is not None:
            hidden_states = hidden_states * mindspore.mint.nn.functional.silu(gate.to(mindspore.float32))
...
def pad_tensor_by_size(input_tensor: mindspore.Tensor, pad_size: int):
    ...
    return mindspore.mint.nn.functional.pad(input_tensor, pad_shape, mode="constant", value=0)
...
def segment_sum(input_tensor):
    ...
    mask = mindspore.mint.tril(mindspore.mint.ones([chunk_size, chunk_size], dtype=mindspore.bool_), diagonal=-1)
    input_tensor = input_tensor.masked_fill(~mask, 0)
    # 3. compute actual cumsum
    tensor_segsum = mindspore.mint.cumsum(input_tensor, dim=-2)

    # 4. apply mask to keep only the lower triangular part of the cumulative sum result (incl diagonal this time)
    mask = mindspore.mint.tril(mindspore.mint.ones([chunk_size, chunk_size], dtype=mindspore.bool_), diagonal=0)
    tensor_segsum = tensor_segsum.masked_fill(~mask, dtype_to_min(input_tensor.dtype))
...
class FalconH1Mixer(mindspore.nn.Cell):
    ...
    def __init__(self, config: FalconH1Config, layer_idx: int):
        ...
        self.in_proj = mindspore.mint.nn.Linear(
            self.hidden_size,
            projection_size,
            bias=self.use_bias,
        )
        ...
        self.dt_bias = mindspore.Parameter(mindspore.mint.ones(self.num_heads))

        # S4D real initialization. These are not discretized!
        # The core is to load them, compute the discrete states, then write the updated state. Keeps the memory bounded
        A = mindspore.mint.arange(1, self.num_heads + 1)
    ...
    def _forward(
        ...
        if use_precomputed_states:
            ...
            hidden_states_B_C = mindspore.mint.sum(
                conv_states * self.conv1d.weight.squeeze(1).squeeze(1), dim=-1
            )
        ...
        hidden_states, B, C = mindspore.mint.split(
            hidden_states_B_C,
        ...
        A = -mindspore.mint.exp(self.A_log.float())                            # [num_heads]
        if use_precomputed_states:
            ...
            dt = mindspore.mint.nn.functional.softplus(dt + dt_bias.to(dt.dtype))
            dt = mindspore.mint.clamp(dt, self.time_step_limit[0], self.time_step_limit[1])
            A = A[..., None, None].broadcast_to([self.num_heads, self.head_dim, self.ssm_state_size]).to(
                dtype=mindspore.float32
            )
            # [bsz, num_heads, head_dim, state_size]
            dA = (mindspore.mint.exp(dt[..., None] * A))

            # Discretize B
            # [bsz, n_groups * state_size] -> [bsz, n_groups, 1, state_size] ->
            # -> [bsz, n_groups, group to head repetition factor, state_size] -> [bsz, num_heads, state_size]
            B = B.reshape(batch_size, self.n_groups, -1)[..., None, :]
            B = B.broadcast_to([batch_size, self.n_groups, self.num_heads // self.n_groups, B.shape[-1]])
            ...
            y = mindspore.mint.bmm(ssm_states_reshaped, C_reshaped)
            ...
            dt = mindspore.mint.nn.functional.softplus(dt + self.dt_bias)
            dt = mindspore.mint.clamp(dt, self.time_step_limit[0], self.time_step_limit[1])
            ...
            A = A.permute(0, 3, 1, 2)
            A_cumsum = mindspore.mint.cumsum(A, dim=-1)

            # 1. Compute the output for each intra-chunk (diagonal blocks)
            # This is the analog of a causal mask
            L = mindspore.mint.exp(segment_sum(A))
            ...
            decay_states = mindspore.mint.exp(A_cumsum[:, :, :, -1:] - A_cumsum)
            ...
            if use_precomputed_states:
                ...
                previous_states = mindspore.mint.zeros_like(states[:, :1])
            states = mindspore.mint.cat([previous_states, states], dim=1)
            decay_chunk = mindspore.mint.exp(segment_sum(mindspore.mint.nn.functional.pad(A_cumsum[:, :, :, -1], (1, 0))))
            ...
            state_decay_out = mindspore.mint.exp(A_cumsum)
        ...
        if self.mamba_rms_norm:
            ...
            scan_output = y * mindspore.mint.nn.functional.silu(gate)
...
class FalconH1DecoderLayer(mindspore.nn.Cell):
    def __init__(self, config: FalconH1Config, layer_idx: int):
        super().__init__()
...
def compute_mup_vector(config):
    ...
    mup_vector = mindspore.mint.ones([1, 1, vector_shape])
...
class FalconH1ForCausalLM(FalconH1PreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # Ensure we always use FalconHybridMambaAttentionDynamicCache
        if empty_past_kv:
            past_key_values = FalconHybridMambaAttentionDynamicCache(
                self.config,
                input_ids.shape[0],
        ...
        elif not isinstance(past_key_values, FalconHybridMambaAttentionDynamicCache):
            # Convert DynamicCache to FalconHybridMambaAttentionDynamicCache
            # This handles cases where a generic DynamicCache is passed
            batch_size = input_ids.shape[0]
            new_cache = FalconHybridMambaAttentionDynamicCache(
                self.config,
                batch_size,
                self.dtype,
            )
