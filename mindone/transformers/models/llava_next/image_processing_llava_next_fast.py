...
class LlavaNextImageProcessorFast(BaseImageProcessorFast):
    ...
    def _resize_for_patching(
        ...
        Returns:
            "ms.Tensor": The resized and padded image.
        """
    ...
    def _pad_for_patching(
        self, image: "ms.Tensor", target_resolution: tuple, input_data_format: ChannelDimension
    ) -> "ms.Tensor":
        """
        ...
        padded_image = mint.nn.functional.pad(image, padding=padding)
    ...
    def _get_image_patches(
        ...
        Returns:
            list["ms.Tensor"]: A list of NumPy arrays containing the processed image patches.
        """
        ...
        resized_original_image = mint.nn.functional.interpolate(image, size=size, interpolation=interpolation)
    ...
    def _pad_for_batching(
        ...
        Returns:
            list[`ms.Tensor`]: The padded images.
        """
        max_patch = max(len(x) for x in pixel_values)
        pixel_values = [
            mint.nn.functional.pad(image, pad=[0, 0, 0, 0, 0, 0, 0, max_patch - image.shape[0]])
        ...
        **kwargs,
        ...
        for image in images:
            ...
            processed_image_patches = (
                mint.stack(processed_image_patches, dim=0) if return_tensors else processed_image_patches
            )
