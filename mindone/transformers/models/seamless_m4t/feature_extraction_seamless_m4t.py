...
class SeamlessM4TFeatureExtractor(SequenceFeatureExtractor):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        Args:
            raw_speech (`np.ndarray`, `ms.Tensor`, `list[float]`, `list[np.ndarray]`, `list[ms.Tensor]`,
            `list[list[float]]`, `list[list[list[float]]]`):
                The sequence or batch of sequences to be padded. Each sequence can be a numpy array,
                a ms tensor, a list of float values, a list of numpy arrays, a list of ms tensors,
                a list of list of float values or a list of a list of list of float values.
                If `raw_speech` is a one-dimensional `np.ndarray`, `ms.Tensor` or a `list[float]`, `raw_speech` is
                considered a single-channel, single-sample sound. In all other cases, the first dimension of
                `raw_speech`, whether from an `np.ndarray`, a `ms.Tensor` or a `list[...]`,
        ...
        acceptable_types = (ms.Tensor, np.ndarray, tuple, list)
