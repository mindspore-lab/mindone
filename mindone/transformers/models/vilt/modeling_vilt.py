...
class ViltForImagesAndTextClassificationOutput(ModelOutput):
    ...
    Args:
        ...
        hidden_states (`List[tuple(ms.Tensor)]`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            List of tuples of `ms.Tensor` (one for each image-text pair, each tuple containing the output of
            the embeddings + one for the output of each layer) of shape `(batch_size, sequence_length, hidden_size)`.
            Hidden-states of the model at the output of each layer plus the initial embedding outputs.
        attentions (`List[tuple(ms.Tensor)]`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            List of tuples of `ms.Tensor` (one for each image-text pair, each tuple containing the attention
    ...
    hidden_states: Optional[List[Tuple[ms.Tensor]]] = None
    attentions: Optional[List[Tuple[ms.Tensor]]] = None


class ViltEmbeddings(nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        self.token_type_embeddings = mint.nn.Embedding(config.modality_type_vocab_size, config.hidden_size)
    ...
    def visual_embed(self, pixel_values, pixel_mask, max_image_length=200):
        ...
        x_mask = mint.nn.functional.interpolate(x_mask, size=(x.shape[2], x.shape[3])).long()
        ...
        patch_index = mint.stack(
            mint.meshgrid(mint.arange(x_mask.shape[-2]), mint.arange(x_mask.shape[-1]), indexing="ij"), dim=-1
        )
        ...
        for i, (v, nv, p) in enumerate(zip(valid_nums, non_valid_nums, pad_nums)):
            if p <= 0:
                valid_choice = mint.multinomial(mint.ones(v).float(), max_image_length)
                select.append(valid_row_idx[i][valid_choice])
            else:
                pad_choice = mint.multinomial(mint.ones(nv).float(), p, replacement=True)
                select.append(mint.cat([valid_row_idx[i], non_valid_row_idx[i][pad_choice]], dim=0))

        select = mint.cat(select, dim=0)
    ...
    def construct(
        ...
        if image_token_type_idx is None:
            image_token_type_idx = 1
        text_embeds = text_embeds + self.token_type_embeddings(mint.zeros_like(attention_mask, dtype=ms.int64))
        image_embeds = image_embeds + self.token_type_embeddings(
            mint.full_like(image_masks, image_token_type_idx, dtype=ms.int64)
        )
...
class ViltPatchEmbeddings(nn.Cell):
    """
...
class ViltSelfAttention(nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class ViltIntermediate(nn.Cell):
    def __init__(self, config: ViltConfig) -> None:
        super().__init__()
...
class ViltEncoder(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.CellList([ViltLayer(config) for _ in range(config.num_hidden_layers)])
...
VILT_START_DOCSTRING = r"""
    This model is a MindSpore `mindspore.nn.Cell <https://www.mindspore.cn/docs/zh-CN/master/api_python/nn/mindspore.nn.Cell.html>`_ subclass. Use
...
VILT_IMAGES_AND_TEXT_CLASSIFICATION_INPUTS_DOCSTRING = r"""
    Args:
        ...
        pixel_values (`ms.Tensor` of shape `(batch_size, num_images, num_channels, height, width)`):
            Pixel values. Pixel values can be obtained using [`AutoImageProcessor`]. See
            [`ViltImageProcessor.__call__`] for details.

        pixel_mask (`ms.Tensor` of shape `(batch_size, num_images, height, width)`, *optional*):
            Mask to avoid performing attention on padding pixel values. Mask values selected in `[0, 1]`:
        ...
        image_embeds (`ms.Tensor` of shape `(batch_size, num_images, num_patches, hidden_size)`, *optional*):
            Optionally, instead of passing `pixel_values`, you can choose to directly pass an embedded representation.
            This is useful if you want more control over how to convert `pixel_values` into patch embeddings.
...
class ViltModel(ViltPreTrainedModel):
    ...
    def construct(
    ...
    ) -> Union[BaseModelOutputWithPooling, Tuple[ms.Tensor]]:
        r"""
        Returns:
        ...
        >>> from mindone.transformers import ViltModel
        ...
        if attention_mask is None:
            attention_mask = mint.ones(((text_batch_size, seq_length)))
        ...
        if pixel_mask is None:
            pixel_mask = mint.ones((image_batch_size, self.config.image_size, self.config.image_size))
...
class ViltForMaskedLM(ViltPreTrainedModel):
    ...
    def construct(
        ...
        ```python
        >>> from transformers import ViltProcessor
        >>> from mindone.transformers import ViltForMaskedLM
        >>> import requests
        >>> from PIL import Image
        >>> import re
        >>> import mindspore as ms
...
class ViltPredictionHeadTransform(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.dense = mint.nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
...
class ViltForQuestionAnswering(ViltPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = nn.SequentialCell(
            mint.nn.Linear(config.hidden_size, config.hidden_size * 2),
            mint.nn.LayerNorm(config.hidden_size * 2),
            mint.nn.GELU(),
            mint.nn.Linear(config.hidden_size * 2, config.num_labels),
        )
    ...
    def construct(
        ...
        r"""
        labels (`ms.Tensor` of shape `(batch_size, num_labels)`, *optional*):
        ...
        ```python
        >>> from transformers import ViltProcessor
        >>> from mindone.transformers import ViltForQuestionAnswering
        >>> import requests
        >>> from PIL import Image
        >>> import mindspore as ms
        ...
        loss = None
        if labels is not None:
            loss = mint.nn.functional.binary_cross_entropy_with_logits(logits, labels) * labels.shape[1]
...
class ViltForImageAndTextRetrieval(ViltPreTrainedModel):
    def __init__(self, config):
        ...
        # Classifier head
        self.rank_output = mint.nn.Linear(config.hidden_size, 1)
    ...
    def construct(
        ...
        ```python
        >>> from transformers import ViltProcessor
        >>> from mindone.transformers import ViltForImageAndTextRetrieval
        >>> import requests
        >>> from PIL import Image
        >>> import mindspore as ms
...
class ViltForImagesAndTextClassification(ViltPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = nn.SequentialCell(
            mint.nn.Linear(config.hidden_size * num_images, config.hidden_size * num_images),
            mint.nn.LayerNorm(config.hidden_size * num_images),
            mint.nn.GELU(),
            mint.nn.Linear(config.hidden_size * num_images, config.num_labels),
        )
    ...
    def construct(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[ViltForImagesAndTextClassificationOutput, Tuple[ms.Tensor]]:
        r"""
        labels (`ms.Tensor` of shape `(batch_size,)`, *optional*):
        ...
        ```python
        >>> from transformers import ViltProcessor
        >>> from mindone.transformers import ViltForImagesAndTextClassification
        >>> import requests
        >>> from PIL import Image
        >>> import mindspore as ms
        ...
        pooled_output = mint.cat(pooler_outputs, dim=-1)
...
class ViltForTokenClassification(ViltPreTrainedModel):
    ...
    def construct(
        ...
        r"""
        labels (`ms.Tensor` of shape `(batch_size, text_sequence_length)`, *optional*):
