...
def _prepare_4d_attention_mask_inverted(mask: mindspore.Tensor, dtype: mindspore.dtype, tgt_len: Optional[int] = None):
    """
...
class LEDLearnedPositionalEmbedding(mindspore.mint.nn.Embedding):
    """
...
class LEDEncoderSelfAttention(mindspore.nn.Cell):
    def __init__(self, config, layer_id):
        ...
        self.query = mindspore.mint.nn.Linear(config.hidden_size, self.embed_dim)
        self.key = mindspore.mint.nn.Linear(config.hidden_size, self.embed_dim)
        self.value = mindspore.mint.nn.Linear(config.hidden_size, self.embed_dim)

        # separate projection layers for tokens with global attention
        self.query_global = mindspore.mint.nn.Linear(config.hidden_size, self.embed_dim)
        self.key_global = mindspore.mint.nn.Linear(config.hidden_size, self.embed_dim)
        self.value_global = mindspore.mint.nn.Linear(config.hidden_size, self.embed_dim)
    ...
    def construct(
        ...
        *attention_window* happens in [`LEDEncoderModel.forward`] to avoid redoing the padding on each layer.
        ...
        float_mask = remove_from_windowed_attention_mask.type_as(query_vectors).masked_fill(
            remove_from_windowed_attention_mask, _DTYPE_2_MIN[query_vectors.dtype]
        )
        ...
        attn_probs = mindspore.ops.masked_fill(attn_probs, is_index_masked[:, :, None, None], 0.0)
    ...
    def _pad_and_transpose_last_two_dims(hidden_states_padded, padding):
        ...
        hidden_states_padded = mindspore.mint.nn.functional.pad(
    ...
    def _pad_and_diagonalize(chunked_hidden_states):
        ...
        chunked_hidden_states = mindspore.mint.nn.functional.pad(
    ...
    def _chunk(hidden_states, window_overlap, onnx_export: bool = False):
        ...
        overlapping_chunks = mindspore.mint.empty(
            chunk_size,
        )
    ...
    def _sliding_chunks_matmul_attn_probs_value(
        ...
        padded_value = mindspore.mint.nn.functional.pad(value, (0, 0, window_overlap, window_overlap), value=-1)
    ...
    def _get_global_attn_indices(is_index_global_attn):
        ...
        # helper variable
        is_local_index_global_attn = mindspore.mint.arange(
            max_num_global_attn_indices,
    ...
    def _concat_with_global_key_attn_probs(
        ...
        ] = _DTYPE_2_MIN[attn_probs_from_global_key.dtype]
        attn_probs_from_global_key = attn_probs_from_global_key.transpose(1, 3)
    ...
    def _compute_global_attn_output_from_hidden(
        ...
        # compute attn scores
        global_attn_scores = mindspore.mint.bmm(global_query_vectors_only_global, global_key_vectors.transpose(1, 2))
        ...
        global_attn_scores[
            is_local_index_no_global_attn_nonzero[0], is_local_index_no_global_attn_nonzero[1], :, :
        ] = _DTYPE_2_MIN[global_attn_scores.dtype]
        global_attn_scores = global_attn_scores.transpose(1, 2)

        global_attn_scores = global_attn_scores.masked_fill(
            is_index_masked[:, None, None, :], _DTYPE_2_MIN[global_attn_scores.dtype]
        )

        global_attn_scores = global_attn_scores.view(batch_size * self.num_heads, max_num_global_attn_indices, seq_len)

        # compute global attn probs
        global_attn_probs_float = mindspore.mint.nn.functional.softmax(
        ...
        global_attn_probs = mindspore.mint.nn.functional.dropout(
            global_attn_probs_float.type_as(global_attn_scores), p=self.dropout, training=self.training
        )

        # global attn output
        global_attn_output = mindspore.mint.bmm(global_attn_probs, global_value_vectors)
...
class LEDEncoderAttention(mindspore.nn.Cell):
    def __init__(self, config, layer_id):
        super().__init__()
        self.longformer_self_attn = LEDEncoderSelfAttention(config, layer_id=layer_id)
        self.output = mindspore.mint.nn.Linear(config.d_model, config.d_model)
...
class LEDDecoderAttention(mindspore.nn.Cell):
...
class LEDEncoderLayer(mindspore.nn.Cell):
...
class LEDDecoderLayer(mindspore.nn.Cell):
...
class LEDClassificationHead(mindspore.nn.Cell):
    ...
    def __init__(
        ...
        super().__init__()
        self.dense = mindspore.mint.nn.Linear(input_dim, inner_dim)
        self.dropout = mindspore.mint.nn.Dropout(p=pooler_dropout)
        self.out_proj = mindspore.mint.nn.Linear(inner_dim, num_classes)
...
class LEDSeq2SeqModelOutput(ModelOutput):
    ...
    Args:
        ...
        decoder_hidden_states (`tuple(mindspore.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed
        ...
        encoder_attentions (`tuple(mindspore.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            self-attention heads.
        encoder_global_attentions (`tuple(mindspore.Tensor)`, *optional*, returned when `output_attentions=True` is\
...
class LEDSeq2SeqLMOutput(ModelOutput):
    ...
    Args:
        ...
        past_key_values (`List[mindspore.Tensor]`, *optional*, returned when `use_cache=True` is passed or when `config.use_cache=True`):
            ...
            Contains pre-computed hidden-states (key and values in the attention blocks) of the decoder that can be
            used (see `past_key_values` input) to speed up sequential decoding.
        decoder_hidden_states (`tuple(mindspore.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when \
            `config.output_hidden_states=True`):
            Tuple of `mindspore.Tensor` (one for the output of the embeddings + one for the output of each layer) of
            shape `(batch_size, sequence_length, hidden_size)`.

            Hidden-states of the decoder at the output of each layer plus the initial embedding outputs.
        decoder_attentions (`tuple(mindspore.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when \
        ...
        encoder_attentions (`tuple(mindspore.Tensor)`, *optional*, returned when `output_attentions=True` is passed or \
            ...
            Attentions weights of the encoder, after the attention softmax, used to compute the weighted average in the
            self-attention heads.
        encoder_global_attentions (`tuple(mindspore.Tensor)`, *optional*, returned when `output_attentions=True` is passed or\
              when `config.output_attentions=True`):
            Tuple of `mindspore.Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length, x)`,
            where `x` is the number of tokens with global attention mask.
...
class LEDSeq2SeqSequenceClassifierOutput(ModelOutput):
    ...
    Args:
        ...
        decoder_attentions (`tuple(mindspore.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when \
            ...
            Attentions weights of the decoder, after the attention softmax, used to compute the weighted average in the
            self-attention heads.
        cross_attentions (`tuple(mindspore.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when\
              `config.output_attentions=True`):
            Tuple of `mindspore.Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
            sequence_length)`.
        ...
        encoder_last_hidden_state (`mindspore.Tensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
            Sequence of hidden-states at the output of the last layer of the encoder of the model.
        encoder_hidden_states (`tuple(mindspore.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or \
            when `config.output_hidden_states=True`):
            Tuple of `mindspore.Tensor` (one for the output of the embeddings + one for the output of each layer) of
            shape `(batch_size, sequence_length, hidden_size)`.

            Hidden-states of the encoder at the output of each layer plus the initial embedding outputs.
        encoder_attentions (`tuple(mindspore.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when \
            `config.output_attentions=True`):
            Tuple of `mindspore.Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
            sequence_length)`.
            ...
            Attentions weights of the encoder, after the attention softmax, used to compute the weighted average in the
            self-attention heads.
        encoder_global_attentions (`tuple(mindspore.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when\
              `config.output_attentions=True`):
            Tuple of `mindspore.Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length, x)`,
            where `x` is the number of tokens with global attention mask.
...
class LEDSeq2SeqQuestionAnsweringModelOutput(ModelOutput):
    ...
    Args:
        ...
        decoder_hidden_states (`tuple(mindspore.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when\
            ...
            Hidden-states of the decoder at the output of each layer plus the initial embedding outputs.
        decoder_attentions (`tuple(mindspore.Tensor)`, *optional*, returned when `output_attentions=True` is passed or\
              when `config.output_attentions=True`):
            Tuple of `mindspore.Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
            sequence_length)`.
...
class LEDEncoder(LEDPreTrainedModel):
    ...
    def __init__(self, config: LEDConfig, embed_tokens: Optional[mindspore.mint.nn.Embedding] = None):
        ...
        if embed_tokens is not None:
            ...
            self.embed_tokens = mindspore.mint.nn.Embedding(config.vocab_size, embed_dim, self.padding_idx)

        self.embed_positions = LEDLearnedPositionalEmbedding(
            self.max_source_positions,
            embed_dim,
        )
        self.layers = mindspore.nn.CellList([LEDEncoderLayer(config, i) for i in range(config.encoder_layers)])
        self.layernorm_embedding = mindspore.mint.nn.LayerNorm(embed_dim)
    ...
    def _pad_to_window_size(
        ...
        if padding_len > 0:
            ...
            if inputs_embeds is not None:
                ...
                inputs_embeds = mindspore.mint.cat([inputs_embeds, inputs_embeds_padding], dim=-2)

            attention_mask = mindspore.mint.nn.functional.pad(
    ...
    def construct(
        ...
        if attention_mask is None:
            attention_mask = mindspore.mint.ones(inputs_embeds.shape[:-1], dtype=mindspore.int64)
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if output_attentions:
                # bzs x seq_len x num_attn_heads x (num_global_attn + attention_window_len + 1) => bzs x num_attn_heads x seq_len x
                # (num_global_attn + attention_window_len + 1)
...
class LEDDecoder(LEDPreTrainedModel):
    ...
    def __init__(self, config: LEDConfig, embed_tokens: Optional[mindspore.mint.nn.Embedding] = None):
        ...
        if embed_tokens is not None:
            ...
            self.embed_tokens = mindspore.mint.nn.Embedding(config.vocab_size, config.d_model, self.padding_idx)

        self.embed_positions = LEDLearnedPositionalEmbedding(
            self.max_target_positions,
            config.d_model,
        )
        self.layers = mindspore.nn.CellList([LEDDecoderLayer(config) for _ in range(config.decoder_layers)])
        self.layernorm_embedding = mindspore.mint.nn.LayerNorm(config.d_model)
...
class LEDModel(LEDPreTrainedModel):
    ...
    def __init__(self, config: LEDConfig):
        ...
        self.shared = mindspore.mint.nn.Embedding(vocab_size, config.d_model, padding_idx)
...
class LEDForConditionalGeneration(LEDPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: LEDConfig):
        ...
        self.register_buffer("final_logits_bias", mindspore.mint.zeros((1, self.led.shared.num_embeddings)))
        self.lm_head = mindspore.mint.nn.Linear(config.d_model, self.led.shared.num_embeddings, bias=False)
...
class LEDForSequenceClassification(LEDPreTrainedModel):
    ...
    def construct(
        ...
        **kwargs,
        ...
        if len(mindspore.mint.unique_consecutive(eos_mask.sum(1))) > 1:
