...
from transformers.models.aimv2.configuration_aimv2 import Aimv2Config, Aimv2TextConfig, Aimv2VisionConfig
...
class Aimv2Output(ModelOutput):
    ...
    logits_per_image: Optional[mindspore.Tensor] = None
    logits_per_text: Optional[mindspore.Tensor] = None
...
# @use_kernel_forward_from_hub("RMSNorm")  # TODO: unsupported
class Aimv2RMSNorm(mindspore.nn.Cell):
...
class Aimv2MLP(mindspore.nn.Cell):
...
class Aimv2VisionEmbeddings(mindspore.nn.Cell):
    def __init__(self, config: Aimv2VisionConfig):
        super().__init__()
        self.config = config
        self.patch_size = config.patch_size
        self.patch_embed = mindspore.mint.nn.Conv2d(
        ...
        if not self.config.is_native:
            self.position_embedding = mindspore.mint.nn.Embedding(num_patches, config.hidden_size)
        self.register_buffer("position_ids", mindspore.mint.arange(num_patches).expand((1, -1)), persistent=False)

    @staticmethod
    def build_2d_sincos_position_embedding(
        height, width, embed_dim=256, temperature=10000.0, dtype=mindspore.float32
    ) -> mindspore.Tensor:
        grid_w = mindspore.mint.arange(
            int(width),
            dtype=dtype,
        )
        grid_h = mindspore.mint.arange(
            int(height),
            dtype=dtype,
        )
        grid_h, grid_w = mindspore.mint.meshgrid(grid_w, grid_h, indexing="xy")
        ...
        return mindspore.mint.concat([out_h.sin(), out_h.cos(), out_w.sin(), out_w.cos()], dim=1)[None, :, :]
...
class Aimv2TextEmbeddings(mindspore.nn.Cell):
    def __init__(self, config: Aimv2TextConfig):
        super().__init__()
        embed_dim = config.hidden_size

        self.token_embedding = mindspore.mint.nn.Embedding(config.vocab_size, embed_dim)
        self.position_embedding = mindspore.mint.nn.Embedding(config.max_position_embeddings, embed_dim)
...
class Aimv2Attention(mindspore.nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.k_proj = mindspore.mint.nn.Linear(self.embed_dim, self.embed_dim, bias=config.qkv_bias)
        self.v_proj = mindspore.mint.nn.Linear(self.embed_dim, self.embed_dim, bias=config.qkv_bias)
        self.q_proj = mindspore.mint.nn.Linear(self.embed_dim, self.embed_dim, bias=config.qkv_bias)
        self.out_proj = mindspore.mint.nn.Linear(self.embed_dim, self.embed_dim, bias=config.qkv_bias)
...
class Aimv2Encoder(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: Aimv2Config):
        super().__init__()
        self.config = config
        self.layers = mindspore.nn.CellList([Aimv2EncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class Aimv2AttentionPoolingHead(mindspore.nn.Cell):
    def __init__(self, config: Aimv2VisionConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.num_heads = config.num_attention_heads

        self.k_proj = mindspore.mint.nn.Linear(self.hidden_size, self.hidden_size, bias=config.qkv_bias)
        self.v_proj = mindspore.mint.nn.Linear(self.hidden_size, self.hidden_size, bias=config.qkv_bias)

        self.cls_token = mindspore.Parameter(mindspore.mint.zeros((1, 1, self.hidden_size)))
        self.output_proj = mindspore.mint.nn.Linear(self.hidden_size, self.hidden_size, bias=True)
...
class Aimv2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if hasattr(module, "logit_scale"):
            if isinstance(module.logit_scale, mindspore.Parameter):
...
def _get_vector_norm(tensor: mindspore.Tensor) -> mindspore.Tensor:
    """
    This method is equivalent to tensor.norm(p=2, dim=-1, keepdim=True) and used to make
    model `executorch` exportable. See issue https://github.com/pytorch/executorch/issues/3566
    """
    square_tensor = mindspore.mint.pow(tensor, 2)
    sum_tensor = mindspore.mint.sum(square_tensor, dim=-1, keepdim=True)
    normed_tensor = mindspore.mint.pow(sum_tensor, 0.5)
...
class Aimv2Model(Aimv2PreTrainedModel):
    ...
    def __init__(self, config: Aimv2Config):
        ...
        self.visual_projection = mindspore.mint.nn.Linear(self.vision_embed_dim, self.projection_dim, bias=False)
        self.text_projection = mindspore.mint.nn.Linear(self.text_embed_dim, self.projection_dim, bias=False)
