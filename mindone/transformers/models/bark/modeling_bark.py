...
class BarkSelfAttention(nn.Cell):
    ...
    def __init__(self, config, is_causal=False):
        ...
        self.resid_dropout = mint.nn.Dropout(config.dropout)
        ...
        self.att_proj = mint.nn.Linear(config.hidden_size, 3 * config.hidden_size, bias=config.bias)
        # output projection
        self.out_proj = mint.nn.Linear(config.hidden_size, config.hidden_size, bias=config.bias)

        self.is_causal = is_causal
        if is_causal:
            block_size = config.block_size
            bias = mint.tril(mint.ones((block_size, block_size), dtype=ms.bool_)).view(1, 1, block_size, block_size)
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        if self.is_causal:
            ...
            attn_weights = attn_weights.masked_fill(
                ...
                dtype_to_min(attn_weights.dtype),
            )
...
class BarkSelfFlashAttention2(BarkSelfAttention):
    ...
    def construct(
        ...
        if past_key_values is not None:
            ...
            key = mint.cat((past_key, key), dim=1)
            value = mint.cat((past_value, value), dim=1)
...
class BarkLayerNorm(nn.Cell):
    ...
    def __init__(self, hidden_size, bias=True):
        ...
        self.bias = Parameter(mint.zeros(hidden_size)) if bias else None
...
class BarkMLP(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.in_proj = mint.nn.Linear(config.hidden_size, 4 * config.hidden_size, bias=config.bias)
        self.out_proj = mint.nn.Linear(4 * config.hidden_size, config.hidden_size, bias=config.bias)
...
class BarkBlock(nn.Cell):
    def __init__(self, config, is_causal=False):
        ...
        if is_causal:
            ...
            self.layernorm_1 = mint.nn.LayerNorm(config.hidden_size)
            self.layernorm_2 = mint.nn.LayerNorm(config.hidden_size)
...
BARK_FINE_INPUTS_DOCSTRING = r"""
    Args:
        ...
        input_ids (`ms.Tensor` of shape `(batch_size, sequence_length, number_of_codebooks)`):
        ...
        labels (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*): NOT IMPLEMENTED YET.
        input_embeds (`ms.Tensor` of shape `(batch_size, input_sequence_length, hidden_size)`, *optional*):
...
BARK_CAUSAL_MODEL_INPUTS_DOCSTRING = r"""
    Args:
        ...
        past_key_values (`tuple(tuple(ms.Tensor))`, *optional*, returned when `use_cache` is passed or when `config.use_cache=True`):
...
class BarkCausalModel(BarkPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.input_embeds_layer = mint.nn.Embedding(config.input_vocab_size, config.hidden_size)
        self.position_embeds_layer = mint.nn.Embedding(config.block_size, config.hidden_size)

        self.drop = mint.nn.Dropout(config.dropout)
        ...
        self.lm_head = mint.nn.Linear(config.hidden_size, config.output_vocab_size, bias=False)
    ...
    def construct(
        ...
        input_embeds: Optional[ms.Tensor] = None,
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[ms.Tensor], CausalLMOutputWithPast]:
...
class BarkSemanticModel(BarkCausalModel):
    ...
    def generate(
        ...
        history_prompt: Optional[Dict[str, ms.Tensor]] = None,
        attention_mask: Optional[ms.Tensor] = None,
        **kwargs,
    ) -> ms.Tensor:
        """
        ...
        Args:
            input_ids (`Optional[ms.Tensor]` of shape (batch_size, seq_len), *optional*):
            ...
            history_prompt (`Optional[Dict[str,ms.Tensor]]`, *optional*):
                Optional `Bark` speaker prompt.
            attention_mask (`Optional[ms.Tensor]`, *optional*):
                Mask to avoid performing attention on padding token indices. Mask values selected in `[0, 1]`:
                ...
                [What are attention masks?](../glossary#attention-mask)
        Returns:
            ms.Tensor: Output semantic tokens.
        """
        ...
        if history_prompt is not None:
            ...
            semantic_history = mint.nn.functional.pad(
                semantic_history,
        ...
        semantic_output = super().generate(
            mint.ones((batch_size, max_input_semantic_length + 1), dtype=ms.int32),
            input_embeds=input_embeds,
...
class BarkCoarseModel(BarkCausalModel):
    ...
    def preprocess_histories(
        ...
        Args:
            ...
            history_prompt (`Optional[Dict[str,ms.Tensor]]`):
                Optional `Bark` speaker prompt.
        Returns: Returns:
            `tuple(ms.Tensor)`:
            - **x_semantic_history** (`ms.Tensor` -- Processed semantic speaker prompt.
            - **x_coarse_history** (`ms.Tensor`) -- Processed coarse speaker prompt.
        """
        if history_prompt is not None:
            x_semantic_history = mint.repeat_interleave(history_prompt["semantic_prompt"][None], batch_size, dim=0)
            ...
            x_coarse_history = mint.transpose(x_coarse_history, 0, 1).reshape(-1)

            x_coarse_history = x_coarse_history + semantic_generation_config.semantic_vocab_size

            x_coarse_history = mint.repeat_interleave(x_coarse_history[None], batch_size, dim=0)
            ...
            # shape: (batch_size, 0)
            x_semantic_history = ms.Tensor([[]] * batch_size, dtype=ms.int32)
            x_coarse_history = ms.Tensor([[]] * batch_size, dtype=ms.int32)

        return x_semantic_history, x_coarse_history

    def generate(
        self,
        semantic_output: ms.Tensor,
        semantic_generation_config: BarkSemanticGenerationConfig = None,
        ...
        **kwargs,
        ...
        Args:
            semantic_output (`ms.Tensor` of shape (batch_size, seq_len), *optional*):
        ...
        Returns:
            By default:
                ms.Tensor: Output coarse acoustics tokens.
            If `return_output_lengths=True`:
                `Tuple(ms.Tensor, ms.Tensor): The output coarse acoustics tokens, and the length of each sample
                of the batch.
        """
        ...
        output_lengths = mint.floor(
            output_lengths * semantic_to_coarse_ratio / coarse_generation_config.n_coarse_codebooks
        )
        output_lengths = mint.round(output_lengths * coarse_generation_config.n_coarse_codebooks).int()

        max_generated_len = mint.max(output_lengths).item()
        ...
        semantic_output = mint.hstack([x_semantic_history, semantic_output])
        ...
        for _ in range(n_window_steps):
            ...
            x_coarse = mint.hstack([x_coarse, output_coarse[:, input_coarse_len:]])
...
class BarkFineModel(BarkPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.input_embeds_layers = nn.CellList(
            [mint.nn.Embedding(config.input_vocab_size, config.hidden_size) for _ in range(config.n_codes_total)]
        )
        self.position_embeds_layer = mint.nn.Embedding(config.block_size, config.hidden_size)
        ...
        self.layernorm_final = mint.nn.LayerNorm(config.hidden_size)

        self.lm_heads = nn.CellList(
            [
                mint.nn.Linear(config.hidden_size, config.output_vocab_size, bias=False)
    ...
    def generate(
        ...
        **kwargs,
        ...
        Args:
            coarse_output (`ms.Tensor` of shape (batch_size, seq_len)):
            ...
            history_prompt (`Optional[Dict[str,ms.Tensor]]`, *optional*):
                Optional `Bark` speaker prompt.
        Returns:
            ms.Tensor: Output fine acoustics tokens.
        """
        if semantic_generation_config is None:
            raise ValueError("`semantic_generation_config` has to be provided")
        ...
        coarse_output = mint.remainder(coarse_output - semantic_generation_config.semantic_vocab_size, codebook_size)
        batch_size = coarse_output.shape[0]

        if history_prompt is not None:
            x_fine_history = mint.repeat_interleave(history_prompt["fine_prompt"].T[None], batch_size, dim=0)
        ...
        for n_outer in range(n_loops):
            ...
            for n_inner in range(n_coarse, fine_generation_config.n_fine_codebooks):
                ...
                if temperature is None or temperature == 1.0:
                    ...
                    codebook_preds = mint.multinomial(probs, num_samples=1).view(batch_size, -1)
                codebook_preds = codebook_preds.to(ms.int32)
...
class BarkModel(BarkPreTrainedModel):
    ...
    def enable_cpu_offload(self, gpu_id: Optional[int] = 0):
        ...
        """
        raise NotImplementedError("`enable_model_cpu_offload` requires `accelerate`.")
    ...
    def generate(
        ...
        **kwargs,
        ...
        Returns:
            By default:
                - **audio_waveform** (`ms.Tensor` of shape (batch_size, seq_len)): Generated audio waveform.
            When `return_output_lengths=True`:
                Returns a tuple made of:
                - **audio_waveform** (`ms.Tensor` of shape (batch_size, seq_len)): Generated audio waveform.
                - **output_lengths** (`ms.Tensor` of shape (batch_size)): The length of each waveform in the batch
        Example:
