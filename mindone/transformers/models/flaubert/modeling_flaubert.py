            ...
            if self.config is not None and self.config.init_std is not None:
                ...
                if module.bias is not None:
                    module.bias.set_data(
                        initializer(Constant(0), shape=module.bias.shape, dtype=module.bias.dtype)
                    )
...
class FlaubertForQuestionAnsweringOutput(ModelOutput):
    ...
    Args:
        ...
        start_top_index (`torch.LongTensor` of shape `(batch_size, config.start_n_top)`, *optional*, returned if `start_positions` or `end_positions`
            is not provided): Indices for the top config.start_n_top start token possibilities (beam-search).
        end_top_log_probs (`torch.FloatTensor` of shape `(batch_size, config.start_n_top * config.end_n_top)`, *optional*, returned if `start_positions`
            ...
            (beam-search).
        end_top_index (`torch.LongTensor` of shape `(batch_size, config.start_n_top * config.end_n_top)`, *optional*, returned if `start_positions`
