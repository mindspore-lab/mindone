...
import numpy as np
from transformers.models.flaubert.configuration_flaubert import FlaubertConfig
...
class MultiHeadAttention(mindspore.nn.Cell):
    ...
    def __init__(self, n_heads, dim, config):
        ...
        self.q_lin = mindspore.mint.nn.Linear(dim, dim)
        self.k_lin = mindspore.mint.nn.Linear(dim, dim)
        self.v_lin = mindspore.mint.nn.Linear(dim, dim)
        self.out_lin = mindspore.mint.nn.Linear(dim, dim)
    ...
    def construct(self, input, mask, kv=None, cache=None, head_mask=None, output_attentions=False):
        ...
        if cache is not None:
            if self.layer_id in cache:
                if kv is None:
                    ...
                    k = mindspore.mint.cat([k_, k], dim=2)  # (bs, n_heads, klen, dim_per_head)
                    v = mindspore.mint.cat([v_, v], dim=2)  # (bs, n_heads, klen, dim_per_head)
                else:
                    k, v = cache[self.layer_id]
            cache[self.layer_id] = (k, v)

        q = q / math.sqrt(dim_per_head)  # (bs, n_heads, qlen, dim_per_head)
        scores = mindspore.mint.matmul(q, k.transpose(2, 3))  # (bs, n_heads, qlen, klen)
        mask = (mask == 0).view(mask_reshape).expand_as(scores)  # (bs, n_heads, qlen, klen)
        scores.masked_fill_(mask, dtype_to_min(scores.dtype))  # (bs, n_heads, qlen, klen)

        weights = mindspore.mint.nn.functional.softmax(scores.float(), dim=-1).type_as(
            scores
        )  # (bs, n_heads, qlen, klen)
        weights = mindspore.mint.nn.functional.dropout(
        ...
        context = mindspore.mint.matmul(weights, v)  # (bs, n_heads, qlen, dim_per_head)
...
class TransformerFFN(mindspore.nn.Cell):
    def __init__(self, in_dim, dim_hidden, out_dim, config):
        super().__init__()
        self.dropout = config.dropout
        self.lin1 = mindspore.mint.nn.Linear(in_dim, dim_hidden)
        self.lin2 = mindspore.mint.nn.Linear(dim_hidden, out_dim)
        self.act = get_activation("gelu") if config.gelu_activation else mindspore.mint.nn.functional.relu
    ...
    def ff_chunk(self, input):
        ...
        x = self.act(x)
        x = self.lin2(x)
        x = mindspore.mint.nn.functional.dropout(x, p=self.dropout, training=self.training)
        return x
...
FLAUBERT_INPUTS_DOCSTRING = r"""
    Args:
        ...
        token_type_ids (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
        ...
        cache (`Dict[str, ms.Tensor]`, *optional*):
            Dictionary strings to `ms.Tensor` that contains precomputed hidden-states (key and values in the
...
class FlaubertPredLayer(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        if config.asm is False:
            self.proj = mindspore.mint.nn.Linear(dim, config.n_words, bias=True)
        else:
    ...
    def construct(self, x, y=None):
        ...
        if self.asm is False:
            ...
            if y is not None:
                loss = mindspore.mint.nn.functional.cross_entropy(
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, mindspore.mint.nn.Embedding):
            if self.config is not None and self.config.embed_init_std is not None:
                module.weight.data.normal_(mean=0, std=self.config.embed_init_std)
            if module.padding_idx is not None:
                module.weight.data[module.padding_idx].zero_()
        if isinstance(module, mindspore.mint.nn.Linear):
            if self.config is not None and self.config.init_std is not None:
                module.weight.data.normal_(mean=0, std=self.config.init_std)
                if module.bias is not None:
                    module.bias.set_data(initializer(Constant(0), shape=module.bias.shape, dtype=module.bias.dtype))
        if isinstance(module, mindspore.mint.nn.LayerNorm):
...
class FlaubertModel(FlaubertPreTrainedModel):
    def __init__(self, config):  # , dico, is_encoder, with_output):
        ...
        # embeddings
        self.position_embeddings = mindspore.mint.nn.Embedding(config.max_position_embeddings, self.dim)
        if config.n_langs > 1 and config.use_lang_emb:
            self.lang_embeddings = mindspore.mint.nn.Embedding(self.n_langs, self.dim)
        self.embeddings = mindspore.mint.nn.Embedding(self.n_words, self.dim, padding_idx=self.pad_index)
        self.layer_norm_emb = mindspore.mint.nn.LayerNorm(self.dim, eps=config.layer_norm_eps)

        # transformer layers
        self.attentions = mindspore.nn.CellList()
        self.layer_norm1 = mindspore.nn.CellList()
        self.ffns = mindspore.nn.CellList()
        self.layer_norm2 = mindspore.nn.CellList()
        # if self.is_decoder:
        ...
        for _ in range(self.n_layers):
            ...
            self.layer_norm1.append(mindspore.mint.nn.LayerNorm(self.dim, eps=config.layer_norm_eps))
            # if self.is_decoder:
            #     self.layer_norm15.append(nn.LayerNorm(self.dim, eps=config.layer_norm_eps))
            #     self.encoder_attn.append(MultiHeadAttention(self.n_heads, self.dim, dropout=self.attention_dropout))
            self.ffns.append(TransformerFFN(self.dim, self.hidden_dim, self.dim, config=config))
            self.layer_norm2.append(mindspore.mint.nn.LayerNorm(self.dim, eps=config.layer_norm_eps))
    ...
    def construct(
        ...
        #     src_mask = mint.arange(src_len.max(), dtype=ms.long, device=lengths.device) < src_len[:, None]
        ...
        tensor = mindspore.mint.nn.functional.dropout(tensor, p=self.dropout, training=self.training)
        ...
        for i in range(self.n_layers):
            ...
            if not self.pre_norm:
                ...
                attn = mindspore.mint.nn.functional.dropout(attn, p=self.dropout, training=self.training)
                tensor = tensor + attn
...
class FlaubertWithLMHeadModel(FlaubertPreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(self, input_ids, **kwargs):
        ...
        input_ids = mindspore.mint.cat([input_ids, mask_token], dim=1)
        if lang_id is not None:
            langs = mindspore.mint.full_like(input_ids, lang_id)
        else:
            langs = None
...
class FlaubertForTokenClassification(FlaubertPreTrainedModel):
    def __init__(self, config):
        ...
        self.transformer = FlaubertModel(config)
        self.dropout = mindspore.mint.nn.Dropout(config.dropout)
...
class FlaubertForQuestionAnsweringOutput(ModelOutput):
    ...
    Args:
        ...
        start_top_index (`ms.Tensor` of shape `(batch_size, config.start_n_top)`, *optional*, returned if `start_positions` or `end_positions`
            is not provided): Indices for the top config.start_n_top start token possibilities (beam-search).
        end_top_log_probs (`ms.Tensor` of shape `(batch_size, config.start_n_top * config.end_n_top)`, *optional*, returned if `start_positions`
            or `end_positions` is not provided): Log probabilities for the top `config.start_n_top * config.end_n_top` end token possibilities
            (beam-search).
        end_top_index (`ms.Tensor` of shape `(batch_size, config.start_n_top * config.end_n_top)`, *optional*, returned if `start_positions`
            or `end_positions` is not provided): Indices for the top `config.start_n_top * config.end_n_top` end token possibilities (beam-search).
...
class FlaubertForMultipleChoice(FlaubertPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.transformer = FlaubertModel(config)
        self.sequence_summary = SequenceSummary(config)
        self.logits_proj = mindspore.mint.nn.Linear(config.num_labels, 1)
