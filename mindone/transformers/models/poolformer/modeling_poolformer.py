...
from transformers.models.poolformer.configuration_poolformer import PoolFormerConfig
...
class PoolFormerEmbeddings(nn.Cell):
    ...
    def __init__(self, hidden_size, num_channels, patch_size, stride, padding, norm_layer=None):
        ...
        self.projection = mint.nn.Conv2d(
            num_channels, hidden_size, kernel_size=patch_size, stride=stride, padding=padding
        )
        self.norm = norm_layer(hidden_size) if norm_layer else mint.nn.Identity()
...
class PoolFormerGroupNorm(mint.nn.GroupNorm):
    """
...
class PoolFormerOutput(nn.Cell):
    def __init__(self, config, dropout_prob, hidden_size, intermediate_size):
        super().__init__()
        self.conv1 = mint.nn.Conv2d(hidden_size, intermediate_size, 1)
        self.conv2 = mint.nn.Conv2d(intermediate_size, hidden_size, 1)
...
class PoolFormerLayer(nn.Cell):
    ...
    def __init__(self, config, num_channels, pool_size, hidden_size, intermediate_size, drop_path):
        ...
        self.drop_path = PoolFormerDropPath(drop_path) if drop_path > 0.0 else mint.nn.Identity()
        self.use_layer_scale = config.use_layer_scale
        if config.use_layer_scale:
            self.layer_scale_1 = ms.Parameter(
                config.layer_scale_init_value * mint.ones((num_channels)), requires_grad=True, name="layer_scale_1"
            )
            self.layer_scale_2 = ms.Parameter(
                config.layer_scale_init_value * mint.ones((num_channels)), requires_grad=True, name="layer_scale_2"
            )
