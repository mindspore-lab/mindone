...
from transformers.models.focalnet.configuration_focalnet import FocalNetConfig
...
class FocalNetEmbeddings(nn.Cell):
    ...
    def __init__(self, config, use_mask_token=False):
        ...
        self.mask_token = ms.Parameter(
            mint.zeros((1, 1, config.embed_dim)), name="mask_token"
        ) if use_mask_token else None

        self.norm = mint.nn.LayerNorm(config.embed_dim, eps=config.layer_norm_eps)
    ...
    def construct(
        ...
        if bool_masked_pos is not None:
            mask_tokens = self.mask_token.broacast_to((batch_size, seq_len, -1))
...
class FocalNetPatchEmbeddings(nn.Cell):
    def __init__(
        ...
        if add_norm:
            self.norm = mint.nn.LayerNorm(embed_dim, eps=config.layer_norm_eps)
        else:
            self.norm = None
...
class FocalNetModulation(nn.Cell):
    def __init__(self, config, index, dim, focal_factor=2, bias=True, projection_dropout=0.0):
        ...
        self.projection_in = mint.nn.Linear(dim, 2 * dim + (self.focal_level + 1), bias=bias)
        self.projection_context = mint.nn.Conv2d(dim, dim, kernel_size=1, stride=1, bias=bias)

        self.activation = mint.nn.GELU()
        self.projection_out = mint.nn.Linear(dim, dim)
        self.projection_dropout = mint.nn.Dropout(projection_dropout)
        self.focal_layers = []
        ...
        self.focal_layers = nn.CellList(self.focal_layers)
        if self.use_post_layernorm_in_modulation:
            self.layernorm = mint.nn.LayerNorm(dim, eps=config.layer_norm_eps)
    ...
    def construct(self, hidden_state):
        ...
        q, ctx, gates = mint.split(x, (num_channels, num_channels, self.focal_level + 1), 1)
...
class FocalNetMlp(nn.Cell):
    def __init__(self, config, in_features, hidden_features=None, out_features=None, drop=0.0):
        ...
        self.drop = mint.nn.Dropout(drop)
...
class FocalNetLayer(nn.Cell):
    ...
    def __init__(self, config, index, dim, input_resolution, drop_path=0.0):
        ...
        self.norm1 = mint.nn.LayerNorm(dim, eps=config.layer_norm_eps)
        ...
        self.drop_path = FocalNetDropPath(drop_path) if drop_path > 0.0 else mint.nn.Identity()
        ...
        if config.use_layerscale:
            self.gamma_1 = ms.Parameter(config.layerscale_value * mint.ones((dim)), requires_grad=True, name="gamma_1")
            self.gamma_2 = ms.Parameter(config.layerscale_value * mint.ones((dim)), requires_grad=True, name="gamma_2")
...
class FocalNetForMaskedImageModeling(FocalNetPreTrainedModel):
    ...
    def construct(
        ...
        Examples:
        ```python
        >>> from transformers import AutoImageProcessor, FocalNetConfig
        >>> from mindone.transformers import FocalNetForMaskedImageModeling
        >>> import mindspore as ms
...
class FocalNetForImageClassification(FocalNetPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # Classifier head
        self.classifier = (
            mint.nn.Linear(self.focalnet.num_features, config.num_labels) if config.num_labels > 0 else mint.nn.Identity()
        )
...
class FocalNetBackbone(FocalNetPreTrainedModel, BackboneMixin):
    ...
    def construct(
        ...
        >>> processor = AutoImageProcessor.from_pretrained("microsoft/focalnet-tiny-lrf", revision="refs/pr/2")
        >>> model = AutoBackbone.from_pretrained("microsoft/focalnet-tiny-lrf", revision="refs/pr/2")
