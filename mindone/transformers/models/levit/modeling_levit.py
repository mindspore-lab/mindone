...
class LevitForImageClassificationWithTeacherOutput(ModelOutput):
    ...
    Args:
        ...
        cls_logits (`ms.tensor` of shape `(batch_size, config.num_labels)`):
            Prediction scores of the classification head (i.e. the linear layer on top of the final hidden state of the
            class token).
        distillation_logits (`ms.tensor` of shape `(batch_size, config.num_labels)`):
...
class LevitConvEmbeddings(nn.Cell):
    """
...
class LevitPatchEmbeddings(nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        self.activation_layer_1 = mint.nn.Hardswish()
        ...
        self.activation_layer_2 = mint.nn.Hardswish()
        ...
        self.activation_layer_3 = mint.nn.Hardswish()
    ...
    def construct(self, pixel_values):
        ...
        return mint.transpose(embeddings.flatten(2), 1, 2)


class MLPLayerWithBN(nn.Cell):
    def __init__(self, input_dim, output_dim, bn_weight_init=1):
        super().__init__()
        self.linear = mint.nn.Linear(in_features=input_dim, out_features=output_dim, bias=False)
        self.batch_norm = mint.nn.BatchNorm1d(output_dim)

    def construct(self, hidden_state):
        hidden_state = self.linear(hidden_state)
        hidden_state = self.batch_norm(hidden_state.flatten(0, 1)).reshape(hidden_state.shape)
        return hidden_state


class LevitSubsample(nn.Cell):
...
class LevitAttention(nn.Cell):
    def __init__(self, hidden_sizes, key_dim, num_attention_heads, attention_ratio, resolution):
        ...
        self.activation = mint.nn.Hardswish()
        ...
        self.attention_biases = ms.Parameter(mint.zeros((num_attention_heads, len(attention_offsets))))
        self.attention_bias_idxs = ms.tensor(indices).view(len_points, len_points)
...
class LevitAttentionSubsample(nn.Cell):
    def __init__(
        self,
        input_dim,
        output_dim,
        ...
        self.attention_biases = ms.Parameter(mint.zeros((num_attention_heads, len(attention_offsets))))
        self.attention_bias_idxs = ms.tensor(indices).view(len_points_, len_points)
...
class LevitResidualLayer(nn.Cell):
    """
...
class LevitEncoder(nn.Cell):
    ...
    def construct(self, hidden_state, output_hidden_states=False, return_dict=True):
...
class LevitClassificationLayer(nn.Cell):
    """
    ...
    def __init__(self, input_dim, output_dim):
        super().__init__()
        self.batch_norm = mint.nn.BatchNorm1d(input_dim)
        self.linear = mint.nn.Linear(input_dim, output_dim)
...
class LevitPreTrainedModel(MSPreTrainedModel):
    """
...
class LevitModel(LevitPreTrainedModel):
    ...
    def construct(
        ...
        pooled_output = mint.mean(last_hidden_state, dim=1)
