...
class Ovis2VisionEmbeddings(nn.Cell):
...
class Ovis2VisionAttention(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.k_proj = mint.nn.Linear(self.embed_dim, self.embed_dim, bias=config.qkv_bias)
        self.v_proj = mint.nn.Linear(self.embed_dim, self.embed_dim, bias=config.qkv_bias)
        self.q_proj = mint.nn.Linear(self.embed_dim, self.embed_dim, bias=config.qkv_bias)
        self.out_proj = mint.nn.Linear(self.embed_dim, self.embed_dim, bias=config.qkv_bias)
...
class Ovis2VisionEncoder(nn.Cell):
    """
    ...
    def __init__(self, config: Ovis2VisionConfig):
        super().__init__()
        self.config = config
        self.layers = nn.CellList([Ovis2VisionEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class Ovis2VisionTransformer(nn.Cell):
    def __init__(self, config: Ovis2VisionConfig):
        super().__init__()
        self.config = config
...
class Ovis2VisualEmbeddingTable(mint.nn.Embedding):
    def construct(self, visual_tokens: Tensor) -> Tensor:
        if visual_tokens.dtype in [mindspore.int8, mindspore.int16, mindspore.int32, mindspore.int64, mindspore.long]:
            return super().construct(visual_tokens)
        return mint.matmul(visual_tokens, self.weight)
...
def hard_softmax(logits: Tensor, dim: int):
...
class Ovis2VisionModel(Ovis2PreTrainedModel):
    ...
    def __init__(self, config: Ovis2VisionConfig):
        ...
        self.head_linear = mint.nn.Linear(
            config.hidden_size * config.hidden_stride * config.hidden_stride,
            self.vocab_size - self.num_visual_indicator_tokens,
            bias=False,
        )
        self.head_norm = mint.nn.LayerNorm(self.vocab_size - self.num_visual_indicator_tokens)

    def construct(self, pixel_values: Tensor, **kwargs) -> tuple[Tensor, Tensor]:
        ...
        if self.config.hidden_stride > 1:
            ...
            last_hidden_state = mint.nn.functional.pad(
                last_hidden_state, (0, 0, 0, pad_size, 0, pad_size), "constant", 0
            )
            sqrt_l += pad_size
        ...
        if self.config.tokenize_function == "gumbel_argmax":
            prob_token = ops.gumbel_softmax(logits, dim=-1, hard=True)
        elif self.config.tokenize_function == "st_argmax":
            prob_token = hard_softmax(logits, dim=-1)
        elif self.config.tokenize_function == "softmax":
            prob_token = mint.nn.functional.softmax(logits, dim=-1)
            ...
            for i, visual_indicator_id in enumerate(self.visual_indicator_token_ids):
                if input_ids is None:
                    mask = inputs_embeds == self.get_input_embeddings()(
                        Tensor(visual_indicator_id, dtype=mindspore.long)
                    )
                    mask = mask.all(-1)
                else:
                    mask = input_ids == visual_indicator_id

                if mask.any():
                    inputs_embeds[mask] = (
                        visual_indicator_features[i].expand_as(inputs_embeds[mask]).to(inputs_embeds.dtype)
                    )
...
class Ovis2ForConditionalGeneration(Ovis2PreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs,
        ...
        >>> from mindone.transformers import Ovis2ForConditionalGeneration
