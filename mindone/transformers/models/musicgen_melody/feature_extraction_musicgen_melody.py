...
class MusicgenMelodyFeatureExtractor(SequenceFeatureExtractor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.spectrogram = mindspore.dataset.audio.Spectrogram(
    ...
    def _ms_extract_fbank_features(self, waveform: mindspore.Tensor) -> mindspore.Tensor:
        """
        Compute the chroma spectrogram of the provided audio using the minddata spectrogram implementation and the librosa chroma features.
        """
        ...
        if wav_length < self.n_fft:
            ...
            waveform = mindspore.mint.nn.functional.pad(waveform, (pad // 2, pad // 2 + rest), "constant", 0)

        # squeeze alongside channel dimension
        spec = mindspore.tensor(self.spectrogram(waveform.numpy()).squeeze(1))

        # sum along the frequency dimension
        raw_chroma = mindspore.mint.einsum("cf, ...ft->...ct", self.chroma_filters, spec)

        # normalise with max value
        norm_chroma = mindspore.mint.nn.functional.normalize(raw_chroma, p=float("inf"), dim=-2, eps=1e-6)
    ...
    def _extract_stem_indices(self, audio, sampling_rate=None):
        ...
        if sampling_rate != self.sampling_rate:
            self.resample = mindspore.dataset.audio.Resample(
                sampling_rate, self.sampling_rate, rolloff=0.945, lowpass_filter_width=24
            )
            wav = mindspore.tensor(self.resample(wav.numpy()))
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if isinstance(audio, mindspore.Tensor) and len(audio.shape) == 4:
            ...
            audio = mindspore.tensor(self.resample(audio.numpy()))
