...
class MusicgenMelodySinusoidalPositionalEmbedding(mindspore.nn.Cell):
    ...
    def get_embedding(num_embeddings: int, embedding_dim: int):
        ...
        emb = mindspore.mint.cat([mindspore.mint.cos(emb), mindspore.mint.sin(emb)], dim=1).view(num_embeddings, -1)
    ...
    def construct(self, inputs_embeds: mindspore.Tensor, past_key_values_length: int = 0):
        ...
        position_ids = mindspore.mint.arange(seq_len) + past_key_values_length
...
class MusicgenMelodyAttention(mindspore.nn.Cell):
...
class MusicgenMelodyFlashAttention2(MusicgenMelodyAttention):
    ...
    def construct(
        ...
        if (
            ...
            key_states = mindspore.mint.cat([past_key_value[0].transpose(1, 2), key_states], dim=1)
            value_states = mindspore.mint.cat([past_key_value[1].transpose(1, 2), value_states], dim=1)
        else:
            # self_attention
            key_states = self._reshape(self.k_proj(hidden_states), -1, bsz)
            value_states = self._reshape(self.v_proj(hidden_states), -1, bsz)
...
class MusicgenMelodyDecoderLayer(mindspore.nn.Cell):
    def __init__(self, config: MusicgenMelodyDecoderConfig):
        ...
        self.fc1 = mindspore.mint.nn.Linear(self.embed_dim, config.ffn_dim, bias=False)
        self.fc2 = mindspore.mint.nn.Linear(config.ffn_dim, self.embed_dim, bias=False)
...
class MusicgenMelodyPreTrainedModel(MSPreTrainedModel):
    """
...
class MusicgenMelodyDecoder(MusicgenMelodyPreTrainedModel):
    ...
    def __init__(self, config: MusicgenMelodyDecoderConfig):
        ...
        self.embed_tokens = mindspore.nn.CellList(
            [mindspore.mint.nn.Embedding(embed_dim, config.hidden_size) for _ in range(config.num_codebooks)]
        )
        ...
        self.layers = mindspore.nn.CellList(
            [MusicgenMelodyDecoderLayer(config) for _ in range(config.num_hidden_layers)]
        )
        self.layer_norm = mindspore.mint.nn.LayerNorm(config.hidden_size)
    ...
    def construct(
        ...
        if encoder_hidden_states is not None:
            ...
            if attention_mask is not None:
                ...
                attention_mask = mindspore.mint.cat([encoder_attention_mask, attention_mask], dim=1)

            # fuse encoder_hidden_states and inputs_embeds
            inputs_embeds = mindspore.mint.cat([encoder_hidden_states, inputs_embeds], dim=1)
...
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenForCausalLM with
# MUSICGEN->MUSICGEN_MELODY,Musicgen->MusicgenMelody,MusicGen->Musicgen Melody
class MusicgenMelodyForCausalLM(MusicgenMelodyPreTrainedModel, GenerationMixin):
    def __init__(self, config: MusicgenMelodyDecoderConfig):
        ...
        self.num_codebooks = config.num_codebooks
        self.lm_heads = mindspore.nn.CellList(
            [
                mindspore.mint.nn.Linear(config.hidden_size, config.vocab_size, bias=False)
    ...
    def construct(
        ...
        lm_logits = mindspore.mint.stack([head(hidden_states) for head in self.lm_heads], dim=1)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if guidance_scale is not None and guidance_scale > 1:
            ...
            if encoder_hidden_states is not None:
                encoder_hidden_states = mindspore.mint.cat(
                    [encoder_hidden_states, mindspore.mint.zeros_like(encoder_hidden_states)], dim=0
                )

            if encoder_attention_mask is not None:
                encoder_attention_mask = mindspore.mint.cat(
                    encoder_attention_mask, mindspore.mint.zeros_like(encoder_attention_mask), dim=0
                )
    ...
    def build_delay_pattern_mask(self, input_ids: mindspore.Tensor, pad_token_id: int, max_length: int = None):
        ...
        delay_pattern = mindspore.mint.triu(
            mindspore.mint.ones((channel_codebooks, max_length), dtype=mindspore.bool_),
            diagonal=max_length - channel_codebooks + 1,
        )
        # then fill the lower triangular part (the BOS padding)
        delay_pattern = delay_pattern + mindspore.mint.tril(
            mindspore.mint.ones((channel_codebooks, max_length), dtype=mindspore.bool_)
        )
    ...
    def apply_delay_pattern_mask(input_ids, decoder_pad_token_mask):
        ...
        input_ids = mindspore.mint.where(decoder_pad_token_mask == -1, input_ids, decoder_pad_token_mask)
        return input_ids
...
class MusicgenMelodyForConditionalGeneration(MSPreTrainedModel, GenerationMixin):
    ...
    def __init__(
        ...
        text_encoder: Optional[MSPreTrainedModel] = None,
        audio_encoder: Optional[MSPreTrainedModel] = None,
        ...
        if self.text_encoder.config.hidden_size != self.decoder.config.hidden_size:
            self.enc_to_dec_proj = mindspore.mint.nn.Linear(
        ...
        if self.config.num_chroma != self.decoder.config.hidden_size:
            self.audio_enc_to_dec_proj = mindspore.mint.nn.Linear(
    ...
    @classmethod
    # Copied from transformers.models.musicgen.modeling_musicgen.MusicgenForConditionalGeneration.
    # from_sub_models_pretrained with Musicgen->MusicgenMelody, musicgen-small->musicgen-melody
    ...
    def construct(
        ...
        **kwargs,
        ...
        >>> from mindone.transformers import MusicgenMelodyForConditionalGeneration
        >>> import mindspore
        ...
        >>> decoder_input_ids = (
        ...     mindspore.mint.ones((inputs.input_ids.shape[0] * model.decoder.num_codebooks, 1), dtype=mindspore.int64)
        ...     * pad_token_id
        ... )
        ...
        if encoder_hidden_states is None:
            ...
            if encoder_hidden_states is not None and input_features is None:
                input_features = mindspore.mint.zeros(
            ...
            if input_features is not None:
                ...
                if encoder_hidden_states is not None:
                    encoder_hidden_states = mindspore.mint.cat([audio_hidden_states, encoder_hidden_states], dim=1)
                else:
    ...
    def _prepare_decoder_input_ids_for_generation(
        ...
        # decoder_attention_mask if provided)
            ...
            decoder_input_ids = mindspore.mint.cat([decoder_input_ids_start, decoder_input_ids], dim=-1)
            if "decoder_attention_mask" in model_kwargs:
                decoder_attention_mask = model_kwargs["decoder_attention_mask"]
                decoder_attention_mask = mindspore.mint.cat(
                    (mindspore.mint.ones_like(decoder_attention_mask)[:, :1], decoder_attention_mask),
                    dim=-1,
                )
    ...
    def _prepare_encoder_hidden_states_kwargs_for_generation(
        ...
        if inputs_tensor is not None:
            ...
            # Compatibility with Accelerate big model inference:
            # as the inputs.
        ...
        if inputs_tensor is not None:
            if audio_hidden_states is not None:
                null_audio_hidden_states = mindspore.mint.zeros_like(audio_hidden_states)
            else:
                null_audio_hidden_states = mindspore.mint.zeros(
        ...
        if audio_hidden_states is not None:
            ...
            if guidance_scale is not None and guidance_scale > 1:
                audio_hidden_states = mindspore.mint.cat([audio_hidden_states, null_audio_hidden_states], dim=0)
    ...
    def generate(
        ...
        **kwargs,
        ...
        if self.decoder.config.audio_channels == 1:
            ...
            output_values = mindspore.mint.cat([output_values_left, output_values_right], dim=1)
