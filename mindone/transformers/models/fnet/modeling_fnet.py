...
import numpy as np
from transformers.models.fnet.configuration_fnet import FNetConfig
...
def _two_dim_matmul(x, matrix_dim_one, matrix_dim_two):
    ...
    return mindspore.mint.einsum("bij,jk,ni->bnk", x, matrix_dim_two, matrix_dim_one)
...
def fftn(x):
    ...
    """
    # fixme:MindSpore currently does NOT provide a native FFT operator.
    #  We therefore fall back to NumPy's FFT implementation (np.fft).
    ms_dtype = x.dtype
    out = x.float().asnumpy()
    for axis in reversed(range(x.ndim)[1:]):  # We don't need to apply FFT to last axis
        out = np.fft.fft(out, axis=axis)
    return mindspore.tensor(out, dtype=ms_dtype)


class FNetEmbeddings(mindspore.nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.projection = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
        ...
        self.register_buffer(
            "token_type_ids", mindspore.mint.zeros(self.position_ids.shape, dtype=mindspore.int64), persistent=False
        )
...
class FNetBasicFourierTransform(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        if self.training:
            raise RuntimeError("FFT layer is inference-only. Back-propagation is prohibited.")
        else:
    ...
    def _init_fourier_transform(self, config):
        ...
        if not config.use_tpu_fourier_optimizations:
            self.fourier_transform = partial(np.fft.fftn, axes=(1, 2))
        elif config.max_position_embeddings <= 4096:
            if is_scipy_available():
                self.register_buffer(
                    "dft_mat_hidden", mindspore.Tensor(linalg.dft(config.hidden_size), dtype=mindspore.complex64)
                )
                self.register_buffer(
                    "dft_mat_seq", mindspore.Tensor(linalg.dft(config.tpu_short_seq_length), dtype=mindspore.complex64)
                )
    ...
    def construct(self, hidden_states):
        ...
        # Note that fourier transform methods will need
        # change accordingly.
        ms_dtype = hidden_states.dtype
        outputs = mindspore.tensor(self.fourier_transform(hidden_states.float()).real, dtype=ms_dtype)
        return (outputs,)


class FNetBasicOutput(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.LayerNorm = mindspore.mint.nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class FNetFourierTransform(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class FNetIntermediate(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class FNetOutput(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class FNetLayer(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class FNetEncoder(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = mindspore.nn.CellList([FNetLayer(config) for _ in range(config.num_hidden_layers)])
...
class FNetPooler(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class FNetPredictionHeadTransform(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.dense = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
...
class FNetLMPredictionHead(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class FNetOnlyMLMHead(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class FNetOnlyNSPHead(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.seq_relationship = mindspore.mint.nn.Linear(config.hidden_size, 2)
...
class FNetPreTrainingHeads(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.predictions = FNetLMPredictionHead(config)
        self.seq_relationship = mindspore.mint.nn.Linear(config.hidden_size, 2)
...
class FNetForPreTrainingOutput(ModelOutput):
    ...
    seq_relationship_logits: Optional[mindspore.Tensor] = None
...
class FNetForPreTraining(FNetPreTrainedModel):
    ...
    def construct(
        ...
        >>> from mindone.transformers import FNetForPreTraining
...
class FNetForNextSentencePrediction(FNetPreTrainedModel):
    ...
    def construct(
        ...
        **kwargs,
        ...
        ```python
        >>> from transformers import AutoTokenizer
        >>> from mindone.transformers import FNetForNextSentencePrediction
        >>> import mindspore as ms
        >>> import numpy as np
