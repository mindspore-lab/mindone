...
class VideoMAEEmbeddings(mindspore.nn.Cell):
    """
    ...
    def construct(self, pixel_values, bool_masked_pos):
        ...
        embeddings = embeddings + self.position_embeddings.type_as(embeddings)
...
class VideoMAEPatchEmbeddings(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        self.projection = mindspore.mint.nn.Conv3d(
...
class VideoMAESelfAttention(mindspore.nn.Cell):
    def __init__(self, config: VideoMAEConfig) -> None:
        ...
        self.query = mindspore.mint.nn.Linear(config.hidden_size, self.all_head_size, bias=False)
        self.key = mindspore.mint.nn.Linear(config.hidden_size, self.all_head_size, bias=False)
        self.value = mindspore.mint.nn.Linear(config.hidden_size, self.all_head_size, bias=False)

        if config.qkv_bias:
            self.q_bias = mindspore.Parameter(mindspore.mint.zeros(self.all_head_size))
            self.v_bias = mindspore.Parameter(mindspore.mint.zeros(self.all_head_size))
        else:
            self.q_bias = None
            self.v_bias = None
    ...
    def construct(
        ...
        k_bias = mindspore.mint.zeros_like(self.v_bias) if self.q_bias is not None else None
        keys = mindspore.mint.nn.functional.linear(input=hidden_states, weight=self.key.weight, bias=k_bias)
        values = mindspore.mint.nn.functional.linear(input=hidden_states, weight=self.value.weight, bias=self.v_bias)
        queries = mindspore.mint.nn.functional.linear(input=hidden_states, weight=self.query.weight, bias=self.q_bias)
...
class VideoMAESelfOutput(mindspore.nn.Cell):
    """
...
class VideoMAEAttention(mindspore.nn.Cell):
    def __init__(self, config: VideoMAEConfig) -> None:
        super().__init__()
...
class VideoMAEIntermediate(mindspore.nn.Cell):
    def __init__(self, config: VideoMAEConfig) -> None:
        super().__init__()
...
class VideoMAEOutput(mindspore.nn.Cell):
    def __init__(self, config: VideoMAEConfig) -> None:
        super().__init__()
...
class VideoMAELayer(mindspore.nn.Cell):
...
class VideoMAEEncoder(mindspore.nn.Cell):
    def __init__(self, config: VideoMAEConfig) -> None:
        super().__init__()
        self.config = config
        self.layer = mindspore.nn.CellList([VideoMAELayer(config) for _ in range(config.num_hidden_layers)])
...
VIDEOMAE_INPUTS_DOCSTRING = r"""
    Args:
        pixel_values (`mindspore.Tensor` of shape `(batch_size, num_frames, num_channels, height, width)`):
...
class VideoMAEModel(VideoMAEPreTrainedModel):
    ...
    def construct(
        ...
        r"""
        bool_masked_pos (`mindspore.Bool_` of shape `(batch_size, sequence_length)`, *optional*):
        ...
        >>> from mindone.transformers import AutoImageProcessor, VideoMAEModel
...
class VideoMAEDecoder(mindspore.nn.Cell):
    def __init__(self, config, num_patches):
        ...
        self.norm = mindspore.mint.nn.LayerNorm(config.decoder_hidden_size)
        self.head = (
            mindspore.mint.nn.Linear(config.decoder_hidden_size, decoder_num_labels)
...
class VideoMAEForPreTraining(VideoMAEPreTrainedModel):
    def __init__(self, config):
        ...
        self.encoder_to_decoder = mindspore.mint.nn.Linear(config.hidden_size, config.decoder_hidden_size, bias=False)
    ...
    def construct(
        ...
        r"""
        bool_masked_pos (`ms.Bool_` of shape `(batch_size, sequence_length)`):
            Boolean masked positions. Indicates which patches are masked (1) and which aren't (0). Each video in the
        ...
        Examples:
        ```python
        >>> from mindone.transformers import AutoImageProcessor, VideoMAEForPreTraining
        >>> import numpy as np
        ...
        >>> bool_masked_pos = mint.randint(0, 2, (1, seq_length)).bool()
        ...
        x_full = mindspore.mint.cat([sequence_output + pos_emb_visible, self.mask_token + pos_emb_mask], dim=1)
...
class VideoMAEForVideoClassification(VideoMAEPreTrainedModel):
    def __init__(self, config):
        ...
        # Classifier head
        self.fc_norm = mindspore.mint.nn.LayerNorm(config.hidden_size) if config.use_mean_pooling else None
        self.classifier = (
    ...
    def construct(
        ...
        >>> from mindone.transformers import AutoImageProcessor, VideoMAEForVideoClassification
        >>> from huggingface_hub import hf_hub_download
