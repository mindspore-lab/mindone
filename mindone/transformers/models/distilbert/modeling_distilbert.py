...
def create_sinusoidal_embeddings(n_pos: int, dim: int, out: ms.Tensor):
    _create_sinusoidal_embeddings(n_pos=n_pos, dim=dim, out=out)


def _create_sinusoidal_embeddings(n_pos: int, dim: int, out: ms.Tensor):
...
class Embeddings(nn.Cell):
    def __init__(self, config: PretrainedConfig):
        super().__init__()
        self.word_embeddings = mint.nn.Embedding(config.vocab_size, config.dim, padding_idx=config.pad_token_id)
        self.position_embeddings = mint.nn.Embedding(config.max_position_embeddings, config.dim)
    ...
    def construct(self, input_ids: ms.Tensor, input_embeds: Optional[ms.Tensor] = None) -> ms.Tensor:
        ...
        Parameters:
            input_ids (ms.Tensor):
                ms.Tensor(bs, max_seq_length) The token ids to embed.
            input_embeds (*optional*, ms.Tensor):
                The pre-computed word embeddings. Can only be passed if the input ids are `None`.


        Returns: ms.Tensor(bs, max_seq_length, dim) The embedded tokens (plus position embeddings, no token_type
        embeddings)
        """
        ...
        if hasattr(self, "position_ids"):
            ...
            position_ids = mint.arange(seq_length, dtype=ms.int64)  # (max_seq_length)
...
class MultiHeadSelfAttention(nn.Cell):
    def __init__(self, config: PretrainedConfig):
        ...
        self.q_lin = mint.nn.Linear(in_features=config.dim, out_features=config.dim)
        self.k_lin = mint.nn.Linear(in_features=config.dim, out_features=config.dim)
        self.v_lin = mint.nn.Linear(in_features=config.dim, out_features=config.dim)
        self.out_lin = mint.nn.Linear(in_features=config.dim, out_features=config.dim)
    ...
    def construct(
        ...
        """
        Parameters:
            query: ms.Tensor(bs, seq_length, dim)
            key: ms.Tensor(bs, seq_length, dim)
            value: ms.Tensor(bs, seq_length, dim)
            mask: ms.Tensor(bs, seq_length)

        Returns:
            weights: ms.Tensor(bs, n_heads, seq_length, seq_length) Attention weights context: ms.Tensor(bs,
        ...
        def shape(x: ms.Tensor) -> ms.Tensor:
            """separate heads"""
            return x.view(bs, -1, self.n_heads, dim_per_head).transpose(1, 2)

        def unshape(x: ms.Tensor) -> ms.Tensor:
            """group heads"""
        ...
        context = mint.matmul(weights, v)  # (bs, n_heads, q_length, dim_per_head)
...
class DistilBertFlashAttention2(MultiHeadSelfAttention):
    ...
    def construct(
        ...
        Returns:
            ...
            seq_length, dim) Contextualized layer. Optional: only if `output_attentions=True`
        """
        batch_size, q_length, dim = query.shape

        dim_per_head = self.dim // self.n_heads

        def reshape(x: ms.Tensor) -> ms.Tensor:
            """separate heads"""
        ...
        if query_states.dtype == ms.float32:
...
class DistilBertSdpaAttention(MultiHeadSelfAttention):
    ...
    def construct(
        ...
        if output_attentions or head_mask is not None:
            logger.warning_once(
                "DistilBertSdpaAttention is used but `scaled_dot_product_attention` does not support"
...
class FFN(nn.Cell):
    def __init__(self, config: PretrainedConfig):
        ...
        self.seq_len_dim = 1
        self.lin1 = mint.nn.Linear(in_features=config.dim, out_features=config.hidden_dim)
        self.lin2 = mint.nn.Linear(in_features=config.hidden_dim, out_features=config.dim)
        self.activation = get_activation(config.activation)

    def construct(self, input: ms.Tensor) -> ms.Tensor:
        return apply_chunking_to_forward(self.ff_chunk, self.chunk_size_feed_forward, self.seq_len_dim, input)

    def ff_chunk(self, input: ms.Tensor) -> ms.Tensor:
        x = self.lin1(input)
        x = self.activation(x)
        x = self.lin2(x)
        x = self.dropout(x)
...
class TransformerBlock(nn.Cell):
    def __init__(self, config: PretrainedConfig):
        ...
        self.sa_layer_norm = mint.nn.LayerNorm(normalized_shape=config.dim, eps=1e-12)

        self.ffn = FFN(config)
        self.output_layer_norm = mint.nn.LayerNorm(normalized_shape=config.dim, eps=1e-12)
    ...
    def construct(
        ...
        """
        Parameters:
            x: ms.Tensor(bs, seq_length, dim)
            attn_mask: ms.Tensor(bs, seq_length)

        Returns:
            sa_weights: ms.Tensor(bs, n_heads, seq_length, seq_length) The attention weights ffn_output:
            ms.Tensor(bs, seq_length, dim) The output of the transformer block contextualization.
        """
        # Self-Attention
        ...
        ffn_output: ms.Tensor = self.output_layer_norm(ffn_output + sa_output)  # (bs, seq_length, dim)
...
class Transformer(nn.Cell):
    ...
    ) -> Union[BaseModelOutput, Tuple[ms.Tensor, ...]]:  # docstyle-ignore
        """
        Parameters:
            x: ms.Tensor(bs, seq_length, dim) Input sequence embedded.
            attn_mask: ms.Tensor(bs, seq_length) Attention mask on the sequence.

        Returns:
            hidden_state: ms.Tensor(bs, seq_length, dim) Sequence of hidden states in the last (top)
            layer all_hidden_states: Tuple[ms.Tensor(bs, seq_length, dim)]
                Tuple of length n_layers with the hidden states from each layer.
                Optional: only if output_hidden_states=True
            all_attentions: Tuple[ms.Tensor(bs, n_heads, seq_length, seq_length)]
...
DISTILBERT_START_DOCSTRING = r"""
    ...
    This model is also a MindSpore [mindspore.nn.Cell](https://mindspore.cn/docs/en/master/api_python/nn/mindspore.nn.Cell.html) subclass.
...
class DistilBertModel(DistilBertPreTrainedModel):
    ...
    def get_position_embeddings(self) -> mint.nn.Embedding:
        """
    ...
    def resize_position_embeddings(self, new_num_position_embeddings: int):
        ...
        self.embeddings.position_embeddings = mint.nn.Embedding(self.config.max_position_embeddings, self.config.dim)
        ...
        if self.config.sinusoidal_pos_embds:
            ...
            if num_position_embeds_diff > 0:
                self.embeddings.position_embeddings.weight[:-num_position_embeds_diff] = ms.Parameter(
                    old_position_embeddings_weight
                )
            else:
                self.embeddings.position_embeddings.weight = ms.Parameter(
    ...
    def set_input_embeddings(self, new_embeddings: mint.nn.Embedding):
    ...
    def construct(
        ...
        return_dict: Optional[bool] = None,
    ) -> Union[BaseModelOutput, Tuple[ms.Tensor, ...]]:
        ...
        if self._use_flash_attention_2:
            ...
            if attention_mask is None:
                attention_mask = mint.ones(input_shape)  # (bs, seq_length)
...
class DistilBertForMaskedLM(DistilBertPreTrainedModel):
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        self.vocab_transform = mint.nn.Linear(config.dim, config.dim)
        self.vocab_layer_norm = mint.nn.LayerNorm(config.dim, eps=1e-12)
        self.vocab_projector = mint.nn.Linear(config.dim, config.vocab_size)

        # Initialize weights and apply final processing
        self.post_init()

        self.mlm_loss_fct = mint.nn.CrossEntropyLoss()
...
class DistilBertForSequenceClassification(DistilBertPreTrainedModel):
    def __init__(self, config: PretrainedConfig):
        ...
        self.distilbert = DistilBertModel(config)
        self.pre_classifier = mint.nn.Linear(config.dim, config.dim)
        self.classifier = mint.nn.Linear(config.dim, config.num_labels)
        self.dropout = mint.nn.Dropout(config.seq_classif_dropout)
    ...
    def construct(
        ...
        pooled_output = mint.nn.ReLU()(pooled_output)  # (bs, dim)
...
class DistilBertForQuestionAnswering(DistilBertPreTrainedModel):
    def __init__(self, config: PretrainedConfig):
        ...
        self.distilbert = DistilBertModel(config)
        self.qa_outputs = mint.nn.Linear(config.dim, config.num_labels)
        if config.num_labels != 2:
            raise ValueError(f"config.num_labels should be 2, but it is {config.num_labels}")

        self.dropout = mint.nn.Dropout(config.qa_dropout)
...
class DistilBertForMultipleChoice(DistilBertPreTrainedModel):
    def __init__(self, config: PretrainedConfig):
        ...
        self.distilbert = DistilBertModel(config)
        self.pre_classifier = mint.nn.Linear(config.dim, config.dim)
        self.classifier = mint.nn.Linear(config.dim, 1)
        self.dropout = mint.nn.Dropout(config.seq_classif_dropout)
    ...
    def construct(
        ...
        >>> from mindone.transformers import DistilBertForMultipleChoice
        >>> import mindspore as ms
        ...
        pooled_output = mint.nn.ReLU()(pooled_output)  # (bs * num_choices, dim)
