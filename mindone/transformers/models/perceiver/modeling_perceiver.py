...
class PerceiverModelOutput(ModelOutput):
    ...
    Args:
        logits (`ms.Tensor` of shape `(batch_size, num_labels)`):
...
class PerceiverMaskedLMOutput(ModelOutput):
    ...
    Args:
        ...
        hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            ...
            plus the initial embedding outputs.
        attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_heads, num_latents,
...
class PerceiverEmbeddings(ms.nn.Cell):
...
class PerceiverSelfAttention(ms.nn.Cell):
    ...
    def __init__(
        ...
        # Layer normalization
        self.layernorm1 = mint.nn.LayerNorm(q_dim)
        self.layernorm2 = mint.nn.LayerNorm(kv_dim) if is_cross_attention else mint.nn.Identity()

        # Projection matrices
        self.query = mint.nn.Linear(q_dim, qk_channels)
        self.key = mint.nn.Linear(kv_dim, qk_channels)
        self.value = mint.nn.Linear(kv_dim, v_channels)
    ...
    def construct(
        ...
        inputs_mask: Optional[ms.Tensor] = None,
        ...
        attention_scores = mint.matmul(queries, keys.transpose(-1, -2))
        ...
        context_layer = mint.matmul(attention_probs, values)
...
class PerceiverSelfOutput(ms.nn.Cell):
    def __init__(self, config, input_channels, output_channels):
        super().__init__()
        self.dense = mint.nn.Linear(input_channels, output_channels)
...
class PerceiverAttention(ms.nn.Cell):
...
class PerceiverMLP(ms.nn.Cell):
    """A Transformer-style dense module to follow attention."""

    def __init__(self, config, input_size, widening_factor):
        super().__init__()
        self.dense1 = mint.nn.Linear(input_size, widening_factor * input_size)
        if isinstance(config.hidden_act, str):
            self.intermediate_act_fn = ACT2FN[config.hidden_act]
        else:
            self.intermediate_act_fn = config.hidden_act
        self.dense2 = mint.nn.Linear(widening_factor * input_size, input_size)
...
class PerceiverLayer(ms.nn.Cell):
    def __init__(
        self,
        config,
        is_cross_attention=False,
        ...
        self.layernorm = mint.nn.LayerNorm(q_dim)
...
class PerceiverEncoder(ms.nn.Cell):
    ...
    def __init__(self, config, kv_dim=None):
        ...
        self.self_attends = ms.nn.CellList(self_attention_layers)
...
PERCEIVER_START_DOCSTRING = r"""
    This model is a MindSpore nn.Cell sub-class. Use
...
class PerceiverModel(PerceiverPreTrainedModel):
    ...
    def construct(
        ...
        inputs: ms.Tensor,
        attention_mask: Optional[ms.Tensor] = None,
        subsampled_output_points: Optional[Dict[str, ms.Tensor]] = None,
        head_mask: Optional[ms.Tensor] = None,
        ...
        >>> criterion = mint.nn.CrossEntropyLoss()
...
class PerceiverForOpticalFlow(PerceiverPreTrainedModel):
    ...
    def construct(
        ...
        >>> patches = ms.Tensor(np.random.randn(1, 2, 27, 368, 496))
...
class PerceiverForMultimodalAutoencoding(PerceiverPreTrainedModel):
    ...
    def construct(
        ...
        ```python
        >>> from mindone.transformers import PerceiverForMultimodalAutoencoding
        >>> import mindspore as ms
        >>> import numpy as np

        >>> # create multimodal inputs
        >>> images = ms.Tensor(np.random.randn(1, 16, 3, 224, 224))
        >>> audio = ms.Tensor(np.random.randn(1, 30720, 1))
        >>> inputs = dict(image=images, audio=audio, label=ms.Tensor(np.zeros((images.shape[0], 700))))
        ...
        >>> chunk_idx = 0
        >>> subsampling = {
        ...     "image": ms.Tensor(np.arange(image_chunk_size * chunk_idx, image_chunk_size * (chunk_idx + 1))),
        ...     "audio": ms.Tensor(np.arange(audio_chunk_size * chunk_idx, audio_chunk_size * (chunk_idx + 1))),
        ...     "label": None,
        ... }
...
def build_position_encoding(
    ...
    positions_projection = mint.nn.Linear(out_channels, project_pos_dim) if project_pos_dim > 0 else mint.nn.Identity()
...
class PerceiverAbstractDecoder(ms.nn.Cell, metaclass=abc.ABCMeta):
...
class PerceiverProjectionDecoder(PerceiverAbstractDecoder):
    ...
    def __init__(self, config):
        super().__init__()
        self.classifier = mint.nn.Linear(config.d_latents, config.num_labels)
...
class PerceiverBasicDecoder(PerceiverAbstractDecoder):
    ...
    def decoder_query(self, inputs, modality_sizes=None, inputs_without_pos=None, subsampled_points=None):
        ...
        if subsampled_points is not None:
            ...
            pos = -1 + 2 * pos / ms.Tensor(self.output_index_dims)[None, :]
            pos = mint.broadcast_to(pos[None], [batch_size, pos.shape[0], pos.shape[1]])
            ...
            pos_emb = mint.reshape(pos_emb, [pos_emb.shape[0], -1, pos_emb.shape[-1]])
        else:
            batch_size = inputs.shape[0]
        ...
        if self.concat_preprocessed_input:
            ...
            pos_emb = mint.cat([inputs_without_pos, pos_emb], dim=-1)
...
class PerceiverBasicVideoAutoencodingDecoder(PerceiverAbstractDecoder):
    ...
    def construct(
        ...
        logits = mint.reshape(logits, self.output_shape + [logits.shape[-1]])
        return PerceiverDecoderOutput(logits=logits, cross_attentions=decoder_outputs.cross_attentions)


def restructure(modality_sizes: ModalitySizeType, inputs: ms.Tensor) -> Mapping[str, ms.Tensor]:
    """
...
class PerceiverMultimodalDecoder(PerceiverAbstractDecoder):
    ...
    def __init__(
        ...
        **decoder_kwargs,
        ...
        super().__init__()
        self.modalities = ms.nn.CellDict(modalities)
        self.subsampled_index_dims = subsampled_index_dims
        ...
        self.padding = nn.ParameterDict(
            {
                modality: ms.Parameter(mint.randn(1, self.num_query_channels - decoder.num_query_channels))
    ...
    def decoder_query(self, inputs, modality_sizes, inputs_without_pos=None, subsampled_points=None):
        ...
        def embed(modality, x):
            x = mint.reshape(x, [x.shape[0], np.prod(x.shape[1:-1]), x.shape[-1]])
            pos = self.padding[modality]
            pos = mint.broadcast_to(pos, [x.shape[0], x.shape[1], self.num_query_channels - x.shape[2]])
            return mint.cat([x, pos], dim=2)
...
def space_to_depth(frames: ms.Tensor, temporal_block_size: int = 1, spatial_block_size: int = 1) -> ms.Tensor:
    """
...
class Conv2dSamePadding(mint.nn.Conv2d):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.zero_pad_2d = mint.nn.ZeroPad2d(
...
class Conv2DDownsample(ms.nn.Cell):
    ...
    def __init__(
        ...
        self.batchnorm = mint.nn.BatchNorm2d(num_features=out_channels) if use_batchnorm else mint.nn.Identity()
...
def generate_fourier_features(pos, num_bands, max_resolution=(224, 224), concat_pos=True, sine_only=False):
    ...
    Args:
      pos (`ms.Tensor` of shape `(batch_size, sequence_length, dim)`):
    ...
    Returns:
      `ms.Tensor` of shape `(batch_size, sequence_length, n_channels)`: The Fourier position embeddings. If
    ...
    freq_bands = mint.stack(
        [mint.linspace(start=min_freq, end=res / 2, steps=num_bands) for res in max_resolution], dim=0
    )

    # Get frequency bands for each spatial dimension.
    # Output is size [n, d * num_bands]
    per_pos_features = pos[0, :, :][:, :, None] * freq_bands[None, :, :]
    per_pos_features = mint.reshape(per_pos_features, [-1, np.prod(per_pos_features.shape[1:])])
    ...
    if concat_pos:
        ...
        per_pos_features = mint.cat([pos, per_pos_features.broadcast_to((batch_size, -1, -1))], dim=-1)
...
def build_linear_positions(index_dims, output_range=(-1.0, 1.0)):
    ...
    Returns:
      `ms.Tensor` of shape `(index_dims[0], index_dims[1], .., index_dims[-1], N)`.
    """

    def _linspace(n_xels_per_dim):
        return mint.linspace(start=output_range[0], end=output_range[1], steps=n_xels_per_dim, dtype=ms.float32)

    dim_ranges = [_linspace(n_xels_per_dim) for n_xels_per_dim in index_dims]
    array_index_grid = meshgrid(*dim_ranges, indexing="ij")

    return mint.stack(array_index_grid, dim=-1)


class PerceiverAbstractPositionEncoding(ms.nn.Cell, metaclass=abc.ABCMeta):
...
class PerceiverTrainablePositionEncoding(PerceiverAbstractPositionEncoding):
    ...
    def interpolate_pos_encoding(self, position_embeddings: ms.Tensor, height: int, width: int) -> ms.Tensor:
        num_positions = position_embeddings.shape[0]
        new_height = new_width = int(num_positions**0.5)
        ...
        position_embeddings = mint.nn.functional.interpolate(
            position_embeddings,
...
def _check_or_build_spatial_positions(pos, index_dims, batch_size):
    ...
    Returns:
        `ms.Tensor` of shape `(batch_size, prod(index_dims))` an array of position features.
    """
    if pos is None:
        pos = build_linear_positions(index_dims)
        # equivalent to `torch.broadcast_to(pos[None], (batch_size,) + pos.shape)`
        # but `torch.broadcast_to` cannot be converted to ONNX, using MindSpore broadcast_to instead
        pos = pos[None].broadcast_to((batch_size,) + pos.shape)
        pos = mint.reshape(pos, [batch_size, np.prod(index_dims), -1])
    else:
...
class PerceiverFourierPositionEncoding(PerceiverAbstractPositionEncoding):
    ...
    def construct(
        ...
        index_dims: List[int],
        batch_size: int,
        # device: torch.device,  # removed device parameter for MindSpore
        dtype: ms.dtype,
        pos: ms.Tensor = None,
    ) -> ms.Tensor:
...
class AbstractPreprocessor(ms.nn.Cell):
    @property
...
class PerceiverTextPreprocessor(AbstractPreprocessor):
    ...
    def __init__(self, config: PerceiverConfig) -> None:
        super().__init__()
        self.config = config
        self.embeddings = mint.nn.Embedding(num_embeddings=config.vocab_size, embedding_dim=config.d_model)
        self.position_embeddings = mint.nn.Embedding(config.max_position_embeddings, config.d_model)
...
class PerceiverEmbeddingDecoder(ms.nn.Cell):
    """
    ...
    def __init__(self, config: PerceiverConfig) -> None:
        ...
        self.bias = ms.Parameter(mint.zeros(self.vocab_size))

    def construct(self, hidden_states: ms.Tensor, embedding_layer: ms.Tensor) -> ms.Tensor:
        batch_size, seq_len, d_model = hidden_states.shape
        # Flatten batch dim
        output = mint.matmul(hidden_states.reshape([-1, d_model]), embedding_layer.weight.transpose(0, 1))
        output = output + self.bias

        return output.reshape([batch_size, seq_len, self.vocab_size])


class PerceiverMultimodalPostprocessor(ms.nn.Cell):
    """
    ...
    def construct(
        self, inputs: ms.Tensor, pos: Optional[ms.Tensor] = None, modality_sizes=None
    ) -> Mapping[str, ms.Tensor]:
...
class PerceiverClassificationPostprocessor(ms.nn.Cell):
    """
    ...
    def __init__(self, config: PerceiverConfig, in_channels: int) -> None:
        super().__init__()
        self.classifier = mint.nn.Linear(in_channels, config.num_labels)

    def construct(self, inputs, pos: Optional[ms.Tensor] = None, modality_sizes=None) -> ms.Tensor:
        logits = self.classifier(inputs)
        return logits[:, 0, :]


class PerceiverAudioPostprocessor(ms.nn.Cell):
    """
    ...
    def __init__(self, config: PerceiverConfig, in_channels: int, postproc_type: str = "patches") -> None:
        ...
        self.classifier = mint.nn.Linear(in_channels, config.samples_per_patch)

    def construct(self, inputs: ms.Tensor, pos: Optional[ms.Tensor] = None, modality_sizes=None) -> ms.Tensor:
        logits = self.classifier(inputs)
        return mint.reshape(logits, [inputs.shape[0], -1])


class PerceiverProjectionPostprocessor(ms.nn.Cell):
    """
    ...
    def __init__(self, in_channels: int, out_channels: int) -> None:
        super().__init__()
        self.classifier = mint.nn.Linear(in_channels, out_channels)
...
class PerceiverImagePreprocessor(AbstractPreprocessor):
    ...
    def __init__(
        ...
        **position_encoding_kwargs,
        ...
        if self.prep_type == "conv":
            ...
            self.convnet_1x1 = mint.nn.Conv2d(
                in_channels=in_channels,
                out_channels=out_channels,
                kernel_size=(1, 1),
    ...
    def _build_network_inputs(
        self, inputs: ms.Tensor, network_input_is_1d: bool = True, interpolate_pos_encoding: bool = False
    ):
        """
        ...
        if len(inputs.shape) > 3 and network_input_is_1d:
            inputs = mint.reshape(inputs, [batch_size, indices, -1])
        ...
        if not network_input_is_1d:
            ...
            sh = inputs.shape
            pos_enc = mint.reshape(pos_enc, list(sh)[:-1] + [-1])
        if self.concat_or_add_pos == "concat":
            inputs_with_pos = mint.cat([inputs, pos_enc], dim=-1)
...
class PerceiverOneHotPreprocessor(AbstractPreprocessor):
    ...
    def construct(self, inputs: ms.Tensor, pos: Optional[ms.Tensor] = None, network_input_is_1d: bool = True):
...
class PerceiverAudioPreprocessor(AbstractPreprocessor):
    ...
    def construct(
        ...
        inputs = mint.reshape(inputs, [inputs.shape[0], -1, self.samples_per_patch])
...
class PerceiverMultimodalPreprocessor(AbstractPreprocessor):
    ...
    def __init__(
        ...
        self.padding = nn.ParameterDict(
            {
                modality: ms.Parameter(mint.randn(1, self.num_channels - preprocessor.num_channels))
                for modality, preprocessor in modalities.items()
            }
        )
        self.mask = nn.ParameterDict(
            {modality: ms.Parameter(mint.randn(1, self.num_channels)) for modality, _ in self.mask_probs.items()}
        )
    ...
    def construct(
        ...
        for modality, preprocessor in self.modalities.items():
            ...
            padding = mint.broadcast_to(
                pos_enc,
                [batch_size, num_samples, self.num_channels - num_channels],
            )
            output_padded = mint.cat([output, padding], dim=2)
            ...
            if modality in self.mask_probs:
                ...
                mask = mint.bernoulli(ms.ops.full([batch_size, num_samples], mask_prob))
                mask = mint.unsqueeze(mask, dim=2)
        ...
        final_inputs = mint.cat(padded_ls, dim=1)
