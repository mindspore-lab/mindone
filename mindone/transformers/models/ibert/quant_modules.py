...
class QuantEmbedding(ms.nn.Cell):
    """
    Quantized version of `mindspore.nn.Embedding`. Adds quantization-specific arguments on top of `mindspore.nn.Embedding`.
    ...
    def __init__(
        ...
        self.register_buffer("weight_integer", mint.zeros_like(self.weight))
...
class QuantLinear(ms.nn.Cell):
    ...
    def __init__(
        ...
        self.weight = ms.Parameter(mint.zeros([out_features, in_features]))
        self.register_buffer("weight_integer", mint.zeros_like(self.weight))
        self.register_buffer("fc_scaling_factor", mint.zeros(self.out_features))
        if bias:
            self.bias = ms.Parameter(mint.zeros(out_features))
            self.register_buffer("bias_integer", mint.zeros_like(self.bias))
    ...
    def construct(self, x, prev_act_scaling_factor=None):
        if not self.quant_mode:
            return mint.nn.functional.linear(x, weight=self.weight, bias=self.bias), None
        ...
        return (
            mint.nn.functional.linear(x_int, weight=self.weight_integer, bias=self.bias_integer) * bias_scaling_factor,
            bias_scaling_factor,
        )
...
class IntGELU(ms.nn.Cell):
    ...
    def __init__(self, quant_mode=True, force_dequant="none"):
        ...
        if not self.quant_mode:
            self.activation_fn = mint.nn.GELU()
    ...
    def int_erf(self, x_int, scaling_factor):
        b_int = mint.floor(self.coeff[1] / scaling_factor)
        c_int = mint.floor(self.coeff[2] / scaling_factor**2)
        sign = mint.sign(x_int)

        abs_int = mint.min(mint.abs(x_int), -b_int)
...
class IntSoftmax(ms.nn.Cell):
    ...
    def int_polynomial(self, x_int, scaling_factor):
        with ms._no_grad():
            b_int = mint.floor(self.coef[1] / scaling_factor)
            c_int = mint.floor(self.coef[2] / scaling_factor**2)
    ...
    def construct(self, x, scaling_factor):
        if not self.quant_mode:
            return mint.nn.functional.softmax(x, dim=-1), None
...
class IntLayerNorm(ms.nn.Cell):
    ...
    def __init__(self, normalized_shape, eps, output_bit=8, quant_mode=False, force_dequant="none"):
        ...
        self.weight = ms.Parameter(mint.zeros(normalized_shape))
        ...
        self.register_buffer("shift", mint.zeros(1))
        self.output_bit = output_bit
        self.max_bit = 32
        self.dim_sqrt = None
...
def linear_quantize(input, scale, zero_point, inplace=False):
    ...
    if inplace:
        ...
        return input
    return mint.round(1.0 / scale * input + zero_point)
...
def symmetric_linear_quantization_params(num_bits, saturation_min, saturation_max, per_channel=False):
    ...
    with ms._no_grad():
        ...
        if per_channel:
            scale, _ = mint.max(mint.stack([saturation_min.abs(), saturation_max.abs()], dim=1), dim=1)
            scale = mint.clamp(scale, min=1e-8) / n
...
class FixedPointMul(ms.nn.Cell):
    ...
    def construct(
        ...
        with ms._no_grad():
            ...
            z_int = mint.round(pre_act / pre_act_scaling_factor)
            _A = pre_act_scaling_factor.type(ms.float64)
            _B = (z_scaling_factor.type(ms.float32)).type(ms.float64)
            new_scale = _A / _B
            new_scale = reshape(new_scale)

            m, e = batch_frexp(new_scale)

            output = z_int.type(ms.float64) * m.type(ms.float64)
            output = mint.round(output / (2.0**e))

            if identity is not None:
                # needs addition of identity activation
                wx_int = mint.round(identity / identity_scaling_factor)

                _A = identity_scaling_factor.type(ms.float64)
                _B = (z_scaling_factor.type(ms.float32)).type(ms.float64)
                new_scale = _A / _B
                new_scale = reshape(new_scale)

                m1, e1 = batch_frexp(new_scale)
                output1 = wx_int.type(ms.float64) * m1.type(ms.float64)
                output1 = mint.round(output1 / (2.0**e1))
