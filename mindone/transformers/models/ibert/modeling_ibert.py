...
class IBertSelfOutput(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.quant_mode = config.quant_mode
        self.act_bit = 8
...
class IBertAttention(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.quant_mode = config.quant_mode
...
class IBertIntermediate(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.quant_mode = config.quant_mode
        self.act_bit = 8
...
class IBertEncoder(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.quant_mode = config.quant_mode
        self.layer = ms.nn.CellList([IBertLayer(config) for _ in range(config.num_hidden_layers)])
...
IBERT_START_DOCSTRING = r"""
    ...
    This model is also a MindSpore [mindspore.nn.Cell](https://mindspore.cn/docs/zh-CN/master/api_python/nn/mindspore.nn.Cell.html#mindspore.nn.Cell) subclass.
...
class IBertClassificationHead(ms.nn.Cell):
