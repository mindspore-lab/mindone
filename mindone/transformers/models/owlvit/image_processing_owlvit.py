...
from ...utils import TensorType, filter_out_non_signature_kwargs, is_mindspore_available, logging
...
def _scale_boxes(boxes, target_sizes):
    ...
    Args:
        boxes (`ms.Tensor` of shape `(batch_size, num_boxes, 4)`):
            Bounding boxes to scale. Each box is expected to be in (x1, y1, x2, y2) format.
        target_sizes (`List[Tuple[int, int]]` or `ms.Tensor` of shape `(batch_size, 2)`):
            Target sizes to scale the boxes to. Each target size is expected to be in (height, width) format.

    Returns:
        `ms.Tensor` of shape `(batch_size, num_boxes, 4)`: Scaled bounding boxes.
    """

    if isinstance(target_sizes, (list, tuple)):
        image_height = mindspore.Tensor([i[0] for i in target_sizes])
        image_width = mindspore.Tensor([i[1] for i in target_sizes])
    elif isinstance(target_sizes, mindspore.Tensor):
        image_height, image_width = target_sizes.unbind(1)
    else:
        raise ValueError("`target_sizes` must be a list, tuple or ms.Tensor")

    scale_factor = mindspore.mint.stack([image_width, image_height, image_width, image_height], dim=1)
...
def box_iou(boxes1, boxes2):
    ...
    left_top = mindspore.mint.max(boxes1[:, None, :2], boxes2[:, :2])  # [N,M,2]
    right_bottom = mindspore.mint.min(boxes1[:, None, 2:], boxes2[:, 2:])  # [N,M,2]
...
class OwlViTImageProcessor(BaseImageProcessor):
    ...
    def post_process(self, outputs, target_sizes):
        ...
        Args:
            ...
            target_sizes (`ms.Tensor` of shape `(batch_size, 2)`):
        ...
        probs = mindspore.mint.max(logits, dim=-1)
        scores = mindspore.mint.sigmoid(probs[0])
        labels = probs[1]
    ...
    def post_process_image_guided_detection(self, outputs, threshold=0.0, nms_threshold=0.3, target_sizes=None):
        ...
        probs = mindspore.mint.max(logits, dim=-1)[0]
        scores = mindspore.mint.sigmoid(probs)
        ...
        if nms_threshold < 1.0:
            for idx in range(target_boxes.shape[0]):
                for i in mindspore.mint.argsort(-scores[idx]):
                    if not scores[idx][i]:
                        continue
        ...
        results = []
        alphas = mindspore.mint.zeros_like(scores)
        ...
        for idx in range(target_boxes.shape[0]):
            ...
            max_score = mindspore.mint.max(query_scores) + 1e-6
            query_alphas = (query_scores - (max_score * 0.1)) / (max_score * 0.9)
            query_alphas = mindspore.mint.clamp(query_alphas, 0.0, 1.0)
