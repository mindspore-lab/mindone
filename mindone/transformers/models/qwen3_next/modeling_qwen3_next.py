...
class Qwen3NextRMSNormGated(nn.Cell):
    ...
    def construct(self, hidden_states, gate=None):
        ...
        hidden_states = hidden_states * F.silu(gate.to(ms.float32))
...
class Qwen3NextRotaryEmbedding(nn.Cell):
...
class Qwen3NextRMSNorm(nn.Cell):
...
class Qwen3NextAttention(nn.Cell):
    ...
    def construct(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        query_states, gate = mint.chunk(self.q_proj(hidden_states).view(*input_shape, -1, self.head_dim * 2), 2, dim=-1)
        ...
        attn_output = attn_output * mint.nn.functional.sigmoid(gate)
...
def torch_causal_conv1d_update(
    ...
    hidden_states_new = mint.cat([conv_state, hidden_states], dim=-1).to(weight.dtype)
...
def l2norm(x: ms.Tensor, dim: int = -1, eps: float = 1e-6):
    """This function is intended to align with the l2norm implementation in the FLA library."""
    inv_norm = mint.rsqrt((x * x).sum(dim=dim, keepdim=True) + eps)
    return x * inv_norm
...
def torch_chunk_gated_delta_rule(
    ...
    query, key, value, beta, g = [x.transpose(1, 2).contiguous().to(ms.float32) for x in (query, key, value, beta, g)]
    ...
    mask = mint.triu(mint.ones((chunk_size, chunk_size), dtype=ms.bool_), diagonal=0)
    ...
    attn = attn + mint.eye(chunk_size, dtype=attn.dtype)
    value = attn @ v_beta
    k_cumdecay = attn @ (k_beta * g.exp().unsqueeze(-1))
    last_recurrent_state = (
        mint.zeros((batch_size, num_heads, k_head_dim, v_head_dim)).to(value)
        if initial_state is None
        else initial_state.to(value)
    )
    core_attn_out = mint.zeros_like(value)
    mask = mint.triu(mint.ones((chunk_size, chunk_size), dtype=ms.bool_), diagonal=1)
...
def torch_recurrent_gated_delta_rule(
    ...
    core_attn_out = mint.zeros((batch_size, num_heads, sequence_length, v_head_dim)).to(value)
    last_recurrent_state = (
        mint.zeros((batch_size, num_heads, k_head_dim, v_head_dim)).to(value)
        if initial_state is None
        else initial_state.to(value)
...
class Qwen3NextGatedDeltaNet(nn.Cell):
    def __init__(self, config: Qwen3NextConfig, layer_idx: int):
        super().__init__()
        ...
        self.in_proj_qkvz = mint.nn.Linear(self.hidden_size, projection_size_qkvz, bias=False)
        self.in_proj_ba = mint.nn.Linear(self.hidden_size, projection_size_ba, bias=False)
        ...
        self.norm = (
            ...
            if FusedRMSNormGated is None
                ...
                dtype=config.dtype if config.dtype is not None else ms.float32,
            )
        )

        self.out_proj = mint.nn.Linear(self.value_dim, self.hidden_size, bias=False)
    ...
    def fix_query_key_value_ordering(self, mixed_qkvz, mixed_ba):
        ...
        query, key, value, z = mint.split(mixed_qkvz, split_arg_list_qkvz, dim=3)
        b, a = mint.split(mixed_ba, split_arg_list_ba, dim=3)
    ...
    def construct(
        ...
        mixed_qkv = mint.cat((query, key, value), dim=-1)
...
class Qwen3NextModel(Qwen3NextPreTrainedModel):
    def __init__(self, config: Qwen3NextConfig):
        ...
        self.embed_tokens = mint.nn.Embedding(config.vocab_size, config.hidden_size, config.pad_token_id)
