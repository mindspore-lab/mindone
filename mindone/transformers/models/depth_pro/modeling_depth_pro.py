...
def split_to_patches(pixel_values: ms.Tensor, patch_size: int, overlap_ratio: float) -> ms.Tensor:
    ...
    stride = mindspore_int(patch_size * (1 - overlap_ratio))
...
def reshape_features(hidden_states: ms.Tensor) -> ms.Tensor:
    """Discard class token and reshape 1D feature map to a 2D grid."""
    n_samples, seq_len, hidden_size = hidden_states.shape
    size = mindspore_int(seq_len**0.5)
...
def merge_patches(patches: ms.Tensor, batch_size: int, padding: int) -> ms.Tensor:
    ...
    sqrt_n_patches_per_batch = mindspore_int(n_patches_per_batch**0.5)
    ...
    if padding == 0:
        ...
        for h in range(sqrt_n_patches_per_batch):
            ...
            for w in range(sqrt_n_patches_per_batch):
                ...
                i += 1
            boxes_in_row = mint.cat(boxes_in_row, dim=-1)
            boxes.append(boxes_in_row)
        merged = mint.cat(boxes, dim=-2)
...
def reconstruct_feature_maps(
    ...
    Args:
        hidden_state (ms.Tensor): Input tensor of shape `(n_patches_per_batch * batch_size, seq_len, hidden_size)`
...
class DepthProPatchEncoder(nn.Cell):
    ...
    def construct(
        ...
        for ratio in self.scaled_images_ratios:
            size = (int(pixel_values.shape[-2] * ratio), int(pixel_values.shape[-1] * ratio))
        ...
        patches = mint.cat(scaled_images[::-1], dim=0)  # -1 as patch encoder expects high res patches first
        ...
        scaled_images_last_hidden_state = mint.split(encodings[0], n_patches_per_scaled_image[::-1])
        ...
        exponent_value = mindspore_int(math.log2(width / self.out_size))
        ...
        for i in range(self.n_intermediate_hooks):
            ...
            padding = mindspore_int(self.merge_padding_value * (1 / self.scaled_images_ratios[-1]))
...
class DepthProImageEncoder(nn.Cell):
    def __init__(self, config: DepthProConfig):
        super().__init__()
        self.config = config
        self.out_size = config.image_model_config.image_size // config.image_model_config.patch_size
...
class DepthProFeatureUpsampleBlock(nn.Cell):
    def __init__(
        self,
...
class DepthProFeatureUpsample(nn.Cell):
    def __init__(self, config: DepthProConfig):
        super().__init__()
        self.config = config
        ...
        self.scaled_images = nn.CellList()
...
class DepthProNeck(nn.Cell):
    def __init__(self, config: DepthProConfig):
        super().__init__()
        self.config = config

        self.feature_upsample = DepthProFeatureUpsample(config)
        self.fuse_image_with_low_res = mint.nn.Conv2d(
...
class DepthProPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, (mint.nn.Conv2d, mint.nn.ConvTranspose2d)):
            module.weight.set_data(
                initializer(
                    HeNormal(mode="fan_out", nonlinearity="relu"), shape=module.weight.shape, dtype=module.weight.dtype
                )
            )
            if module.bias is not None:
                module.bias.data.zero_()
...
class DepthProPreActResidualLayer(nn.Cell):
    """
...
class DepthProFeatureFusionLayer(nn.Cell):
    def __init__(self, config: DepthProConfig, use_deconv: bool = True):
        ...
        if self.use_deconv:
            self.deconv = mint.nn.ConvTranspose2d(
    ...
    def construct(self, hidden_state: ms.Tensor, residual: Optional[ms.Tensor] = None) -> ms.Tensor:
...
class DepthProFeatureFusionStage(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class DepthProFovEncoder(nn.Cell):
    def __init__(self, config: DepthProConfig):
        super().__init__()
        self.config = config
        self.out_size = config.image_model_config.image_size // config.image_model_config.patch_size
        ...
        self.neck = mint.nn.Linear(config.fov_model_config.hidden_size, config.fusion_hidden_size // 2)
...
class DepthProFovHead(nn.Cell):
    def __init__(self, config: DepthProConfig):
        ...
        final_kernel_size = mindspore_int((self.out_size - 1) / 2**config.num_fov_head_layers + 1)
        self.layers.append(
            mint.nn.Conv2d(
...
class DepthProFovModel(nn.Cell):
    def __init__(self, config: DepthProConfig):
        super().__init__()
        self.config = config
        self.fusion_hidden_size = config.fusion_hidden_size
...
class DepthProDepthEstimationHead(nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        self.layers = nn.CellList(
            [
                mint.nn.Conv2d(features, features // 2, kernel_size=3, stride=1, padding=1),
                ...
                mint.nn.Conv2d(features // 2, 32, kernel_size=3, stride=1, padding=1),
                mint.nn.ReLU(),
                mint.nn.Conv2d(32, 1, kernel_size=1, stride=1, padding=0),
                mint.nn.ReLU(),
            ]
        )
    ...
    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        ...
        predicted_depth = hidden_states.squeeze(axis=1)
