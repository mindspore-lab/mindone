...
class BertGenerationDecoder(BertGenerationPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs,
        ...
        >>> from mindone.transformers import BertGenerationDecoder
        >>> import mindspore as ms

        >>> tokenizer = AutoTokenizer.from_pretrained("google/bert_for_seq_generation_L-24_bbc_encoder", revision="refs/pr/1")
        >>> config = BertGenerationConfig.from_pretrained("google/bert_for_seq_generation_L-24_bbc_encoder", revision="refs/pr/1")
        >>> config.is_decoder = True
        >>> model = BertGenerationDecoder.from_pretrained(
        ...     "google/bert_for_seq_generation_L-24_bbc_encoder", config=config, revision="refs/pr/1"
        ... )

        >>> inputs = tokenizer("Hello, my dog is cute", return_token_type_ids=False, return_tensors="np")
