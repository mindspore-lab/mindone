...
def gelu(x):
    ...
    """
    return x * 0.5 * (1.0 + mindspore.mint.erf(x / math.sqrt(2.0)))
...
class RotaryEmbedding(mindspore.nn.Cell):
    ...
    def __init__(self, dim: int):
        ...
        inv_freq = 1.0 / (10000 ** (mindspore.mint.arange(0, dim, 2, dtype=mindspore.int64).float() / dim))
    ...
    def _update_cos_sin_tables(self, x, seq_dimension=2):
        ...
        if seq_len != self._seq_len_cached:
            ...
            t = mindspore.mint.arange(
                x.shape[seq_dimension],
            ).type_as(self.inv_freq)
            freqs = mindspore.mint.outer(t, self.inv_freq)
...
class EsmContactPredictionHead(mindspore.nn.Cell):
    ...
    def __init__(
        ...
        self.regression = mindspore.mint.nn.Linear(in_features, 1, bias)
        self.activation = mindspore.mint.nn.Sigmoid()
...
class EsmEmbeddings(mindspore.nn.Cell):
    """
...
def eager_attention_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    if hasattr(module, "position_embedding_type") and module.position_embedding_type in [
        ...
        position_ids_l = mint.arange(seq_length, dtype=mindspore.int64).view(-1, 1)
        position_ids_r = mint.arange(seq_length, dtype=mindspore.int64).view(1, -1)
        ...
        if module.position_embedding_type == "relative_key":
            relative_position_scores = mint.einsum("bhld,lrd->bhlr", query, positional_embedding)
        elif module.position_embedding_type == "relative_key_query":
            relative_position_scores_query = mint.einsum("bhld,lrd->bhlr", query, positional_embedding)
            relative_position_scores_key = mint.einsum("bhrd,lrd->bhlr", key, positional_embedding)
    ...
    attn_weights = mint.functional.softmax(attn_weights, dim=-1, dtype=mindspore.float32).to(query.dtype)
...
class EsmSelfAttention(mindspore.nn.Cell):
...
class EsmSelfOutput(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class EsmAttention(mindspore.nn.Cell):
...
class EsmIntermediate(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class EsmOutput(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class EsmLayer(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class EsmEncoder(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = mindspore.nn.CellList([EsmLayer(config) for _ in range(config.num_hidden_layers)])
        self.emb_layer_norm_after = mindspore.mint.nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class EsmPooler(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.dense = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
...
class EsmModel(EsmPreTrainedModel):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.config._attn_implementation != "flash_attention_2":
            ...
            attention_mask: mindspore.Tensor = self.get_extended_attention_mask(
    ...
    def predict_contacts(self, tokens, attention_mask):
        ...
        attns = mindspore.mint.stack(attns, dim=1)  # Matches the original model layout
...
class EsmLMHead(mindspore.nn.Cell):
...
class EsmClassificationHead(mindspore.nn.Cell):
