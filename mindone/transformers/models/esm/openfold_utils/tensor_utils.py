...
def masked_mean(mask: mindspore.Tensor, value: mindspore.Tensor, dim: int, eps: float = 1e-4) -> mindspore.Tensor:
    ...
    return mindspore.mint.sum(mask * value, dim=dim) / (eps + mindspore.mint.sum(mask, dim=dim))


def pts_to_distogram(pts: mindspore.Tensor, min_bin=2.3125, max_bin=21.6875, no_bins: int = 64) -> mindspore.Tensor:
    boundaries = mindspore.mint.linspace(
        min_bin,
        max_bin,
        no_bins - 1,
    )
    dists = mindspore.mint.sqrt(mindspore.mint.sum((pts.unsqueeze(-2) - pts.unsqueeze(-3)) ** 2, dim=-1))
    return mindspore.ops.bucketize(dists, boundaries)
...
def one_hot(x: mindspore.Tensor, v_bins: mindspore.Tensor) -> mindspore.Tensor:
    ...
    am = mindspore.mint.argmin(mindspore.mint.abs(diffs), dim=-1)
    return mindspore.mint.nn.functional.one_hot(am, num_classes=len(v_bins)).float()


def batched_gather(
    data: mindspore.Tensor, inds: mindspore.Tensor, dim: int = 0, no_batch_dims: int = 0
) -> mindspore.Tensor:
    ranges: List[Union[slice, mindspore.Tensor]] = []
    for i, s in enumerate(data.shape[:no_batch_dims]):
        r = mindspore.mint.arange(s)
