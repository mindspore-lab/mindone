...
@overload
def pseudo_beta_fn(
    aatype: mindspore.Tensor, all_atom_positions: mindspore.Tensor, all_atom_masks: None
) -> mindspore.Tensor:
    ...
...
def pseudo_beta_fn(aatype, all_atom_positions, all_atom_masks):
    ...
    pseudo_beta = mindspore.mint.where(
    ...
    if all_atom_masks is not None:
        pseudo_beta_mask = mindspore.mint.where(
            is_gly,
...
def build_template_angle_feat(template_feats: Dict[str, mindspore.Tensor]) -> mindspore.Tensor:
    ...
    template_angle_feat = mindspore.mint.cat(
        [
            mindspore.mint.nn.functional.one_hot(template_aatype, 22),
...
def build_template_pair_feat(
    ...
    dgram = mindspore.mint.sum((tpb[..., None, :] - tpb[..., None, :, :]) ** 2, dim=-1, keepdim=True)
    lower = (
    ...
    upper = mindspore.mint.cat([lower[1:], mindspore.tensor([inf], dtype=lower.dtype)], dim=-1)
    dgram = ((dgram > lower) * (dgram < upper)).type(dgram.dtype)

    to_concat = [dgram, template_mask_2d[..., None]]

    aatype_one_hot: mindspore.Tensor = mindspore.mint.nn.functional.one_hot(
    ...
    inv_distance_scalar = mindspore.mint.rsqrt(eps + mindspore.mint.sum(rigid_vec**2, dim=-1))
    ...
    to_concat.extend(mindspore.mint.unbind(unit_vector[..., None, :], dim=-1))
    to_concat.append(template_mask_2d[..., None])

    act = mindspore.mint.cat(to_concat, dim=-1)
...
def build_extra_msa_feat(batch: Dict[str, mindspore.Tensor]) -> mindspore.Tensor:
    msa_1hot: mindspore.Tensor = mindspore.mint.nn.functional.one_hot(batch["extra_msa"], 23)
    msa_feat = [
        msa_1hot,
        batch["extra_has_deletion"].unsqueeze(-1),
        batch["extra_deletion_value"].unsqueeze(-1),
    ]
    return mindspore.mint.cat(msa_feat, dim=-1)
...
def torsion_angles_to_frames(
    ...
    # [*, N, 8, 2]
    alpha = mindspore.mint.cat([bb_rot.expand(*alpha.shape[:-2], -1, -1), alpha], dim=-2)
...
def frames_and_literature_positions_to_atom14_pos(
    ...
    # [*, N, 14, 8]
    group_mask_one_hot: mindspore.Tensor = mindspore.mint.nn.functional.one_hot(
        group_mask,
    ...
    # [*, N, 14]
    t_atoms_to_global = t_atoms_to_global.map_tensor_fn(lambda x: mindspore.mint.sum(x, dim=-1))
