...
from ....models.utils import constant_, kaiming_normal_, normal_, trunc_normal_, zeros_
...
class XcodecOutput(ModelOutput):
    ...
    Args:
        ...
        audio_values (`ms.Tensor` of shape `(batch_size, channels, num_samples)`, *optional*)
...
class ResidualUnit(nn.Cell):
    ...
    def __init__(self, config: XcodecConfig, in_channels: int, out_channels: int, dilation: int):
        ...
        self.conv2 = mint.nn.Conv1d(in_channels=out_channels, out_channels=out_channels, kernel_size=1, bias=False)
...
class SemanticEncoderBlock(nn.Cell):
    def __init__(self, config: XcodecConfig, in_channels: int, out_channels: int, stride: int):
        ...
        self.conv = mint.nn.Conv1d(
            in_channels, out_channels, kernel_size=kernel, stride=stride, padding=padding, bias=True
        )
...
class SemanticDecoderBlock(nn.Cell):
    def __init__(self, config: XcodecConfig, in_channels: int, out_channels: int, stride: int):
        super().__init__()
        if stride == 1:
            self.conv = mint.nn.Conv1d(
            ...
            self.conv = ConvTranspose1d(
                in_channels, out_channels, kernel_size, stride, padding, output_padding, has_bias=False
            )
...
class XcodecEuclideanCodebook(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.cluster_size = Parameter(mint.zeros(config.codebook_size), name="cluster_size")
        self.embed = Parameter(embed, name="embed")
    ...
    def decode(self, embed_ind):
        quantized = mint.nn.functional.embedding(embed_ind, self.embed)
        return quantized


class XcodecVectorQuantization(nn.Cell):
    """
...
class XcodecResidualVectorQuantization(nn.Cell):
    """
    ...
    def __init__(self, config: XcodecConfig):
        super().__init__()
        self.quantizers = nn.CellList([XcodecVectorQuantization(config) for _ in range(config.num_quantizers)])
    ...
    def encode(self, embeddings: Tensor, bandwidth=None) -> Tensor:
        """
...
class XcodecPreTrainedModel(PreTrainedAudioTokenizerBase):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, mint.nn.Linear):
            ...
            for submodule in module.acoustic_encoder.cells():
                if isinstance(submodule, mint.nn.Conv1d):
                    trunc_normal_(submodule.weight, std=0.02)
                    constant_(submodule.bias, 0)
            for submodule in module.acoustic_decoder.cells():
                if isinstance(submodule, mint.nn.Conv1d):
                    trunc_normal_(submodule.weight, std=0.02)
                    constant_(submodule.bias, 0)
    ...
    def remove_weight_norm(self):
        ...
        for module in (self.acoustic_encoder, self.acoustic_decoder):
            for m in module.cells():
                try:
                    WeightNorm.remove_weight_norm(m, name="weight")
...
class XcodecModel(XcodecPreTrainedModel):
    def __init__(self, config):
        ...
        self.semantic_model.set_train(False)
        self.fc = mint.nn.Linear(config.hidden_size, config.hidden_size)
        self.fc1 = mint.nn.Linear(config.hidden_size, config.semantic_model_config.hidden_size)
        self.fc2 = mint.nn.Linear(config.hidden_size, config.acoustic_model_config.hidden_size)
    ...
    @staticmethod
    def _adjust_dac_decoder(decoder: nn.Cell):
        r"""
        ...
        """
        for _, module in decoder.cells_and_names():
            if isinstance(module, ConvTranspose1d):
                stride = module.stride[1] if isinstance(module.stride, tuple) else module.stride
                module.output_padding = (0, stride % 2)
        if hasattr(decoder, "tanh") and isinstance(decoder.tanh, mint.nn.Tanh):
            decoder.tanh = mint.nn.Identity()

    def _extract_semantic_features(self, input_values: Tensor) -> Tensor:
        input_values = input_values[:, 0, :]
        input_values = mint.nn.functional.pad(input_values, (self.pad, self.pad))
        with mindspore._no_grad():
            outputs = self.semantic_model(input_values, output_hidden_states=True)
            hidden_states = outputs.hidden_states

        stacked = mint.stack(hidden_states, dim=1)
    ...
    def encode(
    ...
    ) -> Union[Tensor, XcodecEncoderOutput]:
        r"""
        input_values (`ms.Tensor` of shape `(batch_size, channels, num_samples)`):
        ...
        Returns:
            `ms.Tensor` of shape `(batch_size, num_quantizers, codes_length)` containing the discrete encoded audio codes.
        """
        ...
        if e_acoustic.shape[2] != e_semantic.shape[2]:
            ...
            e_acoustic = self.acoustic_encoder(
                mint.nn.functional.pad(input_values[:, 0, :], (self.pad, self.pad)).unsqueeze(1)
            )

        embeddings = mint.cat([e_acoustic, e_semantic], dim=1)
    ...
    def decode(
        ...
        audio_codes: Tensor,
        return_dict: Optional[bool] = None,
    ) -> Union[Tensor, XcodecDecoderOutput]:
        r"""
        audio_codes (`ms.Tensor`  of shape `(batch_size, num_quantizers, codes_length)`):
            Discrete code indices computed using `model.encode`.
        return_dict (`bool`, *optional*):
            Whether or not to return a [`~utils.ModelOutput`]
    ...
    def construct(
        ...
        bandwidth: Optional[float] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[tuple[Tensor, Tensor], XcodecOutput]:
        r"""
        input_values (`ms.Tensor` of shape `(batch_size, channels, num_samples)`):
