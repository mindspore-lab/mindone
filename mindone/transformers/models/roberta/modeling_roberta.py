...
class RobertaEmbeddings(nn.Cell):
    ...
    def __init__(self, config):
        super().__init__()
        self.word_embeddings = mint.nn.Embedding(config.vocab_size, config.hidden_size, config.pad_token_id)
...
class RobertaSelfAttention(nn.Cell):
    ...
    def construct(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in RobertaModel construct() function)
