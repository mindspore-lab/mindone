...
class Ernie4_5_MoeRMSNorm(nn.Cell):
...
class Ernie4_5_MoeMLP(nn.Cell):
    def __init__(self, config, intermediate_size=None):
        ...
        self.gate_proj = mint.nn.Linear(self.hidden_size, self.intermediate_size, bias=config.use_bias)
        self.up_proj = mint.nn.Linear(self.hidden_size, self.intermediate_size, bias=config.use_bias)
        self.down_proj = mint.nn.Linear(self.intermediate_size, self.hidden_size, bias=config.use_bias)
...
class Ernie4_5_MoeRotaryEmbedding(nn.Cell):
...
class Ernie4_5_MoeAttention(nn.Cell):
...
class Ernie4_5_MoeStatics(nn.Cell):
    """
    ...
    def __init__(self, config):
        ...
        self.e_score_correction_bias = Parameter(
            mint.zeros((num_experts_groups, num_experts), dtype=mindspore.float32),
            requires_grad=False,
        )
    ...
    def construct(self, hidden_states):
        ...
        #   1. All other tensors are local (`ms.Tensor`)
...
class Ernie4_5_MoeForCausalLM(Ernie4_5_MoePreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.vocab_size = config.vocab_size
        self.lm_head = mint.nn.Linear(config.hidden_size, config.vocab_size, bias=config.use_bias)
