...
def repeat_kv(hidden_states: ms.Tensor, n_rep: int) -> ms.Tensor:
    """
    This is the equivalent of mint.nn.functional.repeat_interleave(x, dim=1, repeats=n_rep). The hidden states go from (batch,
...
class Qwen3OmniMoeVisionAttention(nn.Cell):
...
class Qwen3OmniMoeVisionPatchMerger(nn.Cell):
    def __init__(self, config: Qwen3OmniMoeVisionEncoderConfig, use_postshuffle_norm=False) -> None:
        super().__init__()
        self.hidden_size = config.hidden_size * (config.spatial_merge_size**2)
        self.use_postshuffle_norm = use_postshuffle_norm
        self.ln_q = mint.nn.LayerNorm(self.hidden_size if use_postshuffle_norm else config.hidden_size, eps=1e-6)
        self.mlp = nn.CellList(
            [
                mint.nn.Linear(self.hidden_size, self.hidden_size),
                mint.nn.GELU(),
                mint.nn.Linear(self.hidden_size, config.out_hidden_size),
            ]
        )
...
class Qwen3OmniMoeVisionMLP(nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class Qwen3OmniMoeVisionPatchEmbed(nn.Cell):
...
class Qwen3OmniMoeVisionRotaryEmbedding(nn.Cell):
...
class Qwen3OmniMoeVisionEncoder(Qwen3OmniMoePreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.blocks = nn.CellList([Qwen3OmniMoeVisionBlock(config) for _ in range(config.depth)])
...
class Qwen3OmniMoeThinkerTextRotaryEmbedding(nn.Cell):
    inv_freq: ms.Tensor  # fix linting for `register_buffer`
...
class Qwen3OmniMoeThinkerTextMLP(nn.Cell):
...
class Qwen3OmniMoeThinkerTextAttention(nn.Cell):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
...
class Qwen3OmniMoeThinkerForConditionalGeneration(Qwen3OmniMoePreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def get_audio_features(
        ...
        Args:
            ...
            feature_attention_mask (`ms.Tensor`, *optional*):
    ...
    def construct(
        ...
        **kwargs,
        ...
        >>> from mindone.transformers import Qwen3OmniMoeProcessor, Qwen3OmniMoeThinkerForConditionalGeneration
        ...
        >>> generate_ids = generation[:, inputs.input_ids.shape[1]:]
...
class Qwen3OmniMoeTalkerResizeMLP(nn.Cell):
...
class Qwen3OmniMoeRMSNorm(nn.Cell):
...
class Qwen3OmniMoeTalkerCodePredictorAttention(nn.Cell):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
...
class Qwen3OmniMoeRotaryEmbedding(nn.Cell):
    inv_freq: ms.Tensor  # fix linting for `register_buffer`
...
class Qwen3OmniMoeTalkerCodePredictorModel(Qwen3OmniMoePreTrainedModel):
    ...
    def __init__(self, config: Qwen3OmniMoeTalkerCodePredictorConfig):
        ...
        self.codec_embedding = nn.CellList(
            [mint.nn.Embedding(config.vocab_size, config.hidden_size) for _ in range(config.num_code_groups - 1)]
        )
...
class Qwen3OmniMoeTalkerTextMLP(nn.Cell):
    def __init__(self, config, intermediate_size=None):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
...
class Qwen3OmniMoeTalkerTextSparseMoeBlock(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.num_experts = config.num_experts
        self.top_k = config.num_experts_per_tok
...
class Qwen3OmniMoeTalkerModel(Qwen3OmniMoePreTrainedModel):
    ...
    def __init__(self, config: Qwen3OmniMoeTalkerTextConfig):
        ...
        self.gradient_checkpointing = False
        self.codec_embedding = mint.nn.Embedding(config.vocab_size, config.hidden_size)
...
class Qwen3OmniMoeTalkerForConditionalGeneration(Qwen3OmniMoeThinkerTextPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Qwen3OmniMoeTalkerConfig):
        ...
        self.codec_head = mint.nn.Linear(config.text_config.hidden_size, config.text_config.vocab_size, bias=False)
    ...
    def prepare_inputs_for_generation(
        ...
        if cache_position[0] != 0:
            ...
            past_hidden = hidden_states[0][-1][:, -1:]  # hidden, last layer, last token
...
class Qwen3OmniMoeCausalConvNet(nn.Cell):
    def __init__(
        self,
        in_channels,
        out_channels,
    ...
    def _get_extra_padding_for_conv1d(self, hidden_state: ms.Tensor) -> int:
...
class Qwen3OmniMoeCausalTransConvNet(nn.Cell):
...
class Qwen3OmniMoeConvNeXtBlock(nn.Cell):
    def __init__(self, dim: int):
        ...
        self.gamma = Parameter(1e-6 * mint.ones(dim))
...
class Qwen3OmniMoeCode2WavRotatoryEmbedding(nn.Cell):
    inv_freq: ms.Tensor  # fix linting for `register_buffer`
...
class Qwen3OmniMoeCode2WavAttention(nn.Cell):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
...
class Qwen3OmniMoeCode2WavMlp(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
...
class Qwen3OmniMoeCode2WavRMSNorm(nn.Cell):
    def __init__(self, hidden_size, eps: float = 1e-6) -> None:
        """
...
class Qwen3OmniMoeCode2WavLayerScale(nn.Cell):
        ...
        **kwargs,
        ...
        Args:
            ...
            use_cache (`bool`, *optional*):
                If set to `True`, `past_key_values` key value states are returned and can be used to speed up decoding
                (see `past_key_values`).
            past_key_values (`Tuple(ms.Tensor)`, *optional*): cached past key and value projection states
            cache_position (`ms.Tensor` of shape `(sequence_length)`, *optional*):
                Indices depicting the position of the input sequence tokens in the sequence
            kwargs (`dict`, *optional*):
                Arbitrary kwargs to be ignored, used for FSDP and other methods that injects code
...
class Qwen3OmniMoeCode2WavDecoderResidualUnit(nn.Cell):
...
class Qwen3OmniMoeCode2Wav(Qwen3OmniMoePreTrainedModel):
    def __init__(self, config: Qwen3OmniMoeCode2WavConfig):
        ...
        self.register_buffer(
            "code_offset", mint.arange(config.num_quantizers).view(1, -1, 1) * config.codebook_size, persistent=False
        )
        ...
        self.upsample = nn.CellList(upsample)
        ...
        self.decoder = nn.CellList(decoder)

        self.post_init()

    def construct(self, codes):
...
class Qwen3OmniMoeForConditionalGeneration(Qwen3OmniMoePreTrainedModel, GenerationMixin):
    ...
    def _get_talker_assistant_parts(
        ...
        assistant_hidden = self.talker.text_projection(thinker_embed[:, im_start_index:segment_end_index])  # [1 t d]
        assistant_text_hidden = mint.cat(
            (
                assistant_hidden[:, :3],
        ...
        codec_special_tokens = ms.tensor(
            [
                [
        ...
        assistant_codec_hidden = mint.cat(
            (
                mint.zeros(
    ...
    def generate(
        ...
        **kwargs,
        ...
        thinker_embed = mint.cat([hidden_states[0] for hidden_states in thinker_result.hidden_states], dim=1)
        thinker_hidden = mint.cat(
            [
