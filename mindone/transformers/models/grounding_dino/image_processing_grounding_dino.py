...
from ...utils import ExplicitEnum, TensorType, is_scipy_available, is_vision_available, logging
...
def masks_to_boxes(masks: np.ndarray) -> np.ndarray:
    ...
    # see https://github.com/mindspore/mindspore/issues/50276
...
def compute_segments(
    ...
    if target_size is not None:
        mask_probs = mint.mint.nn.functional.interpolate(
...
def _scale_boxes(boxes, target_sizes):
    ...
    if isinstance(target_sizes, (list, tuple)):
        image_height = ms.tensor([i[0] for i in target_sizes])
        image_width = ms.tensor([i[1] for i in target_sizes])
    elif isinstance(target_sizes, ms.Tensor):
        image_height, image_width = target_sizes.unbind(1)
    else:
        raise ValueError("`target_sizes` must be a list, tuple or ms.Tensor")

    scale_factor = mint.stack([image_width, image_height, image_width, image_height], dim=1)
        ...
        Args:
            ...
            return_tensors (`str` or `TensorType`, *optional*):
                The type of tensors to return. Can be one of:
                    ...
                    - `TensorType.MINDSPORE` or `'tf'`: Return a batch of type `mindspore.Tensor`.
