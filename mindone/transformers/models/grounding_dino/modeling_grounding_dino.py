...
def ms_special_logit(x, eps=None):
    z = mint.clamp(x, min=eps, max=1 - eps)
    return mint.log(mint.div(z, 1 - z))
...
def uniform_(
    ...
    data = ops.uniform(tensor.shape, a, b, dtype=tensor.dtype)
    tensor.set_data(data)


def xavier_uniform_(tensor: ms.Parameter, gain=1.0):
...
class GroundingDinoDecoderOutput(ModelOutput):
    ...
    Args:
        ...
        attentions (`tuple(tuple(ms.Tensor))`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
...
class GroundingDinoEncoderOutput(ModelOutput):
    ...
    Args:
        last_hidden_state_vision (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
            Sequence of hidden-states at the output of the last layer of the vision encoder.
        last_hidden_state_text (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
        ...
        text_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
    ...
    last_hidden_state_vision: Optional[ms.Tensor] = None
    last_hidden_state_text: Optional[ms.Tensor] = None
    vision_hidden_states: Optional[Tuple[ms.Tensor]] = None
    text_hidden_states: Optional[Tuple[ms.Tensor]] = None
    attentions: Optional[Tuple[Tuple[ms.Tensor]]] = None
...
class GroundingDinoModelOutput(ModelOutput):
    ...
    Args:
        ...
        decoder_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            ...
            plus the initial embedding outputs.
        decoder_attentions (`tuple(tuple(ms.Tensor))`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of tuples of `ms.Tensor` (one for attention for each layer) of shape `(batch_size, num_heads,
            sequence_length, sequence_length)`. Attentions weights after the attention softmax, used to compute the
            weighted average in the self-attention, cross-attention and multi-scale deformable attention heads.
        encoder_last_hidden_state_vision (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
            Sequence of hidden-states at the output of the last layer of the encoder of the model.
        encoder_last_hidden_state_text (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
            Sequence of hidden-states at the output of the last layer of the encoder of the model.
        encoder_vision_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when \
    ...
    encoder_last_hidden_state_vision: Optional[ms.Tensor] = None
    encoder_last_hidden_state_text: Optional[ms.Tensor] = None
    encoder_vision_hidden_states: Optional[Tuple[ms.Tensor]] = None
    encoder_text_hidden_states: Optional[Tuple[ms.Tensor]] = None
    ...
    encoder_logits: Optional[ms.Tensor] = None
    encoder_pred_boxes: Optional[ms.Tensor] = None
...
class GroundingDinoObjectDetectionOutput(ModelOutput):
    ...
    Args:
        ...
        encoder_text_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or\
            ...
            of shape `(batch_size, sequence_length, hidden_size)`. Hidden-states of the text encoder at the output of
            each layer plus the initial embedding outputs.
        encoder_attentions (`tuple(tuple(ms.Tensor))`, *optional*, returned when `output_attentions=True` is passed or\
        ...
        enc_outputs_coord_logits (`ms.Tensor` of shape `(batch_size, sequence_length, 4)`, *optional*, returned when \
            ...
            Logits of predicted bounding boxes coordinates in the first stage.
        encoder_logits (`ms.Tensor` of shape `(batch_size, sequence_length, config.num_labels)`, *optional*, returned \
            when `config.two_stage=True`):
            Logits of top `config.num_queries` scoring bounding boxes in the first stage.
...
class GroundingDinoFrozenBatchNorm2d(nn.Cell):
    """
...
def replace_batch_norm(model):
    r"""
    Recursively replace all `mint.mint.nn.BatchNorm2d` with `GroundingDinoFrozenBatchNorm2d`.
...
class GroundingDinoConvEncoder(nn.Cell):
    """
    Convolutional backbone, using either the AutoBackbone API or one from the timm library.

    mint.nn.BatchNorm2d layers are replaced by GroundingDinoFrozenBatchNorm2d as defined above.
...
class GroundingDinoConvModel(nn.Cell):
    """
...
class GroundingDinoSinePositionEmbedding(nn.Cell):
    """
...
class GroundingDinoLearnedPositionEmbedding(nn.Cell):
    """
...
# Copied from transformers.models.deformable_detr.modeling_deformable_detr.DeformableDetrMultiscaleDeformableAttention with DeformableDetr->\
# GroundingDino, Deformable DETR->Grounding DINO
class GroundingDinoMultiscaleDeformableAttention(nn.Cell):
    """
...
class GroundingDinoTextEnhancerLayer(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.fc1 = mint.nn.Linear(config.d_model, config.encoder_ffn_dim // 2)
        self.fc2 = mint.nn.Linear(config.encoder_ffn_dim // 2, config.d_model)

        self.layer_norm_before = mint.nn.LayerNorm(config.d_model, config.layer_norm_eps)
        self.layer_norm_after = mint.nn.LayerNorm(config.d_model, config.layer_norm_eps)
    ...
    def construct(
        self,
        hidden_states: ms.Tensor,
        attention_masks: Optional[ms.Tensor] = None,
        position_embeddings: Optional[ms.Tensor] = None,
        ...
        Args:
            ...
            attention_masks (`ms.Tensor`, *optional*):
        ...
        Returns:
            `tuple(ms.Tensor)` comprising two elements:
            - **hidden_states** (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_size)`) --
                Output of the text self-attention layer.
            - **attention_weights** (`ms.Tensor` of shape `(batch_size, num_heads, sequence_length,
              sequence_length)`) --
        ...
        attention_output = mint.nn.functional.dropout(attention_output, p=self.dropout, training=self.training)
...
class GroundingDinoBiMultiHeadAttention(nn.Cell):
    def __init__(self, config):
        super().__init__()
        ...
        self.vision_proj = mint.nn.Linear(self.vision_dim, self.embed_dim)
        self.text_proj = mint.nn.Linear(self.text_dim, self.embed_dim)
        self.values_vision_proj = mint.nn.Linear(self.vision_dim, self.embed_dim)
        self.values_text_proj = mint.nn.Linear(self.text_dim, self.embed_dim)

        self.out_vision_proj = mint.nn.Linear(self.embed_dim, self.vision_dim)
        self.out_text_proj = mint.nn.Linear(self.embed_dim, self.text_dim)
    ...
    def construct(
        ...
        text_features: ms.Tensor,
        vision_attention_mask: Optional[ms.Tensor] = None,
        text_attention_mask: Optional[ms.Tensor] = None,
    ) -> Tuple[Tuple[ms.Tensor, ms.Tensor], Tuple[ms.Tensor, ms.Tensor]]:
        """Image-to-text and text-to-image cross-attention

        Args:
            vision_features (`ms.Tensor` of shape `(batch_size, vision_sequence_length, hidden_dim)`):
                Projected flattened image features generated by the vision backbone.
            text_features (`ms.Tensor` of shape `(batch_size, text_sequence_length, hidden_dim)`):
                Projected text features generated by the text encoder.
            vision_attention_mask (`ms.Tensor`, **optional**):
        ...
        Returns:
            `tuple(tuple(ms.Tensor), tuple(ms.Tensor))` where each inner tuple comprises an attention
            output and weights:
            - **vision_attn_output** (`ms.Tensor` of shape `(batch_size, vision_sequence_length, hidden_din)`)
              --
                Output of the image-to-text cross-attention layer.
            - **vision_attn_weights** (`ms.Tensor` of shape `(batch_size, num_heads, vision_sequence_length,
              vision_sequence_length)`) --
                Attention weights of the image-to-text cross-attention layer.
            - **text_attn_output** (`ms.Tensor` of shape `(batch_size, text_sequence_length, hidden_dim)`) --
                Output of the text-to-image cross-attention layer.
            - **text_attn_weights** (`ms.Tensor` of shape `(batch_size, num_heads, text_sequence_length,
        ...
        attn_weights = mint.clamp(attn_weights, min=-50000, max=50000)

        attn_weights_transposed = attn_weights.transpose(1, 2)
        text_attn_weights = attn_weights_transposed - mint.max(attn_weights_transposed, dim=-1, keepdim=True)[0]

        # Do not increase -50000/50000, data type half has quite limited range
        text_attn_weights = mint.clamp(text_attn_weights, min=-50000, max=50000)
...
class GroundingDinoDropPath(nn.Cell):
    """Drop paths (Stochastic Depth) per sample (when applied in main path of residual blocks)."""
...
class GroundingDinoFusionLayer(nn.Cell):
    def __init__(self, config):
        super().__init__()
        drop_path = config.fusion_droppath

        # pre layer norm
        self.layer_norm_vision = mint.nn.LayerNorm(config.d_model, config.layer_norm_eps)
        self.layer_norm_text = mint.nn.LayerNorm(config.d_model, config.layer_norm_eps)
        self.attn = GroundingDinoBiMultiHeadAttention(config)

        # add layer scale for training stability
        self.drop_path = GroundingDinoDropPath(drop_path) if drop_path > 0.0 else mint.nn.Identity()
        init_values = 1e-4
        self.vision_param = ms.Parameter(init_values * mint.ones((config.d_model)), requires_grad=True)
        self.text_param = ms.Parameter(init_values * mint.ones((config.d_model)), requires_grad=True)

    def construct(
        self,
        vision_features: ms.Tensor,
        text_features: ms.Tensor,
        attention_mask_vision: Optional[ms.Tensor] = None,
        attention_mask_text: Optional[ms.Tensor] = None,
    ) -> Tuple[Tuple[ms.Tensor, ms.Tensor], Tuple[ms.Tensor, ms.Tensor]]:
        ...
        Args:
            ...
            text_features (`ms.Tensor` of shape `(batch_size, text_sequence_length, hidden_dim)`):
                Projected text features generated by the text encoder.
            attention_mask_vision (`ms.Tensor`, **optional**):
                Attention mask for image-to-text cross-attention. False for real tokens and True for padding tokens.
            attention_mask_text (`ms.Tensor`, **optional**):
                Attention mask for text-to-image cross-attention. False for real tokens and True for padding tokens.

        Returns:
            `tuple(tuple(ms.Tensor), tuple(ms.Tensor))` where each inner tuple comprises an enhanced
            feature and attention output and weights:
            - **vision_features** (`ms.Tensor` of shape `(batch_size, vision_sequence_length, vision_dim)`) --
                Updated vision features with attention output from image-to-text cross-attention layer.
            - **vision_attn_weights** (`ms.Tensor` of shape `(batch_size, num_heads, vision_sequence_length,
              vision_sequence_length)`) --
                Attention weights of the image-to-text cross-attention layer.
            - **text_features** (`ms.Tensor` of shape `(batch_size, text_sequence_length, text_dim)`) --
...
class GroundingDinoDeformableLayer(nn.Cell):
    def __init__(self, config: GroundingDinoConfig):
        ...
        self.self_attn_layer_norm = mint.nn.LayerNorm(self.embed_dim, config.layer_norm_eps)
        ...
        self.final_layer_norm = mint.nn.LayerNorm(self.embed_dim, config.layer_norm_eps)
...
def get_sine_pos_embed(
    pos_tensor: ms.Tensor, num_pos_feats: int = 128, temperature: int = 10000, exchange_xy: bool = True
) -> Tensor:
    """
    Generate sine position embeddings from a position tensor.

    Args:
        pos_tensor (ms.Tensor):
    ...
    def sine_func(x: ms.Tensor):
        sin_x = x * scale / dim_t
        sin_x = mint.stack((sin_x[..., 0::2].sin(), sin_x[..., 1::2].cos()), dim=3).flatten(2)
        return sin_x
    ...
    position_embeddings = mint.cat(position_embeddings, dim=-1)
    return position_embeddings


class GroundingDinoEncoderLayer(nn.Cell):
    ...
    def get_text_position_embeddings(
        ...
        text_position_embedding: Optional[ms.Tensor],
        text_position_ids: Optional[ms.Tensor],
    ) -> Tensor:
...
class GroundingDinoMultiheadAttention(nn.Cell):
    ...
    def __init__(self, config, num_attention_heads=None):
        ...
        self.out_proj = mint.nn.Linear(config.hidden_size, config.hidden_size)
    ...
    def construct(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in GroundingDinoModel construct() function)
...
class GroundingDinoDecoderLayer(nn.Cell):
    def __init__(self, config: GroundingDinoConfig):
        super().__init__()
        self.embed_dim = config.d_model
        ...
        self.encoder_attn_text_layer_norm = mint.nn.LayerNorm(self.embed_dim, config.layer_norm_eps)
        # cross-attention
        ...
        self.encoder_attn_layer_norm = mint.nn.LayerNorm(self.embed_dim, config.layer_norm_eps)
    ...
    def construct(
        ...
        spatial_shapes_list=None,
        level_start_index=None,
        vision_encoder_hidden_states: Optional[ms.Tensor] = None,
        vision_encoder_attention_mask: Optional[ms.Tensor] = None,
        text_encoder_hidden_states: Optional[ms.Tensor] = None,
        text_encoder_attention_mask: Optional[ms.Tensor] = None,
...
class GroundingDinoContrastiveEmbedding(nn.Cell):
    def __init__(self, config):
        super().__init__()
    ...
    def construct(
        self,
        vision_hidden_state: ms.Tensor,
        text_hidden_state: ms.Tensor,
        text_token_mask: ms.Tensor,
    ) -> ms.Tensor:
        output = vision_hidden_state @ text_hidden_state.transpose(-1, -2)
        output = output.masked_fill(~text_token_mask[:, None, :], float("-inf"))

        # padding to max_text_len
        new_output = mint.full(
            (*output.shape[:-1], self.max_text_len),
            float("-inf"),
        )
...
class GroundingDinoPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, GroundingDinoLearnedPositionEmbedding):
            ...
            constant_(module.sampling_offsets.weight, 0.0)
            default_dtype = ms.float32
            thetas = mint.arange(module.n_heads, dtype=ms.int32).to(default_dtype) * (2.0 * math.pi / module.n_heads)
            ...
            xavier_uniform_(module.vision_proj.weight)
            constant_(module.vision_proj.bias, 0)
            xavier_uniform_(module.text_proj.weight)
            constant_(module.text_proj.bias, 0)
            xavier_uniform_(module.values_vision_proj.weight)
            constant_(module.values_vision_proj.bias, 0)
            xavier_uniform_(module.values_text_proj.weight)
            constant_(module.values_text_proj.bias, 0)
            xavier_uniform_(module.out_vision_proj.weight)
            constant_(module.out_vision_proj.bias, 0)
            ...
            constant_(module.layers[-1].weight, 0)
            constant_(module.layers[-1].bias, 0)
...
class GroundingDinoEncoder(GroundingDinoPreTrainedModel):
    ...
    def __init__(self, config: GroundingDinoConfig):
        ...
        self.dropout = config.dropout
        self.layers = nn.CellList([GroundingDinoEncoderLayer(config) for _ in range(config.encoder_layers)])
    ...
    def get_reference_points(spatial_shapes, valid_ratios):
        ...
        for level, (height, width) in enumerate(spatial_shapes):
            ...
            ref = mint.stack((ref_x, ref_y), -1)
            reference_points_list.append(ref)
        reference_points = mint.cat(reference_points_list, 1)
    ...
    def construct(
        ...
        r"""
        Args:
            vision_features (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
                Flattened feature map (output of the backbone + projection layer) that is passed to the encoder.
            vision_attention_mask (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            vision_position_embedding (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
            ...
            text_features (`ms.Tensor` of shape `(batch_size, text_seq_len, hidden_size)`):
                Flattened text features that are passed to the encoder.
            text_attention_mask (`ms.Tensor` of shape `(batch_size, text_seq_len)`, *optional*):
                ...
                [What are attention masks?](../glossary#attention-mask)
            text_position_embedding (`ms.Tensor` of shape `(batch_size, text_seq_len)`):
                Position embeddings that are added to the queries and keys in each self-attention layer.
            text_self_attention_masks (`ms.Tensor` of shape `(batch_size, text_seq_len, text_seq_len)`):
            ...
            text_position_ids (`ms.Tensor` of shape `(batch_size, num_queries)`):
...
class GroundingDinoDecoder(GroundingDinoPreTrainedModel):
    ...
    def __init__(self, config: GroundingDinoConfig):
        ...
        self.dropout = config.dropout
        self.layer_norm = mint.nn.LayerNorm(config.d_model, config.layer_norm_eps)
        self.layers = nn.CellList([GroundingDinoDecoderLayer(config) for _ in range(config.decoder_layers)])
    ...
    def construct(
        ...
        Args:
            ...
            vision_encoder_hidden_states (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
                Last hidden state from encoder related to vision feature map.
            vision_encoder_attention_mask (`ms.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                Mask to avoid performing attention on padding pixel features. Mask values selected in `[0, 1]`:
                - 1 for pixel features that are real (i.e. **not masked**),
                - 0 for pixel features that are padding (i.e. **masked**).
            text_encoder_hidden_states (`ms.Tensor` of shape `(batch_size, text_seq_len, hidden_size)`):
                Last hidden state from encoder related to text features.
            text_encoder_attention_mask (`ms.Tensor` of shape `(batch_size, text_seq_len)`, *optional*):
                Mask to avoid performing attention on padding text features. Mask values selected in `[0, 1]`:
                - 0 for text features that are real (i.e. **not masked**),
                - 1 for text features that are padding (i.e. **masked**).
            ...
            valid_ratios (`ms.Tensor` of shape `(batch_size, num_feature_levels, 2)`, *optional*):
                Ratio of valid area in each feature level.
            self_attn_mask (`ms.Tensor` of shape `(batch_size, text_seq_len)`):
        ...
        if text_encoder_attention_mask is not None:
            ...
            text_encoder_attention_mask = text_encoder_attention_mask * _DTYPE_2_MIN[dtype]

        for idx, decoder_layer in enumerate(self.layers):
            num_coordinates = reference_points.shape[-1]
            if num_coordinates == 4:
                reference_points_input = (
                    reference_points[:, :, None] * mint.cat([valid_ratios, valid_ratios], -1)[:, None]
                )
            ...
            if self.bbox_embed is not None:
                ...
                num_coordinates = reference_points.shape[-1]
                if num_coordinates == 4:
                    new_reference_points = tmp + ms_special_logit(reference_points, eps=1e-5)
                    new_reference_points = new_reference_points.sigmoid()
                elif num_coordinates == 2:
                    new_reference_points = tmp
                    new_reference_points[..., :2] = tmp[..., :2] + ms_special_logit(reference_points, eps=1e-5)
                    new_reference_points = new_reference_points.sigmoid()
                else:
                    raise ValueError(
...
def generate_masks_with_special_tokens_and_transfer_map(input_ids: ms.Tensor) -> Tuple[Tensor, Tensor]:
    """Generate attention mask between each pair of special tokens and positional ids.
    Args:
        input_ids (`ms.Tensor` of shape `(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary.
    Returns:
        `tuple(ms.Tensor)` comprising attention mask between each special tokens and position_ids:
...
class GroundingDinoModel(GroundingDinoPreTrainedModel):
    def __init__(self, config: GroundingDinoConfig):
        ...
        if config.num_feature_levels > 1:
            ...
            for i in range(num_backbone_outs):
                ...
                input_proj_list.append(
                    nn.SequentialCell(
                        mint.nn.Conv2d(in_channels, config.d_model, kernel_size=1),
                        mint.nn.GroupNorm(32, config.d_model),
                    )
                )
            for _ in range(config.num_feature_levels - num_backbone_outs):
                input_proj_list.append(
                    nn.SequentialCell(
                        mint.nn.Conv2d(in_channels, config.d_model, kernel_size=3, stride=2, padding=1),
                        mint.nn.GroupNorm(32, config.d_model),
                    )
                )
                in_channels = config.d_model
            self.input_proj_vision = nn.CellList(input_proj_list)
        else:
            self.input_proj_vision = nn.CellList(
                [
                    nn.SequentialCell(
                        mint.nn.Conv2d(backbone.intermediate_channel_sizes[-1], config.d_model, kernel_size=1),
                        mint.nn.GroupNorm(32, config.d_model),
                    )
                ]
            )
        ...
        self.text_projection = mint.nn.Linear(config.text_config.hidden_size, config.d_model)
        ...
        if config.two_stage:
            ...
            self.reference_points = mint.nn.Embedding(config.num_queries, 4)
    ...
    def get_valid_ratio(self, mask):
        ...
        valid_height = mint.sum(mask[:, :, 0], 1)
        valid_width = mint.sum(mask[:, 0, :], 1)
        valid_ratio_height = valid_height.float() / height
        valid_ratio_width = valid_width.float() / width
        valid_ratio = mint.stack([valid_ratio_width, valid_ratio_height], -1)
        return valid_ratio
    ...
    def generate_encoder_output_proposals(self, enc_output, padding_mask, spatial_shapes):
        ...
        Args:
            enc_output (`ms.Tensor[batch_size, sequence_length, hidden_size]`): Output of the encoder.
            padding_mask (`ms.Tensor[batch_size, sequence_length]`): Padding mask for `enc_output`.
            spatial_shapes (`ms.Tensor[num_feature_levels, 2]`): Spatial shapes of the feature maps.
        ...
        output_proposals = mint.cat(proposals, 1)
        output_proposals_valid = ((output_proposals > 0.01) & (output_proposals < 0.99)).all(-1, keepdim=True)
        output_proposals = mint.log(output_proposals / (1 - output_proposals))  # inverse sigmoid
    ...
    def construct(
        ...
        if token_type_ids is None:
            token_type_ids = mint.zeros_like(input_ids)
        ...
        if pixel_mask is None:
            pixel_mask = mint.ones(((batch_size, height, width)), dtype=ms.int32)
        ...
        if self.config.num_feature_levels > len(feature_maps):
            ...
            for level in range(_len_sources, self.config.num_feature_levels):
                ...
                mask = mint.nn.functional.interpolate(pixel_mask[None].float(), size=source.shape[-2:]).to(ms.bool_)[0]
...
class GroundingDinoMLPPredictionHead(nn.Cell):
    """
...
def build_label_maps(logits: ms.Tensor, input_ids: ms.Tensor) -> Tuple[ms.Tensor]:
    """
    ...
    delimiter_token_masks = mint.isin(input_ids, delimiter_tokens)
    label_groups = mint.cumsum(delimiter_token_masks, dim=1) * (~delimiter_token_masks).to(ms.int32)
...
def build_text_mask(logits, attention_mask):
    ...
    text_mask = mint.zeros_like(logits, dtype=attention_mask.dtype)
...
class GroundingDinoForObjectDetection(GroundingDinoPreTrainedModel):
    ...
    def __init__(self, config: GroundingDinoConfig):
        ...
        if config.decoder_bbox_embed_share:
            ...
            self.bbox_embed = nn.CellList([shared_head] * config.decoder_layers)
        else:
    ...
    def construct(
        ...
        for level in range(num_levels):
            ...
            reference = ms_special_logit(reference, eps=1e-5)
        ...
        outputs_class = mint.stack(outputs_classes)
