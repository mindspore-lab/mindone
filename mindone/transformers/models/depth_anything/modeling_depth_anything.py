...
class DepthAnythingReassembleLayer(nn.Cell):
    ...
    # Copied from transformers.models.dpt.modeling_dpt.DPTReassembleLayer.construct
...
class DepthAnythingDepthEstimationHead(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.conv2 = mint.nn.Conv2d(features // 2, config.head_hidden_size, kernel_size=3, stride=1, padding=1)
        self.activation1 = mint.nn.ReLU()
        self.conv3 = mint.nn.Conv2d(config.head_hidden_size, 1, kernel_size=1, stride=1, padding=0)
        if config.depth_estimation_type == "relative":
            self.activation2 = mint.nn.ReLU()
        elif config.depth_estimation_type == "metric":
            self.activation2 = mint.nn.Sigmoid()
        else:
            raise ValueError(f"Unknown depth estimation type: {config.depth_estimation_type}")
        self.max_depth = config.max_depth

    def construct(self, hidden_states: List[ms.Tensor], patch_height, patch_width) -> ms.Tensor:
