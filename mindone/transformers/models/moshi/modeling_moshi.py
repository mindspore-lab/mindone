...
from transformers import MoshiConfig, MoshiDepthConfig
from transformers.generation.configuration_utils import GenerationConfig
from transformers.utils import ModelOutput, logging

import mindspore
from mindspore import Parameter, Tensor, mint, nn, numpy, ops
...
from ...modeling_outputs import BaseModelOutputWithPast, CausalLMOutputWithPast, Seq2SeqLMOutput
...
class MoshiConditionalGenerationGenerateOutput(ModelOutput):
    ...
    Args:
        audio_sequences (`ms.Tensor` of shape `(batch_size*num_return_sequences, 1, sequence_length)`, *optional*):
        ...
        audio_codes (`ms.Tensor` of shape `(batch_size*num_return_sequences, num_codeooks, sequence_length)`, *optional*):
            The generated audio codes. Returned if `return_audio_codes=True`.
            Intermediate audio "tokens" which transforms to `audio_sequences` once passed through the audio decoder.
    """

    audio_sequences: Optional[Tensor] = None
...
class MoshiConditionalGenerationOutputWithPast(ModelOutput):
    ...
    Args:
        loss (`ms.Tensor` of shape `(1,)`, *optional*, returned when `text_labels` is provided):
        ...
        attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            Attentions weights after the attention softmax, used to compute the weighted average in the self-attention
            heads.
        depth_loss (`ms.Tensor` of shape `(1,)`, *optional*, returned when `audio_labels` is provided):
            Audio language modeling loss (for next-token prediction).
        audio_logits (`ms.Tensor` of shape `(batch_size, sequence_length, config.vocab_size)`):
            Prediction scores of the audio language modeling heads.
        depth_past_key_values (`tuple(tuple(ms.Tensor))`, *optional*, returned when `use_cache=True` is passed or when `config.use_cache=True`):
            Past key-values of the depth decoder.
        depth_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Hidden states of the depth decoder
        depth_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
    ...
    hidden_states: Optional[Tuple[Tensor, ...]] = None
    attentions: Optional[Tuple[Tensor, ...]] = None
    depth_loss: Optional[Tensor] = None
    audio_logits: Tensor = None
    depth_past_key_values: Optional[Tuple[Tuple[Tensor]]] = None
    depth_hidden_states: Optional[Tuple[Tensor, ...]] = None
    depth_attentions: Optional[Tuple[Tensor, ...]] = None
...
class MoshiUnconditionalInput(ModelOutput):
    ...
    Args:
        ...
        user_audio_codes (`ms.Tensor `of shape `(batch_size, num_codebooks, sequence_length), *optional*):
            The audio codes used as audio user prompt for the generation.
            Has priority over `user_input_values` and represents the audio "tokens" of `user_input_values` once passed through the audio encoder.
        moshi_audio_codes (`ms.Tensor `of shape `(batch_size, num_codebooks, sequence_length), *optional*):
            The audio codes used as audio Moshi prompt for the generation.
            Has priority over `moshi_input_values` and represents the audio "tokens" of `moshi_input_values` once passed through the audio encoder.
...
class MoshiFlexibleLinear(nn.Cell):
...
class MoshiRotaryEmbedding(nn.Cell):
...
class MoshiGatingMLP(nn.Cell):
    ...
    def construct(self, hidden_states: Tensor, layer_idx: int = None) -> Tensor:
...
class MoshiSdpaAttention(MoshiAttention):
    """
    Moshi attention module using scaled_dot_product_attention. This module inherits from
...
class MoshiDecoderLayer(nn.Cell):
...
class MoshiDepthDecoder(MoshiPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: MoshiDepthConfig):
        ...
        self.text_embed_tokens = mint.nn.Embedding(config.vocab_size + 1, config.hidden_size)

        # the last codebook is never used as input
        self.embed_tokens = nn.CellList(
            [
                mint.nn.Embedding(config.audio_vocab_size + 1, config.hidden_size)
    ...
    def construct(
        ...
        Args:
            input_ids (`ms.Tensor` of shape `(batch_size, sequence_length)`):
                ...
                The `cache_position` argument can be used to indicate to which index is associated each token.
            last_hidden_state (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
        ...
        if cache_position is None:
            cache_position = mint.arange(past_seen_tokens, past_seen_tokens + input_ids.shape[1])
        ...
        if inputs_embeds is None:
            ...
            inputs_embeds = mint.cat(inputs_embeds, dim=1)
...
class MoshiModel(MoshiPreTrainedModel):
    ...
    def __init__(self, config: MoshiConfig):
        ...
        self.embed_tokens = mint.nn.Embedding(config.vocab_size + 1, config.hidden_size, self.padding_idx)
        self.layers = nn.CellList(
            [
...
class MoshiForCausalLM(MoshiPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs,
        ...
        >>> from mindone.transformers.models.moshi import MoshiForCausalLM
        ...
        >>> # Generate
        >>> generate_ids = model.generate(inputs.input_ids, max_length=30, use_cache=False)
...
class MoshiForConditionalGeneration(MoshiPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        input_ids: Optional[Tensor] = None,
        attention_mask: Optional[Tensor] = None,
        user_input_values: Optional[Tensor] = None,
        user_audio_codes: Optional[Tensor] = None,
        moshi_input_values: Optional[Tensor] = None,
        moshi_audio_codes: Optional[Tensor] = None,
        past_key_values: Tuple[Tuple[Tensor]] = None,
        inputs_embeds: Optional[Tensor] = None,
        text_labels: Optional[Tensor] = None,
        audio_labels: Optional[Tensor] = None,
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        **kwargs,
        ...
        Examples:
        ```python
        >>> from mindone.transformers.models.moshi import MoshiForConditionalGeneration
        >>> import mindspore as ms
        ...
        if inputs_embeds is None:
            ...
            audio_codes = mint.cat([moshi_audio_codes, user_audio_codes], dim=1)
        ...
        if text_labels is not None and audio_labels is not None:
            ...
            depth_input_ids = mint.cat([text_labels, audio_labels], dim=1)
    ...
    def _prepare_attention_mask_for_generation(
        ...
        default_attention_mask = mint.ones(input_ids.shape, dtype=mindspore.int64)
        if pad_token_id is None:
            return default_attention_mask

        is_pad_token_in_inputs = (pad_token_id is not None) and numpy.isin(input_ids, pad_token_id).any()
        is_pad_token_not_equal_to_eos_token_id = (eos_token_id is None) or ~numpy.isin(eos_token_id, pad_token_id).any()
    ...
    def _prepare_inputs_embeds_for_generation(
        ...
        if (
            ...
            raise ValueError(
                "You must provide at least one of `input_ids`, `user_input_values`, "
                "`moshi_input_values`, `user_audio_codes`, `moshi_audio_codes` or `inputs_embeds`."
            )
        ...
        if inputs_embeds is None and concat_unconditional_inputs:
            ...
            moshi_audio_codes = mint.cat([unconditional_inputs.moshi_audio_codes, moshi_audio_codes], dim=2)
            user_audio_codes = mint.cat([unconditional_inputs.user_audio_codes, user_audio_codes], dim=2)
            input_ids = mint.cat([unconditional_inputs.input_ids, input_ids], dim=1)
            if attention_mask is not None:
                attention_mask = mint.cat([unconditional_inputs.attention_mask, attention_mask], dim=1)
    ...
    def generate(
        ...
        **kwargs,
        ...
        Parameters:
            input_ids (`ms.Tensor `of shape `(batch_size, sequence_length), *optional*):
                The sequence used as a text prompt for the generation.
            user_input_values (`ms.Tensor `of shape `(batch_size, 1, audio_sequence_length), *optional*):
                The audio waveforms used as audio user prompt for the generation.
            user_audio_codes (`ms.Tensor `of shape `(batch_size, num_codebooks, sequence_length), *optional*):
                The audio codes used as audio user prompt for the generation.
                Has priority over `user_input_values` and represents the audio "tokens" of `user_input_values` once passed through the audio encoder.
            moshi_input_values (`ms.Tensor `of shape `(batch_size, 1, audio_sequence_length), *optional*):
            ...
            return_audio_codes (`bool`, *optional*):
                If `True`, will also returns the generated audio codes,
                i.e the intermediate audio "tokens" which transforms to `audio_sequences` once passed through the audio decoder.
            concat_unconditional_inputs (`bool`, *optional*, defaults to `True`):
                If `False`, won't concatenate initial audio and text tokens.
            kwargs (`Dict[str, Any]`, *optional*):
                Remaining dictionary of keyword arguments that are passed to the `generate` method. Refers to the original
                [`generate` docstrings](https://huggingface.co/docs/transformers/main/en/main_classes/text_generation#transformers.GenerationMixin.generate)
        ...
        kwargs["moshi_delay_pattern_mask"] = (
            moshi_delay_pattern_mask if moshi_delay_pattern_mask is not None else kwargs.get("moshi_delay_pattern_mask")
        )

        self.generated_audio_codes = mint.repeat_interleave(
        ...
        if generation_config.num_return_sequences > 1:
            moshi_delay_pattern_mask = mint.repeat_interleave(
        ...
        if generation_config.num_beams > 1:
            ...
            generated_audio_codes = mint.gather(generated_audio_codes, dim=0, index=expanded_beam_indices)

            # now, rebuild generated audio codes, this time with the right beam tracking
            moshi_audio_codes = mint.repeat_interleave(moshi_audio_codes, generation_config.num_return_sequences, dim=0)
            self.generated_audio_codes = mint.cat((moshi_audio_codes, generated_audio_codes), dim=2)

            # use the last beam indice to retrieve the right self.last_hidden_state
            self.last_hidden_state = mint.index_select(self.last_hidden_state, dim=0, index=beam_indices[:, -1])
        ...
        self.generated_audio_codes = mint.cat([self.generated_audio_codes, last_generated_audio_codes], dim=2)
    ...
    def prepare_inputs_for_generation(
        ...
        blank_user_audio_codes: Optional[Tensor] = None,
        **kwargs,
    ):
        ...
        if model_inputs["input_ids"] is not None:
            ...
            user_audio_codes = self.apply_delay_pattern_mask(
                mint.cat([self.generated_audio_codes, blank_user_audio_codes], dim=2),
                user_delay_pattern_mask,
            )[:, :, -1:]
            self.generated_audio_codes = self.apply_delay_pattern_mask(
                mint.cat([self.generated_audio_codes, generated_audio_codes], dim=2), moshi_delay_pattern_mask
            )
    ...
    def apply_delay_pattern_mask(input_ids, decoder_pad_token_mask):
        ...
        input_ids = mint.where(decoder_pad_token_mask == -1, input_ids, decoder_pad_token_mask)
        return input_ids

    def build_delay_pattern_mask(self, input_ids: Tensor, bos_token_id: int, pad_token_id: int, max_length: int = None):
        ...
        input_ids_shifted = mint.ones((bsz, num_codebooks, max_length), dtype=mindspore.int64) * -1
    ...
    def get_unconditional_inputs(self, num_samples=1):
        ...
        input_ids = mint.ones((num_samples, 1), dtype=mindspore.int64) * self.config.vocab_size
        user_audio_codes = (
            mint.ones((num_samples, self.num_codebooks, 1), dtype=mindspore.int64) * self.config.audio_vocab_size
        )
        moshi_audio_codes = (
            mint.ones((num_samples, self.num_codebooks, 1), dtype=mindspore.int64) * self.config.audio_vocab_size
        )
        attention_mask = mint.ones((num_samples, 1), dtype=mindspore.int64)
    ...
    def _check_and_maybe_initalize_inputs(
        ...
        if one_input_has_been_passed and (user_input is None):
            raise ValueError(
                "No user audio inputs have been passed alongside the other inputs. "
                "Make sure either `user_input_values` or `user_audio_codes` is passed or use `MoshiForConditionalGeneration.get_unconditional_inputs`. "
            ...
            raise ValueError(
                "No Moshi audio inputs have been passed alongside the other inputs. "
                "Make sure either `moshi_input_values` or `moshi_audio_codes` is passed or use `MoshiForConditionalGeneration.get_unconditional_inputs`. "
                "Check the `MoshiForConditionalGeneration` docstrings for more information."
            )
        elif one_input_has_been_passed and (inputs is None):
            raise ValueError(
                "No `input_ids` or `inputs_embeds` have been passed alongside the other inputs. "
                "Make sure `input_ids` is passed or use `MoshiForConditionalGeneration.get_unconditional_inputs`. "
                "Check the `MoshiForConditionalGeneration` docstrings for more information."
            )
    ...
    @staticmethod
    def _reorder_cache(past_key_values: Tuple[Tuple[Tensor]], beam_idx: Tensor) -> Tuple[Tuple[Tensor]]:
        """
