...
"""Mindspore YOSO model."""

import math
from typing import Optional, Tuple, Union

import torch.utils.checkpoint
from transformers.models.yoso.configuration_yoso import YosoConfig
...
def normalize(input_tensors):
    if isinstance(input_tensors, list):
        out = []
        for tensor in input_tensors:
            out.append(mindspore.mint.nn.functional.normalize(tensor, p=2, dim=-1))
        return out
    else:
        return mindspore.mint.nn.functional.normalize(input_tensors, p=2, dim=-1)
...
def hashing(query, key, num_hash, hash_len):
    ...
    rmat = mindspore.mint.randn(
        query.shape[0],
        query.shape[2],
        num_hash * hash_len,
    )
    raise_pow = 2 ** mindspore.mint.arange(
        hash_len,
    )

    query_projection = mindspore.mint.matmul(query, rmat).reshape(query.shape[0], query.shape[1], num_hash, hash_len)
    key_projection = mindspore.mint.matmul(key, rmat).reshape(key.shape[0], key.shape[1], num_hash, hash_len)
    query_binary = (query_projection > 0).int()
    key_binary = (key_projection > 0).int()
    query_hash = mindspore.mint.sum(query_binary * raise_pow, dim=-1)
    query_hash = mindspore.mint.sum(key_binary * raise_pow, dim=-1)

    return query_hash.int(), query_hash.int()


class YosoCumulation(mindspore.nn.Cell):
    @staticmethod
    def construct(query_mask, key_mask, query, key, value, config):
        hash_code_len = config["hash_code_len"]

        expectation = (
            1 - mindspore.mint.acos(mindspore.mint.matmul(query, key.transpose(-1, -2))) / math.pi
        ) ** hash_code_len
        expectation = expectation * query_mask[:, :, None] * key_mask[:, None, :]
        cumulation_value = mindspore.mint.matmul(expectation, value)

        return cumulation_value


class YosoLSHCumulation(mindspore.nn.Cell):
    @staticmethod
    def construct(query_mask, key_mask, query, key, value, config):
        if query_mask.shape[0] != key_mask.shape[0]:
            raise ValueError("Query mask and Key mask differ in sizes in dimension 0")
        if query_mask.shape[0] != query.shape[0]:
            raise ValueError("Query mask and Query differ in sizes in dimension 0")
        if query_mask.shape[0] != key.shape[0]:
            raise ValueError("Query mask and Key differ in sizes in dimension 0")
        if query_mask.shape[0] != value.shape[0]:
...
class YosoEmbeddings(mindspore.nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.register_buffer(
            "position_ids", mindspore.mint.arange(config.max_position_embeddings).expand((1, -1)) + 2, persistent=False
        )
...
class YosoSelfAttention(mindspore.nn.Cell):
    ...
    def construct(self, hidden_states, attention_mask=None, output_attentions=False):
        ...
        if self.use_expectation:
            context_layer = YosoCumulation.construct(
                attention_mask, attention_mask, query_layer, key_layer, value_layer, self.lsh_config
            )
        else:
            context_layer = YosoLSHCumulation.construct(
                attention_mask, attention_mask, query_layer, key_layer, value_layer, self.lsh_config
            )
...
class YosoSelfOutput(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class YosoAttention(mindspore.nn.Cell):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
...
class YosoIntermediate(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class YosoOutput(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class YosoLayer(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class YosoEncoder(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = mindspore.nn.CellList([YosoLayer(config) for _ in range(config.num_hidden_layers)])
...
class YosoPredictionHeadTransform(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.dense = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
...
class YosoLMPredictionHead(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class YosoOnlyMLMHead(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class YosoClassificationHead(mindspore.nn.Cell):
