...
class Lfm2RMSNorm(mindspore.nn.Cell):
...
class Lfm2RotaryEmbedding(mindspore.nn.Cell):
...
class Lfm2MLP(mindspore.nn.Cell):
    def __init__(self, config: Lfm2Config):
        super().__init__()
        ...
        self.w1 = mindspore.mint.nn.Linear(config.hidden_size, intermediate_size, bias=False)
        self.w3 = mindspore.mint.nn.Linear(config.hidden_size, intermediate_size, bias=False)
        self.w2 = mindspore.mint.nn.Linear(intermediate_size, config.hidden_size, bias=False)

    def construct(self, x):
        return self.w2(mindspore.mint.nn.functional.silu(self.w1(x)) * self.w3(x))
...
class Lfm2HybridConvCache:
    ...
    def __init__(
        ...
        self.conv_cache: list[mindspore.Tensor] = []

        for _ in range(config.num_hidden_layers):
            conv_state = mindspore.mint.zeros(
                (
                self.max_batch_size,
                config.hidden_size,
                self.conv_L_cache,
        ...
        if key_states is not None:
            if len(self.key_cache) <= layer_idx:
                ...
                for _ in range(len(self.key_cache), layer_idx):
                    self.key_cache.append(mindspore.Tensor([]))
                    self.value_cache.append(mindspore.Tensor([]))
    ...
    def get_mask_sizes(self, cache_position: mindspore.Tensor, layer_idx: int) -> tuple[int, int]:
        """
    ...
    def __getitem__(self, layer_idx: int) -> tuple[mindspore.Tensor, mindspore.Tensor]:
        return self.key_cache[layer_idx], self.value_cache[layer_idx]
...
class Lfm2Attention(mindspore.nn.Cell):
    ...
    def __init__(self, config: Lfm2Config, layer_idx: int):
        ...
        self.is_causal = True
        self.q_proj = mindspore.mint.nn.Linear(config.hidden_size, config.num_attention_heads * self.head_dim, bias=False)
        self.k_proj = mindspore.mint.nn.Linear(config.hidden_size, config.num_key_value_heads * self.head_dim, bias=False)
        self.v_proj = mindspore.mint.nn.Linear(config.hidden_size, config.num_key_value_heads * self.head_dim, bias=False)
        self.out_proj = mindspore.mint.nn.Linear(config.num_attention_heads * self.head_dim, config.hidden_size, bias=False)
...
class Lfm2ShortConv(mindspore.nn.Cell):
    def __init__(
        self,
        config: Lfm2Config,
        layer_idx: int,
        ...
        self.conv = mindspore.mint.nn.Conv1d(
        ...
        self.in_proj = mindspore.mint.nn.Linear(config.hidden_size, 3 * config.hidden_size, bias=self.bias)
        self.out_proj = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size, bias=self.bias)
    ...
    def cuda_kernels_forward(
        ...
        conv_weights = self.conv.weight.view(self.conv.weight.shape[0], self.conv.weight.shape[2])
        if past_key_values is not None and cache_position[0] > 0:
            ...
            if past_key_values is not None:
                conv_state = mindspore.mint.nn.functional.pad(Bx, (self.L_cache - Bx.shape[-1], 0))
    ...
    def slow_forward(
        ...
        if past_key_values is not None and cache_position[0] > 0:
            ...
            conv_state[:, :, cache_position] = Bx.to(dtype=conv_state.dtype)
            past_key_values.conv_cache[self.layer_idx].copy_(conv_state)
            conv_out = mindspore.mint.sum(conv_state * self.conv.weight[:, 0, :], dim=-1)
            if self.bias:
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if use_cache and past_key_values is None:
            ...
            past_key_values = Lfm2HybridConvCache(
                config=self.config, max_batch_size=batch_size, dtype=self.dtype, )
...
class Lfm2ForCausalLM(Lfm2PreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        >>> from mindone.transformers import Lfm2ForCausalLM
