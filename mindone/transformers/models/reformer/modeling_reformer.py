...
def _stable_argsort(vector, dim):
    ...
    scale_offset = mindspore.mint.arange(
        vector.shape[dim],
    ).view(1, 1, -1)
    scale_offset = scale_offset.expand(vector.shape)
    scaled_vector = vector.shape[dim] * vector + (scale_offset % vector.shape[dim])
    return mindspore.mint.argsort(scaled_vector, dim=dim)
...
class AxialPositionEmbeddings(mindspore.nn.Cell):
    """
    ...
    def construct(self, position_ids):
        ...
        if self.training is True:
            ...
            if self.dropout > 0:
                weights = mindspore.mint.cat(broadcasted_weights, dim=-1)
                ...
                dropped_transposed_weights = mindspore.mint.nn.functional.dropout2d(
                    transposed_weights, p=self.dropout, training=self.training
                )
                dropped_weights = dropped_transposed_weights.transpose(2, 1)

                position_encodings = mindspore.mint.reshape(dropped_weights, (batch_size, sequence_length, -1))

            else:
                position_encodings = mindspore.mint.cat(
                    [
                        mindspore.mint.reshape(weight, (batch_size, sequence_length, -1))
            ...
            position_encodings = mindspore.mint.reshape(
            ...
            position_encodings = mindspore.mint.cat(
                [
                    mindspore.mint.index_select(position_encodings[i], 0, position_ids[i]).unsqueeze(0)
...
class PositionEmbeddings(mindspore.nn.Cell):
    """Constructs conventional position embeddings of shape `[max_pos_embeddings, hidden_size]`."""

    def __init__(self, config):
        super().__init__()
        self.dropout = config.hidden_dropout_prob
        self.embedding = mindspore.mint.nn.Embedding(config.max_position_embeddings, config.hidden_size)

    def construct(self, position_ids):
        position_embeddings = self.embedding(position_ids)
        position_embeddings = mindspore.mint.nn.functional.dropout(
            position_embeddings, p=self.dropout, training=self.training
        )
        return position_embeddings


class ReformerEmbeddings(mindspore.nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.word_embeddings = mindspore.mint.nn.Embedding(config.vocab_size, config.hidden_size)
    ...
    def construct(self, input_ids=None, position_ids=None, inputs_embeds=None, start_idx_pos_encodings=0):
        ...
        # dropout
        embeddings = mindspore.mint.nn.functional.dropout(inputs_embeds, p=self.dropout, training=self.training)
...
class EfficientAttentionMixin:
    ...
    def _look_adjacent(self, vectors, num_chunks_before, num_chunks_after):
        ...
        for i in range(-num_chunks_before, num_chunks_after + 1):
            if i == 0:
                ...
                slices.append(mindspore.mint.cat([vectors[:, :, i:, ...], vectors[:, :, :i, ...]], dim=2))
        return mindspore.mint.cat(slices, dim=3)
    ...
    def _merge_hidden_size_dims(self, x, num_attn_heads, attn_head_size):
        ...
        """
        x = x.permute(0, 2, 1, 3)
        return mindspore.mint.reshape(x, (x.shape[0], -1, num_attn_heads * attn_head_size))
    ...
    def _split_seq_length_dim_to(self, vectors, dim_factor_1, dim_factor_2, num_attn_heads, attn_head_size=None):
        ...
        if len(vectors.shape) == 4:
            return mindspore.mint.reshape(vectors, split_dim_shape + (attn_head_size,))
        elif len(vectors.shape) == 3:
            return mindspore.mint.reshape(vectors, split_dim_shape)
        else:
            raise ValueError(f"Input vector rank should be one of [3, 4], but is: {len(vectors.shape)}")


class LSHSelfAttention(mindspore.nn.Cell, EfficientAttentionMixin):
    def __init__(self, config):
        super().__init__()
        self.config = config
        ...
        # projection matrices
        self.query_key = mindspore.mint.nn.Linear(self.hidden_size, self.all_head_size, bias=False)
        self.value = mindspore.mint.nn.Linear(self.hidden_size, self.all_head_size, bias=False)
        ...
        **kwargs,
        ...
        if do_cached_attention:
            ...
            if past_buckets is not None:
                ...
                key_value_hidden_states = mindspore.mint.cat([past_states, hidden_states], dim=1)
        ...
        if not do_standard_self_attention:
            ...
            sorted_bucket_idx_per_hash = mindspore.mint.arange(
                sequence_length,
        ...
        if not do_standard_self_attention:
            ...
            out_vectors, logits = ReverseSort.construct(out_vectors, logits, sorted_bucket_idx, undo_sorted_bucket_idx)
        ...
        if not do_standard_self_attention or (do_cached_attention and past_buckets is not None):
            ...
            if num_hashes > 1:
                ...
                probs_vectors = mindspore.mint.exp(logits - mindspore.mint.logsumexp(logits, dim=2, keepdim=True))
                out_vectors = mindspore.mint.sum(out_vectors * probs_vectors, dim=2)
                # free memory
                del probs_vectors
    ...
    def _query_per_attn_head(self, hidden_states):
        ...
        query_key_vectors = mindspore.mint.einsum("balh,ahr->balr", hidden_states, per_head_query_key)
    ...
    def _value_per_attn_head(self, hidden_states):
        ...
        # only relevant for inference and no bias => we can use einsum here
        value_vectors = mindspore.mint.einsum("balh,ahr->balr", hidden_states, per_head_value)
    ...
    def _hash_vectors(self, vectors, num_hashes, attention_mask, increase_num_buckets=False):
        ...
        random_rotations = mindspore.mint.randn(rotations_shape, dtype=vectors.dtype)
        # Output dim: Batch_Size x Num_Attn_Heads x Num_Hashes x Seq_Len x Num_Buckets/2
        rotated_vectors = mindspore.mint.einsum("bmtd,mdhr->bmhtr", vectors, random_rotations)

        if isinstance(self.num_buckets, int) or len(self.num_buckets) == 1:
            rotated_vectors = mindspore.mint.cat([rotated_vectors, -rotated_vectors], dim=-1)
            buckets = mindspore.mint.argmax(rotated_vectors, dim=-1)
        else:
            ...
            for bucket_factor in self.num_buckets:
                ...
                rotated_vectors_factor = mindspore.mint.cat([rotated_vectors_factor, -rotated_vectors_factor], dim=-1)
                if buckets is None:
                    buckets = mindspore.mint.argmax(rotated_vectors_factor, dim=-1)
                else:
                    buckets = buckets + (cur_product * mindspore.mint.argmax(rotated_vectors_factor, dim=-1))
    ...
    def _attend(
        ...
        # get logits and dots
        # (BS, NumAttn, NumHash x NumChunk, Chunk_L x Hidden),
        # (BS, NumAttn, NumHash x NumChunk, Chunk_L * (Num_bef + Num_aft + 1) x Hidden)
        # -> (BS, NumAttn, NumHash x NumChunk, Chunk_L, Chunk_L * (1 + Num_bef + Num_aft))
        query_key_dots = mindspore.mint.matmul(query_vectors, key_vectors.transpose(-1, -2))
        ...
        if not do_standard_self_attention:
            ...
            query_bucket_idx = (query_key_dots.shape[-1] - 1) * mindspore.mint.ones_like(query_key_dots)[:, :, :, -1]
            key_value_bucket_idx = mindspore.mint.arange(
                query_key_dots.shape[-1],
                dtype=mindspore.int64,
            )[
                None, None, :
            ].expand(query_bucket_idx.shape[:2] + (-1,))
        else:
        ...
        if not do_cached_attention:
            ...
            if mask is not None:
                query_key_dots = mindspore.mint.where(mask, query_key_dots, mask_value)
        ...
        self_mask = mindspore.mint.ne(query_bucket_idx.unsqueeze(-1), key_value_bucket_idx.unsqueeze(-2))

        # apply self_mask
        query_key_dots = mindspore.mint.where(self_mask, query_key_dots, self_mask_value)
        ...
        # dropout
        attention_probs = mindspore.mint.nn.functional.dropout(attention_probs, p=self.dropout, training=self.training)
    ...
    def _compute_attn_mask(
        ...
        if attention_mask is not None:
            ...
            if not do_standard_self_attention:
                ...
                attention_mask = mindspore.mint.gather(attention_mask, -1, key_indices)

            attention_mask = attention_mask.unsqueeze(-2).expand(query_key_dot_shape)

        # Causal mask
        if self.is_decoder is True:
            causal_mask = mindspore.mint.ge(query_indices.unsqueeze(-1), key_indices.unsqueeze(-2))
    ...
    def _get_relevant_hid_states_and_buckets(
        ...
        hidden_states = mindspore.mint.cat([past_states, hidden_states], dim=1)
        ...
        # concat buckets
        concat_buckets = mindspore.mint.cat([past_buckets, query_buckets.unsqueeze(-1)], dim=-1)
        ...
        offset = mindspore.mint.arange(relevant_bucket_idx_chunk.shape[-1], dtype=mindspore.int64)
        bucket_idx_batch_offset = sequence_length * (
            batch_size * mindspore.mint.div(offset, relevant_bucket_idx_chunk.shape[-1], rounding_mode="floor")
        )
    ...
    def _expand_to_indices_in_relevant_chunk(self, indices, sequence_length):
        ...
        chunk_sequence_indices = expanded_start_indices + mindspore.mint.arange(
    ...
    def _len_norm(self, x, epsilon=1e-6):
        ...
        """
        variance = mindspore.mint.mean(x**2, -1, keepdim=True)
        norm_x = x * mindspore.mint.rsqrt(variance + epsilon)
        return norm_x
    ...
    def _gather_by_expansion(self, vectors, idxs, num_hashes):
        ...
        return mindspore.mint.gather(vectors, 2, expanded_idxs)


class ReverseSort(mindspore.nn.Cell):
    """
    ...
    @staticmethod
    def construct(out_vectors, logits, sorted_bucket_idx, undo_sorted_bucket_idx):
        # save sorted_bucket_idx for backprop
        # ctx.sorted_bucket_idx = sorted_bucket_idx

        # undo sort to have correct order for next layer
        expanded_undo_sort_indices = undo_sorted_bucket_idx.unsqueeze(-1).expand(out_vectors.shape)
        out_vectors = mindspore.mint.gather(out_vectors, 2, expanded_undo_sort_indices)
        logits = mindspore.mint.gather(logits, 2, undo_sorted_bucket_idx)
        return out_vectors, logits


class LocalSelfAttention(mindspore.nn.Cell, EfficientAttentionMixin):
    def __init__(self, config):
        super().__init__()
        ...
        # projection matrices
        self.query = mindspore.mint.nn.Linear(self.hidden_size, self.all_head_size, bias=False)
        self.key = mindspore.mint.nn.Linear(self.hidden_size, self.all_head_size, bias=False)
        self.value = mindspore.mint.nn.Linear(self.hidden_size, self.all_head_size, bias=False)
    ...
    def construct(
        ...
        **kwargs,
        ...
        if use_cache and past_buckets_states[1] is not None:
            ...
            key_value_hidden_states = mindspore.mint.cat([key_value_hidden_states, hidden_states], dim=1)
...
class ReformerSelfOutput(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        all_head_size = config.num_attention_heads * config.attention_head_size
        self.dropout = config.hidden_dropout_prob

        self.dense = mindspore.mint.nn.Linear(all_head_size, config.hidden_size, bias=False)
...
class ReformerAttention(mindspore.nn.Cell):
    ...
    def construct(
        ...
        if use_cache:
            if past_buckets_states[self.layer_id][0] is None:
                ...
                past_buckets = mindspore.mint.cat([past_buckets_states[self.layer_id][0], buckets], dim=-1)

            if past_buckets_states[self.layer_id][1] is None:
                # padded input should not be cached
                past_states = hidden_states[:, :orig_sequence_length]
            else:
                past_states = mindspore.mint.cat([past_buckets_states[self.layer_id][1], hidden_states], dim=1)
...
class ReformerFeedForwardDense(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.dropout = config.hidden_dropout_prob
        ...
        self.dense = mindspore.mint.nn.Linear(config.hidden_size, config.feed_forward_size)
...
class ReformerFeedForwardOutput(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.dropout = config.hidden_dropout_prob

        self.dense = mindspore.mint.nn.Linear(config.feed_forward_size, config.hidden_size)
...
class ChunkReformerFeedForward(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class ReformerLayer(mindspore.nn.Cell):
    def __init__(self, config, layer_id=0):
        super().__init__()
...
class _ReversibleFunction(mindspore.nn.Cell):
    """
    ...
    def construct(
        ...
        hidden_states, attn_output = mindspore.mint.chunk(hidden_states, 2, dim=-1)
        ...
        return mindspore.mint.cat([attn_output, hidden_states], dim=-1)


class ReformerEncoder(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.dropout = config.hidden_dropout_prob

        self.layers = mindspore.nn.CellList([ReformerLayer(config, i) for i in range(config.num_hidden_layers)])
        # Reformer is using Rev Nets, thus last layer outputs are concatenated and
        # Layer Norm is done over 2 * hidden_size
        self.layer_norm = mindspore.mint.nn.LayerNorm(2 * config.hidden_size, eps=config.layer_norm_eps)
    ...
    def construct(
        ...
        hidden_states = mindspore.mint.cat([hidden_states, hidden_states], dim=-1)
        hidden_states = _ReversibleFunction.construct(
            hidden_states,
            self.layers,
            attention_mask,
            head_mask,
...
class ReformerOnlyLMHead(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        # Reformer is using Rev Nets, thus last layer outputs are concatenated and
        # Layer Norm is done over 2 * hidden_size
        ...
        self.decoder = mindspore.mint.nn.Linear(2 * config.hidden_size, config.vocab_size, bias=False)
...
class ReformerModelOutput(ModelOutput):
    ...
    Args:
        ...
        past_buckets_states (`List[Tuple(ms.Tensor, ms.Tensor)]`, *optional*, returned when `use_cache=True` is passed or when `config.use_cache=True`):
            List of `Tuple(ms.Tensor, ms.Tensor` of length `config.n_layers`, with the first element being
            the previous *buckets* of shape `(batch_size, num_heads, num_hashes, sequence_length)`) and the
        ...
        hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple of `ms.Tensor` (one for the output of the embeddings and one for the output of each layer)
...
class ReformerModel(ReformerPreTrainedModel):
    ...
    def _pad_to_mult_of_chunk_length(
        ...
        padded_input_ids = mindspore.ops.full(
        ...
        if attention_mask is not None:
            pad_attention_mask = mindspore.mint.zeros(input_shape[0], padding_length, dtype=attention_mask.dtype)

            attention_mask = mindspore.mint.cat([attention_mask, pad_attention_mask], dim=-1)
        else:
            attention_mask = mindspore.mint.cat(
                [
                    mindspore.mint.ones(input_shape, dtype=mindspore.bool_),
                    mindspore.mint.zeros((input_shape[0], padding_length), dtype=mindspore.bool_),
                ],
                dim=-1,
            )
        ...
        if input_ids is not None:
            input_ids = mindspore.mint.cat([input_ids, padded_input_ids], dim=-1)
            input_shape = input_ids.shape

            # Pad position ids if given
            if position_ids is not None:
                padded_position_ids = mindspore.mint.arange(
                    input_shape[-1],
                    padded_seq_length,
                    dtype=mindspore.int64,
                )
                padded_position_ids = position_ids.unsqueeze(0).expand(input_shape[0], padding_length)
                position_ids = mindspore.mint.cat([position_ids, padded_position_ids], dim=-1)

        # Extend `inputs_embeds` with padding to match least common multiple chunk_length
        if inputs_embeds is not None:
            padded_inputs_embeds = self.embeddings(padded_input_ids, position_ids)
            inputs_embeds = mindspore.mint.cat([inputs_embeds, padded_inputs_embeds], dim=-2)
...
class ReformerClassificationHead(mindspore.nn.Cell):
    """Head for sentence-level classification tasks."""

    def __init__(self, config):
        super().__init__()
        self.dense = mindspore.mint.nn.Linear(2 * config.hidden_size, config.hidden_size)
        classifier_dropout = (
            config.classifier_dropout if config.classifier_dropout is not None else config.hidden_dropout_prob
        )
        self.dropout = mindspore.mint.nn.Dropout(classifier_dropout)
    ...
    def construct(self, hidden_states, **kwargs):
        ...
        hidden_states = self.dense(hidden_states)
        hidden_states = mindspore.mint.tanh(hidden_states)
        hidden_states = self.dropout(hidden_states)
...
class ReformerForQuestionAnswering(ReformerPreTrainedModel):
    def __init__(self, config):
        ...
        self.qa_outputs = mindspore.mint.nn.Linear(2 * config.hidden_size, config.num_labels)
