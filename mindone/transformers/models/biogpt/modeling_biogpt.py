...
class BioGptLearnedPositionalEmbedding(mindspore.mint.nn.Embedding):
    """
...
class BioGptScaledWordEmbedding(mindspore.mint.nn.Embedding):
    """
...
class BioGptAttention(mindspore.nn.Cell):
...
class BioGptSdpaAttention(BioGptAttention):
    def construct(
        ...
        if output_attentions or layer_head_mask is not None:
            ...
            logger.warning_once(
                "BioGptModel is using BioGptSdpaAttention, but `scaled_dot_product_attention` does not support `output_attentions=True` "
...
class BioGptDecoderLayer(mindspore.nn.Cell):
    def __init__(self, config: BioGptConfig):
        ...
        self.fc1 = mindspore.mint.nn.Linear(self.embed_dim, config.intermediate_size)
        self.fc2 = mindspore.mint.nn.Linear(config.intermediate_size, self.embed_dim)
...
class BioGptModel(BioGptPreTrainedModel):
    def __init__(self, config: BioGptConfig):
        ...
        self.layers = mindspore.nn.CellList([BioGptDecoderLayer(config) for _ in range(config.num_hidden_layers)])
        self.layer_norm = mindspore.mint.nn.LayerNorm(self.embed_dim)
...
class BioGptForCausalLM(BioGptPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.output_projection = mindspore.mint.nn.Linear(config.hidden_size, config.vocab_size, bias=False)
...
class BioGptForSequenceClassification(BioGptPreTrainedModel):
    ...
    def construct(
        ...
        if self.config.pad_token_id is None:
            ...
            if input_ids is not None:
                sequence_length = mindspore.mint.ne(input_ids, self.config.pad_token_id).sum(-1) - 1
            else:
                sequence_length = -1
                logger.warning_once(
