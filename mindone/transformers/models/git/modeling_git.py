...
class GitSelfAttention(nn.Cell):
    ...
    def construct(
        ...
        if past_key_value is not None:
            ...
            key_layer = mint.cat([key_layer[:, :, :cutoff, :], key_layer_past], dim=2)
            value_layer = mint.cat([value_layer[:, :, :cutoff, :], value_layer_past], dim=2)
...
class GitEncoder(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.CellList([GitLayer(config, i) for i in range(config.num_hidden_layers)])
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, GitVisionEmbeddings):
            module.class_embedding.set_data(
                initializer(
                    Normal(sigma=self.config.initializer_range, mean=0.0), module.class_embedding.shape, module.class_embedding.dtype
                )
            )
            module.patch_embedding.weight.set_data(
                initializer(
                    Normal(sigma=self.config.initializer_range), module.patch_embedding.weight.shape, module.patch_embedding.weight.dtype
                )
            )
            module.position_embedding.weight.set_data(
                initializer(
                    Normal(sigma=self.config.initializer_range), module.position_embedding.weight.shape, module.position_embedding.weight.dtype
                )
            )
...
GIT_START_DOCSTRING = r"""
    ...
    This model is also a MindSpore [ms.nn.Cell](https://www.ms.cn/docs/zh-CN/master/api_python/nn/ms.nn.Cell.html) subclass.
...
class GitProjection(nn.Cell):
    def __init__(self, config: GitConfig):
        super().__init__()
        self.config = config
        self.visual_projection = ms.nn.SequentialCell(
            mint.nn.Linear(config.vision_config.hidden_size, config.hidden_size),
            mint.nn.LayerNorm(config.hidden_size, eps=config.vision_config.layer_norm_eps),
        )

    def construct(self, embeddings: ms.Tensor) -> ms.Tensor:
...
class GitModel(GitPreTrainedModel):
    def __init__(self, config):
        ...
        if config.num_image_with_embedding is not None:
            self.img_temperal_embedding = [
                ms.Parameter(mint.zeros((1, 1, config.vision_config.hidden_size)))
    ...
    def _generate_future_mask(self, size: int, dtype: ms.dtype) -> ms.Tensor:
        # Default mask is for forward direction. Flip for backward direction.
        mask = mint.triu(mint.ones((size, size), dtype=dtype), diagonal=1)
    ...
    def create_attention_mask(self, tgt, memory, tgt_mask, past_key_values_length, memory_key_padding_mask=None):
        ...
        dtype = tgt.dtype
        top_left = mint.zeros((num_memory, num_memory), dtype=dtype)
        top_right = ms.ops.full(
            (num_memory, num_tgt + past_key_values_length),
            float("-inf"),
            dtype=dtype,
        )
        bottom_left = mint.zeros(
            (num_tgt, num_memory),
            dtype=dtype,
            )
        ...
        left = mint.cat((top_left, bottom_left), dim=0)
        right = mint.cat((top_right, tgt_mask.to(dtype)), dim=0)

        full_attention_mask = mint.cat((left, right), dim=1)[None, :]

        if memory_key_padding_mask is None:
            memory_key_padding_mask = ms.ops.full((memory.shape[0], memory.shape[1]), fill_value=False, dtype=ms.bool_)
        # if it is False, it means valid. That is, it is not a padding
        if memory_key_padding_mask.dtype != ms.bool_:
            raise ValueError("Memory key padding mask must be a boolean tensor.")
        zero_negative_infinity = mint.zeros_like(memory_key_padding_mask, dtype=tgt.dtype)
    ...
    def construct(
        ...
        if pixel_values is not None:
            if pixel_values.ndim == 4:
                ...
                visual_features = mint.cat(visual_features, dim=1)
        ...
        if projected_visual_features is None:
            projected_visual_features = mint.zeros(
        ...
        hidden_states = mint.cat((projected_visual_features, embedding_output), dim=1)
...
class GitForCausalLM(GitPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.output = mint.nn.Linear(config.hidden_size, config.vocab_size)
    ...
    def construct(
        ...
        **kwargs,
        ...
        >>> generated_ids = model.generate(pixel_values=ms.tensor(pixel_values), max_length=50)
        ...
        >>> generated_ids = model.generate(pixel_values=ms.tensor(pixel_values), input_ids=input_ids, max_length=50)
        ...
        >>> pixel_values = processor(images=list(frames), return_tensors="np").pixel_values
