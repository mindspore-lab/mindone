...
from transformers.models.squeezebert.configuration_squeezebert import SqueezeBertConfig
...
class MatMulWrapper(nn.Cell):
    """
    Wrapper for mint.matmul(). This makes flop-counting easier to implement. Note that if you directly call
    mint.matmul() in your code, the flop counter will typically ignore the flops of the matmul.
    """
    ...
    def construct(self, mat1, mat2):
        ...
        """
        return mint.matmul(mat1, mat2)


class SqueezeBertLayerNorm(mint.nn.LayerNorm):
    """
    ...
    def __init__(self, hidden_size, eps=1e-12):
        mint.nn.LayerNorm.__init__(self, normalized_shape=hidden_size, eps=eps)  # instantiates self.{weight, bias, eps}
...
class ConvDropoutLayerNorm(nn.Cell):
    ...
    def __init__(self, cin, cout, groups, dropout_prob):
        ...
        self.dropout = mint.nn.Dropout(dropout_prob)
...
SQUEEZEBERT_START_DOCSTRING = r"""
    ...
    *squeezebert/squeezebert-mnli-headless* checkpoint as a starting point.
    ...
    Data inside the encoder is in [batch, hidden_size, sequence_length] format. But, if `output_hidden_states == True`, the data from inside the encoder is
