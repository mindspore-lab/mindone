...
class Dinov2Embeddings(nn.Cell):
    ...
    def __init__(self, config: Dinov2Config) -> None:
        ...
        if config.use_mask_token:
            self.mask_token = ms.Parameter(mint.zeros((1, config.hidden_size)))
...
# Copied from transformers.models.vit.modeling_vit.eager_attention_construct
...
class Dinov2SwiGLUFFN(nn.Cell):
    def __init__(self, config) -> None:
        ...
        self.weights_in = mint.nn.Linear(in_features, 2 * hidden_features, bias=True)
        self.weights_out = mint.nn.Linear(hidden_features, out_features, bias=True)
...
class Dinov2Layer(nn.Cell):
    ...
    def __init__(self, config: Dinov2Config) -> None:
        ...
        self.drop_path = Dinov2DropPath(config.drop_path_rate) if config.drop_path_rate > 0.0 else mint.nn.Identity()
...
class Dinov2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: Union[mint.nn.Linear, mint.nn.Conv2d, mint.nn.LayerNorm]) -> None:
        ...
        if isinstance(module, (mint.nn.Linear, mint.nn.Conv2d)):
            ...
            trunc_normal_(module.position_embeddings, mean=0.0, std=self.config.initializer_range)
...
class Dinov2ForImageClassification(Dinov2PreTrainedModel):
    def __init__(self, config: Dinov2Config) -> None:
        ...
        # Classifier head
        self.classifier = (
            mint.nn.Linear(config.hidden_size * 2, config.num_labels) if config.num_labels > 0 else mint.nn.Identity()
        )
    ...
    def construct(
        ...
        linear_input = mint.cat([cls_token, patch_tokens.mean(dim=1)], dim=1)
