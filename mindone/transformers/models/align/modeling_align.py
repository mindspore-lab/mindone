...
ALIGN_START_DOCSTRING = r"""
    ...
    This model follows the Hugging Face modeling interface for MindSpore.
...
def align_loss(similarity: ms.Tensor) -> ms.Tensor:
    caption_loss = contrastive_loss(similarity)
    image_loss = contrastive_loss(mint.transpose(similarity, -1, -2))
...
class AlignVisionEmbeddings(ms.nn.Cell):
    r"""
    ...
    def __init__(self, config: AlignVisionConfig):
        ...
        self.padding = mint.nn.ZeroPad2d(padding=(0, 1, 0, 1))
        self.convolution = mint.nn.Conv2d(
            config.num_channels, self.out_dim, kernel_size=3, stride=2, padding="valid", bias=False
        )
        self.batchnorm = mint.nn.BatchNorm2d(
...
class AlignVisionDepthwiseConv2d(mint.nn.Conv2d):
    def __init__(
        self,
        in_channels,
        depth_multiplier=1,
...
class AlignVisionExpansionLayer(ms.nn.Cell):
    r"""
    ...
    def __init__(self, config: AlignVisionConfig, in_dim: int, out_dim: int, stride: int):
        super().__init__()
        self.expand_conv = mint.nn.Conv2d(
            in_channels=in_dim,
        ...
        self.expand_bn = mint.nn.BatchNorm2d(num_features=out_dim, eps=config.batch_norm_eps)
...
class AlignVisionDepthwiseLayer(ms.nn.Cell):
    r"""
    ...
    def __init__(
        ...
        self.depthwise_conv_pad = mint.nn.ZeroPad2d(padding=padding)
        self.depthwise_conv = AlignVisionDepthwiseConv2d(
            in_dim, kernel_size=kernel_size, stride=stride, padding=conv_pad, bias=False
        )
        self.depthwise_norm = mint.nn.BatchNorm2d(
...
class AlignVisionSqueezeExciteLayer(ms.nn.Cell):
    r"""
    ...
    def __init__(self, config: AlignVisionConfig, in_dim: int, expand_dim: int, expand: bool = False):
        ...
        self.squeeze = mint.nn.AdaptiveAvgPool2d(output_size=1)
        self.reduce = mint.nn.Conv2d(
            in_channels=self.dim,
            out_channels=self.dim_se,
            kernel_size=1,
            padding="same",
        )
        self.expand = mint.nn.Conv2d(
        ...
        self.act_expand = mint.nn.Sigmoid()
    ...
    def construct(self, hidden_states: ms.Tensor) -> ms.Tensor:
        ...
        hidden_states = mint.mul(inputs, hidden_states)

        return hidden_states


class AlignVisionFinalBlockLayer(ms.nn.Cell):
    r"""
    ...
    def __init__(
        ...
        self.project_conv = mint.nn.Conv2d(
            in_channels=in_dim,
            out_channels=out_dim,
            kernel_size=1,
            padding="same",
        ...
        self.project_bn = mint.nn.BatchNorm2d(
            num_features=out_dim, eps=config.batch_norm_eps, momentum=config.batch_norm_momentum
        )
        self.dropout = mint.nn.Dropout(p=drop_rate)

    def construct(self, embeddings: ms.Tensor, hidden_states: ms.Tensor) -> ms.Tensor:
...
class AlignVisionBlock(ms.nn.Cell):
    r"""
...
class AlignVisionEncoder(ms.nn.Cell):
    r"""
    ...
    def __init__(self, config: AlignVisionConfig):
        ...
        self.blocks = ms.nn.CellList(blocks)
...
class AlignTextEmbeddings(ms.nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.register_buffer("position_ids", mint.arange(config.max_position_embeddings).unsqueeze(0), persistent=False)
...
class AlignTextSelfAttention(ms.nn.Cell):
    ...
    def construct(
        ...
        if self.position_embedding_type == "relative_key" or self.position_embedding_type == "relative_key_query":
            ...
            positional_embedding = positional_embedding.astype(query_layer.dtype)  # fp16 compatibility
...
class AlignTextSelfOutput(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class AlignTextAttention(ms.nn.Cell):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
...
class AlignTextIntermediate(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class AlignTextOutput(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class AlignTextLayer(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
...
class AlignTextEncoder(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = ms.nn.CellList([AlignTextLayer(config) for _ in range(config.num_hidden_layers)])
...
class AlignTextPooler(ms.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.dense = mint.nn.Linear(config.hidden_size, config.hidden_size)
...
class AlignPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (mint.nn.Linear, mint.nn.Conv2d)):
            ...
            xavier_uniform_(module.text_projection.weight)
            constant_(module.text_projection.bias, 0.0)
...
class AlignModel(AlignPreTrainedModel):
    ...
    def __init__(self, config: AlignConfig):
        ...
        self.text_projection = mint.nn.Linear(self.text_embed_dim, self.projection_dim)
        self.temperature = ms.Parameter(ms.Tensor(self.config.temperature_init_value))
    ...
    def construct(
        ...
        text_embeds = text_embeds / mint.linalg.norm(text_embeds, ord=2, dim=-1, keepdim=True)

        # cosine similarity as logits
        logits_per_text = mint.matmul(text_embeds, mint.transpose(image_embeds, -1, -2)) / self.temperature
        logits_per_image = mint.transpose(logits_per_text, -1, -2)
