...
class VisualBertEmbeddings(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.visual_token_type_embeddings = mint.nn.Embedding(config.type_vocab_size, config.hidden_size)
        self.visual_position_embeddings = mint.nn.Embedding(config.max_position_embeddings, config.hidden_size)

        if config.special_visual_initialize:
            self.visual_token_type_embeddings.weight.set_data(
                ms.Parameter(self.token_type_embeddings.weight.data.clone(), requires_grad=True)
            )
            self.visual_position_embeddings.weight.set_data(
                ms.Parameter(self.position_embeddings.weight.data.clone(), requires_grad=True)
            )
    ...
    def construct(
        ...
        if visual_embeds is not None:
            if visual_token_type_ids is None:
                visual_token_type_ids = mint.ones(visual_embeds.shape[:-1], dtype=ms.int64)
            ...
            if image_text_alignment is not None:
                ...
                visual_position_ids = mint.zeros(visual_embeds.shape[:-1], dtype=ms.int64)
            ...
            embeddings = mint.cat((embeddings, visual_embeddings), dim=1)
...
class VisualBertPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, mint.nn.Embedding)):
...
VISUAL_BERT_START_DOCSTRING = r"""
    ...
    subclass. Use it as a regular MindSpore Cell and refer to the MindSpore documentation for all matter related to general usage
    and behavior.
...
VISUAL_BERT_INPUTS_DOCSTRING = r"""
    Args:
        ...
        visual_embeds (`ms.Tensor` of shape `(batch_size, visual_seq_length, visual_embedding_dim)`, *optional*):
            The embedded representation of the visual inputs, generally derived using using an object detector.

        visual_attention_mask (`ms.Tensor` of shape `(batch_size, visual_seq_length)`, *optional*):
            ...
            [What are attention masks?](../glossary#attention-mask)
        visual_token_type_ids (`ms.Tensor` of shape `(batch_size, visual_seq_length)`, *optional*):
        ...
        image_text_alignment (`ms.Tensor` of shape `(batch_size, visual_seq_length, alignment_number)`, *optional*):
...
class VisualBertModel(VisualBertPreTrainedModel):
    ...
    def construct(
        ...
        visual_embeds: Optional[ms.Tensor] = None,
        visual_attention_mask: Optional[ms.Tensor] = None,
        visual_token_type_ids: Optional[ms.Tensor] = None,
        image_text_alignment: Optional[ms.Tensor] = None,
        ...
        from mindone.transformers import VisualBertModel

        tokenizer = AutoTokenizer.from_pretrained("google-bert/bert-base-uncased")
        model = VisualBertModel.from_pretrained("uclanlp/visualbert-vqa-coco-pre", revision="refs/pr/1")

        inputs = tokenizer("The capital of France is Paris.", return_tensors="np")
        inputs = {k: ms.tensor(v) for k, v in inputs.items()}

        visual_embeds = get_visual_embeddings(image).unsqueeze(0)
        visual_token_type_ids = mint.ones(visual_embeds.shape[:-1], dtype=ms.int64)
        visual_attention_mask = mint.ones(visual_embeds.shape[:-1], dtype=ms.int64)
        ...
        if visual_embeds is not None and visual_attention_mask is None:
            visual_attention_mask = mint.ones(visual_input_shape)

        # We can provide a self-attention mask of dimensions [batch_size, from_seq_length, to_seq_length]
        # ourselves in which case we just need to make it broadcastable to all heads.
        if visual_embeds is not None:
            combined_attention_mask = mint.cat((attention_mask, visual_attention_mask), dim=-1)
        ...
        if self.bypass_transformer and visual_embeds is not None:
            ...
            concatenated_input = mint.cat((sequence_output, visual_embedding_output), dim=1)
...
class VisualBertForPreTraining(VisualBertPreTrainedModel):
    ...
    def construct(
        ...
        sentence_image_labels: Optional[ms.Tensor] = None,
    ) -> Union[tuple[ms.Tensor], VisualBertForPreTrainingOutput]:
        r"""
        labels (`ms.Tensor` of shape `(batch_size, total_sequence_length)`, *optional*):
            Labels for computing the masked language modeling loss. Indices should be in `[-100, 0, ...,
            config.vocab_size]` (see `input_ids` docstring) Tokens with indices set to `-100` are ignored (masked), the
            loss is only computed for the tokens with labels in `[0, ..., config.vocab_size]`
        sentence_image_labels (`ms.Tensor` of shape `(batch_size,)`, *optional*):
        ...
        from mindone.transformers import VisualBertForPreTraining
        ...
        labels = ms.tensor(
            tokenizer("The capital of France is Paris.", return_tensors="np", padding="max_length", max_length=max_length)["input_ids"]
        )
        sentence_image_labels = ms.tensor(1).unsqueeze(0)  # Batch_size
...
class VisualBertForMultipleChoice(VisualBertPreTrainedModel):
    ...
    def construct(
        ...
        from mindone.transformers import VisualBertForMultipleChoice
        ...
        visual_embeds = mint.broadcast_to(visual_embeds, (1, 2, *visual_embeds.shape))
        visual_token_type_ids = mint.ones(visual_embeds.shape[:-1], dtype=ms.int64)
        visual_attention_mask = mint.ones(visual_embeds.shape[:-1], dtype=ms.int64)
        ...
        # batch size is 1
        inputs_dict = {k: ms.tensor(v).unsqueeze(0) for k, v in encoding.items()}
        inputs_dict.update(
            {
                "visual_embeds": visual_embeds,
                "visual_attention_mask": visual_attention_mask,
...
class VisualBertForQuestionAnswering(VisualBertPreTrainedModel):
    ...
    def construct(
        ...
        from mindone.transformers import VisualBertForQuestionAnswering
        ...
        labels = ms.Tensor([[0.0, 1.0]]).unsqueeze(0)  # Batch size 1, Num labels 2
        ...
        index_to_gather = mint.broadcast_to(
            index_to_gather.unsqueeze(-1).unsqueeze(-1), (index_to_gather.shape[0], 1, sequence_output.shape[-1])
        )
        pooled_output = mint.gather(sequence_output, 1, index_to_gather)
        ...
        loss = None
        if labels is not None:
            loss_fct = mint.nn.KLDivLoss(reduction="batchmean")
...
class VisualBertForVisualReasoning(VisualBertPreTrainedModel):
    ...
    def construct(
        ...
        from mindone.transformers import VisualBertForVisualReasoning
        ...
        labels = ms.tensor(1).unsqueeze(0)  # Batch size 1, Num choices 2
...
class VisualBertRegionToPhraseAttention(nn.Cell):
    ...
    def construct(self, query, key, attention_mask):
        ...
        attention_mask = (1.0 - attention_mask) * _DTYPE_2_MIN[query.dtype]
...
class VisualBertForRegionToPhraseAlignment(VisualBertPreTrainedModel):
    ...
    def construct(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        region_to_phrase_position: Optional[ms.Tensor] = None,
        labels: Optional[ms.Tensor] = None,
    ) -> Union[tuple[ms.Tensor], SequenceClassifierOutput]:
        r"""
        region_to_phrase_position (`ms.Tensor` of shape `(batch_size, total_sequence_length)`, *optional*):
            The positions depicting the position of the image embedding corresponding to the textual tokens.

        labels (`ms.Tensor` of shape `(batch_size, total_sequence_length, visual_sequence_length)`, *optional*):
        ...
        from mindone.transformers import VisualBertForRegionToPhraseAlignment

        tokenizer = AutoTokenizer.from_pretrained("google-bert/bert-base-uncased")
        model = VisualBertForRegionToPhraseAlignment.from_pretrained("uclanlp/visualbert-vqa-coco-pre", revision="refs/pr/1")
        ...
        visual_token_type_ids = mint.ones(visual_embeds.shape[:-1], dtype=ms.int64)
        visual_attention_mask = mint.ones(visual_embeds.shape[:-1], dtype=ms.int64)
        region_to_phrase_position = mint.ones((1, inputs["input_ids"].shape[-1] + visual_embeds.shape[-2]), dtype=ms.int64)
        ...
        expanded_region_to_phrase_positions = mint.broadcast_to(
            region_to_phrase_position.unsqueeze(2),
