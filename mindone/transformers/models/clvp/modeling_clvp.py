...
def clvp_loss(similarity: ms.Tensor) -> ms.Tensor:
...
def _pad_extra_bos_eos_tokens(
    ...
    if add_bos_token:
        input_ids = mint.nn.functional.pad(input_ids, (1, 0), value=bos_token_id)
        attention_mask = (
...
class ClvpSelfAttention(nn.Cell):
    ...
    def __init__(self, config, layer_idx=None):
        ...
        self.k_proj = mint.nn.Linear(self.embed_dim, self.embed_dim, bias=config.use_attention_bias)
        self.v_proj = mint.nn.Linear(self.embed_dim, self.embed_dim, bias=config.use_attention_bias)
    ...
    def construct(
        ...
        if rotary_pos_emb is not None:
            ...
            value_states = mint.cat((value_rot, value_pass), dim=-1)
...
class ClvpGatedLinearUnit(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.proj = mint.nn.Linear(config.hidden_size, config.intermediate_size * 2)
...
class ClvpEncoderLayer(nn.Cell):
    ...
    def construct(
        ...
        Args:
            ...
            attention_mask (`ms.Tensor` of shape `(batch, 1, tgt_len, src_len)`):
...
class ClvpConditioningEncoder(nn.Cell):
    ...
    def __init__(self, config: ClvpConfig):
        ...
        self.text_token_embedding = mint.nn.Embedding(self.text_config.vocab_size, self.decoder_config.hidden_size)
        self.text_position_embedding = mint.nn.Embedding(
        ...
        self.group_norms = nn.CellList(
            [
                mint.nn.GroupNorm(num_groups, self.decoder_config.hidden_size, eps=1e-5, affine=True)
    ...
    def compute_groupnorm_groups(self, channels: int, groups: int = 32):
        """
        Calculates the value of `num_groups` for mint.nn.GroupNorm. This logic is taken from the official tortoise
        repository. link :
...
class ClvpPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: nn.Cell):
        ...
        if isinstance(module, mint.nn.Embedding):
        ...
        elif isinstance(module, (mint.nn.Linear, Conv1D, nn.Conv1d)):
            normal_(module.weight, mean=0.0, std=factor * 0.02)
            ...
            normal_(module.fc1.proj.weight if getattr(module.fc1, "proj") else module.fc1.weight, std=fc_std)
            normal_(module.fc2.weight, std=in_proj_std)
        elif isinstance(module, ClvpEncoder):
            config = self.config.get_text_config()
            factor = config.initializer_factor
            normal_(module.projection.weight, mean=0.0, std=factor * (config.hidden_size**-0.5))
        elif isinstance(module, ClvpConditioningEncoder):
            normal_(module.mel_conv.weight, mean=0.0, std=factor)
            zeros_(module.mel_conv.bias)
...
class ClvpForCausalLM(ClvpPreTrainedModel, GenerationMixin):
    def __init__(self, config):
        ...
        self.final_norm = mint.nn.LayerNorm(self.config.hidden_size)
        self.lm_head = mint.nn.Linear(self.config.hidden_size, self.config.vocab_size, bias=True)
...
class ClvpModelForConditionalGeneration(ClvpPreTrainedModel, GenerationMixin):
    ...
    def fix_speech_decoder_output(self, speech_ids: ms.Tensor) -> ms.Tensor:
        """
        ...
        stop_token_indices = mint.where(speech_ids == self.speech_decoder_model.config.eos_token_id, 1, 0)
    ...
    def get_text_features(
        ...
        Returns:
            `ms.Tensor` of shape `(batch_size, output_dim)`:
    ...
    def get_speech_features(
        ...
        **kwargs,
        ...
        Args:
            speech_ids (`ms.Tensor` of shape `(batch_size, num_speech_ids)`, *optional*):
    ...
    def construct(
        ...
        logits_per_text = mint.matmul(text_embeds, speech_embeds.t()) * logit_scale
    ...
    def generate(
        ...
        **kwargs,
        ...
        if pad_to_max_mel_tokens is not None:
            ...
            speech_ids = mint.nn.functional.pad(
