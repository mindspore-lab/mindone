...
from transformers.models.pop2piano.configuration_pop2piano import Pop2PianoConfig
...
POP2PIANO_INPUTS_DOCSTRING = r"""
    Args:
        ...
        input_features (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
...
class Pop2PianoBlock(nn.Cell):
    def __init__(self, config, has_relative_attention_bias=False, layer_idx: Optional[int] = None):
        ...
        if self.is_decoder:
            layer.append(Pop2PianoLayerCrossAttention(config, layer_idx=layer_idx))

        layer.append(Pop2PianoLayerFF(config))
    ...
    def construct(
        ...
        if hidden_states.dtype == ms.float16:
            clamp_value = mint.where(
                ...
                dtype_to_max(hidden_states.dtype) - 1000,
                dtype_to_max(hidden_states.dtype),
            )
        ...
        # hidden-states, past_key_value, (self-attention position bias),
...
class Pop2PianoStack(Pop2PianoPreTrainedModel):
    ...
    def construct(
        ...
        if self.config.is_decoder:
            ...
            causal_mask = (1.0 - causal_mask) * dtype_to_min(inputs_embeds.dtype)
    ...
    def _update_causal_mask(
        ...
        if self.config._attn_implementation == "flex_attention":
            raise NotImplementedError("FlexAttention is not supported.")
...
class Pop2PianoConcatEmbeddingToMel(nn.Cell):
    ...
    def __init__(self, config):
        super().__init__()
        self.embedding = mint.nn.Embedding(num_embeddings=config.composer_vocab_size, embedding_dim=config.d_model)
    ...
    def construct(self, feature, index_value, embedding_offset):
        ...
        inputs_embeds = mint.cat([composer_embedding, feature], dim=1)
...
class Pop2PianoForConditionalGeneration(Pop2PianoPreTrainedModel, GenerationMixin):
    ...
    def get_mel_conditioner_outputs(
        ...
        Args:
            input_features (`ms.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
        ...
        composer_value = ms.tensor(composer_value)
        ...
        if attention_mask is not None:
            ...
            attention_mask = mint.cat([attention_mask[:, 0].view(-1, 1), attention_mask], axis=1)
    ...
    def _reorder_cache(self, past_key_values, beam_idx):
        ...
        for layer_past_states in past_key_values:
            ...
            if reordered_layer_past_states[0].shape != layer_past_states[0].shape:
                raise ValueError(
                    f"reordered_layer_past_states[0] shape {reordered_layer_past_states[0].shape} and layer_past_states[0] shape {layer_past_states[0].shape} mismatched"  # noqa
                )
            if len(reordered_layer_past_states) != len(layer_past_states):
                raise ValueError(
                    f"length of reordered_layer_past_states {len(reordered_layer_past_states)} and length of layer_past_states {len(layer_past_states)} mismatched"  # noqa
                )
