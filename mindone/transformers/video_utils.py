...
def make_batched_videos(videos) -> list[Union["np.ndarray", "ms.Tensor", "URL", "Path"]]:
    """
...
def read_video_decord(
    ...
    **kwargs,
    ...
    video = sample_indices_fn(metadata=metadata, **kwargs)
...
def read_video_mindspore(
    ...
    **kwargs,
    ...
    Returns:
        tuple[`ms.Tensor`, `VideoMetadata`]: A tuple containing:
            -  MindSpore Tensor of frames in RGB (shape: [num_frames, height, width, 3]).
            - `VideoMetadata` object.
    """
...
# There is no substitution for "torchdec" in mindspore, so we abandon this method
VIDEO_DECODERS = {
    ...
    **kwargs,
    ...
    Args:
        ...
        backend (`str`, *optional*, defaults to `"pyav"`):
            The backend to use when loading the video. Can be any of ["decord", "pyav", "opencv",]. Defaults to "pyav".
        sample_indices_fn (`Callable`, *optional*):
            A callable function that will return indices at which the video should be sampled. If the video has to be loaded using
            by a different sampling technique than provided by `num_frames` or `fps` arguments, one should provide their own `sample_indices_fn`.
    ...
    # can also load with decord, but not cv2
    # both will fail in case of url links
    video_is_url = video.startswith("http://") or video.startswith("https://")
    if video_is_url and backend == "opencv":
        raise ValueError(
            "If you are trying to load a video from URL, you can decode the video only with `pyav`, `decord` as backend"
        )
...
) -> tuple[dict[tuple[int, int], "ms.Tensor"], dict[int, tuple[tuple[int, int], int]]]:
    """
...
def reorder_videos(
    processed_videos: dict[tuple[int, int], "ms.Tensor"], grouped_videos_index: dict[int, tuple[tuple[int, int], int]]
) -> list["ms.Tensor"]:
    """
