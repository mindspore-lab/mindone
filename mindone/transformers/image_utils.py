...
def is_valid_image(img):
    return is_pil_image(img) or is_numpy_array(img) or is_mindspore_tensor(img)
    ...
    if image.shape[first_dim] in num_channels and image.shape[last_dim] in num_channels:
        logger.warning(
            f"The channel dimension is ambiguous. Got image shape {image.shape}. Assuming channels are the first dimension. Use the\
                  [input_data_format](https://huggingface.co/docs/transformers/main/internal/image_processing_utils#transformers.image_transforms.rescale.input_data_format) \
...
class ImageFeatureExtractionMixin:
    ...
    def to_pil_image(self, image, rescale=None):
        ...
        """
        # check image if there is any unsupported image format
        self._ensure_format_supported(image)

        # change to mindspore_tensor
        if is_mindspore_tensor(image):
            image = image.numpy()
    ...
    def center_crop(self, image, size):
        ...
        # PIL Image.size is (width, height) but NumPy array and mindspore Tensors have (height, width)
