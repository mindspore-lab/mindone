...
def _encode_image(preprocessed_image, clip_model):
    image_features = clip_model.get_image_features(preprocessed_image)
...
def _clip_directional_similarity_update(
    ...
    if not input_valid:
        raise ValueError(
            f"Expected the number of images and text examples to be the same"
        )

    processed_input1 = processor(image1, text1)
    processed_input2 = processor(image2, text2)
    image1_feature = _encode_image(processed_input1['image'], model)
    text1_feature = _encode_text(processed_input1['text'], model)
    image2_feature = _encode_image(processed_input2['image'], model)
    text2_feature = _encode_text(processed_input2['text'], model)
