...
__all__ = ["get_1d_sincos_pos_embed", "get_2d_sincos_pos_embed", "precompute_freqs_cis_1d", "precompute_freqs_cis_2d"]
...
def precompute_freqs_cis_1d(
    dim: int, length: int, theta: float = 10000.0, max_length: Optional[int] = None
) -> np.ndarray:
    """Precompute the frequency tensor for complex exponentials (cis) with given dimensions.

    If max_length is not None, then a length extrapolation algo. `NTK` will be used for tensor calculation.
    ...
    freqs_cis = _precompute_freqs_cis_1d_from_grid(dim, grid, theta=theta, max_length=max_length)
    freqs_cis = np.reshape(freqs_cis, (freqs_cis.shape[0], -1))
    return freqs_cis
