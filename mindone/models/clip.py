...
from mindone.transformers.models.clip import CLIPTextModel, CLIPModel
...
class CLIPEmbedder:
    ...
    def __init__(
        ...
        self.model = CLIPModel.from_pretrained(self.cache_dir)
        self.text_model = CLIPTextModel.from_pretrained(self.cache_dir)
        self.tokenizer = CLIPTokenizer.from_pretrained(self.cache_dir)
        image_processor = CLIPImageProcessor.from_pretrained(self.cache_dir)
        self.processor = CLIPProcessor(image_processor, self.tokenizer)
    ...
    def get_text_embeddings(self, texts):
        ...
        inputs = {k: ms.Tensor(v, ms.int32) for k, v in inputs.items()}
    ...
    def encode_text(self, texts):
        ...
        batch_encoding = {k: ms.Tensor(v, ms.int32) for k, v in batch_encoding.items()}

        outputs = self.text_model(**batch_encoding) # (last_hidden_state, pooled_output) + encoder_outputs[1:]
    ...
    def _validate_input_list(self, input_list, expected_type):
        ...
        if not isinstance(input_list, list) or not all(
            isinstance(item, expected_type) for item in input_list
        ):
